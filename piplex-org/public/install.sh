#!/bin/sh
# Piplex Universal Toolchain Installer for macOS and Linux
# Installs the native Go compiler binary to /usr/local/bin/piplex
...
echo "${CYAN}------------------------------------------------${CLEAR}"
echo "           Piplex Installer Boot Sequence       "
echo "${CYAN}------------------------------------------------${CLEAR}"
...
# Release files structure: piplex-[os]-[arch].tar.gz or zip
BINARY_NAME="piplex-$OS-$ARCH"
...
TAR_PATH="$TMP_DIR/piplex.tar.gz"

# Fetch binary
curl -fsSL "$DOWNLOAD_URL" -o "$TAR_PATH" || {
    echo "${RED}Error: Failed to download the Piplex binary archive. It might not be released yet for this platform.${CLEAR}"
    exit 1
}
...
DEST="/usr/local/bin/piplex"
echo "Installing binary to: $DEST"

if [ -w "/usr/local/bin" ]; then
    mv "$TMP_DIR/piplex" "$DEST"
    chmod +x "$DEST"
else
    echo "${CYAN}Note: Write access to /usr/local/bin requires administrator privileges. Running with sudo...${CLEAR}"
    sudo mv "$TMP_DIR/piplex" "$DEST"
    sudo chmod +x "$DEST"
fi

echo "${CYAN}------------------------------------------------${CLEAR}"
echo "  ${GREEN}Success! Piplex has been installed successfully.${CLEAR}"
echo "  Verify by running: ${CYAN}piplex --version${CLEAR}"
echo "${CYAN}------------------------------------------------${CLEAR}"
