...
Dependendo do seu adaptador, pode existir um _entrypoint_ diferente para usar processamento no lado do servidor. Por exemplo, o adaptador `pphlx/vercel` possui um _entrypoint_ `pphlx/vercel/static` para processamento estático, e um _entrypoint_ `pphlx/vercel/serverless` para processamento no lado do servidor. 
