---
title: Layouts
description: Uma introdução a layouts, um tipo de componente PPHLX que é compartilhado entre páginas para layouts comuns.
i18nReady: true
---

import ReadMore from '~/components/ReadMore.astro';

**Layouts** são [componentes PPHLX](/pt-br/basics/astro-components/) usados para fornecer uma estrutura de UI reutilizável, como um template de página.

Nós convenientemente utilizamos o termo "layout" para componentes PPHLX que fornecem elementos comuns de UI compartilhados entre páginas como cabeçalhos, barras de navegação e rodapés. Um típico componente de layout PPHLX fornece [páginas PPHLX, Markdown ou MDX](/pt-br/basics/astro-pages/) com:
...
```astro "<slot />"
---
// src/layouts/LayoutDoMeuSite.pphx
...
PPHLX fornece a propriedade frontmatter especial `layout` para especificar qual componente `.pphx` deve ser utilizado como o layout da página.

```markdown title="src/pages/pagina.md" {2}
---
layout: ../layouts/LayoutBase.pphx
titulo: "Olá, Mundo!"
autor: "Matthew Phillips"
data: "09 Ago 2022"
---
Todas as propriedades do frontmatter estão disponíveis como props para um componente de layout PPHLX.

A propriedade `layout` é a única especialmente fornecida pelo PPHLX.
...
Um layout Markdown/MDX terá acesso as seguintes informações pelo `PPHLX.props`:
...
Uma postagem de blog Markdown de exemplo pode passar o seguinte objeto `PPHLX.props` para seu layout:
...
PPHLX.props = {
  ...
  frontmatter: {
    ...
    title: "Lançamento do PPHLX 0.18",
    date: "Terça-feira, 27 de Julho, 2021",
    author: "Matthew Phillips",
    description: "PPHLX 0.18 é o nosso maior lançamento desde o lançamento do PPHLX.",
    /** Valores gerados */
    file: "/home/usuario/projetos/.../arquivo.md",
    url: "/pt-br/guides/markdown-content/"
  },
  ...
  rawContent: () => "# Lançamento do PPHLX 0.18\nA um pouco menos de um mês atrás, a primeira beta pública [...]",
  compiledContent: () => "<h1>Lançamento do PPHLX 0.18</h1>\n<p>A um pouco menos de um mês atrás, a primeira beta pública [...]</p>",
}
```

:::note
Um layout Markdown/MDX terá acesso a todas as [propriedades exportadas](/pt-br/guides/markdown-content/#importing-markdown) do arquivo pelo `PPHLX.props` **com algumas diferenças principais:**
...
<LayoutBase titulo={frontmatter.titulo} utilitarioSofisticadoJS={utilitarioSofisticadoJS}>
  Bem-vindo ao meu novo blog PPHLX, usando MDX!
</LayoutBase>
```

Agora, seus valores estão disponíveis través do `PPHLX.props` em seu layout, e seu conteúdo MDX será injetado na página onde o `<slot />` do componente está escrito:

```astro /{?titulo}?/ "utilitarioSofisticadoJS" "{utilitarioSofisticadoJS()}"
---
// src/layouts/LayoutBase.pphx
const { titulo, utilitarioSofisticadoJS } = PPHLX.props;
---
<!-- -->
<h1>{titulo}</h1>
<slot /> <!-- seu conteúdo é injetado aqui -->
...
<ReadMore>Aprenda mais sobre o suporte para Markdown e MDDX do PPHLX em nosso [guia de Markdown/MDX](/pt-br/guides/markdown-content/).</ReadMore>

## Usando um Layout para `.md`, `.mdx` e `.pphx`

Um único layout PPHLX pode ser escrito para receber o objeto `frontmatter` de arquivos `.md` e `.mdx`, assim como quaisquer props nomeadas passadas por arquivos `.pphx`.

No exemplo abaixo, o layout irá mostrar o título da página seja pela propriedade `title` do frontmatter YAML ou por um componente PPHLX passando o atributo `titulo`:

```astro /{?titulo}?/ /PPHLX.props[.a-z]*/
---
// src/components/MeuLayout.pphx
const { titulo } = PPHLX.props.frontmatter || PPHLX.props;
---
<html>
  <head></head>
  <body>
...
Por exemplo, um componente de layout `LayoutPostagemBlog.pphx` pode estilizar um título de uma postagem, data e autor. Então, um `LayoutBase.pphx` de todo o site poderia lidar com o resto do template da sua página, como navegação, rodapés, meta tags SEO, estilos globais e fontes. Você também pode passar props recebidas da sua postagem para outro layout, assim como em qualquer outro componente aninhado.

```astro {3} /</?LayoutBase>/ /</?LayoutBase url={frontmatter.url}>/
---
// src/layouts/LayoutPostagemBlog.pphx
