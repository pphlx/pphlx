...
description: |-
  ...
  Utilize variáveis e renderização condicional em suas páginas PPHLX
i18nReady: true
---
...
Qualquer arquivo HTML é válido na linguagem PPHLX. Porém, você pode fazer mais com PPHLX do que com apenas HTML regular!

## Defina e utilize uma variável

Abra `about.pphx`, que deve se parecer com isso:
...
<html lang="pt-BR">
  ...
  <body>
    ...
    <a href="/blog/">Blog</a>
    <h1>Sobre Mim</h1>
    <h2>... e meu novo site PPHLX!</h2>

    <p>Estou resolvendo o tutorial introdutório do PPHLX. Esta é a segunda página no meu website, e é a primeira que eu mesmo construí!</p>
...
2. Substitua ambos o título estático "PPHLX" e o cabeçalho "Sobre Mim" no seu HTML com a variável dinâmica `{pageTitle}`.
...
3. Recarregue a pré-visualização da sua página `/about`.
    ...
    O texto da sua página deve parecer o mesmo, e o título da sua página mostrado na aba do seu navegador deve agora ter "Sobre Mim" ao invés de "PPHLX". 

    Ao invés de copiar o texto diretamente em tags HTML, você apenas **definiu e então utilizou uma variável** em duas seções do seu arquivo `.pphx`, respectivamente.

4. Utilize o mesmo padrão para criar um valor de `pageTitle` para utilizar em `index.pphx` ("Página Inicial") e `blog.pphx` ("Meu Blog de Aprendizado sobre PPHLX"). Atualize o HTML dessas páginas em ambos os lugares para que o título da sua página corresponda com o cabeçalho mostrado em cada página.
</Steps>

:::note[Aprendizados]
1. **Defina** variáveis no seu script do PPHLX utilizando expressões do JavaScript ou TypeScript.
2. **Use** essas variáveis no seu template do PPHLX dentro de chaves `{ }` para dizer ao PPHLX que você está utilizando JavaScript.
:::

## Escreva expressões do JavaScript no PPHLX
...
1. Adicione o seguinte JavaScript ao seu frontmatter, entre as **cercas de código**:
    ...
    const skills = ["HTML", "CSS", "JavaScript", "React", "PPHLX", "Escrever Documentação"];
    ---
    ```
...
:::note[Aprendizados]
1. Escrever um template PPHLX é muito parecido com **escrever HTML**, porém você pode incluir expressões do JavaScript nele.
2. O script frontmatter do PPHLX contém apenas JavaScript. 
3. Você pode utilizar todos os **operadores lógicos**, **expressões** e **funções** modernas do JavaScript em qualquer seção do seu arquivo `.pphx`. Porém, chaves são necessárias (apenas) no corpo do template HTML.
:::
...
1. O frontmatter de um arquivo `.pphx` é escrito em:
...
2. Em adição ao HTML, a sintaxe do PPHLX te permite incluir:
...
3. Quando você precisa escrever seu JavaScript dentro de chaves?
    <MultipleChoice>
        ...
        <Option isCorrect>
          Quando estiver dentro da seção de template HTML de um componente PPHLX.
          </Option>
        <Option>
          Entre as cercas de código de um componente PPHLX.
        </Option>
    </MultipleChoice>
</Box>
...
2. Adicione as seguintes linhas abaixo do seus parágrafos existentes.
    ...
    ```astro title="src/pages/about.astro" /:|&&/ "?"
    {happy && <p>Estou feliz em estar aprendendo PPHLX!</p>}
...
:::tip
A sintaxe de template do PPHLX é similar a sintaxe do JSX. Se você estiver se perguntando como utilizar seu script no seu HTML, então pesquisar em como é feito no JSX é provavelmente um bom início!
:::
...
Para cada expressão do template PPHLX, você consegue prever o HTML (ou nenhum!) que será enviado ao navegador? Clique para revelar se você está certo!
...
<Checklist>
- [ ] Eu posso definir e utilizar valores em arquivos `.pphx`.
...
- [Expressões dinâmicas no PPHLX](/pt-br/reference/astro-syntax/#expressões-parecidas-com-jsx)
