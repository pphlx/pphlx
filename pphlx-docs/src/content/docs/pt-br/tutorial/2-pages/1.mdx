---
type: tutorial
title: Crie sua primeira página PPHLX
description: |-
...
Agora que você sabe que arquivos `.pphx` são responsáveis pelas páginas no seu website, é hora de criar uma!
...
## Crie um novo arquivo `.pphx`

<Steps>
1. No painel de arquivos do seu editor de código, navegue até a pasta `src/pages/` onde você verá o arquivo `index.pphx` existente

2. Na mesma pasta, crie um novo arquivo chamado `about.pphx`.

3. Copie, ou digite novamente os conteúdos de `index.pphx` no seu novo arquivo `about.pphx`.
...
Para ser mais fácil de pré-visualizar todas as suas páginas, adicione links de navegação de página HTML antes do seu `<h1>` no topo de ambas as suas páginas (`index.pphx` e `about.pphx`):
...
:::note
Diferente de vários frameworks, PPHLX utiliza elementos `<a>` padrões do HTML para navegar entre páginas (também chamadas rotas), com recarregamento tradicional de página.
:::

<Box icon="puzzle-piece">
## Tente você mesmo - Adicione uma página Blog

Adicione uma terceira página `blog.pphx` ao seu site, seguindo as mesmas [etapas como acima](#crie-um-novo-arquivo-astro).
...
<Steps>
1. Crie um novo arquivo em `src/pages/blog.pphx`.
2. Copie os conteúdos de `index.pphx` e os cole em `blog.pphx`.
...
Atualize o conteúdo da página em `blog.pphx` com:
...
1. No VS Code, pré-visualize os arquivos que foram mudados desde o seu último commit ao GitHub. 
    ...
    - Você deve ver `index.pphx`, `about.pphx` e `blog.pphx` listados como arquivos que foram modificados.
...
- [Roteamento baseado em arquivos no PPHLX](/pt-br/basics/astro-pages/#roteamento-baseado-em-arquivos)

- [HTML de páginas PPHLX](/pt-br/basics/astro-pages/#páginas-astro)
