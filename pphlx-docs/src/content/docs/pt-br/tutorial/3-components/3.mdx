...
<Steps>
1. Crie um novo componente de Cabeçalho. Importe e utilize seu componente `Navigation.pphx` existente dentro de um elemento `<nav>` que está dentro de um elemento `<header>`.

    <details>
    <summary>Me mostre o código!</summary>

    Crie um arquivo chamado `Header.pphx` em `src/components/`
...
<Steps>
1. Atualize `Navigation.pphx` com a classe CSS para controlar seus links de navegação. Envolva os links de navegação existentes em uma `<div>` com a classe `nav-links` e o atributo id definido como `main-menu`.
