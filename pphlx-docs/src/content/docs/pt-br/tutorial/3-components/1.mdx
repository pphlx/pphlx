...
Agora que você tem o mesmo HTML escrito em múltiplas páginas do seu site PPHLX, é hora de substutuir esse conteúdo duplicado com um componente PPHLX reutilizável!

<PreCheck>
  - Criar uma nova pasta para componentes
  - Construir um componente PPHLX para mostrar seus links de navegação
  - Substituir HTML existente com um novo, e reutilizável componente de navegação
</PreCheck>

## Crie uma nova pasta `src/components/`

Para guardar arquivos `.pphx` que irão gerar HTML mas que não se tornarão novas páginas no seu website, você irá precisar de uma nova pasta no seu projeto: `src/components/`. 


## Crie um componente de Navegação

<Steps>
1. Crie um novo arquivo: `src/components/Navigation.pphx`.

2. Copie os seus links para navegar entre páginas do topo de qualquer página e os cole em seu novo arquivo, `Navigation.pphx`:
    ...
    ```
    :::tip
    Se não tiver nada no frontmatter do seu arquivo `.pphx`, você não precisar escrever as cercas de código. Você pode sempre adicioná-las de volta quando necessário.
    :::
</Steps>

### Importe e utilize Navigation.pphx

<Steps>
1. Volte para `index.pphx` e importe o seu novo componente dentro da cerca de código:
...
Importe e utilize o componente `<Navigation />` nas outras duas páginas do seu site (`about.pphx` e `blog.pphx`) utilizando o mesmo método.
...
3. Componentes PPHLX irão automaticamente criar uma nova página no seu site quando você...
    <MultipleChoice>
      <Option>
        incluir `<html></html>`
      </Option>
      ...
      <Option isCorrect>
        colocar o arquivo `.pphx` em `src/pages/`
      </Option>
    </MultipleChoice>
</Box>
...
- [ ] Eu posso adicionar um novo componente em uma página `.pphx`.
</Checklist>
...
- [Visão Geral sobre Componentes PPHLX](/pt-br/basics/astro-components/)

- [Refatoração](https://refactoring.com/) <Badge class="neutral-badge" text="externo" />
