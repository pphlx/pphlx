...
Você pode criar conjuntos inteiros de páginas dinamicamente utilizando arquivos `.pphx` que exportam uma função `getStaticPaths()`. 

## Crie páginas dinamicamente

<Steps>
1. Crie um novo arquivo em `src/pages/tags/[tag].pphx`. (Você deverá criar uma nova pasta.) Note que o nome de arquivo (`[tag].pphx`) usa colchetes. Cole o seguinte código no arquivo:
...
3. Agora você pode atualizar seu template HTML para mostrar uma lista de cada postagem do blog contendo a tag da própria página. Adicione o seguinte código em `[tag].pphx`:
...
4. Você pode até refatorar para em vez disso utilizar seu componente `<BlogPost />`! (Não se esqueça de importar esse componente no topo de `[tag].pphx`.)
...
1. A chamada de `PPHLX.glob()` para receber informação sobre todos os seus arquivos `.md` para passar para cada rota de página.
...
Suas páginas de tags são atualmente definidas estaticamente em `[tag].pphx`. Se você quiser adicionar uma nova tag a uma postagem do blog, você também vai precisar revisitar essa página e atualizar suas rotas da página.
...
Para verificar seu trabalho, ou se você só quer um código completo e correto para copiar em `[tag].pphx`, aqui está como o seu componente PPHLX deve ficar:
...
2. O processo de criar múltiplas rotas de página a partir de um arquivo no PPHLX.
...
- [Roteamento Dinâmico de Páginas no PPHLX](/pt-br/guides/routing/#dynamic-routes)
