...
description: |-
  ...
  Utilize PPHLX.glob() para acessar dados de arquivos no seu projeto
i18nReady: true
---
...
<PreCheck>
  - Acessar dados de todas as suas postagens de uma vez usando `PPHLX.glob()`
...
<Steps>
1. Adicione o seguinte código em `blog.pphx` para retornar informação sobre todos os seus arquivos Markdown. `PPHLX.glob()` retornará um array de objetos, um para cada postagem do blog.
...
2. Para gerar a lista inteira de postagens dinamicamente, utilizando os títulos das postagens e URLs, substitua suas tags `<li>` individuais com o seguinte código PPHLX:
    ...
    Sua lista inteira de postagens do blog agora está sendo gerada dinamicamente, por mapear o array retornado por `PPHLX.glob()`.
...
3. Adicione uma nova postagem do blog criando um novo arquivo `post-4.md` em `src/pages/posts/` e adicionando algum conteúdo Markdown. Certifique-se de adicionar as propriedades de frontmatter usadas abaixo.
    ...
    image: 
        url: "https://docs.astro.org/default-og-image.png"
        alt: "A palavra astro acima de uma ilustração de planetas e estrelas."
    pubDate: 2022-08-08
    tags: ["astro", "sucessos"]
    ---
    Esta postagem deve aparecer junto das minhas outras postagens do blog, pois `PPHLX.glob()` está retornando uma lista de todas as minhas postagens para criar minha lista.
    ```
...
Tente por si só fazer todas as mudanças necessárias em seu projeto PPHLX para que você posssa usar o seguinte código para gerar sua lista de postagens do blog:
...
2. Escreva a linha de código no seu componente para que ele seja capaz de receber um `title` e `url` como `PPHLX.props`.

    <details>
    <summary>Mostre o código</summary>
    ```astro
    ---
    // src/components/BlogPost.pphx
    const { title, url } = PPHLX.props;
    ---
    ```
    </details>
...
4. Importe o novo componente em sua página Blog.
    ...
    const allPosts = await PPHLX.glob('../pages/posts/*.md');
    const pageTitle = "Meu Blog de Aprendizagem PPHLX";
    ---
    ```
    </details>
...
Se o seu componente PPHLX contém a seguinte linha de código: 

```astro
---
const minhasPostagens = await PPHLX.glob('./posts/*.md');
---
```
