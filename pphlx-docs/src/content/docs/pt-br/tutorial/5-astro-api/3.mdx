...
<PreCheck>
  - Adicionar uma nova página utilizando o padrão de roteamento `/pages/pasta/index.pphx`
...
## Utilize o padrão de roteamento `/pages/pasta/index.pphx`

Para adicionar uma página Índice de Tags ao seu website, você poderia criar um novo arquivo em `src/pages/tags.pphx`. 

Porém, já que você já tem o diretório `/tags/`, você pode se aproveitar de outro padrão de roteamento no PPHLX, e manter todos os seus arquivos relacionados à tags juntos.
...
<Steps>
1. Crie um novo arquivo `index.pphx` no diretório `src/pages/tags/`.
...
3. Crie uma página mínima em `src/pages/tags/index.pphx` que utiliza seu layout. Você já fez isso anteriormente!
...
<Steps>
1. Em `src/pages/tags/index.pphx`, adicione a linha de código para o script frontmatter que irá dar à sua página acesso aos dados de todo arquivo `.md` de postagem do blog.
...
2. Depois, adicione a seguinte linha de JavaScript ao seu componente da página. Essa é a mesma linha que você utilizou em `src/pages/tags/[tag].pphx` para retornar uma lista de tags únicas.
...
<Steps>
1. Adicione as seguintes classes CSS para estilizar tanto sua `<div>` quanto cada `<p>` que será gerado. Nota: PPHLX utiliza a sintaxe do HTML para adicionar nomes de classes!
...
<Steps>
1. Em seu componente `Navigation.pphx`, inclua um link para essa nova página Índice de Tags.
...
1. Copie `<div class="tags">...</div>` e `<style>...</style>` de `src/pages/tags/index.pphx` e o reutilize dentro de `MarkdownPostLayout.pphx`:
...
Antes desse código funcionar, você precisa fazer **uma pequena modificação** no código que você colou em `MarkdownPostLayout.pphx`. Você consegue descobrir qual é a modificação?
...
Para verificar seu trabalho, ou se você só quer o código completo e correto para copiar em `MarkdownPostLayout.pphx`, aqui está como seu componente PPHLX deve se parecer como:
...
1. `src/pages/categorias.pphx`

    <MultipleChoice>
      <Option>`src/pages/posts/post.pphx`</Option>
      <Option>`src/pages/posts/index.pphx`</Option>
      <Option>`src/components/sapatos/sapato.pphx`</Option>
      <Option isCorrect>`src/pages/categorias/index.pphx`</Option>
    </MultipleChoice>

2. `src/pages/posts.pphx`

    <MultipleChoice>
      <Option>`src/pages/products/sapatos.pphx`</Option>
      <Option>`src/pages/posts/post.pphx`</Option>
      <Option isCorrect>`src/pages/posts/index.pphx`</Option>
      <Option>`src/pages/categorias/index.pphx`</Option>
    </MultipleChoice>

3. `src/pages/products/sapatos/index.pphx`

    <MultipleChoice>
      <Option isCorrect>`src/pages/products/sapatos.pphx`</Option>
      <Option>`src/pages/posts/post.pphx`</Option>
      <Option>`src/pages/posts/index.pphx`</Option>
      <Option>`src/components/sapatos/sapato.pphx`</Option>
    </MultipleChoice>
...
<Checklist>
- [ ] Eu posso utilizar a funcionalidade de padrão de roteamento `/pages/pasta/index.pphx`.
</Checklist>
</Box>

### Recursos

- [Roteamento Estático no PPHLX](/pt-br/guides/routing/#static-routes)
