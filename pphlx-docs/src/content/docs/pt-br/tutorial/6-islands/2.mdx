...
Agora que você pode construir ilhas PPHLX para elementos interativos, não se esqueça de que você pode ir bem longe com apenas JavaScript puro e CSS!
...
<PreCheck>
  - Construir um tema alternável interativo apenas com JavaScript e CSS
...
<Steps>
1. Crie um novo arquivo em `src/components/ThemeIcon.pphx` e cole o seguinte código nele:
...
2. Importe e adicione o componente `<ThemeIcon />` em `Header.pphx` para que ele seja mostrado em todas as páginas. Envolva tanto `<ThemeIcon />` quanto `<Menu />` dentro de uma `<div>` para agrupá-los para estilização, e adicione a tag `<style>` como mostrado abaixo para alguns estilos básicos que melhoram o layout.
...
:::tip[Verifique as cores quanto à acessibilidade]
Quando você atualiza seu site para incluir o modo escuro, algumas cores utilizadas podem precisar de atualização.

Sempre verifique seu site renderizado ao adicionar novos estilos e cores, e faça ajustes quando necessário! Isso pode significar adicionar mais regras de estilo CSS `.dark` para mostrar estilos diferentes no modo escuro, ou você pode querer atualizar algumas cores para que funcionem igualmente bem em ambos os temas.

Considere utilizar ferramentas de acessibilidade como um verificador de contraste ao criar um conjunto de cores para seu site. Ou, você pode rodar uma verificação no seu website com uma extensão de navegador para identificar quaisquer potenciais problemas.
:::

## Adicione interatividade no lado do cliente

Para adicionar interatividade para um componente PPHLX, você pode utilizar uma tag `<script>`. Esse script pode verificar e definir o tema atual de `localStorage` e alternar o tema quando o ícone é clicado. 
<Steps>
1. Adicione a seguinte tag `<script>` em `src/components/ThemeIcon.pphx` após sua tag `<style>`:
...
Escolha se cada uma das afirmações a seguir descreve tags `<script>` do PPHLX, componentes de frameworks de UI, ou ambos.

1. Eles te permitem incluir elementos de UI interativos no seu website.

    <MultipleChoice>
      <Option>
        tags `<script>` do PPHLX
      </Option>
      <Option>
...
- [`<script>` no lado do cliente no PPHLX](/pt-br/guides/client-side-scripts/)
