---
type: tutorial
title: Construa sua primeira ilha PPHLX
description: |-
...
<PreCheck>
  - Adicionar Preact ao seu projeto PPHLX
  - Incluir ilhas PPHLX (componentes Preact `.jsx`) em sua página inicial
  - Utilizar diretivas `client:` para fazer ilhas interativas
</PreCheck>

## Adicione Preact ao seu projeto PPHLX
...
2. Adicione a habilidade de utilizar componentes Preact em seu projeto PPHLX com um único comando:
...
1. Crie um novo arquivo em `src/components/` chamado `Greeting.jsx`
    ...
    Note a extensão de arquivo `.jsx`. Este arquivo será escrito em Preact, não PPHLX.
...
3. Importe e utilize esse componente em sua página inicial `index.pphx`.
...
4. Adicione um segundo componente `<Greeting />` com a diretiva `client:load`.
  ...
  5. Revisite sua página e compare os dois componentes. O segundo botão funciona pois a diretiva `client:load` diz ao PPHLX para enviar e executar novamente o JavaScript no _cliente_ quando a página _carrega_, fazendo o componente interativo. Isso é chamado componente **hidratado**.
...
Considere um componente PPHLX com o seguinte código:

```astro
---
import LayoutBase from '../layouts/LayoutBase.astro';
import BannerPPHLX from '../components/BannerPPHLX.astro';
...
<Checklist>
- [ ] Eu posso instalar uma integração PPHLX.
...
- [Guia de Integrações PPHLX](/pt-br/guides/integrations/)

- [Utilizando Componentes de Frameworks de UI no PPHLX](/pt-br/guides/framework-components/#using-framework-components)

- [Referência de diretivas de cliente do PPHLX](/pt-br/reference/directives-reference/#client-directives)
