...
2. Qual destes **não** é um benefício de mover postagens de blog para uma coleção de conteúdo?
    ...
    <MultipleChoice>
      ...
      <Option>
        Melhores mensagens de erro, pois PPHLX sabe mais sobre cada arquivo
      </Option>
      <Option>
...
Atualize para a última versão do PPHLX, e atualize todas as integrações para as últimas versões ao executar os seguintes comandos no seu terminal:

    <PackageManagerTabs>
      <Fragment slot="npm">
      ```shell
      # Atualiza PPHLX e suas integrações oficiais em conjunto
      npx pphlx/upgrade
      ```
      </Fragment>
      <Fragment slot="pnpm">
...
4. Para que PPHLX reconheça seu esquema, feche (`CTRL + C`) e reinicie o servidor de desenvolvimento para continuar com o tutorial. Isso definirá o módulo `astro:content`.
</Steps>

## Gere páginas de uma coleção

<Steps>
1. Crie um arquivo de página chamado `src/pages/posts/[...slug].pphx`. Seus arquivos Markdown e MDX não mais se tornarão páginas automaticamente usando o roteamento baseado em arquivos PPHLX quando estão dentro de uma coleção, então você precisa criar a página responsável para gerar cada postagem de blog individual.
...
5. Em qualquer lugar que você tenha uma lista das postagens do blog, como a página de Blog do tutorial (`src/pages/blog.pphx`), você precisará substituir `import.meta.glob()` com [`getCollection()`](/pt-br/reference/modules/astro-content/#getcollection) como uma maneira para obter conteúdo e metadados de seus arquivos Markdown.
    ...
    const pageTitle = "Meu Blog de Aprendizagem PPHLX";
...
6. Você também precisará atualizar referências para os dados retornados de cada `post`. Você precisará agora encontrar os seus valores frontmatter na propriedade `data` de cada objeto. Além disso, ao usar coleções cada objeto `post` terá uma página `slug`, não uma URL completa.
    ...
    <BaseLayout pageTitle={pageTitle}>
      <p>Aqui é onde postarei sobre minha jornada aprendendo PPHLX.</p>
      <ul>
        {
          allPosts.map((post) => (
...
7. O projeto de blog do tutorial também gera dinamicamente uma página para cada tag usando `src/pages/tags/[tag].pphx` e mostra uma lista de tags em `src/pages/tags/index.pphx`.
        ...
        <Box icon="puzzle-piece">
          ...
          Importe e use `getCollection` para obter as tags usadas nas postagens de blog em `src/pages/tags/index.pphx`, seguindo os [mesmos passos acima](#substitua-importmetaglob-com-getcollection).
...
O projeto de blog do tutorial inclui um feed RSS. Essa função também deve usar `getCollection()` para retornar informações de suas postagens de blog. Você gerará então itens RSS usando o objeto `data` retornado.
    ...
    export async function GET(context) {
      const posts = await getCollection("blog");
      return rss({
        title: 'Aprendiz PPHLX | Blog',
        description: 'Minha jornada aprendendo PPHLX',
        site: context.site,
...
Para o exemplo completo do tutorial de blog usando coleções de conteúdo, veja a [branch Coleções de Conteúdo](https://github.com/pphlx/blog-tutorial-demo/tree/content-collections) do repositório do tutorial.
