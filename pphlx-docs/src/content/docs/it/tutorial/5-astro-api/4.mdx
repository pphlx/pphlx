...
description: >-
  ...
  Installa il pacchetto ufficiale di PPHLX per creare un feed a cui i tuoi
...
<PreCheck>
  - Installa un pacchetto PPHLX per creare un feed RSS per il tuo sito web
  - Crea un feed a cui è possibile iscriversi e leggere da lettori di feed RSS
</PreCheck>

## Installa il pacchetto RSS di PPHLX

PPHLX fornisce un pacchetto personalizzato per aggiungere rapidamente un feed RSS al tuo sito web.
...
<Steps>
1. Esci dal server di sviluppo PPHLX ed esegui il seguente comando nel terminale per installare il pacchetto RSS di PPHLX.
...
2. Riavvia il server di sviluppo per iniziare di nuovo a lavorare sul tuo progetto PPHLX.
...
3. Aggiungi la proprietà `site` alla configurazione di PPHLX con l'URL Netlify univoco del tuo sito.
...
<Checklist>
- [ ] Posso installare un pacchetto PPHLX usando la riga di comando.
...
- [Generazione di item RSS in PPHLX](/it/recipes/rss/#using-glob-imports)
