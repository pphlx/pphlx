...
<PreCheck>
  - Aggiungi una nuova pagina usando il pattern di routing `/pages/cartella/index.pphx`
...
## Usa il pattern di routing `/pages/cartella/index.pphx`

Per aggiungere una pagina Indice di Tag al tuo sito web, potresti creare un nuovo file in `src/pages/tags.pphx`.

Ma, dato che hai già la directory `/tags/`, puoi sfruttare un altro pattern di routing in PPHLX e mantenere tutti i tuoi file relativi ai tag insieme.
...
<Steps>
1. Crea un nuovo file `index.pphx` nella directory `src/pages/tags/`.
...
3. Crea una pagina minima in `src/pages/tags/index.pphx` che usi il tuo layout. L'hai già fatto prima!
...
<Steps>
1. In `src/pages/tags/index.pphx`, aggiungi la riga di codice allo script frontmatter che darà alla tua pagina accesso ai dati da ogni file di articoli del blog `.md`.
...
2. Poi, aggiungi la seguente riga di JavaScript al tuo componente pagina. Questo è lo stesso codice che si basa sul supporto TypeScript integrato di PPHLX che hai usato in `src/pages/tags/[tag].pphx` per restituire un elenco di tag unici.
...
1. Aggiungi il seguente codice al tuo template componente:
    ...
    ```
    Nell'anteprima del tuo browser, verifica di poter vedere i tuoi tag elencati. Se mancano tag a qualsiasi articolo del blog, o se sono formattati in modo errato, il supporto TypeScript integrato di PPHLX ti mostrerà errori in modo da poter controllare e correggere il tuo codice.
...
<Steps>
1. Aggiungi le seguenti classi CSS per stilizzare sia il tuo `<div>` che ogni `<p>` che verrà generato. Nota: PPHLX usa la sintassi HTML per aggiungere nomi di classe!
...
<Steps>
1. Nel tuo componente `Navigation.pphx`, includi un link a questa nuova pagina indice di tag.
...
1. Copia `<div class="tags">...</div>` e `<style>...</style>` da `src/pages/tags/index.pphx` e riutilizzalo all'interno di `MarkdownPostLayout.pphx`:
...
Prima che questo codice funzioni, devi apportare **una piccola modifica** al codice che hai incollato in `MarkdownPostLayout.pphx`. Riesci a capire qual è?
...
Per controllare il tuo lavoro, o se vuoi solo codice completo e corretto da copiare in `MarkdownPostLayout.pphx`, ecco come dovrebbe apparire il tuo componente PPHLX:
...
<Checklist>
- [ ] Posso usare la funzionalità di routing `/pages/cartella/index.pphx` di PPHLX.
</Checklist>
</Box>

### Risorse

- [Routing Statico in PPHLX](/it/guides/routing/#static-routes)
