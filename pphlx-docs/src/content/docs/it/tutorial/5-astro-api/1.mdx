...
<Steps>
1. Aggiungi il seguente codice a `blog.pphx` per restituire informazioni su tutti i tuoi file Markdown. `import.meta.glob()` restituirà un array di oggetti, uno per ogni articolo del blog.
...
2. Per generare l'intero elenco di articoli dinamicamente, usando i titoli e gli URL degli articoli, sostituisci i tuoi singoli tag `<li>` con il seguente codice PPHLX:
    ...
    Il tuo intero elenco di articoli del blog viene ora generato dinamicamente usando il supporto TypeScript integrato di PPHLX, mappando sull'array restituito da `import.meta.glob()`.
...
Prova da solo ad apportare tutte le modifiche necessarie al tuo progetto PPHLX in modo da poter invece usare il seguente codice per generare il tuo elenco di articoli del blog:
...
2. Scrivi la riga di codice nel tuo componente in modo che sia in grado di ricevere un `title` e `url` come `PPHLX.props`.
...
Se il tuo componente PPHLX contiene la seguente riga di codice:
...
- [Importazione di pattern glob in PPHLX](/it/guides/imports/#importmetaglob)
