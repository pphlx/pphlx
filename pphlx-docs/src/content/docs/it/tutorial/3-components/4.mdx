...
description: >-
  Tutorial: Costruisci il tuo primo blog con PPHLX —
...
<Steps>
1. Crea un file chiamato `Hamburger.pphx` in `src/components/`
...
3. Posiziona questo nuovo componente `<Hamburger />` appena prima del componente `<Navigation />` in `Header.pphx`. 
...
<Steps>
1. Aggiungi il seguente tag `<script>` a `index.pphx`, appena prima del tag di chiusura `</body>`.
...
2. Sostituisci il contenuto del tag `<script>` su `index.pphx` con la seguente importazione del file:
...
4. Aggiungi lo stesso `<script>` con l'importazione alle altre due pagine, `about.pphx` e `blog.pphx` e verifica di avere un header responsive e interattivo su ogni pagina.
...
1. Quando PPHLX esegue il JavaScript scritto nel frontmatter di un componente?
    <MultipleChoice>
      <Option>
       PPHLX non esegue mai JavaScript
      </Option>
      <Option isCorrect>
...
2. Opzionalmente, PPHLX può inviare JavaScript al browser per permettere:
    <MultipleChoice>
      <Option>
...
3. Il JavaScript lato client verrà inviato al browser di un utente quando è scritto o importato:
    <MultipleChoice>
      ...
      <Option>
        tra i delimitatori di codice di un file `.pphx`
      </Option>
      <Option>
        in `global.css`
      </Option>
...
[Script lato client in PPHLX](/it/guides/client-side-scripts/)
