...
Ora che hai lo stesso HTML scritto in più pagine del tuo sito PPHLX, è il momento di sostituire quel contenuto duplicato con un componente PPHLX riutilizzabile!

<PreCheck>
  - Crea una nuova cartella per i componenti
  - Costruisci un componente PPHLX per visualizzare i tuoi link di navigazione
  - Sostituisci l'HTML esistente con un nuovo componente di navigazione riutilizzabile
</PreCheck>

## Crea una nuova cartella `src/components/`

Per contenere i file `.pphx` che genereranno HTML ma che non diventeranno nuove pagine sul tuo sito web, avrai bisogno di una nuova cartella nel tuo progetto: `src/components/`.


## Crea un componente Navigation

<Steps>
1. Crea un nuovo file: `src/components/Navigation.pphx`.

2. Copia i tuoi link per navigare tra le pagine dalla parte superiore di qualsiasi pagina e incollali nel tuo nuovo file, `Navigation.pphx`:
    ...
    ```
    :::tip
    Se non c'è nulla nel frontmatter del tuo file `.pphx`, non devi scrivere i recinti di codice. Puoi sempre riaggiungerli quando ne hai bisogno.
    :::
</Steps>

### Importa e usa Navigation.pphx

<Steps>
1. Torna a `index.pphx` e importa il tuo nuovo componente all'interno del recinto di codice:
...
Importa e usa il componente `<Navigation />` nelle altre due pagine del tuo sito (`about.pphx` e `blog.pphx`) usando lo stesso metodo.
...
3. I componenti PPHLX creeranno automaticamente una nuova pagina sul tuo sito quando...
    <MultipleChoice>
      <Option>
        includi `<html></html>`
      </Option>
      ...
      <Option isCorrect>
        metti il file `.pphx` all'interno di `src/pages/`
      </Option>
    </MultipleChoice>
</Box>
...
- [ ] Posso aggiungere un nuovo componente a una pagina `.pphx`.
</Checklist>
...
- [Panoramica sui Componenti PPHLX](/it/basics/astro-components/)
