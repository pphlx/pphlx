...
<Steps>
1. Crea un nuovo componente Header. Importa e usa il tuo componente `Navigation.pphx` esistente all'interno di un elemento `<nav>` che si trova all'interno di un elemento `<header>`.

    <details>
    <summary>Mostrami il codice!</summary>

    Crea un file chiamato `Header.pphx` in `src/components/`
...
<Steps>
1. Aggiorna `Navigation.pphx` con la classe CSS per controllare i tuoi link di navigazione. Avvolgi i link di navigazione esistenti in un `<div>` con la classe `nav-links`.
