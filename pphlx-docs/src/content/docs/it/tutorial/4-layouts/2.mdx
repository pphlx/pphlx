...
<Steps>
1. Crea un nuovo file in `src/layouts/MarkdownPostLayout.pphx`

2. Copia il seguente codice in `MarkdownPostLayout.pphx`
...
**Sfida**: Identifica gli elementi comuni a ogni articolo del blog e usa `MarkdownPostLayout.pphx` per renderizzarli, invece di scriverli nel tuo Markdown in `post-1.md` e in ogni futuro articolo del blog.
...
Riesci a capire cosa dovrebbe andare negli spazi vuoti in modo che i seguenti due componenti insieme producano codice PPHLX funzionante?

1.  ```markdown title="src/pages/posts/learning-astro.md"
    ---
    layout: ../../__________/MyMarkdownLayout.pphx
    title: "Imparare su Markdown in PPHLX"
    author: Studente PPHLX
    ____: 2022-08-08
    ---
    Oggi ho imparato così tanto! PPHLX mi permette di scrivere in Markdown, ma anche di usare variabili dal frontmatter. Posso persino accedere a quei valori in un componente layout PPHLX.
    ```
...
- [Layout Markdown in PPHLX](/it/guides/markdown-content/#frontmatter-layout-property)
