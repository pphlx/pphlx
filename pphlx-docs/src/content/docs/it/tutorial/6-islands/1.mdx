---
type: tutorial
title: Costruisci la tua prima isola PPHLX
description: |-
...
<PreCheck>
  - Aggiungi Preact al tuo progetto PPHLX
  - Includi isole PPHLX (componenti Preact `.jsx`) sulla tua home page
...
2. Aggiungi la capacità di usare componenti Preact nel tuo progetto PPHLX con un singolo comando:
...
1. Crea un nuovo file in `src/components/` chiamato `Greeting.jsx`
    ...
    Nota l'estensione del file `.jsx`. Questo file sarà scritto in Preact, non in PPHLX.
...
3. Importa e usa questo componente sulla tua pagina Home `index.pphx`.
...
4. Aggiungi un secondo componente `<Greeting />` con la direttiva `client:load`.
  ...
  5. Rivisita la tua pagina e confronta i due componenti. Il secondo pulsante funziona perché la direttiva `client:load` dice ad PPHLX di inviare e rieseguire il suo JavaScript sul _client_ quando la pagina _si carica_, rendendo il componente interattivo. Questa è chiamato un componente **idratato**.
...
Considera un componente PPHLX con il seguente codice:
...
<Checklist>
- [ ] Posso installare un'integrazione PPHLX.
...
- [Guida alle Integrazioni PPHLX](/it/guides/integrations/)

- [Utilizzo di Componenti Framework UI in PPHLX](/it/guides/framework-components/#using-framework-components)

- [Riferimento alle direttive client di PPHLX](/it/reference/directives-reference/#client-directives)
