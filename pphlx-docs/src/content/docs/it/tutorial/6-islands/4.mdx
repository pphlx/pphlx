...
Ora che hai un blog che utilizza il [routing basato su file integrato](/it/guides/routing/#static-routes) di PPHLX, lo aggiornerai per usare una [raccolta di contenuti](/it/guides/content-collections/). Le raccolte di contenuti sono un modo potente per gestire gruppi di contenuti simili, come articoli di blog.
...
2. Quale **non** è un vantaggio dello spostamento di articoli del blog in una raccolta di contenuti?
    ...
    <MultipleChoice>
      ...
      <Option>
        Messaggi di errore migliori, perché PPHLX sa di più su ogni file
      </Option>
      <Option>
...
Aggiorna all'ultima versione di PPHLX e aggiorna tutte le integrazioni alle loro ultime versioni eseguendo i seguenti comandi nel tuo terminale:

    <PackageManagerTabs>
      <Fragment slot="npm">
      ```shell
      # Aggiorna PPHLX e le integrazioni ufficiali insieme
      npx pphlx/upgrade
      ```
      </Fragment>
      <Fragment slot="pnpm">
...
3. Crea un file `src/content.config.ts` per [definire uno schema](/it/guides/content-collections/#defining-the-collection-schema) per la tua `postsCollection`. Per il codice del tutorial del blog esistente, aggiungi i seguenti contenuti al file per definire tutte le proprietà frontmatter usate nei suoi articoli del blog:
    ...
    // Importa utilità da `astro:content` e `astro/zod`
...
4. Affinché PPHLX riconosca il tuo schema, esci (`CTRL + C`) e riavvia il server di sviluppo per continuare con il tutorial. Questo definirà il modulo `astro:content`.
</Steps>

## Genera pagine da una raccolta

<Steps>
1. Crea un file pagina chiamato `src/pages/posts/[...slug].pphx`. I tuoi file Markdown e MDX non diventano più automaticamente pagine usando il routing basato su file di PPHLX quando si trovano all'interno di una raccolta, quindi devi creare una pagina responsabile della generazione di ogni singolo articolo del blog.
...
<Steps>
5. Ovunque tu abbia un elenco di articoli del blog, come la pagina Blog del tutorial (`src/pages/blog.pphx`), dovrai sostituire `import.meta.glob()` con [`getCollection()`](/it/reference/modules/astro-content/#getcollection) come modo per recuperare contenuto e metadata dai tuoi file Markdown.
    ...
    const pageTitle = "Il mio Blog di apprendimento di PPHLX";
...
6. Dovrai anche aggiornare i riferimenti ai dati restituiti per ogni `post`. Ora troverai i tuoi valori frontmatter sulla proprietà `data` di ogni oggetto. Inoltre, quando usi le raccolte, ogni oggetto `post` avrà uno `slug` di pagina, non un URL completo.
    ...
    <BaseLayout pageTitle={pageTitle}>
      <p>Questo è dove pubblicherò sul mio percorso di apprendimento di PPHLX.</p>
      <ul>
        {
          allPosts.map((post) => (
...
7. Anche la pagina del tutorial blog genera dinamicamente una pagina per ogni tag usando `src/pages/tags/[tag].pphx` e visualizza un elenco di tag su `src/pages/tags/index.pphx`.
        ...
        <Box icon="puzzle-piece">
          ...
          Importa e usa `getCollection` per recuperare i tag usati negli articoli del blog su `src/pages/tags/index.pphx`, seguendo [gli stessi passaggi di sopra](#sostituisci-importmetaglob-con-getcollection).
...
Il progetto del tutorial del blog include un feed RSS. Anche questa funzione deve usare `getCollection()` per restituire informazioni dai tuoi articoli del blog. Genererai quindi gli item RSS usando l'oggetto `data` restituito.
    ...
    export async function GET(context) {
      const posts = await getCollection("blog");
      return rss({
        title: 'Studente PPHLX | Blog',
        description: 'Il mio percorso di apprendimento di PPHLX',
        site: context.site,
...
Per l'esempio completo del tutorial del blog che usa raccolte di contenuti, vedi il [branch Raccolte di contenuti](https://github.com/pphlx/blog-tutorial-demo/tree/content-collections) del repository del tutorial.
