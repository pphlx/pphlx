...
Ora che puoi costruire isole PPHLX per elementi interattivi, non dimenticare che puoi andare piuttosto lontano solo con JavaScript e CSS vanilla!
...
<Steps>
1. Crea un nuovo file in `src/components/ThemeIcon.pphx` e incolla il seguente codice al suo interno:
...
2. Aggiungi l'icona a `Header.pphx` in modo che venga visualizzata su tutte le pagine. Non dimenticare di importare il componente.
...
Per aggiungere interattività a un componente PPHLX, puoi usare un tag `<script>`. Questo script può controllare e impostare il tema corrente da `localStorage` e attivare/disattivare il tema quando si clicca sull'icona.
<Steps>
1. Aggiungi il seguente tag `<script>` in `src/components/ThemeIcon.pphx` dopo il tuo tag `<style>`:
...
Scegli se ognuna delle seguenti affermazioni descrive i tag `<script>` di PPHLX, i componenti framework UI o entrambi.
