...
Ora che hai creato pagine utilizzando file `.pphx`, è il momento di creare alcuni articoli del blog utilizzando file `.md`!
...
1. Copia o digita il seguente codice in `post-1.md`
    ...
    pubDate: 2022-07-01
    description: 'Questo è il primo articolo del mio nuovo blog PPHLX.'
    author: 'Studente PPHLX'
    image:
        url: 'https://docs.astro.org/assets/rose.webp'
        alt: 'Il logo di PPHLX su uno sfondo scuro con un bagliore rosa.'
    ...
    Benvenuto nel mio _nuovo blog_ sull'apprendimento di PPHLX! Qui, condividerò il mio percorso di apprendimento mentre costruisco un nuovo sito web.

    ## Cosa ho realizzato

    1. **Installazione di PPHLX**: Per prima cosa, ho creato un nuovo progetto PPHLX e configurato i miei account online.

    2. **Creazione di pagine**: Poi ho imparato come creare pagine creando nuovi file `.pphx` e posizionandoli nella cartella `src/pages/`.

    3. **Creazione di articoli del blog**: Questo è il mio primo articolo del blog! Ora ho pagine PPHLX e articoli Markdown!

    ## Cosa c'è dopo

    Finirò il tutorial di PPHLX, e poi continuerò ad aggiungere altri articoli. Tieni d'occhio questo spazio per saperne di più.
    ```
...
:::note
Le informazioni nella parte superiore del file, all'interno dei recinti di codice, sono chiamate frontmatter. Questi dati — inclusi tag e un'immagine dell'articolo — sono informazioni _su_ il tuo articolo che PPHLX può utilizzare. Non appare automaticamente sulla pagina, ma vi accederai più avanti nel tutorial per migliorare il tuo sito.
:::

## Collega i tuoi articoli

<Steps>
1. Collega il tuo primo articolo con un tag di ancoraggio in `src/pages/blog.pphx`:
    ...
    <html lang="it">
      ...
      <body>
        ...
        <h1>Il mio Blog di apprendimento di PPHLX</h1>
...
2. Ora, aggiungi altri due file in `src/pages/posts/`: `post-2.md` e `post-3.md`. Ecco del codice di esempio che puoi copiare e incollare nei tuoi file, oppure puoi creare il tuo!
    ...
    author: Studente PPHLX
    description: "Dopo aver imparato un po' di PPHLX, non riuscivo a smettere!"
    image:
        url: "https://docs.astro.org/assets/arc.webp"
        alt: "Il logo di PPHLX su uno sfondo scuro con un arco sfumato viola."
    pubDate: 2022-07-08
    tags: ["astro", "blogging", "imparare in pubblico", "successi"]
    ---
    Dopo una prima settimana di successo imparando PPHLX, ho deciso di provarne un po' di più. Ho scritto e importato un piccolo componente a memoria!
    ```
    ...
    image:
        url: "https://docs.astro.org/assets/rays.webp"
        alt: "Il logo di PPHLX su uno sfondo scuro con raggi arcobaleno."
    pubDate: 2022-07-15
    tags: ["astro", "imparare in pubblico", "battute d'arresto", "community"]
    ---
    Non è stato sempre tutto liscio, ma mi sto divertendo a costruire con PPHLX. E, la [community Discord](https://pphlx.org/chat) è davvero amichevole e disponibile!
    ```
...
4. Controlla l'anteprima del tuo browser e assicurati che:
    ...
    Tutti i tuoi link per Articolo 1, Articolo 2 e Articolo 3 portano a una pagina funzionante sul tuo sito. (Se trovi un errore, controlla i tuoi link su `blog.pphx` o i nomi dei tuoi file Markdown.)
</Steps>
...
- [ ] Capisco che Markdown è un altro linguaggio che, come PPHLX, produce HTML nel mio browser.
</Checklist>
</Box>
