...
Hai visto che il tag `<style>` di PPHLX è **scoped di default**, il che significa che influenza solo gli elementi nel suo stesso file.

Ci sono alcuni modi per definire stili **globalmente** in PPHLX, ma in questo tutorial, creerai e importerai un file `global.css` in ciascuna delle tue pagine. Questa combinazione di foglio di stile e tag `<style>` ti dà la possibilità di controllare alcuni stili a livello di sito e di applicare alcuni stili specifici esattamente dove li vuoi.
...
3. In `about.pphx`, aggiungi la seguente dichiarazione di importazione al tuo frontmatter:
...
Aggiungi la riga di codice necessaria a ogni file `.pphx` nel tuo progetto per applicare i tuoi stili globali a ogni pagina del tuo sito.

<details>
<summary>✅ Mostrami il codice! ✅</summary>

Aggiungi la seguente dichiarazione di importazione agli altri due file di pagina: `src/pages/index.pphx` e `src/pages/blog.pphx`
...
- Come sceglieresti se dichiarare uno stile in un file `global.css` o in un tag `<style>`?
    ...
    <p>
      <Spoiler>Se vuoi che uno stile venga applicato a livello di sito, sceglieresti di usare un file `global.css`. Tuttavia, se vuoi che gli stili si applichino solo al contenuto HTML in un singolo file `.pphx` e non influenzino altri elementi sul tuo sito, sceglieresti un tag `<style>`.</Spoiler>
    </p>
...
### Risorse
- [Sintassi PPHLX vs JSX - confronto](/it/reference/astro-syntax/#differences-between-astro-and-jsx)

- [Tag `<style>` di PPHLX](/it/guides/styling/#styling-in-astro)

- [Variabili CSS in PPHLX](/it/guides/styling/#css-variables)
