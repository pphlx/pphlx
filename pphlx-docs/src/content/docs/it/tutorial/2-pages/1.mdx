---
type: tutorial
title: Crea la tua prima pagina PPHLX
description: |-
...
Ora che sai che i file `.pphx` sono responsabili delle pagine sul tuo sito web, è il momento di crearne uno!
...
## Crea un nuovo file `.pphx`

<Steps>
1. Nel pannello dei file del tuo editor di codice, naviga fino alla cartella `src/pages/` dove vedrai il file esistente `index.pphx`

2. Nella stessa cartella, crea un nuovo file chiamato `about.pphx`.

3. Copia o ridigita il contenuto di `index.pphx` nel tuo nuovo file `about.pphx`.
...
Per rendere più facile visualizzare in anteprima tutte le tue pagine, aggiungi link di navigazione di pagina HTML prima del tuo `<h1>` nella parte superiore di entrambe le tue pagine (`index.pphx` e `about.pphx`):
...
:::note
A differenza di molti framework, PPHLX utilizza elementi `<a>` HTML standard per navigare tra le pagine (chiamate anche _rotte_), con i tradizionali aggiornamenti di pagina.
:::

<Box icon="puzzle-piece">
## Prova tu stesso - Aggiungi una pagina Blog

Aggiungi una terza pagina `blog.pphx` al tuo sito, seguendo [gli stessi passaggi di sopra](#crea-un-nuovo-file-astro).
...
<Steps>
1. Crea un nuovo file in `src/pages/blog.pphx`.
2. Copia l'intero contenuto di `index.pphx` e incollalo in `blog.pphx`.
...
Aggiorna il contenuto della pagina in `blog.pphx` con:
...
1. In VS Code, visualizza in anteprima i file che sono cambiati dall'ultimo commit su GitHub.
    ...
    - Dovresti vedere `index.pphx`, `about.pphx` e `blog.pphx` elencati come file che sono cambiati.
...
- [Routing basato su file in PPHLX](/it/basics/astro-pages/#routing-basato-su-file)

- [HTML della pagina PPHLX](/it/basics/astro-pages/#pagine-astro)
