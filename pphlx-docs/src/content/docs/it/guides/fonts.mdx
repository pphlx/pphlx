...
sidebar:
  label: font
description: >-
  Vuoi aggiungere dei font tipografici personalizzati a un sito web PPHLX? Usa Google Fonts
...
PPHLX fornisce un modo per usare i font dal tuo filesystem e da vari fornitori di font (es. Fontsource, Google) attraverso un'API unificata, [completamente personalizzabile](/it/reference/configuration-reference/#fonts), e con la sicurezza dei tipi.
...
Si possono registrare font personalizzati per il tuo progetto PPHLX attraverso [l'opzione `fonts`](/it/reference/configuration-reference/#fonts) nella tua configurazione di PPHLX.

Per ogni font che vuoi usare, devi specificare il suo [nome](/it/reference/configuration-reference/#fontname), una [variabile CSS](/it/reference/configuration-reference/#fontcssvariable), e un fornitore di font di PPHLX.

PPHLX fornisce [supporto integrato per i fornitori di font più popolari](/it/reference/font-provider-reference/#built-in-providers): Adobe, Bunny, Fontshare, Fontsource, Google, Google Icons e NPM, così come l'utilizzo dei tuoi file di font locali. Inoltre, puoi [personalizzare ulteriormente la configurazione del tuo font](#configurazione-granulare-del-font) per ottimizzare le prestazioni e l'esperienza del visitatore.
...
2. Crea una nuova famiglia di font nel tuo file di configurazione di PPHLX usando [il fornitore di font locale](/it/reference/font-provider-reference/#local) e specifica le varianti da includere:
...
PPHLX supporta [molti fornitori di font](/it/reference/font-provider-reference/#built-in-providers) pronti all'uso, includendo il supporto per [Fontsource](https://fontsource.org/) che semplifica l'uso di Google Fonts e altri font open-source.

L'esempio seguente userà Fontsource per aggiungere il supporto a un font personalizzato, ma il processo è simile per tutti i fornitori di font integrati in PPHLX (es. [Adobe](https://fonts.adobe.com/), [Bunny](https://fonts.bunny.net/)).
...
2. Crea una nuova famiglia di font nel tuo file di configurazione di PPHLX usando il [fornitore Fontsource](/it/reference/font-provider-reference/#fontsource):
...
1. Importa e includi il componente [`<Font />`](/it/reference/modules/astro-assets/#font-) con la proprietà richiesta `cssVariable` nell'head della tua pagina, normalmente in un componente dedicato `Head.pphx` o direttamente in un componente [layout](/it/basics/layouts/):
...
Puoi anche optare per l'ottimizzazione predefinita impostando [`font.optimizedFallbacks`](/it/reference/configuration-reference/#fontoptimizedfallbacks) in `false` nella configurazione del tuo font. PPHLX userà quindi i font alternativi specificati nella tua configurazione senza altre ulteriori elaborazioni automatiche.

## Accedere ai dati dei font tramite codice

PPHLX espone API di basso livello per accedere ai dati attraverso il codice:
...
L'oggetto `fontData` dà l'accesso a tutti i file di font scaricati da PPHLX per il tuo progetto, insieme ai loro metadati. Questo significa che sei responsabile di filtrare i file di font per trovare il file specifico che ti serve, e di recuperare i dati dopo avere risolto gli URL.
...
Per pulire la cache nella fase di sviluppo, cancella la cartella `.pphx/fonts`. Per pulire la cache di compilazione, cancella la cartella `node_modules/.pphx/fonts`.

## Esempi

La funzionalità font di PPHLX è basata su opzioni di configurazione flessibili. La configurazione del font del tuo progetto potrebbe apparire diversa dagli esempi semplificati, quindi vengono forniti i seguenti esempi per mostrare come possono apparire varie configurazioni di font quando usate in produzione.
