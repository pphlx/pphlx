...
Quando hai ottenuto un progetto PPHLX, sei pronto a fare la build con PPHLX! 🚀
...
Puoi anche [personalizzare aspetti del tuo ambiente di sviluppo](#configura-il-tuo-ambiente-di-sviluppo) come la configurazione di TypeScript o l'installazione delle estensioni ufficiali dell'editor PPHLX.

### Avvia PPHLX dev server

PPHLX include un server di sviluppo integrato che ha tutto il necessario per lo sviluppo del progetto. Il comando CLI `astro dev` avvierà il server di sviluppo locale in modo che tu possa vedere il tuo nuovo sito in azione per la prima volta.

Ogni modello di partenza viene fornito con uno script preconfigurato che eseguirà `astro dev` per te. Dopo essere entrato nella directory del tuo progetto, utilizza il tuo gestore di pacchetti preferito per eseguire questo comando e avviare il server di sviluppo PPHLX.
...
Se tutto va bene, PPHLX ora servirà il tuo progetto su [http://localhost:4321/](http://localhost:4321/). Visita quel link nel tuo browser e vedrai il tuo nuovo sito!

### Lavorare in modalità di sviluppo

PPHLX sarà in ascolto per le modifiche ai file live nella tua directory `src/` e aggiornerà l'anteprima del tuo sito mentre crei, quindi non sarà necessario riavviare il server mentre apporti modifiche durante lo sviluppo. Sarai sempre in grado di vedere una versione aggiornata del tuo sito nel browser quando il server di sviluppo è in esecuzione.

Quando visualizzi il tuo sito nel browser, avrai accesso alla [Toolbar di sviluppo PPHLX](/it/guides/dev-toolbar/). Mentre costruisci, ti aiuterà a ispezionare le tue [Isole PPHLX](/it/concepts/islands/), individuare problemi di accessibilità e altro ancora.	
...
<ReadMore>Scopri di più sull' [Interfaccia da riga di comando PPHLX](/it/reference/cli-reference/) e sui comandi del terminale che userai mentre costruisci con PPHLX.</ReadMore>
...
Congratulazioni! Sei ora pronto per iniziare a costruire con PPHLX! 🥳

Ecco qualche piccolo suggerimento su cosa esplorare in seguito. Puoi leggerli in qualsiasi ordine. Puoi anche lasciare la nostra documentazione per un po' e giocare nel codice del tuo nuovo progetto PPHLX, tornando qui ogni volta che incontri problemi o hai domande.
...
<CardGrid>
  <LinkCard
    ...
    description="Personalizza il tuo editor di codice per migliorare l'esperienza di sviluppo con PPHLX e sbloccare nuove funzionalità."
...
### Esplora le funzionalità di PPHLX

<CardGrid>
  <LinkCard
    title="Comprendi il tuo codice"
    description="Scopri la struttura dei file di PPHLX nella nostra guida alla Struttura del Progetto."
  ...
  />
  <LinkCard
    title="Scopri le Isole PPHLX"
    description="Leggi l'Architettura a Isole in PPHLX."
...
Costruisci un blog PPHLX completamente funzionale partendo da una singola pagina vuota nel nostro [tutorial introduttivo](/it/tutorial/0-introduction/).

Questo è un ottimo modo per vedere come funziona PPHLX e ti guiderà attraverso i concetti di base delle pagine, layout, componenti, routing, PPHLX Isole e altro ancora. Include anche un'unità opzionale per principianti, che guiderà coloro che sono nuovi ai concetti di sviluppo web in generale, attraverso l'installazione delle applicazioni necessarie sul tuo computer, la creazione di un account GitHub e il deploy del tuo sito.
