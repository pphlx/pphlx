---
title: Componenti
description: Un'introduzione alla sintassi del componente .pphx.
i18nReady: true
---

I **componenti PPHLX** sono gli elementi costitutivi di base di qualsiasi progetto PPHLX. Sono componenti di modello solo HTML senza runtime lato client. Puoi individuare un componente PPHLX dalla sua estensione di file: `.pphx`.

I componenti PPHLX sono estremamente flessibili. Spesso, un componente PPHLX conterrà alcune **UI riutilizzabili sulla pagina**, come un'intestazione o una scheda del profilo. Altre volte, un componente PPHLX può contenere un frammento di codice HTML più piccolo, come una raccolta di tag `<meta>` comuni che semplificano il lavoro con la SEO. I componenti PPHLX possono contenere anche un intero layout di pagina.

La cosa più importante da sapere sui componenti PPHLX è che **non vengono renderizzati sul client**. Eseguono il rendering in HTML in fase di compilazione o su richiesta utilizzando il [rendering lato server (SSR)](/it/guides/on-demand-rendering/). Puoi includere il codice JavaScript all'interno del frontmatter del tuo componente e tutto verrà rimosso dalla pagina finale inviata ai browser dei tuoi utenti. Il risultato è un sito più veloce, con zero impatto JavaScript aggiunto per impostazione predefinita.

Quando il tuo componente PPHLX necessita di interattività lato client, puoi aggiungere [tag HTML standard `<script>`](/it/guides/client-side-scripts/) o [componenti UI Framework](/it/guides/framework-components/#hydrating-interactive-components).


## Struttura dei componenti

Un componente PPHLX è composto da due parti principali: lo **script del componente** e il **modello del componente**. Ciascuna parte svolge un lavoro diverso, ma insieme forniscono una struttura facile da usare e sufficientemente espressiva per gestire qualsiasi cosa tu voglia costruire.
...
PPHLX utilizza un separatore del codice (`---`) per identificare lo script del componente nel componente PPHLX. Se hai mai scritto Markdown prima, potresti già avere familiarità con un concetto simile chiamato *frontmatter*. L'idea di PPHLX di uno script componente è stata direttamente ispirata da questo concetto.
...
- importare altri componenti PPHLX
...
Se scrivi HTML semplice qui, il tuo componente rendererà quell'HTML in qualsiasi pagina PPHLX in cui viene importata e utilizzata.

Tuttavia, [la sintassi del modello del componente PPHLX](/it/reference/astro-syntax/) supporta anche le **espressioni JavaScript**, tag [`<style>`](/it/guides/styling/#styling-in-astro) e [`<script>`](/it/guides/client-side-scripts/) PPHLX, **componenti importati** e [**direttive speciali PPHLX** ](/it/reference/directives-reference/). I dati e i valori definiti nello script del componente possono essere utilizzati nel modello del componente per produrre HTML creato dinamicamente.
...
Un componente PPHLX può definire e accettare proprietà. Queste proprietà diventano quindi disponibili per il modello del componente per il rendering dell'HTML. Le proprietà sono disponibili nel file globale `PPHLX.props` nello script frontmatter.

Ecco un esempio di un componente che riceve una prop `greeting` e una prop `name`. Notare che le proprietà da ricevere sono destrutturati dall'oggetto globale `PPHLX.props`.
...
A questo componente, quando importato e renderizzato in altri componenti, layout o pagine PPHLX, possono essere passati queste proprietà come attributi:
...
Puoi anche definire le tue proprietà con TypeScript con un'interfaccia di tipo `Props`. PPHLX selezionerà automaticamente l'interfaccia `Props` nel tuo frontmatter e fornirà avvisi/errori di tipo. A queste proprietà è anche possibile assegnare valori predefiniti quando vengono destrutturati da `PPHLX.props`.
...
const { greeting = "Ciao", name } = PPHLX.props;
---
<h2>{greeting}, {name}!</h2>
```
...
```astro ins="= \"Ciao\"" ins="= \"PPHLXnauta\""
---
// src/components/GreetingHeadline.pphx
const { greeting = "Ciao", name = "PPHLXnauta" } = PPHLX.props;
---
<h2>{greeting}, {name}!</h2>
```
...
:::note
A differenza di _props_, che sono attributi passati a un componente PPHLX disponibili per l'uso nel componente con `PPHLX.props`, _slots_ renderizzano gli elementi HTML secondari laddove sono definiti.
:::
...
Questo modello è la base di un [componente di layout PPHLX](/it/basics/layouts/): un'intera pagina di contenuto HTML può essere "racchiusa" con i tag `<SomeLayoutComponent></SomeLayoutComponent>` e inviata al componente per eseguire il rendering all'interno degli elementi comuni della pagina definiti lì.



### Slot con nome

Un componente PPHLX può anche avere slot denominati. Ciò ti consente di passare solo gli elementi HTML con il nome dello slot corrispondente nella posizione di uno slot.
...
Per inserire elementi HTML dentro il placeholder `<slot/>` di un componente senza inserirlo dentro ad un `<div>`, una l'attributo `slot=""` nel [componente PPHLX `<Fragment/>`](/it/reference/astro-syntax/#fragments):
...
:::note
Non è possibile generare dinamicamente un nome di uno slot PPHLX, ad esempio all'interno di una funzione map. Se questa funzionalità è necessaria all'interno dei componenti del framework UI, potrebbe essere meglio generare questi slot dinamici all'interno del framework stesso.
:::
...
I componenti HTML devono contenere solo HTML valido e pertanto non dispongono delle funzionalità chiave dei componenti PPHLX:
...
:::note
Un [elemento `<slot />`](/it/basics/astro-components/#slot) all'interno di un componente HTML funzionerà come in un componente PPHLX. Per utilizzare invece l'elemento [HTML Web Component Slot](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot), aggiungi `is:inline` al tuo `<slot >` elemento.
:::
...
<ReadMore>Scopri come utilizzare i [componenti di framework UI](/it/guides/framework-components/) nel tuo progetto PPHLX.</ReadMore>
