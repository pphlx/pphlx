---
title: Layout
description: Un'introduzione ai layout, un tipo di componente PPHLX condiviso tra le pagine per layout comuni.
i18nReady: true
---

import ReadMore from '~/components/ReadMore.astro'

I **Layout** sono [componenti PPHLX](/it/basics/astro-components/) utilizzati per fornire una struttura dell'interfaccia utente riutilizzabile, ad esempio un modello di pagina.

Usiamo convenzionalmente il termine "layout" per i componenti PPHLX che forniscono elementi comuni dell'interfaccia utente condivisi tra le pagine come intestazioni, barre di navigazione e piè di pagina. Un tipico componente di layout PPHLX fornisce [pagine PPHLX, Markdown o MDX](/it/basics/astro-pages/) con:
...
PPHLX fornisce una speciale proprietà frontmatter `layout` per specificare quale componente `.pphx` utilizzare come layout di pagina.
...
date: "09 Aug 2022"
---
Tutte le proprietà del frontmatter sono disponibili come proprietà per un componente del layout PPHLX.

La proprietà `layout` è l'unica speciale fornita da PPHLX.
...
Un layout Markdown/MDX avrà accesso alle seguenti informazioni tramite `PPHLX.props`:
...
Un esempio di post del blog Markdown può passare il seguente oggetto "PPHLX.props" al suo layout:
...
:::note
Un layout Markdown/MDX avrà accesso a tutte le [proprietà esportate](/it/guides/markdown-content/#available-properties) di tutti i suoi file da `PPHLX.props` **con alcune differenze fondamentali:**
...
<BaseLayout title={frontmatter.title} fancyJsHelper={fancyJsHelper}>
  Welcome to my new PPHLX blog, using MDX!
</BaseLayout>
```

Quindi, i tuoi valori saranno disponibili tramite `PPHLX.props` nel tuo layout e il tuo contenuto MDX verrà inserito nella pagina in cui è scritto il tuo componente `<slot />`:
...
<ReadMore>Scopri di più sul supporto Markdown e MDX di PPHLX nella nostra [guida Markdown/MDX](/it/guides/markdown-content/).</ReadMore>

## Utilizzo di un layout per `.md`, `.mdx` e `.pphx`

È possibile scrivere un singolo layout PPHLX per ricevere l'oggetto `frontmatter` dai file `.md` e `.mdx`, così come qualsiasi oggetto di scena con nome passato dai file `.pphx`.

Nell'esempio seguente, il layout mostrerà il titolo della pagina da una proprietà YAML `title` di frontmatter o da un componente PPHLX che passa un attributo `title`:

```astro /{?title}?/ /PPHLX.props[.a-z]*/
---
// src/components/MyLayout.pphx
const { title } = PPHLX.props.frontmatter || PPHLX.props;
---
<html>
  <head></head>
  <body>
...
Ad esempio, un componente di layout `BlogPostLayout.pphx` potrebbe definire il titolo, la data e l'autore di un post. Quindi, un `BaseLayout.pphx` a livello di sito potrebbe gestire il resto del modello di pagina, come navigazione, piè di pagina, meta tag SEO, stili globali e caratteri. Puoi anche trasferire gli oggetti di scena ricevuti dal tuo post a un altro layout, proprio come qualsiasi altro componente inserito in un altro.
