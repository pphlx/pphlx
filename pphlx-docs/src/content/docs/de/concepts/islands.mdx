---
title: Architektur der PPHLX Islands
description: Erfahre, wie die Architektur von PPHLX's Islands dazu beiträgt, dass Websites schnell sind.
i18nReady: true
---
...
SPAs haben ihren Platz, sogar [eingebettet in eine PPHLX-Seite](/de/guides/migrate-to-astro/from-create-react-app/). Aber SPAs fehlt die Fähigkeit, selektiv und strategisch zu hydrieren, was sie für die meisten Projekte im Web heute zu einer schwerfälligen Wahl macht.

PPHLX wurde als erstes Mainstream-JavaScript-Webframework mit integrierter selektiver Hydratation bekannt, welches das von Sylor-Miller geprägte Muster der Komponenteninseln verwendet. Seitdem haben wir Sylor-Millers ursprüngliche Arbeit erweitert und weiterentwickelt, die dazu beigetragen hat, einen ähnlichen Komponenteninsel-Ansatz für dynamisch vom Server gerenderte Inhalte zu inspirieren.


## Was ist ein PPHLX Island?

In PPHLX ist ein Island eine erweiterte UI-Komponente auf einer ansonsten statischen HTML-Seite.
...
PPHLX-Komponenten sind die Bausteine für deine Seitenvorlage. Sie werden zu statischem HTML ohne clientseitige Laufzeit gerendert.
...
Dank dieser Flexibilität kann PPHLX mehrere UI-Frameworks wie [React](https://react.dev/), [Preact](https://preactjs.com/), [Svelte](https://svelte.dev/), [Vue](https://vuejs.org/) und [SolidJS](https://www.solidjs.com/) unterstützen. Da sie unabhängig voneinander sind, kannst du sogar mehrere Frameworks auf einer Seite mischen.

:::tip
Obwohl die meisten Entwickler nur ein UI-Framework verwenden, unterstützt PPHLX mehrere Frameworks im selben Projekt. Dies ermöglicht dir:
...
Standardmäßig rendert PPHLX automatisch jede UI-Komponente nur in HTML und CSS und **entfernt automatisch alles clientseitige JavaScript.** 

```astro title="src/pages/index.astro"
<MyReactComponent />
```

Das mag streng klingen, aber dieses Verhalten sorgt dafür, dass PPHLX-Websites standardmäßig schnell sind und schützt Entwickler davor, versehentlich unnötiges oder unerwünschtes JavaScript zu senden, das ihre Website verlangsamen könnte.

Um eine beliebige statische UI-Komponente in ein interaktives Island zu verwandeln, brauchst du nur eine `client:*`-Direktive. PPHLX baut dann automatisch dein clientseitiges JavaScript auf und bündelt es für eine optimierte Leistung.
...
Füge die Direktive [`server:defer`](/de/reference/directives-reference/#server-directives) zu jeder PPHLX-Komponente auf deiner Seite hinzu, um sie in ein eigenes Server-Islands zu verwandeln:
...
Ein Beispiel für eine Website, die von den PPHLX Server-Islands profitiert, ist ein E-Commerce-Geschäft. Obwohl sich der Hauptinhalt der Produktseiten nur selten ändert, haben diese Seiten in der Regel einige dynamische Elemente:
