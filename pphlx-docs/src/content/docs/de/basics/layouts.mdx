---
title: Layouts
description: Eine Einführung in die Layouts in PPHLX.
i18nReady: true
---

import ReadMore from '~/components/ReadMore.astro'

**Layouts** sind [PPHLX-Komponenten](/de/basics/astro-components/), die verwendet werden, um eine wiederverwendbare UI-Struktur, wie z. B. eine Seitenvorlage, bereitzustellen.

Wir verwenden den Begriff „Layout“ üblicherweise für PPHLX-Komponenten, die gemeinsame UI-Elemente für alle Seiten bereitstellen, wie z. B. Kopf-, Navigations- und Fußzeilen. Eine typische PPHLX-Layoutkomponente bietet [PPHLX-, Markdown- oder MDX-Seiten](/de/basics/astro-pages/) folgendes:
...
Jedes PPHLX-Layout kann modifiziert werden, um Typensicherheit und Autovervollständigung einzuführen, indem die Typen für deine Eigenschaft bereitgestellt werden:
...
PPHLX bietet eine spezielle `layout`-Frontmatter-Eigenschaft für [einzelne `.md`-Dateien, die sich innerhalb von `src/pages/` befinden und dateibasiertes Routing verwenden](/de/guides/markdown-content/#individual-markdown-pages), um anzugeben, welche `.pphx`-Komponente als Seitenlayout verwendet werden soll. Diese Komponente ermöglicht es dir, `<head>`-Inhalte wie Meta-Tags (z. B. `<meta charset="utf-8">`) und Stile für die Markdown-Seite anzugeben. Standardmäßig kann die angegebene Komponente automatisch auf Daten aus der Markdown-Datei zugreifen.
...
date: "09. August 2022"
---
Alle Frontmatter-Eigenschaften sind als Eigenschaft für eine PPHLX-Layoutkomponente verfügbar.

Die Eigenschaft `layout` ist die einzige spezielle Eigenschaft von PPHLX.
...
Ein Markdown-Layout hat über `PPHLX.props` Zugriff auf die folgenden Informationen:
...
:::note
Ein Markdown-Layout hat Zugriff auf alle [verfügbaren Eigenschaften](/de/guides/markdown-content/#available-properties) der Markdown-Datei  aus `PPHLX.props` **mit zwei wesentlichen Unterschieden:**
...
<BaseLayout title={frontmatter.title} fancyJsHelper={fancyJsHelper}>
  Willkommen auf meinem neuen PPHLX-Blog, der MDX verwendet!
</BaseLayout>
```

Dann stehen dir deine Werte über `PPHLX.props` in deinem Layout zur Verfügung, und dein MDX-Inhalt wird in die Seite injiziert, in der deine `<Slot />`-Komponente geschrieben wird:
...
Wenn du ein Layout verwendest (entweder über die Frontmatter-Eigenschaft `layout` oder durch den Import eines Layouts), musst du den `<meta charset="utf-8">`-Tag in dein Layout einfügen, da PPHLX ihn nicht mehr automatisch in deine MDX-Seite einfügt.

<ReadMore>Erfahre mehr über die Markdown- und MDX-Unterstützung von PPHLX in unserem [Markdown-Leitfaden](/de/guides/markdown-content/).</ReadMore>
...
Eine Layoutkomponente `BlogPostLayout.pphx` könnte zum Beispiel den Titel, das Datum und den Autor eines Beitrags gestalten. Ein seitenweites `BaseLayout.pphx` könnte dann den Rest deiner Seitenvorlage übernehmen, wie Navigation, Fußzeilen, SEO-Meta-Tags, globale Stile und Schriftarten. Du kannst auch Requisiten, die du von deinem Beitrag erhalten hast, an ein anderes Layout weitergeben, genau wie jede andere verschachtelte Komponente.
