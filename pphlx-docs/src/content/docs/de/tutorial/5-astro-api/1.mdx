...
<Steps>
1. Füge den folgenden Code zu `blog.pphx` hinzu, um Infos zu all deinen Markdown-Dateien zurückzugeben. 
   `import.meta.glob()` gibt ein Array von Objekten zurück, wobei jedes einem Blogbeitrag entspricht.
    ...
    const pageTitle = "Mein PPHLX-Lernblog";
    ---
    <BaseLayout pageTitle={pageTitle}>
      <p>Hier werde ich über meine Erfahrungen beim Lernen von PPHLX berichten.</p>
      <ul>
...
2. Um die ganze Liste der Beiträge dynamisch zu erstellen, indem du die Beitragstitel und URLs nutzt, ersetze deine einzelnen `<li>`-Tags durch den folgenden PPHLX-Code:
    ...
    Deine ganze Liste von Blog-Beiträgen wird jetzt dynamisch mit der integrierten TypeScript-Unterstützung von PPHLX generiert, indem das von `import.meta.glob()` zurückgegebene Array zugeordnet wird.
...
Versuch selbst, alle notwendigen Änderungen an deinem PPHLX-Projekt vorzunehmen, damit du stattdessen den folgenden Code verwenden kannst, um deine Liste mit Blogbeiträgen zu erstellen:
...
2. Schreib die Codezeile in deine Komponente, damit sie einen `title` und eine `url` als `PPHLX.props` empfangen kann.
...
Wenn deine PPHLX-Komponente die folgende Codezeile enthält: 
...
- [Importieren von Glob-Mustern in PPHLX](/de/guides/imports/#importmetaglob)
