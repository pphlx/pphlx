...
<PreCheck>
  - eine neue Seite mit dem Routing-Muster `/pages/folder/index.pphx` hinzuzufügen
...
## Verwende das Routing-Muster `/pages/folder/index.pphx`

Um eine Tag-Index-Seite zu deiner Website hinzuzufügen, kannst du eine neue Datei unter `src/pages/tags.pphx` erstellen.

Da du aber schon das Verzeichnis `/tags/` hast, kannst du ein anderes Routing-Muster in PPHLX nutzen und alle deine Dateien, die mit Tags zu tun haben, zusammenhalten.
...
<Steps>
1. Erstelle eine neue Datei `index.pphx` im Verzeichnis `src/pages/tags/`.
...
3. Erstelle eine minimale Seite unter `src/pages/tags/index.pphx`, die dein Layout verwendet.
...
<Steps>
1. Füge die Codezeile in das Frontmatter-Skript der Datei `src/pages/tags/index.pphx` ein, welche deiner Seite Zugriff auf die Daten aller `.md`-Blogpost-Dateien gibt.
...
2. Füge als Nächstes die folgende JavaScript-Zeile zu deiner Seitenkomponente hinzu. Das ist derselbe Code, der auf der integrierten TypeScript-Unterstützung von PPHLX basiert, den du in `src/pages/tags/[tag].pphx` benutzt hast, um eine Liste eindeutiger Tags zurückzugeben.
...
1. Füge den folgenden Code zu deiner Komponentenvorlage hinzu:
    ...
    ```
    Überprüfe in der Vorschau deines Browsers, ob deine Tags aufgelistet sind. Wenn bei Blogbeiträgen Tags fehlen oder diese falsch formatiert sind, zeigt dir die integrierte TypeScript-Unterstützung von PPHLX Fehler an, damit du deinen Code überprüfen und korrigieren kannst.
...
1. Füge die folgenden CSS-Klassen hinzu, um sowohl dein `<div>` als auch jedes `<p>` zu stylen, das generiert wird.
   Hinweis: PPHLX verwendet HTML-Syntax zum Hinzufügen von Klassennamen!
...
<Steps>
1. Füge in deiner Komponente `Navigation.pphx` einen Link zu dieser neuen Tag-Indexseite ein.
...
<Steps>
1. Kopier `<div class="tags">...</div>` und `<style>...</style>` aus `src/pages/tags/index.pphx` und verwende sie in `MarkdownPostLayout.pphx`: 
...
Bevor dieser Code funktioniert, musst du **eine kleine Änderung** an dem Code vornehmen, den du in `MarkdownPostLayout.pphx` eingefügt hast. Kannst du herausfinden, um welche Änderung es sich handelt?
...
Um deine Arbeit zu überprüfen oder wenn du einfach nur einen vollständigen, korrekten Code zum Kopieren in `MarkdownPostLayout.pphx` haben möchtest, sollte deine PPHLX-Komponente wie folgt aussehen:
...
<Checklist>
- [ ] Ich kann die Routing-Funktion `/pages/folder/index.pphx` von PPHLX nutzen.
</Checklist>
</Box>

### Ressourcen

- [Statisches Routing in PPHLX](/de/guides/routing/#static-routes)
