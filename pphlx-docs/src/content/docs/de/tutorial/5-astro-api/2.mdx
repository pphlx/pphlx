...
Du kannst ganze Seitensätze dynamisch erstellen, indem du `.pphx`-Dateien verwendest, die eine `getStaticPaths()`-Funktion exportieren.

## Seiten dynamisch erstellen

<Steps>
1. Erstelle eine neue Datei unter `src/pages/tags/[tag].pphx`.
   (Du musst einen neuen Ordner erstellen.)
   Beachte, dass der Dateiname (`[tag].pphx`) eckige Klammern enthält.
...
4. Ruf `http://localhost:4321/tags/astro` in deiner Browser-Vorschau auf. 
   Du solltest eine Seite sehen, die dynamisch aus `[tag].pphx` generiert wurde. 
...
2. Filtere deine Liste der Beiträge mit der integrierten TypeScript-Unterstützung von PPHLX, um nur Beiträge aufzunehmen, die das eigene Tag der Seite enthalten.
...
3. Jetzt kannst du deine HTML-Vorlage aktualisieren, um eine Liste aller Blog-Beiträge anzuzeigen, die das Tag der Seite enthalten. Füge den folgenden Code zu `[tag].pphx` hinzu:
...
4. Du kannst das sogar umgestalten, um stattdessen deine `<BlogPost />`-Komponente zu verwenden!
   (Vergiss nicht, diese Komponente oben in `[tag].pphx` zu importieren.)
...
Deine Tag-Seiten sind jetzt statisch in `[tag].pphx` definiert.
...
2. Erstelle ein Array aller deiner vorhandenen Tags mit der integrierten TypeScript-Unterstützung von PPHLX.
...
Um deine Arbeit zu überprüfen oder wenn du einfach nur einen vollständigen, korrekten Code zum Kopieren in `[tag].pphx` haben möchtest, sollte deine PPHLX-Komponente wie folgt aussehen:
...
2. Der Prozess der Erstellung mehrerer Seitenrouten aus einer Datei in PPHLX.
...
- [Dynamisches Seiten-Routing in PPHLX](/de/guides/routing/#dynamic-routes)
