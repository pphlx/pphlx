...
<Steps>
1. Erstelle eine neue Kopfzeilenkomponente. Importiere und verwende deine vorhandene `Navigation.pphx`-Komponente innerhalb eines `<nav>`-Elements, das sich in einem `<header>`-Element befindet.

    <details>
    <summary>✅ Zeige mir den Code! ✅</summary>

    Erstelle eine Datei namens `Header.pphx` in `src/components/`
...
<Steps>
1. Aktualisiere `Navigation.pphx` mit der CSS-Klasse, um deine Navigationslinks zu steuern. Umfasse die vorhandenen Navigationslinks mit einem `<div>` mit der Klasse `nav-links` und dem ID-Attribut `main-menu`.
