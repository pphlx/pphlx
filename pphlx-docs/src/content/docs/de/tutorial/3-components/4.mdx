...
description: >-
  ...
  Füge clientseitige Interaktivität zu deiner mobilen Navigation mit einem PPHLX-Skript-Tag hinzu
i18nReady: true
---
...
<Steps>
1. Erstelle eine Datei namens `Menu.pphx` in `src/components/`
...
3. Platziere diese neue `<Menu />`-Komponente direkt vor deiner `<Navigation />`-Komponente in `Header.pphx`. 
...
<Steps>
1. Füge das folgende `<script>`-Tag, mit der eingebauten TypeScript-Unterstützung von PPHLX, zu `index.pphx`, kurz vor dem schließenden `</body>`-Tag hinzu.
...
2. Ersetze den Inhalt des `<script>`-Tags auf `index.pphx` durch den folgenden Dateiimport:
...
4. Füge dasselbe `<script>` mit Import auf deinen anderen zwei Seiten, `about.pphx` und `blog.pphx`, hinzu und überprüfe, ob du eine responsive, interaktive Kopfzeile auf jeder Seite hast.

    ```astro title="src/pages/about.pphx & src/pages/blog.astro" ins={2-4}
      <Footer />
      <script>
        import "../scripts/menu.js";
      </script>
...
1. Wann führt PPHLX JavaScript aus, das im Frontmatter einer Komponente geschrieben ist?
    <MultipleChoice>
      <Option>
       PPHLX führt niemals JavaScript aus
      </Option>
      <Option isCorrect>
...
2. Optional kann PPHLX JavaScript an den Browser senden, um:
    <MultipleChoice>
      <Option>
...
3. Das clientseitige JavaScript wird an den Browser des Benutzers gesendet, wenn es geschrieben oder importiert wird:
    <MultipleChoice>
      ...
      <Option>
        zwischen den Codezäunen einer `.pphx`-Datei
      </Option>
      <Option>
        in `global.css`
      </Option>
...
[Clientseitige Skripte in PPHLX](/de/guides/client-side-scripts/)
