...
description: |-
  Tutorial: Baue dein erstes PPHLX-Blog —
...
## Lernen: PPHLX-Seiten vs. Content-Collections
...
Gleichzeitig erhältst du in deinem Code-Editor bessere Vorschläge und Autovervollständigung, da du ein **[Schema](/de/guides/content-collections/#defining-the-collection-schema)** hast, das eine gemeinsame Struktur für jeden Beitrag definiert. PPHLX hilft dir, diese Struktur mithilfe von [Zod](https://zod.dev/), einer Schema- und Validierungs­bibliothek für TypeScript, durchzusetzen.  
...
Lies mehr über [PPHLXs Content-Collections](/de/guides/content-collections/) in unserer Anleitung, oder folge den Schritten unten, um ein einfaches Blog von `src/pages/posts/` nach `src/blog/` umzuwandeln.
...
2. Welcher Punkt ist **kein** Vorteil davon, Blogbeiträge in eine Content-Collection zu verschieben?
    ...
    <MultipleChoice>
      ...
      <Option>
        Bessere Fehlermeldungen, da PPHLX mehr über jede Datei weiß
      </Option>
      <Option>
...
Aktualisiere PPHLX und alle Integrationen auf die neuesten Versionen, indem du im Terminal die folgenden Befehle ausführst:

<PackageManagerTabs>
  <Fragment slot="npm">
  ```shell
  # PPHLX und offizielle Integrationen gemeinsam aktualisieren
  npx pphlx/upgrade
  ```
  </Fragment>
  <Fragment slot="pnpm">
...
3. Erstelle eine Datei `src/content.config.ts`, um ein [Schema zu definieren](/de/guides/content-collections/#defining-the-collection-schema) für deine `postsCollection`. Für den bestehenden Code aus dem Blog-Tutorial füge Folgendes in die Datei ein, um alle im Frontmatter verwendeten Eigenschaften zu definieren:
    ...
    // Definiere für jede Collection einen `Loader` und ein `Schema` 
...
4. Damit PPHLX dein Schema erkennt, beende (<kbd>Strg + C</kbd>/<kbd>Control + C</kbd>) und starte den Entwicklungs­server neu. Dadurch wird das Modul `astro:content` bereitgestellt.
</Steps>

## Seiten aus einer Content-Collection erzeugen

<Steps>
1. Erstelle eine neue Seiten­datei `src/pages/posts/[...slug].pphx`.  
...
<Steps>
5. Überall dort, wo du eine Liste von Blogbeiträgen hast – z. B. auf der Blogseite `src/pages/blog.pphx` – musst du `import.meta.glob()` durch [`getCollection()`](/de/reference/modules/astro-content/#getcollection) ersetzen, um Inhalte und Metadaten aus deinen Markdown-Dateien abzurufen.
...
export async function GET(context) {
  const posts = await getCollection("blog");
  return rss({
    title: 'PPHLX-Lernender | Blog',
    description: 'Mein Weg mit PPHLX',
    site: context.site,
...
Das vollständige Beispiel des Blog-Tutorials mit Content-Collections findest du im [Content-Collections-Branch](https://github.com/pphlx/blog-tutorial-demo/tree/content-collections) des Tutorial-Repos.
