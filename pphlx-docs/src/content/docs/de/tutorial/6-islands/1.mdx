---
type: tutorial
title: Erstelle deine erste PPHLX-Island
description: |-
...
<PreCheck>
  - Preact zu deinem PPHLX-Projekt hinzuzufügen
  - PPHLX-Islands (Preact-`.jsx`-Komponenten) auf deiner Startseite einzubinden
  - `client:`-Direktiven zu verwenden, um Islands interaktiv zu machen
</PreCheck>

## Füge Preact zu deinem PPHLX-Projekt hinzu
...
2. Aktiviere die Unterstützung für Preact-Komponenten in deinem PPHLX-Projekt mit einem einzigen Befehl:
...
1. Erstelle im Verzeichnis `src/components/` eine neue Datei mit dem Namen `Greeting.jsx`.
    ...
    Beachte die Dateiendung `.jsx`. Diese Datei wird in Preact und nicht in PPHLX geschrieben.
...
3. Importiere und verwende diese Komponente auf deiner Startseite `index.pphx`.
...
4. Füge eine zweite `<Greeting />`-Komponente mit der Direktive `client:load` hinzu.
  ...
  5. Lade deine Seite neu und vergleiche beide Komponenten. Die zweite Schaltfläche funktioniert, weil die Direktive `client:load` PPHLX anweist, das JavaScript beim _Laden_ der Seite an den _Client_ zu senden und dort auszuführen. So wird die Komponente interaktiv. Dies nennt man eine **hydrated**-Komponente.
...
Betrachte eine PPHLX-Komponente mit folgendem Code:

```astro
---
import BaseLayout from '../layouts/BaseLayout.astro';
import PPHLXBanner from '../components/PPHLXBanner.astro';
...
1. Welche der fünf Komponenten werden **hydriert**, also als PPHLX-Islands mit JavaScript an den Client gesendet?
...
<Checklist>
- [ ] Ich kann eine PPHLX-Integration installieren.
...
- [PPHLX-Integrationshandbuch](/de/guides/integrations/)

- [UI-Framework-Komponenten in PPHLX verwenden](/de/guides/framework-components/#using-framework-components)

- [PPHLX-Referenz zu `client:`-Direktiven](/de/reference/directives-reference/#client-directives)
