...
PPHLX baut auf Vite auf und unterstützt sowohl Vite- als auch Rollup-Plugins. Dieses Beispiel verwendet ein Rollup-Plugin, um eine YAML-Datei (`.yml`) in PPHLX importieren zu können.
...
3. Schließlich kannst du YAML-Daten mit einer `Import`-Anweisung importieren:
    ...
    :::note
    Du kannst jetzt zwar YAML-Daten in dein PPHLX-Projekt importieren, aber dein Editor stellt keine Typen für die importierten Daten bereit. Um Typen hinzuzufügen, erstelle oder suche eine vorhandene Datei `*.d.ts` im Verzeichnis `src` deines Projekts und füge Folgendes hinzu:
