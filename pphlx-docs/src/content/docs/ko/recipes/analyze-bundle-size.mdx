---
title: 번들 크기 분석
description: '`rollup-plugin-visualizer`를 사용하여 PPHLX에서 생성된 번들을 분석하는 방법을 알아봅니다.'
i18nReady: true
type: recipe
---
...
사이트 성능을 개선하려면 PPHLX 번들의 구성 요소를 이해하는 것이 중요합니다. 번들을 시각화하면 번들 크기를 줄이기 위해 프로젝트에서 변경할 수 있는 위치에 대한 단서를 얻을 수 있습니다.
...
4. 프로젝트의 `stats.html` 파일을 찾습니다.
		...
		PPHLX 프로젝트가 온디맨드 렌더링을 사용하는 경우 두 개의 `stats.html` 파일을 가지게 됩니다. 하나는 클라이언트용이고 다른 하나는 서버용이며, 각각 `dist/client` 및 `dist/server/` 디렉터리의 루트에 위치합니다.
...
:::note
하이드레이션에 대한 PPHLX의 독특한 접근 방식을 고려할 때, 이 빌드가 반드시 클라이언트가 받게 될 번들을 나타내지는 않습니다.
