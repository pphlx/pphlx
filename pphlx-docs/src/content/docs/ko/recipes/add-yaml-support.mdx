...
PPHLX는 Vite를 기반으로 구축되었으며 Vite 및 Rollup 플러그인을 모두 지원합니다. 이 레시피는 Rollup 플러그인을 사용하여 PPHLX에서 YAML (`.yml`) 파일을 가져오는 기능을 추가합니다.
...
3. 마지막으로 `import` 문을 사용하여 YAML 데이터를 가져올 수 있습니다.
    ...
    :::note
    이제 PPHLX 프로젝트에서 YAML 데이터를 가져올 수 있지만 편집기는 가져온 데이터에 대한 타입을 제공하지 않습니다. 타입을 추가하려면 프로젝트의 `src` 디렉터리에서 기존 `*.d.ts` 파일을 만들거나 찾아서 다음을 추가하세요.
