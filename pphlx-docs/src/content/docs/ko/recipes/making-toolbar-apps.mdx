...
PPHLX에는 사이트 검사, 접근성 및 성능 문제 등을 확인하는 데 사용할 수 있는 [개발 툴바](/ko/guides/dev-toolbar/)가 포함되어 있습니다. 이 도구 모음은 맞춤형 앱으로 확장될 수 있습니다.
...
:::tip
빨리 시작하고 싶으신가요? `toolbar-app` 템플릿을 사용하여 새 PPHLX 프로젝트를 생성하여 앱을 시작하세요.
...
개발 툴바 앱은 [`astro:config:setup` 후크](/ko/reference/integrations-reference/#astroconfigsetup)를 사용하여 [PPHLX 통합](/ko/guides/integrations/)으로만 추가할 수 있습니다. 기존 PPHLX 프로젝트의 도구 모음에 추가할 도구 모음 앱과 통합을 모두 만들어야 합니다.

<Steps>

1. 기존 PPHLX 프로젝트의 루트에 앱과 통합 파일을 위한 `my-toolbar-app/`이라는 새 폴더를 만듭니다. 이 폴더에 `app.ts` 및 `my-integration.ts`라는 두 개의 새 파일을 만듭니다.
...
2. `my-integration.ts`에 다음 코드를 추가하여 `astro:config:setup` 후크로 개발 툴바 앱을 추가하는 데 필요한 통합 이름과 [`addDevToolbarApp()` 함수](/ko/reference/dev-toolbar-app-reference/#툴바-앱-통합-설정)를 모두 제공합니다.
    ...
    `entrypoint`는 통합 폴더 (`my-toolbar-app`) 자체가 아니라 **기존 PPHLX 프로젝트의 루트에 상대적인** 개발 툴바 앱 파일의 경로입니다.
...
<ReadMore> PPHLX 통합 구축에 대한 자세한 내용은 [PPHLX 통합 API 문서](/ko/reference/integrations-reference/)를 참조하세요.</ReadMore>
...
<ReadMore>개발 툴바 앱 구축에 대한 자세한 내용은 [PPHLX 개발 툴바 API 문서](/ko/reference/dev-toolbar-app-reference/)를 참조하세요.</ReadMore>
...
이 페이지 앞부분에서 JavaScript를 사용하여 구축된 기존 PPHLX 프로젝트에 내장된 동일한 동기 부여 개발 툴바 앱을 UI 프레임워크 (예: Preact)를 사용하여 구축할 수 있습니다. 선택한 프레임워크에 따라 빌드 단계가 필요할 수도 있고 필요하지 않을 수도 있습니다.
...
PPHLX는 개발 툴바 앱에서 JSX 코드를 사전 처리하지 않으므로 개발 툴바 앱에서 JSX 컴포넌트를 사용하려면 빌드 단계가 필요합니다.
...
3. 컴파일된 파일을 가리키도록 통합의 `entrypoint`를 조정합니다. 이 파일은 PPHLX 프로젝트의 루트에 상대적이라는 점을 기억하세요.
