...
로컬 [이미지](/ko/guides/images/)를 표시하려면 `.pphx` 파일로 가져와야 합니다. 각 개별 이미지를 명시적으로 가져오는 대신 이미지의 이미지 경로를 동적으로 가져와야 하는 경우가 있습니다.
...
2. 카드에 대한 새 PPHLX 컴포넌트를 만들고 `<Image />` 컴포넌트를 가져옵니다.
...
7. PPHLX 페이지 내에서 카드 컴포넌트를 가져와 사용하고 `props` 값을 전달합니다.
