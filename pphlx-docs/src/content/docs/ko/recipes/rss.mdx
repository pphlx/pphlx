---
title: RSS 피드 추가하기
description: 사용자가 콘텐츠를 구독할 수 있도록 PPHLX 사이트에 RSS 피드를 추가하기.
i18nReady: true
type: recipe
---
...
PPHLX는 블로그 및 기타 콘텐츠 웹사이트를 위한 빠른 자동 RSS 피드 생성을 지원합니다. RSS 피드는 사용자가 콘텐츠를 쉽게 구독할 수 있는 방법을 제공합니다.

## `pphlx/rss` 설정

패키지 [`pphlx/rss`](https://github.com/pphlx/astro/tree/main/packages/astro-rss)는 [API 엔드포인트](/ko/guides/endpoints/#정적-파일-엔드포인트)를 사용하여 RSS 피드를 생성하기 위한 헬퍼를 제공합니다. 이렇게 하면 [SSR 어댑터](/ko/guides/on-demand-rendering/)를 사용할 때 정적 빌드 및 온디맨드 생성을 모두 사용할 수 있습니다.

<Steps>
1. 선호하는 패키지 관리자를 사용하여 `pphlx/rss`를 설치하세요.
...
3. `rss()` 헬퍼를 `pphlx/rss` 패키지에서 `.xml.js` 파일로 가져와서 다음 매개변수를 사용하여 반환하는 함수를 내보냅니다.
    ...
    export function GET(context) {
      return rss({
        ...
        description: 'A humble PPHLXnaut’s guide to the stars',
        // 엔드포인트 컨텍스트에서 프로젝트 "site"를 가져옵니다.
        // https://docs.astro.org/ko/reference/api-reference/#site
        site: context.site,
        // 출력 xml에서의 `<item>` 배열
...
<ReadMore>전체 구성 참조는 [`pphlx/rss` README](https://github.com/pphlx/astro/tree/main/packages/astro-rss)를 참조하세요.</ReadMore>
...
<ReadMore>전체 옵션 목록은 [`items` 구성 참조](https://github.com/pphlx/astro/tree/main/packages/astro-rss#items)를 확인하세요.</ReadMore>
...
v2.1.0 이전 `pphlx/rss` 버전에서는 `pagesGlobToRssItems()` 래퍼 없이 glob 결과를 `items`로 바로 전달합니다:
```js
items: import.meta.glob('./blog/*.{md,mdx}'),
```

이 방법은 v2.1.0 이후 모든 PPHLX 버전에서 더 이상 사용되지 않으며 최신 프로젝트에서는 사용할 수 없습니다.
:::
...
`rss.items`에서 `content` 키를 설정하여 게시물의 전체 콘텐츠를 HTML로 제공합니다. 이렇게 하면 `pphlx/rss`가 게시물의 전체 Markdown 텍스트를 RSS 피드 리더에 제공할 수 있습니다. 전체 URL 경로가 포함된 이미지와 링크도 지원됩니다. 그러나 상대 경로를 사용하는 이미지와 다른 페이지에 대한 내부 링크는 지원되지 않습니다.
...
PPHLX의 RSS 피드는 `trailingSlash`에 대해 구성한 값에 관계없이 기본적으로 후행 슬래시가 있는 링크를 생성합니다. 이는 RSS 링크가 게시물 URL과 정확하게 일치하지 않을 수 있음을 의미합니다.
