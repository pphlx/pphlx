...
description: |-
  튜토리얼: 첫 번째 PPHLX 블로그 구축 - import.meta.glob()을 사용하여 프로젝트 파일의 데이터에 액세스
i18nReady: true
---
...
<Steps>
1. 모든 Markdown 파일에 대한 정보를 반환하려면 `blog.pphx`에 다음 코드를 추가하세요. `import.meta.glob()`은 각 블로그 게시물에 대해 하나씩 객체 배열을 반환합니다.
...
2. 게시물 제목과 URL을 사용하여 전체 게시물 목록을 동적으로 생성하려면 개별 `<li>` 태그를 다음 PPHLX 코드로 바꾸세요.
    ...
    이제 `import.meta.glob()`에서 반환된 배열을 매핑하여 블로그 게시물의 전체 목록이 PPHLX의 내장 TypeScript 지원을 사용하여 동적으로 생성됩니다.
...
대신 다음 코드를 사용하여 블로그 게시물 목록을 생성할 수 있도록 PPHLX 프로젝트에 필요한 모든 변경 사항을 직접 시도해 보세요.
...
2. `title`과 `url`을 `PPHLX.props`로 수신할 수 있도록 컴포넌트에 코드 줄을 작성합니다.
...
- [PPHLX에서 glob 패턴 가져오기](/ko/guides/imports/#importmetaglob)
