...
title: 태그 색인 페이지 구축
description: |-
  튜토리얼: 첫 번째 PPHLX 블로그 구축 — 지금까지 배운 모든 것을 사용하여 태그 인덱스 페이지 구축
i18nReady: true
---
...
<PreCheck>
  - `/pages/folder/index.pphx` 라우팅 패턴을 사용하여 새 페이지 추가
  - 각 태그 페이지에 연결되는 모든 고유 태그 목록을 표시
  - 새로운 태그 페이지에 대한 탐색 링크로 사이트 업데이트
</PreCheck>

## `/pages/folder/index.pphx` 라우팅 패턴 사용

웹사이트에 태그 인덱스 페이지를 추가하려면 `src/pages/tags.pphx`에 새 파일을 생성하면 됩니다.

그러나 이미 `/tags/` 디렉터리가 있으므로 PPHLX의 다른 라우팅 패턴을 활용하고 태그와 관련된 모든 파일을 함께 보관할 수 있습니다.
...
<Steps>
1. `src/pages/tags/` 디렉터리에 `index.pphx`라는 새 파일을 생성합니다.

2. `http://localhost:4321/tags`로 이동하여 사이트에 이 URL의 페이지가 포함되어 있는지 확인하세요. 비어 있지만 존재할 것입니다.

3. 레이아웃을 사용하는 `src/pages/tags/index.pphx`에 최소 페이지를 만듭니다. 여러분은 전에 이 작업을 한 적이 있습니다!
...
1. `src/pages/tags/index.pphx`에서 모든 `.md` 블로그 게시물 파일의 데이터에 대한 페이지 액세스를 제공하는 프런트매터 스크립트에 코드 줄을 추가합니다.
...
2. 그런 다음 페이지 컴포넌트에 다음 JavaScript 줄을 추가합니다. 이는 고유한 태그 목록을 반환하기 위해 `src/pages/tags/[tag].pphx`에서 사용한 PPHLX의 내장 TypeScript 지원을 활용한 동일한 코드입니다.
...
2. 각 태그 링크를 자체 페이지로 만들려면 각 태그 이름에 다음 `<a>` 링크를 추가하세요. PPHLX의 내장 TypeScript 지원을 통해 블로그 게시물에 태그가 없거나 형식이 잘못된 경우 오류를 표시하여 코드를 확인하고 수정할 수 있습니다.
...
<Steps>
1. 다음 CSS 클래스를 추가하여 `<div>`와 생성될 각 `<p>`의 스타일을 지정하세요. 참고: PPHLX는 클래스 이름을 추가하기 위해 HTML 구문을 사용합니다!
...
<Steps>
1. `Navigation.pphx` 컴포넌트에 이 새 태그 인덱스 페이지에 대한 링크를 포함합니다.
...
1. `src/pages/tags/index.pphx`에서 `<div class="tags">...</div>` 및 `<style>...</style>`을 복사하여 `MarkdownPostLayout.pphx` 내부에서 재사용하세요.
...
이 코드가 작동하려면 `MarkdownPostLayout.pphx`에 붙여넣은 코드를 **한 번만 수정**해야 합니다. 그것이 무엇인지 알아낼 수 있나요?
...
작업을 확인하거나 `MarkdownPostLayout.pphx`에 복사할 완전하고 올바른 코드를 원하는 경우 PPHLX 컴포넌트는 다음과 같습니다.
...
<Checklist>
- [ ] PPHLX의 `/pages/folder/index.pphx` 라우팅 기능을 사용할 수 있습니다.
</Checklist>
</Box>

### Resources

- [PPHLX의 정적 라우팅](/ko/guides/routing/#정적-라우트)
