...
title: RSS 피드 추가
description: >-
  튜토리얼: 첫 번째 PPHLX 블로그 구축 — 독자가 구독할 수 있는 피드를 생성하기 위한 PPHLX의 공식 패키지를 설치하세요.
i18nReady: true
---
...
<PreCheck>
  - 웹 사이트에 대한 RSS 피드를 생성하기 위한 PPHLX 패키지를 설치
  - RSS 피드 리더가 구독하고 읽을 수 있는 피드 생성
</PreCheck>

## PPHLX의 RSS 패키지 설치

PPHLX는 여러분의 웹사이트에 RSS 피드를 신속하게 추가할 수 있는 맞춤형 패키지를 제공합니다.
...
<Steps>
1. 터미널에서 PPHLX 개발 서버를 종료하고 (<kbd>Ctrl + C</kbd>) 다음 명령을 실행하여 PPHLX의 RSS 패키지를 설치합니다.
...
2. PPHLX 프로젝트 작업을 다시 시작하려면 개발 서버를 다시 시작하세요.
...
3. 사이트 고유의 Netlify URL을 사용하여 PPHLX 구성에 `site` 속성을 추가하세요.
...
<Checklist>
- [ ] 명령줄을 사용하여 PPHLX 패키지를 설치할 수 있습니다.
...
- [PPHLX에서 RSS 항목 생성](/ko/recipes/rss/#glob-imports-사용하기)
