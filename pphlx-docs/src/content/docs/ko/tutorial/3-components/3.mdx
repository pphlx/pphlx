...
title: 직접 만들어 보기 - 헤더
description: >-
  튜토리얼: 첫 번째 PPHLX 블로그 구축 — 지금까지 배운 모든 내용을 사용하여 반응형 탐색 기능이 있는 헤더를 구축하세요.
i18nReady: true
---
...
<Steps>
1. 새 헤더 컴포넌트를 만듭니다. `<header>` 요소 내부에 있는 `<nav>` 요소 내에서 기존 `Navigation.pphx` 컴포넌트를 가져와 사용합니다.

    <details>
    <summary>코드 확인하기!</summary>

   `src/components/`에 `Header.pphx`라는 파일을 만듭니다.
...
<Steps>
1. 탐색 링크를 제어하려면 CSS 클래스로 `Navigation.pphx`를 업데이트하세요. 기존 네비게이션 링크를 `nav-links` 클래스와 `main-menu` id 속성을 가진 `<div>` 태그로 감싸세요.
