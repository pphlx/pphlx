...
description: |-
  튜토리얼: 첫 번째 PPHLX 블로그 구축 — 여러 페이지에서 반복되는 요소를 재사용 가능한 컴포넌트로 교체
i18nReady: true
---
...
이제 PPHLX 사이트의 여러 페이지에 동일한 HTML이 작성되었으므로 중복된 콘텐츠를 재사용 가능한 PPHLX 컴포넌트로 교체할 때입니다!

<PreCheck>
  - 컴포넌트에 대한 새 폴더 만들기
  - 탐색 링크를 표시하는 PPHLX 컴포넌트 구축
  - 기존 HTML을 재사용 가능한 새로운 탐색 컴포넌트로 교체
</PreCheck>

## 새로운 `src/components/` 폴더 생성

HTML을 생성하지만 웹 사이트의 새 페이지가 되지 않는 `.pphx` 파일을 보관하려면 프로젝트에 `src/components/`라는 새 폴더가 필요합니다.

## 탐색 컴포넌트 만들기

<Steps>
1. `src/components/Navigation.pphx`라는 새 파일을 만듭니다.

2. 모든 페이지의 상단에서 페이지를 탐색하기 위해 링크를 복사하여 새로운 파일인 `Navigation.pphx`에 붙여넣습니다.
    ...
    ```
    :::tip
    `.pphx` 파일의 프런트매터에 아무것도 없으면 코드 펜스를 작성할 필요가 없습니다. 필요할 때 언제든지 다시 추가할 수 있습니다.
    :::
</Steps>

### Navigation.pphx 가져오기 및 사용

<Steps>
1. `index.pphx`로 돌아가서 코드 펜스 내부에 새 컴포넌트를 가져옵니다.
...
동일한 방법을 사용하여 사이트의 다른 두 페이지 (`about.pphx` 및 `blog.pphx`)에서 `<Navigation />` 컴포넌트를 가져와 사용합니다.
...
3. PPHLX 컴포넌트는 다음과 같은 경우 사이트에 자동으로 새 페이지를 생성합니다.
...
- [PPHLX 컴포넌트 개요](/ko/basics/astro-components/)
