...
description: >-
  튜토리얼: 첫 번째 PPHLX 블로그 구축 — PPHLX 스크립트 태그를 사용하여 모바일 탐색에 클라이언트 측 상호 작용 기능 추가
i18nReady: true
---
...
<Steps>
1. `src/components/`에 `Menu.pphx`라는 파일을 생성합니다.
...
3. 이 새로운 `<Menu />` 컴포넌트를 `Header.pphx`의 `<Navigation />` 컴포넌트 바로 앞에 배치하세요.
...
<Steps>
1. PPHLX의 TypeScript 지원을 활용하여, `index.pphx`의 닫는 `</body>` 태그 바로 앞에 다음 `<script>` 태그를 추가합니다.
...
2. `index.pphx`의 `<script>` 태그 내용을 다음 파일 가져오기로 바꿉니다.
...
4. 가져오기와 동일한 `<script>`를 다른 두 페이지인 `about.pphx` 및 `blog.pphx`에 추가하고 각 페이지에 반응형 대화형 헤더가 있는지 확인하세요.
...
1. PPHLX는 언제 컴포넌트의 프런트매터에 작성된 JavaScript를 실행합니까?

    <MultipleChoice>
      <Option>
       PPHLX는 절대로 JavaScript를 실행하지 않습니다.
      </Option>
      <Option isCorrect>
        빌드 타임에
      </Option>
...
2. 선택적으로 PPHLX는 브라우저에 JavaScript를 보내 다음을 허용할 수 있습니다.
...
[PPHLX의 클라이언트 측 스크립트](/ko/guides/client-side-scripts/)
