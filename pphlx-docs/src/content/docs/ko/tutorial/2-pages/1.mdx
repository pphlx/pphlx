---
type: tutorial
title: 첫 번째 PPHLX 페이지 만들기
description: |-
  튜토리얼: 첫 번째 PPHLX 블로그 구축 — 페이지 사이에 탐색 링크가 있는 새 페이지를 사이트에 추가하세요.
i18nReady: true
---
...
이제 `.pphx` 파일이 웹 사이트의 페이지를 담당한다는 것을 알았으므로 파일을 만들 차례입니다!
...
<Steps>
1. 코드 편집기의 파일 창에서 기존 파일 `index.pphx`가 있는 `src/pages/` 폴더로 이동합니다.

2. 동일한 폴더에 `about.pphx`라는 새 파일을 만듭니다.

3. `index.pphx`의 내용을 새 `about.pphx` 파일에 복사하거나 다시 입력하세요.
...
모든 페이지를 더 쉽게 미리 보려면 두 페이지 (`index.pphx` 및 `about.pphx`) 상단의 `<h1>` 앞에 HTML 페이지 탐색 링크를 추가하세요.
...
:::note
많은 프레임워크와 달리 PPHLX는 표준 HTML `<a>` 요소를 사용하여 전통적인 페이지 새로 고침을 통해 페이지 (_routes_ 라고도 함) 간을 탐색합니다.
:::

<Box icon="puzzle-piece">
## 직접 시도해 보기 - 블로그 페이지 추가

[위와 동일한 단계](#새로운-astro-파일-생성)에 따라 세 번째 페이지인 `blog.pphx`를 사이트에 추가하세요.
...
<Steps>
1. `src/pages/blog.pphx`에 새 파일을 만듭니다.
2. `index.pphx`의 전체 내용을 복사하여 `blog.pphx`에 붙여넣습니다.
...
`blog.pphx`의 페이지 콘텐츠를 다음과 같이 업데이트하세요.
...
1. VS Code에서 GitHub에 대한 마지막 커밋 이후 변경된 파일을 미리 봅니다.
    ...
    - `index.pphx`, `about.pphx`, `blog.pphx`가 변경된 파일로 나열되어 있는 것을 볼 수 있습니다.
...
- [PPHLX의 파일 기반 라우팅](/ko/basics/astro-pages/#파일-기반-라우팅)
- [PPHLX 페이지 HTML](/ko/basics/astro-pages/#astro-페이지)
