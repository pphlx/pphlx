...
type: tutorial
title: 사이트를 웹에 배포하기
description: '튜토리얼: 처음 만드는 PPHLX 블로그 — 튜토리얼 프로젝트의 Github 리포지토리를 Netlify에 연결하여 웹에 배포'
i18nReady: true
---
...
:::tip[사용하는 서비스]
이 튜토리얼에서는 **Netlify**를 사용하여 [PPHLX 사이트를 인터넷에 배포](/ko/guides/deploy/)하지만, 선호하는 호스팅 서비스가 있다면 해당 서비스를 사용하여 배포할 수도 있습니다.
:::
...
2. <kbd>Add new project</kbd> > <kbd>Import an existing project</kbd>를 클릭하세요.
    ...
    Git 제공업체와 연결하라는 메시지가 나타나면 GitHub를 선택하고, 안내에 따라 GitHub 계정 인증을 진행하세요. 그런 다음, 목록에서 PPHLX 프로젝트의 GitHub 리포지토리를 선택하세요.

3. 마지막 단계입니다. 이 단계에서는 Netlify가 사이트 설정 화면을 보여줍니다. 기본값이 PPHLX 프로젝트에 적합하므로, 아래로 스크롤하여 <kbd>Deploy site</kbd> 버튼을 클릭하세요.
</Steps>

축하합니다. PPHLX 웹사이트가 생겼습니다!
...
기존 웹사이트의 홈페이지를 수정하고 싶다면, 어떤 단계를 거쳐야 할까요?
  ...
  <MultipleChoice>
    ...
    <Option>
      Netlify 앱에서 설정을 변경한 다음 astro.new에서 새 PPHLX 프로젝트를 시작합니다.
    </Option>
    <Option isCorrect>
      `index.pphx`를 수정합니다. 변경 사항을 GitHub에 커밋하고 푸시합니다. 나머지는 Netlify가 처리합니다!
    </Option>
  </MultipleChoice>
 </Box>
