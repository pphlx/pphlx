...
PPHLX는 서버 측 렌더링(SSR)으로 알려진 요청 시 렌더링을 위해 모든 클라우드 제공업체에 쉽게 배포할 수 있도록 설계되었습니다. 이 기능은 [통합](/ko/reference/integrations-reference/)으로 제공되는 __어댑터__ 를 통해 사용할 수 있습니다. 기존 어댑터를 사용하는 방법을 알아보려면 [요청 시 렌더링 가이드](/ko/guides/on-demand-rendering/)를 참조하세요.
...
다음은 PPHLX 정적 출력을 안정적으로 지원하며 서버 엔트리포인트를 포함하는 어댑터를 생성하는 예시입니다.
...
[마이그레이션 방법](/ko/guides/upgrade-to/v6/#지원-중단됨-createexports-및-start-어댑터-api)을 참조하세요.
:::

PPHLX가 서버 엔트리포인트를 확인하는 방법을 지정합니다: `"auto"` (권장) 또는 `"explicit"` (기본값이지만 더 이상 사용되지 않음):
...
- **`"explicit"` (더 이상 사용되지 않음)**: 호스트가 요구하는 내보내기를 `createExports()` 함수를 사용하여 서버 엔트리포인트에 제공한 다음, 이를 [`exports`](#exports) 목록으로 `setAdapter()`에 전달해야 합니다. 이는 PPHLX 5 버전의 어댑터 API를 사용하여 빌드된 어댑터를 지원합니다. 기본적으로 모든 어댑터는 하위 호환성을 위해 이 값을 받게 됩니다. **그러나 이 값을 사용하여 새로운 어댑터를 만들어서는 안 됩니다.** 기존 어댑터는 새 v6 API로 마이그레이션할 수 있는 즉시 이 기본값을 `"auto"`로 재정의해야 합니다.

다음 예시는 `entrypointResolution`과 `serverEntrypoint`를 정의하여 PPHLX에 사용자 정의 엔트리포인트가 제공되었음을 알립니다.
...
다음 예시는 `entrypointResolution`과 Rollup 옵션을 정의하여 PPHLX에 사용자 정의 엔트리포인트가 제공되었음을 알립니다.
...
**타입:** `PPHLXAdapterFeatureMap`<br />
<Since v="3.0.0" />
</p>

어댑터가 지원하는 PPHLX 내장 기능에 대한 맵입니다. 이를 통해 PPHLX는 어댑터가 어떤 기능을 지원하는지 파악하여 적절한 오류 메시지를 제공할 수 있습니다.

<ReadMore>어댑터가 구성할 수 있는 [사용 가능한 PPHLX의 기능](#astro-기능)을 알아보세요.</ReadMore>

### `adapterFeatures`

<p>

**타입:** `PPHLXAdapterFeatures`<br />
<Since v="3.0.0" />
</p>
...
PPHLX의 클라이언트 측 코드를 위한 구성 객체입니다.
...
PPHLX의 내부 fetch 호출(예: 액션, 뷰 전환, 서버 아일랜드, 프리페치)에 주입할 헤더를 정의합니다. 헤더 객체 또는 헤더를 반환하는 함수일 수 있습니다.
...
:::caution[더 이상 사용되지 않음]
이 속성은 더 이상 사용되지 않으며 향후 주요 버전에서 제거될 예정입니다. [어댑터를 `entrypointResolution: "auto"`로 업데이트하는 방법](/ko/guides/upgrade-to/v6/#지원-중단됨-createexports-및-start-어댑터-api)을 알아보세요.
:::

런타임에 어댑터의 서버 엔트리포인트로 전달될 JSON 직렬화 가능한 값입니다. 이는 빌드 타임 구성(예: 경로, 비밀)이 포함된 객체를 서버 런타임 코드로 전달하는 데 유용합니다.

다음 예시는 PPHLX에서 생성된 자산이 어디에 위치하는지 식별하는 속성을 가진 `args` 객체를 정의합니다.
...
특정 호스트에서 요청 시 렌더링을 활성화하려면 서버 측 요청 중에 실행되는 파일을 생성해야 합니다. PPHLX의 어댑터 API는 모든 유형의 호스트와 작동하도록 시도하며, 호스트 API에 맞게 유연하게 구성할 수 있는 방법을 제공합니다.
...
서버 엔트리포인트에서 빌드 타임 구성에 접근해야 하는 경우, 가상 모듈을 통해 전달할 수 있습니다. 예를 들어, 서버가 PPHLX에 의해 생성된 자산이 어디에 위치하는지 식별해야 할 수 있습니다.
...
PPHLX 기능은 어댑터가 특정 기능을 지원할 수 있는지 여부와 지원 수준을 PPHLX에 알리는 방법입니다.

이러한 속성을 사용할 때 PPHLX는 다음을 수행합니다.
- 특정 유효성 검사를 실행합니다.
- 로그에 컨텍스트 정보를 내보냅니다.

이러한 작업은 지원되거나 지원되지 않는 기능, 지원 수준, [원하는 로깅 양](#adaptersupportwithmessagesuppress), 사용자의 자체 구성을 기반으로 실행됩니다.

다음 구성은 이 어댑터가 Sharp 기반 내장 이미지 서비스에 대한 실험적 지원을 제공한다고 PPHLX에 알립니다.
...
Sharp 이미지 서비스를 사용하는 경우, PPHLX는 어댑터의 지원에 따라 터미널에 경고 및 오류 메시지를 기록합니다.
...
`classic` 모드는 PPHLX의 기본 동작과 일치합니다. 사전 렌더링된 페이지에서는 빌드 시점에 미들웨어가 실행되며, 페이지가 요청될 때는 미들웨어가 다시 실행되지 않습니다. 동적 페이지에서는 요청 시점에만 미들웨어가 실행됩니다. 미들웨어 코드는 서버 번들의 일부입니다.
...
어댑터가 정적 페이지에 대한 응답 헤더 설정을 지원하는지 여부입니다. 이 기능이 활성화되면 PPHLX는 정적 페이지에서 발생한 `Headers`의 맵을 반환합니다. 이 맵은 [`astro:build:generated` 훅](/ko/reference/integrations-reference/#astrobuildgenerated)에서 `routeToHeaders`로 제공되며, HTTP 헤더를 제어하는 플랫폼별 출력을 생성하는 데 사용될 수 있습니다. 예를 들어, 지원되는 플랫폼을 위한 `_headers` 파일을 생성할 수 있습니다.
...
이 옵션은 빌드 출력 유형과 관계없이 일관된 `dist/client/` 및 `dist/server/` 구조를 요구하는 어댑터에 유용합니다. 예를 들어, 정적 사이트라도 서버 엔트리가 필요한 서버 아일랜드나 이미지 엔드포인트를 포함할 수 있는 경우에 적합합니다.
...
PPHLX의 기본 로깅 메시지가 필요하지 않거나 [사용자 정의 `message`](#adaptersupportwithmessagemessage)와 함께 사용할 때 유저에게 혼란을 줄 수 있는 경우, `suppress: "default"`를 사용하여 기본 메시지를 제거하고 사용자 정의 메시지만 로깅할 수 있습니다.
...
**타입:** [`PPHLXIntegrationLogger`](/ko/reference/integrations-reference/#astrointegrationlogger)
</p>

프리뷰 서버가 실행 중일 때 로그를 기록하는 데 사용할 수 있는 PPHLX 로거 인스턴스를 정의합니다.
