...
`handler()`는 사용자 입력을 첫 번째 인수로 받아 호출됩니다. [`input`](#input-유효성-검사기) 유효성 검사기가 설정되어 있으면 사용자 입력은 핸들러로 전달되기 전에 유효성이 검사됩니다. 두 번째 인수는 [PPHLX `context` 객체의 하위 집합](#actionapicontext)입니다.
...
PPHLX가 이 속성을 인식하게 하려면 개발 서버를 다시 시작하거나 [`astro sync` 명령을 실행](/ko/reference/cli-reference/#astro-sync)(`s + enter`)해야 할 수 있습니다. 
...
**타입:** <code>(context: <a href="/ko/reference/api-reference/">APIContext</a>) => PPHLXActionContext</code><br />
<Since v="5.0.0" />
</p>

미들웨어 핸들러에서 호출되어, 들어오는 액션 요청에 대한 정보를 검색하는 함수입니다. 이 함수는 요청에 대한 정보가 담긴 `action` 객체, `deserializeActionResult()` 메서드, 그리고 `PPHLX.getActionResult()`가 반환하는 값을 프로그래밍 방식으로 설정하기 위한 `setActionResult()` 및 `serializeActionResult()` 함수를 반환합니다.
...
미들웨어에서 `PPHLX.getActionResult()`가 반환하는 값을 프로그래밍 방식으로 설정하는 함수입니다. 이 함수에는 액션 이름과 [`serializeActionResult()`](#serializeactionresult)로 직렬화된 액션 결과가 전달됩니다. 이 함수를 미들웨어에서 호출하면 PPHLX 자체의 액션 결과 처리가 비활성화됩니다.
...
양식 액션 제출을 처리할 때 PPHLX 내부적으로 사용하는 쿼리 매개변수 이름을 포함하는 객체입니다.
...
[PPHLX 컨텍스트 객체](/ko/reference/api-reference/)의 하위 집합입니다. 다음 속성은 사용할 수 없습니다: `callAction`, `getActionResult`, `props`, `redirect`.
...
const feedbackUrl = new URL('/feedback', PPHLX.url);
