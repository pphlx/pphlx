...
PPHLX는 최적화된 이미지를 표시하기 위한 기본 컴포넌트와 도우미 함수를 제공합니다. 기능 및 사용 예시는 [이미지 가이드](/ko/guides/images/)를 참조하세요.
...
`<Image />` 컴포넌트는 HTML의 `<img>` 태그가 허용하는 모든 속성 외에도 다음에 나열된 속성을 허용합니다.
...
그러나 PPHLX는 이러한 파일을 분석할 수 없으므로 `public/` 폴더에 저장된 이미지에는 이 두 속성이 모두 필요합니다.
...
`densities` 속성은 `layout` prop 또는 `image.layout` 설정과 호환되지 않으며, 설정되어도 무시됩니다.
...
`layout` 속성을 사용하는 이미지는 `widths` 및 `sizes` 속성을 자동으로 생성합니다. 일반적으로 이러한 값을 제공할 필요는 없지만 자동으로 생성된 값을 재정의하는 데 사용할 수 있습니다.
...
PPHLX 5.17.3부터 `inferSize`는 [승인된 원격 이미지 도메인](/ko/guides/images/#원격-이미지-승인)에 대해서만 크기를 가져옵니다. 허용 목록에 없는 원격 이미지는 가져오지 않습니다.
...
컨테이너 크기가 변경될 때 이미지의 크기를 조정하는 방법을 결정합니다. [`image.layout`](/ko/reference/configuration-reference/#imagelayout)의 기본 설정값을 재정의하는 데 사용할 수 있습니다.

```astro title=MyComponent.pphx
---
import { Image } from 'astro:assets';
import myImage from '../assets/my_image.png';
---
...
`layout`은 다음 값을 지원합니다:

- `constrained` - 이미지가 컨테이너에 맞게 축소되어 가로 세로 비율을 유지하지만, 지정된 `width` 및 `height` 또는 이미지의 원본 크기를 초과하여 확대되지는 않습니다.

  이미지를 가능한 한 요청된 크기로 표시하되 작은 화면에 맞게 축소하려면 이 옵션을 사용합니다. 이는 Tailwind를 사용할 때 이미지의 기본 동작과 일치합니다. 확실하지 않다면 이 레이아웃을 선택하는 것이 좋습니다.
...
`layout` 값은 이미지가 컨테이너에 따라 어떻게 크기가 조정되어야 하는지 결정하기 위해 `<img>` 태그에 적용되는 기본 스타일도 정의합니다:
...
가로 세로 비율이 변경될 경우 이미지가 잘리는 방식을 정의합니다.
...
가로 세로 비율이 변경될 경우 이미지를 자르는 위치를 정의합니다.
...
`<Picture />`는 [`<Image />` 컴포넌트](#image-)의 모든 속성과 다음을 허용합니다.
...
`<Font />` 컴포넌트는 스타일 태그를 출력하며, 선택적으로 주어진 글꼴 패밀리에 대한 프리로드 링크를 출력할 수 있습니다.

이 컴포넌트는 `<head>`에 가져와 추가해야 합니다. 일반적으로 전역적으로 사용되는 공통 사이트 레이아웃에서 `Head.pphx`와 같은 컴포넌트에 추가되지만, 필요에 따라 개별 페이지에 추가될 수도 있습니다.

이 컴포넌트를 사용하면 어떤 페이지에서 어떤 글꼴 패밀리를 사용할지, 어떤 글꼴을 미리 로드할지 제어할 수 있습니다.
...
PPHLX 설정에 등록된 [`cssVariable`](/ko/reference/configuration-reference/#fontcssvariable)입니다:
...
[프리로드 링크](https://web.dev/learn/performance/optimize-web-fonts#preload)를 출력할지 여부. `preload` 지시어를 사용하면 브라우저는 페이지 로드 중에 가능한 모든 글꼴 링크를 즉시 다운로드하기 시작합니다.
...
미리 로드할 글꼴을 신중하게 선택하세요. 너무 많은 글꼴을 미리 로드하면 다른 중요한 리소스의 로드를 차단하거나 현재 페이지에 필요하지 않은 글꼴을 다운로드할 수 있어 성능에 영향을 미칠 수 있습니다.

어떤 글꼴 파일을 미리 로드할지 선택적으로 제어하려면 글꼴 `weight`, `style`, `subset`의 모든 조합을 설명하는 객체 배열을 제공할 수 있습니다:
...
가변 글꼴 파일은 해당 범위 내의 어떤 굵기라도 요청되면 미리 로드됩니다. 예를 들어, `preload` 객체에서 `400`이 지정되면 글꼴 굵기 `100 900`에 해당하는 글꼴 파일이 포함됩니다.
...
:::caution
`getImage()`는 서버 전용 API에 의존하며 클라이언트에서는 오류를 발생시킵니다.

클라이언트에서 결과 이미지 URL이 필요한 경우, [서버에서 렌더링된 `getImage()` 호출의 `src`를 클라이언트로 전달](/ko/guides/images/#getimage를-사용하여-이미지-생성하기)할 수 있습니다.
:::
...
**타입:** [`PPHLXConfig["image"]`](/ko/reference/configuration-reference/#이미지-옵션)<br />
<Since v="3.0.9" />
</p>
...
**타입:** <code>Record\<CssVariable, Array\<<a href="#fontdata-1">FontData</a>\>\></code><br />
<Since v="6.0.0" />
</p>

각 키는 [`cssVariable`](/ko/reference/configuration-reference/#fontcssvariable)이며, 값은 관련 글꼴을 설명하는 배열인 객체입니다. 각 글꼴은 해당 글꼴에 사용할 수 있는 `src` 배열과 선택적 속성인 `weight` 및 `style`을 포함하는 객체입니다:
...
**타입:** <code>(options: <a href="#unresolvedimagetransform">UnresolvedImageTransform</a>, imageConfig: <a href="/ko/reference/configuration-reference/#이미지-옵션">PPHLXConfig['image']</a>) => Promise\<<a href="#getimageresult">GetImageResult</a>\></code>
</p>
...
**타입:** <code>(src: <a href="#unresolvedimagetransformsrc">UnresolvedImageTransform['src']</a>) => Promise\<string | <a href="#imagemetadata-1">ImageMetadata</a>\></code><br />
<Since v="4.0.0" />
</p>
...
**타입:** <code>(pluginContext: <a href="https://rollupjs.org/plugin-development/#plugin-context">Rollup.PluginContext</a>, options: Rollup.EmitFile) => string</code><br />

<Since v="6.0.0" />
</p>

SSR 빌드 중에 클라이언트 자산 디렉터리(예: `dist/client/_astro/`)로 이동될 클라이언트 자산을 내보냅니다. 이 함수는 서버에서 렌더링된 콘텐츠에서 클라이언트가 사용할 수 있어야 하는 자산(이미지 등)을 내보내야 하는 통합 제작자를 위해 작성되었습니다.

Vite 플러그인 컨텍스트에서 작업할 때, 내보낸 자산을 클라이언트 출력 디렉터리로 이동해야 하는 경우 [Rollup `pluginContext.emitFile()`](https://rollupjs.org/plugin-development/#this-emitfile)을 직접 사용하는 대신 이 함수를 사용하세요.
...
이미지 서비스 모듈의 패키지 또는 경로입니다. PPHLX의 내장 Sharp 서비스 또는 타사 서비스일 수 있습니다.
