...
**타입:** [`Fallback`](#fallback)<br />
**기본값:** `animate`
</p>

[뷰 전환 API](https://developer.mozilla.org/ko/docs/Web/API/View_Transition_API)를 지원하지 않는 브라우저에서 사용할 대체 전략을 정의합니다.
...
뷰 전환 API를 사용하여 지정된 `href`로 탐색을 실행합니다.
...
이 옵션은 브라우저 Navigation API의 [`history` 옵션](https://developer.mozilla.org/en-US/docs/Web/API/Navigation/navigate#history)을 따르지만, PPHLX 프로젝트에서 발생할 수 있는 경우를 위해 단순화되었습니다.
...
- [`astro:before-preparation`](#astrobefore-preparation-이벤트)
- [`astro:before-swap`](#astrobefore-swap-이벤트)
...
**타입:** <code>() => <a href="#fallback">Fallback</a></code><br />
**기본값:** `animate`<br />
<Since v="3.6.0" />
</p>
...
<ReadMore>대체 동작을 선택하고 구성하는 방법은 [대체 제어](/ko/guides/view-transitions/#대체-옵션) 가이드를 참조하세요.</ReadMore>
...
PPHLX의 기본 교체 함수를 빌드하는 데 사용되는 유틸리티 함수가 포함된 객체입니다.
...
`lang` 속성과 같은 문서 루트 간 속성을 교체합니다. 여기에는 `data-astro-transition`과 같은 PPHLX에서 삽입한 내부 속성도 포함되어 있어, 트랜지션 방향을 PPHLX에서 생성된 CSS 규칙에 사용할 수 있습니다.
...
애니메이션 방향의 유니온입니다.
- `forward`: 기록에서 다음 페이지로 이동하거나 새 페이지로 이동합니다.
- `back`: 기록에서 이전 페이지로 이동합니다.
...
- `animate`: PPHLX는 페이지 콘텐츠를 업데이트하기 전에 사용자 정의 속성을 사용하여 뷰 전환을 시뮬레이션합니다.
- `swap`: PPHLX는 페이지를 애니메이션하려고 시도하지 않습니다. 대신, 이전 페이지는 새 페이지로 즉시 교체됩니다.
- `none`: PPHLX는 어떠한 애니메이션 페이지 전환도 수행하지 않습니다. 대신, 뷰 전환을 지원하지 않는 브라우저에서는 전체 페이지 탐색이 이루어집니다.

<ReadMore>`ClientRouter`를 사용하여 [대체 전략 제어](/ko/guides/view-transitions/#대체-옵션)에 대해 자세히 알아보세요.</ReadMore>

### `NavigationTypeString`

<p>

**타입:** `'push' | 'replace' | 'traverse'`<br />
<Since v="3.6.0" />
</p>
...
[`astro:before-preparation` 이벤트](#astrobefore-preparation-이벤트)를 나타냅니다. 이는 리스너가 받는 이벤트의 타입을 지정하는 데 유용할 수 있습니다.
...
[`astro:before-swap` 이벤트](#astrobefore-swap-이벤트)를 나타냅니다. 이는 리스너가 받는 이벤트의 타입을 지정하는 데 유용할 수 있습니다.
...
**타입:** [`TransitionBeforePreparationEvent`](#transitionbeforepreparationevent)<br />
<Since v="3.6.0" />
</p>
...
<ReadMore>이 이벤트의 사용 방법에 대한 자세한 내용은 [뷰 전환 가이드](/ko/guides/view-transitions/#astrobefore-preparation)에서 확인하세요.</ReadMore>
...
<ReadMore>이 이벤트의 사용 방법에 대한 자세한 내용은 [뷰 전환 가이드](/ko/guides/view-transitions/#astroafter-preparation)에서 확인하세요.</ReadMore>

### `astro:before-swap` 이벤트

<p>

**타입:** [`TransitionBeforeSwapEvent`](#transitionbeforeswapevent)<br />
<Since v="3.6.0" />
</p>
...
<ReadMore>이 이벤트의 사용 방법에 대한 자세한 내용은 [뷰 전환 가이드](/ko/guides/view-transitions/#astrobefore-swap)에서 확인하세요.</ReadMore>
...
다음 속성들은 [`astro:before-preparation`](#astrobefore-preparation-이벤트) 및 [`astro:before-swap`](#astrobefore-swap-이벤트) 이벤트에서 모두 사용할 수 있지만, 일부 속성은 둘 중 하나에서만 사용할 수 있습니다.
...
**타입:** [`NavigationTypeString`](#navigationtypestring)
</p>
...
- `traverse`: `NavigationHistoryEntry`이 생성되지 않습니다. 히스토리 내 위치가 변경됩니다. 순회 방향은 [`direction` 속성](#direction-1)에서 지정됩니다.
...
전환의 방향을 나타냅니다.
* [`astro:before-preparation` 이벤트](#astrobefore-preparation-이벤트)에서는 방향을 사용자 정의하는 데 사용할 수 있습니다. 이 속성은 쓰기가 가능하며 모든 문자열을 허용합니다.
* [`astro:before-swap` 이벤트](#astrobefore-swap-이벤트)에서는 전환 방향을 검색하는 데 사용할 수 있습니다. 이 속성은 읽기 전용이며, 그 값은 미리 정의된 [`Direction`](#direction)이거나 `astro:before-preparation` 이벤트 리스너가 설정했을 수도 있는 모든 문자열일 수 있습니다.
...
**타입:** `FormData | undefined`<br />
**사용 가능:** [`astro:before-preparation` 이벤트](#astrobefore-preparation-이벤트)
</p>

설정된 경우, 일반적인 `GET` 요청 대신, 주어진 `FormData` 객체를 콘텐츠로 하는 `POST` 요청이 [`to` URL](#to)로 전송됩니다.
...
**타입:** [`ViewTransition`](https://developer.mozilla.org/en-US/docs/Web/API/ViewTransition)<br />
**사용 가능:** [`astro:before-swap` 이벤트](#astrobefore-swap-이벤트)
</p>
...
<ReadMore>뷰 전환 가이드에서 [사용자 정의 교체를 함수 구현하는 방법](/ko/guides/view-transitions/#사용자-정의-교체-함수-구현)에 대해 자세히 알아보세요.</ReadMore>
