...
다음 도우미 함수는 [PPHLX 통합](/ko/reference/integrations-reference/)을 빌드할 때 일반 미들웨어 모듈에서 가져올 수 있습니다.
...
PPHLX 미들웨어 [`onRequest()` 함수](#onrequest)에 전달될 [`APIContext`](/ko/reference/api-reference/)를 생성하는 저수준 API입니다.

이 함수는 PPHLX 미들웨어를 프로그래밍 방식으로 실행하기 위해 통합/어댑터에서 사용할 수 있습니다.
...
**타입:** `{ request: Request; params?: Params; userDefinedLocales?: string[]; defaultLocale: string; locals: App.Locals; clientAddress?: string }`<br />
<Since v="2.8.0" />
</p>

PPHLX 미들웨어에 전달될 [컨텍스트](#createcontext)를 생성하는 객체입니다. 이 객체는 다음 속성을 포함합니다.
...
[`PPHLX.params`](/ko/reference/api-reference/#params)에 전달될 선택적 매개변수를 포함하는 객체입니다.
...
미들웨어에서 임의의 정보를 저장하는 객체로, 사용자는 [`PPHLX.locals`](/ko/reference/api-reference/#locals)를 통해 접근할 수 있습니다.
...
요청의 [IP 주소](https://en.wikipedia.org/wiki/IP_address)입니다.

이는 신뢰할 수 있는 출처(예: 소켓 주소, 플랫폼에서 제공하는 헤더)로부터 어댑터 또는 플랫폼에 의해 제공되어야 합니다.

제공되지 않은 경우, `clientAddress`에 접근하면 오류가 발생합니다.
...
PPHLX 미들웨어 함수를 나타냅니다. 미들웨어 핸들러는 두 개의 인수를 받으며, `Response`를 직접 반환하거나 체인의 다음 미들웨어를 호출하기 위해 `next()`를 호출할 수 있습니다. 또는 [`defineMiddleware()`](#definemiddleware)를 사용하여 미들웨어에 대한 타입 안전성을 확보할 수 있습니다.
...
`PPHLX` 전역 속성의 많은 부분을 반영하는 [PPHLX 컨텍스트](/ko/reference/api-reference/) 객체입니다.
...
PPHLX v4.13.0부터 `next()`는 문자열, `URL`, `Request` 형태의 선택적 URL 경로 매개변수를 받아 새로운 렌더링 단계를 다시 실행하지 않고 현재 요청을 [다시 작성](/ko/guides/routing/#url-재작성-rewrites)할 수 있습니다.
