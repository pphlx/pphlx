...
이 함수를 사용하여 로케일의 상대 경로를 검색합니다. 해당 로케일이 존재하지 않으면 PPHLX는 오류를 발생시킵니다.
...
[`site`]의 값이 존재할 때 로케일의 절대 경로를 검색하려면 이 함수를 사용하세요. [`site`]가 구성되지 않은 경우 함수는 상대 URL을 반환합니다. 해당 로케일이 존재하지 않으면 PPHLX는 오류를 발생시킵니다.
...
**타입:** `(options: { fallbackType: "redirect" | "rewrite", prefixDefaultLocale: boolean, redirectToDefaultLocale: boolean }) => MiddlewareHandler`<br />
<Since v="4.6.0" />
</p>
...
PPHLX i18n 미들웨어를 프로그래밍 방식으로 생성할 수 있는 함수입니다.
...
const customLogic = defineMiddleware(async (context, next) => {
  ...
  // 응답을 해결한 후 사용자 정의 로직.
  // PPHLX i18n 미들웨어에서 오는 응답을 캐치할 수 있습니다.
...
normalizeTheLocale(PPHLX.currentLocale) // `pt-pt`를 반환합니다.
---
```
