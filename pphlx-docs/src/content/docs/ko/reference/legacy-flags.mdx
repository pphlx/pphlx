...
일부 사용자들의 PPHLX 버전 간 마이그레이션을 돕기 위해, 때때로 우리는 `legacy` 플래그를 도입합니다.

이러한 플래그를 통해 최신 버전의 PPHLX에서 더 이상 사용되지 않거나 구식이 된 동작을 선택적으로 사용할 수 있으며, 프로젝트 코드를 완전히 업데이트할 수 있을 때까지 새로운 PPHLX 릴리스의 이점을 계속해서 활용할 수 있습니다.
