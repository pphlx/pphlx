...
기본적으로 모든 PPHLX 페이지 라우트와 엔드포인트는 빌드 타임에 생성되어 사전 렌더링됩니다. [요청 시 서버 렌더링](/ko/guides/on-demand-rendering/)은 개별 라우트에 대해 설정하거나 기본값으로 설정할 수 있습니다.
...
기본적으로 예약된 `src/pages/` 디렉터리에 위치한 모든 파일은 자동으로 `<!DOCTYPE html>` 선언과 PPHLX의 스코프가 지정된 스타일 및 스크립트와 같은 추가적인 `<head>` 콘텐츠를 포함합니다.
...
파일 경로에 하나 이상의 [매개변수](#params)가 있는 단일 `.pphx` 페이지 컴포넌트에서 여러 개의 사전 렌더링된 페이지 라우트를 생성하는 함수입니다. 이는 정적 사이트 빌딩이라고도 하는, 빌드 타임에 생성될 라우트에 사용됩니다.

`getStaticPaths()` 함수는 PPHLX가 사전 렌더링할 URL 경로를 결정하기 위한 객체 배열을 반환해야 합니다. 각 객체는 라우트 경로를 지정하는 `params` 객체를 포함해야 합니다. 선택적으로 각 페이지 템플릿에 [전달할 데이터](#props를-사용한-데이터-전달)가 있는 `props` 객체를 포함할 수 있습니다.
...
`getStaticPaths()`가 반환하는 배열의 각 객체에 있는 `params` 키는 PPHLX에게 어떤 라우트를 빌드할지 알려줍니다.

`params`의 키는 컴포넌트 파일 경로에 정의된 매개변수와 일치해야 합니다. 각 `params` 객체의 값은 페이지 이름에 사용된 매개변수와 일치해야 합니다. `params`는 URL로 인코딩되므로 문자열 값만 지원됩니다.

예를 들어, `src/pages/posts/[id].pphx`는 파일 이름에 `id` 매개변수를 가지고 있습니다. 다음은 `.pphx` 컴포넌트의 `getStaticPaths()` 함수가 빌드 시점에 PPHLX에게 `posts/1`, `posts/2`, `posts/3`을 정적으로 생성하도록 지시합니다.
...
예를 들어, 원격 API에서 가져온 데이터로 페이지를 생성하는 경우 `getStaticPaths()`에서 전체 데이터 객체를 페이지 컴포넌트에 전달할 수 있습니다. 페이지 템플릿은 `PPHLX.props`를 사용하여 각 게시물의 데이터를 참조할 수 있습니다.
...
이는 일반적으로 `getStaticPaths()` 범위에서는 접근할 수 없는 [PPHLX 렌더링 컨텍스트](/ko/reference/api-reference/)의 데이터를 제공하며, 각 페이지 라우트에 대한 `params`와 `props`를 계산하는 데 유용할 수 있습니다.
...
다음은 사용자 정의 `getLocalizedData()` 도우미 함수에 `routePattern`을 전달하여 라우트 세그먼트를 현지화하고 정적 경로 배열을 반환하는 방법을 보여주는 예시입니다. [params](/ko/reference/routing-reference/#params) 객체는 각 라우트 세그먼트(`locale`, `files`, `slug`)에 대한 명시적 값으로 설정됩니다. 이 값들은 라우트를 생성하는 데 사용되며 `PPHLX.params`를 통해 페이지 템플릿에서 활용할 수도 있습니다.
...
`paginate()`는 `[page].pphx` 또는 `[...page].pphx`의 파일 이름을 가정합니다. `page` 매개변수는 URL에서 페이지 번호가 됩니다:

- `/posts/[page].pphx`는 `/posts/1`, `/posts/2`, `/posts/3` 등의 URL을 생성합니다.
- `/posts/[...page].pphx`는 `/posts`, `/posts/2`, `/posts/3` 등의 URL을 생성합니다.
