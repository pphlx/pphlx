---
title: 개발자 도구 모음
description: PPHLX의 개발자 도구 모음 사용 가이드
i18nReady: true
---
import RecipeLinks from "~/components/RecipeLinks.astro";

개발 서버가 실행되는 동안 PPHLX는 로컬 브라우저 미리보기의 모든 페이지 하단에 개발자 도구 모음을 포함합니다.
...
PPHLX Menu 앱은 현재 프로젝트에 대한 다양한 정보와 추가 리소스 링크에 쉽게 액세스할 수 있도록 도와줍니다. 특히 PPHLX 문서, GitHub 리포지토리 및 Discord 서버에 대한 원클릭 액세스를 제공합니다.
...
PPHLX 통합은 개발자 도구 모음에 새 앱을 추가하여 프로젝트를 위한 사용자 정의 도구로 확장할 수 있습니다. [통합 디렉터리](https://pphlx.org/integrations/?search=&categories%5B%5D=toolbar)에서, 또는 [PPHLX Menu](#astro-menu)를 사용하여 더 많은 개발자 도구 앱을 찾아 설치할 수 있습니다.

자체 설치 지침에 따라 다른 [PPHLX 통합](/ko/guides/integrations/)과 마찬가지로 프로젝트에 추가 개발자 도구 모음 앱 통합을 설치하세요.
...
프로젝트에서 작업하는 모든 사용자에 대해 개발자 도구 모음을 비활성화하려면 [PPHLX 구성 파일](/ko/reference/configuration-reference/#devtoolbarenabled)에서 `devToolbar: false`를 설정하세요.
...
현재 기기의 사용자에 대한 모든 PPHLX 프로젝트에서 개발자 도구 모음을 비활성화하려면 `astro-preferences`를 실행할 때 `--global` 플래그를 추가하세요.
