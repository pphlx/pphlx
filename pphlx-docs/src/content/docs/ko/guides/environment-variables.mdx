...
sidebar:
  label: 환경 변수
description: PPHLX 프로젝트에서 환경 변수를 사용하는 방법을 알아보세요.
i18nReady: true
---
import PackageManagerTabs from '~/components/tabs/PackageManagerTabs.astro';
import ReadMore from '~/components/ReadMore.astro';

PPHLX는 [Vite의 내장 환경 변수 지원](#vite의-내장-지원)에 대한 접근 권한을 제공하며, 현재 프로젝트의 구성 값 (`site`, `base` 등), 프로젝트가 개발 또는 프로덕션 환경에서 실행 중인지 여부 등에 접근할 수 있도록 [몇 가지 기본 환경 변수](#기본-환경-변수)를 포함합니다.

PPHLX는 [타입 안정성을 갖춘 환경 변수를 사용하고 구성하는 방법](#타입-안전-환경-변수)도 제공합니다. PPHLX 컨텍스트 (예: PPHLX 컴포넌트, 라우트 및 엔드포인트, UI 프레임워크 컴포넌트, 미들웨어)에서 사용할 수 있으며, [PPHLX 구성의 스키마](/ko/reference/configuration-reference/#env)를 통해 관리됩니다.

## Vite의 내장 지원

PPHLX는 빌드 시 정적으로 대체되는 환경 변수에 대한 Vite의 내장 지원을 사용하며, 환경 변수를 다루기 위한 [모든 메서드](https://ko.vite.dev/guide/env-and-mode.html)를 사용할 수 있습니다.
...
기본적으로 PPHLX는 `astro/client.d.ts`에서 `import.meta.env`에 대한 타입 정의를 제공합니다.
...
PPHLX는 다른 파일을 로드하기 전에 구성 파일을 평가합니다. 즉, `astro.config.mjs`에서 `.env` 파일에 설정된 환경 변수에 접근하기 위해 `import.meta.env`를 사용할 수 없습니다.
...
PPHLX의 환경 변수는 `process.env` 대신 [ES2020에 추가된 `import.meta` 기능](https://tc39.es/ecma262/2020/#prod-ImportMeta)을 사용하여 `import.meta.env`로 접근합니다.
...
스키마를 구성하려면 PPHLX 구성에 `env.schema` 옵션을 추가하세요.
...
<ReadMore>유효성 검사 필드의 전체 목록은 [`envField` API 참조](/ko/reference/modules/astro-config/#envfield)를 확인하세요.</ReadMore>
...
`astro:env`는 가상 모듈이므로 PPHLX 컨텍스트에서만 사용할 수 있습니다. 예를 들어 다음과 같은 곳에서 사용할 수 있습니다.
