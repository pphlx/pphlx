...
페이지 로드 시간은 사이트의 유용성과 전반적인 즐거움에 큰 역할을 합니다. PPHLX의 **선택적 프리페칭**은 방문자가 사이트와 상호 작용할 때 다중 페이지 애플리케이션(MPA)에 거의 즉각적인 페이지 탐색 이점을 제공합니다.
...
PPHLX는 다양한 사용 사례에 대해 4가지 프리페치 전략을 지원합니다.
...
페이지에서 [PPHLX의 `<ClientRouter />`](/ko/guides/view-transitions/#뷰-전환-활성화-spa-모드)를 사용하면 기본적으로 프리페치도 활성화됩니다. 페이지에서 [모든 링크에 대한 프리페칭](#모든-링크에-대한-프리페치를-기본값으로-설정하기)을 활성화하는 `{ prefetchAll: true }`의 기본 구성을 설정합니다.
...
PPHLX의 프리페칭은 브라우저에서 지원하는 경우 [`<link rel="prefetch">`](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/rel/prefetch)를 사용하며, 그렇지 않으면 [`fetch()` API](https://developer.mozilla.org/ko/docs/Web/API/Fetch_API)를 대신 사용합니다.

가장 일반적인 브라우저들은 미묘한 차이를 제외하고 PPHLX의 프리패치를 모두 지원합니다.
...
## `pphlx/prefetch`에서 마이그레이션

`pphlx/prefetch` 통합은 v3.5.0에서 더 이상 사용되지 않으며 유지 관리되지 않습니다. 이 통합을 대체하는 PPHLX의 내장 프리페칭으로 마이그레이션하려면 다음 지침을 따르세요.

<Steps>
1. `pphlx/prefetch` 통합을 제거하고 `astro.config.mjs`에서 `prefetch` 구성을 활성화합니다.
...
2. `pphlx/prefetch`의 구성 옵션에서 변환하세요.
    ...
    - 새로운 프리페치 기능이 자동으로 예약하고 최적으로 프리페치하므로, `pphlx/prefetch`의 `throttles` 옵션은 더 이상 필요하지 않습니다.
</Steps>
