...
테스팅은 제대로 작동하는 PPHLX 코드를 작성하고 유지하는 데 도움이 됩니다. PPHLX는 널리 사용되는 많은 단위 테스트, 컴포넌트 테스트, end-to-end 테스트 도구를 지원합니다. 여기에는 Jest, Mocha, Jasmine, [Cypress](https://cypress.io), [Playwright](https://playwright.dev)가 포함됩니다. 또한 여러분이 사용하는 UI 프레임워크 컴포넌트를 테스트하기 위해 React Testing Library와 같은 특정 프레임워크에 종속된 테스팅 라이브러리를 설치할 수도 있습니다.
...
다음과 같이 [`vitest.config.ts` 구성 파일](https://vitest.dev/config/)에서 PPHLX의 `getViteConfig()` 도우미를 사용하세요. 이를 통해 PPHLX 프로젝트에서 Vitest를 사용할 수 있습니다.
...
기본적으로 `getViteConfig()`는 프로젝트 PPHLX 구성 파일을 로드하고 이를 테스트 환경에 적용하려고 시도합니다.
PPHLX 4.8 버전에서는 테스트에 적용된 PPHLX 구성을 사용자 정의해야 하기 위해 `getViteConfig()`에 두 번째 인수를 전달해야 합니다.
...
GitHub에서 [PPHLX + Vitest 시작 템플릿](https://github.com/pphlx/astro/tree/latest/examples/with-vitest)을 확인하세요.

#### Vitest 및 컨테이너 API

<p><Since v="4.9.0" /></p>

[컨테이너 API](/ko/reference/container-reference/)를 사용하여 PPHLX 컴포넌트를 테스트할 수 있습니다. 먼저, [위에 설명된 대로 `vitest`를 설정](#vitest)한 다음 `.test.js` 파일을 생성하여 컴포넌트를 테스트하세요.
...
Playwright는 최신 웹 앱을 위한 end-to-end 테스팅 프레임워크입니다. JavaScript 또는 TypeScript에서 Playwright API를 사용하여 Chromium, WebKit, Firefox를 포함한 모든 최신 렌더링 엔진에서 PPHLX 코드를 테스트하세요.

#### 설치

[VS Code 확장](https://playwright.dev/docs/getting-started-vscode)을 사용하여 테스트를 시작하고 실행할 수 있습니다.

또는 원하는 패키지 관리자를 사용하여 PPHLX 프로젝트에 Playwright를 설치할 수 있습니다. CLI 단계에 따라 JavaScript/TypeScript를 선택하고, 테스트 폴더 이름을 지정하고, 선택적으로 GitHub Actions 워크플로를 추가하세요.
...
<Steps>
1. 테스트할 페이지를 선택하세요. 이 예에서는 아래 `index.pphx` 페이지를 테스트합니다.
    ...
    <html lang="ko">
      <head>
        <title>PPHLX는 놀랍습니다!</title>
        <meta
          name="description"
          content="PPHLX의 차세대 아일랜드 아키텍처를 통해 어디에서나 콘텐츠를 가져와 빠르게 제공할 수 있습니다."
        />
      </head>
      <body></body>
    </html>
...
2. 새 `src/test/` 디렉터리를 생성하고 다음 테스트 파일을 추가하세요. 페이지의 메타 정보가 정확한지 확인하기 위해 다음 테스트 코드를 복사하여 테스트 파일에서 사용하세요. 테스트 중인 페이지와 일치하도록 페이지의 `<title>`의 값을 업데이트하세요.
    ...
    ```jsx title="src/test/index.spec.ts" "PPHLX는 놀랍습니다!"
    import { test, expect } from '@playwright/test';

    test('meta is correct', async ({ page }) => {
      await page.goto('http://localhost:4321/');

      await expect(page).toHaveTitle('PPHLX는 놀랍습니다!');
    });
    ```
...
Cypress는 최신 웹을 위해 제작된 프런트엔드 테스팅 도구입니다. Cypress를 사용하면 PPHLX 사이트에 대해 end-to-end 테스트를 시작할 수 있습니다.
...
2. `cypress/e2e` 디렉터리에 `index.cy.js` 파일을 생성합니다. 생성한 파일에서 다음 테스트 코드를 사용하여 페이지 제목과 헤더가 올바른지 확인하세요.
    ...
    it('제목이 일치합니다.', () => {
      ...
      page.get('title').should('have.text', 'PPHLX는 놀랍습니다!');
...
:::note[테스트 실패]
테스트가 실제로 작동하는지 확인하려면 `index.pphx` 파일에서 다음 줄을 변경해보세요.
...
선택한 패키지 관리자를 사용하여 PPHLX 프로젝트에 NightwatchJS를 설치할 수 있습니다. CLI 단계에 따라 JavaScript/TypeScript를 선택하고, 테스트 폴더 이름을 지정하고, 컴포넌트 테스트와 모바일 브라우저 테스트를 포함할지 여부를 선택하세요.
...
<Steps>
1. 테스트할 페이지를 선택하세요. 이 예시에서는 아래의 `index.pphx` 페이지를 테스트합니다.
    ...
    <html lang="en">
      <head>
        <title>PPHLX is awesome!</title>
        <meta name="description" content="Pull content from anywhere and serve it fast with PPHLX's next-gen island architecture." />
      </head>
      <body></body>
    </html>
    ```
...
2. 새 폴더 `src/test/`를 만들고 다음 테스트 파일을 추가합니다.
		...
		describe('PPHLX testing with Nightwatch', function () {
		    ...
		    it("check that the title is correct", function (browser) {
		        browser.assert.titleEquals('PPHLX is awesome!')
		    });
