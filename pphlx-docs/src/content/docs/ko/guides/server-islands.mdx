...
서버 아일랜드는 콘텐츠가 사용 가능할 때까지 렌더링을 지연하도록 지시된 일반적인 서버 렌더링 [PPHLX 컴포넌트](/ko/basics/astro-components/)입니다.
...
서버 아일랜드 컴포넌트에 제공되는 Props는 [직렬화](https://developer.mozilla.org/ko/docs/Glossary/Serialization)할 수 있습니다. 즉, 네트워크를 통해 전송하거나 저장하기에 적합한 형식으로 변환할 수 있어야 합니다. 그리고 PPHLX는 직렬화 가능한 모든 유형의 데이터 구조를 직렬화하지는 않습니다. 따라서 서버 아일랜드에 props로 전달할 수 있는 항목에는 제한이 있습니다.
...
`server:defer`로 표시된 각 아일랜드는 런타임에 스크립트가 가져오는 자체 특수 라우트로 분리됩니다. PPHLX가 사이트를 빌드할 때 컴포넌트를 생략하고 그 자리에 스크립트와 `slot="fallback"`으로 표시한 콘텐츠를 주입합니다.
...
서버 아일랜드의 데이터는 URL 쿼리에 암호화된 문자열 형태로 props를 전달하는 `GET` 요청을 통해 검색됩니다. 이를 통해 표준 `Cache-Control` 지시어를 사용하여 [`Cache-Control` HTTP 헤더](https://developer.mozilla.org/ko/docs/Web/HTTP/Reference/Headers/Cache-Control)로 데이터를 캐싱할 수 있습니다.

하지만 브라우저는 실용적인 이유와 서비스 거부 문제를 방지하기 위해 [URL 길이를 최대 2048바이트로 제한합니다](https://chromium.googlesource.com/chromium/src/+/master/docs/security/url_display_guidelines/url_display_guidelines.md#url-length). 만약 쿼리 문자열로 인해 URL이 이 제한을 초과하게 되면, PPHLX는 대신 모든 props를 본문에 포함하는 `POST` 요청을 보냅니다.
...
하지만 서버 아일랜드는 페이지 요청 외부의 독립된 컨텍스트에서 실행됩니다. 서버 아일랜드 컴포넌트의 `PPHLX.url`과 `PPHLX.request.url`은 브라우저의 현재 페이지 URL 대신 `/_server-islands/Avatar`와 같은 URL을 반환합니다. 또한 페이지를 사전 렌더링하는 경우 props로 전달하기 위한 쿼리 매개변수와 같은 정보에 접근할 수 없습니다.
...
PPHLX는 [cryptography](https://developer.mozilla.org/ko/docs/Glossary/Cryptography)를 사용하여 서버 아일랜드로 전달되는 props를 암호화합니다. 이를 통해 민감한 데이터가 우발적으로 노출되지 않도록 보호합니다. 이 암호화는 각 빌드에서 생성되어 서버 번들에 포함되는 새로운 무작위 키를 사용합니다.
...
이러한 상황에서는 PPHLX CLI를 사용하여 재사용 가능한 인코딩된 암호화 키를 생성하고 이를 빌드 환경의 환경 변수로 설정하세요.
