...
PPHLX를 사용하면 모든 종류의 데이터를 제공하는 사용자 정의 엔드포인트를 만들 수 있습니다. 이를 사용하여 이미지를 생성하거나, RSS 문서를 노출하거나, 사이트의 전체 API를 빌드하기 위한 API 라우트로 사용할 수 있습니다.
...
엔드포인트는 전역 `PPHLX` 객체와 유사한 속성을 가진 [컨텍스트 객체](/ko/reference/api-reference/)를 받는 `GET` 함수 (선택적으로 `async`)를 내보냅니다. 여기서 `name` 및 `url`을 사용하는 [`Response`](https://developer.mozilla.org/ko/docs/Web/API/Response) 객체를 반환하며, PPHLX는 빌드 시 이를 호출하고 본문 콘텐츠를 사용하여 파일을 생성합니다.
...
PPHLX v3.0부터는 반환된 `Response` 객체가 더 이상 `encoding` 속성을 포함할 필요가 없습니다. 예를 들어 바이너리 `.png` 이미지를 생성하려면 다음을 수행합니다.
...
`APIRoute` 타입과 `satisfies` 연산자를 사용하여 엔드포인트 함수에서 타입 안전성을 확보할 수도 있습니다.
...
URL에 파일 확장자가 포함된 엔드포인트(예: `src/pages/sitemap.xml.ts`)는 항상 후행 슬래시 없이만 접근할 수 있습니다(예: `/sitemap.xml`). 이는 [`build.trailingSlash`](/ko/reference/configuration-reference/#trailingslash) 설정과 관계없이 동일하게 적용됩니다.
...
이는 빌드 시 `/api/0.json`, `/api/1.json`, `/api/2.json`, `/api/3.json`의 네 가지 JSON 엔드포인트를 생성합니다. 엔드포인트의 동적 라우팅은 페이지와 동일하게 작동합니다. 정적 모드에서는 [`getStaticPaths()`를 사용하여 엔드포인트에 props를 전달](/ko/reference/routing-reference/#props를-사용한-데이터-전달)할 수 있습니다. 하지만 요청 시 렌더링의 경우, 엔드포인트는 컴포넌트가 아닌 함수이므로 props 전달은 지원되지 않습니다.

### `request`

모든 엔드포인트는 `request` 속성을 받지만, 정적 모드에서는 `request.url`에만 액세스할 수 있습니다. 이는 현재 엔드포인트의 전체 URL을 반환하며 페이지의 [PPHLX.request.url](/ko/reference/api-reference/#request)과 동일하게 작동합니다.
...
정적 파일 엔드포인트 섹션에서 설명된 모든 내용은 SSR 모드에서도 사용할 수 있습니다. 즉, 파일은 전역 `PPHLX` 객체와 유사한 속성을 가진 [컨텍스트 객체](/ko/reference/api-reference/)를 받는 `GET` 함수를 내보낼 수 있습니다.
...
`GET` 함수 외에도 모든 [HTTP 메서드](https://developer.mozilla.org/ko/docs/Web/HTTP/Reference/Methods)의 이름을 가진 함수를 내보낼 수 있습니다. 요청이 들어오면 PPHLX는 메서드를 확인하고 해당 함수를 호출합니다.
...
`GET` 함수는 정의했지만 `HEAD` 함수는 정의하지 않은 경우, PPHLX는 `GET` 함수를 호출하고 응답에서 본문을 제거하여 `HEAD` 요청을 자동으로 처리합니다.
...
엔드포인트 컨텍스트는 `PPHLX.redirect`와 유사한 `redirect()` 유틸리티를 내보냅니다.
