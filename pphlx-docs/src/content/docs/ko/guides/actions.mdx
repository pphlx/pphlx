...
PPHLX 액션을 사용하면 타입 안전성을 갖춘 백엔드 함수를 정의하고 호출할 수 있습니다. 액션은 데이터 가져오기, JSON 구문 분석, 입력 유효성 검사를 수행합니다. 이렇게 하면 [API 엔드포인트](/ko/guides/endpoints/)를 사용할 때보다 필요한 상용구의 양을 크게 줄일 수 있습니다.
...
액션은 `astro:actions` 모듈에서 함수로 사용할 수 있습니다. `actions`를 가져와서 [UI 프레임워크 컴포넌트](/ko/guides/framework-components/), [양식 POST 요청](#html-양식-액션에서-액션-호출) 또는 PPHLX 컴포넌트에서 `<script>` 태그를 사용하여 클라이언트 측에서 호출합니다.
...
다음 단계에 따라 액션을 정의하고 PPHLX 페이지의 `script` 태그에서 호출합니다.
...
4. 클릭 시 `getGreeting` 액션을 사용하여 인사말을 가져오는 버튼이 있는 PPHLX 컴포넌트를 만듭니다.
...
클라이언트에서 액션을 호출할 때 `react-router`와 같은 클라이언트 측 라이브러리와 통합하거나, 액션이 성공하면 새 페이지로 리디렉션하는 PPHLX의 [`navigate()` 함수](/ko/guides/view-transitions/#탐색-트리거)를 사용할 수 있습니다.
...
또한 PPHLX는 편의를 위해 다음과 같은 유형의 필드 입력에 대한 유효성 검사를 내부적으로 특별히 처리합니다.
...
2. 제출된 양식을 처리할 `newsletter` 액션을 정의합니다. `z.email()` 유효성 검사기를 사용하여 `email` 필드의 유효성을 검사하고 `z.boolean()`을 사용하여 `terms` 체크박스의 유효성을 검사합니다:
...
서버에서 [PPHLX.getActionResult()](/ko/reference/api-reference/#getactionresult)를 호출하면 양식 제출 결과 (`data` 또는 `error`)가 반환되며, 동적 리디렉션, 양식 오류 처리, UI 업데이트 등에 사용할 수 있습니다.
...
예를 들어, 이 PPHLX 컴포넌트는 버튼을 클릭하면 `logout` 액션을 호출하고 현재 페이지를 다시 로드합니다:
...
PPHLX 컴포넌트에서 `PPHLX.getActionResult()`를 호출하여 액션 결과를 검색할 수 있습니다. 액션이 호출되면 `data` 또는 `error` 속성이 포함된 객체를 반환하고, 이 요청 중에 액션이 호출되지 않은 경우 `undefined`를 반환합니다.

`data` 속성을 사용하여 `PPHLX.redirect()`와 함께 사용할 URL을 구성합니다:
...
양식이 포함된 PPHLX 컴포넌트에서 `PPHLX.getActionResult()`를 호출하면 사용자 정의 오류 처리를 위해 `data` 및 `error` 객체에 액세스할 수 있습니다.
...
성공 시 사용자에게 알림을 표시하기 위해 액션의 반환 값을 사용하려면, 액션을 `PPHLX.getActionResult()`에 전달하세요. 반환된 `data` 속성을 사용하여 표시하고자 하는 UI를 렌더링하세요.
...
[미들웨어 파일을 생성](/ko/guides/middleware/)하고 `astro:actions`에서 [`getActionContext()` 유틸리티](/ko/reference/modules/astro-actions/#getactioncontext)를 가져오는 것부터 시작하세요. 이 함수는 액션 핸들러와 액션이 HTML 양식에서 호출되었는지 여부를 포함한 액션 요청에 대한 정보가 있는 `action` 객체를 반환합니다. `getActionContext()`는 또한 `PPHLX.getActionResult()`에 의해 반환되는 값을 프로그래밍 방식으로 설정하는 `setActionResult()`와 `serializeActionResult()` 함수를 반환합니다:
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  // 액션 결과가 쿠키로 전달되었다면,
  // `PPHLX.getActionResult()`에서 접근할 수 있도록 결과를 설정하세요
...
PPHLX는 권한 수준과 액션별 속도 제한을 존중하기 위해 액션 핸들러에서 사용자 세션을 인증하는 것을 권장합니다. 하지만 미들웨어에서 모든 액션(또는 액션의 하위 집합)에 대한 요청을 제한할 수도 있습니다.
...
## PPHLX 컴포넌트 및 서버 엔드포인트에서 액션 호출

PPHLX 컴포넌트 스크립트에서 `PPHLX.callAction()` 래퍼 (또는 [서버 엔드포인트](/ko/guides/endpoints/#서버-엔드포인트-api-라우트) 사용 시 `context.callAction()`)를 사용하여 액션을 직접 호출할 수 있습니다. 서버 코드에서 액션의 로직을 재사용하는 것은 일반적인 방법입니다.
