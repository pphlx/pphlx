...
title: 스크립트 및 이벤트 처리
description: >-
  기본 브라우저 JavaScript API를 사용하여 PPHLX 컴포넌트에 클라이언트 측 상호 작용을 추가하는 방법.
i18nReady: true
---

import ReadMore from '~/components/ReadMore.astro'

컴포넌트 템플릿의 `<script>` 태그를 사용하여 브라우저로 JavaScript를 전송하고 PPHLX 컴포넌트에 기능을 추가할 수 있습니다.
...
PPHLX는 번들링, TypeScript 등을 통해 HTML 표준 `<script>` 태그를 개선합니다. 자세한 내용은 [PPHLX가 스크립트를 처리하는 방법](#스크립트-처리하기)을 참조하세요.
...
기본적으로 `<script>` 태그는 PPHLX에서 처리됩니다.

기본적으로 PPHLX는 `src` 외에 다른 속성이 없는 `<script>` 태그를 다음과 같은 방식으로 처리합니다.
...
- **자동 인라인 처리:** 스크립트가 충분히 작은 경우, PPHLX는 요청 횟수를 줄이기 위해 스크립트를 HTML에 직접 인라인 처리합니다.
...
`<script>` 태그에 `src` 이외의 속성이 있는 경우 PPHLX는 이를 처리하지 않습니다.
...
<script is:inline>
  ...
  // 변환되지 않음: TypeScript 처리 및 PPHLX의 가져오기 경로 해석이 적용되지 않습니다.
...
PPHLX는 [스크립트 처리 규칙](#스크립트-처리하기)에 따라 이러한 스크립트를 처리합니다.
...
프로젝트의 `src/` 폴더 외부에 있는 스크립트를 로드하려면 `is:inline` 지시어를 포함하세요. 이 접근 방식은 위에서 설명한 대로 스크립트를 가져올 때 PPHLX에서 제공하는 JavaScript 처리, 번들링 및 최적화를 건너뜁니다.
...
일부 UI 프레임워크는 이벤트 처리를 위해 `onClick={...}` (React/Preact) 또는 `@click="..."` (Vue)과 같은 사용자 정의 구문을 사용합니다. PPHLX는 표준 HTML을 더 밀접하게 따르며 이벤트에 사용자 정의 구문을 사용하지 않습니다.
...
한 페이지에 여러 `<AlertButton />` 컴포넌트가 있는 경우 PPHLX는 스크립트를 여러 번 실행하지 않습니다. 스크립트는 번들로 제공되며 페이지당 한 번만 포함됩니다. `querySelectorAll`을 사용하면 이 스크립트가 페이지에 있는 `alert` 클래스를 가진 모든 버튼에 이벤트 리스너를 연결합니다.

### 사용자 정의 요소가 있는 웹 컴포넌트

웹 컴포넌트 표준을 사용하여 사용자 정의 동작으로 자신만의 HTML 요소를 만들 수 있습니다. `.pphx` 컴포넌트에 [사용자 정의 요소](https://developer.mozilla.org/ko/docs/Web/API/Web_components/Using_custom_elements)를 정의하면 UI 프레임워크 라이브러리 없이도 대화형 컴포넌트를 구축할 수 있습니다.
...
<script>
  ...
  // 브라우저에 <astro-heart> 요소에 대해 PPHLXHeart 클래스를 사용하도록 지정합니다.
...
PPHLX 컴포넌트의 [프런트매터](/ko/basics/astro-components/#컴포넌트-스크립트) (`---` 펜스 내부)에 있는 코드는 서버에서 실행되며, 브라우저에서는 사용할 수 없습니다.
...
:::tip[알고 계셨나요?]
이것은 실제로 React와 같은 UI 프레임워크를 사용하여 작성된 컴포넌트에 props를 전달할 때 PPHLX가 뒤에서 수행하는 작업입니다! `client:*` 지시어가 있는 컴포넌트의 경우 PPHLX는 HTML 출력에 서버 측 props를 저장하는 `props` 속성이 있는 `<astro-island>` 사용자 정의 요소를 생성합니다.
:::
