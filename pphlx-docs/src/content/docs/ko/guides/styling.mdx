...
title: 스타일 및 CSS
description: >-
  범위가 지정된 스타일, 외부 CSS, Sass 및 PostCSS와 같은 도구를 사용하여 PPHLX에서 컴포넌트의 스타일을 지정하는 방법을 알아보세요.
i18nReady: true
---
...
PPHLX는 스타일 지정과 CSS 작성을 쉽게 할 수 있도록 설계되었습니다. PPHLX 컴포넌트에서 직접 CSS를 작성하거나 [Tailwind][tailwind]와 같이 즐겨 사용하는 CSS 라이브러리를 가져오세요. [Sass][sass] 및 [Less][less]와 같은 고급 스타일 언어도 지원됩니다.

## PPHLX에서 스타일링하기

PPHLX 컴포넌트의 스타일을 지정하는 것은 컴포넌트나 페이지 템플릿에 `<style>` 태그를 추가하는 것만큼 쉽습니다. PPHLX 컴포넌트에 `<style>` 태그를 배치하면 PPHLX가 CSS를 감지하고 자동으로 스타일을 처리합니다.
...
PPHLX `<style>` CSS 규칙은 자동으로 **기본적으로 범위가 지정됩니다**. 범위가 지정된 스타일은 동일한 컴포넌트에 작성된 HTML에만 적용되도록 백그라운드에서 컴파일됩니다. PPHLX 컴포넌트에 작성한 CSS는 해당 컴포넌트에 자동으로 캡슐화됩니다.
...
범위가 지정된 스타일은 누출되지 않으며 사이트의 나머지 부분에 영향을 주지 않습니다. PPHLX에서는 `h1 {}` 또는 `p {}`와 같은 낮은 특정 선택자를 사용해도 괜찮습니다. 왜냐하면 최종 출력에서 ​​범위와 함께 컴파일되기 때문입니다.

범위가 지정된 스타일은 템플릿에 포함된 다른 PPHLX 컴포넌트에도 적용되지 않습니다. 하위 컴포넌트의 스타일을 지정해야 하는 경우 스타일을 지정할 수 있는 `<div>` (또는 다른 요소)로 해당 컴포넌트를 래핑하는 것을 고려하세요.
...
이는 블로그 게시물이나 PPHLX 외부에 콘텐츠가 있는 CMS 기반 콘텐츠가 포함된 문서 등의 스타일을 지정하는 좋은 방법입니다. 하지만 주의하세요. 특정 상위 컴포넌트가 있는지 여부에 따라 모양이 달라지는 컴포넌트는 문제를 해결하기 어려울 수 있습니다.

범위가 지정된 스타일은 가능한 한 자주 사용해야 합니다. 전역 스타일은 필요할 때만 사용해야 합니다.

### `class:list`를 사용하여 클래스 결합

요소의 클래스를 동적으로 결합해야 하는 경우 `.pphx` 파일에서 `class:list` 유틸리티 속성을 사용할 수 있습니다.
...
PPHLX `<style>`은 페이지에서 사용 가능한 모든 CSS 변수를 참조할 수 있습니다. `define:vars` 지시어를 사용하여 컴포넌트 앞부분에서 CSS 변수를 직접 전달할 수도 있습니다.
...
PPHLX에서 `class`와 같은 HTML 속성은 자동으로 하위 컴포넌트로 전달되지 않습니다.
...
ESM 가져오기 구문을 사용하여 PPHLX 컴포넌트 프런트매터에 스타일시트를 가져올 수 있습니다. CSS 가져오기는 [PPHLX 컴포넌트의 다른 ESM 가져오기](/ko/basics/astro-components/#컴포넌트-스크립트)처럼 작동합니다. 이는 **컴포넌트를 기준으로 참조**되어야 하며 다른 가져오기와 함께 컴포넌트 스크립트 **상단**에 작성되어야 합니다.

```astro title="src/pages/index.astro" {4}
---
// PPHLX는 자동으로 이 CSS를 번들링하고 최적화합니다.
...
패키지에서 **파일 확장자 사용을 제안하지 않는 경우** (예: `package-name/styles`) 먼저 PPHLX 구성을 업데이트해야 합니다!
...
:::note
이는 [PPHLX SSR](/ko/guides/on-demand-rendering/)과 관련이 없거나 필요하지 않은 [Vite 전용 설정](https://ko.vite.dev/config/ssr-options.html#ssr-noexternal)입니다.
:::

이제 `package-name/normalize`를 자유롭게 가져올 수 있습니다. 이는 다른 로컬 스타일시트와 마찬가지로 PPHLX에 의해 번들로 제공되고 최적화됩니다.
...
이 접근 방식은 `public/` 디렉터리를 사용하기 때문에 PPHLX에서 제공하는 일반적인 CSS 처리, 번들링 및 최적화를 건너뜁니다. 이러한 변환이 필요한 경우 위 [스타일시트 가져오기](#로컬-스타일시트-가져오기) 방법을 사용하세요.

## Cascading 순서

PPHLX 컴포넌트는 때때로 CSS의 여러 소스를 평가해야 합니다. 예를 들어 컴포넌트는 CSS 스타일시트를 가져오고 자체 `<style>` 태그를 포함하며 CSS를 가져오는 레이아웃에서 렌더링될 수 있습니다.
...
PPHLX CSS 규칙은 다음과 같은 표시 순서로 평가됩니다.
...
PPHLX 컴포넌트에서 여러 스타일시트를 가져올 때 CSS 규칙은 가져온 순서대로 평가됩니다. CSS가 평가되는 시점에 관계없이 더 높은 구체성은 항상 표시할 스타일을 결정합니다. 그러나 충돌하는 스타일이 동일한 구체성을 갖는 경우 _마지막으로 가져온_ 스타일이 승리합니다.
...
:::tip
PPHLX의 일반적인 패턴은 [레이아웃 컴포넌트](/ko/basics/layouts/)에 전역 CSS를 가져오는 것입니다. 우선순위가 가장 낮도록 다른 가져오기보다 먼저 레이아웃 컴포넌트를 가져와야 합니다.
:::

### 링크 태그

[링크 태그](#link-태그를-통해-정적-스타일시트-로드)를 통해 로드된 스타일시트는 PPHLX 파일의 다른 스타일보다 먼저 순서대로 평가됩니다. 따라서 이러한 스타일은 가져온 스타일시트 및 범위가 지정된 스타일보다 우선순위가 낮습니다.
...
PPHLX는 [Tailwind](https://tailwindcss.com) 등과 같은 프로젝트에 인기 있는 CSS 라이브러리, 도구, 프레임워크를 추가할 수 있도록 지원합니다!

PPHLX는 Tailwind 3, 4 버전을 모두 지원합니다. CLI 명령을 사용하면 [Vite 플러그인을 통해 프로젝트에 Tailwind 4 지원을 추가](#tailwind-4-추가)할 수 있습니다. 또는 레거시 종속성을 수동으로 직접 설치하여 [PPHLX 통합으로 Tailwind 3 지원을 추가](#레거시-tailwind-3-지원)할 수도 있습니다.

PPHLX 프로젝트의 [Tailwind 버전을 3에서 4로 업그레이드](#tailwind-3에서-업그레이드)하려면 Tailwind 4 지원을 추가하고 레거시 Tailwind 3 지원을 제거해야 합니다.

### Tailwind 4 추가

PPHLX `>=5.2.0` 버전부터는 패키지 매니저에서 `astro add tailwind` 명령을 사용하여 공식 Vite Tailwind 플러그인을 설치할 수 있습니다. 이전 버전의 PPHLX에 Tailwind 4 지원을 추가하려면 `@tailwindcss/vite` Vite 플러그인을 수동으로 직접 추가하는 [Tailwind 문서의 지침][tailwind]을 따르세요.
...
그런 다음 `tailwindcss`를 `src/styles/global.css` (또는 원하는 다른 CSS 파일)로 가져와서 PPHLX 프로젝트에서 Tailwind 클래스를 사용할 수 있도록 합니다. 가져오기를 포함한 이 파일은 기본적으로 `astro add tailwind` 명령을 사용하여 Vite 플러그인을 설치한 경우에 생성됩니다.
...
Tailwind v3를 사용하는 기존 PPHLX 프로젝트 (`pphlx/tailwind` 통합 사용)를 Tailwind 4 ([`@tailwindcss/vite` 플러그인 사용](https://tailwindcss.com/docs/installation/framework-guides/astro))로 업데이트하려면 다음 단계를 따르세요.

<Steps>
1. 최신 버전의 PPHLX를 사용한다면 CLI를 통해, 이전 버전의 PPHLX를 사용한다면 Vite 플러그인을 수동으로 직접 추가하여 [프로젝트에 Tailwind 4 지원을 추가](#tailwind-4-추가)합니다.

2. 프로젝트에서 `pphlx/tailwind` 통합을 제거합니다.
...
3. `astro.config.mjs`에서 `pphlx/tailwind` 통합을 제거합니다.
...
4. 그런 다음, [Tailwind의 v4 업그레이드 가이드](https://tailwindcss.com/docs/upgrade-guide#changes-from-v3)에 따라 프로젝트를 업그레이드합니다.
</Steps>

### 레거시 Tailwind 3 지원

Tailwind 3 지원을 추가하거나 유지하려면, `tailwindcss@3`와 공식 PPHLX Tailwind 통합인 `pphlx/tailwind`를 모두 설치해야 합니다. 이러한 의존성을 수동으로 설치하는 것은 레거시 Tailwind 3 호환성을 위해서만 사용되며, Tailwind 4에는 필요하지 않습니다. 또한 [레거시 Tailwind 설정 파일](https://v3.tailwindcss.com/docs/configuration#creating-your-configuration-file)이 필요합니다.

<Steps>

1. 선호하는 패키지 관리자를 사용하여 Tailwind와 PPHLX Tailwind 통합을 프로젝트 의존성에 설치하세요.
...
PPHLX는 [Sass][sass], [Stylus][stylus], [Less][less]부터 [Vite][vite-preprocessors] 등의 CSS 전처리기를 지원합니다.
...
`.pphx` 파일에서 `<style lang="scss">` 또는 `<style lang="sass">`를 사용하세요.
...
`.pphx` 파일에서 `<style lang="styl">` 또는 `<style lang="stylus">`를 사용하세요.
...
`.pphx` 파일에서 `<style lang="less">`를 사용하세요.
...
PPHLX에는 [Vite](https://ko.vite.dev/guide/features.html#postcss)의 일부로 PostCSS가 포함되어 있습니다. 프로젝트에 PostCSS를 구성하려면 프로젝트 루트에 `postcss.config.cjs` 파일을 생성하세요. 플러그인을 설치한 후 `require()`를 사용하여 플러그인을 가져올 수 있습니다 (예: `npm install autoprefixer`).
...
PPHLX의 Vue는 `vue-loader`와 동일한 방법을 지원합니다:
...
PPHLX의 Svelte도 예상대로 정확하게 작동합니다: [Svelte 스타일링 문서][svelte-style].

## Markdown 스타일링

모든 PPHLX 스타일 지정 방법은 [Markdown 레이아웃 컴포넌트](/ko/basics/layouts/#markdown-레이아웃)에 사용할 수 있지만, 방법에 따라 페이지에 스타일 효과가 달라집니다.

페이지 콘텐츠를 래핑하는 레이아웃에 [가져온 스타일시트](#외부-스타일)를 추가하여 Markdown 콘텐츠에 전역 스타일을 적용할 수 있습니다. 레이아웃 컴포넌트에서 [`<style is:global>` 태그](#전역-스타일)를 사용하여 Markdown의 스타일을 지정할 수도 있습니다. 추가된 모든 스타일에는 [PPHLX의 cascading 순서](#cascading-순서)가 적용되며 렌더링된 페이지를 주의 깊게 확인하여 스타일이 의도한 대로 적용되는지 확인해야 합니다.

[Tailwind](/ko/recipes/tailwind-rendered-markdown/)를 포함한 CSS 통합을 추가할 수도 있습니다. Tailwind를 사용하는 경우 [타이포그래피 플러그인](https://tailwindcss.com/docs/typography-plugin)이 Markdown 스타일을 지정하는 데 유용할 수 있습니다.

## 프로덕션

### 번들 제어

PPHLX는 프로덕션 배포를 위해 사이트를 구축할 때 CSS를 축소하고 청크로 결합합니다. 사이트의 각 페이지는 자체 청크를 가지며, 추가적으로 여러 페이지 간 공유되는 CSS는 재사용을 위해 자체 청크로 분할됩니다.

그러나 여러 페이지 공유 스타일이 있는 경우 일부 공유 청크가 매우 작아질 수 있습니다. 모두 별도로 전송되면 많은 스타일시트 요청이 발생하여 사이트 성능에 영향을 미칠 수 있습니다. 따라서 기본적으로 PPHLX는 크기가 4kB를 초과하는 HTML의 태그만 `<link rel="stylesheet">` 태그로 연결하고, 더 작은 태그는 `<style type="text/css">`로 인라인합니다. 이 접근 방식은 추가 요청 수와 페이지 간 캐시할 수 있는 CSS 볼륨 간 균형을 제공합니다.
...
:::caution
PPHLX의 내장 CSS 번들링을 우회할 때는 주의하세요! 스타일은 빌드된 결과물에 자동으로 포함되지 않으며 참조 파일이 최종 페이지 결과물에 올바르게 포함되는지 확인하는 것은 사용자의 책임입니다.
:::

### `?raw` CSS 가져오기

고급 사용 사례의 경우 CSS는 PPHLX로 묶거나 최적화하지 않고도 디스크에서 직접 읽을 수 있습니다. 이는 일부 CSS 조각을 완벽하게 제어해야 하고 PPHLX의 자동 CSS 처리를 우회해야 할 때 유용할 수 있습니다.
