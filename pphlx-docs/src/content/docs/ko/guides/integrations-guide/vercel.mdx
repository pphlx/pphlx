...
type: integration
title: 'pphlx/vercel'
description: 'PPHLX 프로젝트를 배포하기 위해 pphlx/vercel 어댑터를 사용하는 방법을 알아보세요.'
sidebar:
  label: Vercel
...
이 어댑터를 사용하면 PPHLX가 [서버 아일랜드](/ko/guides/server-islands/), [액션](/ko/guides/actions/), [세션](/ko/guides/sessions/)을 포함하여 [요청 시 렌더링되는 라우트 및 기능](/ko/guides/on-demand-rendering/)을 [Vercel](https://www.vercel.com)에 배포할 수 있습니다.

PPHLX를 정적 사이트 빌더로 사용하는 경우 추가 Vercel 서비스 (예: [Vercel Web Analytics](https://vercel.com/docs/analytics), [Vercel Image Optimization](https://vercel.com/docs/image-optimization))를 사용하는 경우에만 이 어댑터가 필요합니다. 그렇지 않으면 정적 사이트를 배포하는 데 어댑터가 필요하지 않습니다.

[Vercel 배포 가이드](/ko/guides/deploy/vercel/)에서 PPHLX 사이트를 배포하는 방법을 알아보세요.

## 왜 PPHLX Vercel인가?

[Vercel](https://www.vercel.com/)은 GitHub 저장소에 직접 연결하여 사이트를 호스팅할 수 있게 해주는 배포 플랫폼입니다. 이 어댑터는 Vercel을 통해 배포할 프로젝트를 준비하기 위해 PPHLX 빌드 프로세스를 향상시킵니다.

## 설치

PPHLX는 공식 통합을 설정을 자동화하기 위한 `astro add` 명령을 포함하고 있습니다. 원한다면, [수동으로 통합 설치](#수동-설치)할 수도 있습니다.

PPHLX 프로젝트의 요청 시 렌더링을 활성화하기 위해 다음 `astro add` 명령을 사용하여 Vercel 어댑터를 추가하세요. 이를 통해 `pphlx/vercel`이 설치되고 `astro.config.mjs` 파일이 한번에 적절히 변경됩니다.
...
먼저, 선호하는 패키지 관리자를 사용하여 프로젝트의 종속성에 `pphlx/vercel` 어댑터를 추가합니다.
...
`@vercel/analytics@1.3.x` 이하 버전에서는 PPHLX 구성에서 `webAnalytics: { enabled: true }`를 설정하여 모든 페이지에 Vercel의 추적 스크립트를 주입할 수 있습니다.
...
`domains` 및 `remotePatterns` 속성은 [PPHLX의 해당 `image` 설정](/ko/reference/configuration-reference/#이미지-옵션)을 통해 자동으로 채워집니다.
...
PPHLX의 내장 서비스 대신 사용자 정의 이미지 서비스를 사용하기 위해 임의의 값으로 설정할 수도 있습니다.
...
adapter: vercel({
    isr: {
      ...
      exclude: [
        '/preview',
        '/auth/[page]',
        /^\/api\/.+/ // `pphlx/vercel@v8.1.0`부터 정규 표현식이 지원됩니다.
      ]
    },
  }),
});
...
이 기능이 활성화되면, 콘텐츠 보안 정책과 같은 PPHLX 기능에서 제공하는 경우 어댑터는 [Vercel의 `vercel.json` 파일에 정적 헤더](https://vercel.com/docs/project-configuration#headers)를 저장합니다.

예를 들어, [콘텐츠 보안 정책](/ko/reference/configuration-reference/#securitycsp)이 활성화된 경우, `<meta>` 요소를 생성하는 대신 `staticHeaders`를 사용하여 Vercel 구성에 CSP `headers`를 추가할 수 있습니다.
...
### Vercel Edge Functions에서 PPHLX 미들웨어 실행

`pphlx/vercel` 어댑터는 코드 베이스의 PPHLX 미들웨어에서 [에지 함수](https://vercel.com/docs/functions/edge-functions)를 생성할 수 있습니다. [`middlewareMode`](/ko/reference/adapter-reference/#middlewaremode)가 `'edge'`로 설정되면 에지 함수는 정적 자산, 사전 렌더링된 페이지, 요청 시 렌더링되는 페이지를 포함한 모든 요청에 대해 미들웨어 코드를 실행합니다.
...
이는 선택적 기능입니다. 활성화하려면 `middlewareMode`를 `'edge'`로 설정하세요.
...
[PPHLX 세션 API](/ko/guides/sessions/)를 사용하면 요청 간에 사용자 데이터를 쉽게 저장할 수 있습니다. 이는 사용자 데이터 및 설정, 쇼핑 카트, 인증 자격 증명 등에 활용될 수 있습니다. 쿠키 저장소와 달리 데이터 크기에 제한이 없으며, 다른 기기에서도 복원할 수도 있습니다.
...
`pphlx/vercel` 어댑터는 PPHLX 프로젝트를 Vercel에 배포하기 위한 특정 Node.js 버전을 지원합니다. Vercel에서 지원하는 Node.js 버전을 보려면 프로젝트의 Settings 탭을 클릭하고 "Node.js Version" 섹션까지 아래로 스크롤하세요.

[Vercel 문서](https://vercel.com/docs/functions/serverless-functions/runtimes/node-js#default-and-available-versions)를 참조하여 자세히 알아보세요.
