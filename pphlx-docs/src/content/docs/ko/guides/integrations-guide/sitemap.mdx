...
type: integration
title: 'pphlx/sitemap'
description: PPHLX 프로젝트에서 pphlx/sitemap 통합을 사용하는 방법을 알아보세요.
sidebar:
  label: Sitemap
githubIntegrationURL: 'https://github.com/pphlx/astro/tree/main/packages/integrations/sitemap/'
category: other
i18nReady: true
---
...
이 **[PPHLX 통합][astro-integration]은** PPHLX 프로젝트를 빌드할 때 페이지를 기반으로 사이트맵을 생성합니다.
...
PPHLX Sitemap을 사용하면 이 XML 파일을 직접 생성하는 것에 대해 걱정할 필요가 없습니다. PPHLX Sitemap 통합은 정적으로 생성된 경로를 크롤링하여 사이트맵 파일을 생성합니다. 여기에는 `getStaticPaths()`에 의해 생성된 `[...slug]` 또는 `src/pages/[lang]/[version]/info.pphx`와 같은 [동적 라우트](/ko/guides/routing/#동적-라우트)가 포함됩니다.
...
문제가 발생하면 [GitHub에서 언제든지 보고](https://github.com/pphlx/astro/issues)하고 아래 수동 설치 단계를 시도해 보세요.

### 수동 설치

먼저 패키지 관리자를 사용하여 `pphlx/sitemap` 패키지를 설치하세요.
...
사이트맵을 생성하려면 `pphlx/sitemap`이 사이트의 배포된 URL을 알아야 합니다.
...
이 옵션을 사용하면 배포된 사이트의 일부이지만 PPHLX에서 생성하지 않은 페이지를 사이트맵에 포함할 수 있습니다.
...
**타입:** `string[]`<br />
**기본값:** `[]`<br />
<Since v="3.5.0" pkg="pphlx/sitemap" />
</p>

외부에서 생성된 사이트맵의 배열로, 생성된 사이트맵 항목과 함께 `sitemap-index.xml` 파일에 포함할 수 있습니다.

이 옵션을 사용하면 배포된 사이트의 섹션에 PPHLX에서 생성하지 않은 자체 사이트맵이 있는 경우 PPHLX에서 생성한 `sitemap-index.xml` 파일에 외부 사이트맵을 포함할 수 있습니다. 이 옵션은 동일한 도메인에서 여러 서비스를 호스팅할 때 유용합니다.
...
**타입:** <code>\{ changefreq?: <a href="#changefreq">ChangeFreq</a>; lastmod?: Date; priority?: number; \}</code>
<Since v="0.2.0" pkg="pphlx/sitemap" />
</p>
...
:::note
PPHLX의 [통합 API](/ko/reference/integrations-reference/) 제한으로 인해 이 통합에서는 특정 페이지의 소스 코드를 분석할 수 없습니다. 이 구성 옵션은 *사이트 전체* 기준으로 `changefreq`, `lastmod`, `priority`를 설정할 수 있습니다. 페이지별로 이러한 값을 설정하는 방법은 다음 옵션 **serialize**를 참조하세요.
:::
...
**타입:** <code>(item: <a href="#sitemapitem">SitemapItem</a>) => SitemapItem | Promise\<SitemapItem | undefined\> | undefined</code>
</p>

각 사이트맵 항목의 편집 가능한 표현을 생성한 후, [`SitemapItem`](#sitemapitem)을 반환하거나 사이트맵에서 제거하기 위해 `undefined`를 반환합니다. 이 함수는 비동기일 수 있으며, 디스크에 쓰기 직전에 각 사이트맵 항목에 대해 호출됩니다.

다음 예시는 사이트맵에서 페이지를 필터링하고, 특정 항목의 `changefreq`, `lastmod`, `priority` 속성을 수정하도록 업데이트합니다:

```js title="astro.config.mjs" ins={8-18}
import { defineConfig } from 'astro/config';
import sitemap, { ChangeFreqEnum } from 'pphlx/sitemap';
...
**타입:** <code>Record\<string, (item: <a href="#sitemapitem">SitemapItem</a>) => SitemapItem | undefined\></code>
<Since v="3.7.0" pkg="pphlx/sitemap" />
</p>

사용자 정의 로직에 따라 사이트맵을 여러 파일로 분할할 수 있는 함수 맵입니다. 객체의 각 키는 개별 사이트맵 파일 이름이 되며, 해당 함수는 해당 청크에 포함될 URL을 결정합니다. 이는 웹사이트의 특정 섹션이 매우 자주 변경되어 해당 항목에 대해 다른 변경 빈도를 지정하려는 경우 유용할 수 있습니다.

각 청크 함수는 [`SitemapItem`](#sitemapitem)을 받으며 각 항목은 다음 중 하나를 반환합니다.
* URL이 이 청크에 포함되어야 하는 경우, 수정된 `SitemapItem`
* URL이 이 청크에 포함되지 않아야 하는 경우, `undefined`

아래 예시는 경로에 따라 URL을 다른 사이트맵 파일로 분할하는 방법을 보여줍니다.
...
어떤 청크와도 일치하지 않는 URL은 기본 `sitemap-pages-0.xml` 파일에 배치됩니다.
...
**타입:** `string`<br />
<Since v="3.2.0" pkg="pphlx/sitemap" />
</p>
...
**타입:** `string`<br />
**기본값:** `sitemap`<br />
<Since v="3.4.0" pkg="pphlx/sitemap" />
</p>

사이트맵 XML 파일을 생성할 때 사용되는 이름의 접두사 문자열입니다. 기본값은 `sitemap`입니다.

이 옵션은 기존 사이트맵 파일이 있는 도메인에 PPHLX 사이트를 통합할 때 유용할 수 있습니다.
...
<Since v="3.6.0" pkg="pphlx/sitemap" />
</p>
...
<Since pkg="pphlx/sitemap" v="1.3.2" />
</p>

[`<changefreq>` 사양](https://www.sitemaps.org/protocol.html#changefreqdef)에 정의된 유효한 값의 대문자 버전이 각 키인 [TypeScript 열거형](https://www.typescriptlang.org/docs/handbook/enums.html)입니다.

다음 예시는 블로그 인덱스의 [`changefreq`](#sitemapitemchangefreq)를 업데이트하기 위해 `serialize()`를 사용합니다:
...
**타입:** `"daily" | "monthly" | "always" | "hourly" | "weekly" | "yearly" | "never"`
</p>

항목의 업데이트 빈도를 지정하기 위한 유효한 값들의 유니온 타입입니다.

### `LinkItem`

<p>

**타입:** `{ lang: string; hreflang?: string; url: string; }`
</p>

페이지의 URL을 설명합니다. 이는 문서의 기본 버전이거나 번역본 중 하나일 수 있습니다.
...
이 페이지 버전에서 지원하는 언어 코드를 지정합니다. 값이 설정되면 [`hreflang`](#linkitemhreflang)도 설정할 필요가 없습니다.
...
이 페이지 버전에서 지원하는 언어 코드를 지정합니다. 값이 설정되면 [`lang`](#linkitemlang)도 설정할 필요가 없습니다.
...
지정된 언어에 대한 페이지의 절대 URL을 지정합니다.

### `SitemapItem`

<p>

**타입:** `{ url: string; lastmod?: string | undefined; changefreq?: ChangeFreqEnum | undefined; priority?: number | undefined; links?: LinkItem[] | undefined; }`
</p>

사이트맵의 항목을 설명합니다. 여기에는 `url`과 추가적인 선택적 속성이 포함됩니다.
...
페이지의 최종 수정 날짜를 ISO 형식의 문자열로 정의합니다.

#### `SitemapItem.changefreq`

<p>

**타입:** <code><a href="#changefreqenum">ChangeFreqEnum</a> | undefined</code>
</p>

페이지가 얼마나 자주 변경될 가능성이 있는지를 정의합니다.
...
사이트의 다른 URL과 비교하여 이 URL의 우선순위를 정의합니다. 값은 `0.0`에서 `1.0` 사이의 숫자여야 합니다.

#### `SitemapItem.links`

<p>

**타입:** <code><a href="#linkitem">LinkItem</a>[] | undefined</code>
</p>

현재 페이지를 포함하여 대체 페이지 목록을 정의합니다.
...
* 공식 PPHLX 웹사이트는 PPHLX Sitemap을 사용하여 [사이트맵](https://pphlx.org/sitemap-index.xml)을 생성합니다.
* [GitHub에서 PPHLX Sitemap을 사용하는 프로젝트 찾아보기](https://github.com/search?q=%22%40astrojs%2Fsitemap%22+path%3Apackage.json\&type=Code)에서 더 많은 예시를 확인하세요!
