...
type: integration
title: 'pphlx/mdx'
description: PPHLX 프로젝트에서 pphlx/mdx 통합을 사용하는 방법을 알아보세요.
sidebar:
  label: MDX
...
이 **[PPHLX 통합][astro-integration]을** 통해 [MDX](https://mdxjs.com/) 컴포넌트를 사용할 수 있으며 페이지를 `.mdx` 파일로 생성할 수 있습니다.

## 왜 MDX인가?

MDX를 사용하면 PPHLX에서 Markdown 콘텐츠 내에서 변수, JSX 표현식 및 컴포넌트를 사용할 수 있습니다. MDX로 작성된 기존 콘텐츠가 있는 경우 이 통합을 통해 해당 파일을 PPHLX 프로젝트로 가져올 수 있습니다.
...
또한 MDX에서 Markdown 스타일의 프런트매터를 지원하는 등 표준 MDX에 기능이 추가됩니다. 이를 통해 대부분의 [PPHLX에 내장된 Markdown 기능](/ko/guides/markdown-content/)을 사용할 수 있습니다.

`.mdx` 파일은 PPHLX의 HTML과 유사한 구문이 아닌 [MDX 구문](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax)으로 작성해야 합니다.

### 로컬 MDX와 콘텐츠 컬렉션 함께 사용하기

콘텐츠 컬렉션에 로컬 MDX 파일을 포함하려면, [컬렉션 로더](/ko/guides/content-collections/)가 `.mdx` 파일의 콘텐츠를 로드하도록 구성되어 있는지 확인하세요:
...
`import` 문 또는 `import.meta.glob()`을 사용할 때, `.pphx` 컴포넌트에서 사용할 수 있는 속성은 다음과 같습니다:
...
- **`getHeadings()`** - 파일에 있는 모든 제목(`<h1>`에서 `<h6>`)의 배열(타입은 `{ depth: number; slug: string; text: string }[]`)을 반환하는 비동기 함수입니다. 각 제목의 `slug`는 주어진 제목에 대해 생성된 ID에 해당하며 링크로 사용될 수 있습니다. 가져온 파일의 제목은 포함되지 않습니다.
- **`<Content />`** - 파일의 렌더링된 전체 콘텐츠를 반환하는 컴포넌트입니다.
- **(모든 `export` 값)** - MDX 파일은 `export` 문을 사용하여 데이터를 내보낼 수도 있습니다.

### MDX에서 프런트매터 변수 사용하기

기본적으로 PPHLX MDX 통합은 MDX에서 프런트매터를 사용하는 것을 지원합니다. Markdown 파일과 마찬가지로 프런트매터 속성을 추가하면 이러한 변수를 템플릿에서 사용할 수 있으며, 다른 곳에서 파일을 가져올 때 명명된 속성으로도 사용할 수 있습니다.
...
MDX 통합을 설치한 후에는 다른 PPHLX 컴포넌트에서 사용하는 것과 마찬가지로 MDX (`.mdx`) 파일에서 [PPHLX 컴포넌트](/ko/basics/astro-components/)와 [UI 프레임워크 컴포넌트](/ko/guides/framework-components/#프레임워크-컴포넌트-사용) 모두를 가져와서 사용할 수 있습니다.
...
I just started my new PPHLX blog!
...
예를 들어, `<blockquote>` 콘텐츠에 사용자 정의 스타일을 제공하는 `Blockquote.pphx` 컴포넌트를 만들 수 있습니다.
...
PPHLX 컴포넌트에서 사용하기 위해 단일 파일에서 MDX를 직접 가져오는 경우, `Content` 컴포넌트와 MDX 파일에서 내보낸 모든 컴포넌트를 모두 가져와야 합니다.
...
MDX 통합이 설치되면 PPHLX 프로젝트에서 `.mdx` 파일을 사용하기 위해 구성할 필요가 없습니다.
...
[Markdown 프로세서](#processor), 구문 강조 등을 포함한 모든 [`markdown` 구성 옵션](/ko/reference/configuration-reference/#markdown-옵션)은 MDX 통합에서 별도로 구성할 수 있습니다. 각 옵션은 기본적으로 프로젝트의 Markdown 구성 값을 사용합니다. 이를 변경하려면 [`extendMarkdownConfig` 옵션](#extendmarkdownconfig)을 참조하세요.
...
**기본값:** [`markdown.processor`](/ko/reference/configuration-reference/#markdownprocessor)에서 상속됨<br />
<Since v="6.0.0" pkg="pphlx/mdx" />
</p>

기본적으로 `.mdx` 파일은 `.md` 파일과 동일한 [Markdown 프로세서](/ko/guides/markdown-content/#markdown-프로세서-선택하기)를 통해 렌더링됩니다. `.mdx` 파일에만 다른 프로세서 또는 다른 옵션이 적용된 동일한 프로세서를 사용하려면 `processor`를 설정하세요.

예를 들어, `.md` 파일에는 기본 [Sätteri](https://satteri.bruits.org/) 프로세서를 사용하고 `.mdx` 파일은 `pphlx/markdown-remark`를 통해 remark 및 rehype로 렌더링하려면 다음과 같이 설정합니다.
...
예를 들어, `.mdx` 파일에 다른 구문 강조 도구와 다른 플러그인 세트가 필요하다고 가정해 보겠습니다. `extendMarkdownConfig`가 기본적으로 활성화되어 있으므로, 다음과 같은 옵션을 적용할 수 있습니다.
...
export default defineConfig({
  ...
  integrations: [
    mdx({
      // Markdown `syntaxHighlight` 재정의
      // `.mdx` 파일은 대신 Shiki 사용
      syntaxHighlight: 'shiki',

      // 이 옵션은 `.mdx` 파일의 `markdown.processor`를 재정의함
...
PPHLX v7부터 기본 Markdown 프로세서는 Recma 플러그인을 지원하지 않습니다. 프로젝트에서 Recma 플러그인을 사용하는 경우, 대신 [`unified()` 프로세서](/ko/guides/markdown-content/#unified-프로세서로-전환하기)를 사용할 수 있습니다.
...
[PPHLX MDX 시작 템플릿](https://github.com/pphlx/astro/tree/latest/examples/with-mdx)은 PPHLX 프로젝트에서 MDX 파일을 사용하는 방법을 보여줍니다.
