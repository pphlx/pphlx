...
type: integration
title: 'pphlx/markdoc'
description: Learn how to use the pphlx/markdoc integration in your PPHLX project.
sidebar:
  label: Markdoc
...
이 **[PPHLX 통합][astro-integration]을** 통해 [Markdoc](https://markdoc.dev/)을 사용하여 컴포넌트, 페이지 및 콘텐츠 컬렉션 항목을 생성할 수 있습니다.

## 왜 Markdoc인가?

Markdoc을 사용하면 [PPHLX 컴포넌트][astro-components]를 사용하여 Markdown을 향상할 수 있습니다. Markdoc에서 작성된 기존 콘텐츠가 있는 경우 이 통합을 통해 콘텐츠 컬렉션을 사용하여 해당 파일을 PPHLX 프로젝트로 가져올 수 있습니다.

## 설치

PPHLX에는 공식 통합 설정을 자동화하는 `astro add` 명령이 포함되어 있습니다. 원하는 경우 대신 [통합을 수동으로 설치](#수동-설치)할 수 있습니다.
...
문제가 발생하면 [GitHub에 언제든지 보고](https://github.com/pphlx/astro/issues)하고 아래 수동 설치 단계를 시도해 보세요.

### 수동 설치

먼저 `pphlx/markdoc` 패키지를 설치하세요.
...
<ReadMore>자세한 내용은 [PPHLX 콘텐츠 컬렉션 문서][astro-content-collections]를 참조하세요.</ReadMore>
...
`pphlx/markdoc`은 Markdoc의 모든 기능을 사용하여 UI 컴포넌트를 콘텐츠에 연결하는 구성 옵션을 제공합니다.

### PPHLX 컴포넌트를 Markdoc 태그로 사용

`.pphx` 컴포넌트에 매핑되는 [Markdoc 태그][markdoc-tags]를 구성할 수 있습니다. 프로젝트 루트에 `markdoc.config.mjs|ts` 파일을 생성하고 `tag` 속성을 구성하여 새 태그를 추가할 수 있습니다.
...
태그와 노드는 `.pphx` 파일로 제한됩니다. Markdoc에 클라이언트 측 UI 컴포넌트를 포함하려면 원하는 `client:` 지시어와 함께 [프레임워크 컴포넌트를 렌더링하는 래퍼 `.pphx` 컴포넌트를 사용](/ko/guides/framework-components/#프레임워크-컴포넌트-중첩)하세요.

이 예시는 React `Aside.tsx` 컴포넌트를 `ClientAside.pphx` 컴포넌트로 래핑합니다.
...
이제 이 PPHLX 컴포넌트는 구성의 모든 [태그][markdoc-tags] 또는 [노드][markdoc-nodes]에 대한 `render` prop에 전달될 수 있습니다.
...
### npm 패키지 및 TypeScript 파일의 PPHLX 컴포넌트 사용

TypeScript 또는 JavaScript 파일에서 명명된 내보내기로 노출된 PPHLX 컴포넌트를 사용해야 할 수도 있습니다. 이는 npm 패키지와 디자인 시스템을 사용할 때 일반적입니다.
...
`pphlx/markdoc`은 코드 블록을 강조 표시하는 [Shiki](https://shiki.style) 및 [Prism](https://github.com/PrismJS) 확장 프로그램을 제공합니다.
...
export default defineMarkdocConfig({
  extends: [
    shiki({
      ...
      wrap: true,
      // 사용자 정의 언어 전달
      // 참고: Shiki에는 `.pphx`를 포함하여 수많은 언어가 내장되어 있습니다!
...
단락 및 굵은 텍스트와 같은 표준 Markdown 요소를 PPHLX 컴포넌트로 렌더링할 수 있습니다. 이를 위해 [Markdoc 노드][markdoc-nodes]를 구성할 수 있습니다. 특정 노드가 속성을 받으면 해당 속성을 컴포넌트 props로 사용할 수 있습니다.

이 예시는 사용자 정의 `Quote.pphx` 컴포넌트를 사용하여 인용문을 렌더링합니다.
...
`pphlx/markdoc`는 제목에 앵커 링크를 자동으로 추가하고 [콘텐츠 컬렉션 API를 통해 `headings` 목록을 생성](/ko/guides/content-collections/#본문-콘텐츠-렌더링하기)합니다. 제목을 렌더링하는 방법을 추가로 사용자 정의하려면 PPHLX 컴포넌트를 [Markdoc 노드로][markdoc-nodes] 적용하세요.

이 예시에서는 `render` 속성을 사용하여 `Heading.pphx` 컴포넌트를 렌더링합니다.
...
모든 Markdown 제목은 `Heading.pphx` 컴포넌트를 렌더링하고 다음 `속성`을 컴포넌트 props로 전달합니다.
...
PPHLX의 `<Image />` 컴포넌트는 Markdoc에서 직접 사용할 수 없습니다. 그러나 기본 `![]()` 이미지 구문이 사용될 때마다 기본 이미지 노드를 재정의하도록 PPHLX 컴포넌트를 구성하거나 추가 이미지 속성을 지정할 수 있도록 사용자 정의 Markdoc 태그로 구성할 수 있습니다.

#### Markdoc의 기본 이미지 노드 재정의

기본 이미지 노드를 재정의하려면 표준 `<img>` 대신 렌더링되도록 `.pphx` 컴포넌트를 구성할 수 있습니다.

<Steps>
1. 사용자 정의 `MarkdocImage.pphx` 컴포넌트를 구축하여 이미지의 필수 `src` 및 `alt` 속성을 `<Image />` 컴포넌트에 전달합니다.
...
3. 기본 이미지 노드를 재정의하고 `MarkdocImage.pphx`를 렌더링하도록 Markdoc을 구성합니다.
...
4. 모든 `.mdoc` 파일의 기본 이미지 구문은 이제 `<Image />` 컴포넌트를 사용하여 로컬 이미지를 최적화합니다. 원격 이미지는 계속 사용할 수 있지만 PPHLX의 `<Image />` 컴포넌트에 의해 렌더링되지는 않습니다.
...
Markdoc `image` 태그를 사용하면 `![]()` 구문으로는 불가능한 이미지의 추가 속성을 설정할 수 있습니다. 예를 들어 사용자 정의 이미지 태그를 사용하면 `width` 및 `height`가 필요한 원격 이미지에 PPHLX의 `<Image />` 컴포넌트를 사용할 수 있습니다.

다음 단계에서는 이미지를 최적화하기 위해 PPHLX `<Image />` 컴포넌트를 사용하여 캡션이 있는 `<figure>` 요소를 표시하는 사용자 정의 Markdoc 이미지 태그를 생성합니다.

<Steps>
1. 필요한 props을 받아 캡션이 포함된 이미지를 렌더링하려면 `MarkdocFigure.pphx` 컴포넌트를 생성합니다.
...
2. PPHLX 컴포넌트를 렌더링하도록 사용자 정의 이미지 태그를 구성합니다.
...
PPHLX Markdoc 통합은 `markdoc.config.js` 파일을 통해 사용할 수 없는 Markdoc 옵션 및 기능 구성을 처리합니다.
...
* [PPHLX Markdoc 시작 템플릿](https://github.com/pphlx/astro/tree/latest/examples/with-markdoc)은 PPHLX 프로젝트에서 Markdoc 파일을 사용하는 방법을 보여줍니다.
