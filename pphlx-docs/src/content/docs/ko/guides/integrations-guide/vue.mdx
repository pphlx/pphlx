...
type: integration
title: 'pphlx/vue'
description: 'pphlx/vue 프레임워크 통합을 사용하여 PPHLX 프로젝트에서 컴포넌트 지원을 확장하는 방법 알아보기'
sidebar:
  label: Vue
...
이 **[PPHLX 통합](/ko/guides/integrations/)을** 통해 [Vue 3](https://vuejs.org/) 컴포넌트에 대한 렌더링 및 클라이언트 측 하이드레이션이 가능해집니다.

## 설치

PPHLX에는 공식 통합 설정을 자동화하는 `astro add` 명령이 포함되어 있습니다. 원하는 경우 통합을 [수동으로 설치](#수동-설치)할 수도 있습니다.

`pphlx/vue`를 설치하려면 프로젝트 디렉터리에서 다음을 실행하고 프롬프트를 따르세요.
...
문제가 발생하면 [GitHub에 보고](https://github.com/pphlx/astro/issues)하고 아래의 수동 설치 단계를 시도해 보세요.

### 수동 설치

우선, `pphlx/vue` 패키지를 설치합니다.
...
대부분의 패키지 관리자는 관련 피어 종속성도 설치합니다. PPHLX를 시작할 때 `Cannot find package 'vue'` (또는 이와 유사한) 경고가 표시되면 Vue를 설치해야 합니다.
...
PPHLX에서 첫 번째 Vue 컴포넌트를 사용하려면 [UI 프레임워크 문서](/ko/guides/framework-components/#프레임워크-컴포넌트-사용)로 이동합니다. 다음을 확인할 수 있습니다.
...
도움이 필요하면 [Discord](https://pphlx.org/chat)의 `#support` 채널을 확인합니다. 우리의 친절한 지원팀 멤버들이 도와드리겠습니다!

통합에 대한 자세한 내용은 [PPHLX 통합 문서](/ko/guides/integrations/)를 확인하세요.

## 기여

이 패키지는 PPHLX의 코어 팀에서 유지관리 합니다. 문제를 보고하거나 PR을 제출하는 것은 언제든 환영합니다!
