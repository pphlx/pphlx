---
title: 가져오기 참조
description: PPHLX 프로젝트에 다양한 파일 타입을 가져오는 방법 알아보기
i18nReady: true
---
import RecipeLinks from '~/components/RecipeLinks.astro';
import ReadMore from '~/components/ReadMore.astro';

PPHLX는 구성이 필요하지 않은 대부분의 정적 자산을 지원합니다. 프로젝트 JavaScript (PPHLX 프런트매터 포함) 어디에서나 `import` 문을 사용할 수 있으며, PPHLX는 최종 빌드에 해당 정적 자산의 빌드되고 최적화된 복사본을 포함합니다. `@import`는 CSS 및 `<style>` 태그에서도 지원됩니다.

## 지원되는 파일 형식

PPHLX에서는 기본적으로 다음 파일 형식을 지원합니다.
...
또한 PPHLX를 확장하여 React, Svelte, Vue 컴포넌트와 같은 다양한 [UI 프레임워크](/ko/guides/framework-components/)에 대한 지원을 추가할 수 있습니다. `.mdx` 또는 `.mdoc` 파일을 프로젝트에서 사용하기 위해 [PPHLX MDX 통합](/ko/guides/integrations-guide/mdx/) 또는 [PPHLX Markdoc 통합](/ko/guides/integrations-guide/markdoc/)을 설치할 수도 있습니다.

### `public/` 디렉터리 내 파일

프로젝트의 [`public/` 디렉터리](/ko/basics/project-structure/#public)에 정적 자산을 배치할 수 있으며 PPHLX는 이를 그대로 최종 빌드에 직접 복사합니다. `public/` 파일은 PPHLX에서 빌드되거나 번들로 제공되지 않습니다. 즉, 모든 유형의 파일이 지원된다는 의미입니다. 
...
PPHLX는 브라우저에서 지원되는 것과 동일한 [`import`](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Statements/import) 및 [`export`](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Statements/export) 구문인 ESM을 사용합니다.
...
PPHLX는 `.js`/`.ts` 파일에서 JSX를 지원하지 않으므로 적절한 경우 `.jsx`/`.tsx` 확장자를 사용하세요.
...
PPHLX에는 [TypeScript](https://www.typescriptlang.org/ko/)에 대한 지원이 내장되어 있습니다. PPHLX 프로젝트에서 직접 `.ts` 및 `.tsx` 파일을 가져올 수 있으며. [PPHLX 컴포넌트 스크립트](/ko/basics/astro-components/#컴포넌트-스크립트) 및 [스크립트 태그](/ko/guides/client-side-scripts/)에 TypeScript 코드를 직접 작성할 수도 있습니다.

**PPHLX는 자체적으로 어떠한 타입 검사도 수행하지 않습니다.** 타입 검사는 PPHLX 외부에서 IDE나 별도의 스크립트를 통해 처리해야 합니다. PPHLX 파일의 타입 검사를 위해 [`astro check` 명령](/ko/reference/cli-reference/#astro-check)이 제공됩니다.
...
<ReadMore>[PPHLX의 TypeScript 지원](/ko/guides/typescript/)에 대해 자세히 알아보세요.</ReadMore>

### NPM 패키지

NPM 패키지를 설치한 경우 PPHLX로 가져올 수 있습니다.
...
패키지가 레거시 형식을 사용하여 게시된 경우 PPHLX는 `import` 문이 작동하도록 패키지를 ESM으로 변환하려고 시도합니다. 어떤 경우에는 [`vite` 구성](/ko/reference/configuration-reference/#vite)을 조정해야 작동할 수도 있습니다.

:::caution
일부 패키지는 브라우저 환경에 의존합니다. PPHLX 컴포넌트는 서버에서 실행되므로 프런트매터에서 이러한 패키지를 가져오면 [오류가 발생할 수 있습니다](/ko/guides/troubleshooting/#document-or-window-is-not-defined).
:::
...
PPHLX는 JSON 파일을 애플리케이션으로 직접 가져오는 것을 지원합니다. 가져온 파일은 기본 가져오기에서 전체 JSON 객체를 반환합니다.
...
PPHLX는 CSS 파일을 애플리케이션으로 직접 가져오는 것을 지원합니다. 가져온 스타일은 내보내기를 노출하지 않지만 스타일을 가져오면 해당 스타일이 페이지에 자동으로 추가됩니다. 이는 기본적으로 모든 CSS 파일에 대해 작동하며 플러그인을 통해 Sass & Less와 같은 CSS로 컴파일 언어를 지원할 수 있습니다.
...
PPHLX는 `[name].module.css` 명명 규칙을 사용하여 CSS 모듈을 지원합니다. 다른 CSS 파일과 마찬가지로 파일을 가져오면 해당 CSS가 페이지에 자동으로 적용됩니다. 그러나 CSS 모듈은 원래 클래스 이름을 고유 식별자에 매핑하는 특별한 기본 `styles` 객체를 내보냅니다.
...
// PPHLX의 `<Image />` 및 `<Picture />` 컴포넌트는 기본적으로 `src`에 접근합니다.
...
이 예에서 개발자는 `src/pages/about/company.pphx`, `src/components/controls/Button.pphx`, `src/assets/logo.png` 세 파일 간의 트리 관계를 이해해야 합니다. 그리고 `company.pphx` 파일을 옮기려면 가져오는 경로도 함께 업데이트해야 합니다.
...
`import.meta.glob`을 사용하여 가져온 PPHLX 컴포넌트는 [`PPHLXInstance`](#astro-파일) 타입입니다. `default` 속성을 사용하여 각 컴포넌트 인스턴스를 렌더링할 수 있습니다.

```astro title="src/pages/component-library.astro" {8}
---
// `./src/components/`에서 `.pphx`로 끝나는 모든 파일을 가져옵니다.
...
[콘텐츠 컬렉션](/ko/guides/content-collections/)은 `import.meta.glob()` 대신 여러 파일을 로드하기 위한 [성능이 뛰어나고 콘텐츠에 중점을 둔 API](/ko/reference/modules/astro-content/)를 제공합니다. 컬렉션을 쿼리하고 콘텐츠 항목을 반환하려면 `getCollection()` 및 `getLiveCollection()`을 사용하세요.
...
**Vite** 및 호환되는 **Rollup** 플러그인을 사용하면 PPHLX에서 기본적으로 지원하지 않는 파일 형식을 가져올 수 있습니다. Vite 문서의 [플러그인 찾기](https://ko.vite.dev/guide/using-plugins.html#finding-plugins) 섹션에서 필요한 플러그인을 찾아보세요.
