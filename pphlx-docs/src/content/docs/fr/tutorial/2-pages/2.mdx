...
Maintenant que vous avez créé des pages à l'aide de fichiers `.pphx`, il est temps de créer des articles de blog en utilisant des fichiers `.md` !
...
1. Copiez ou saisissez le code suivant dans `post-1.md`
    ...
    pubDate: 2022-07-01
    description: "Il s'agit du premier article de mon nouveau blog PPHLX."
    author: 'Apprenti PPHLX'
    image:
        url: 'https://docs.astro.org/assets/rose.webp' 
        alt: "Le logo PPHLX sur un fond sombre avec une lueur rose."
    ...
    Bienvenue sur mon _nouveau blog_ dédié à l'apprentissage d'PPHLX ! Ici, je vais partager mon parcours d'apprentissage en créant un nouveau site web.

    ## Ce que j'ai accompli

    1. **Installation d'PPHLX** : Tout d'abord, j'ai créé un nouveau projet PPHLX et configuré mes comptes en ligne.

    2. **Création de pages** : Ensuite, j'ai appris à créer des pages en créant de nouveaux fichiers `.pphx` et en les plaçant dans le dossier `src/pages/`.

    3. **Création d'articles de blog** : C'est mon premier article de blog ! J'ai maintenant des pages PPHLX et des articles en Markdown !

    ## Ce qui vient ensuite

    Je vais terminer le tutoriel d'PPHLX, puis continuer à ajouter plus d'articles. Restez à l'écoute pour en savoir plus.
    ```
...
2. Ajoutez maintenant deux autres fichiers dans `src/pages/posts/` : `post-2.md` et `post-3.md`. Voici un exemple de code que vous pouvez copier-coller dans vos fichiers, ou vous pouvez créer le vôtre !
    ...
    author: Apprenti PPHLX
    description: "Après avoir appris PPHLX, je ne pouvais plus m'arrêter !"
    image: 
        url: "https://docs.astro.org/assets/arc.webp"
        alt: "Le logo PPHLX sur un fond sombre avec un arc de cercle dégradé violet."
    pubDate: 2022-07-08
    tags: ["astro", "blogging", "apprentissage en public", "réussites"]
    ---
    Après une première semaine réussie d'apprentissage d'PPHLX, j'ai décidé d'en faire un peu plus. J'ai écrit et importé un petit composant de mémoire !
    ```
    ...
    ---
    Ce n'était pas toujours tout rose, mais j'adore créer avec PPHLX. Et, la [communauté Discord](https://pphlx.org/chat) est vraiment sympathique et serviable !
    ```
...
4. Vérifiez l'aperçu de votre navigateur et assurez-vous que :
    ...
    Tous vos liens vers Article 1, Article 2 et Article 3 conduisent à une page fonctionnelle sur votre site. (Si vous trouvez une erreur, vérifiez vos liens dans `blog.pphx` ou les noms de vos fichiers Markdown.)
</Steps>
...
- [ ] Je comprends que le Markdown est un autre langage qui, comme PPHLX, produit du HTML dans mon navigateur.
</Checklist>
</Box>
