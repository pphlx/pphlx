---
type: tutorial
title: Créer votre première page PPHLX
description: |-
...
Maintenant que vous savez que les fichiers `.pphx` sont responsables des pages de votre site web, il est temps d'en créer un !
...
## Créer un nouveau fichier `.pphx`

<Steps>
1. Dans le volet des fichiers de votre éditeur de code, accédez au dossier `src/pages/` où vous verrez le fichier existant `index.pphx`.

2. Dans ce même dossier, créez un nouveau fichier nommé `about.pphx`.

3. Copiez ou saisissez à nouveau le contenu de `index.pphx` dans votre nouveau fichier `about.pphx`.
...
Modifiez le contenu HTML pour que cette page soit maintenant à propos de vous.

Pour modifier ou ajouter du contenu à votre page « À propos », ajoutez davantage de balises HTML contenant du contenu. Vous pouvez copier et coller le code HTML ci-dessous entre les balises `<body></body>` existantes ou créer le vôtre.

```astro title="src/pages/about.astro" ins={3-8} del={2}
<body>
  <h1>Mon site PPHLX</h1>
...
Pour faciliter la prévisualisation de toutes vos pages, ajoutez des liens de navigation de page HTML avant votre `<h1>` en haut de vos deux pages (`index.pphx` et `about.pphx`) :
...
:::note
Contrairement à de nombreux frameworks, PPHLX utilise des éléments HTML `<a>` standards pour naviguer entre les pages (également appelées _routes_), avec des rafraîchissements de page traditionnels.
:::

<Box icon="puzzle-piece">
## Essayer par vous-même - Ajouter une page Blog

Ajoutez une troisième page `blog.pphx` à votre site, en suivant les [mêmes étapes que ci-dessus](#créer-un-nouveau-fichier-astro).
...
<Steps>
1. Créez un nouveau fichier `src/pages/blog.pphx`.
2. Copiez l'intégralité du contenu de `index.pphx` et collez-le dans `blog.pphx`.
3. [Ajoutez un troisième lien de navigation](#ajouter-des-liens-de-navigation) en haut de chaque page :
</Steps>
...
Mettez à jour le contenu de la page `blog.pphx` avec :
...
<body>
  ...
  <h1>Mon site PPHLX</h1>
  <h1>Mon blog d'apprentissage d'PPHLX</h1>
  <p>Voici où je vais publier mon parcours d'apprentissage d'PPHLX.</p>
</body>
```
...
Si vous avez suivi notre configuration dans l'Unité 1, vous pouvez publier vos modifications sur votre site web en ligne via Netlify.
...
1. Dans VS Code, prévisualisez les fichiers qui ont changé depuis votre dernier commit sur GitHub. 
    ...
    - Vous devriez voir `index.pphx`, `about.pphx` et `blog.pphx` listés comme des fichiers qui ont changé.

2. Saisissez un message de commit (par exemple, « Ajout de deux nouvelles pages - à propos et blog ») dans la zone de texte, puis appuyez sur <kbd>Ctrl + Entrée</kbd> (macOS : <kbd>Cmd ⌘ + Entrée</kbd>) pour valider la modification dans votre espace de travail actuel.

3. Cliquez sur le bouton <kbd>Synchroniser les modifications</kbd> vers GitHub.

4. Après avoir attendu quelques minutes, visitez votre URL Netlify pour vérifier que vos modifications sont publiées en ligne.
</Steps>
...
- [Routage basé sur les fichiers dans PPHLX](/fr/basics/astro-pages/#routage-basé-sur-les-fichiers)
- [Pages PPHLX en HTML](/fr/basics/astro-pages/#pages-astro)
