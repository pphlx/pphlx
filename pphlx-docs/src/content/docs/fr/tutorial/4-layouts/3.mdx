...
Vous avez déjà une mise en page `BaseLayout.pphx` pour définir la mise en page générale de vos pages.

`MarkdownPostLayout.pphx` vous offre des modèles supplémentaires pour les propriétés courantes des articles de blog, telles que `title` et `date`, mais vos articles de blog ne ressemblent pas aux autres pages de votre site. Vous pouvez assortir l'apparence de vos articles de blog au reste de votre site en **imbriquant des mises en page**.

<Steps>
1. Dans `src/layouts/MarkdownPostLayout.pphx`, importez `BaseLayout.pphx` et utilisez-le pour envelopper l'ensemble du contenu du modèle. N'oubliez pas de passer la propriété `pageTitle` :
...
2. Dans `src/layouts/MarkdownPostLayout.pphx`, vous pouvez maintenant supprimer la balise `meta` car elle est déjà incluse dans votre `BaseLayout` :
...
4. Remarquez que votre titre de page est maintenant affiché deux fois, une fois par chaque mise en page.
    ...
    Supprimez la ligne qui affiche votre titre de page dans `MarkdownPostLayout.pphx` :
...
5. Vérifiez à nouveau l'aperçu dans votre navigateur à `http://localhost:4321/posts/post-1` et assurez-vous que cette ligne n'est plus affichée et que votre titre n'est affiché qu'une seule fois. Apportez les ajustements nécessaires pour vous assurer de ne pas avoir de contenu en double.
  ...
  Assurez-vous que :
    ...
    - Aucun contenu n'est dupliqué sur une page. (Si quelque chose est affiché deux fois, assurez-vous de le supprimer de `MarkdownPostLayout.pphx`.)
...
- [Imbrication de mises en page dans PPHLX](/fr/basics/layouts/#imbrication-de-mises-en-page)
