...
<Steps>
1. Créez un nouveau fichier à l'emplacement `src/layouts/MarkdownPostLayout.pphx`

2. Copiez le code suivant dans `MarkdownPostLayout.pphx`
...
3. Ajoutez la propriété de frontmatter suivante dans `post-1.md`
    ...
    pubDate: 2022-07-01
    description: 'Ceci est le premier article de mon nouveau blog PPHLX.'
...
**Défi** : Identifiez les éléments communs à chaque article de blog et utilisez `MarkdownPostLayout.pphx` pour les afficher, au lieu de les écrire dans le contenu Markdown de votre fichier `post-1.md` et dans tous les futurs articles de blog.
...
Bienvenue sur mon _nouveau blog_ sur l'apprentissage d'PPHLX ! Ici, je partagerai mon parcours d'apprentissage alors que je construis un nouveau site web.
```
...
Pouvez-vous trouver ce qui doit aller dans les blancs pour que les deux composants suivants produisent ensemble un code PPHLX fonctionnel ?

1.  ```markdown title="src/pages/posts/learning-astro.md"
    ---
    layout: ../../__________/MyMarkdownLayout.pphx
    title: "Découverte de Markdown dans PPHLX"
    author: Apprenti PPHLX
    ____: 2022-08-08
    ---
    J'ai tellement appris aujourd'hui ! PPHLX me permet d'écrire en Markdown, mais aussi d'utiliser les variables du frontmatter. Je peux même accéder à ces valeurs dans un composant de mise en page PPHLX.
    ```
...
- [Mises en page Markdown dans PPHLX](/fr/guides/markdown-content/#propriété-layout-du-frontmatter)
