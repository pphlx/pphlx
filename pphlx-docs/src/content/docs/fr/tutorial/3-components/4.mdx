...
<Steps>
1. Créez un fichier nommé `Menu.pphx` dans `src/components/`
...
3. Placez ce nouveau composant `<Menu />` juste avant votre composant `<Navigation />` dans `Header.pphx`. 
...
<Steps>
1. Ajoutez la balise `<script>` suivante à `index.pphx`, en utilisant la prise en charge intégrée de TypeScript par PPHLX, juste avant la balise de fermeture `</body>`.
...
2. Remplacez le contenu de la balise `<script>` dans `index.pphx` par l'importation de fichier suivante :
...
4. Ajoutez la même balise `<script>` avec l'importation sur vos deux autres pages, `about.pphx` et `blog.pphx`, et vérifiez que vous avez un en-tête réactif et interactif sur chaque page.
...
1. Quand PPHLX exécute-t-il le JavaScript écrit dans le frontmatter d'un composant ?
    <MultipleChoice>
      <Option>
       PPHLX n'exécute jamais de JavaScript
      </Option>
      <Option isCorrect>
...
2. En option, PPHLX peut envoyer du JavaScript au navigateur pour permettre :
    <MultipleChoice>
      <Option>
...
3. Le JavaScript côté client sera envoyé au navigateur d'un utilisateur quand il est écrit ou importé :
    <MultipleChoice>
      ...
      <Option>
        entre les délimiteurs de code d'un fichier `.pphx`
      </Option>
      <Option>
...
[Les scripts côté client dans PPHLX](/fr/guides/client-side-scripts/)
