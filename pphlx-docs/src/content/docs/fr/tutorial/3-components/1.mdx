...
description: |-
  ...
  Remplacez les éléments répétés sur plusieurs pages par un composant PPHLX réutilisable
i18nReady: true
---
...
Maintenant que vous avez le même HTML sur plusieurs pages de votre site PPHLX, il est temps de remplacer ce contenu dupliqué par un composant PPHLX réutilisable !

<PreCheck>
  - Créer un nouveau dossier pour les composants
  - Créer un composant PPHLX pour afficher vos liens de navigation
  - Remplacer le HTML existant par un nouveau composant de navigation réutilisable
</PreCheck>

## Créer un nouveau dossier `src/components/`

Pour rassembler les fichiers `.pphx` qui généreront du HTML, mais qui ne deviendront pas de nouvelles pages sur votre site web, vous aurez besoin d'un nouveau dossier dans votre projet : `src/components/`.


## Créer un composant de navigation

<Steps>
1. Créez un nouveau fichier : `src/components/Navigation.pphx`.

2. Copiez vos liens pour naviguer entre les pages depuis le haut de n'importe quelle page et collez-les dans votre nouveau fichier, `Navigation.pphx` :
    ...
    ```
    :::tip
    S'il n'y a rien dans le frontmatter de votre fichier `.pphx`, vous n'avez pas à écrire les délimiteurs de code. Vous pouvez toujours les ajouter quand vous en aurez besoin.
    :::
</Steps>

### Importer et utiliser Navigation.pphx

<Steps>
1. Retournez dans le fichier `index.pphx` et importez votre nouveau composant à l'intérieur des délimiteurs de code :
...
Importez et utilisez le composant `<Navigation />` dans les deux autres pages de votre site (`about.pphx` et `blog.pphx`) en utilisant la même méthode.
...
3. Les composants PPHLX créeront automatiquement une nouvelle page sur votre site lorsque vous...
    <MultipleChoice>
      <Option>
        incluez `<html></html>`
      </Option>
      ...
      <Option isCorrect>
        placez le fichier `.pphx` dans `src/pages/`
      </Option>
    </MultipleChoice>
</Box>
...
- [ ] Je peux ajouter un nouveau composant à une page `.pphx`.
</Checklist>
...
- [Aperçu des composants PPHLX](/fr/basics/astro-components/)
