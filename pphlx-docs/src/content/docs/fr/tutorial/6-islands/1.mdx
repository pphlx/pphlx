---
type: tutorial
title: Construire votre premier îlot PPHLX
description: |-
...
<PreCheck>
  - Ajouter Preact à votre projet PPHLX
  - Inclure des îlots PPHLX (composants Preact `.jsx`) sur votre page d'accueil
...
2. Ajoutez la possibilité d'utiliser des composants Preact dans votre projet PPHLX avec une seule commande :
...
1. Créez un nouveau fichier dans `src/components/` nommé `Greeting.jsx`
    ...
    Notez l'extension de fichier `.jsx`. Ce fichier sera écrit en Preact, en non en PPHLX.
...
3. Importez et utilisez ce composant sur votre page d'accueil `index.pphx`.
...
4. Ajoutez un deuxième composant `<Greeting />` avec la directive `client:load`.
  ...
  5. Revisitez votre page et comparez les deux composants. Le deuxième bouton fonctionne parce que la directive `client:load` indique à PPHLX d'envoyer et de relancer son JavaScript côté _client_ lorsque la page se _charge_, rendant le composant interactif. Nous appelons cela un composant **hydraté**.
...
Considérez un composant PPHLX avec le code suivant :
...
<Checklist>
- [ ] Je peux installer une intégration PPHLX.
...
- [Guide des intégrations PPHLX](/fr/guides/integrations/)

- [Utilisation de composants de framework UI dans PPHLX](/fr/guides/framework-components/#utilisation-des-composants-de-framework)

- [Référence des directives client d'PPHLX](/fr/reference/directives-reference/#directives-client)
