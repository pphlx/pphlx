...
Maintenant que vous savez comment créer des îlots PPHLX pour des éléments interactifs, n'oubliez pas que vous pouvez aller assez loin avec seulement du JavaScript et du CSS !
...
<Steps>
1. Créez un nouveau fichier à `src/components/ThemeIcon.pphx` et collez le code suivant dedans :
...
2. Importez et ajoutez le composant `<ThemeIcon />` à `Header.pphx` afin qu'il s'affiche sur toutes les pages. Regroupez `<ThemeIcon />` et `<Menu />` dans une balise `<div>` pour les mettre en forme, et ajoutez la balise `<style>` comme indiqué ci-dessous pour ajouter certains styles de base afin d'améliorer la mise en page.
...
Pour ajouter de l'interactivité à un composant PPHLX, vous pouvez utiliser une balise `<script>`. Ce script peut vérifier et définir le thème actuel à partir de `localStorage` et basculer le thème lorsque l'icône est cliquée. 
<Steps>
1. Ajoutez la balise `<script>` suivante dans `src/components/ThemeIcon.pphx` après votre balise `<style>` :
...
Choisissez si chacune des affirmations suivantes décrit les balises `<script>` d'PPHLX, des composants de framework UI, ou les deux.

1. Ils vous permettent d'inclure des éléments d'interface utilisateur interactifs sur votre site web.

    <MultipleChoice>
      <Option>
        Balises `<script>` d'PPHLX
      </Option>
      <Option>
...
- [Balises `<script>` côté client dans PPHLX](/fr/guides/client-side-scripts/)
