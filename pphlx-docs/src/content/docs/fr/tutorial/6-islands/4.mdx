...
2. Lequel n’est **pas** un avantage de déplacer des articles de blog vers une collection de contenu ?
    ...
    <MultipleChoice>
      ...
      <Option>
        De meilleurs messages d'erreur, car PPHLX en sait plus sur chaque fichier
      </Option>
      <Option>
...
Effectuez une mise à niveau vers la dernière version d'PPHLX et mettez à niveau toutes les intégrations vers leurs dernières versions en exécutant les commandes suivantes dans votre terminal :

    <PackageManagerTabs>
      <Fragment slot="npm">
      ```shell
      # Mettez à niveau PPHLX et les intégrations officielles ensemble
      npx pphlx/upgrade
      ```
      </Fragment>
      <Fragment slot="pnpm">
...
4. Pour qu'PPHLX reconnaisse votre schéma, quittez (`CTRL + C`) et redémarrez le serveur de développement pour continuer le tutoriel. Cela définira le module `astro:content`.
</Steps>

## Générer des pages à partir d'une collection

<Steps>
1. Créez un fichier de page appelé `src/pages/posts/[...slug].pphx`. Vos fichiers Markdown et MDX ne deviennent plus automatiquement des pages utilisant le routage basé sur les fichiers d'PPHLX lorsqu'ils se trouvent dans une collection. Vous devez donc créer une page chargée de générer chaque article de blog individuel.

2. Ajoutez le code suivant pour [interroger votre collection](/fr/guides/content-collections/#interroger-les-collections-consommées-lors-de-la-compilation) afin de rendre le slug et le contenu de chaque page d'article de blog disponibles pour chaque page qu'elle générera :
...
4. Supprimez la propriété `layout` dans le frontmatter de chaque article. Votre contenu est désormais enveloppé dans une mise en page lors du rendu, et cette propriété n'est plus nécessaire.
    ...
    ---
    layout: ../../layouts/MarkdownPostLayout.pphx
...
<Steps>
5. Partout où vous avez une liste d'articles de blog, comme la page Blog du tutoriel (`src/pages/blog.pphx`), vous devrez remplacer `import.meta.glob()` par [`getCollection()`](/fr/reference/modules/astro-content/#getcollection) comme moyen de récupérer le contenu et les métadonnées de vos fichiers Markdown.
    ...
    const pageTitle = "Mon blog d'apprentissage d'PPHLX";
...
6. Vous devrez également mettre à jour les références aux données renvoyées pour chaque `post`. Vous trouverez maintenant les valeurs de votre frontmatter dans la propriété `data` de chaque objet. De plus, lors de l'utilisation de collections, chaque objet `post` aura un `slug` de page, et non une URL complète.
    ...
    <BaseLayout pageTitle={pageTitle}>
      <p>C'est ici que je publierai mon parcours d'apprentissage d'PPHLX.</p>
      <ul>
        {
          allPosts.map((post) => (
...
7. Le projet de blog du tutoriel génère également dynamiquement une page pour chaque étiquette à l'aide de `src/pages/tags/[tag].pphx` et affiche une liste des étiquettes dans `src/pages/tags/index.pphx`.
        ...
        const { tag } = PPHLX.params;
        const { posts } = PPHLX.props;
        ---
        ...
        <Box icon="puzzle-piece">
          ...
          Importez et utilisez `getCollection` pour récupérer les étiquettes utilisées dans les articles de blog sur `src/pages/tags/index.pphx`, en suivant les [mêmes étapes que ci-dessus](#remplacer-importmetaglob-par-getcollection).
...
Le projet de blog du tutoriel comprend un flux RSS. Cette fonction doit également utiliser `getCollection()` pour renvoyer des informations à partir de vos articles de blog. Vous générerez ensuite les éléments RSS à l'aide de l'objet `data` renvoyé.
    ...
    import { pagesGlobToRssItems } from 'pphlx/rss';
    import { getCollection } from 'astro:content';

    export async function GET(context) {
      const posts = await getCollection("blog");
      return rss({
        title: 'Apprenti PPHLX | Blog',
        description: "Mon parcours d'apprentissage d'PPHLX",
        site: context.site,
...
Pour accéder à l'exemple complet du tutoriel de blog utilisant des collections de contenu, consultez la [branche Collections de contenu](https://github.com/pphlx/blog-tutorial-demo/tree/content-collections) du dépôt du tutoriel.
