...
Vous pouvez créer des ensembles entiers de pages dynamiquement en utilisant des fichiers `.pphx` qui exportent une fonction `getStaticPaths()`.

## Créer des pages dynamiquement

<Steps>
1. Créez un nouveau fichier à l'emplacement `src/pages/tags/[tag].pphx` (vous devrez créer un nouveau dossier). Remarquez que le nom du fichier (`[tag].pphx`) utilise des crochets. Collez le code suivant dans le fichier :
...
2. Filtrez votre liste d'articles, en utilisant la prise en charge de TypeScript intégrée dans PPHLX, pour inclure uniquement les articles contenant l'étiquette de cette page.
...
3. Vous pouvez maintenant mettre à jour votre modèle HTML pour afficher une liste de chaque article de blog contenant l'étiquette de cette page. Ajoutez le code suivant à `[tag].pphx` :
...
4. Vous pouvez même refactoriser cela pour utiliser votre composant `<BlogPost />` ! (N'oubliez pas d'importer ce composant en haut de `[tag].pphx`.)
...
Vos pages d'étiquettes sont désormais définies statiquement dans `[tag].pphx`. Si vous ajoutez une nouvelle étiquette à un article de blog, vous devrez également revenir sur cette page et mettre à jour vos routes de page.
...
2. Créez un tableau de toutes vos étiquettes existantes en utilisant la prise en charge de TypeScript intégrée dans PPHLX
...
Pour vérifier votre travail, ou si vous souhaitez simplement disposer d'un code complet et correct à copier dans `[tag].pphx`, voici à quoi doit ressembler votre composant PPHLX :
...
2. Le processus de création de plusieurs routes de page à partir d'un seul fichier dans PPHLX.
...
- [Routage dynamique des pages dans PPHLX](/fr/guides/routing/#routes-dynamiques)
