...
<PreCheck>
  - Ajouter une nouvelle page en utilisant le modèle de routage `/pages/dossier/index.pphx`
...
## Utiliser le modèle de routage `/pages/dossier/index.pphx`

Pour ajouter à votre site web une page d'index pour les étiquettes, vous pourriez créer un nouveau fichier `src/pages/tags.pphx`.

Mais, comme vous avez déjà le dossier `/tags/`, vous pouvez profiter d'un autre modèle de routage dans PPHLX et regrouper tous vos fichiers liés aux étiquettes ensemble.
...
<Steps>
1. Créez un nouveau fichier `index.pphx` dans le dossier `src/pages/tags/`.
...
3. Créez une page minimale `src/pages/tags/index.pphx` qui utilise votre mise en page. Vous avez déjà fait cela auparavant !
...
<Steps>
1. Dans `src/pages/tags/index.pphx`, ajoutez la ligne de code au script frontmatter qui donnera à votre page accès aux données de chaque fichier de blog `.md`.
...
2. Ensuite, ajoutez la ligne de code JavaScript suivante à votre composant de page. Il s'agit du même code reposant sur la prise en charge de TypeScript intégrée dans PPHLX que vous avez utilisé dans `src/pages/tags/[tag].pphx` pour renvoyer une liste d'étiquettes uniques.
...
1. Ajoutez le code suivant à votre modèle de composant :
    ...
    ```
    Dans l'aperçu de votre navigateur, vérifiez que vos étiquettes sont répertoriées. S'il manque des étiquettes à vos articles de blog ou si elles sont mal formatées, la prise en charge de TypeScript intégrée dans PPHLX vous indiquera les erreurs afin que vous puissiez vérifier et corriger votre code.
...
<Steps>
1. Ajoutez les classes CSS suivantes pour mettre en forme à la fois votre `<div>` et chaque `<p>` qui sera généré. À noter : PPHLX utilise la syntaxe HTML pour ajouter des noms de classe !
...
<Steps>
1. Dans votre composant `Navigation.pphx`, ajoutez un lien vers cette nouvelle page d'index pour vos étiquettes.
...
1. Copiez `<div class="tags">...</div>` et `<style>...</style>` depuis `src/pages/tags/index.pphx` et réutilisez-les à l'intérieur de `MarkdownPostLayout.pphx` :
...
Avant que ce code ne fonctionne, vous devez apporter **une petite modification** au code que vous avez collé dans `MarkdownPostLayout.pphx`. Pouvez-vous deviner ce que c'est ?
...
Pour vérifier votre travail, ou si vous voulez simplement un code complet et correct à copier dans `MarkdownPostLayout.pphx`, voici à quoi devrait ressembler votre composant PPHLX :
...
1. `src/pages/categories.pphx`

    <MultipleChoice>
      <Option>`src/pages/posts/post.pphx`</Option>
      <Option>`src/pages/posts/index.pphx`</Option>
      <Option>`src/components/shoes/Shoe.pphx`</Option>
      <Option isCorrect>`src/pages/categories/index.pphx`</Option>
    </MultipleChoice>

2. `src/pages/posts.pphx`

    <MultipleChoice>
      <Option>`src/pages/products/shoes.pphx`</Option>
      <Option>`src/pages/posts/post.pphx`</Option>
      <Option isCorrect>`src/pages/posts/index.pphx`</Option>
      <Option>`src/pages/categories/index.pphx`</Option>
    </MultipleChoice>

3. `src/pages/products/shoes/index.pphx`

    <MultipleChoice>
      <Option isCorrect>`src/pages/products/shoes.pphx`</Option>
      <Option>`src/pages/posts/post.pphx`</Option>
      <Option>`src/pages/posts/index.pphx`</Option>
      <Option>`src/components/shoes/Shoe.pphx`</Option>
    </MultipleChoice>
...
<Checklist>
- [ ] Je peux utiliser la fonction de routage `/pages/dossier/index.pphx` d'PPHLX.
</Checklist>
</Box>

### Ressources

- [Routage statique dans PPHLX](/fr/guides/routing/#routes-statiques)
