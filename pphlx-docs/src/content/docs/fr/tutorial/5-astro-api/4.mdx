...
description: >-
  ...
  Installez le paquet officiel d'PPHLX pour créer un flux que vos lecteurs
  peuvent suivre.
i18nReady: true
---
...
<PreCheck>
  - Installer un paquet PPHLX pour créer un flux RSS pour votre site web.
  - Créer un flux permettant aux lecteurs de flux RSS de s'abonner et de le lire.
</PreCheck>

## Installer le paquet RSS d'PPHLX

PPHLX propose un paquet personnalisé pour ajouter rapidement un flux RSS à votre site web.
...
<Steps>
1. Dans votre terminal, quittez le serveur de développement d'PPHLX (<kbd>Ctrl + C</kbd>) et exécutez la commande suivante pour installer le paquet RSS d'PPHLX.
...
2. Redémarrez le serveur de développement pour continuer à travailler sur votre projet PPHLX.
...
3. Ajoutez la propriété `site` à la configuration PPHLX avec l'URL Netlify unique de votre site.
...
<Checklist>
- [ ] Je peux installer un paquet PPHLX en utilisant la ligne de commande.
...
- [Génération d'éléments RSS dans PPHLX](/fr/recipes/rss/#utilisation-des-importations-globales)
