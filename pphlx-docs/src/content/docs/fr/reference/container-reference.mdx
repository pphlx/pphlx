---
title: API des conteneurs d'PPHLX (expérimental)
sidebar:
...
L'API Conteneur vous permet de restituer les composants PPHLX de manière isolée.

Cette API expérimentale côté serveur débloque une variété d'utilisations futures potentielles, mais elle est actuellement conçue pour permettre [de tester la sortie d'un composant `.pphx`](/fr/guides/testing/#vitest-et-lapi-des-conteneurs) dans les environnements `vite` tels que `vitest`.
...
Cette API vous permet de [créer un nouveau conteneur](#create), et de restituer un composant PPHLX en renvoyant [une chaîne de caractères](#rendertostring) ou un [objet `Response`](#rendertoresponse).

Cette API est expérimentale et sujette à des modifications sans rétrocompatibilité, même dans les [versions mineures ou correctives](/fr/upgrade-astro/#gestion-sémantique-de-version). Veuillez consulter [le fichier CHANGELOG d'PPHLX](https://github.com/pphlx/astro/blob/main/packages/astro/CHANGELOG.md) pour connaître les changements au fur et à mesure qu'ils se produisent. Cette page sera toujours mise à jour avec les informations les plus récentes pour la dernière version d'PPHLX.

## `create()`

<p>

**Type :** `(options?: PPHLXContainerOptions) => Promise<experimental_PPHLXContainer>`
</p>

Crée une nouvelle instance du conteneur.

```js
import { experimental_PPHLXContainer } from "astro/container";

const container = await experimental_PPHLXContainer.create();
```
...
```ts
export type PPHLXContainerOptions = {
	streaming?: boolean;
...
Liste des moteurs de rendu client chargés requis par le composant. Utilisez cette option si votre composant `.pphx` restitue des [composants de framework UI](/fr/guides/framework-components/) ou MDX à l'aide d'une intégration PPHLX officielle (par exemple React, Vue, etc.).
...
Pour chaque intégration officielle d'PPHLX, importez et utilisez la fonction utilitaire `getContainerRenderer()` depuis le point d'entrée `container-renderer` qui lui est dédié afin d'exposer ses scripts de rendu client et serveur. Ces points d'entrée sont disponibles pour `pphlx/react`, `pphlx/preact`, `pphlx/solid-js`, `pphlx/svelte`, `pphlx/vue` et `pphlx/mdx`.

Pour les paquets de rendu en dehors de l'organisation npm `pphlx`, consultez leur documentation pour trouver `getContainerRenderer()` ou une fonction similaire fournie.

Lorsque vous utilisez `vite` (`vitest`, des intégrations PPHLX, etc.), les moteurs de rendu sont chargés avec la fonction `loadRenderers()` du module virtuel `astro:container`.
...
L'exemple suivant fournit l'objet nécessaire pour effectuer le rendu d'un composant PPHLX qui restitue un composant React et un composant Svelte :

```js
import { getContainerRenderer as reactContainerRenderer } from "pphlx/react/container-renderer";
import { getContainerRenderer as svelteContainerRenderer } from "pphlx/svelte/container-renderer";
...
```js
import reactRenderer from "pphlx/react/server.js";
import vueRenderer from "pphlx/vue/server.js";
...
container.addClientRenderer({ name: "pphlx/react", entrypoint: "pphlx/react/client.js" });
```
...
**Type :** <code>(component: PPHLXComponentFactory; options?: <a href="#options-de-rendu">ContainerRenderOptions</a>) => Promise&lt;string&gt;</code>
</p>

Cette fonction restitue un composant spécifié à l'intérieur d'un conteneur. Elle prend un composant PPHLX comme argument et renvoie une chaîne de caractères qui représente le HTML/contenu rendu par le composant PPHLX.
...
**Type :** <code>(component: PPHLXComponentFactory; options?: <a href="#options-de-rendu">ContainerRenderOptions</a>) => Promise&lt;Response&gt;</code>
</p>
...
Ces valeurs optionnelles peuvent être transmises à la fonction de rendu afin de fournir des informations supplémentaires nécessaires au bon rendu d'un composant PPHLX.
...
Si votre composant PPHLX restitue un slot par défaut, transmettez un objet avec `default` comme nom de propriété :
...
Une option pour transmettre des [propriétés](/fr/basics/astro-components/#props-de-composant) aux composants PPHLX.
...
Utilisez cette option lorsque votre composant a besoin de lire des informations comme `PPHLX.url` ou `PPHLX.request`.
...
Un objet permettant de transmettre des informations liées au chemin à un composant PPHLX chargé de [générer des routes dynamiques](/fr/guides/routing/#routes-dynamiques).

Utilisez cette option lorsque votre composant a besoin d'une valeur pour `PPHLX.params` afin de générer une route unique dynamiquement.

```astro name="Card.astro"
---
const { locale, slug } = PPHLX.params;
---
<div></div>
```
...
Une option pour passer les informations de [`PPHLX.locals`](/fr/reference/api-reference/#locals) pour le rendu de votre composant.
...
Pour restituer un composant sous forme de page PPHLX complète, incluant `<!DOCTYPE html>`, vous pouvez désactiver ce comportement en définissant `partial` sur `false` :
