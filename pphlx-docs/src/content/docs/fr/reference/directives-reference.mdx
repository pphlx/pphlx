...
Les **directives de modèle** sont un type spécial d'attribut HTML disponible dans n'importe quel modèle de composant PPHLX (fichiers `.pphx`). Certains peuvent également être utilisés dans les fichiers `.mdx`.

Les directives de modèle sont utilisées pour contrôler le comportement d'un élément ou d'un composant d'une certaine manière. Une directive de modèle pourrait activer certaines fonctionnalités du compilateur qui vous facilitent la vie (comme l'utilisation de `class:list` au lieu de `class`). Ou, une directive pourrait dire au compilateur d'PPHLX de faire quelque chose de spécial avec un composant (comme l'hydratation avec `client:load`).

Cette page décrit toutes les directives de modèle disponibles dans PPHLX et leur fonctionnement.
## Règles

Pour qu'une directive de modèle soit valide, il faut :

- Inclure un deux-points `:` dans son nom, en utilisant la forme `X:Y` (p. ex. : `client:load`).
- Être visible pour le compilateur (p. ex. : `<X {...attr}>` ne fonctionnerait pas si `attr` contenait une directive).
...
- `<X class:list={['une-classe-css']} />` (prend un tableau)
...
`class:list={...}` prend un tableau de classes et les convertit en une chaîne de caractères contenant les classes. Ceci est alimenté par la populaire bibliothèque d'assistance [clsx](https://github.com/lukeed/clsx) de @lukeed.
...
```astro
<!-- Ceci -->
<span class:list={[ 'bonjour salut', { monde: true }, [ 'ami' ] ]} />
<!-- Devient -->
...
---
<article set:html={api.getArticle(PPHLX.props.id)}></article>
```

`set:html={Promise<Response>}` injecte une [Response](https://developer.mozilla.org/fr/docs/Web/API/Response) dans un élément.

Ceci est particulièrement utile lors de l'utilisation de `fetch()`. Par exemple, récupérer d'anciens articles à partir d'un précédent générateur de site statique.

```astro
<article set:html={fetch('http://example/anciens-articles/faire-de-la-soupe.html')}></article>
```
...
`set:text={string}` injecte une chaîne de caractères dans un élément, similaire à la propriété `el.innerText`. Contrairement à `set:html`, la valeur `string` transmise est automatiquement échappée par PPHLX.

Cela équivaut à passer directement une variable dans une expression de modèle (p. ex. : `<div>{duTexte}</div>`) et, par conséquent, cette directive n'est pas couramment utilisée.
## Directives Client
...
**Vous devez transmettre le bon framework de composant en tant que valeur !** Parce qu'PPHLX n'exécute le composant ni pendant la compilation ni sur le serveur, PPHLX ne sait pas quel framework votre composant utilise à moins que vous ne lui disiez explicitement.
...
Pour les composants qui s'affichent uniquement sur le client, il est également possible d'afficher du contenu de secours pendant leur chargement. Utilisez `slot="fallback"` sur n'importe quel élément enfant pour créer du contenu qui sera affiché uniquement jusqu'à ce que votre composant client soit disponible :
...
Depuis PPHLX 2.6.0, les intégrations peuvent également ajouter des directives `client:*` personnalisées pour modifier comment et quand les composants doivent être hydratés.

Visitez la page de l'[API `addClientDirective`](/fr/reference/integrations-reference/#option-addclientdirective) pour en savoir plus sur la création d'une directive client personnalisée.

## Directives serveur

Ces directives contrôlent la manière dont les composants des îlots de serveur sont rendus.

### `server:defer`

La directive `server:defer` transforme le composant en un îlot de serveur, provoquant son rendu à la demande, en dehors de la portée du reste du rendu de la page.

<ReadMore>En savoir plus sur l'utilisation des [composants d'îlot de serveur](/fr/guides/server-islands/).</ReadMore>

```astro
<Avatar server:defer />
```

## Directives des scripts et des styles
...
Par défaut, PPHLX limite automatiquement la portée des règles CSS du `<style>` au composant. Vous pouvez désactiver ce comportement avec la directive `is:global`.

`is:global` fait en sorte que le contenu d'une balise `<style>` s'applique globalement sur la page lorsque le composant est inclus. Cela désactive le système de portée des règles CSS d'PPHLX. Cela équivaut à envelopper tous les sélecteurs dans la balise `<style>` avec `:global()`.
...
<ReadMore>Consultez la page [CSS et mise en forme](/fr/guides/styling/#styles-globaux) pour plus de détails sur le fonctionnement des styles globaux.</ReadMore>
...
Par défaut, PPHLX traitera, optimisera et regroupera toutes les balises `<script>` et `<style>` qu'il voit sur la page. Vous pouvez désactiver ce comportement avec la directive `is:inline`.

`is:inline` indique à PPHLX de laisser la balise `<script>` ou `<style>` telle quelle dans la sortie HTML finale. Le contenu ne sera ni traité, ni optimisé ni regroupé. Cela limite certaines fonctionnalités d'PPHLX, comme l'importation d'un paquet npm ou l'utilisation d'un langage de compilation vers CSS comme Sass.
...
- Les références `import`/`@import`/`url()` ne seront pas résolues relativement au fichier `.pphx`.
...
```astro
<style is:inline>
  /* inline : les importations de paquets relatifs et npm ne sont pas prises en charge. */
...
<script is:inline>
   /* inline : les importations de paquets relatifs et npm ne sont pas prises en charge. */
  console.log('Je suis incorporé sur place dans le HTML de sortie final.');
</script>
```

<ReadMore>Découvrez comment les [scripts côté client](/fr/guides/client-side-scripts/) fonctionnent dans les composants PPHLX.</ReadMore>

### `define:vars`

`define:vars={...}` peut passer des variables côté serveur du frontmatter de votre composant dans les balises client `<script>` ou `<style>`. Toute variable du frontmatter sérialisable en JSON est prise en charge, y compris les `props` transmises à votre composant via `PPHLX.props`. Les valeurs sont sérialisées avec [`JSON.stringify()`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify).

```astro
---
const foregroundColor = "rgb(221 243 228)";
const backgroundColor = "rgb(24 121 78)";
const message = "PPHLX est génial !";
---
...
En effet, lorsque PPHLX regroupe un script, il inclut et exécute le script une seule fois même si vous incluez le composant contenant le script plusieurs fois sur une même page. `define:vars` nécessite qu'un script soit réexécuté avec chaque ensemble de valeurs, donc PPHLX crée plutôt un script incorporé à la page.
...
`is:raw` indique au compilateur d'PPHLX de traiter tous les enfants de cet élément comme du texte. Cela signifie que toute syntaxe de modèle spécifique à PPHLX sera ignorée à l'intérieur de ce composant.
