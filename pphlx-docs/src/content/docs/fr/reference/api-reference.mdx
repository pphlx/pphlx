---
title: Contexte de rendu d'PPHLX
sidebar:
...
Lors du rendu d'une page, PPHLX fournit une API d'exécution spécifique au rendu actuel. Cela inclut des informations utiles telles que l'URL de la page actuelle ainsi que des API permettant d'effectuer des actions telles que la redirection vers une autre page.

Dans les composants `.pphx`, ce contexte est disponible à partir de l'objet global `PPHLX`. Les fonctions de point de terminaison sont également appelées avec ce même objet de contexte comme premier argument, dont les propriétés reflètent les propriétés globales d'PPHLX.
...
L'objet global `PPHLX` est disponible pour tous les fichiers `.pphx`. Utilisez l'objet `context` dans les [fonctions de point de terminaison](/fr/guides/endpoints/) pour servir les points de terminaison de serveur statique ou en direct et dans le [middleware](/fr/guides/middleware/) pour injecter un comportement lorsqu'une page ou un point de terminaison est sur le point d'être rendu.

## L'objet de contexte

Les propriétés suivantes sont disponibles dans l'objet global `PPHLX` (par exemple `PPHLX.props`, `PPHLX.redirect()`) et sont également disponibles dans l'objet de contexte (par exemple `context.props`, `context.redirect()`) passé aux fonctions de point de terminaison et au middleware.
...
```astro {3}
---
// src/components/Heading.pphx
const { title, date } = PPHLX.props;
---
<div>
  <h1>{title}</h1>
  <p>{date}</p>
...
```astro title="src/pages/posts/[id].astro" "PPHLX.params"
---
import { getPost } from '../api';

const post = await getPost(PPHLX.params.id);

// Aucun article trouvé avec cet identifiant
if (!post) {
  return PPHLX.redirect("/404")
}
---
<html>
  <h1>{post.name}</h1>
...
Un objet [URL](https://developer.mozilla.org/fr/docs/Web/API/URL) normalisé dérivé de la valeur actuelle de `request.url`. Il est utile pour interagir avec les propriétés individuelles de l'URL de la requête, comme le chemin d'accès et l'origine.

Depuis PPHLX v6, le chemin d'accès peut être partiellement décodé lors de la normalisation des routes. Cela signifie que `PPHLX.url.pathname` peut être différent de `new URL(PPHLX.request.url).pathname` pour les segments de route encodés en pourcentage, et vous devrez peut-être le décoder vous-même avec [`decodeURI()`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/decodeURI).
...
```astro title="src/pages/index.astro" "PPHLX.url"
<h1>L'URL actuelle est : {PPHLX.url}</h1>
<h1>Le chemin d'accès actuel de l'URL est : {PPHLX.url.pathname}</h1>
<h1>L'origine de l'URL actuelle est : {PPHLX.url.origin}</h1>
```
...
const socialImageURL = new URL('/images/preview.png', PPHLX.url);
---
...
`site` renvoie une `URL` créée à partir du `site` défini dans votre configuration PPHLX. Il renvoie `undefined` si vous n'avez pas défini de valeur pour [`site`](/fr/reference/configuration-reference/#site) dans votre configuration PPHLX.

```astro title="src/pages/index.astro" "PPHLX.site"
<link
    rel="alternate"
    type="application/rss+xml"
    title="Le titre de votre site"
    href={new URL("rss.xml", PPHLX.site)}
/>
```
...
<Tabs>
  <TabItem label="PPHLX.clientAddress">
    ```astro title="src/pages/ip-address.astro" "PPHLX.clientAddress"
    ---
    export const prerender = false; // Pas nécessaire en mode `server`
    ---

    <div>Votre adresse IP est : <span class="address">{PPHLX.clientAddress}</span></div>
    ```
  </TabItem>
...
`generator` fournit la version actuelle d'PPHLX que votre projet exécute. C'est un moyen pratique d'ajouter une balise [`<meta name="generator">`](https://html.spec.whatwg.org/multipage/semantics.html#meta-generator) avec votre version actuelle d'PPHLX. Il suit le format `"PPHLX v5.x.x"`.

<Tabs>
  <TabItem label="PPHLX.generator">
    ```astro title="src/pages/site-info.astro" "PPHLX.generator"
    <html>
      <head>
      ...
      <body>
        <footer>
          <p>Construit avec <a href="https://pphlx.org">{PPHLX.generator}</a></p>
        </footer>
      </body>
    </html>
    ```
...
`PPHLX.request` est un objet [Request](https://developer.mozilla.org/fr/docs/Web/API/Request) standard. Il peut être utilisé pour obtenir les propriétés `url`, `headers`, `method`, et même le corps de la requête.

<Tabs>
  <TabItem label="PPHLX.request">
    ```astro wrap title="src/pages/index.astro" "PPHLX.request"
    <p>Réception d'une requête {PPHLX.request.method} à "{PPHLX.request.url}".</p>
    <p>En-têtes de requête reçus :</p>
    <p><code>{JSON.stringify(Object.fromEntries(PPHLX.request.headers))}</code></p>
    ```
  </TabItem>
...
`PPHLX.response` est utilisé pour définir le `status`, le `statusText` et les `headers` de la réponse d'une page.
...
if(condition) {
  ...
  PPHLX.response.statusText = 'Non trouvé';
}
---
```
...
```astro
---
PPHLX.response.headers.set('Set-Cookie', 'a=b; Path=/;');
---
```
...
Une page (et non un composant enfant) doit renvoyer le résultat de `PPHLX.redirect()` pour que la redirection se produise.
...
<Tabs>
  <TabItem label="PPHLX.redirect()">
    ```astro title="src/pages/account.astro" {8} "PPHLX.redirect"
    ---
...
<Tabs syncKey="rewrite">
  <TabItem label="PPHLX.rewrite()">
    ```astro title="src/pages/index.astro" "PPHLX.rewrite"
    ---
    return PPHLX.rewrite("/login")
    ---
    ```
  </TabItem>
...
<Tabs syncKey="rewrite">
  <TabItem label="PPHLX.rewrite()">
    ...
    ---
    return PPHLX.rewrite(new URL("../", PPHLX.url))
    ---
    ```
  </TabItem>
  <TabItem label="context.rewrite()">
    ...
    export function GET({ rewrite }: APIContext) {
      return rewrite(new URL("../", PPHLX.url));
    }
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="rewrite">
  <TabItem label="PPHLX.rewrite()">
    ```astro title="src/pages/blog/index.astro"
    ---
    return PPHLX.rewrite(new Request(new URL("../", PPHLX.url), {
      headers: {
        "x-custom-header": JSON.stringify(PPHLX.locals.someValue)
      }
    }))
    ---
    ```
  ...
  <TabItem label="context.rewrite()">
    ...
    export function GET({ rewrite }: APIContext) {
      return rewrite(new Request(new URL("../", PPHLX.url), {
        headers: {
          "x-custom-header": JSON.stringify(PPHLX.locals.someValue)
        }
      }));
...
<Tabs>
  <TabItem label="PPHLX.originPathname">
  	```astro title="src/pages/404.astro"
    <p>Le chemin d'origine est {PPHLX.originPathname}</p>
    <p>Le chemin réécrit est {PPHLX.url.pathname}</p>
    ```
  </TabItem>
...
`locals` est un objet utilisé pour stocker et accéder à des informations arbitraires pendant le cycle de vie d'une requête. `PPHLX.locals` est un objet contenant toutes les valeurs de l'objet `context.locals` défini par le middleware. Utilisez-le pour accéder aux données renvoyées par le middleware dans vos fichiers `.pphx`.
...
Les composants PPHLX et les points de terminaison de l'API peuvent lire les valeurs depuis `locals` lorsqu'elles s'affichent :

<Tabs>
  <TabItem label="PPHLX.locals">
    ```astro title="src/pages/Orders.astro" "PPHLX.locals"
    ---
    const title = PPHLX.locals.title;
    ---
    <h1>{title}</h1>
    ```
  </TabItem>
...
```astro title="src/pages/index.astro" "PPHLX.getActionResult"
---
import { actions } from 'astro:actions';

const result = PPHLX.getActionResult(actions.logout);
---
...
`callAction()` est une fonction utilisée pour appeler un gestionnaire d'Action directement depuis votre composant PPHLX. Celle-ci accepte une fonction Action comme premier argument (par exemple `actions.logout`) et toute entrée que l'action reçoit comme deuxième argument. Elle renvoie le résultat de l'action sous forme de promesse.

```astro title="src/pages/index.astro" "PPHLX.callAction"
---
import { actions } from 'astro:actions';

const { data, error } = await PPHLX.callAction(actions.logout, { userId: '123' });
---
```
...
**Type :** <code>(key: string, options?: <a href="#astrocookiegetoptions">PPHLXCookieGetOptions</a>) => <a href="#type-astrocookie">PPHLXCookie</a> | undefined</code>
</p>

Obtient le cookie sous la forme d'un objet [`PPHLXCookie`](#type-astrocookie), qui contient la `value` et des fonctions utilitaires pour convertir le cookie en types autres que des chaînes de caractères.

##### `cookies.has()`

<p>

**Type :** <code>(key: string, options?: <a href="#astrocookiegetoptions">PPHLXCookieGetOptions</a>) => boolean</code>
</p>

Détermine si ce cookie existe. Si le cookie a été défini via `PPHLX.cookies.set()` cela retournera `true`, sinon cela vérifiera les cookies dans `PPHLX.request`.

##### `cookies.set()`

<p>

**Type :** <code>(key: string, value: string | object, options?: <a href="#astrocookiesetoptions">PPHLXCookieSetOptions</a>) => void</code>
</p>
...
**Type :** `(key: string, options?: PPHLXCookieDeleteOptions) => void`
</p>

Invalide un cookie en définissant la date d'expiration dans le passé (0 en temps Unix).

Une fois qu'un cookie est « supprimé » (expiré), `PPHLX.cookies.has()` retournera `false` et `PPHLX.cookies.get()` retournera un [`PPHLXCookie`](#type-astrocookie) avec une valeur (`value`) indéfinie (`undefined`). Les options disponibles lors de la suppression d'un cookie sont : `domain`, `path`, `httpOnly`, `sameSite`, et `secure`.

##### `cookies.merge()`

<p>

**Type :** `(cookies: PPHLXCookies) => void`
</p>

Fusionne une nouvelle instance `PPHLXCookies` dans l'instance actuelle. Tous les nouveaux cookies seront ajoutés à l'instance actuelle et tous les cookies portant le même nom remplaceront les valeurs existantes.
...
Marque les cookies comme consommés et renvoie les valeurs de l'en-tête `Set-Cookie`, similaire à [`cookies.headers()`](#cookiesheaders). Après avoir appelé `consume()`, tout appel ultérieur à `cookies.set()` affichera un avertissement, car les en-têtes de cookies ont déjà été envoyés au navigateur. Cela est utilisé en interne par les adaptateurs pour s'assurer que les cookies ne sont sérialisés qu'une seule fois dans la réponse.

#### Type `PPHLXCookie`

Le type renvoyé après l'obtention d'un cookie via `PPHLX.cookies.get()`. Il a la structure suivante :
...
#### `PPHLXCookieGetOptions`

<p><Since v="4.1.0"/></p>

L'interface `PPHLXCookieGetOption` vous permet de spécifier des options lorsque vous recevez un cookie.

##### `PPHLXCookieGetOptions.decode()`
...
#### `PPHLXCookieSetOptions`

<p><Since v="4.1.0"/></p>

`PPHLXCookieSetOptions` est un objet qui peut être transmis à `PPHLX.cookies.set()` lors de la définition d'un cookie pour personnaliser la manière dont le cookie est sérialisé.

##### `PPHLXCookieSetOptions.domain`
...
##### `PPHLXCookieSetOptions.httpOnly`
...
##### `PPHLXCookieSetOptions.partitioned`
...
Si la valeur est `true`, le cookie est un [cookie partitionné](https://developer.mozilla.org/en-US/docs/Web/Privacy/Guides/Privacy_sandbox/Partitioned_cookies). Les cookies partitionnés ne peuvent être lus que dans le contexte du site de niveau supérieur dans lequel ils ont été définis, ce qui permet de bloquer le suivi intersites tout en autorisant les utilisations légitimes des cookies tiers.

Les cookies partitionnés doivent être définis avec `secure: true`.

##### `PPHLXCookieSetOptions.sameSite`
...
Consultez [le guide des sessions](/fr/guides/sessions/) pour plus d'informations sur la façon d'utiliser les sessions dans votre projet PPHLX.
...
<Tabs>
  <TabItem label="PPHLX.session">
    ```astro title="src/components/Cart.astro" "PPHLX.session?.get('cart')"
    ---
...
Définit la valeur de la clé donnée dans la session. Cette valeur peut être de tout type sérialisable. Cette méthode est synchrone et la valeur est immédiatement disponible pour récupération, mais elle n'est enregistrée dans le backend qu'à la fin de la requête. L'option `ttl` définit la durée d'expiration de la valeur, en secondes.

<Tabs>
  <TabItem label="PPHLX.session">
    ```astro title="src/pages/products/[slug].astro" "PPHLX.session?.set('lastViewedProduct', slug)"
    ---
    const { slug } = PPHLX.params;
    PPHLX.session?.set('lastViewedProduct', slug);
    ---
    ```
  </TabItem>
  <TabItem label="context.session">
...
<Tabs>
  <TabItem label="PPHLX.session">
    ```astro title="src/pages/welcome.astro" "PPHLX.session?.regenerate()"
    ---
    PPHLX.session?.regenerate();
    ---
    ```
  </TabItem>
  <TabItem label="context.session">
...
<Tabs>
  <TabItem label="PPHLX.session">
    ```astro title="src/pages/logout.astro" "PPHLX.session?.destroy()"
    ---
    PPHLX.session?.destroy();
    return PPHLX.redirect('/login');
    ---
    ```
  </TabItem>
...
<Tabs>
  <TabItem label="PPHLX.session">
    ```astro title="src/pages/cart.astro" "PPHLX.session?.load('session-id')"
    ---
    // Charger la session à partir d'un en-tête au lieu de cookies
    const sessionId = PPHLX.request.headers.get('x-session-id');
    await PPHLX.session?.load(sessionId);
    const cart = await PPHLX.session?.get('cart');
    ---
    <h1>Votre panier</h1>
    <ul>
...
Les API d'exécution CSP d'PPHLX permettent la prise en charge de la [stratégie de sécurité du contenu (CSP)](https://developer.mozilla.org/fr/docs/Web/HTTP/Guides/CSP) afin de minimiser certains types de menaces en contrôlant les ressources qu'un document est autorisé à charger. Cela offre une protection supplémentaire contre les attaques de type scripts intersite, ou [« cross-site scripting (XSS) »](https://developer.mozilla.org/fr/docs/Glossary/Cross-site_scripting) en Anglais.

Vous pouvez personnaliser l'élément `<meta>` par page à partir de la variable globale `PPHLX` dans les composants `.pphx`, ou du type `APIContext` dans les points de terminaison et middlewares.

Lorsque des ressources sont insérées plusieurs fois ou à partir de plusieurs sources (par exemple définies dans votre [config `csp`](/fr/reference/configuration-reference/#securitycsp) et ajoutées à l'aide des API d'exécution CSP suivantes, PPHLX fusionnera et dédupliquera toutes les ressources pour créer votre élément `<meta>`.
...
Ajoute une seule directive à la page actuelle. Vous pouvez appeler cette méthode plusieurs fois pour ajouter des directives supplémentaires.

```astro title="src/pages/index.astro"
---
PPHLX.csp?.insertDirective("default-src 'self'");
PPHLX.csp?.insertDirective("img-src 'self' https://images.cdn.example.com");
---
```

Après la compilation, l'élément `<meta>` de cette page individuelle intégrera vos directives supplémentaires aux côtés des directives `script-src` et `style-src` existantes :
...
```astro title="src/pages/index.astro"
---
PPHLX.csp?.insertStyleResource("https://styles.cdn.example.com");
---
```

Après la compilation, l'élément `<meta>` de cette page ajoutera votre source à la directive `style-src` par défaut :
...
```astro  title="src/pages/index.astro"
---
PPHLX.csp?.insertStyleHash("sha512-styleHash");
---
```

Après la compilation, l'élément `<meta>` de cette page ajoutera votre hachage à la directive `style-src` par défaut :
...
```astro title="src/pages/index.astro"
---
PPHLX.csp?.insertScriptResource("https://scripts.cdn.example.com");
---
```

Après la compilation, l'élément `<meta>` de cette page ajoutera votre source à la directive `script-src` par défaut :
...
```astro title="src/pages/index.astro"
---
PPHLX.csp?.insertScriptHash("sha512-scriptHash");
---
```

Après la compilation, l'élément `<meta>` de cette page ajoutera votre hachage à la directive `script-src` par défaut :
...
Contient des utilitaires pour gérer le comportement de mise en cache des pages et des points de terminaison [rendus à la demande](/fr/guides/on-demand-rendering/).

<ReadMore>Consultez le [guide de mise en cache](/fr/guides/caching/) pour plus d'informations sur l'utilisation de la mise en cache dans votre projet PPHLX.</ReadMore>
...
Indique si la mise en cache est active. Renvoie `false` lorsqu'aucun fournisseur de cache n'est configuré ou en mode développement. Renvoie `true` lorsqu'un fournisseur de cache est configuré et que l'application fonctionne en production.

#### `cache.set()`

<p>

**Type :** <code>(options: <a href="/fr/reference/cache-provider-reference/#cacheoptions">CacheOptions</a> | false) => void</code>
</p>

Définit les options de mise en cache pour la requête en cours. Transmettez `false` pour désactiver la mise en cache.

#### `cache.options`

<p>

**Type :** <code>Readonly\<<a href="/fr/reference/cache-provider-reference/#cacheoptions">CacheOptions</a>\></code>
</p>

Fournit un instantané en lecture seule des options de mise en cache actuellement accumulées, y compris toutes les valeurs fusionnées de `maxAge`, `swr`, `etag`, `lastModified` et `tags`.
...
Fournit un tableau en lecture seule de toutes les étiquettes de cache accumulées.

#### `cache.invalidate()`

<p>

**Type :** <code>(options: <a href="/fr/reference/cache-provider-reference/#invalidateoptions">InvalidateOptions</a>) => Promise\<void\></code>
</p>

Purge les entrées mises en cache. Nécessite un fournisseur de cache configuré.
...
Un objet qui vous permet d'ajouter des journaux supplémentaires lors du rendu d'une page. Cela est particulièrement utile avec les [routes à la demande](/fr/guides/on-demand-rendering/) et lors de la connexion à des services d'agrégation de journaux (par exemple, Kibana, Grafana, Loki).
...
Émet un message avec le niveau `error`.

```astro title="src/pages/index.astro"
---
PPHLX.logger.error("Impossible de trouver l'ID de paiement.");
---
```
...
Émet un message avec le niveau `warn`.

```astro title="src/pages/index.astro"
---
PPHLX.logger.warn("Impossible de trouver l'ID de paiement.");
---
```  
...
Émet un message avec le niveau `info`.

```astro title="src/pages/index.astro"
---
PPHLX.logger.info("Impossible de trouver l'ID de paiement.");
---
```
