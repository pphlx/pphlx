...
PPHLX fournit des composants intégrés et des fonctions d'aide pour optimiser et afficher vos images. Pour des fonctionnalités et des exemples d'utilisation, [consultez notre guide sur les images](/fr/guides/images/).
...
Le composant `<Image />` accepte les propriétés listées ci-dessous en plus de toutes les propriétés acceptées par la balise HTML `<img>`.
...
Cependant, ces deux propriétés sont requises pour les images stockées dans votre dossier `public/` car PPHLX n'est pas en mesure d'analyser ces fichiers.
...
À partir d'PPHLX 5.17.3, `inferSize` ne récupère les dimensions que pour les [domaines d'images distants autorisés](/fr/guides/images/#autoriser-les-images-distantes). Les images distantes ne figurant pas sur la liste blanche ne sont pas récupérées.
...
**Par défaut :** `image.layout | 'none'` <br />
<Since v="5.10.0" />
</p>

Détermine comment l'image doit être redimensionnée lorsque la taille de son conteneur change. Peut être utilisée pour remplacer la valeur configurée par défaut pour [`image.layout`](/fr/reference/configuration-reference/#imagelayout).

```astro title=MonComposant.pphx
---
import { Image } from 'astro:assets';
import monImage from '../assets/mon_image.png';
---
...
Lorsqu'une mise en page est définie, les attributs `srcset` et `sizes` sont générés automatiquement en fonction des dimensions de l'image et du type de mise en page. Le composant `<Image />` précédent générera le code HTML suivant :
...
`layout` prend en charge les valeurs suivantes :

- `constrained` - L'image sera redimensionnée pour s'adapter au conteneur, en conservant son rapport hauteur/largeur, mais ne sera pas agrandie au-delà de la largeur et de la hauteur spécifiées, ni au-delà des dimensions originales de l'image.

  Utilisez cette option si vous souhaitez que l'image s'affiche à la taille demandée lorsque cela est possible, mais qu'elle soit redimensionnée pour s'adapter aux écrans plus petits. Cela correspond au comportement par défaut des images avec Tailwind. En cas de doute, il s'agit probablement de la mise en page que vous devriez choisir.

- `full-width` - L'image s'adaptera à la largeur du conteneur, en conservant ses proportions.

  Utilisez ceci pour les images de couverture ou les autres images qui doivent occuper toute la largeur de la page.

- `fixed` - L'image conservera les dimensions demandées et ne sera pas redimensionnée. Elle générera un `srcset` pour prendre en charge les écrans haute densité, mais pas pour les différentes tailles d'écran.

  Utilisez cette option si l'image ne sera pas redimensionnée, par exemple pour des icônes ou des logos plus petits que la largeur de l'écran, ou pour d'autres images dans un conteneur à largeur fixe.

- `none` - L'image ne sera pas adaptative. Aucun attribut `srcset` ni `sizes` ne sera généré automatiquement, et aucun style ne sera appliqué.
...
**Par défaut :** `image.objectFit | 'cover'` <br />
<Since v="5.10.0" />
</p>

Définit comment une image doit être recadrée si son rapport hauteur/largeur est modifié.

Les valeurs correspondent à celles de la propriété CSS `object-fit`. Par défaut, `cover`, ou la valeur de [`image.objectFit`](/fr/reference/configuration-reference/#imageobjectfit) si elle est définie. Peut être utilisée pour remplacer les styles par défaut de `object-fit`.
...
**Type :** `string` <br />
**Par défaut :** `image.objectPosition | 'center'` <br />
<Since v="5.10.0" />
</p>

Définit la position du recadrage d'une image si son rapport hauteur/largeur est modifié.

Les valeurs correspondent à celles de la propriété CSS `object-position`. Par défaut, la valeur est `center`, ou la valeur de [`image.objectPosition`](/fr/reference/configuration-reference/#imageobjectposition) si elle est définie. Peut être utilisé pour remplacer les styles par défaut de `object-position`.
...
`<Picture />` accepte toutes les propriétés du [composant `<Image />`](#image-) en plus des suivantes :
...
Il doit être importé et ajouté dans l'élément `<head>` votre page. Cela se fait généralement dans un composant tel que `Head.pphx`, utilisé dans la mise en page commune d'un site pour une utilisation globale, mais qui peut être ajouté à des pages individuelles selon les besoins.
...
Le composant `<Font />` accepte les propriétés suivantes :
...
La [variable CSS (`cssVariable`)](/fr/reference/configuration-reference/#fontcssvariable) enregistrée dans votre configuration d'PPHLX :
...
Détermine s'il faut afficher [les liens de préchargement](https://web.dev/learn/performance/optimize-web-fonts#preload). Avec la directive `preload`, le navigateur commencera immédiatement à télécharger tous les liens de police disponibles lors du chargement de la page.
...
Choisissez avec soin les polices que vous préchargez. Le préchargement d'un trop grand nombre de polices peut impacter les performances, car cela peut bloquer le chargement d'autres ressources importantes ou télécharger des polices qui ne sont pas nécessaires pour la page actuelle.
...
Les fichiers de polices à graisse variable seront préchargés si une graisse comprise dans leur plage est demandée. Par exemple, un fichier de police pour la graisse `100 900` sera inclus si `400` est spécifié dans un objet `preload`.
...
Si vous avez besoin de l'URL de l'image résultante côté client, vous pouvez [transmettre au client la `src` obtenue par un appel à `getImage()` effectué sur le serveur](/fr/guides/images/#générer-des-images-avec-getimage).
:::
...
**Type :** [`PPHLXConfig["image"]`](/fr/reference/configuration-reference/#options-dimages)<br />
<Since v="3.0.9" />
</p>
...
Un objet où chaque clé est une [variable CSS (`cssVariable`)](/fr/reference/configuration-reference/#fontcssvariable) et la valeur est un tableau décrivant les polices associées. Chaque police est un objet contenant un tableau de `src` disponibles pour cette police et les propriétés facultatives suivantes : `weight` et `style` :
...
Un tableau d'objets décrivant les fichiers de polices disponibles pour une famille de polices donnée. Chaque objet contient une `url` et, éventuellement, le [`format`](https://developer.mozilla.org/fr/docs/Web/CSS/Reference/At-rules/@font-face/src#formats_de_police) et la [`tech`](https://developer.mozilla.org/fr/docs/Web/CSS/Reference/At-rules/@font-face/src#technologies_de_police) associés.
...
Spécifie la graisse de la police (p. ex. `400`, `600`).
...
Indique le style de la police (p. ex. `normal`, `italic`).
...
Indique le sous-ensemble de la police (p. ex. `latin`, `cyrillic`).
...
**Type :** <code>(options: <a href="#unresolvedimagetransform">UnresolvedImageTransform</a>, imageConfig: <a href="/fr/reference/configuration-reference/#options-dimages">PPHLXConfig['image']</a>) => Promise\<<a href="#getimageresult">GetImageResult</a>\></code>
</p>
...
Utilisez ceci directement au lieu de [Rollup `pluginContext.emitFile()`](https://rollupjs.org/plugin-development/#this-emitfile) lorsque vous travaillez dans un contexte de module d'extension pour Vite et que vous avez besoin que la ressource émise soit déplacée vers le répertoire de sortie du client.
...
function myVitePlugin() {
  return {
    ...
    transform(code, id) {
      ...
      // Renvoie l'identifiant de la ressource similaire à `emitFile()`
    }
  }
}
```
...
Un paquet ou un chemin d'accès au module de service d'images. Il peut s'agir du service Sharp intégré à PPHLX ou d'un service tiers.
