...
Les fonctions d'assistance suivantes peuvent être importées depuis le module middleware standard lors de la création d'une [intégration PPHLX](/fr/reference/integrations-reference/) :
...
Une API de bas niveau pour créer un objet [`APIContext`](/fr/reference/api-reference/) à transmettre à une [fonction `onRequest()`](#onrequest) d'un middleware d'PPHLX.

Cette fonction peut être utilisée par les intégrations/adaptateurs pour exécuter par programmation le middleware d'PPHLX.
...
Un objet pour [créer un contexte](#createcontext) à transmettre à un middleware PPHLX. Il contient les propriétés suivantes :
...
Un objet contenant les paramètres optionnels à transmettre à [`PPHLX.params`](/fr/reference/api-reference/#params).
...
Un objet permettant de stocker des informations arbitraires provenant d'un middleware, accessible à l'utilisateur via [`PPHLX.locals`](/fr/reference/api-reference/#locals).
...
L'[adresse IP](https://fr.wikipedia.org/wiki/Adresse_IP) de la requête.

Elle doit être fournie par l'adaptateur ou la plateforme à partir d'une source fiable (par exemple, l'adresse du socket, l'en-tête fourni par la plateforme).

Si cette valeur n'est pas fournie, l'accès à `clientAddress` générera une erreur.
...
Représente une fonction middleware pour PPHLX. Les gestionnaires de middleware reçoivent deux arguments et peuvent soit renvoyer directement un objet `Response`, soit appeler `next()` pour invoquer le middleware suivant dans la chaîne. Vous pouvez également utiliser [`defineMiddleware()`](#definemiddleware) pour garantir la sûreté du typage pour votre middleware.
...
Un objet de [contexte PPHLX](/fr/reference/api-reference/) reflétant de nombreuses propriétés globales disponibles dans `PPHLX`.
...
Depuis PPHLX v4.13.0, `next()` accepte un paramètre de chemin d'URL facultatif sous la forme d'une chaîne de caractères, d'une `URL` ou d'un objet `Request` pour [réécrire](/fr/guides/routing/#réécritures) la requête actuelle sans déclencher une nouvelle phase de rendu.
