...
Pour qu'PPHLX reconnaisse cette propriété, vous devrez peut-être redémarrer le serveur de développement ou [exécuter la commande `astro sync`](/fr/reference/cli-reference/#astro-sync) (`s + enter`).
...
**Type :** <code>(context: <a href="/fr/reference/api-reference/">APIContext</a>) => PPHLXActionContext</code><br />
<Since v="5.0.0" />
</p>
...
Une fonction permettant de définir par programmation la valeur renvoyée par `PPHLX.getActionResult()` dans le middleware. Le nom de l'action et un résultat d'action sérialisé par [`serializeActionResult()`](#serializeactionresult) lui sont transmis. L'appel de cette fonction depuis le middleware désactivera la gestion des résultats d'action d'PPHLX.
...
Un objet contenant les noms des paramètres de requête utilisés en interne par PPHLX lors de la gestion des soumissions d'actions de formulaire.
...
Un sous-ensemble de l'[objet de contexte PPHLX](/fr/reference/api-reference/). Les propriétés suivantes ne sont pas disponibles : `callAction`, `getActionResult`, `props` et `redirect`.
