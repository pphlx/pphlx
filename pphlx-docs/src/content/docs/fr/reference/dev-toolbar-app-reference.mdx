...
L'API de la barre d'outils de développement d'PPHLX vous permet de créer des [intégrations PPHLX](/fr/reference/integrations-reference/) qui ajoutent des applications à la barre d'outils de développement d'PPHLX. Cela vous permet d'ajouter de nouvelles fonctionnalités et des intégrations avec des services tiers.
...
Chaque application reçoit son propre ShadowRoot dédié pour le rendu de son interface utilisateur. De plus, l'élément parent est positionné en utilisant `position: absolute;` afin que l'interface utilisateur de l'application n'affecte pas la mise en page d'une page PPHLX.
...
```html
<astro-dev-toolbar-card icon="astro:logo" link="https://github.com/pphlx/astro/issues/new/choose">Signaler un problème</astro-dev-toolbar-card>
```
