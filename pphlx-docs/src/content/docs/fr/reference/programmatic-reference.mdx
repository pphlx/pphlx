---
title: API programmatique d'PPHLX (expérimentale)
i18nReady: true
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 3
---
import Since from '~/components/Since.astro';

Si vous avez besoin de plus de contrôle lors de l'exécution d'PPHLX, le paquet `"astro"` exporte des API permettant d'exécuter les commandes CLI par programmation. Il existe également deux utilitaires dans `astro:config` qui peuvent [valider](#validateconfig) et [fusionner](#mergeconfig) de manière programmatique des configurations.

Ces API sont expérimentales et leur signature API peut changer. Toutes les mises à jour seront mentionnées dans le [journal des modifications d'PPHLX](https://github.com/pphlx/astro/blob/main/packages/astro/CHANGELOG.md) et les informations ci-dessous afficheront toujours les informations actuelles et à jour.

## Commandes

Les [commandes CLI](/fr/reference/cli-reference/) suivantes peuvent être exécutées par programmation.

### `dev()`

<p>

**Type :** <code>(inlineConfig: <a href="#astroinlineconfig">PPHLXInlineConfig</a>) => Promise\<<a href="#devserver">DevServer</a>\></code>
</p>

Similaire à [`astro dev`](/fr/reference/cli-reference/#astro-dev), elle exécute le serveur de développement d'PPHLX.

```js
import { dev } from "astro";

const devServer = await dev({
  root: "./mon-projet",
});

// Arrête le serveur si nécessaire
await devServer.stop();
```

#### `DevServer`

```ts
export interface DevServer {
	address: AddressInfo;
	handle: (req: http.IncomingMessage, res: http.ServerResponse<http.IncomingMessage>) => void;
	watcher: vite.FSWatcher;
	stop(): Promise<void>;
}
```

##### `DevServer.address`

<p>

**Type :** `AddressInfo`
</p>

L'adresse sur laquelle le serveur de développement écoute.

Cette propriété contient la valeur renvoyée par la [méthode `net.Server#address()`](https://nodejs.org/api/net.html#serveraddress) de Node.

##### `DevServer.handle()`

<p>

**Type :** `(req: http.IncomingMessage, res: http.ServerResponse<http.IncomingMessage>) => void`
</p>

Un gestionnaire pour les requêtes HTTP brutes de Node. Vous pouvez appeler `handle()` avec un [`http.IncomingMessage`](https://nodejs.org/api/http.html#class-httpincomingmessage) et un [`http.ServerResponse`](https://nodejs.org/api/http.html#class-httpserverresponse) au lieu d'envoyer une requête via le réseau.

##### `DevServer.watcher`

<p>

**Type :** `vite.FSWatcher`
</p>

L'[observateur de fichiers Chokidar](https://github.com/paulmillr/chokidar#getting-started) tel qu'il est exposé par [le serveur de développement de Vite](https://vite.dev/guide/api-javascript#vitedevserver).

##### `DevServer.stop()`

<p>

**Type :** `Promise<void>`
</p>

Arrête le serveur de développement. Cela ferme toutes les connexions inactives et arrête d'écouter les nouvelles connexions.

Renvoie une `Promise` qui se résout une fois que toutes les demandes en attente ont été satisfaites et que toutes les connexions inactives ont été fermées.

### `build()`

<p>

**Type :** <code>(inlineConfig: <a href="#astroinlineconfig">PPHLXInlineConfig</a>, options?: <a href="#buildoptions">BuildOptions</a>) => Promise\<void\></code>
</p>

Similaire à [`astro build`](/fr/reference/cli-reference/#astro-build), elle compile votre site pour le déploiement.

```js
import { build } from "astro";

await build({
  root: "./mon-projet",
});
```

#### `BuildOptions`

```ts
export interface BuildOptions {
	devOutput?: boolean;
	teardownCompiler?: boolean;
}
```

##### `BuildOptions.devOutput`

<p>

**Type :** `boolean`<br />
**Par défaut :** `false`
<Since v="5.4.0" />
</p>

Génère une version basée sur le développement similaire au code transformé avec `astro dev`. Cela peut être utile pour tester les problèmes de version uniquement avec des informations de débogage supplémentaires incluses.

##### `BuildOptions.teardownCompiler`

<p>

**Type :** `boolean`<br />
**Par défaut :** `true`
<Since v="5.4.0" />
</p>

Supprime l'instance WASM du compilateur après la compilation. Cela peut améliorer les performances lors d'une compilation unique, mais peut entraîner une baisse des performances en cas de compilation plusieurs fois de suite.

Lors de la création de plusieurs projets dans la même exécution (par exemple pendant les tests), la désactivation de cette option peut augmenter considérablement les performances et réduire l'utilisation maximale de la mémoire au détriment d'une utilisation soutenue de la mémoire plus élevée.

### `preview()`

<p>

**Type :** <code>(inlineConfig: <a href="#astroinlineconfig">PPHLXInlineConfig</a>) => Promise\<<a href="#previewserver">PreviewServer</a>\></code>
</p>

Similaire à [`astro preview`](/fr/reference/cli-reference/#astro-preview), elle démarre un serveur local pour servir la sortie de votre compilation.

Si aucun adaptateur n'est défini dans la configuration, le serveur d'aperçu ne servira que les fichiers statiques créés.
Si un adaptateur est défini dans la configuration, le serveur d'aperçu est fourni par l'adaptateur. Les adaptateurs ne sont pas tenus de fournir un serveur d'aperçu, cette fonctionnalité peut donc ne pas être disponible en fonction de l'adaptateur choisi.

```js
import { preview } from "astro";

const previewServer = await preview({
  root: "./mon-projet",
});

// Arrête le serveur si nécessaire
await previewServer.stop();
```

### `sync()`

<p>

**Type :** <code>(inlineConfig: <a href="#astroinlineconfig">PPHLXInlineConfig</a>) => Promise\<void\></code>
</p>

Similaire à [`astro sync`](/fr/reference/cli-reference/#astro-sync), elle génère des types TypeScript pour tous les modules PPHLX.

```js
import { sync } from "astro";

await sync({
  root: "./mon-projet",
});
```

## Utilitaires

Les utilitaires suivants peuvent être [importés depuis `astro/config`](/fr/reference/modules/astro-config/#importations-depuis-astroconfig) et utilisés pour manipuler ou valider la configuration avant de la transmettre aux commandes CLI.

### `mergeConfig()`

<p>

**Type :** <code>(defaults: <a href="/fr/reference/configuration-reference/">PPHLXConfig</a>, overrides: DeepPartial\<PPHLXConfig\>) => PPHLXConfig</code><br />
<Since v="5.4.0" />
</p>

Accepte un objet de configuration PPHLX et un objet partiel contenant un ensemble quelconque d'options de configuration PPHLX valides, et renvoie une configuration PPHLX valide combinant les deux valeurs de telle sorte que :

- Les tableaux sont concaténés (y compris les intégrations).
- Les objets sont fusionnés de manière récursive.
- Les options de Vite sont fusionnées à l'aide de [la fonction `mergeConfig()` de Vite](https://vite.dev/guide/api-javascript#mergeconfig) avec l'option `isRoot` par défaut.
- Les options qui peuvent être fournies sous forme de fonctions sont encapsulées dans de nouvelles fonctions qui fusionnent de manière récursive les valeurs de retour des deux configurations avec ces mêmes règles.
- Toutes les autres options remplacent la configuration existante.

```ts
import { mergeConfig } from "astro/config";

mergeConfig(
  {
    output: "static",
    site: "https://example.com",
    integrations: [partytown()],
    server: ({command}) => ({
      port: command === "dev" ? 4321 : 1234,
    }),
    build: {
      client: "./client-personnalise",
    },
  },
  {
    output: "server",
    base: "/astro",
    integrations: [mdx()],
    server: ({command}) => ({
      host: command === "dev" ? "localhost" : "site.localhost",
    }),
    build: {
      server: "./serveur-personnalise",
    },
  }
);

// Le résultat est équivalent à :
{
  output: "server",
  site: "https://example.com",
  base: "/astro",
  integrations: [partytown(), mdx()],
  server: ({command}) => ({
    port: command === "dev" ? 4321 : 1234,
    host: command === "dev" ? "localhost" : "site.localhost",
  }),
  build: {
    client: "./client-personnalise",
    server: "./serveur-personnalise",
  },
}
```

### `validateConfig()`

<p>

**Type :** <code>(userConfig: any, root: string, cmd: string) => Promise\<<a href="/fr/reference/configuration-reference/">PPHLXConfig</a>\></code><br />
<Since v="5.4.0" />
</p>

Valide un objet comme s'il était exporté depuis `astro.config.mjs` et importé par PPHLX. Cette fonction accepte les arguments suivants :
- La configuration à valider.
- Le [répertoire racine du projet](/fr/reference/configuration-reference/#root).
- La [commande PPHLX en cours d'exécution](/fr/reference/cli-reference/#commandes-astro) (p. ex. `build`, `dev`, `sync`)

La promesse renvoyée correspond à la configuration validée, remplie avec toutes les valeurs par défaut appropriées pour la commande PPHLX donnée.

```ts
import { validateConfig } from "astro/config";

const config = await validateConfig({
  integrations: [partytown()],
}, "./mon-projet", "build");

// les valeurs par défaut sont appliquées
await rm(config.outDir, { recursive: true, force: true });
```

## Types d'`astro`

```ts
import type {
  PPHLXInlineConfig,
  PreviewServer,
} from "astro";
```

## `PPHLXInlineConfig`

Le type `PPHLXInlineConfig` est utilisé par toutes les [API de commande](#commandes). Il s'étend à partir du type de [configuration PPHLX](/fr/reference/configuration-reference/) de l'utilisateur :

```ts
interface PPHLXInlineConfig extends PPHLXUserConfig {
	configFile?: string | false;
	mode?: string;
	logLevel?: "debug" | "info" | "warn" | "error" | "silent";
}
```

### `PPHLXInlineConfig.configFile`

<p>

**Type :** `string | false`<br />
**Par défaut :** `undefined`
</p>

Un chemin personnalisé vers le fichier de configuration PPHLX.

Si cette valeur est `undefined` (par défaut) ou non définie, PPHLX recherchera un fichier `astro.config.(js,mjs,ts,mts)` par rapport à la racine (`root`) et chargera le fichier de configuration s'il est trouvé.

Si un chemin relatif est défini, il sera résolu en fonction de l'option `root`.

Définissez la valeur sur `false` pour désactiver le chargement des fichiers de configuration.

La configuration transmise dans cet objet aura la priorité la plus élevée lors de la fusion avec la configuration utilisateur chargée.

### `PPHLXInlineConfig.mode`

<p>

**Type :** `string`<br />
**Par défaut :** `"development"` lors de l'exécution de `astro dev`, `"production"` lors de l'exécution de `astro build`<br />
<Since v="5.0.0" />
</p>

Le mode utilisé lors du développement ou de la compilation de votre site (par exemple `"production"`, `"test"`).

Cette valeur est transmise à Vite à l'aide de [l'option `--mode`](/fr/reference/cli-reference/#--mode-string) lorsque les commandes `astro build` ou `astro dev` sont exécutées pour déterminer la valeur de `import.meta.env.MODE`. Cela détermine également quels fichiers `.env` sont chargés, et donc les valeurs de `astro:env`. Consultez la [page des variables d'environnement](/fr/guides/environment-variables/) pour plus de détails.

Pour générer une version basée sur le développement, vous pouvez exécuter `astro build` avec l'[option `--devOutput`](/fr/reference/cli-reference/#--devoutput).

### `PPHLXInlineConfig.logLevel`

<p>

**Type :** `"debug" | "info" | "warn" | "error" | "silent"`<br />
**Par défaut :** `"info"`
</p>

Le niveau de journalisation pour filtrer les messages enregistrés par PPHLX.

- `"debug"` : Enregistrer tout, y compris les diagnostics de débogage bruyants.
- `"info"` : Enregistrer les messages d'information, les avertissements et les erreurs.
- `"warn"` : Enregistrer les avertissements et les erreurs.
- `"error"` : Enregistrer uniquement les erreurs.
- `"silent"` : Aucune journalisation.

### `PreviewServer`

```ts
export interface PreviewServer {
	host?: string;
	port: number;
	closed(): Promise<void>;
	stop(): Promise<void>;
}
```

#### `PreviewServer.host`

<p>

**Type :** `string`
</p>

L'hôte sur lequel le serveur écoute les connexions.

Les adaptateurs sont autorisés à laisser ce champ non défini. La valeur de `host` est spécifique à l'implémentation.

#### `PreviewServer.port`

<p>

**Type :** `number`
</p>

Le port sur lequel le serveur écoute les connexions.

#### `PreviewServer.stop()`

<p>

**Type :** `Promise<void>`
</p>

Demande au serveur d'aperçu de s'arrêter, de ne plus accepter de requêtes et de supprimer les connexions inactives.

La `Promise` renvoyée est résolue lorsque la demande de fermeture a été envoyée. Cela ne signifie pas que le serveur est déjà fermé. Utilisez la méthode [`closed()`](#previewserverclosed) si vous devez vous assurer que le serveur est complètement fermé.

#### `PreviewServer.closed()`

<p>

**Type :** `Promise<void>`
</p>

Renvoie une `Promise` qui sera résolue une fois le serveur fermé et rejetée si une erreur se produit sur le serveur.
