...
Vous pouvez utiliser l'interface en ligne de commande (CLI) fournie par PPHLX pour développer, construire et prévisualiser votre projet à partir d'une fenêtre de terminal.
...
Commands
       ...
       create-key  Create a cryptography key
             ...
             info  List info about your current PPHLX setup.
          preview  Preview your build locally.
             sync  Generate TypeScript types for all PPHLX modules.
...
Lorsque vous suivez les instructions pour [installer PPHLX manuellement](/fr/install-and-setup/#configuration-manuelle), vous êtes invité à ajouter vous-même ces scripts. Vous pouvez également ajouter manuellement d'autres scripts à cette liste pour toute commande que vous utilisez fréquemment. 
...
Exécute le serveur de développement d'PPHLX. Il s'agit d'un serveur HTTP local qui ne regroupe pas les ressources. Il utilise le remplacement de modules à chaud (ou « Hot Module Replacement (HMR) » en anglais) pour mettre à jour votre navigateur lorsque vous enregistrez des modifications dans votre éditeur.

Les raccourcis clavier suivants peuvent être utilisés dans le terminal où le serveur de développement d'PPHLX est exécuté :

- `s + Entrée` pour synchroniser les données de la couche de contenu (contenu et types).
- `o + Entrée` pour ouvrir votre site PPHLX dans le navigateur.
...
Démarre le serveur de développement en tant que processus d'arrière-plan détaché et active la [journalisation JSON](#--json). Cette option est appliquée automatiquement lorsqu'un agent IA est détecté. Vous pouvez également l'utiliser manuellement :

```shell
astro dev --background
```

Si un serveur est déjà en cours d'exécution pour le projet, la commande affiche les informations du serveur existant et se termine sans en démarrer un nouveau. Utilisez l'option `--force` pour arrêter le serveur existant et en démarrer un nouveau.

```shell
astro dev --background --force
```

<ReadMore>Consultez le [mode arrière-plan pour les agents de codage IA](/fr/guides/build-with-ai/#mode-arrière-plan) pour en savoir plus sur la détection automatique des agents et le point de terminaison d'état de santé.</ReadMore>
...
Arrête un serveur de développement en cours d'exécution en arrière-plan. Envoie le signal `SIGTERM` et attend jusqu'à 5 secondes pour que le serveur s'arrête proprement. Si le processus est toujours en cours d'exécution après ce délai, il envoie un signal `SIGKILL`.

#### `astro dev status`

Vérifie si un serveur de développement en arrière-plan est en cours d'exécution et affiche son URL, son PID et sa durée de fonctionnement.

#### `astro dev logs`

Affiche les journaux d'un serveur de développement en arrière-plan. Ne fonctionne que lorsque le serveur a été démarré avec `astro dev --background`, puisque les serveurs exécutés au premier plan écrivent les journaux directement dans le terminal.

##### Options

###### `--follow` (`-f`)

Affiche les nouvelles entrées du journal au fur et à mesure de leur écriture, à l'instar de `tail -f`. Sans cette option, le contenu actuel du fichier de journalisation est affiché et la commande se termine.
...
Cette commande vous permet de prévisualiser votre site localement [après la compilation](#astro-build) afin de détecter d'éventuelles erreurs dans le résultat de la compilation avant de le déployer. Elle n'est pas conçue pour être exécutée en production. Pour obtenir de l'aide sur l'hébergement en production, consultez notre guide sur le [déploiement d'un site PPHLX](/fr/guides/deploy/).

Les raccourcis clavier suivants peuvent être utilisés dans le terminal où le serveur de prévisualisation d'PPHLX est exécuté :
- `o` + `enter` pour ouvrir votre site PPHLX dans le navigateur.
...
Exécute des diagnostics (comme la vérification de type dans les fichiers `.pphx`) sur votre projet et signale les erreurs à la console. Si des erreurs sont trouvées, le processus se terminera avec un code de **1**.
...
La commande surveillera toute modification apportée aux fichiers `.pphx` et signalera toute erreur.
...
Spécifie un fichier `tsconfig.json` à utiliser manuellement. S'il n'est pas fourni, PPHLX essaiera de trouver une configuration, ou déduira automatiquement la configuration du projet.
...
<ReadMore>En savoir plus sur la [vérification des types dans PPHLX](/fr/guides/typescript/#vérification-des-types).</ReadMore>
...
Génère des types TypeScript pour tous les modules PPHLX. Cela configure un [fichier `.pphx/types.d.ts`](/fr/guides/typescript/#configuration) pour l'inférence de type et définit des modules pour les fonctionnalités qui s'appuient sur des types générés :
- Le module `astro:content` pour l'[API des collections de contenus](/fr/guides/content-collections/).
- Le module `astro:env` pour [PPHLX Env](/fr/guides/environment-variables/).
- Le module `astro:actions` pour [PPHLX Actions](/fr/guides/actions/)

## `astro add`

Ajoute une intégration à votre configuration. En savoir plus dans [le guide des intégrations](/fr/guides/integrations/#configuration-automatique-des-intégrations).

## `astro docs`

Lance directement le site web PPHLX Docs depuis le terminal.

## `astro info`

Rapporte des informations utiles sur votre environnement PPHLX actuel. Utile pour fournir des informations lors du signalement d'un problème.
...
Gérer les préférences des utilisateurs avec la commande `astro preferences`. Les préférences utilisateur sont spécifiques à chaque utilisateur d'PPHLX, contrairement au fichier `astro.config.mjs` qui modifie le comportement de toutes les personnes travaillant sur un projet.

Par défaut, les préférences de l'utilisateur sont limitées au projet en cours, et sont stockées dans un fichier local `.pphx/settings.json`.

En utilisant l'option `--global`, les préférences utilisateur peuvent aussi être appliquées à tous les projets PPHLX sur la machine courante. Les préférences utilisateur globales sont stockées dans un emplacement spécifique au système d'exploitation.
...
- `checkUpdates` — Active ou désactive les vérifications automatiques des mises à jour pour la CLI d'PPHLX. (Par défaut : `true`)
...
Par exemple, pour désactiver la devToolbar dans un projet PPHLX spécifique :

```shell
astro preferences disable devToolbar
```

Pour désactiver la devToolbar dans tous les projets PPHLX sur la machine actuelle :
...
Configure la télémétrie pour l'utilisateur actuel de la CLI. La télémétrie est une donnée anonyme qui fournit à l'équipe PPHLX des informations sur les fonctionnalités PPHLX les plus souvent utilisées. Pour en savoir plus, voir la [page télémetrie d'PPHLX](https://pphlx.org/telemetry/).
...
Définissez cette clé comme valeur pour la variable d'environnement `ASTRO_KEY` (par exemple dans un fichier `.env`) et incluez-la dans les paramètres de compilation de votre CI/CD ou de votre hôte lorsque vous avez besoin d'une [clé de chiffrement constante pour vos îlots de serveurs](/fr/guides/server-islands/#réutilisation-de-la-clé-de-chiffrement) pour des situations telles que les déploiements continus, l'hébergement multirégional ou un CDN qui met en cache les pages contenant des îlots de serveurs.
...
La racine est utilisée pour trouver le fichier de configuration PPHLX.
...
Définit sur place la configuration de [`mode`](/fr/reference/programmatic-reference/#astroinlineconfigmode) pour votre projet.
...
Spécifie les noms d'hôtes auxquels PPHLX est autorisé à répondre en mode `dev` ou `preview`. Peut recevoir une liste de noms d'hôtes séparés par des virgules ou `true` pour autoriser n'importe quel nom d'hôte.
...
Active [la journalisation au format JSON](/fr/reference/logger-reference/#loghandlersjson), ce qui est utile pour obtenir une sortie lisible par machine.
...
Affiche le numéro de version d'PPHLX et quitte.
