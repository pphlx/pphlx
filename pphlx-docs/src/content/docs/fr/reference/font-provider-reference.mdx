---
title: API des fournisseurs de polices d'PPHLX
sidebar:
...
L'[API des polices](/fr/guides/fonts/) vous permet d'accéder aux polices de manière unifiée. Chaque famille nécessite l'utilisation d'un fournisseur de polices pour PPHLX, qui télécharge les fichiers de polices à partir d'un service distant ou charge les fichiers de polices locaux depuis le disque.

## Fournisseurs intégrés

PPHLX exporte les fournisseurs de polices intégrés depuis `astro/config` :
...
Transmettez au fournisseur de polices Adobe un ID chargé en tant que [variable d'environnement dans votre fichier de configuration d'PPHLX](/fr/guides/environment-variables/#dans-le-fichier-de-configuration-dastro).
...
Lorsque la valeur n'est pas définie, PPHLX essaiera par défaut de déduire la valeur en fonction de la première [`source`](#src).
...
Vous pouvez ensuite enregistrer ce fournisseur de polices dans la configuration d'PPHLX :
...
Vous pouvez définir un fournisseur de polices pour PPHLX en utilisant un fournisseur Unifont en interne :
