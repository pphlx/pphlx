---
title: API des intégrations d'PPHLX
sidebar:
...
**Les intégrations d'PPHLX** ajoutent de nouvelles fonctionnalités et de nouveaux comportements à votre projet avec seulement quelques lignes de code.

Cette page de référence est destinée aux personnes qui écrivent leur propre intégration. Pour apprendre à utiliser une intégration dans votre projet, consultez plutôt notre guide sur l'[utilisation des intégrations](/fr/guides/integrations/).

## Exemples

Les intégrations officielles d'PPHLX peuvent vous servir de référence pour construire vos propres intégrations.
...
PPHLX fournit des hooks que les intégrations peuvent implémenter pour s'exécuter pendant certaines parties du cycle de vie d'PPHLX. Les hooks PPHLX sont définis dans l'interface `IntegrationHooks`, qui fait partie de l'espace de noms global `PPHLX`. Chaque hook dispose d'une [option `logger`](#astrointegrationlogger) qui vous permet d'utiliser le logger d'PPHLX pour écrire des journaux.

Les hooks suivants sont intégrés à PPHLX :

### `astro:config:setup`

**Prochain hook :** [`astro:route:setup`](#astroroutesetup)

**Quand :** Lors de l'initialisation, avant que les configurations de [Vite](https://vite.dev/config/) ou d'[PPHLX](/fr/reference/configuration-reference/) ne soient résolues.

**Pourquoi :** Pour étendre la configuration du projet. Cela comprend la mise à jour de la [configuration PPHLX](/fr/reference/configuration-reference/), l'application des [modules d'extension de Vite](https://vite.dev/guide/api-plugin.html), l'ajout de moteurs de rendu de composants et l'injection de scripts dans la page.
...
Une copie en lecture seule de la [configuration PPHLX](/fr/reference/configuration-reference/) fournie par l'utilisateur. Celle-ci est résolue _avant_ que toutes les autres intégrations aient été exécutées. Si vous avez besoin d'une copie de la configuration après que toutes les intégrations ont terminé leurs mises à jour, [consultez le hook `astro:config:done`](#astroconfigdone).
...
**Type :** <code>(renderer: <a href="/fr/reference/renderer-reference/#astrorenderer">PPHLXRenderer</a>) => void;</code><br />
**Exemples :** [`svelte`](https://github.com/pphlx/astro/blob/main/packages/integrations/svelte/src/index.ts), [`react`](https://github.com/pphlx/astro/blob/main/packages/integrations/react/src/index.ts), [`preact`](https://github.com/pphlx/astro/blob/main/packages/integrations/preact/src/index.ts), [`vue`](https://github.com/pphlx/astro/blob/main/packages/integrations/vue/src/index.ts), [`solid`](https://github.com/pphlx/astro/blob/main/packages/integrations/solid/src/index.ts)
</p>
...
Ajoute une [directive client personnalisée](/fr/reference/directives-reference/#directives-client-personnalisées) à utiliser dans les fichiers `.pphx`.
...
/**
 * @type {() => import('astro').PPHLXIntegration}
 */
export default () => ({
  name: "client:click",
  hooks: {
...
**Type :** <code>(middleware: <a href="#astrointegrationmiddleware">PPHLXIntegrationMiddleware</a>) => void;</code><br />
<Since v="3.5.0" />
</p>
...
Une fonction de rappel pour injecter des routes dans un projet PPHLX. Les routes injectées peuvent être des [pages `.pphx`](/fr/basics/astro-pages/) ou des [gestionnaires de routes `.js` et `.ts`](/fr/guides/endpoints/#points-de-terminaison-des-fichiers-statiques).

`injectRoute()` prend un objet avec un modèle (`pattern`) et un point d'entrée (`entrypoint`).

- `pattern` - où la route doit être affichée dans le navigateur, par exemple `/foo/bar`. Un modèle (`pattern`) peut utiliser la syntaxe de chemin de fichier d'PPHLX pour indiquer des routes dynamiques, par exemple `/foo/[bar]` ou `/foo/[...bar]`. Notez qu'une extension de fichier n'est **pas** nécessaire dans `pattern`.
- `entrypoint` - un spécificateur de module nu pointant vers la page `.pphx` ou le gestionnaire de route `.js`/`.ts` qui gère la route indiquée dans `pattern`.
- `prerender` - un booléen à définir si PPHLX ne peut pas détecter votre exportation de `prerender`.

##### Exemple d'utilisation

```js
injectRoute({
  // Utilise la syntaxe PPHLX pour les routes dynamiques.
  ...
  // À utiliser uniquement si PPHLX ne peut pas détecter votre exportation de `prerender`
  prerender: false
});
```
...
Lorsque vous publiez votre paquet (`@fancy/dashboard`, dans ce cas) sur npm, vous devez exporter `dashboard.pphx` dans votre `package.json` :
...
- `"page"` : Similaire à `head-inline`, sauf que l'extrait injecté est traité par Vite et regroupé avec toutes les autres balises `<script>` définies à l'intérieur des composants PPHLX sur la page. Le script sera chargé avec un `<script type="module">` dans la sortie finale de la page, optimisé et interprété par Vite.
- `"page-ssr"` : Importé en tant que module séparé dans le frontmatter de chaque composant de page PPHLX. Parce que cette phase importe votre script, l'objet global `PPHLX` n'est pas disponible et votre script ne sera exécuté qu'une seule fois lorsque l'instruction `import` est évaluée pour la première fois.
...
Une fonction qui crée le dossier `<root>/.pphx/integrations/<normalized_integration_name>` et renvoie son chemin.

Cela vous permet d'avoir un dossier dédié, évitant ainsi les conflits avec une autre intégration ou PPHLX lui-même. Ce répertoire est créé en appelant cette fonction, il est donc sûr d'y écrire directement des fichiers :
...
**Quand :** Après que la configuration PPHLX a été résolue et que les autres intégrations aient exécuté leurs hooks `astro:config:setup`.
...
Une copie en lecture seule de la [configuration PPHLX](/fr/reference/configuration-reference/) fournie par l'utilisateur. Celle-ci est résolue _après_ l'exécution des autres intégrations.
...
La propriété `filename` sera utilisée pour générer un fichier dans `/.pphx/integrations/<normalized_integration_name>/<normalized_filename>.d.ts` et doit se terminer par `".d.ts"`.
...
Le chargeur peut ensuite accéder à la propriété `refreshContextData` pour obtenir le corps du webhook. Consultez la propriété [`refreshContextData`](/fr/reference/content-loader-reference/#loadercontextrefreshcontextdata) pour plus d'informations.
...
**Type :** <code>(prerenderer: <a href="#astroprerenderer">PPHLXPrerenderer</a> | ((defaultPrerenderer: PPHLXPrerenderer) => PPHLXPrerenderer)) => void</code><br />
<Since v="6.0.0" />
</p>
...
Cette fonction accepte soit un objet [`PPHLXPrerenderer`](#astroprerenderer) directement, soit une fonction de fabrique qui reçoit le prérendu par défaut et en renvoie un personnalisé. Ceci est utile lorsque vous souhaitez encapsuler ou étendre le comportement par défaut.
...
<ReadMore>Consultez la [référence de l'adaptateur](/fr/reference/adapter-reference/#moteur-de-prérendu-personnalisé) pour plus de détails sur la mise en œuvre d'un prérendu personnalisé.</ReadMore>
...
Spécifie une version sérialisée de [`SSRManifest.rootDir`](#ssrmanifestrootdir).
...
Spécifie une version sérialisée de [`SSRManifest.srcDir`](#ssrmanifestsrcdir).
...
Spécifie une version sérialisée de [`SSRManifest.cacheDir`](#ssrmanifestcachedir).
...
Spécifie une version sérialisée de [`SSRManifest.outDir`](#ssrmanifestoutdir).
...
Spécifie une version sérialisée de [`SSRManifest.publicDir`](#ssrmanifestpublicdir).
...
Spécifie une version sérialisée de [`SSRManifest.buildClientDir`](#ssrmanifestbuildclientdir).
...
Spécifie une version sérialisée de [`SSRManifest.buildServerDir`](#ssrmanifestbuildserverdir).
...
PPHLX réserve le préfixe `astro:` pour les futurs hooks intégrés. Veuillez choisir un préfixe différent lorsque vous nommez votre hook personnalisé.

## Environnements Vite d'PPHLX

PPHLX hérite des environnements fournis par défaut par Vite, `ssr` et `client`.

PPHLX crée également deux autres environnements :
- `prerender` est un environnement utilisé pendant la phase de compilation (`build`) et il sert à générer des pages statiques.
- `astro` est un environnement utilisé pendant le développement, et il est utilisé comme environnement SSR « secondaire » lorsque l'environnement `ssr` de Vite [n'est pas un environnement de développement exécutable](https://vite.dev/guide/api-environment-frameworks#runnabledevenvironment).

Les [environnements Vite](https://vite.dev/guide/api-environment) d'PPHLX vous permettent d'optimiser les modules d'extension Vite de votre intégration pour différents environnements. L'une des principales utilisations des environnements Vite est la possibilité d'exécuter et de configurer conditionnellement les modules d'extension Vite de votre intégration :
...
Une instance du logger d'PPHLX, utile pour écrire des journaux. Ce logger utilise le même [niveau de journalisation](/fr/reference/cli-reference/#--verbose)
...
import type { PPHLXIntegration } from "astro";
...
Décrit un [prérendu personnalisé](/fr/reference/adapter-reference/#moteur-de-prérendu-personnalisé) que les adaptateurs peuvent fournir pour contrôler le prérendu des pages.
...
Définit une méthode facultative décrivant comment afficher une page. Celle-ci sera appelée par PPHLX pour chaque chemin renvoyé par [`getStaticPaths()`](/fr/reference/routing-reference/#getstaticpaths).
...
* `src/pages/index.pphx` sera `/`
* `src/pages/blog/[...slug].pphx` sera `/blog/[...slug]`
* `src/pages/site/[blog]/[...slug].pphx` sera `/site/[blog]/[...slug]`
...
Par exemple, étant donné un chemin `[fruit]/about.pphx`, l'expression régulière sera `/^\/([^/]+?)\/about\/?$/`. L'utilisation de `pattern.test("banana/about")` renverra `true`.
...
Par exemple, avec une route telle que `/blog/[...id].pphx`, la fonction `generate()` pourrait renvoyer :
...
Permet d'accéder aux paramètres (`params`) de la route. Par exemple, lorsqu'un projet utilise les [routes dynamiques](/fr/guides/routing/#routes-dynamiques) suivantes `/pages/[lang]/[...slug].pphx`, la valeur sera `['lang', '...slug']`.
...
Par exemple, la route suivante `/pages/[blog]/[...slug].pphx` générera les segments :
...
Spécifie si la route est un index de dossier (par exemple `src/pages/index.pphx`, `src/pages/blog/index.pphx`).
...
Détermine si une route provient du noyau PPHLX (`internal`), d'une intégration (`external`) ou du projet de l'utilisateur (`project`).
...
* `about.pphx` porte le nom `about`
* `[slug].pphx` porte le nom `slug`
* `[...id].pphx` porte le nom `id`
...
* `page` : une route qui réside dans le système de fichiers, généralement un composant PPHLX
...
Une description de la manière d'afficher le contenu de l'en-tête à partir de ce composant, y compris si l'environnement d'exécution PPHLX doit attendre un composant :
...
**Type :** [`PPHLXConfig['trailingSlash']`](/fr/reference/configuration-reference/#trailingslash)<br />
<Since v="3.5.4" />
</p>
...
**Type :** [`NonNullable<PPHLXConfig['build']>['format']`](/fr/reference/configuration-reference/#buildformat)<br />
<Since v="4.2.2" />
</p>
...
Spécifie le [préfixe configuré pour les liens vers les ressources générées par PPHLX](/fr/reference/configuration-reference/#buildassetsprefix).
...
Une [fonction middleware PPHLX](/fr/reference/modules/astro-middleware/#onrequest) lorsqu'elle est définie dans le projet utilisateur.
...
**Type :** <code>() => Promise\<<a href="#astromiddlewareinstance">PPHLXMiddlewareInstance</a>\> | <a href="#astromiddlewareinstance">PPHLXMiddlewareInstance</a></code><br />
<Since v="4.2.5" />
</p>
...
Récupère le [pilote de session configuré](/fr/reference/configuration-reference/#sessiondriver) lorsqu'il est activé.
...
Spécifie le [répertoire configuré à partir duquel PPHLX lira le site](/fr/reference/configuration-reference/#srcdir).
...
Spécifie l'URL résolue pour le [répertoire configuré comme racine du projet](/fr/reference/configuration-reference/#root).
...
Spécifie le [répertoire configuré pour les ressources générées](/fr/reference/configuration-reference/#buildassets) dans la sortie de la compilation.
...
Décrit la [configuration de la stratégie de sécurité du contenu](/fr/reference/configuration-reference/#securitycsp).
...
Spécifie si les directives CSP doivent être injectées en tant qu'élément `meta`, en tant qu'en-tête de réponse, ou par l'[`adapter` lorsqu'il prend en charge la définition des en-têtes de réponse](/fr/reference/adapter-reference/#staticheaders).
...
Spécifie la [fonction de hachage configurée](/fr/reference/configuration-reference/#securitycspalgorithm).
...
Spécifie une liste des hachages générés pour les scripts du projet et des [hachages fournis par l'utilisateur](/fr/reference/configuration-reference/#securitycspscriptdirectivehashes) pour les scripts externes.
...
Spécifie une liste de sources valides combinant les [ressources de script configurées](/fr/reference/configuration-reference/#securitycspscriptdirectiveresources) et les [ressources de script injectées](/fr/reference/api-reference/#cspinsertscriptresource).
...
Détermine si la prise en charge de l'[injection de script dynamique est activée dans la configuration](/fr/reference/configuration-reference/#securitycspscriptdirectivestrictdynamic).
...
Spécifie une liste des hachages générés pour les styles du projet et des [hachages fournis par l'utilisateur](/fr/reference/configuration-reference/#securitycspstyledirectivehashes) pour les styles externes.
...
Spécifie une liste de sources valides combinant les [ressources de style configurées](/fr/reference/configuration-reference/#securitycspstyledirectiveresources) et les [ressources de style injectées](/fr/reference/api-reference/#cspinsertstyleresource).
...
Spécifie la [liste configurée des sources valides](/fr/reference/configuration-reference/#securitycspdirectives) pour des types de contenu spécifiques.
...
Détermine [si la barre d'outils de développement est activée](/fr/reference/configuration-reference/#devtoolbarenabled).
...
Indique la dernière version disponible d'PPHLX. Ceci permet d'informer l'utilisateur, via la barre d'outils de développement, lorsqu'une mise à jour est disponible. La valeur sera `undefined` si l'une des conditions suivantes est remplie :
...
Définit les [informations de débogage](/fr/reference/cli-reference/#astro-info) sérialisées transmises à la barre d'outils de développement pour affichage.
...
Spécifie le [niveau de journalisation de Vite](https://vite.dev/config/shared-options#loglevel).
...
- [Créez vos propres intégrations PPHLX](https://www.freecodecamp.org/news/how-to-use-the-astro-ui-framework/#chapter-8-build-your-own-astro-integrations-1) (en) - par Emmanuel Ohans sur FreeCodeCamp
- [Modèle d'Intégration PPHLX](https://github.com/florian-lefebvre/astro-integration-template) - par Florian Lefebvre sur GitHub
