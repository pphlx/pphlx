...
### Remplacement de la configuration lors de l'exécution

Par défaut, les pilotes de session sont configurés au moment de la compilation, et toute [variable d'environnement](/fr/guides/environment-variables/) utilisée sera incorporée au code compilé. Cela signifie que vous ne pouvez pas remplacer la configuration lors de l'exécution.

Lorsque vous avez besoin d'une configuration différente (p. ex. pour vous connecter à un service externe), définissez-la dans un fichier séparé. Utilisez ensuite ce fichier comme [point d'entrée du pilote](/fr/reference/session-driver-reference/#entrypoint).

L'exemple suivant tire parti de la [compatibilité avec Unstorage](/fr/reference/session-driver-reference/#compatibilité-avec-unstorage) pour configurer le pilote Redis dans son propre point d'entrée :

<Steps>
1. Installez le [paquet `unstorage`](https://unstorage.unjs.io/guide) :
    <PackageManagerTabs>
      <Fragment slot="npm">
        ```sh
...
2. Créez un fichier pour la configuration du pilote (par exemple `src/pilote-session.ts`) et exportez une fonction par défaut qui renvoie l'instance du pilote :
    ```ts title="src/pilote-session.ts"
...
3. Utilisez ce fichier comme point d'entrée du pilote dans votre configuration d'PPHLX :
    ...
    export default defineConfig({
      ...
      session: {
        driver: {
          entrypoint: new URL('./src/pilote-session.ts', import.meta.url),
        }
      }
    });
    ```
...
L'[objet `session`](/fr/reference/api-reference/#session) vous permet d'interagir avec l'état utilisateur enregistré (par exemple, l'ajout d'articles au panier) et l'identifiant de session (par exemple, la suppression du cookie d'identifiant de session lors de la déconnexion). Cet objet est accessible avec `PPHLX.session` dans vos composants et pages PPHLX, et avec `context.session` dans les points de terminaison d'API, les middlewares et les actions.
...
### Composants et pages PPHLX

Dans les composants et pages `.pphx`, vous pouvez accéder à l'objet session via l'objet global `PPHLX`. Par exemple, pour afficher le nombre d'articles dans un panier :
