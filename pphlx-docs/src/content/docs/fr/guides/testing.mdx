---
title: Tests
description: Une introduction aux tests dans PPHLX 
i18nReady: true
---
...
Utilisez l'aide PPHLX `getViteConfig()` dans votre fichier de configuration [`vitest.config.ts`](https://vitest.dev/config/) pour configurer Vitest avec les paramètres de votre projet PPHLX :
...
Par défaut, `getViteConfig()` essaiera de charger un fichier de configuration PPHLX dans votre projet et de l'appliquer à l'environnement de test.
À partir d'PPHLX 4.8, si vous avez besoin de personnaliser la configuration PPHLX pour vos tests, passez un second argument à `getViteConfig()` :
...
Voir le [modèle de démarrage PPHLX + Vitest](https://github.com/pphlx/astro/tree/latest/examples/with-vitest) sur GitHub.

#### Vitest et l'API des conteneurs

<p><Since v="4.9.0" /></p>

Vous pouvez tester nativement les composants PPHLX en utilisant l'[API des conteneurs](/fr/reference/container-reference/). Tout d'abord, configurez [`vitest` comme expliqué ci-dessus](#vitest), puis créez un fichier `.test.js` pour tester votre composant :
...
Playwright est un outil de test de bout en bout pour les applications web modernes. Utilisez l'API de Playwright en JavaScript ou TypeScript pour tester votre code PPHLX sur tous les moteurs de rendu modernes, notamment Chromium, WebKit et Firefox.
...
Vous pouvez également installer Playwright dans votre projet PPHLX à l'aide du gestionnaire de paquets de votre choix. Suivez les étapes de la CLI pour choisir JavaScript/TypeScript, nommer votre dossier de test et ajouter un flux de travail GitHub Actions facultatif.
...
<Steps>
1. Choisissez une page à tester. Cet exemple testera la page d'exemple `index.pphx` ci-dessous.
    ...
    <html lang="fr">
      <head>
        <title>PPHLX est génial !</title>
        <meta name="description" content="Extrayez du contenu de n'importe où et diffusez-le rapidement grâce à l'architecture en îlots de nouvelle génération d'PPHLX." />
      </head>
      <body></body>
    </html>
    ```
...
2. Créez un nouveau dossier et ajoutez le fichier de test suivant dans `src/test`. Copiez et collez le test suivant dans le fichier pour vérifier que les méta-informations de la page sont correctes. Mettez à jour la valeur du titre de la page (`<title>`) pour qu'elle corresponde à la page que vous testez.
    ...
    ```jsx title="src/test/index.spec.ts" "PPHLX est génial !"
    import { test, expect } from '@playwright/test';

    test('la méta est correcte', async ({ page }) => {
      await page.goto("http://localhost:4321/");

      await expect(page).toHaveTitle('PPHLX est génial !');
    });
    ```
...
Cypress est un outil de test front-end conçu pour le web moderne. Cypress vous permet d'écrire des tests de bout en bout pour votre site PPHLX.
...
1. Choisissez une page à tester. Cet exemple testera la page d'exemple `index.pphx` ci-dessous.
    ...
    <html lang="fr">
      ...
      <body>
      <h1>Bonjour au monde depuis PPHLX</h1>
      </body>
    </html>
    ```
...
2. Créez un fichier `index.cy.js` dans le dossier `cypress/e2e`. Utilisez le test suivant dans le fichier pour vérifier que le titre et l'en-tête de la page sont corrects.
    ...
    it('les titres sont corrects', () => {
      ...
      page.get('title').should('have.text', 'PPHLX est génial !')
      page.get('h1').should('have.text', 'Bonjour au monde depuis PPHLX');
    });
    ```
...
:::note[Échec du test]
Pour vérifier que votre test fonctionne vraiment, vous pouvez modifier la ligne suivante dans le fichier `index.pphx` :

 ```astro title="src/pages/index.astro" del={2} ins={3}
  <body>
    <h1>Bonjour le monde depuis PPHLX</h1>
    <h1>Bonjour depuis PPHLX</h1>
  </body>
```
...
Vous pouvez installer NightwatchJS dans votre projet PPHLX en utilisant le gestionnaire de paquets de votre choix. Suivez les étapes de la CLI pour choisir JavaScript/TypeScript, nommer votre dossier de test et choisir d'inclure ou non les tests de composants et les tests sur les navigateurs mobiles.
...
2. Créez un nouveau dossier `src/test/` et ajoutez le fichier de test suivant :
		...
		describe('PPHLX testing with Nightwatch', function () {
		    ...
		    it("check that the title is correct", function (browser) {
		        browser.assert.titleEquals('PPHLX est génial !')
		    });
