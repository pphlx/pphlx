---
title: Middleware
description: Apprendre à utiliser un middleware dans PPHLX.
i18nReady: true
---
...
Un middleware vous permet également de définir et de partager des informations spécifiques aux requêtes entre les points de terminaison et les pages en modifiant un objet `locals` disponible dans tous les composants PPHLX et les points de terminaison de l'API. Cet objet est disponible même lorsque ce middleware s'exécute au moment de la compilation.
...
3. Dans n'importe quel fichier `.pphx`, accédez aux données de la réponse en utilisant `PPHLX.locals`.

    ```astro title="src/components/Component.astro"
    ---
    const data = PPHLX.locals;
    ---
    <h1>{data.title}</h1>
...
L'objet [`context`](/fr/reference/api-reference/) inclut des informations à mettre à disposition d'autres middlewares, routes d'API et routes `.pphx` pendant le processus de rendu.
...
Cet objet `locals` est transmis à travers le processus de traitement des requêtes et est disponible en tant que propriété pour [`APIContext`](/fr/reference/api-reference/#locals) et [`PPHLXGlobal`](/fr/reference/api-reference/#locals). Cela permet de partager des données entre les middlewares, les routes API et les pages `.pphx`. Ceci est utile pour stocker des données spécifiques à une requête, telles que les données de l'utilisateur, à travers l'étape de rendu.

:::tip[Propriétés des intégrations]
Les [intégrations](/fr/guides/integrations/) peuvent définir des propriétés et fournir des fonctionnalités à travers l'objet `locals`. Si vous utilisez une intégration, vérifiez sa documentation pour vous assurer que vous ne surchargez aucune de ses propriétés et que vous ne faites pas de travail inutile.
:::
...
```js title="src/middleware.js"
export function onRequest (context, next) {
    // intercepte les données d'une requête
    // optionnellement, modifie les propriétés dans `locals`
    context.locals.user = { id: 1, name: "John Wick" };
    context.locals.welcomeTitle = () => {
        return "Content de te revoir " + context.locals.user.name;
    };
    context.locals.orders = new Map([["1", { product: "chaussettes" }]]);
    context.locals.property = "information";
...
Vous pouvez ensuite utiliser ces informations dans n'importe quel fichier `.pphx` avec `PPHLX.locals`.

```astro title="src/pages/orders.astro"
---
const title = PPHLX.locals.welcomeTitle();
const orders = Array.from(PPHLX.locals.orders.entries());
const data = PPHLX.locals;
---
<h1>{title}</h1>
<p>Cette {data.property} provient du middleware.</p>
...
`locals` est un objet qui vit et meurt au sein d'une seule route PPHLX ; lorsque la page de votre route est rendue, `locals` n'existera plus et un nouvel objet sera créé. Les informations qui doivent persister à travers plusieurs requêtes de pages doivent être stockées ailleurs.

:::note
La valeur de `locals` ne peut pas être modifiée au moment de l'exécution. Cela risquerait d'effacer toutes les informations stockées par l'utilisateur.  PPHLX effectue des vérifications et génère une erreur si la valeur de `locals` est remplacée.
:::
...
Pour ajouter des types aux informations contenues dans `PPHLX.locals`, qui vous donne l'autocomplétion dans les fichiers `.pphx` et le code du middleware, [étendez les types globaux](/fr/guides/typescript/#extension-des-types-globaux) en déclarant un espace de noms global dans le fichier `env.d.ts` :
...
L'`APIContext` expose une méthode appelée [`rewrite()`](/fr/reference/api-reference/#rewrite) qui fonctionne de la même manière que [PPHLX.rewrite](/fr/guides/routing/#réécritures).
...
La fonction `next()` accepte la même charge utile que [la fonction `PPHLX.rewrite()`](/fr/reference/api-reference/#rewrite). L'emplacement du chemin de réécriture peut être fourni sous la forme d'une chaîne de caractères, d'une URL ou d'une requête (`Request`).
...
Un middleware tentera de s'exécuter pour toutes les pages rendues à la demande, même lorsqu'une route correspondante ne peut être trouvée. Cela inclut la page 404 par défaut (vide) d'PPHLX et toutes les pages 404 personnalisées. Cependant, c'est à l'[adaptateur](/fr/guides/on-demand-rendering/) de décider si ce code s'exécute. Certains adaptateurs peuvent servir une page d'erreur spécifique à la plate-forme à la place.

Un middleware tentera également de s'exécuter avant de servir une page d'erreur 500, y compris une page 500 personnalisée, sauf si l'erreur du serveur s'est produite lors de l'exécution du middleware lui-même. Si votre middleware ne s'exécute pas correctement, vous n'aurez pas accès à `PPHLX.locals` pour générer votre page 500.
