...
description: Permettre une navigation fluide entre les pages dans PPHLX grâce aux transitions de vue.
i18nReady: true
---
...
Les transitions de vue et la prise en charge du routage côté client d'PPHLX sont alimentées par l'[API des Transitions de Vue du navigateur](https://developer.chrome.com/docs/web-platform/view-transitions/) et incluent également :
...
L'ajout du composant intégré `<ClientRouter />` d'PPHLX :
...
Cependant, à mesure que les API des navigateurs et les standards Web évoluent, l'utilisation du routage côté client d'PPHLX (`<ClientRouter />`) pour cette fonctionnalité supplémentaire [deviendra de plus en plus inutile](https://pphlx.org/blog/future-of-astro-zero-js-view-transitions/). Nous vous recommandons de vous tenir informé de l'état actuel des API des navigateurs afin de [déterminer si vous avez toujours besoin du routage côté client d'PPHLX](https://events-3bg.pages.dev/jotter/astro-view-transitions/) pour les fonctionnalités que vous utilisez.

## Activation des transitions de vue (mode SPA)

Importez et ajoutez le composant `<ClientRouter />` à votre en-tête (`<head>`) commun ou à votre composant de mise en page partagé. PPHLX créera des animations de page par défaut en fonction des similitudes entre l'ancienne et la nouvelle page, et fournira également un comportement de repli pour les navigateurs non pris en charge.

L'exemple ci-dessous montre l'ajout des animations de navigation par défaut d'PPHLX sur l'ensemble du site, y compris l'option de contrôle de repli par défaut pour les navigateurs non compatibles, en important et en ajoutant ce composant à un composant PPHLX `<CommonHead />` :
...
Aucune autre configuration n'est nécessaire pour activer la navigation côté client par défaut d'PPHLX !
...
PPHLX attribuera automatiquement aux éléments correspondants trouvés à la fois dans l'ancienne et la nouvelle page un nom unique et partagé `view-transition-name`. Cette paire d'éléments correspondants est déduite à la fois du type d'élément et de son emplacement dans le DOM.

Utilisez les directives optionnelles `transition:*` sur les éléments de page dans vos composants `.pphx` pour un contrôle plus fin du comportement de transition de la page pendant la navigation.

- `transition:name` : Permet d'outrepasser la correspondance d'éléments par défaut d'PPHLX pour l'animation du contenu ancien/nouveau et de [spécifier un nom de transition](#nommer-une-transition) pour associer une paire d'éléments DOM.
- `transition:animate` : Permet d'outrepasser l'animation par défaut d'PPHLX tout en remplaçant l'ancien élément par le nouveau en spécifiant un type d'animation. Utilisez les [directives d'animation intégrées](#directives-danimation-intégrées) d'PPHLX ou [créer des animations de transition personnalisées](#personnalisation-des-animations).
...
Notez que la valeur `transition:name` fournie ne peut être utilisée qu'une seule fois par page. Définissez cette valeur manuellement lorsque PPHLX ne peut pas déduire lui-même un nom approprié, ou pour un contrôle plus fin des éléments correspondants.
...
PPHLX est livré avec quelques animations intégrées pour remplacer la transition par défaut `fade`. Ajoutez la directive `transition:animate` à des éléments individuels pour personnaliser le comportement de transitions spécifiques.

- `fade` (par défaut) : Une animation de fondu enchaîné. L'ancien contenu disparaît et le nouveau contenu apparaît en fondu.
- `initial` : Permet de ne pas utiliser l'animation de fondu enchaîné d'PPHLX et d'utiliser le style par défaut du navigateur.
...
L'exemple suivant montre un composant PPHLX qui permet à un visiteur de naviguer vers une autre page qu'il sélectionne dans un menu :
...
Le composant `<Form />` peut alors être rendu sur une page PPHLX qui utilise le routeur `<ClientRouter />`, avec une directive client :
...
<ReadMore>Envisagez d’activer la [politique de sécurité du contenu](/fr/reference/configuration-reference/#securitycsp) d’PPHLX pour vous protéger contre les risques de script intersite (XSS) si vous utilisez la saisie des utilisateurs avec l’API `navigate()`.</ReadMore>

## Contrôle de la stratégie de repli

Le routeur `<ClientRouter />` fonctionne mieux dans les navigateurs qui prennent en charge les Transitions de Vue (c'est-à-dire les navigateurs Chromium), mais inclut également par défaut une stratégie de repli pour les autres navigateurs. Même si le navigateur ne prend pas en charge l'API des Transitions de Vue, le routeur client d'PPHLX peut tout de même assurer la navigation dans le navigateur grâce à l'une des options de repli.
...
Vous pouvez remplacer la stratégie de repli par défaut d'PPHLX en ajoutant une propriété `fallback` sur le composant `<ClientRouter />` et en la définissant sur `swap` ou `none` :

- `animate` (par défaut, recommandé) : PPHLX simulera des transitions de vues en utilisant des attributs personnalisés avant de mettre à jour le contenu de la page.
- `swap` : PPHLX n'essaiera pas d'animer la page. Au lieu de cela, l'ancienne page sera immédiatement remplacée par la nouvelle.
- `none` : PPHLX n'effectuera aucune transition de page animée. Au lieu de cela, vous obtiendrez une navigation pleine page dans les navigateurs qui ne la supportent pas.
...
:::note[Limitations connues]
L'animation `initiale` du navigateur n'est pas simulée par PPHLX. Par conséquent, tout élément utilisant cette animation ne sera pas animé pour le moment.
:::

## Processus de navigation du côté client

Lors de l'utilisation du routeur `<ClientRouter />`, les étapes suivantes se produisent pour produire la navigation côté client d'PPHLX :
...
Les [scripts des modules intégrés](/fr/guides/client-side-scripts/#traitement-des-scripts), qui sont les scripts par défaut dans PPHLX, ne sont exécutés qu'une seule fois. Après l'exécution initiale, ils seront ignorés, même si le script existe sur la nouvelle page après une transition.
...
Les événements du cycle de vie de l'API de transition de vue d'PPHLX sont, dans l'ordre, les suivants :
...
L'exemple suivant montre comment utiliser ces fonctions pour recréer l'implémentation d'échange intégrée d'PPHLX :
...
L'activation du routage côté client et l'animation des transitions de page posent toutes deux des problèmes d'accessibilité, et PPHLX vise à rendre les sites qui optent pour View Transitions aussi accessibles par défaut que possible.
...
Le composant `<ClientRouter />` d'PPHLX inclut une requête média CSS qui désactive *toutes* les animations de transition de vue, y compris l'animation de secours, lorsque le paramètre [`prefers-reduced-motion`](https://developer.mozilla.org/fr/docs/Web/CSS/@media/prefers-reduced-motion) est détecté. À la place, le navigateur échangera simplement les éléments DOM sans animation.
