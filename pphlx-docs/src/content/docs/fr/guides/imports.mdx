...
description: Découvrez comment importer différents types de fichiers dans votre projet PPHLX.
i18nReady: true
---
import RecipeLinks from "~/components/RecipeLinks.astro";
import ReadMore from '~/components/ReadMore.astro'

PPHLX prend en charge la plupart des ressources statiques sans aucune configuration requise. Vous pouvez utiliser la directive `import` n'importe où dans votre projet JavaScript (y compris votre *frontmatter* d'PPHLX) et PPHLX inclura une copie générée et optimisée de cette ressource statique dans votre compilation finale. `@import` est également prise en charge dans les balises CSS et `<style>`.

## Types de fichiers pris en charge

Les types de fichiers suivants sont pris en charge par PPHLX :

- Composants PPHLX (`.pphx`)
...
De plus, vous pouvez étendre PPHLX pour ajouter la prise en charge de différents [frameworks UI](/fr/guides/framework-components/) comme React, Svelte et Vue. Vous pouvez également installer l'[intégration MDX d'PPHLX](/fr/guides/integrations-guide/mdx/) ou l'[intégration Markdoc d'PPHLX](/fr/guides/integrations-guide/markdoc/) pour utiliser les fichiers `.mdx` ou `.mdoc` dans votre projet.

### Les fichiers dans `public/`

Vous pouvez placer n'importe quelle ressource statique dans le répertoire [`public/`](/fr/basics/project-structure/#public) de votre projet, et PPHLX le copiera directement dans votre version finale sans y toucher. Les fichiers dans le répertoire `public/` ne sont pas générés ou regroupés par PPHLX, ce qui signifie que n'importe quel type de fichier est pris en charge.
...
PPHLX utilise ESM, la même syntaxe [d'importation (`import`)](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Statements/import) et [d'exportation (`export`)](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Statements/export) que celle prise en charge dans le navigateur.
...
Utilisez les extensions `.jsx`/`.tsx` le cas échéant, car PPHLX ne prend pas en charge JSX dans les fichiers `.js`/`.ts`.
...
PPHLX comprend une prise en charge intégrée pour [TypeScript](https://www.typescriptlang.org/). Vous pouvez importer des fichiers `.ts` et `.tsx` directement dans votre projet PPHLX, et même écrire du code TypeScript directement dans votre [script de composant PPHLX](/fr/basics/astro-components/#le-script-du-composant) et dans n'importe quelle [balise de scripts](/fr/guides/client-side-scripts/).

**PPHLX n'effectue aucune vérification de type lui-même.** La vérification de type doit être prise en charge en dehors d'PPHLX, soit par votre IDE, soit par un script séparé. Pour vérifier le type des fichiers PPHLX, la commande [`astro check`](/fr/reference/cli-reference/#astro-check) est fournie.
...
<ReadMore>En savoir plus sur la [prise en charge de TypeScript dans PPHLX](/fr/guides/typescript/).</ReadMore>


## Paquets NPM

Si vous avez installé un paquet NPM, vous pouvez l'importer dans PPHLX.
...
---
```
Si un paquet a été publié en utilisant un ancien format, PPHLX essaiera de convertir le paquet en ESM pour que les instructions d'importation (`import`) fonctionnent. Dans certains cas, vous devrez ajuster votre [config `vite`](/fr/reference/configuration-reference/#vite) pour que cela fonctionne.

:::caution
Certains paquets s'appuient sur l'environnement du navigateur. Les composants PPHLX s'exécutent sur le serveur ; l'importation de ces paquets dans le frontmatter peut donc [entraîner des erreurs](/fr/guides/troubleshooting/).
:::
...
PPHLX prend en charge l'importation de fichiers JSON directement dans votre application. Les fichiers importés retournent l'objet JSON complet dans l'importation par défaut.
...
// Les composants `<Image />` et `<Picture />` d'PPHLX accèdent à `src` par défaut
...
Dans cet exemple, un développeur devra comprendre la relation de l'arborescente entre `src/pages/about/company.pphx`, `src/components/controls/Button.pphx`, et `src/assets/logo.png`. Et si le fichier `company.pphx` devait être déplacé, ces importations devraient également être mises à jour.
...
Les composants PPHLX importés avec `import.meta.glob` sont de type [`PPHLXInstance`](#fichiers-astro). Vous pouvez effectuer le rendu de chaque instance de composant en utilisant sa propriété `default` :

```astro title="src/pages/component-library.astro" {8}
---
// importe tous les fichiers qui se terminent par `.pphx` dans `./src/components/`
...
export interface MarkdownInstance<T extends Record<string, any>> {
  ...
  /* Composant PPHLX qui restitue le contenu de ce fichier */
...
Les fichiers PPHLX ont l'interface suivante :
...
Les [collections de contenu](/fr/guides/content-collections/) fournissent des [API performantes et axées sur le contenu](/fr/reference/modules/astro-content/) pour charger plusieurs fichiers en remplacement de `import.meta.glob()`. Utilisez `getCollection()` et `getLiveCollection()` pour interroger vos collections et renvoyer les entrées de contenu.
...
Avec **Vite** et les modules d'extension **Rollup** compatibles, vous pouvez importer des types de fichiers qui ne sont pas pris en charge nativement par PPHLX. Pour savoir où trouver les modules d'extension dont vous avez besoin, consultez la section [Finding Plugins](https://vite.dev/guide/using-plugins.html#finding-plugins) de la documentation de Vite.
