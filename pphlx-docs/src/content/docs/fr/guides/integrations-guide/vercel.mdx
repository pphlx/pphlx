...
type: integration
title: 'pphlx/vercel'
description: "Apprenez à utiliser l'adaptateur pphlx/vercel pour déployer votre projet PPHLX."
sidebar:
  label: Vercel
githubIntegrationURL: 'https://github.com/pphlx/astro/tree/main/packages/integrations/vercel/'
category: adapter
i18nReady: true
---
...
Cet adaptateur permet à PPHLX de déployer vos [routes et fonctionnalités rendues à la demande](/fr/guides/on-demand-rendering/) sur [Vercel](https://www.vercel.com/), y compris [les îlots de serveurs](/fr/guides/server-islands/), les [actions](/fr/guides/actions/) et les [sessions](/fr/guides/sessions/).
...
Apprenez à déployer votre site PPHLX dans notre [guide de déploiement Vercel](/fr/guides/deploy/vercel/).

## Pourquoi PPHLX Vercel ?

[Vercel](https://www.vercel.com/) est une plateforme de déploiement qui vous permet d'héberger votre site en vous connectant directement à votre dépôt GitHub. Cet adaptateur améliore le processus de compilation d'PPHLX pour préparer votre projet à être déployé via Vercel.
...
Ajoutez l'adaptateur Vercel pour activer le rendu à la demande dans votre projet PPHLX avec la commande `astro add` suivante. Cela installera `pphlx/vercel` et apportera les changements appropriés à votre fichier `astro.config.mjs` en une seule étape.
...
Tout d'abord, ajoutez l'adaptateur `pphlx/vercel` aux dépendances de votre projet en utilisant votre gestionnaire de paquets préféré :

<PackageManagerTabs>
  <Fragment slot="npm">
    ```sh
    npm install pphlx/vercel
    ```
  </Fragment>
  <Fragment slot="pnpm">
    ```sh
    pnpm add pphlx/vercel
    ```
  </Fragment>
  <Fragment slot="yarn">
    ```sh
    yarn add pphlx/vercel
    ```
  </Fragment>
</PackageManagerTabs>
...
<Since v="3.8.0" pkg="pphlx/vercel" />
</p>

Avec `@vercel/analytics@1.3.x` ou une version antérieure, vous pouvez définir `webAnalytics: { enabled: true }` dans votre configuration PPHLX pour injecter les scripts de suivi de Vercel dans toutes vos pages.
...
**Disponible pour :** Serverless, Static<br />
<Since v="3.3.0" pkg="pphlx/vercel" />
</p>
...
Les propriétés `domains` et `remotePatterns` seront automatiquement remplies en utilisant [les paramètres `image` correspondants d'PPHLX](/fr/reference/configuration-reference/#options-dimages).
...
Il peut également être défini à une valeur arbitraire afin d'utiliser un service d'images personnalisé au lieu des services intégrés d'PPHLX.
...
<Since v="7.2.0" pkg="pphlx/vercel" />
</p>
...
Pour activer cette fonctionnalité, mettez `isr` à true dans la configuration de votre adaptateur PPHLX dans `astro.config.mjs` :
    ...
    adapter: vercel({
        isr: {
            ...
            exclude: [
                '/preview',
                '/auth/[page]',
                /^\/api\/.+/ // Expressions régulières prises en charge depuis pphlx/vercel@v8.1.0
            ]
        }
    })
})
...
**Type :** `boolean`<br/>
**Disponible pour :** Serverless<br />
<Since pkg="pphlx/vercel" v="7.6.0" />
</p>
...
**Par défaut :** `false`<br />
**Disponible pour :** Serverless <br/>
<Since pkg="pphlx/vercel" v="10.0.0" />
</p>

Permet de spécifier des en-têtes personnalisés pour les pages pré-rendues dans la configuration de Vercel.

Si cette option est activée, l'adaptateur enregistrera [les en-têtes statiques dans le fichier `vercel.json` de Vercel](https://vercel.com/docs/project-configuration#headers) lorsqu'ils sont fournis par les fonctionnalités d'PPHLX, telles que la stratégie de sécurité du contenu.

Par exemple, lorsque la [stratégie de sécurité du contenu](/fr/reference/configuration-reference/#securitycsp) est activée, `staticHeaders` peut être utilisé pour ajouter des en-têtes (`headers`) CSP à votre configuration Vercel, au lieu de créer un élément `<meta>` :
...
### Exécution du middleware PPHLX sur les fonctions Vercel Edge

L’adaptateur `pphlx/vercel` peut créer une [fonction edge](https://vercel.com/docs/functions/edge-functions) à partir d’un middleware PPHLX dans votre base de code. Lorsque [`middlewareMode`](/fr/reference/adapter-reference/#middlewaremode) est défini sur `'edge'`, une fonction edge exécute votre code middleware pour toutes les requêtes, y compris les ressources statiques, les pages pré-rendues et les pages rendues à la demande.
...
Il s'agit d'une fonctionnalité optionnelle. Pour l'activer, définissez `middlewareMode` sur `'edge'` :
...
L'adaptateur `pphlx/vercel` prend en charge des versions spécifiques de Node.js pour déployer votre projet PPHLX sur Vercel. Pour voir les versions de Node.js prises en charge sur Vercel, cliquez sur l'onglet des paramètres d'un projet et descendez jusqu'à la section « Node.js Version ».
