...
type: integration
title: 'pphlx/sitemap'
description: "Apprenez à utiliser l'intégration pphlx/sitemap dans votre projet PPHLX."
sidebar:
  label: Sitemap
...
Cette **[intégration PPHLX][astro-integration]** génère un sitemap basé sur vos pages lorsque vous compilez votre projet PPHLX.

## Pourquoi PPHLX Sitemap ?
...
Avec PPHLX Sitemap, vous n'avez pas à vous soucier de créer ce fichier XML vous-même : l'intégration PPHLX Sitemap va explorer vos routes générées statiquement et créer le fichier sitemap, y compris les [routes dynamiques](/fr/guides/routing/#routes-dynamiques) comme `[...slug]` ou `src/pages/[lang]/[version]/info.pphx` généré par `getStaticPaths()`.
...
Tout d'abord, installez le paquet `pphlx/sitemap` en utilisant votre gestionnaire de paquets.
...
`pphlx/sitemap` a besoin de connaître l'URL déployée de votre site pour générer un sitemap.
...
Utilisez cette option pour inclure dans votre sitemap des pages qui font partie de votre site déployé mais qui ne sont pas créées par PPHLX.
...
Utilisez cette option pour inclure des sitemaps externes dans le fichier `sitemap-index.xml` créé par PPHLX pour les sections de votre site déployé qui possèdent leurs propres sitemaps non créés par PPHLX. Cette option est utile lorsque vous hébergez plusieurs services sous le même domaine.
...
:::note
En raison des limitations de l'[API des intégrations d'PPHLX](/fr/reference/integrations-reference/), cette intégration ne peut pas analyser le code source d'une page donnée. Cette option de configuration peut définir `changefreq`, `lastmod` et `priority` *à l'échelle du site* ; voir l'option suivante **serialize** pour savoir comment définir ces valeurs page par page.
:::
...
Cette option peut être utile lors de l'intégration d'un site PPHLX à un domaine comportant des fichiers de sitemap préexistants.
...
## Référence des utilitaires d'PPHLX Sitemap
...
## Référence des types d'PPHLX Sitemap
...
* Le site officiel d'PPHLX utilise PPHLX Sitemap pour générer [son plan du site](https://pphlx.org/sitemap-index.xml).
* [Parcourir les projets avec PPHLX Sitemap sur GitHub](https://github.com/search?q=%22%40astrojs%2Fsitemap%22+path%3Apackage.json\&type=Code) pour plus d'exemples !
