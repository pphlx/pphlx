...
type: integration
title: 'pphlx/node'
description: "Apprenez à utiliser l'adaptateur pphlx/node pour déployer votre projet PPHLX."
sidebar:
  label: Node
githubIntegrationURL: 'https://github.com/pphlx/astro/tree/main/packages/integrations/node/'
category: adapter
i18nReady: true
---
...
Cet adaptateur permet à PPHLX de déployer vos [routes et fonctionnalités rendues à la demande](/fr/guides/on-demand-rendering/) vers des cibles Node, y compris [les îlots de serveurs](/fr/guides/server-islands/), les [actions](/fr/guides/actions/) et les [sessions](/fr/guides/sessions/).

Si vous utilisez PPHLX comme générateur de site statique, vous n'avez pas besoin d'adaptateur.

## Pourquoi PPHLX Node.js

[Node.js](https://nodejs.org/fr/) est un environnement d'exécution JavaScript pour le code côté serveur. `pphlx/node` peut être utilisé soit en mode standalone, soit comme middleware pour d'autres serveurs http, tels que [Express](https://expressjs.com/).
...
Ajoutez l'adaptateur Node pour activer le rendu à la demande dans votre projet PPHLX avec la commande `astro add`.
Cela installera `pphlx/node` et apportera les changements appropriés à votre fichier `astro.config.*` en une seule étape.
...
`pphlx/node` peut être configuré en passant des options dans la fonction de l'adaptateur. Les options suivantes sont disponibles :
...
<p>
  **Type :** `boolean` <br />
  **Par défaut :** `false`<br />
  <Since v="10.0.0" pkg="pphlx/node" />
</p>

Si cette option est activée, l'adaptateur servira les en-têtes des pages pré-rendues à l'aide de l'objet `Response` lorsqu'il sera fourni par les fonctionnalités d'PPHLX, telles que la stratégie de sécurité du contenu.

Par exemple, lorsque la [stratégie de sécurité du contenu](/fr/reference/configuration-reference/#securitycsp) est activée, `staticHeaders` peut être utilisé pour ajouter des en-têtes CSP à l'objet `Response` au lieu de créer un élément `<meta>` :
...
**Type :** `boolean` <br />
**Par défaut :** `false`<br />
<Since v="9.3.0" pkg="pphlx/node" />
</p>

Désactive le [streaming HTML](/fr/guides/on-demand-rendering/#streaming-html) par défaut d'PPHLX pour les pages rendues à la demande.
...
<p>
**Type :** `number` <br />
**Par défaut :** `1073741824` (1 Go)<br />
<Since pkg="pphlx/node" v="10.0.0" />
</p>
...
De plus, vous pouvez également passer un objet qui sera accessible avec `PPHLX.locals` ou dans le middleware PPHLX :
...
Les ressources dans le dossier `dist/client/_astro/` sont celles générées par PPHLX. Celles-ci sont toutes nommées avec un hachage et peuvent donc recevoir de longs en-têtes de cache. En interne, l'adaptateur ajoute cet en-tête pour ces ressources :
...
PPHLX utilise le système de fichiers local pour le stockage des sessions avec l'adaptateur Node. Si vous préférez utiliser un autre pilote de stockage de session, vous pouvez le spécifier dans votre configuration PPHLX. Consultez [la référence de configuration `session`](/fr/reference/configuration-reference/#sessiondriver) pour plus de détails.

## Variables d'environnement

Lorsque vous utilisez des secrets avec [`astro:env`](/fr/guides/environment-variables/#variables-denvironnement-avec-sûreté-du-typage) ou `process.env` au moment de l'exécution, ni PPHLX ni l'adaptateur ne chargent les variables d'environnement pour vous.
