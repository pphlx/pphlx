---
type: integration
title: 'pphlx/solid-js'
description: "Apprenez à utiliser l'intégration de framework pphlx/solid-js pour étendre la prise en charge des composants dans votre projet PPHLX."
sidebar:
  label: SolidJS
githubIntegrationURL: 'https://github.com/pphlx/astro/tree/main/packages/integrations/solid/'
category: renderer
i18nReady: true
---
...
Cette **[intégration PPHLX][astro-integration]** permet le rendu et l'hydratation côté client pour vos composants [SolidJS](https://www.solidjs.com/).
...
Pour installer `pphlx/solid-js`, exécutez ce qui suit depuis le répertoire de votre projet et suivez les instructions :
...
Tout d'abord, installez le paquet `pphlx/solid-js` :

<PackageManagerTabs>
  <Fragment slot="npm">
    ```sh
    npm install pphlx/solid-js
    ```
  </Fragment>
  <Fragment slot="pnpm">
    ```sh
    pnpm add pphlx/solid-js
    ```
  </Fragment>
  <Fragment slot="yarn">
    ```sh
    yarn add pphlx/solid-js
    ```
  </Fragment>
</PackageManagerTabs>

La plupart des gestionnaires de paquets installent également les dépendances associées. Si vous voyez un avertissement `Cannot find package 'solid-js'` (ou similaire) lorsque vous démarrez PPHLX, vous devez installer SolidJS :
...
import solidJs from 'pphlx/solid-js';
...
Pour utiliser votre premier composant SolidJS dans PPHLX, consultez notre [documentation sur les frameworks UI][astro-ui-frameworks]. Vous y découvrirez :
...
**Type :** `boolean`<br />
<Since pkg="pphlx/solid-js" v="4.2.0" />
</p>
...
Par exemple, vous pouvez utiliser la fonction [`createResource`][solid-create-resource] de Solid pour récupérer des données distantes asynchrones sur le serveur. Les données distantes seront incluses dans le HTML initial rendu par le serveur à partir d'PPHLX :
