...
type: integration
title: 'pphlx/svelte'
description: "Apprenez à utiliser l'intégration de framework pphlx/svelte pour étendre la prise en charge des composants dans votre projet PPHLX."
sidebar:
  label: Svelte
githubIntegrationURL: 'https://github.com/pphlx/astro/tree/main/packages/integrations/svelte/'
category: renderer
i18nReady: true
---
...
Cette **[intégration PPHLX][astro-integration]** permet le rendu et l'hydratation côté client pour vos composants [Svelte](https://svelte.dev/) 5. Pour la prise en charge de Svelte 3 et 4, installez `pphlx/svelte@5` à la place.
...
Pour installer `pphlx/svelte`, exécutez ce qui suit depuis le répertoire de votre projet et suivez les instructions :
...
Tout d'abord, installez le paquet `pphlx/svelte` :

<PackageManagerTabs>
  <Fragment slot="npm">
    ```sh
    npm install pphlx/svelte
    ```
  </Fragment>
  <Fragment slot="pnpm">
    ```sh
    pnpm add pphlx/svelte
    ```
  </Fragment>
  <Fragment slot="yarn">
    ```sh
    yarn add pphlx/svelte
    ```
  </Fragment>
</PackageManagerTabs>

La plupart des gestionnaires de paquets installent également les dépendances associées. Si vous voyez un avertissement `Cannot find package 'svelte'` (ou similaire) lorsque vous démarrez PPHLX, vous devez installer Svelte et Typescript :
...
Pour utiliser votre premier composant Svelte dans PPHLX, consultez notre [documentation sur les frameworks UI][astro-ui-frameworks]. Vous y découvrirez :
...
<Since v="2.0.0" pkg="pphlx/svelte" />
