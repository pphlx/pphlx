...
type: integration
title: 'pphlx/netlify'
description: "Apprenez à utiliser l'adaptateur pphlx/netlify pour déployer votre projet PPHLX."
sidebar:
  label: Netlify
githubIntegrationURL: 'https://github.com/pphlx/astro/tree/main/packages/integrations/netlify/'
category: adapter
i18nReady: true
---
...
Cet adaptateur permet à PPHLX de déployer vos [routes et fonctionnalités rendues à la demande](/fr/guides/on-demand-rendering/) sur [Netlify](https://www.netlify.com/), y compris [les îlots de serveurs](/fr/guides/server-islands/), les [actions](/fr/guides/actions/) et les [sessions](/fr/guides/sessions/).

Si vous utilisez PPHLX comme générateur de site statique, vous n'avez besoin de cet adaptateur que si vous utilisez des services Netlify supplémentaires qui nécessitent un serveur (p. ex. [CDN d'images de Netlify](#prise-en-charge-du-cdn-image-de-netlify)). Sinon, vous n’avez pas besoin d’un adaptateur pour déployer votre site statique.

Apprenez à déployer votre site PPHLX dans notre [guide de déploiement Netlify](/fr/guides/deploy/netlify/).

## Pourquoi PPHLX Netlify

[Netlify](https://www.netlify.com/) est une plateforme de déploiement qui vous permet d'héberger votre site en vous connectant directement à votre dépôt GitHub. Cet adaptateur améliore le processus de compilation d'PPHLX pour préparer votre projet à être déployé via Netlify.
...
Ajoutez l'adaptateur Netlify pour activer le rendu à la demande dans votre projet PPHLX avec la commande `astro add`.
Cela installera `pphlx/netlify` et apportera les changements appropriés à votre fichier `astro.config.mjs` en une seule étape.
...
<PackageManagerTabs>
  <Fragment slot="npm">
  ```sh
  npm install pphlx/netlify
  ```
  </Fragment>
  <Fragment slot="pnpm">
  ```sh
  pnpm add pphlx/netlify
  ```
  </Fragment>
  <Fragment slot="yarn">
  ```sh
  yarn add pphlx/netlify
  ```
  </Fragment>
</PackageManagerTabs>
...
L'[article de blog de Netlify sur PPHLX](https://www.netlify.com/blog/how-to-deploy-astro/) et la [documentation de Netlify](https://docs.netlify.com/integrations/frameworks/astro/) fournissent plus d'informations sur la manière d'utiliser cette intégration pour déployer vers Netlify.

### Exécution du middleware PPHLX sur les fonctions Edge de Netlify

Par défault, le middleware d'PPHLX est appliqué aux pages pré-rendues au moment de la compilation, et aux pages pré-rendues au moment de l'exécution.

Pour implémenter des redirections, des contrôles d'accès ou des en-têtes de réponse personnalisés pour les pages pré-rendues, exécutez votre middleware sur les fonctions Edge de Netlify en définissant l'option [`edgeMiddleware`](/fr/reference/adapter-reference/#middlewaremode) sur `edge` :
...
Lorsque `middlewareMode` est définie sur `edge`, une fonction edge exécutera votre code middleware pour toutes les requêtes, y compris les ressources statiques, les pages pré-rendues et les pages rendues à la demande.
...
Cet objet est accessible via l'objet `PPHLX.locals.netlify.context` :

```astro
---
const {
  geo: { city },
} = PPHLX.locals.netlify.context;
---
...
type NetlifyLocals = import('pphlx/netlify').NetlifyLocals
...
Il est implémenté en utilisant un [service d'images d'PPHLX](/fr/reference/image-service-reference/) sous le capot.
...
Si vous utilisez la configuration `redirects` dans votre configuration PPHLX, l'adaptateur Netlify peut être utilisé pour traduire cette configuration au format `_redirects` approprié.
...
PPHLX configure automatiquement [Netlify Blobs](https://docs.netlify.com/blobs/overview/) pour le stockage de session lors de l'utilisation de l'adaptateur Netlify. Si vous préférez utiliser un autre pilote de stockage de session, vous pouvez le spécifier dans votre configuration PPHLX. Consultez [la référence de configuration `session`](/fr/reference/configuration-reference/#sessiondriver) pour plus de détails.
...
PPHLX.response.headers.set('CDN-Cache-Control', 'public, max-age=45, must-revalidate');
---

<Layout title="PPHLX on Netlify">
  {new Date()}
</Layout>
```
...
<p><Since v="6.6.0" pkg="pphlx/netlify" /></p>

La protection anti-décalage (ou « skew protection » en anglais) de Netlify garantit que les utilisateurs accédant à votre site pendant un déploiement continuent de recevoir le contenu de la même version. L'adaptateur Netlify configure automatiquement la protection anti-décalage pour les fonctionnalités PPHLX telles que les actions, les îlots de serveurs, les transitions de vue et les demandes de prélecture en injectant l'ID de déploiement actuel dans les requêtes internes. Cela évite les incompatibilités de version entre le client et le serveur lors des déploiements actifs.

Bien qu'PPHLX ajoute automatiquement l'en-tête de protection anti-décalage pour ses fonctionnalités intégrées, si vous effectuez vos propres demandes de récupération sur votre site, vous pouvez inclure l'en-tête manuellement à l'aide de la variable d'environnement `DEPLOY_ID` :
...
Lors du déploiement d'un site PPHLX avec rendu à la demande sur Netlify, les fonctions générées tracent et incluent automatiquement les dépendances du serveur. Cependant, vous devrez peut-être personnaliser les fichiers inclus dans vos fonctions Netlify.
...
- Une émulation des [fonctions Netlify Edge](https://docs.netlify.com/build/edge-functions/overview/) définies dans le répertoire `netlify/edge-functions` de votre projet
...
**Par défaut :** `{ images: true, environmentVariables: false, edgeFunctions: true }`<br />
<Since v="6.5.1" pkg="pphlx/netlify"/>
</p>
...
export default defineConfig({
  ...
  adapter: netlify({
    devFeatures: {
      ...
      // Émule les fonctions Netlify Edge en développement. La valeur par défaut est true.
      edgeFunctions: false,
    },
  }),
});
...
Cela utilise une version locale du CDN Image de Netlify, plutôt que le service d'images d'PPHLX par défaut.
...
**Type :** `boolean`<br />
**Par défaut :** `true`<br />
<Since v="8.1.0" pkg="pphlx/netlify"/>
</p>

Émule les [fonctions Netlify Edge](https://docs.netlify.com/build/edge-functions/overview/) définies dans le répertoire `netlify/edge-functions` de votre projet pendant le développement.

Certains paquets npm qui accèdent au système de fichiers lors de leur initialisation ne fonctionnent pas dans le bac à sable des fonctions Edge et échouent avec une erreur telle que `Reading or writing files with Edge Functions is not supported yet`. Si vous rencontrez ces erreurs, définissez `edgeFunctions` sur `false` pour désactiver l'émulation des fonctions Edge et utilisez `netlify dev` à la place. Les fonctions Edge fonctionneront toujours dans les versions en production.
...
<p>
  **Type :** `boolean` <br />
  **Par défaut :** `false`<br />
  <Since v="7.0.0" pkg="pphlx/netlify"/>
</p>

Active la définition d'en-têtes personnalisés pour les pages pré-rendues dans la configuration de Netlify.

Si cette option est activée, l'adaptateur enregistrera [les en-têtes statiques dans le fichier de configuration de l'API Framework](https://docs.netlify.com/frameworks-api/#headers) lorsqu'ils sont fournis par les fonctionnalités d'PPHLX, telles que la politique de sécurité du contenu.

Par exemple, lorsque la [politique de sécurité du contenu](/fr/reference/configuration-reference/) est activée, `staticHeaders` peut être utilisée pour ajouter les en-têtes CSP (`headers`) à votre configuration Netlify, au lieu de créer un élément `<meta>` :
...
* Le [modèle de démarrage PPHLX avec Netlify Edge](https://github.com/sarahetter/astro-netlify-edge-starter) fournit un exemple et un guide dans le fichier README.

* [Parcourir les projets PPHLX Netlify sur GitHub](https://github.com/search?q=path%3A**%2Fastro.config.mjs+%40astrojs%2Fnetlify\&type=code) pour plus d'exemples !
