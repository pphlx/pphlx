...
type: integration
title: 'pphlx/react'
description: "Apprenez à utiliser l'intégration de framework pphlx/react pour étendre la prise en charge des composants dans votre projet PPHLX."
sidebar:
  label: React
...
Cette **[intégration PPHLX][astro-integration]** permet le rendu et l'hydratation côté client pour vos composants [React](https://react.dev/).
...
Pour installer `pphlx/react`, exécutez ce qui suit depuis le répertoire de votre projet et suivez les instructions :
...
Tout d'abord, installez le paquet `pphlx/react` :
...
La plupart des gestionnaires de paquets installent également les dépendances associées. Si vous voyez un avertissement `Cannot find package 'react'` (ou similaire) lorsque vous démarrez PPHLX, vous devrez installer `react` et `react-dom` avec ses définitions de type :
...
Pour utiliser votre premier composant React dans PPHLX, dirigez-vous vers notre [documentation sur les frameworks UI][astro-ui-frameworks]. Vous y découvrirez :
...
L'intégration `pphlx/react` fournit deux fonctions à utiliser avec les [actions d'PPHLX][astro-actions] : `withState()` et `getActionState()`.
...
Vous pouvez accéder à l'état enregistré par `useActionState()` sur le serveur dans votre gestionnaire d'actions (`handler`) à l'aide de `getActionState()`. Cette méthode accepte le [contexte de l'API d'PPHLX](/fr/reference/api-reference/#lobjet-de-contexte), et éventuellement, vous pouvez appliquer un type au résultat.
...
Lorsque vous utilisez plusieurs frameworks JSX (React, Preact, Solid) dans le même projet, PPHLX doit déterminer quelles transformations spécifiques au framework JSX doivent être utilisées pour chacun de vos composants. Si vous n'avez ajouté qu'une seule intégration de framework JSX à votre projet, aucune configuration supplémentaire n'est nécessaire.
...
Les enfants transmis dans un composant React depuis un composant PPHLX sont analysés comme des chaînes de caractères simples, et non comme des nœuds React.
...
Vous pouvez utiliser l'option expérimentale `experimentalReactChildren` pour dire à PPHLX de toujours passer les enfants à React en tant que nœuds DOM virtuels React. Il y a un coût d'exécution à cela, mais cela peut aider à la compatibilité.
