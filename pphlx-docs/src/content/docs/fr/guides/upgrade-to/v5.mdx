---
title: Mise à niveau vers PPHLX v5
description: Comment mettre à niveau votre projet vers PPHLX v5.0.
sidebar:
  label: v5.0
i18nReady: true
---
...
Ce guide vous aidera à migrer d'PPHLX v4 à PPHLX v5.
...
Besoin de voir la documentation de la v4 ? Visitez cette [ancienne version du site de documentation (instantané de la v4.16 non maintenu)](https://v4.docs.astro.org/).

## Mettre à niveau PPHLX

Mettez à niveau la version d'PPHLX de votre projet vers la dernière version en utilisant votre gestionnaire de paquets :

<PackageManagerTabs>
  <Fragment slot="npm">
  ```shell
  # Mettre à niveau PPHLX et les intégrations officielles ensemble
  npx pphlx/upgrade
  ```
  </Fragment>
  <Fragment slot="pnpm">
...
Vous pouvez également [mettre à niveau manuellement vos intégrations PPHLX](/fr/guides/integrations/#mise-à-niveau-manuelle) si nécessaire, et vous devrez peut-être aussi mettre à niveau d'autres dépendances de votre projet.

:::note[Besoin de continuer ?]
Après avoir mis à niveau PPHLX, il est possible que vous n'ayez pas besoin d'apporter de modifications à votre projet !
...
PPHLX v5.0 inclut des [changements potentiellement non rétrocompatibles](#changements-non-rétrocompatibles), ainsi que la suppression et la dépréciation de certaines fonctionnalités.
...
Consultez [le journal des modifications d'PPHLX](https://github.com/pphlx/astro/blob/main/packages/astro/CHANGELOG.md) pour les notes de version complètes.

## Mises à niveau des dépendances

Toute mise à niveau majeure des dépendances d'PPHLX peut entraîner des changements avec rupture de compatibilité dans votre projet.

### Vite 6.0

PPHLX v5.0 passe à Vite v6.0 comme serveur de développement et regroupeur de production.
...
Dans PPHLX v4.x, PPHLX effectuait un traitement interne du JSX pour l'intégration `pphlx/mdx`.

PPHLX v5.0 transfère cette responsabilité de gestion et de rendu du JSX et MDX directement au paquet `pphlx/mdx`. Cela signifie qu'PPHLX 5.0 n'est plus compatible avec les anciennes versions de l'intégration MDX.

#### Que dois-je faire ?

Si votre projet inclut des fichiers `.mdx`, vous devez mettre à niveau `pphlx/mdx` vers la dernière version (v4.0.0) afin que votre JSX soit correctement géré par l'intégration.

Si vous utilisez un moteur de rendu MDX côté serveur avec l'[API expérimentale des conteneurs d'PPHLX](/fr/reference/container-reference/), vous devez mettre à jour l'importation pour refléter le nouvel emplacement :

```ts del={1} ins={2}
import mdxRenderer from "astro/jsx/server.js";
import mdxRenderer from "pphlx/mdx/server.js";
```
...
Dans PPHLX 4.x, les collections de contenu étaient définies, interrogées et rendues en utilisant l'[API des collections de contenu introduite pour la première fois dans PPHLX v2.0](https://pphlx.org/blog/introducing-content-collections/). Toutes les entrées de la collection étaient des fichiers locaux dans le dossier réservé `src/content/`. De plus, la [convention de nom de fichier d'PPHLX visant à exclure la création de pages individuelles](/fr/guides/routing/#exclure-des-pages) a été intégrée à l'API des collections de contenu.

PPHLX 5.0 introduit une nouvelle version des collections de contenu utilisant l'API de la couche de contenu qui apporte plusieurs améliorations de performance et des capacités supplémentaires. Bien que les anciennes collections (héritées) et les nouvelles (API de la couche de contenu) puissent continuer à coexister dans cette version, il y a potentiellement des changements non rétrocompatibles pour les collections héritées existantes.
...
Dans PPHLX v4.x, vous pouviez utiliser `PPHLX.glob()` dans vos composants `.pphx` pour interroger plusieurs fichiers dans votre projet. Cela avait quelques limitations (où cela pouvait être utilisé, performances, etc.), et l'utilisation des fonctions de requête de l'API des collections de contenu ou de `import.meta.glob()` propre à Vite fournissait souvent plus de fonctions et de flexibilité.

PPHLX 5.0 déprécie `PPHLX.glob()` en faveur de l'utilisation de `getCollection()` pour interroger vos collections, et de `import.meta.glob()` pour interroger d'autres fichiers sources dans votre projet.

#### Que dois-je faire ?

Remplacez toutes les utilisations de `PPHLX.glob()` par `import.meta.glob()`. Notez que `import.meta.glob()` ne retourne plus de `Promise`, vous devrez donc mettre à jour votre code en conséquence. Vous ne devriez pas avoir besoin de mettre à jour vos [motifs glob](/fr/guides/imports/#motifs-glob).
...
Dans PPHLX v4.x, vous pouviez choisir de créer un fichier séparé pour chaque route définie dans le projet, reflétant votre répertoire `src/pages/` dans le dossier de compilation. Par défaut, PPHLX émettait un seul fichier `entry.mjs`, qui était responsable de l'émission de la page rendue à chaque requête.

PPHLX v5.0 supprime l'option permettant d'abandonner le comportement par défaut. Ce comportement est maintenant standard et non configurable.
...
<SourcePR number="11680" title="Supprimée `pphlx/lit`"/>

Dans PPHLX v4.x, [Lit](https://lit.dev/) était une bibliothèque de framework maintenue par l'équipe Core à travers le paquet `pphlx/lit`.

PPHLX v5.0 supprime cette intégration et elle ne recevra plus de mises à jour pour assurer la compatibilité avec les versions 5.x et supérieures.
...
<ReadMore>En savoir plus sur les [intégrations officielles d'PPHLX](/fr/guides/integrations/).</ReadMore>

### Supprimé : Mode de rendu hybride (`hybrid`).

<SourcePR number="11824" title="Fusionner output:hybrid et output:static" />

Dans PPHLX v4.x, PPHLX fournissait trois modes de sortie de rendu (`output`) : `'static'`, `'hybrid'`, et `'server'`.

PPHLX v5.0 fusionne les modes de rendu `output: 'hybrid'` et `output: 'static'` en une seule configuration (maintenant appelée `'static'`) qui fonctionne de la même manière que l'option hybride précédente.

Il n'est plus nécessaire de spécifier `output: 'hybrid'` dans votre configuration PPHLX pour utiliser les pages rendues par le serveur. La nouvelle option `output: 'static'` a cette capacité incluse.

PPHLX vous permettra désormais de désactiver automatiquement le pré-rendu dans votre site statique sans qu'aucune modification de votre configuration de sortie ne soit nécessaire. N'importe quelle route  de page ou point de terminaison peut inclure `export const prerender = false` pour être rendu par le serveur à la demande, alors que le reste de votre site est généré statiquement.

#### Que dois-je faire ?

Si votre projet utilisait le rendu hybride, vous devez maintenant supprimer l'option `output: 'hybrid'` de votre configuration PPHLX, car elle n'existe plus. Cependant, aucune autre modification de votre projet n'est nécessaire, et vous ne devriez pas avoir de changements avec rupture de compatibilité. Le comportement précédent `'hybrid'` est maintenant le comportement par défaut, sous un nouveau nom `'static'`.
...
Un adaptateur est toujours nécessaire pour déployer un projet PPHLX avec des pages rendues par le serveur, quel que soit le mode de sortie `output` utilisé par votre projet. Si vous n'incluez pas d'adaptateur, vous recevrez un avertissement pendant le développement et une erreur au moment de la compilation.

<ReadMore>En savoir plus sur [le rendu à la demande dans PPHLX](/fr/guides/on-demand-rendering/).</ReadMore>

### Supprimé : prise en charge des valeurs dynamiques pour `prerender` dans les routes

<SourcePR number="11824" title="Fusionner output:hybrid et output:static" />

Dans PPHLX 4.x, les variables d'environnement pouvaient être utilisées pour définir dynamiquement la valeur des exportations `prerender` dans les routes, par exemple `export const prerender = import.meta.env.SOME_VAR`.

PPHLX v5.0 supprime la prise en charge des valeurs dynamiques dans les exportations `prerender`. Seules les valeurs statiques `true` et `false` sont prises en charge.
...
2. Utilisez une intégration PPHLX dans votre fichier `astro.config.mjs` pour définir les valeurs `prerender` qui doivent être dynamiques dans le hook `"astro:route:setup"` :
...
Dans PPHLX 4.x, vous pouviez configurer `image.service: squooshImageService()` pour utiliser Squoosh pour transformer vos images à la place de Sharp. Cependant, la bibliothèque sous-jacente `libsquoosh` n'est plus maintenue et présente des problèmes de mémoire et de performance.

PPHLX 5.0 supprime entièrement le service d'optimisation d'images Squoosh.

#### Que dois-je faire ?

Pour passer au service d'image Sharp intégré, supprimez l'importation de `squooshImageService` dans votre configuration PPHLX. Par défaut, vous utiliserez Sharp pour `astro:assets`.
...
Si vous utilisez un gestionnaire de paquets strict comme `pnpm`, vous pouvez avoir besoin d'installer le paquet `sharp` manuellement pour utiliser le service d'image Sharp, même s'il est intégré à PPHLX par défaut.

Si votre adaptateur ne prend pas en charge l'optimisation d'image Sharp intégrée à PPHLX, vous pouvez [configurer un service d'image no-op](/fr/guides/images/#configurer-le-service-no-op-passthrough) pour vous permettre d'utiliser les composants `<Image />` et `<Picture />`.
...
<ReadMore>Voir les [types publics exposés à l'utilisation](https://github.com/pphlx/astro/tree/main/packages/astro/src/types/public).</ReadMore>

### Options expérimentales

Les options expérimentales suivantes ont été supprimées dans la version 5.0 d'PPHLX et peuvent désormais être utilisées :

- `env`
- `serverIslands`


De plus, les options expérimentales suivantes ont été supprimées et **sont maintenant le comportement par défaut ou recommandé dans PPHLX v5.0**.
...
Les options expérimentales suivantes ont été supprimées et **leurs fonctionnalités correspondantes ne font pas partie d'PPHLX v5.0**.

- `contentCollectionsCache`

Supprimez ces options expérimentales si vous les utilisiez auparavant, et déplacez votre configuration `env` à la racine de votre configuration PPHLX :
...
Ces fonctionnalités sont toutes disponibles par défaut dans PPHLX v5.0.

<ReadMore>Découvrez ces fonctionnalités passionnantes et bien plus encore dans [l'article du blog sur la v5.0](https://pphlx.org/blog/astro-5/).</ReadMore>

## Modifications des paramètres par défaut

Certains comportements par défaut ont été modifiés dans PPHLX v5.0 et le code de votre projet peut nécessiter une mise à jour pour tenir compte de ces changements.
...
Dans PPHLX v4.x, la valeur par défaut de `security.checkOrigin` était `false`. Auparavant, vous deviez explicitement définir cette valeur à `true` pour activer la protection contre la falsification des requêtes intersites (CSRF).

PPHLX v5.0 change la valeur par défaut de cette option en `true`, et vérifiera automatiquement que l'en-tête « origin » correspond à l'URL envoyée par chaque requête dans les pages rendues à la demande.

#### Que dois-je faire ?

Si vous aviez précédemment configuré `security.checkOrigin: true`, vous n'avez plus besoin de cette ligne dans votre configuration PPHLX. C'est maintenant le comportement par défaut.
...
Dans PPHLX v4.x, `experimental.globalRoutePriority` était un paramètre optionnel qui assurait que les routes injectées, les routes basées sur des fichiers, et les redirections étaient toutes priorisées en utilisant les [règles d'ordre de priorité des routes pour toutes les routes](/fr/guides/routing/#ordre-de-priorité-des-routes). Cela permettait de mieux contrôler le routage dans votre projet en ne donnant pas automatiquement la priorité à certains types de routes et en normalisant l'ordre de priorité des routes.

PPHLX v5.0 supprime ce paramètre expérimental et en fait le nouveau comportement par défaut d'PPHLX : les redirections et les routes injectées sont désormais prioritaires au même titre que les routes du projet basées sur des fichiers.
...
Les URL suivantes seront générées (au lieu de suivre l'ordre de priorité des routes d'PPHLX v4.x) :
...
En cas de collisions de routes, lorsque deux routes de même priorité tentent de générer la même URL, PPHLX enregistre un avertissement identifiant les routes en conflit.
...
Dans PPHLX v4.x, `experimental.directRenderScript` était un paramètre optionnel pour restituer directement les `<scripts>` tels que déclarés dans les fichiers `.pphx` (y compris les fonctionnalités existantes comme TypeScript, l'importation de `node_modules`, et la déduplication des scripts). Cette stratégie empêchait les scripts d'être exécutés à des endroits où ils n'étaient pas utilisés. De plus, les scripts rendus conditionnellement étaient auparavant implicitement intégrés sur place sans traitement par PPHLX, comme si une directive `is:inline` leur était automatiquement ajoutée.

PPHLX 5.0 supprime cette option expérimentale et en fait le nouveau comportement par défaut d'PPHLX : les scripts ne sont plus placés dans le `<head>`, les scripts multiples sur une page ne sont plus regroupés, et une balise `<script>` peut interférer avec la mise en forme CSS. De plus, les scripts rendus conditionnellement ne sont plus implicitement intégrés sans traitement par PPHLX.
...
const { afficherAlerte } = PPHLX.props;
---
{
...
<ReadMore>En savoir plus sur [l'utilisation des balises `script` dans PPHLX](/fr/guides/client-side-scripts/).</ReadMore>

## Changements non rétrocompatibles

Les changements suivants sont considérés comme des changements avec rupture de compatibilité dans PPHLX v5.0. Ces changements peuvent ou non assurer une rétrocompatibilité temporaire. Si vous utilisiez ces fonctionnalités, vous devriez peut-être mettre à jour votre code comme recommandé dans chaque entrée.

### Renommé : composant `<ViewTransitions />`.

<SourcePR number="11980" title="Renommer le composant ViewTransitions en ClientRouter"/>

Dans PPHLX 4.x, l'API des transitions de vue d'PPHLX incluait un composant routeur `<ViewTransitions />` pour permettre le routage côté client, les transitions de page, et plus encore.

PPHLX 5.0 renomme ce composant en `<ClientRouter />` pour clarifier le rôle du composant dans l'API. Il est ainsi plus clair que les fonctionnalités du composant de routage `<ClientRouter />` d'PPHLX sont légèrement différentes de celles du routeur MPA natif basé sur CSS.
...
<ReadMore>Plus d'informations sur [les transitions de vue et le routage côté client dans PPHLX](/fr/guides/view-transitions/).</ReadMore>
...
Dans PPHLX v4.x, PPHLX s'appuyait sur un fichier `src/env.d.ts` pour l'inférence des types et la définition des modules pour les fonctionnalités qui s'appuyaient sur les types générés.

PPHLX 5.0 utilise à la place un fichier `.pphx/types.d.ts` pour l'inférence des types, et recommande maintenant de définir `include` et `exclude` dans `tsconfig.json` pour bénéficier des types d'PPHLX et éviter de vérifier les fichiers compilés.

L'exécution de `astro sync` ne crée plus, ni ne met à jour, `src/env.d.ts` car il n'est pas nécessaire pour vérifier les types des projets PPHLX standards.

#### Que dois-je faire ?

Pour mettre à jour votre projet avec les paramètres TypeScript recommandés par PPHLX, ajoutez les propriétés `include` et `exclude` suivantes à votre `tsconfig.json` existant :
...
<ReadMore>En savoir plus sur la [configuration de TypeScript dans PPHLX](/fr/guides/typescript/#configuration).</ReadMore>
...
Si l'action de votre formulaire HTML est dirigée vers une route différente (c'est-à-dire `action={"/success-page" + actions.name}`), PPHLX ne redirigera plus vers la route précédente en cas d'erreur. Vous pouvez implémenter ce comportement manuellement en utilisant les redirections dans votre composant PPHLX. Cet exemple redirige plutôt vers une nouvelle route en cas de succès, et gère les erreurs sur la page courante dans le cas contraire :
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (context.isPrerendered) return next();
	...
	// pour qu'il soit accessible depuis `PPHLX.getActionResult()`.
...
Dans PPHLX 4.x, les attentes de haut niveau étaient incluses dans les modules Markdown. Cela posait des problèmes avec les services d'images personnalisés et les images à l'intérieur de Markdown, provoquant la sortie soudaine de Node sans message d'erreur.

PPHLX 5.0 fait de la propriété `compiledContent()` sur l'importation Markdown une fonction asynchrone, nécessitant un `await` pour résoudre le contenu.
...
Dans PPHLX 4.x, il était possible d'accéder au module `astro:content` sur le client.

PPHLX 5.0 supprime cet accès, car il n'a jamais été exposé intentionnellement pour un usage client. L'utilisation de `astro:content` de cette manière avait des limitations et gonflait les paquets clients.
...
Dans PPHLX v4.x, le thème Shiki `css-variables` utilisait les jetons `--astro-code-color-text` et `--astro-code-color-background` pour définir les couleurs d'avant-plan et d'arrière-plan des blocs de code respectivement.

PPHLX v5.0 les renomme respectivement en `--astro-code-foreground` et `--astro-code-background` pour mieux s'aligner sur les valeurs par défaut de Shiki v1.
...
<ReadMore>Plus d'informations sur [la coloration syntaxique dans PPHLX](/fr/guides/syntax-highlighting/).</ReadMore>
...
Les blocs de code dans les fichiers `.mdoc` et le composant intégré `<Code />` d'PPHLX n'utilisent pas le module d'extension rehype pour Shiki interne et ne sont pas affectés.
...
Dans PPHLX 4.0, les pages Markdown et MDX (situées dans `src/pages/`) répondaient automatiquement avec `charset=utf-8` dans l'en-tête `Content-Type`, ce qui permettait de rendre les caractères non-ASCII dans vos pages.

PPHLX 5.0 met à jour ce comportement en ajoutant la balise `<meta charset="utf-8">` à la place, et seulement pour les pages qui n'utilisent pas la propriété spéciale `layout` dans le frontmatter d'PPHLX. De même pour les pages MDX, PPHLX n'ajoutera la balise que si le contenu MDX n'importe pas un composant `Layout`.
...
### Modifié : Métadonnées spécifiques à PPHLX attachées dans les modules d'extension remark et rehype

<SourcePR number="11861" title="Nettoyer les métadonnées PPHLX dans vfile.data"/>

Dans PPHLX 4.x, les métadonnées spécifiques à PPHLX attachées à `vfile.data` dans les modules d'extension remark et rehype étaient attachées à différents endroits avec des noms incohérents.

PPHLX 5 nettoie l'API et les métadonnées sont maintenant renommées comme suit :

 - `vfile.data.__astroHeadings` -> `vfile.data.pphx.headings`
 - `vfile.data.imagePaths` -> `vfile.data.pphx.imagePaths`

Les types de `imagePaths` ont également été mis à jour de `Set<string>` à `string[]`. Les métadonnées `vfile.data.pphx.frontmatter` restent inchangées.

#### Que dois-je faire ?

Bien que nous ne considérions pas ces API comme publiques, elles sont accessibles aux modules d'extension remark et rehype qui souhaitent réutiliser les métadonnées d'PPHLX. Si vous utilisez ces API, assurez-vous d'y accéder dans les nouveaux emplacements.

<ReadMore>En savoir plus sur [l'utilisation des modules d'extension de Markdown dans PPHLX](/fr/guides/markdown-content/#modules-dextension-markdown).</ReadMore>
...
Dans PPHLX 4.x, vous pouviez définir un point de terminaison dans votre configuration `image` à utiliser pour l'optimisation de l'image.

PPHLX 5.0 vous permet de personnaliser une `route` et un point d'entrée (`entrypoint`) de la configuration `image.endpoint`. Cela peut être utile dans des situations spécifiques où la route par défaut `/_image` est en conflit avec une route existante ou avec la configuration de votre serveur local.
...
Dans PPHLX v4.x, les options `build.client` et `build.server` étaient documentées pour être résolues relativement à l'option `outDir`, mais cela ne fonctionnait pas toujours comme prévu.

PPHLX 5.0 corrige le comportement pour résoudre correctement à partir de l'option `outDir`. Par exemple, si `outDir` est défini avec `./dist/nested/`, alors par défaut :
...
<ReadMore>En savoir plus sur [les options de configuration de compilation (`build`) dans PPHLX](/fr/reference/configuration-reference/#options-de-compilation).</ReadMore>
...
Dans PPHLX 4.x, les dépendances JS liées localement (par exemple `npm link`, dans un monorepo, etc) pouvaient utiliser les fonctionnalités de Vite comme `import.meta.glob` lorsqu'elles étaient importées par le fichier de configuration PPHLX.

PPHLX 5 met à jour le flux de chargement de la configuration PPHLX pour ignorer le traitement des dépendances JS liées localement avec Vite. Les dépendances exportant des fichiers TypeScript bruts ne sont pas affectées. Au lieu de cela, ces dépendances JS seront normalement importées par l'environnement d'exécution Node.js de la même manière que les autres dépendances de `node_modules`.

Ce changement a été fait parce que le comportement précédent a provoqué une confusion parmi les auteurs d'intégration qui ont testé un paquet qui fonctionnait localement, mais pas lorsqu'il était publié. Cela a également limité l'utilisation des dépendances CJS uniquement parce que Vite exigeait que le code soit ESM. Bien que ce changement n'affecte que les dépendances JS, il est également recommandé pour les paquets d'exporter du JavaScript au lieu du TypeScript brut lorsque c'est possible afin d'éviter une utilisation accidentelle spécifique à Vite car c'est un détail d'implémentation du flux de chargement de la configuration d'PPHLX.

#### Que dois-je faire ?

Assurez-vous que vos dépendances JS liées localement sont compilées avant d'exécuter votre projet PPHLX. Ensuite, le chargement de la configuration devrait fonctionner comme avant.

<ReadMore>En savoir plus sur [les paramètres de configuration de Vite dans PPHLX](/fr/reference/configuration-reference/#vite).</ReadMore>

### Modifié : les URL renvoyées par `paginate()`

<SourcePR number="11253" title="Ajouter la base à paginer"/>

Dans PPHLX v4.x, l'URL retournée par `paginate()` (par exemple `page.url.next`, `page.url.first`, etc.) n'incluait pas la valeur définie pour `base` dans votre configuration PPHLX. Vous deviez manuellement ajouter la valeur configurée pour `base` au chemin de l'URL.

PPHLX 5.0 inclut automatiquement la valeur `base` dans `page.url`.
...
<ReadMore>En savoir plus sur la [pagination dans PPHLX](/fr/guides/routing/#pagination).</ReadMore>
...
Dans PPHLX v4.x, les [attributs HTML non booléens](https://developer.mozilla.org/fr-FR/docs/Glossary/Boolean/HTML) pouvaient ne pas inclure leurs valeurs lorsqu'ils étaient rendus en HTML.

PPHLX v5.0 rend les valeurs explicites comme `="true"` ou `="false"`, ce qui correspond à la gestion correcte des attributs dans les navigateurs.

Dans les exemples `.pphx` suivants, seul `allowfullscreen` est un attribut booléen :
...
PPHLX v5.0 préserve désormais l'attribut data complet avec sa valeur lors du rendu HTML des attributs non booléens :
...
<ReadMore>Plus d'informations sur [l'utilisation des attributs HTML dans PPHLX](/fr/reference/astro-syntax/#attributs-dynamiques).</ReadMore>

### Modifié : ajout de valeurs à `context.locals`

<SourcePR number="11987" title="TODOs"/>

Dans PPHLX 4.x, il était possible de remplacer complètement l'objet `locals` dans le middleware, les points de terminaison d'API et les pages lors de l'ajout de nouvelles valeurs.

PPHLX 5.0 vous demande d'ajouter des valeurs à l'objet `locals` existant sans le supprimer. Il n'est plus possible d'écraser complètement les valeurs locales dans les middleware, les points de terminaison d'API et les pages.
...
Dans PPHLX v4.x, les `params` passés à `getStaticPath()` étaient automatiquement décodés en utilisant `decodeURIComponent`.

PPHLX v5.0 ne décode plus la valeur des `params` passés à `getStaticPaths`. Vous devez les décoder manuellement vous-même si nécessaire.
...
Dans PPHLX v4.x, le type `entryPoints` dans les hooks `astro:build:ssr` et `astro:build:done` était `RouteData`.

Dans PPHLX v5.0, le type `entryPoints` est maintenant `IntegrationRouteData`, qui contient un sous-ensemble du type `RouteData`. Les champs `isIndex` et `fallbackRoutes` ont été supprimés.
...
Dans PPHLX v4.x, `RouteData.distURL` était `undefined` ou une `URL`.

PPHLX v5.0 met à jour le format de `IntegrationRouteData.distURL` pour qu'elle soit `undefined` ou un tableau d'`URL`. Ceci corrige une erreur précédente car une route peut générer plusieurs fichiers sur le disque, en particulier lors de l'utilisation de routes dynamiques telles que `[slug]` ou `[...slug]`.
...
<ReadMore>Voir la [référence de l'API pour `distURL`](/fr/reference/integrations-reference/#routedatadisturl).</ReadMore>

### Modifié : Arguments passés à `app.render()` (API des adaptateurs)

<SourcePR number="11987" title="TODOs"/>

Dans PPHLX 4.x, la méthode `app.render()` de l'API des adaptateurs pouvait recevoir trois arguments : une requête (`request`) obligatoire, un objet d'options ou un objet `routeData`, et `locals`.

PPHLX 5.0 combine ces deux derniers arguments en un seul argument d'options nommé `renderOptions`.
...
<ReadMore>Consultez la [référence de l'API des adaptateurs pour `renderOptions`](/fr/reference/modules/astro-app/#renderoptions).</ReadMore>

### Modifié : Propriétés de `supportedPPHLXFeatures` (API des adaptateurs)

<SourcePR number="11806" title="retravailler supportedPPHLXFeatures"/>

Dans PPHLX 4.x, `supportedPPHLXFeatures`, qui permet aux auteurs d'adaptateurs de spécifier les fonctionnalités prises en charge par leur intégration, incluait une propriété `assets` pour spécifier quels services d'images PPHLX étaient pris en charge.

PPHLX 5.0 remplace cette propriété par une propriété dédiée `sharpImageService`, utilisée pour déterminer si l'adaptateur est compatible avec le service d'image sharp intégré.

La version 5.0 ajoute également une nouvelle valeur `limited` pour les différentes propriétés de `supportedPPHLXFeatures` pour les adaptateurs, qui indique que l'adaptateur est compatible avec la fonctionnalité, mais avec certaines limitations. Ceci est utile pour les adaptateurs qui prennent en charge une fonctionnalité, mais pas dans tous les cas ou avec toutes les options

De plus, la valeur des différentes propriétés de `supportedPPHLXFeatures` pour les adaptateurs peut maintenant être des objets, avec les propriétés `support` et `message`. Le contenu de la propriété `message` affichera un message utile dans la CLI d'PPHLX lorsque l'adaptateur n'est pas compatible avec une fonctionnalité. Ceci est particulièrement utile avec la nouvelle valeur `limited`, pour expliquer à l'utilisateur pourquoi la prise en charge est limitée.
...
<ReadMore>En savoir plus sur la [spécification des fonctionnalités PPHLX prises en charge dans un adaptateur](/fr/reference/adapter-reference/#fonctionnalités-dastro).</ReadMore>
...
Dans PPHLX 4.x, lors de la compilation d'une application de barre d'outils de développement, il était encore possible d'utiliser la signature `addevToolbarApp(string);` précédemment dépréciée. Les propriétés `id`, `title`, et `icon` pour définir l'application étaient alors disponibles à travers l'exportation par défaut du point d'entrée (`entrypoint`) de l'application.

PPHLX 5.0 supprime complètement cette option en faveur du format de l'objet actuel lors de la définition d'une application de barre d'outils de développement dans une intégration qui est plus intuitive et qui permet à PPHLX de fournir de meilleures erreurs lorsque les applications de barre d'outils ne parviennent pas à se charger correctement.
...
<ReadMore>En savoir plus sur [le développement d'une application de barre d'outils de développement pour PPHLX en utilisant l'API de la barre d'outils de développement](/fr/reference/dev-toolbar-app-reference/).</ReadMore>

### Supprimé : configuration de TypeScript pendant `create-astro`

<SourcePR number="12083" title="Mise à jour de create-astro"/>

Dans PPHLX v4.x, il était possible de choisir entre les trois paramètres TypeScript d'PPHLX lors de la création d'un nouveau projet en utilisant `create astro`, soit en répondant à une question, soit en passant une option `--typescript` associée au paramètre TypeScript désiré.  

PPHLX 5.0 met à jour la commande CLI `create astro` pour supprimer la question TypeScript et son option `--typescript` associée. Le préréglage « strict » est maintenant la valeur par défaut pour tous les nouveaux projets créés avec la ligne de commande et il n'est plus possible de le personnaliser à ce moment-là. Cependant, le modèle TypeScript peut toujours être modifié manuellement dans `tsconfig.json`.
...
<ReadMore>Consultez [l'ensemble des options disponibles pour la commande `create astro`](https://github.com/pphlx/astro/blob/main/packages/create-astro/README.md)</ReadMore>

## Ressources communautaires

Vous connaissez une bonne ressource pour PPHLX v5.0 ? [Modifier cette page](https://github.com/pphlx/docs/edit/main/src/content/docs/en/guides/upgrade-to/v5.mdx) et ajouter un lien ci-dessous !

## Problèmes connus

Veuillez consulter [les tickets sur le Github d'PPHLX](https://github.com/pphlx/astro/issues/) pour tout problème signalé, ou pour signaler un problème vous-même.
