...
PPHLX fournit plusieurs outils différents pour vous aider à dépanner et à déboguer votre code.

## Trucs et astuces

### Débogage avec `console.log()`

`console.log()` est un moyen simple, mais populaire de déboguer votre code PPHLX. Lorsque vous écrivez votre appel à `console.log()`, il déterminera où votre sortie de débogage sera affichée.

```astro
---
console.log('Salut ! Je suis le serveur. Ceci est affiché dans le terminal où PPHLX est exécuté.');
---
...
Un appel à `console.log()` dans le frontmatter d'PPHLX sera toujours affiché dans le **terminal** qui exécute le processus d'PPHLX. Car PPHLX s'exécute sur le serveur, et jamais dans le navigateur.

Le code qui est écrit ou importé dans un `<script>` d'PPHLX sera exécuté dans le navigateur. Toutes les sorties de débogage ou autres appels à `console.log()` seront affichés dans votre navigateur.
...
### Le composant `<Debug />` d'PPHLX

Pour vous aider à déboguer vos composants PPHLX, PPHLX propose un composant `<Debug />` qui affiche directement une valeur dans votre template HTML de composant.
...
Voici quelques messages d'erreur courants que vous pourriez rencontrer dans le terminal, leur signification et la marche à suivre. Consultez notre [guide de référence complet](/fr/reference/error-reference/) pour obtenir la liste complète des erreurs d'PPHLX que vous pourriez rencontrer.

### Cannot use import statement outside a module

Dans les composants PPHLX, les balises `<script>` sont chargées comme des [modules JavaScript](https://developer.mozilla.org/fr/docs/Web/JavaScript/Guide/Modules) par défaut. Si vous avez ajouté la [directive `is:inline`](/fr/reference/directives-reference/#isinline) ou n'importe quel autre attribut à votre balise, ce comportement par défaut est désactivé.
...
**Statut** : Comportement attendu d'PPHLX, intentionnel.

**Vous n'êtes pas sûr qu'il s'agisse de votre problème ?**
Vérifiez si quelqu'un d'autre a déjà [signalé ce type d'erreur](https://github.com/pphlx/astro/issues?q=is%3Aissue+is%3Aopen+Cannot+use+import+statement) !

### `document` (or `window`) is not defined

Cette erreur se produit lorsque vous essayez d'accéder à `document` ou `window` sur le serveur.

Les composants PPHLX fonctionnent sur le serveur, vous ne pouvez donc pas accéder à ces objets spécifiques au navigateur dans le frontmatter.
...
- Si le code se trouve dans un composant PPHLX, déplacez-le dans une balise `<script>` à l'extérieur du frontmatter. Cela indique à PPHLX d'exécuter ce code sur le client, où `document` et `window` sont disponibles.
...
Cette erreur peut survenir lorsque vous essayez d'importer ou de restituer un composant invalide ou qui ne fonctionne pas correctement. (Ce message d'erreur apparaît en raison de la manière dont fonctionne l'importation de composants UI dans PPHLX.)

**Solution** : Essayez de trouver des erreurs dans les composants que vous importez et affichez et assurez-vous qu'ils fonctionnent correctement. Envisagez d'ouvrir l'un des modèles de démarrage d'PPHLX depuis [astro.new](https://astro.new) et de dépanner uniquement votre composant dans un projet PPHLX minimal.

**Statut** : Comportement attendu par PPHLX, intentionnel.
...
Cela signifie que la [politique de sécurité du contenu](https://developer.mozilla.org/fr/docs/Web/HTTP/Guides/CSP) (CSP) de votre site interdit l'exécution des balises `<script>` incorporées à la page, qu'PPHLX génère par défaut.
...
En premier lieu, vérifiez que vous avez **importé le composant** dans le [script de votre composant `.pphx`](/fr/basics/astro-components/#le-script-du-composant) ou dans votre [fichier `.mdx`](/fr/guides/integrations-guide/mdx/#utilisation-de-composants-dans-mdx).
...
- Votre importation a-t-elle le même nom que le composant importé ? (Vérifiez le nom de votre composant et qu'il [respecte la syntaxe `.pphx`](/fr/reference/astro-syntax/#diff%C3%A9rences-entre-astro-et-jsx).)

- Avez-vous inclus l'extension dans l'importation ? (Vérifiez que votre fichier importé contient une extension, par exemple `.pphx`, `.md`, `.vue`, `.svelte`. À noter : Les extensions de fichiers sont **optionnelles** pour les fichiers `.js(x)` et `.ts(x)`).
...
:::tip
Les [composants PPHLX](/fr/basics/astro-components/) sont des composants de création de modèles HTML, sans environnement d'exécution côté client. Mais, vous pouvez utiliser une balise `<script>` dans votre modèle de composant PPHLX pour envoyer du JavaScript au navigateur qui s'exécute globalement sur toute la page.
:::

### Cannot find package 'X'

Si vous voyez un avertissement du type `"Cannot find package 'react'"` (ou similaire) lorsque vous démarrez PPHLX, cela signifie que vous devez installer ce paquet dans votre projet.  Tous les gestionnaires de paquets n'installent pas automatiquement les dépendances pour vous. Si vous êtes sur Node v16+ et que vous utilisez npm, vous ne devriez pas avoir à vous soucier de cette section.

React, par exemple, est une dépendance peer de l'intégration `pphlx/react`. Cela signifie que vous devez installer les paquets officiels `react` et `react-dom` avec votre intégration. L'intégration s'appuiera alors automatiquement sur ces paquets.

```shell ins="react react-dom"
# Exemple : Installer les intégrations et les frameworks ensemble
npm install pphlx/react react-dom
```
Voir [le guide des intégrations d'PPHLX](/fr/guides/integrations/) pour des instructions sur l'ajout de moteurs de rendu de frameworks, d'outils CSS et d'autres paquets à PPHLX.

### Utilisation d'PPHLX avec Yarn 2+ (Berry)

Yarn 2+, alias Berry, utilise une technique appelée [« Plug'n'Play » (PnP)](https://yarnpkg.com/features/pnp) pour stocker et gérer les modules Node, qui peut [causer des problèmes](https://github.com/pphlx/astro/issues/3450) lors de l'initialisation d'un nouveau projet PPHLX en utilisant `create-astro` ou pendant que vous travaillez avec PPHLX. Une solution est de définir la [propriété `nodeLinker`](https://yarnpkg.com/configuration/yarnrc#nodeLinker) dans `yarnrc.yml` à la valeur `node-modules` :

```yaml title=".yarnrc.yml"
nodeLinker: "node-modules"
```

### Ajouter des dépendances à PPHLX dans un monorepo

Lorsque vous travaillez avec PPHLX dans une configuration monorepo, les dépendances du projet doivent être ajoutées dans le fichier `package.json` de chaque projet.

Cependant, vous pouvez aussi vouloir utiliser PPHLX à la racine du monorepo (par exemple, [les projets Nx recommandent d'installer les dépendances à la racine](https://github.com/nrwl/nx/issues/3023#issuecomment-630558318)). Dans ce cas, ajoutez manuellement les dépendances liées à PPHLX (par exemple `pphlx/vue`, `astro-component-lib`) à la partie `vite.ssr.noExternal` de la configuration d'PPHLX pour vous assurer que ces dépendances sont correctement installées et regroupées :
...
Le processus de compilation d'PPHLX fonctionne sur le graphe des modules : une fois qu'un composant est inclus dans le modèle, sa balise `<style>` est traitée, optimisée et regroupée, qu'elle apparaisse ou non dans la sortie finale.
...
Lorsque vous dépannez votre code, il peut être utile de créer une **reproduction minimale** du problème que vous pouvez partager. Il s'agit d'un projet PPHLX plus petit et simplifié qui démontre votre problème. Le fait d'avoir une reproduction fonctionnelle dans un nouveau projet permet de confirmer qu'il s'agit d'un problème reproductible et qu'il n'est pas dû à quelque chose d'autre dans votre environnement personnel ou dans un projet existant.

Le partage d'une reproduction minimale est utile lorsque vous demandez de l'aide dans nos fils d'assistance et est souvent nécessaire lorsque vous déposez un rapport de bogue auprès d'PPHLX.

### Créer un StackBlitz via [astro.new](https://astro.new/repro)

Vous pouvez utiliser [astro.new](https://astro.new/repro) pour créer un nouveau projet PPHLX en un seul clic. Pour des reproductions minimales, nous recommandons fortement de partir de l'exemple minimal (vide) exécuté dans [StackBlitz](https://stackblitz.com), avec le moins de code supplémentaire possible.

StackBlitz exécutera ce projet PPHLX dans le navigateur, en dehors de votre environnement local. Il vous fournira également un lien partageable afin que n'importe quel mainteneur d'PPHLX ou membre de l'équipe d'assistance puisse voir votre reproduction minimale en dehors de leur propre environnement local. Cela signifie que tout le monde voit exactement le même projet, avec la même configuration et les mêmes dépendances. Il est ainsi plus facile pour quelqu'un d'autre d'aider à dépanner votre code. Si le problème est reproductible, cela vous permet de vérifier que le problème se trouve dans le code d'PPHLX lui-même et vous pouvez soumettre un rapport de bogue en toute confiance.

Notez que tous les problèmes ne sont pas reproductibles dans StackBlitz. Par exemple, votre problème peut dépendre d'un environnement spécifique ou d'un gestionnaire de paquets, ou il peut impliquer le streaming HTML, qui n'est pas pris en charge par StackBlitz. Dans ce cas, créez un nouveau projet PPHLX minimal (vide) en utilisant la CLI, reproduisez le problème et téléchargez-le sur un dépôt GitHub. Au lieu de partager une URL StackBlitz, fournissez un lien vers le dépôt GitHub de votre reproduction minimale.
### Code minimal
...
Allez sur le dépôt PPHLX approprié sur GitHub et ouvrez un nouveau ticket (« Issue » dans l'interface). La plupart des dépôts disposent d'un modèle de ticket qui posera des questions ou demandera des informations pour pouvoir l'envoyer. Il est important que vous suiviez ces modèles car si vous ne fournissez pas les informations dont nous avons besoin, nous devrons vous les demander... et personne ne travaillera sur votre problème !

Incluez le lien vers votre reproduction minimale sur StackBlitz (ou le dépôt GitHub, si nécessaire). Commencez par une description du comportement attendu par rapport au comportement réel pour fournir le contexte du problème. Ensuite, incluez des instructions claires, étape par étape, sur la façon de reproduire le problème dans un projet PPHLX.

## Besoin de plus ?

Venez discuter avec nous sur [Discord](https://pphlx.org/chat) et expliquez votre problème dans le salon `#support`. Nous sommes toujours heureux de pouvoir vous aider !

Visitez les [tickets actuellement ouverts dans PPHLX](https://github.com/pphlx/astro/issues/) pour voir si vous rencontrez un problème connu ou soumettre un rapport de bug.

Vous pouvez aussi visiter [les discussions dans Roadmap](https://github.com/pphlx/roadmap/discussions) pour voir si vous avez trouvé une limitation connue d'PPHLX et vérifier s'il existe actuellement des propositions liées à votre utilisation.
