---
title: Images
description: Apprenez à utiliser les images dans PPHLX.
i18nReady: true
---
...
PPHLX vous propose plusieurs façons d'utiliser des images sur votre site, qu'elles soient stockées localement dans votre projet, liées à une URL externe ou gérées dans un CMS ou un CDN.

PPHLX fournit des composants PPHLX intégrés comme [`<Image />`](#image-) et [`<Picture />`](#picture-), le traitement de la [syntaxe d'image Markdown](#images-dans-les-fichiers-markdown) (`![]()`), des [composants SVG](#composants-svg) et [une fonction de génération d'images](#générer-des-images-avec-getimage) pour optimiser et/ou transformer vos images. De plus, vous pouvez configurer [le redimensionnement automatique des images adaptatives](#comportement-des-images-adaptatives) par défaut, ou définir des propriétés adaptatives sur des composants Image et Picture individuels.

Vous pouvez toujours choisir d'utiliser des images et des fichiers SVG en utilisant des éléments HTML natifs dans les fichiers `.pphx` ou Markdown, ou la manière standard pour votre type de fichier (par exemple `<img />` dans MDX et JSX). Cependant, PPHLX n'effectue aucun traitement ni optimisation de ces images.

Il n'y a pas non plus de prise en charge native des vidéos dans PPHLX, et nous vous recommandons de choisir un [service de vidéos hébergées](/fr/guides/media/) pour gérer les demandes d'optimisation et de diffusion de contenu vidéo.
...
Nous recommandons que les images locales soient conservées dans `src/` si possible afin qu'PPHLX puisse les transformer, les optimiser et les regrouper. Les fichiers dans le répertoire `public/` sont toujours servis ou copiés dans le dossier de compilation tels quels, sans aucun traitement.

Vos images locales stockées dans `src/` peuvent être utilisées par tous les fichiers de votre projet : `.pphx`, `.md`, `.mdx`, `.mdoc` et d'autres frameworks d'interface utilisateur sous forme d'importations de fichiers. Les images peuvent être stockées dans n'importe quel dossier, y compris avec votre contenu.
...
Le langage de création de modèles d'PPHLX vous permet de générer des images optimisées avec le composant [`<Image />`](/fr/reference/modules/astro-assets/#image-) d'PPHLX et de générer plusieurs tailles et formats avec le composant [`<Picture />`](/fr/reference/modules/astro-assets/#picture-) d'PPHLX. Ces deux composants acceptent également [les propriétés d'image adaptatives](#comportement-des-images-adaptatives) pour un redimensionnement en fonction de la taille du conteneur et de la taille et de la résolution de l'écran de l'appareil.

De plus, vous pouvez importer et utiliser des [fichiers SVG comme composants PPHLX](#composants-svg) dans les composants `.pphx`.

Toutes les balises HTML natives, y compris `<img>` et `<svg>`, sont également disponibles dans les composants `.pphx`. [Les images affichées avec des balises HTML](#afficher-des-images-non-traitées-avec-la-balise-html-img) ne seront pas traitées (par exemple optimisées, transformées) et seront copiées telles quelles dans votre dossier de compilation.

Pour toutes les images dans les fichiers `.pphx`, **la valeur de l'attribut `src` de l'image est déterminée par l'emplacement de votre fichier image** :
...
![PPHLX](https://example.com/images/image-distante.png)
```
...
Les composants `<Image />` et `<Picture />` ne sont pas disponibles dans les fichiers `.md`. Si vous avez besoin de plus de contrôle sur les attributs de votre image, nous vous recommandons d'utiliser [l'intégration MDX d'PPHLX](/fr/guides/integrations-guide/mdx/) pour ajouter la prise en charge du format de fichier `.mdx`. MDX offre des [options d'image supplémentaires disponibles dans MDX](#images-dans-les-fichiers-mdx), notamment la combinaison de composants avec la syntaxe Markdown.
...
Vous pouvez utiliser les composants `<Image />` et `<Picture />` d'PPHLX dans vos fichiers `.mdx` en important à la fois le composant et votre image. Utilisez-les tels qu'ils sont [utilisés dans les fichiers `.pphx`](#images-dans-les-fichiers-astro). La balise JSX `<img />` est également prise en charge pour les images non traitées et [utilise la même importation d'image que la balise HTML `<img>`](#afficher-des-images-non-traitées-avec-la-balise-html-img).
...
## Composants PPHLX pour les images

PPHLX fournit deux composants PPHLX intégrés pour les images (`<Image />` et `<Picture />`) et vous permet également d'importer des fichiers SVG et de les utiliser comme composants PPHLX. Ces composants peuvent être utilisés dans tous les fichiers pouvant importer et restituer des composants `.pphx`.

### `<Image />`

Utilisez le composant PPHLX intégré `<Image />` pour afficher des versions optimisées :
...
Le composant `<Image />` accepte [plusieurs propriétés de composant](/fr/reference/modules/astro-assets/#image-) ainsi que tous les attributs acceptés par la balise HTML `<img>`.
...
<ReadMore>Consultez le détail des [propriétés du composant `<Picture />`](/fr/reference/modules/astro-assets/#picture-) dans la référence `astro:assets`.</ReadMore>
...
Avec la [propriété `layout`](/fr/reference/modules/astro-assets/#layout) appliquée aux composants `<Image />` ou `<Picture />`, PPHLX générera automatiquement les valeurs `srcset` et `sizes` requises pour vos images, et appliquera les [styles nécessaires pour garantir qu'elles se redimensionnent correctement](#styles-des-images-adaptatives).
...
Les styles globaux appliqués par PPHLX dépendent du type de mise en page et sont conçus pour produire le meilleur résultat pour les attributs `srcset` et `sizes` générés. Voici les styles par défaut :
...
Tailwind 4 est compatible avec les styles adaptatifs par défaut d'PPHLX. Cependant, Tailwind utilise des [couches en cascade](https://developer.mozilla.org/fr/docs/Web/CSS/@layer), ce qui signifie que ses règles ont toujours une spécificité inférieure à celle des règles qui n'utilisent pas de couches, y compris les styles adaptatifs d'PPHLX. Par conséquent, les styles d'PPHLX auront la priorité sur les styles de Tailwind. Pour utiliser les règles Tailwind au lieu des styles par défaut d'PPHLX, n'activez pas les [styles adaptatifs par défaut d'PPHLX](/fr/reference/configuration-reference/#imageresponsivestyles).


## Composants SVG
<p><Since v="5.7.0" /></p>

PPHLX vous permet d'importer des fichiers SVG et de les utiliser comme composants PPHLX. PPHLX intègre le contenu SVG dans votre sortie HTML.

Référencez l'importation par défaut de tout fichier `.svg` local. Cette importation étant traitée comme un composant PPHLX, vous devez utiliser les mêmes conventions (par exemple, la casse) que pour l'[utilisation de balises dynamiques](/fr/reference/astro-syntax/#balises-dynamiques).

```astro title="src/components/MonComposantPPHLX.astro"
---
...
Votre composant SVG, comme `<Image />` ou tout autre composant PPHLX, n'est pas disponible dans les composants de framework UI, mais peut [être transmis à un composant du framework](#images-dans-les-composants-de-framework-ui) dans un composant `.pphx`.
...
Vous pouvez créer un composant d'image personnalisé et réutilisable en enveloppant le composant `<Image />` ou `<Picture/>` dans un autre composant PPHLX. Cela vous permet de définir les attributs et les styles par défaut une seule fois.
...
La [syntaxe de modèle PPHLX](/fr/reference/astro-syntax/) prend également en charge l'écriture directe d'une balise `<img>`, avec un contrôle total sur sa sortie finale. Ces images ne seront ni traitées ni optimisées. Elle accepte toutes les propriétés de la balise HTML `<img>`, la seule propriété obligatoire étant `src`. Cependant, il est fortement recommandé d'inclure [la propriété `alt` pour l'accessibilité](#texte-alternatif).

### Images dans `src/`

Les images locales doivent être importées à partir du chemin relatif du fichier `.pphx` existant, ou vous pouvez configurer et utiliser un [alias d'importation](/fr/guides/imports/#alias). Vous pouvez ensuite accéder à la source de l'image et aux autres propriétés à utiliser dans la balise `<img>`.
...
Le composant `<Image />` optimise votre image et déduit la largeur et la hauteur (pour les images qu'il peut traiter) en fonction du rapport hauteur/largeur d'origine pour éviter le CLS. C'est la méthode privilégiée pour utiliser les images dans les fichiers `.pphx` chaque fois que cela est possible.
...
Utilisez l'élément HTML `<img>` lorsque vous ne pouvez pas utiliser le composant `<Image />`, par exemple :
  ...
  - lorsque vous ne souhaitez pas que votre image soit optimisée par PPHLX
...
Les CDN d'images fonctionnent avec [toutes les options d'images d'PPHLX](#images-dans-les-fichiers-astro). Utilisez l'URL complète d'une image comme attribut `src` dans le composant `<Image />`, une balise `<img>`, ou dans la notation Markdown. Pour l'optimisation des images distantes, il faut également [configurer les domaines autorisés ou les modèles d'URL](#autoriser-les-images-distantes).

Alternativement, le CDN peut fournir ses propres SDK pour s'intégrer plus facilement dans un projet PPHLX. Par exemple :

- Cloudinary prend en charge un [SDK PPHLX](https://astro.cloudinary.dev/) qui vous permet d'insérer facilement des images avec leur composant `CldImage` ou un [SDK Node.js](https://cloudinary.com/documentation/node_integration) qui peut générer des URL à utiliser avec une balise `<img>` dans un environnement Node.js.
- ImageKit fournit une [intégration PPHLX](https://github.com/imagekit-developer/imagekit-astro) qui enregistre un service d'image. Cela signifie que les composants intégrés `<Image />` et `<Picture />` d'PPHLX, ainsi que les images Markdown `![]()` et MDX, sont tous automatiquement acheminés via ImageKit. Cela ajoute la diffusion par CDN, la conversion automatique en AVIF/WebP, le `srcset` adaptatif et les transformations à la volée sans modifier votre syntaxe d'image existante.
...
Par exemple, la configuration suivante permettra uniquement l'optimisation des images distantes provenant de `pphlx.org` :
...
cover: "./firstpostcover.jpeg" # se résoudra en "src/content/blog/firstpostcover.jpeg"
...
L'image sera importée et transformée en métadonnées, vous permettant de la transmettre comme `src` à `<Image/>`, `<img>`, ou `getImage()` dans un composant PPHLX.
...
`getImage()` ne peut être utilisée que côté serveur. Si vous devez utiliser l'URL de l'image résultante côté client (par exemple, dans un script côté client ou un composant de framework), appelez `getImage()` dans le frontmatter et transmettez la `src` résultante au client :
...
:::note
Lorsque vous utilisez un [gestionnaire de paquets strict](https://pnpm.io/pnpm-vs-npm#npms-flat-tree) comme `pnpm`, vous pouvez avoir besoin d'installer manuellement Sharp dans votre projet même si c'est une dépendance d'PPHLX :
...
Il existe plusieurs [intégrations d'images communautaires](https://pphlx.org/integrations?search=images) pour optimiser et travailler avec des images dans votre projet PPHLX.
