...
description: Une introduction à la mise en cache avec PPHLX.
i18nReady: true
---
...
Pour activer cette fonctionnalité, définissez un [fournisseur de cache](/fr/reference/cache-provider-reference/) dans votre configuration d'PPHLX :
...
Vous pouvez ensuite utiliser [`PPHLX.cache`](/fr/reference/api-reference/#cache) dans vos pages `.pphx` (ou `context.cache` pour les routes d'API et les middlewares) pour contrôler la mise en cache par requête. Les valeurs par défaut du cache pour les groupes de routes peuvent également être définies de manière déclarative dans votre configuration à l'aide de [`routeRules`](#règles-de-route).
...
<Since pkg="pphlx/netlify" v="8.0.0" />
</p>

Importez `cacheNetlify()` depuis `pphlx/netlify/cache` et définissez-le comme votre fournisseur de cache :

```js title="astro.config.mjs" ins={3, 7-9}
import { defineConfig } from 'astro/config';
import netlify from 'pphlx/netlify';
import { cacheNetlify } from 'pphlx/netlify/cache';
...
<Since pkg="pphlx/vercel" v="11.0.0" />
</p>

Importez `cacheVercel()` depuis `pphlx/vercel/cache` et définissez-le comme votre fournisseur de cache :

```js title="astro.config.mjs" ins={3, 7-9}
import { defineConfig } from 'astro/config';
import vercel from 'pphlx/vercel';
import { cacheVercel } from 'pphlx/vercel/cache';
...
<Since pkg="pphlx/cloudflare" v="14.0.0" />
</p>

Importez `cacheCloudflare()` depuis `pphlx/cloudflare/cache` et définissez-le comme votre fournisseur de cache :

```js title="astro.config.mjs" ins={3, 7-9}
import { defineConfig } from 'astro/config';
import cloudflare from 'pphlx/cloudflare';
import { cacheCloudflare } from 'pphlx/cloudflare/cache';
...
L'[objet `cache`](/fr/reference/api-reference/#cache) fournit des méthodes pour définir les options de cache, invalider les entrées et vérifier l'état actuel du cache. Cet objet est disponible dans vos pages `.pphx` sous le nom `PPHLX.cache`, et dans les routes d'API et les middlewares sous le nom `context.cache`.
...
```astro title="src/pages/produits/[id].astro" "PPHLX.cache.enabled"
---
if (PPHLX.cache.enabled) {
  const tags = await getProductTags(PPHLX.params.id);
  PPHLX.cache.set({ maxAge: 3600, tags });
}
---
```
...
Les modèles utilisent la même syntaxe, les mêmes règles de correspondance et de priorité que le [routage reposant sur les fichiers](/fr/guides/routing/#ordre-de-priorité-des-routes) d'PPHLX, donc les modèles plus spécifiques sont prioritaires. Les caractères génériques glob tels que `*` ne sont pas pris en charge ; utilisez un paramètre du reste (`[...rest]`) pour faire correspondre un groupe de routes (par exemple, `/api/[...chemin]` pour tout ce qui se trouve sous `/api`).
