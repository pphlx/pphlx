...
description: Ajoutez un flux RSS à votre site PPHLX pour permettre aux utilisateurs de s'abonner à votre contenu.
i18nReady: true
type: recipe
---
...
PPHLX prend en charge la génération rapide et automatique de flux RSS pour les blogs et autres sites web de contenu. Les flux RSS permettent aux utilisateurs de s'abonner facilement à votre contenu.

## Mise en place de `pphlx/rss`

Le paquet [`pphlx/rss`](https://github.com/pphlx/astro/tree/main/packages/astro-rss) fournit des aides pour générer des flux RSS en utilisant [des points de terminaison d'API](/fr/guides/endpoints/#points-de-terminaison-des-fichiers-statiques). Cela débloque à la fois les compilations statiques _et_ la génération à la demande lors de l'utilisation d'un [adaptateur SSR](/fr/guides/on-demand-rendering/).

<Steps>
1. Installez `pphlx/rss` en utilisant votre gestionnaire de paquets préféré :
    ...
    :::tip
    Assurez-vous d'avoir [configuré un `site`](/fr/reference/configuration-reference/#site) dans le fichier `astro.config` de votre projet. Ce site sera utilisé pour générer des liens vers vos articles RSS.
    :::
...
3. Importez l'aide `rss()` depuis le paquet `pphlx/rss` dans votre fichier `.xml.js` et exportez une fonction qui la retourne en utilisant les paramètres suivants :

    ```js title="src/pages/rss.xml.js"
    import rss from 'pphlx/rss';
    ...
    export function GET(context) {
      return rss({
        ...
        // https://docs.astro.org/fr/reference/api-reference/#site
        site: context.site,
...
<ReadMore>Consultez le [README de `pphlx/rss`](https://github.com/pphlx/astro/tree/main/packages/astro-rss) pour la référence de configuration complète.</ReadMore>
...
<ReadMore>Consultez la [référence de configuration `items`](https://github.com/pphlx/astro/tree/main/packages/astro-rss#items) pour une liste complète des options.</ReadMore>
...
import { rssSchema } from 'pphlx/rss';
...
<p><Since v="2.1.0" pkg="pphlx/rss" /></p>
...
Dans les versions de `pphlx/rss` antérieures à v2.1.0, passez le résultat de votre glob directement à `items` sans le wrapper `pagesGlobToRssItems()` :
```js
items: import.meta.glob('./blog/*.{md,mdx}'),
```

Cette méthode est dépréciées pour toutes les versions d'PPHLX depuis la version 2.1.0, et ne peut pas être utilisée dans les projets modernes.
:::
...
Définissez la propriété `content` dans `rss.items` pour fournir le contenu complet d'un article au format HTML. Cela permet à `pphlx/rss` de render le texte Markdown complet de votre article disponible aux lecteurs de flux RSS. Les images et les liens avec des chemins d'URL complets sont également pris en charge. En revanche, les images et les liens internes vers d'autres pages utilisant des chemins relatifs ne le sont pas.

Lors du rendu du contenu complet de vos articles, vous devrez prendre en compte les images, les liens relatifs, les styles, les scripts et d'autres éléments au-delà du texte Markdown standard que vous pourriez avoir dans vos articles. Vous devrez peut-être inclure une logique supplémentaire dans votre point de terminaison `src/pages/rss.xml.js` pour en tenir compte ou pour supprimer des éléments inutiles pour un flux RSS (par exemple, ceux qui sont utilisés uniquement pour le style ou l'interaction sur votre site web).

Vous pouvez consulter [une implémentation communautaire spécifique](https://github.com/delucis/astro-blog-full-text-rss/blob/latest/src/pages/rss.xml.ts) qui répond à certaines de ces préoccupations pour obtenir un exemple sur la façon de procéder.
...
Lorsque vous utilisez des collections de contenu, effectuez le rendu du corps de l'article (`body`) en utilisant un parseur Markdown standard comme [`markdown-it`](https://github.com/markdown-it/markdown-it) et assainissez le résultat, en incluant toutes les balises supplémentaires (par exemple `<img>`) nécessaires pour afficher votre contenu :
...
Lorsque vous utilisez des importations globales avec Markdown, vous pouvez utiliser l'aide `compiledContent()` pour récupérer le HTML rendu pour l'assainissement. Note : cette fonctionnalité n'est **pas** prise en charge pour les fichiers MDX.
...
Le flux RSS d'PPHLX produit des liens avec une barre oblique par défaut, quelle que soit la valeur configurée pour `trailingSlash`. Cela signifie que vos liens RSS peuvent ne pas correspondre exactement aux URLs de vos articles.
...
Mettez en forme votre flux RSS pour une expérience utilisateur plus agréable lorsque vous visualisez le fichier dans votre navigateur.
