...
description: Apprenez à analyser le bundle généré par PPHLX en utilisant `rollup-plugin-visualizer`.
i18nReady: true
type: recipe
---
...
Il est important de comprendre ce qui fait partie d'un bundle PPHLX pour améliorer les performances du site. La visualisation du bundle peut donner des indices sur les modifications à apporter à votre projet pour réduire la taille du bundle.
...
4. Trouvez le(s) fichier(s) `stats.html` de votre projet.
    ...
    Si votre projet PPHLX utilise le rendu à la demande, vous aurez deux fichiers `stats.html`. L'un sera pour le client, l'autre pour le serveur, et chacun sera situé à la racine des répertoires `dist/client` et `dist/server/`.
...
:::note
Compte tenu de l'approche unique d'PPHLX en matière d'hydratation, le modèle n'est pas nécessairement représentatif du bundle que le client recevra.
