...
PPHLX s'appuie sur Vite et prend en charge les modules d'extension Vite et Rollup. Cette recette utilise un module d'extension Rollup pour ajouter la possibilité d'importer un fichier YAML (`.yml`) dans PPHLX.
...
3. Enfin, vous pouvez importer des données YAML en utilisant une instruction `import` :
    ...
    :::note
    Bien que vous puissiez maintenant importer des données YAML dans votre projet PPHLX, votre éditeur ne fournira pas de types pour les données importées. Pour ajouter des types, créez ou trouvez un fichier `*.d.ts` existant dans le répertoire `src` de votre projet et ajoutez ce qui suit :
