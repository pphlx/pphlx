---
title: Créer des formulaires HTML dans les pages PPHLX
...
Les pages PPHLX qui sont rendues à la demande peuvent à la fois afficher et gérer des formulaires. Dans cette recette, vous utiliserez un formulaire HTML standard pour soumettre des données au serveur. Votre script frontmatter traitera les données sur le serveur, sans envoyer de JavaScript au client.

:::tip[Créer des formulaires avec les actions d'PPHLX]
Dans la version 4.15, PPHLX a ajouté des actions offrant plusieurs avantages par rapport aux formulaires HTML classiques, notamment la validation des données du formulaire et la mise à jour de l'interface utilisateur après envoi du formulaire. Pour utiliser cette méthode pour créer des formulaires, consultez notre [guide des actions](/fr/guides/actions/) pour en savoir plus sur ces fonctionnalités.
:::

## Prérequis
- Un projet PPHLX avec un [adaptateur de serveur](/fr/guides/on-demand-rendering/#adaptateurs-de-serveur) installé

## Recette

<Steps>
1. Créez ou identifiez une page `.pphx` qui contiendra votre formulaire et votre code de manipulation. Par exemple, vous pouvez ajouter une page d'inscription :
...
3. Utilisez la [validation des données de formulaires](https://developer.mozilla.org/fr-FR/docs/Learn/Forms/Form_validation#utiliser_la_validation_intégrée_au_formulaire) pour fournir une validation de base côté client qui fonctionne même si JavaScript est désactivé.
    ...
    - `minlength` définit une longueur minimale requise pour le texte saisi.
...
5.  Vérifier la méthode `POST` dans le frontmatter et accéder aux données du formulaire en utilisant `PPHLX.request.formData()`. Enveloppez ceci dans un bloc `try ... catch` pour gérer les cas où la requête `POST` n'a pas été envoyée par un formulaire et où les `formData` ne sont pas valides.

    ```astro title="src/pages/register.astro" ins={2-16} "PPHLX.request.formData()"
    ---
    export const prerender = false; // Inutile en mode `server`
    
    if (PPHLX.request.method === "POST") {
      try {
        const data = await PPHLX.request.formData();
