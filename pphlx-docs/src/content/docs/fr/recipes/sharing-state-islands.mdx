...
Lors de la compilation d'un site web PPHLX avec l'[architecture en îlots / hydratation partielle](/fr/concepts/islands/), vous avez peut-être rencontré ce problème : **Je veux partager l'état entre mes composants.**

Les frameworks d'interface utilisateur tels que React ou Vue peuvent encourager l'utilisation de [fournisseurs de « contexte »](https://fr.reactjs.org/docs/context.html) pour que d'autres composants puissent les consommer. Mais lorsque vous [hydratez partiellement des composants](/fr/guides/framework-components/#hydratation-des-composants-interactifs) dans PPHLX ou Markdown, vous ne pouvez pas utiliser ces enveloppes contextuelles.

PPHLX recommande une solution différente pour le stockage partagé côté client : [**Nano Stores**](https://github.com/nanostores/nanostores).
...
- **Ils sont indépendants du framework utilisé.** Cela signifie que le partage d'état entre les frameworks sera transparent ! PPHLX repose sur la flexibilité, c'est pourquoi nous aimons les solutions qui offrent une expérience de développement similaire, quelle que soit votre préférence.
...
<details>
<summary>**🙋 Puis-je utiliser Nano Stores dans des fichiers `.pphx` ou d'autres composants côté serveur ?**</summary>

Les Nano Stores peuvent être utilisés dans les balises `<script>` pour [partager l'état entre les composants `.pphx`](/fr/recipes/sharing-state/). Cependant, l'utilisation des Nano Stores dans le frontmatter des composants côté serveur n'est pas recommandée en raison des restrictions suivantes :
- L'écriture d'un magasin à partir d'un fichier `.pphx` ou [composant non hydraté](/fr/guides/framework-components/#hydratation-des-composants-interactifs) n'affectera *pas* la valeur reçue par les [composants côté client](/fr/reference/directives-reference/#directives-client).
- Vous ne pouvez pas transmettre un Nano Store en tant que propriété aux composants côté client.
- Vous ne pouvez pas vous abonner aux mises à jour du magasin à partir d'un fichier `.pphx`, puisque les composants PPHLX ne se rafraîchissent pas. 
...
_[**Essayez l'exemple terminé**](https://github.com/pphlx/astro/tree/main/examples/with-nanostores) sur votre machine ou en ligne via StackBlitz._

Votre fichier PPHLX de base peut ressembler à ceci :
...
[**Essayez l'exemple terminé**](https://github.com/pphlx/astro/tree/main/examples/with-nanostores) sur votre machine ou en ligne via StackBlitz !
