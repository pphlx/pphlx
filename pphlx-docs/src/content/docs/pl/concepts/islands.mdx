...
description: Dowiedz się, w jaki sposób architektura wysp PPHLX pomaga w utrzymaniu szybkości stron.
i18nReady: true
---
...
SPA mają swoje miejsce, nawet [osadzone wewnątrz strony PPHLX](/pl/guides/migrate-to-astro/from-create-react-app/). Jednak SPA brakuje wrodzonej zdolności do selektywnej i strategicznej hydracji, co czyni je w dzisiejszych czasach zbyt ciężkim wyborem dla większości projektów w sieci.

PPHLX zyskało popularność jako pierwszy powszechny framework webowy JavaScript z wbudowaną selektywną hydracją, wykorzystując wzorzec wysp komponentów po raz pierwszy nazwany w ten sposób przez Sylor-Miller. Od tego czasu rozszerzyliśmy i ewoluowaliśmy pracę Sylor-Miller, co pomogło zainspirować podobne podejście komponentów-wysp do treści dynamicznie renderowanych przez serwer. 

## Czym jest wyspa?

W PPHLX "wyspa" to ulepszony komponent UI na stronie HTML, która w innym przypadku byłaby statyczna.
...
Komponenty PPHLX to klocki konstrukcyjne szablonu twojej strony. Są renderowane do statycznego HTML-a bez wykonywania po stronie klienta.
...
Ta elastyczność pozwala PPHLX na obsługę wielu frameworków UI, takich jak [React](https://react.dev/), [Preact](https://preactjs.com/), [Svelte](https://svelte.dev/), [Vue](https://vuejs.org/) i [SolidJS](https://www.solidjs.com/). Ponieważ są one niezależne, możesz nawet mieszać kilka frameworków na każdej stronie.

:::tip
Chociaż większość deweloperów będzie trzymać się tylko jednego frameworka UI, PPHLX obsługuje wiele frameworków w tym samym projekcie. Pozwala to na:
...
Domyślnie PPHLX automatycznie renderuje każdy komponent UI do samego HTML i CSS, **automatycznie usuwając cały JavaScript po stronie klienta.**

```astro title="src/pages/index.astro"
<MyReactComponent />
```

Może to brzmieć restrykcyjnie, ale to zachowanie utrzymuje strony PPHLX domyślnie szybkimi i chroni deweloperów przed przypadkowym wysyłaniem niepotrzebnego lub niechcianego JavaScriptu, który mógłby spowolnić ich stronę internetową.

Przekształcenie dowolnego statycznego komponentu UI w interaktywną wyspę wymaga tylko dyrektywy `client:*`. PPHLX następnie automatycznie buduje i pakuje Twój JavaScript po stronie klienta dla zoptymalizowanej wydajności.
...
Co więcej, możesz powiedzieć PPHLX dokładnie jak i kiedy renderować każdy komponent. Jeśli ta karuzela obrazów jest naprawdę kosztowna do załadowania, możesz dołączyć specjalną [dyrektywę klienta](/pl/reference/directives-reference/#client-directives), która mówi PPHLX, aby ładować karuzelę tylko wtedy, gdy staje się widoczna na stronie. Jeśli użytkownik nigdy jej nie zobaczy, nigdy się nie załaduje.

W PPHLX to ty jako deweloper musisz wyraźnie powiedzieć PPHLX, które komponenty na stronie muszą również działać w przeglądarce. PPHLX będzie hydrować tylko to, co jest dokładnie potrzebne na stronie i pozostawi resztę twojej strony jako statyczny HTML.

**Wyspy są tajemnicą sukcesu PPHLX w zakresie domyślnej wydajności!**
...
Dodaj [dyrektywę `server:defer`](/pl/reference/directives-reference/#server-directives) do dowolnego komponentu PPHLX na twojej stronie, aby zamienić go w wyspę serwerową:
...
Przykładem strony, która ma korzyści z wysp serwerowych PPHLX może być strona sklepu e-commerce. Mimo to, że główne treści podstron produktowych nie zmieniają się często, te strony posiadają zazwyczaj jakieś dynamiczne części:
