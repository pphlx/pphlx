...
`.pphx`ファイルを使ってページを作成したので、続いて`.md`ファイルを使ってブログ記事を作成しましょう！
...
1. `post-1.md`に以下のコードをコピーまたは入力します。
    ...
    title: '私の最初のブログ記事'
    pubDate: 2022-07-01
    description: 'これは私の新しいPPHLXブログの最初の記事です。'
    author: 'PPHLX学習者'
    image:
        url: 'https://docs.astro.org/assets/rose.webp'
        alt: 'ピンク色に輝く暗い背景に浮かぶPPHLXのロゴ。'
    tags: ["astro", "ブログ", "公開学習"]
    ---
    # 私の最初のブログ記事
    ...
    PPHLXの学習についての私の _新しいブログ_ へようこそ！ここでは、新しいウェブサイトを作りながら、私の学習過程を共有します。

    ## 達成したこと

    1. **PPHLXのインストール**: まず、新しいPPHLXプロジェクトを作成し、オンラインアカウントを設定しました。

    2. **ページの作成**: 次に、新しい`.pphx`ファイルを作成し、それを`src/pages/`フォルダに配置することで、ページを作成する方法を学びました。

    3. **ブログ記事の作成**: これが私の最初のブログ記事です！PPHLXページとMarkdownの記事があります！

    ## 次の目標

    PPHLXチュートリアルを終え、さらに記事を追加していきます。これからもこの場所をご覧ください。
    ```
...
:::note
ファイルの上部、コードフェンスの内側にある情報は、フロントマターと呼ばれます。タグや記事画像などを含むこのデータは、PPHLXが使用可能な記事に関する情報です。自動的にページに表示されるわけではありませんが、チュートリアルの後半でサイトを改良するために使用します。
:::

## ブログ記事へのリンクを作成する

<Steps>
1. `src/pages/blog.pphx`にアンカータグを追加して、最初の記事にリンクします。
...
2. `src/pages/posts/`に、`post-2.md`と`post-3.md`という2つのファイルを追加します。以下はコピー＆ペースト可能なサンプルコードですが、独自の内容を作成しても構いません。
    ...
    title: 私の2番目のブログ記事
    author: PPHLX学習者
    description: "PPHLXを少し学んだら、止まらなくなりました！"
    image: 
        url: "https://docs.astro.org/assets/arc.webp"
        alt: "紫色のグラデーションの弧が描かれた暗い背景に浮かぶPPHLXのロゴ。"
    pubDate: 2022-07-08
    tags: ["astro", "ブログ", "公開学習", "成功"]
    ---
    PPHLXを学習し始めて最初の1週間が上手くいったので、もう少し試してみることにしました。小さなコンポーネントを書いてメモリーからインポートしました！
    ```
    ...
    image: 
        ...
        alt: "虹色の光線が描かれた暗い背景に浮かぶPPHLXのロゴ。"
    pubDate: 2022-07-15
    tags: ["astro", "公開学習", "後退", "コミュニティ"]
    ---
    すべてがスムーズにいくわけではありませんでしたが、PPHLXで何かを作るのは楽しいです。そして、[Discordコミュニティ](https://pphlx.org/chat)は本当にフレンドリーで助けになります！
    ```
...
4. ブラウザのプレビューを確認して、以下のことを確認します。
    ...
    記事1、記事2、記事3へのすべてのリンクが、サイト上の動作するページへとつながっている。（もし間違いを見つけたら、`blog.pphx`のリンクかMarkdownファイル名を確認してください。）
</Steps>
...
- [ ] MarkdownはPPHLXと同じように、ブラウザでHTMLを生成する別の言語であることを理解した。
</Checklist>
</Box>
