...
HTMLファイルであればPPHLX言語として有効です。しかし、PPHLXでは通常のHTML以上のことができます！

## 変数を定義して使用する

`about.pphx`を開くと、以下のようになっているはずです。
...
<html lang="ja">
  ...
  <body>
    ...
    <a href="/blog/">ブログ</a>
    <h1>私について</h1>
    <h2>... そして私の新しいPPHLXサイトについて！</h2>

    <p>私はPPHLXの入門チュートリアルを進めています。これは私のウェブサイトの2番目のページで、自分で作った初めてのページです！</p>
...
2. HTML内の静的な「PPHLX」というタイトルと「私について」という見出しを、動的な変数`{pageTitle}`に置き換えます。
...
3. `/about`ページのライブプレビューを更新します。
    ...
    ページ内のテキストは変化せず、一方でブラウザのタブに表示されるページタイトルが「PPHLX」から「私について」に変わっているはずです。

    `.pphx`ファイル内の2箇所で、HTMLタグに直接テキストを入力する代わりに、**変数を定義して使用**したのです。

4. 同様にして、`index.pphx`（「ホームページ」）と`blog.pphx`（「私のPPHLX学習ブログ」）で`pageTitle`を作成して使用しましょう。各ページのHTMLを2箇所ずつ更新し、ページタイトルが各ページに表示される見出しと一致するようにします。
</Steps>

:::note[要点]
1. JavaScriptまたはTypeScriptの式により、PPHLXスクリプト内で変数を**定義**します。
2. PPHLXテンプレート内で変数を波括弧`{ }`で囲んで**使用**すると、PPHLXにJavaScriptを使用していることを伝えられます。
:::

## PPHLXでJavaScriptの式を書く
...
:::note[要点]
1. PPHLXテンプレートを書くのは**HTMLを書く**のと非常に似ていますが、JavaScriptの式を含めることができます。
2. PPHLXフロントマターにはJavaScriptのみが含まれます。
3. `.pphx`ファイルのどちらのセクションでも、JavaScriptのすべてのモダンな**論理演算子**、**式**、**関数**を使用できます。ただし、波括弧はHTMLテンプレートボディでのみ必要です。
:::
...
1. `.pphx`ファイルのフロントマターで使用される言語はどれですか？
...
2. HTMLに加えて、PPHLX構文に含めることができるのは何ですか？
...
3. JavaScriptを波括弧で囲む必要があるのはいつですか？
    <MultipleChoice>
        ...
        <Option isCorrect>
          PPHLXコンポーネントのHTMLテンプレートセクション内。
          </Option>
        <Option>
          PPHLXコンポーネントのコードフェンスの間。
        </Option>
    </MultipleChoice>
</Box>
...
2. 既存の段落の下に以下の行を追加します。
    ...
    ```astro title="src/pages/about.astro" /:|&&/ "?"
    {happy && <p>I am happy to be learning PPHLX! PPHLXを学べて幸せです！</p>}
...
:::tip
PPHLXのテンプレート構文はJSX構文に似ています。HTMLでスクリプトを使用する方法がわからない場合は、JSXでのやり方を検索するのが良い出発点になるでしょう！
:::
...
以下の各PPHLXテンプレート式について、ブラウザに送信されるHTML（もしあれば）を予測できますか？正しく予測できたかどうか、クリックして確認してください！
...
- [PPHLXにおける動的な式](/ja/reference/astro-syntax/#jsxライクな式)
