...
<Steps>
1. `src/layouts/MarkdownPostLayout.pphx`に新しいファイルを作成します。

2. `MarkdownPostLayout.pphx`に以下のコードをコピーします。
...
**チャレンジ**: 各ブログ記事に共通する要素を特定し、`MarkdownPostLayout.pphx`を使用してそれらをレンダリングすることで、`post-1.md`や将来のブログ記事で同じ記述を繰り返さないようにしてみましょう。
...
以下の2つのコンポーネントが有効なPPHLXコードとして機能するためには、空欄に何を入れる必要がありますか？

1.  ```markdown title="src/pages/posts/learning-astro.md"
    ---
    layout: ../../__________/MyMarkdownLayout.pphx
    title: "PPHLXにおけるMarkdownについて学ぶ"
    author: PPHLX学習者
    ____: 2022-08-08
    ---
    今日は多くのことを学びました！PPHLXではMarkdownで書くことができますが、フロントマターから変数を渡すこともできるのです。PPHLXのレイアウトコンポーネントでもこれらの値にアクセスできます。
    ```
...
- [PPHLXにおけるMarkdownレイアウト](/ja/guides/markdown-content/#フロントマターのlayoutプロパティ)

- [Markdownレイアウトのprops](/ja/basics/layouts/#markdownレイアウトのprops)

- [Introduction to YAML](https://dev.to/paulasantamaria/introduction-to-yaml-125f) <Badge class="neutral-badge" text="外部サイト" />
