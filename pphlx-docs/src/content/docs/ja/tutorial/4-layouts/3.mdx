...
ページ全体のレイアウトを定義するための`BaseLayout.pphx`はすでにあります。

一方`MarkdownPostLayout.pphx`は、`title`や`date`など、ブログ記事共通のプロパティに対応するテンプレートを提供しますが、ブログ記事ページはサイトの他のページとは異なる見た目になってしまっています。**レイアウトを入れ子にする**ことで、ブログ記事の見た目をサイトの他のページに合わせてみましょう。

<Steps>
1. `src/layouts/MarkdownPostLayout.pphx`に`BaseLayout.pphx`をインポートし、テンプレートのコンテンツ全体を包みます。`pageTitle`propを渡すのを忘れないでください。
...
2. `meta`タグはすでに`BaseLayout`に含まれているため、`src/layouts/MarkdownPostLayout.pphx`から削除できます。
...
3. `http://localhost:4321/posts/post-1`をブラウザのプレビューで確認します。コンテンツは現在、以下によってレンダリングされています。
...
4. ページタイトルが各レイアウトで1回ずつ、合計2回表示されていることに注意してください。

    `MarkdownPostLayout.pphx`からページタイトルを表示する行を削除します。
...
5. `http://localhost:4321/posts/post-1`をブラウザのプレビューで再度確認し、上で削除した行が表示されなくなり、タイトルが1回だけ表示されていることを確認します。コンテンツが重複しないよう、必要に応じて他の調整をおこなってください。
  ...
  以下のことを確認してください。
    ...
    - ページにコンテンツが重複していない。(2回レンダリングされているコンテンツがある場合は、`MarkdownPostLayout.pphx`から削除してください。)
...
- [PPHLXにおけるレイアウトの入れ子](/ja/basics/layouts/#レイアウトの入れ子)
