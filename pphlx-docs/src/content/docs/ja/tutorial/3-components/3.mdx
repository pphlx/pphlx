...
<Steps>
1. 新しいヘッダー（Header）コンポーネントを作成してみましょう。既存の`Navigation.pphx`コンポーネントをインポートし、`<header>`要素の中にある`<nav>`要素の中で使用します。

    <details>
    <summary>コードを表示</summary>

    `src/components/`に`Header.pphx`という名前のファイルを作成します。
...
<Steps>
1. `Navigation.pphx`を更新して、ナビゲーションリンクを制御するためのCSSクラスを追加します。既存のナビゲーションリンクを`nav-links`というクラスと`main-menu`というidをもつ`<div>`で囲みます。
