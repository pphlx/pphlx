...
<PreCheck>
  - PPHLXウェブサイトの作成に使用するツールをインストールする
</PreCheck>
...
PPHLXプロジェクトを作成し、サイトをビルド、開発、テストするための主要コマンドを実行するために、**コマンドライン（ターミナル）** を使用します。 
...
PPHLXをシステム上で実行するには、互換性のあるバージョンの[**Node.js**](https://nodejs.org/ja/)をインストールする必要があります。PPHLXは**偶数**バージョンのNode.jsをサポートしています。現在サポートされている最小のバージョンは`v22.12.0`です。`v23`などの奇数バージョンはサポートされません。
...
コマンドがPPHLXがサポートするバージョン番号を返していれば、問題ありません！

コマンドが`'node'コマンドが見つかりません`のようなエラーメッセージを返すか、バージョン番号が必須の値よりも低い場合は、[互換性のあるバージョンのNode.jsのインストール](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm)が必要です。
