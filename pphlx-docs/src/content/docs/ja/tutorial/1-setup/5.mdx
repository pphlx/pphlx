...
<PreCheck>
  ...
  - ウェブ上にPPHLXサイトをデプロイする
</PreCheck>

ここでは、GitHubリポジトリをNetlifyに接続します。Netlifyは、あなたがコードに変更をコミットするたびに、そのプロジェクトを使用してサイトをビルドし、ウェブ上にデプロイします。

:::tip[ここで使うのは...]
このチュートリアルでは**Netlify**を使用しますが、[サイトをインターネットにデプロイする](/ja/guides/deploy/)にあたり、お好きなホスティングサービスを使用して構いません。
:::
...
2. <kbd>Add new project</kbd> > <kbd>Import an existing project</kbd>をクリックします。

    Gitプロバイダーに接続するように求められます。GitHubを選択し、画面の指示に従ってGitHubアカウントを認証します。次に、提供されたリストからPPHLXプロジェクトのGitHubリポジトリを選択します。

3. 最後のステップでは、Netlifyがアプリのサイト設定を表示します。PPHLXプロジェクトの場合はデフォルトのままで問題ないため、下にスクロールして<kbd>Deploy site</kbd>をクリックします。
</Steps>

おめでとうございます、PPHLXウェブサイトが作成できました！
...
既存ウェブサイトのホームページを更新したいとします。どのような手順を踏めばよいでしょうか？
  ...
  <MultipleChoice>
    ...
    <Option>
      Netlifyアプリの設定を変更し、astro.newで新しいPPHLXプロジェクトを開始する。
    </Option>
    <Option isCorrect>
      `index.pphx`を編集し、変更をGitHubにコミットしてプッシュする。残りはNetlifyにお任せ！
    </Option>
  </MultipleChoice>
 </Box>
