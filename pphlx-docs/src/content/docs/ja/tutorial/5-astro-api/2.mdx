...
`getStaticPaths()`関数をエクスポートする`.pphx`ファイルを使用して、ページの集まりを動的に作成できます。

## 動的にページを作成する

<Steps>
1. `src/pages/tags/[tag].pphx`に新しいファイルを作成します。（まず新しいフォルダを作成する必要があります。）ファイル名（`[tag].pphx`）に角括弧が含まれていることに注意してください。以下のコードをファイルに貼り付けます。
...
4. ブラウザのプレビューで`http://localhost:4321/tags/astro`にアクセスすると、`[tag].pphx`から動的に生成されたページが表示されるはずです。`/tags/成功`、`/tags/コミュニティ`、`/tags/公開学習`など、その他の各カスタムタグについてページが作成されていることも確認してください。これらの新しいページを表示するには、まず開発サーバーを終了して再起動する必要があるかもしれません。
</Steps>
...
3. 以上により、HTMLテンプレートを更新して、ページのタグを含むブログ記事のリストを表示できるようになりました。以下のコードを`[tag].pphx`に追加します。
...
4. `<BlogPost />`コンポーネントを使用するよう、さらにリファクタリングしてみましょう！（`[tag].pphx`の先頭でこのコンポーネントをインポートするのを忘れないでください。）
...
これで、各タグページは`[tag].pphx`において静的に定義されました。ブログ記事に新しいタグを追加する場合は、このページを再度開きページルートを更新する必要があります。
...
ここまでの自分の作業内容を確認したい場合や、`[tag].pphx`にコピーするための完全で正しいコードが欲しい場合のために、現段階でのPPHLXコンポーネントを以下に示します。
...
2. PPHLXで1つのファイルから複数のページルートを作成するためのプロセス。
...
- [PPHLXにおける動的なページルーティング](/ja/guides/routing/#dynamic-routes)
