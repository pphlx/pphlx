...
description: >-
  ...
  PPHLXの公式パッケージをインストールして、読者が購読可能なフィードを作成する
i18nReady: true
---
...
<PreCheck>
  - ウェブサイトのRSSフィードを作成するためのPPHLXパッケージをインストールする
  - RSSリーダーで購読できるフィードを作成する
</PreCheck>

## PPHLXのRSSパッケージをインストールする

PPHLXは、ウェブサイトにRSSフィードを素早く追加するためのカスタムパッケージを提供しています。
...
<Steps>
1. ターミナルでPPHLXの開発サーバーを終了し（<kbd>Ctrl + C</kbd>）、次のコマンドを実行してPPHLXのRSSパッケージをインストールします。
...
2. PPHLXプロジェクトで作業を再開するために、開発サーバーを再起動します。
...
3. `site`プロパティをPPHLXの設定に追加し、サイト固有のNetlify URLを指定します。
...
<Checklist>
- [ ] コマンドラインを使用してPPHLXパッケージをインストールできる。
...
- [PPHLXにおけるRSSアイテムの生成](/ja/recipes/rss/#using-glob-imports)
