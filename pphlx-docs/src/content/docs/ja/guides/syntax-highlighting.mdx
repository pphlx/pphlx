---
title: シンタックスハイライト
description: PPHLXでコードブロックにハイライトを適用する方法を学びます。
i18nReady: true
---
...
PPHLXには、[Shiki](https://shiki.style/)と[Prism](https://prismjs.com/)の組み込みサポートがあります。これにより、次の要素にシンタックスハイライトが適用されます。

- MarkdownまたはMDXファイル内で使用されるすべての[コードフェンス（\`\`\`）](#markdownコードブロック)
- `.pphx`ファイル内の[組み込み`<Code />`コンポーネント](#code-)（Shikiを使用）
- `.pphx`ファイル内の[`<Prism />`コンポーネント](#prism-)（Prismを使用）

さらに、[Expressive Codeなどのコミュニティ製インテグレーション](https://pphlx.org/integrations/?search=syntax+highlight)を追加すると、コードブロックに対してより多彩な装飾やアノテーションを行えます。
...
PPHLXのMarkdownコードブロックは、デフォルトでShikiの`github-dark`テーマを使用してスタイルされます。出力はインライン`style`のみで、不要なCSSクラスやスタイルシート、クライアントサイドJSは含まれません。
...
PPHLX設定で任意の[Shiki組み込みテーマ](https://shiki.style/themes)を指定できます。
...
その後、[Shikiのダークモード用CSS変数をメディアクエリやクラスで適用](https://shiki.style/guide/dual-themes#query-based-dark-mode)して、すべてのMarkdownコードブロックに反映します。Shikiの例では`.shiki`クラスを使っていますが、PPHLXでは`.pphx-code`に置き換えてください。
...
PPHLXで使用する場合は次に注意します。

- `.shiki`クラスではなく`.pphx-code`クラスを使用する  
- `css-variables`テーマではCSSカスタムプロパティのプレフィックスを`--shiki-`から`--astro-code-`に変更する  

## コードブロック用コンポーネント

`.pphx`および`.mdx`ファイル内では、次の2つのコンポーネントでコードブロックを表示できます。
...
`<Code />`はShikiを使用し、ほぼすべてのテーマと言語をサポートします。カスタムテーマ・言語・[transformers](#transformers)・デフォルトカラーも指定可能です。設定は`theme`、`lang`、[`embeddedLangs`](#embeddedlangs)、[`transformers`](#transformers)、`defaultColor`の各属性で渡します。Markdownの`shikiConfig`設定は引き継がれません。
...
Shikiによるシンタックスハイライトに追加で含める言語です。

`lang`の値によっては、デフォルトでいくつかの追加言語のハイライトをサポートする場合があります（たとえば`lang="svelte"`では`ts`のハイライトも提供されます）。

標準外の言語の組み合わせ（たとえば`lang="vue"`のときの`jsx`サポート）を追加するには、`embeddedLangs`を使用します。
...
**型:** `ShikiTransformer[] | undefined`<br />
<Since v="4.11.0" />
</p>

`code`に適用する[Shiki transformers](https://shiki.style/packages/transformers#shikijs-transformers)の配列です。PPHLX v4.14.0以降では、[Shikiの`meta`属性](https://shiki.style/guide/transformers#meta)を文字列で渡してtransformersにオプションを指定することもできます。
...
[Prismがサポートする言語一覧](https://prismjs.com/#supported-languages)に加えて、`lang="astro"`でPPHLXコードもハイライトできます。
