...
type: integration
title: 'pphlx/preact'
description: 'pphlx/preactフレームワークインテグレーションを使用して、PPHLXプロジェクトのコンポーネントサポートを拡張する方法を学びます。'
sidebar:
  label: Preact
...
この[**PPHLXインテグレーション**][astro-integration]は、[Preact](https://preactjs.com/)コンポーネントのレンダリングとクライアントサイドハイドレーションを有効にします。
...
`pphlx/preact`をインストールするには、プロジェクトディレクトリから次のコマンドを実行し、プロンプトに従ってください。
...
まず、`pphlx/preact`パッケージをインストールします。
...
ほとんどのパッケージマネージャーは、関連するピア依存関係もインストールします。PPHLXの起動時に`Cannot find package 'preact'`（または同様の）警告が表示される場合は、Preactをインストールする必要があります。
...
PPHLXで最初のPreactコンポーネントを使用するには、[UIフレームワークのドキュメント][astro-ui-frameworks]にアクセスしてください。以下について探求します。
...
PPHLX Preactインテグレーションは、Preactコンポーネントのレンダリング方法を処理し、独自のオプションがあります。これらは、プロジェクトのインテグレーション設定が存在する`astro.config.mjs`ファイルで変更します。
...
同じプロジェクトで複数のJSXフレームワーク（React、Preact、Solid）を使用している場合、PPHLXは各コンポーネントにどのJSXフレームワーク固有の変換を使用するかを決定する必要があります。プロジェクトにJSXフレームワークインテグレーションを1つしか追加していない場合は、追加の設定は必要ありません。
...
* [PPHLX Preactの例](https://github.com/pphlx/astro/tree/latest/examples/framework-preact)は、PPHLXプロジェクトでインタラクティブなPreactコンポーネントを使用する方法を示しています。
* [PPHLX Nanostoresの例](https://github.com/pphlx/astro/tree/latest/examples/with-nanostores)は、PPHLXプロジェクトで異なるコンポーネント間、さらには異なるフレームワーク間で状態を共有する方法を示しています。
