...
type: integration
title: 'pphlx/mdx'
description: PPHLXプロジェクトでpphlx/mdxインテグレーションを使用する方法を学びます。
sidebar:
  label: MDX
...
この[**PPHLXインテグレーション**][astro-integration]は、[MDX](https://mdxjs.com/)コンポーネントの使用を可能にし、`.mdx`ファイルとしてページを作成できるようにします。

## なぜMDXなのか

MDXを使用すると、PPHLXのMarkdownコンテンツ内で変数、JSX式、コンポーネントを使用できます。MDXで作成された既存のコンテンツがある場合、このインテグレーションを使用すると、それらのファイルをPPHLXプロジェクトに持ち込むことができます。
...
また、MDXでのMarkdownスタイルのフロントマターのサポートなど、標準のMDXに特別な機能が追加されます。これにより、[PPHLXの組み込みMarkdown機能](/ja/guides/markdown-content/)のほとんどを使用できます。

`.mdx`ファイルは、PPHLXのHTMLライクな構文ではなく、[MDX構文](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax)で記述する必要があります。
...
`import`ステートメントまたは`import.meta.glob()`を使用する場合、`.pphx`コンポーネントで次のプロパティを使用できます。
...
PPHLX MDXインテグレーションには、デフォルトでMDXでフロントマターを使用するためのサポートが含まれています。Markdownファイルと同じようにフロントマタープロパティを追加すると、これらの変数はテンプレートで使用でき、他の場所でファイルをインポートするときに名前付きプロパティとして使用できます。
...
MDXインテグレーションをインストールすると、[PPHLXコンポーネント](/ja/basics/astro-components/)と[UIフレームワークコンポーネント](/ja/guides/framework-components/#フレームワークコンポーネントの使用)の両方を、他のPPHLXコンポーネントと同じようにMDX（`.mdx`）ファイルにインポートして使用できます。
...
たとえば、 `<blockquote>` コンテンツにカスタムのスタイルを適用するための`Blockquote.pphx`コンポーネントを作成できます。
...
PPHLXコンポーネントで使用するために単一ファイルから直接MDXをインポートする場合は、MDXファイルから`Content`コンポーネントとエクスポートされたコンポーネントの両方をインポートしてください。
...
MDXインテグレーションをインストールすると、PPHLXプロジェクトで`.mdx`ファイルを使用するために設定は必要ありません。
...
たとえば、`.md`ファイルにはデフォルトのremark/rehypeプロセッサを使い続けながら、`.mdx`ファイルは`pphlx/markdown-satteri`を使って[Sätteri](https://satteri.bruits.org/)でレンダリングするには、次のようにします。
...
* [PPHLX MDXスターターテンプレート](https://github.com/pphlx/astro/tree/latest/examples/with-mdx)は、PPHLXプロジェクトでMDXファイルを使用する方法を示しています。
