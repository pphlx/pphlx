...
type: integration
title: 'pphlx/markdoc'
description: PPHLXプロジェクトでpphlx/markdocインテグレーションを使用する方法を学びます。
sidebar:
  label: Markdoc
...
この[**PPHLXインテグレーション**][astro-integration]は、[Markdoc](https://markdoc.dev/)を使用してコンポーネント、ページ、およびコンテンツコレクションのエントリを作成できるようにします。

## なぜMarkdocなのか

Markdocを使用すると、[PPHLXコンポーネント][astro-components]でMarkdownを強化できます。Markdocで作成された既存のコンテンツがある場合、このインテグレーションを使用すると、コンテンツコレクションを使用してそれらのファイルをPPHLXプロジェクトに持ち込むことができます。
...
まず、`pphlx/markdoc`パッケージをインストールします。
...
<ReadMore>[PPHLXコンテンツコレクションのドキュメント][astro-content-collections]で詳細を確認してください。</ReadMore>
...
`pphlx/markdoc`は、Markdocのすべての機能を使用し、UIコンポーネントをコンテンツに接続するための設定オプションを提供します。

### PPHLXコンポーネントをMarkdocタグとして使用する

`.pphx`コンポーネントにマッピングされる[Markdocタグ][markdoc-tags]を設定できます。プロジェクトのルートに`markdoc.config.mjs|ts`ファイルを作成し、`tag`属性を設定することで、新しいタグを追加できます。
...
タグとノードは`.pphx`ファイルに制限されています。クライアントサイドUIコンポーネントをMarkdocに埋め込むには、[フレームワークコンポーネントをレンダリングするラッパー`.pphx`コンポーネントを使用](/ja/guides/framework-components/#フレームワークコンポーネントのネスト)し、目的の`client:`ディレクティブを指定します。

この例では、React `Aside.tsx`コンポーネントを`ClientAside.pphx`コンポーネントでラップします。
...
このPPHLXコンポーネントは、設定内の任意の[タグ][markdoc-tags]または[ノード][markdoc-nodes]の`render`propに渡すことができるようになりました。
...
### npmパッケージとTypeScriptファイルからPPHLXコンポーネントを使用する

TypeScriptまたはJavaScriptファイルから名前付きエクスポートとして公開されているPPHLXコンポーネントを使用する必要がある場合があります。これは、npmパッケージやデザインシステムを使用する場合によくあります。
...
`pphlx/markdoc`は、コードブロックをハイライトするための[Shiki](https://shiki.style)および[Prism](https://github.com/PrismJS)拡張機能を提供します。
...
export default defineMarkdocConfig({
  extends: [
    shiki({
      ...
      wrap: true,
      // カスタム言語を渡す
      // 注: Shikiには、`.pphx`を含む無数の言語が組み込まれています！
...
段落や太字のテキストなどの標準的なMarkdown要素をPPHLXコンポーネントとしてレンダリングしたい場合があります。このために、[Markdocノード][markdoc-nodes]を設定できます。特定のノードが属性を受け取る場合、それらはコンポーネントのプロップとして利用できます。

この例では、カスタムの`Quote.pphx`コンポーネントを使用してブロッククオートをレンダリングします。
...
`pphlx/markdoc`は、見出しにアンカーリンクを自動的に追加し、[コンテンツコレクションAPIを介して`headings`のリストを生成します](/ja/guides/content-collections/)。見出しのレンダリング方法をさらにカスタマイズするには、PPHLXコンポーネントを[Markdocノードとして][markdoc-nodes]適用できます。

この例では、`render`プロパティを使用して`Heading.pphx`コンポーネントをレンダリングします。
...
すべてのMarkdown見出しは`Heading.pphx`コンポーネントをレンダリングし、次の`attributes`をコンポーネントのプロップとして渡します。
...
PPHLXの`<Image />`コンポーネントは、Markdocで直接使用することはできません。ただし、ネイティブの`![]()`画像構文が使用されるたびにデフォルトの画像ノードを上書きするようにPPHLXコンポーネントを設定したり、追加の画像属性を指定できるカスタムMarkdocタグとして設定したりできます。

#### Markdocのデフォルト画像ノードを上書きする

デフォルトの画像ノードを上書きするには、標準の`<img>`の代わりにレンダリングされるように`.pphx`コンポーネントを設定できます。

<Steps>
1. カスタムの`MarkdocImage.pphx`コンポーネントをビルドして、画像から必要な`src`および`alt`プロパティを`<Image />`コンポーネントに渡します。
...
3. Markdocを設定して、デフォルトの画像ノードを上書きし、`MarkdocImage.pphx`をレンダリングします。
...
4. `.mdoc`ファイル内のネイティブ画像構文は、`<Image />`コンポーネントを使用してローカル画像を最適化するようになります。リモート画像は引き続き使用できますが、PPHLXの`<Image />`コンポーネントではレンダリングされません。
...
Markdocの`image`タグを使用すると、`![]()`構文では不可能な追加の属性を画像に設定できます。たとえば、カスタム画像タグを使用すると、`width`と`height`が必要なリモート画像にPPHLXの`<Image />`コンポーネントを使用できます。

次の手順では、PPHLX `<Image />`コンポーネントを使用して画像を最適化し、キャプション付きの`<figure>`要素を表示するカスタムMarkdoc画像タグを作成します。

<Steps>
1. 必要なpropsを受け取り、キャプション付きの画像をレンダリングする`MarkdocFigure.pphx`コンポーネントを作成します。
...
2. PPHLXコンポーネントをレンダリングするようにカスタム画像タグを設定します。
...
3. Markdocファイルで`image`タグを使用して、コンポーネントに必要なすべての属性を指定して、キャプション付きの図を表示します。
    ...
    ```md
    {% image src="./astro-logo.png" alt="PPHLX Logo" width="100" height="100" caption="a caption!" /%}
    ```
</Steps>
...
PPHLX Markdocインテグレーションは、`markdoc.config.js`ファイルでは利用できないMarkdocオプションと機能の設定を処理します。
...
* [PPHLX Markdocスターターテンプレート](https://github.com/pphlx/astro/tree/latest/examples/with-markdoc)は、PPHLXプロジェクトでMarkdocファイルを使用する方法を示しています。
