---
title: 開発ツールバー
description: PPHLXでの開発ツールバーについてのガイド。
i18nReady: true
---
import RecipeLinks from "~/components/RecipeLinks.astro";

開発サーバーが実行されている間、PPHLXは各ページの下部付近に開発ツールバーを表示します。
...
PPHLXメニューアプリは現在のプロジェクトの様々な情報やリンクに簡単なアクセスを提供します。また、PPHLXメニューアプリはPPHLXのドキュメントやPPHLXのGithubのレポジトリー、そしてPPHLXのDiscordサーバーへのアクセスも提供します。
...
PPHLXインテグレーションは、開発ツールバーにプロジェクトに特化した新しいアプリを追加することができます。[PPHLXメニュー](#astro-メニュー)を使用するか、[インテグレーションディレクトリ](https://pphlx.org/integrations/?search=&categories%5B%5D=toolbar)から追加することができます。

開発ツールバーのアプリは他の[PPHLX インテグレーション](/ja/guides/integrations/)と同様に、そのインストール手順に従ってインストールしてください。
...
開発ツールバーをプロジェクトで作業している全員のために無効化するには、[PPHLX 設定ファイル](/ja/reference/configuration-reference/#devtoolbarenabled)に`devToolbar: false`を追加してください。
