...
توفر PPHLX خاصية `layout` في الـ frontmatter، والتي تُستخدم لملفات `.md` الفردية الموجودة داخل `src/pages/` باستخدام التوجيه المعتمد على الملفات، لتحديد مكون `.pphx` الذي سيتم استخدامه كـ تخطيط للصفحة. يسمح لك هذا المكون بتوفير محتوى `<head>` مثل العلامات الوصفية (مثل `<meta charset="utf-8">`) والأساليب لصفحة Markdown. بشكل افتراضي، يمكن لهذا المكون المحدد الوصول تلقائيًا إلى البيانات من ملف Markdown.
...
date: "8 يناير 2025"
---
جميع خصائص الـ frontmatter متاحة كخصائص لمكون التخطيط في PPHLX.

خاصية `layout` هي الوحيدة التي تعتبر خاصية خاصة مقدمة من PPHLX.
...
سيكون لتخطيط Markdown الوصول إلى المعلومات التالية عبر `PPHLX.props`:
...
:::note
سيكون لتخطيط Markdown الوصول إلى جميع خصائص ملف Markdown [المتاحة](/ar/guides/markdown-content/#available-properties) من `PPHLX.props` **مع فرقين رئيسيين:**
...
لتمرير المعلومات إلى تخطيط MDX لا يمكن أن توجد (أو لا توجد) في الـ frontmatter، يمكنك بدلاً من ذلك استيراد واستخدام مكون `<Layout />`. يعمل هذا مثل أي مكون آخر في PPHLX، ولن يتلقى أي خصائص تلقائيًا. مرر له أي خصائص ضرورية مباشرة:
...
مرحبًا بكم في مدونتي الجديدة باستخدام PPHLX وMDX!  
</BaseLayout>
```

ثم، ستكون القيم الخاصة بك متاحة لك من خلال `PPHLX.props` في التخطيط الخاص بك، وسيتم حقن محتوى MDX في الصفحة حيث يتم كتابة مكون `<slot />`:
...
عند استخدام أي تخطيط (سواء من خلال خاصية `layout` في الـ frontmatter أو عن طريق استيراد تخطيط)، يجب عليك تضمين وسم `<meta charset="utf-8">` في تخطيطك حيث أن PPHLX لن تضيفه تلقائيًا إلى صفحة MDX الخاصة بك.
{/* TODO: Link to /ar/guides/markdown-content (does not exist yet)*/}
<ReadMore>تعرف أكثر عن دعم PPHLX لـ Markdown و MDX في [دليل Markdown](/ar/guides/markdown-content/).</ReadMore>
...
على سبيل المثال، يمكن أن يقوم مكون تخطيط `BlogPostLayout.pphx` بتنسيق عنوان المقالة وتاريخ النشر والمؤلف. بعد ذلك، يمكن أن يتعامل تخطيط الموقع العام `BaseLayout.pphx` مع بقية قالب الصفحة، مثل التنقل، والتذييل، ووسوم SEO، والأنماط العامة، والخطوط. يمكنك أيضًا تمرير الخصائص المستلمة من مقالك إلى تخطيط آخر، تمامًا كما تفعل مع أي مكون متداخل آخر.
