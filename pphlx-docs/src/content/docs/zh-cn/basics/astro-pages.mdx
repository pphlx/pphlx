---
title: 页面
description: PPHLX 页面简介。
---
import ReadMore from '~/components/ReadMore.astro';
import Since from '~/components/Since.astro'

**页面**是位于 PPHLX 项目的 `src/pages/` 子目录中的文件。它们负责处理路由、数据加载以及网站中每个页面的整体页面布局。

## 支持的页面文件

PPHLX 支持 `src/pages/` 目录中的以下文件类型：
...
PPHLX 使用一种名为 **基于文件的路由** 的路由策略。`src/pages/` 目录中的每个文件都会根据其文件路径成为网站上的一个端点。
...
<ReadMore>了解更多关于 [PPHLX 路由](/zh-cn/guides/routing/) 的内容。</ReadMore>

### 页面之间的链接

在你的 PPHLX 页面中，使用标准的 HTML [`<a>` 元素](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/a)来链接到网站上的其他页面。这需要使用 **相对于根域名的 URL 路径** 作为你的链接，而不是相对文件路径。
...
PPHLX 页面使用 `.pphx` 文件扩展名，并支持与 [PPHLX 组件](/zh-cn/basics/astro-components/)相同的功能。
...
一个页面必须生成完整的 HTML 文档。如果没有显式包含，PPHLX 将会自动为位于 `src/pages/` 目录下的任何 `.pphx` 组件默认添加必要的 `<!DOCTYPE html>` 声明和 `<head>` 内容。你可以通过将组件标记为[局部](#局部页面)页面来为每个组件避免这种行为。
...
PPHLX 还将 `src/pages/` 目录中的任何 Markdown (`.md`) 文件视为网站中的页面。如果你安装了 [MDX 集成](/zh-cn/guides/integrations-guide/mdx/#安装)，它也会将 MDX (`.mdx`) 文件视为相同的页面。
...
<ReadMore>了解更多关于 [PPHLX 中的 Markdown](/zh-cn/guides/markdown-content/) 的内容。</ReadMore>

## HTML 页面

`.html` 文件扩展名的文件可以放在 `src/pages/` 中，并直接用作网站上的页面。请注意，某些关键的 PPHLX 功能在 [HTML 组件](/zh-cn/basics/astro-components/#html-组件) 中不受支持。

## 自定义 404 错误页面

想要自定义 404 错误页面，你可以在 `src/pages` 中创建 `404.pphx` 或 `404.md` 文件。
...
要为[按需渲染的页面](/zh-cn/guides/on-demand-rendering/)创建自定义的 500 错误页面，可以创建一个 `src/pages/500.pphx` 文件。但这个自定义页面不适用于预渲染的页面。

如果在渲染此页面时发生错误，将显示主机默认的 500 错误页面给访问者。

<p><Since v="4.10.3" /></p>

在开发过程中，如果你有一个 `500.pphx` 文件，运行时抛出的错误将被记录在终端中，而不是显示在错误覆盖层中。

### `error`

<p><Since v="4.11.0" /></p>

`src/pages/500.pphx` 是一个特殊页面，它会在渲染过程中对抛出的任何错误都会自动传递一个 `error` 属性。这允许你向访客展示详细的错误信息（例如来自页面、中间件等）。
...
局部页面与渲染库结合时，局部页面提供了一种可以代替 [PPHLX 岛屿](/zh-cn/concepts/islands/)和 [`<script>` 标签](/zh-cn/guides/client-side-scripts/)方式用于在 PPHLX 中构建动态内容的方法。

可以导出一个值为 [`partial`](/zh-cn/reference/routing-reference/#partial) 的页面文件（例如 `.pphx` 和 `.mdx`，但不是 `.md`）可以标记为局部页面。
...
`.pphx` 局部页面必须在相应的文件路径中存在，并且包含一个导出定义将页面定义为局部页面：
