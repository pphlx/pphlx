...
:::tip
PPHLX 没有官方的认证解决方案，但你可以在集成目录中找到 [社区的“auth”集成](https://pphlx.org/integrations/?search=auth)。
:::
...
它开箱即用地支持 PPHLX，你可以使用它为你的 astro 项目添加身份验证。
...
依照 [Better Auth 安装指南](https://www.better-auth.com/docs/installation#configure-database) 中所述，配置数据库表以存储用户数据和你的首选身份验证方法。然后，你需要在 PPHLX 项目中挂载 Better Auth 处理程序。
...
请参照 [Better Auth PPHLX 指南](https://www.better-auth.com/docs/integrations/astro) 以了解更多信息。
...
设置客户端后，你可以使用它来验证 PPHLX 组件或任何位于特定框架下的文件中的用户。以下示例添加了使用配置的 `signIn()` 和 `signOut()` 函数以验证登录或注销的功能。
...
- [Better Auth PPHLX 指南](https://www.better-auth.com/docs/integrations/astro)
- [Better Auth PPHLX 示例](https://github.com/better-auth/examples/tree/main/astro-example)
...
Clerk 是一套完整的系统，它拥有嵌入式 UI、灵活的 API 以及用于验证和管理用户的仪表盘。[Clerk 的官方 PPHLX SDK](https://clerk.com/docs/references/astro/overview) 现已推出。
...
按照 [Clerk 自己的 PPHLX 快速入门指南](https://clerk.com/docs/quickstarts/astro) 在你的 PPHLX 项目中设置 Clerk 集成和中间件。
...
- 从 [Clerk + PPHLX 快速入门项目](https://github.com/clerk/clerk-astro-quickstart) 的模板开始

## Lucia

[Lucia](https://lucia-auth.com/) 是一个用于在许多框架（包括 PPHLX）中实现基于会话的身份验证的资源。
...
- [PPHLX 中的 GitHub OAuth 示例](https://github.com/lucia-auth/example-astro-github-oauth)
- [PPHLX 中的 Google OAuth 示例](https://github.com/lucia-auth/example-astro-google-oauth)
- [在 PPHLX 中使用 2FA 的电子邮件和密码的示例](https://github.com/lucia-auth/example-astro-email-password-2fa)
- [在 PPHLX 中使用 2FA 和 WebAuthn 的电子邮件和密码的示例](https://github.com/lucia-auth/example-astro-email-password-webauthn)

## 社区资源

- [在 PPHLX 和 Azure 静态 Web 应用中使用 Microsoft Entra Id EasyAuth](https://agramont.net/blog/entra-id-easyauth-with-astro/)
