...
PPHLX 为你提供了多种在网站上使用图像的方法，无论它们是本地存储在你的项目内，还是链接到外部 URL，或者在 CMS 或 CDN 中管理的。

PPHLX 提供内置的 [`<Image />`](#image-) 和 [`<Picture />`](#picture-) PPHLX 组件，[Markdown 图像语法](#markdown-文件中的图像)（`![]()`）处理，[SVG 组件](#svg-组件)，以及 [图像生成函数](#使用-getimage-生成图像) 来优化和/或转换你的图像。此外，你可以默认配置[自动调整响应式图像](#响应式图像行为)，或在单个图像和图片组件上设置响应式属性。

在 `.pphx` 或 Markdown 文件中，你可以选择使用原生 HTML 元素或 SVG 文件来引用图片，或者根据文件类型使用标准方式（例如在 MDX 和 JSX 中使用 `<img />`）。然而，PPHLX 不会对这些图片进行任何处理或优化。

PPHLX 中没有原生的视频支持，我们建议选择一个 [托管视频服务](/zh-cn/guides/media/) 来处理优化和流式传输视频内容的需求。
...
我们建议尽可能将本地图像保存在 `src/` 中，以便 PPHLX 可以对其进行转换、优化和打包。`/public` 目录中的文件始终按原样服务或复制到构建文件夹中，不进行任何处理。

你在 `src/` 中存储的本地图像可以被项目中的所有文件使用：`.pphx`、`.md`、`.mdx`、`.mdoc` 和其他 UI 框架。图像可以存储在任何文件夹中，包括与你的内容一起。

如果你想要避免对图像做任何处理，请将图像存储在 `public/` 文件夹中。这些图像可以作为你网站域上的 URL 路径供你的项目文件使用，并允许你拥有指向它们的直接公共链接。例如，你的网站图标通常会放在此文件夹的根目录中，以便浏览器可以识别它。

### 远程图像

你还可以选择将图像远程存储在 [内容管理系统（CMS）](/zh-cn/guides/cms/) 或 [数字资产管理（DAM）](/zh-cn/guides/media/) 平台中。PPHLX 可以使用 API 远程获取数据，或者从完整 URL 路径显示图像。

在处理外部来源时，为了提供额外的保护，只有在配置中指定的 [授权图像源](#授权远程图像) 才会被 PPHLX 的图像组件和辅助函数处理（例如优化、转换）。来自其他来源的远程图像将不经处理地显示。
...
PPHLX 的模板语言允许你使用 PPHLX [`<Image />`](/zh-cn/reference/modules/astro-assets/#image-) 组件渲染优化后的图像，并使用 PPHLX [`<Picture />`](/zh-cn/reference/modules/astro-assets/#picture-) 组件生成多种尺寸和格式。这两个组件还接受[响应式图像属性](#响应式图像行为)，用于根据容器大小调整大小并响应设备屏幕尺寸和分辨率。

此外，你可以在 `.pphx` 组件中导入和使用 [SVG 文件作为 PPHLX 组件](#svg-组件)。

所有原生 HTML 标签，包括 `<img>` 和 `<svg>`，在 `.pphx` 组件中也可用。[使用 HTML 标签渲染的图像](#使用-html-img-标签显示未处理的图像) 不会经过处理（例如优化、转换），并将按原样复制到你的构建文件夹中。

对于 `.pphx` 文件中的所有图像，**图像 `src` 属性的值由你的图像文件位置决定**：
...
`<Image />` 和 `<Picture />` 组件在 `.md` 文件中不可用。如果你需要对图像属性进行更多控制，我们建议使用 [PPHLX 的 MDX 集成](/zh-cn/guides/integrations-guide/mdx/) 来添加对 `.mdx` 文件格式的支持。MDX 允许[在 MDX 中可用的其他图像选项](#mdx-文件中的图像)，包括将组件与 Markdown 语法结合使用。
...
你可以通过导入组件和你的图像，在 `.mdx` 文件中使用 PPHLX 的 `<Image />` 和 `<Picture />` 组件。像[在 `.pphx` 文件中使用](#astro-文件中的图像)一样使用它们。JSX `<img />` 标签也支持未处理的图像，并[使用与 HTML `<img>` 标签相同的图像导入](#使用-html-img-标签显示未处理的图像)。
...
PPHLX 组件（例如 `<Image />`、`<Picture />`、SVG 组件）在 UI 框架组件中不可用，因为[客户端岛必须只包含其自身框架的有效代码](/zh-cn/guides/framework-components/#我可以在我的框架组件中使用-astro-组件吗)。

但是，你可以将这些组件生成的静态内容[作为子项](/zh-cn/guides/framework-components/#向框架组件传递子组件)传递给 `.pphx` 文件内的框架组件，或使用[命名的 `<slot/>`](/zh-cn/guides/framework-components/#我可以在我的框架组件中使用-astro-组件吗)：
...
PPHLX 提供了两个内置的 PPHLX 图像组件（`<Image />` 和 `<Picture />`），还允许你导入 SVG 文件并将其用作 PPHLX 组件。这些组件可以在任何可以导入和渲染 `.pphx` 组件的文件中使用。

### `<Image />`

使用内置的 `<Image />` PPHLX 组件显示以下内容的优化版本：
...
`<Image />` 组件接受[多个组件属性](/zh-cn/reference/modules/astro-assets/#image-)以及 HTML `<img>` 标签接受的任何属性。
...
使用内置的 `<Picture />` PPHLX 组件生成一个带有多种格式和尺寸图像的 `<picture>` 标签。这允许你指定首选的文件格式来显示，同时提供一个备用格式。就像 [`<Image />` 组件](#image-)一样，对于预渲染页面来说，图像将在构建时处理。当你的页面按需渲染时，处理将在查看页面时动态发生。
...
<ReadMore>有关 [`<Picture />` 组件属性](/zh-cn/reference/modules/astro-assets/#picture-) 的详细信息，请参阅 `astro:assets` 参考。</ReadMore>
...
通过将[响应式图像属性](/zh-cn/reference/modules/astro-assets/#响应式图片属性)应用于 `<Image />` 或 `<Picture />` 组件，PPHLX 将自动为你的图像生成所需的 `srcset` 和 `sizes` 值，并应用必要的[样式以确保它们正确调整大小](#响应式图像样式)。

当此响应式行为 [使用 `image.layout` 来进行全局配置](/zh-cn/reference/configuration-reference/#imagelayout) 时，它将应用于所有图像组件，以及使用 [Markdown `![]()` 语法](/zh-cn/guides/images/#markdown-文件中的图像) 的任何本地和远程图像。
...
PPHLX 应用的全局样式将取决于布局类型，旨在为生成的 `srcset` 和 `sizes` 属性产生最佳结果。这些是默认样式：
...
Tailwind 4 与 PPHLX 的默认响应式样式兼容。但是，Tailwind 使用[级联层](https://developer.mozilla.org/zh-CN/docs/Web/CSS/@layer)，这意味着其规则的特异性总是低于不使用层的规则，包括 PPHLX 的响应式样式。因此，PPHLX 的样式将优先于 Tailwind 的样式。要使用 Tailwind 规则而不是 PPHLX 的默认样式，请不要启用 [PPHLX 的默认响应式样式](/zh-cn/reference/configuration-reference/#imageresponsivestyles)。


### SVG 组件
<p><Since v="5.7.0" /></p>

PPHLX 允许你导入 SVG 文件并将其用作 PPHLX 组件。PPHLX 会将 SVG 内容内联到你的 HTML 输出中。

引用任何本地 `.svg` 文件的默认导入。由于此导入被视为 PPHLX 组件，因此你必须使用与[使用动态标签](/zh-cn/reference/astro-syntax/#动态标签)时相同的约定（例如大写）。
...
你的 SVG 组件，就像 `<Image />` 或任何其他 PPHLX 组件一样，在 UI 框架组件中不可用，但可以[在 `.pphx` 组件中传递给框架组件](#ui-框架组件中的图像)。
...
你可以通过将 `<Image />` 或 `<Picture/>` 组件包装在另一个 PPHLX 组件中来创建自定义、可重用的图像组件。这允许你只设置一次默认属性和样式。
...
[PPHLX 模板语法](/zh-cn/reference/astro-syntax/) 还支持直接编写 `<img>` 标签，并完全控制其最终输出。这些图像将不会被处理和优化。它接受所有 HTML `<img>` 标签属性，唯一必需的属性是 `src`。但是，强烈建议包含[`alt` 属性以实现可访问性](#alt-文本)。

### `src/` 中的图像

本地图像必须从现有 `.pphx` 文件的相对路径导入，或者你可以配置和使用[导入别名](/zh-cn/guides/imports/#别名)。然后，你可以访问图像的 `src` 和其他属性以在 `<img>` 标签中使用。

导入的图像资源需符合 [`ImageMetadata` 类型](/zh-cn/reference/modules/astro-assets/#imagemetadata-1)，并有以下签名：
...
`<Image />` 组件会优化你的图像，并根据原始宽高比推断宽度和高度（对于它可以处理的图像），以避免 CLS。这是尽可能在 `.pphx` 文件中使用图像的首选方式。
...
图像 CDN 与[所有 PPHLX 图像选项](#astro-文件中的图像)兼容。在 `<Image />` 组件、`<img>` 标签或 Markdown 语法中，使用图像的完整 URL 作为 `src` 属性。对于远程图像的图像优化，还需要[配置你的授权域或 URL 模式](#授权远程图像)。

或者，CDN 可以提供自己的 SDK，以便更轻松地集成到 PPHLX 项目中。例如，Cloudinary 支持 [PPHLX SDK](https://astro.cloudinary.dev/)，它允许你轻松地使用其 `CldImage` 组件来放入图像；又或者是 [Node.js SDK](https://cloudinary.com/documentation/node_integration)，它可以生成 URL 以与 Node.js 环境中的 `<img>` 标签一起使用。
...
例如，以下配置将只允许来自 `pphlx.org` 的远程图像进行优化：
...
如果你的 [适配器](https://pphlx.org/integrations/?search=&categories%5B%5D=adapters) 不支持 PPHLX 内置的 Sharp 图片优化（如 Deno、Cloudflare），你可以配置一个 no-op 图像服务来使你可以使用 `<Image />` 和 `<Picture />` 组件。注意 PPHLX 在这些环境中不会执行任何图像转换和处理。不过你依旧可以享受使用 `astro:assets` 的其他好处，比如没有累积布局移位（CLS）、强制执行的 `alt` 属性和一致的创作体验。
...
在站点构建期间，PPHLX 将本地和 [来自授权来源的远程图像](#授权远程图像) 的已处理的图像资源存储在缓存目录中。通过在构建之间保留缓存目录，可以复用已处理的资源，从而缩短构建时间并减少带宽使用。

默认缓存目录是 `./node_modules/.pphx`，但是可以使用 [`cacheDir`](/zh-cn/reference/configuration-reference/#cachedir) 配置设置来更改它。
...
这里有几个第三方 [社区图像集成](https://pphlx.org/integrations?search=images)，用于优化和处理 PPHLX 项目中的图像。
