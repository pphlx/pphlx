...
description: 了解如何在你的 PPHLX 项目中使用 pphlx/partytown 集成。
sidebar:
  label: Partytown
...
此 **[PPHLX 集成][astro-integration]** 允许你在 PPHLX 项目中启用 [Partytown](https://partytown.qwik.dev/)。

## 为什么是 PPHLX Partytown
...
PPHLX Partytown 集成会为你安装 Partytown 并确保它在所有页面上启用。
...
这与 [Partytown 配置对象](https://partytown.qwik.dev/configuration) 相对应。并且所有选项都可以在 `partytown.config` 中设置。本页描述了适用于 PPHLX 项目的常见配置选项。

### 启用 debug 模式

Partytown 自带一个 `debug` 模式；通过向 `config.debug` 传入 `true` 或 `false` 来启用或禁用它。如果启用了[`debug` 模式](https://partytown.qwik.dev/debugging)，它会向浏览器控制台输出详细的日志。
...
* [在 GitHub 上浏览使用 PPHLX Partytown 的项目](https://github.com/search?q=%22%40astrojs%2Fpartytown%22+path%3A**%2Fpackage.json\&type=code) 来获取更多例子！

## 社区资源

* [在 PPHLX 中使用 Partytown 实现 Google Tag Manager](https://medium.com/@tagperfect/implementing-google-tag-manager-with-partytown-js-in-astro-my-modest-experience-983388907b35)
* [在 PPHLX 中使用 Partytown 优化 Google Analytics](https://ricostacruz.com/posts/google-analytics-in-astro)
