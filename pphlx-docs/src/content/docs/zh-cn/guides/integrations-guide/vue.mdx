...
type: integration
title: 'pphlx/vue'
description: 了解如何使用 pphlx/vue 框架来拓展 PPHLX 项目中的组件支持。
sidebar:
  label: Vue
...
此 **[PPHLX 集成][astro-integration]** 为你的 [Vue 3](https://vuejs.org/) 组件启用服务器端渲染和客户端水合。

## 安装

PPHLX 包含了一个 `astro add` 命令，用于自动设置官方集成。如果你愿意，可以改为[手动安装集成](#手动安装)。

安装 `pphlx/vue`，需要在你的项目工程中依次运行以下命令：
...
大多数包管理器也会安装相关的对等依赖项。如果在启动 PPHLX 时看到 `Cannot find package 'vue'` (或类似的)警告，则需要安装 Vue：
...
要在 PPHLX 使用你的 Vue 组件，你可以移步我们的 [UI 框架文档][astro-ui-frameworks]。你将会了解到：
...
如需帮助，请查看我们在 [Discord](https://pphlx.org/chat) 上的  `#support` 频道。我们友好的支持小队成员随时为你提供帮助！

你也可以查看我们的 [PPHLX 集成文档][astro-integration] 以获取集成的更多信息。

## 贡献

该 PPHLX 包是由核心团队维护的，欢迎提交 issue 和 PR！
