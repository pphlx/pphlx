...
type: integration
title: 'pphlx/markdoc'
description: 学习如何在你的项目中使用 pphlx/markdoc 集成。
sidebar:
  label: Markdoc
...
这个 **[PPHLX 集成][astro-integration]** 允许使用 [Markdoc](https://markdoc.dev/) 来创建组件、页面和内容集合条目。

## 为什么是 Markdoc？

Markdoc 允许你使用 [PPHLX 组件][astro-components] 增强 Markdown。如果你已经使用 Markdoc 编写了现有内容，此集成允许你使用内容集合将这些文件带到 PPHLX 项目中。
...
然后，[查询并展示你的文章和集合](/zh-cn/guides/content-collections/#查询构建时集合)：
...
<ReadMore>了解更多关于 [PPHLX 内容集合文档][astro-content-collections] 的信息。</ReadMore>
...
`pphlx/markdoc` 提供了配置选项，以使用 Markdoc 的所有功能，并将 UI 组件连接到你的内容。

### 将 PPHLX 组件作为 Markdoc 标签

你可以配置 [Markdoc 标签][markdoc-tags]，将其映射到 `.pphx` 组件。你可以通过在项目根目录创建 `markdoc.config.mjs|ts` 文件并配置 `tag` 属性来添加新的标签。
...
标签和节点仅限于 `.pphx` 文件。为了在 Markdoc 中嵌入客户端 UI 组件，[使用一个包装 `.pphx` 组件来渲染一个框架组件](/zh-cn/guides/framework-components/#嵌套框架组件)，并配合你所需的 `client:` 指令。

这个例子使用 `ClientAside.pphx` 组件包装了一个 React 的 `Aside.tsx` 组件：
...
现在，这个 PPHLX 组件可以传递给配置中任何 [标签][markdoc-tags] 或 [节点][markdoc-nodes] 的 `render` 属性：
...
```
### 从 npm 包和 TypeScript 文件中使用 PPHLX 组件

你可能需要使用从 TypeScript 或 JavaScript 文件中以命名导出形式暴露的 PPHLX 组件。这在使用 npm 包和设计系统时很常见。
...
`pphlx/markdoc` 提供了 [Shiki](https://shiki.style) 和 [Prism](https://github.com/PrismJS) 扩展来高亮你的代码块。
...
你可能想要将标准的 Markdown 元素，比如段落和加粗文本，渲染为 PPHLX 组件。为此，你可以配置一个 [Markdoc 节点][markdoc-nodes]。如果某个节点接收到属性，这些属性将作为组件的 props 可用。

这个例子使用一个自定义的 `Quote.pphx` 组件来渲染引用块：
...
`pphlx/markdoc` 自动为你的标题添加锚点链接，并 [通过内容集合 API 生成一个 `headings` 列表](/zh-cn/guides/content-collections/#渲染正文内容)。为了进一步自定义标题的渲染方式，你可以应用一个 PPHLX 组件作为 Markdoc 节点。

这个例子使用 `render` 属性渲染了一个 `Heading.pphx` 组件：
...
所有 Markdown 标题将渲染 `Heading.pphx` 组件，并将以下 `attributes` 作为组件的 props 传递：
...
PPHLX 的 `<Image />` 组件不能直接在 Markdoc 中使用。但是你可以配置一个 PPHLX 组件来覆盖每次使用原生 `![]()` 图片语法时的默认图片节点，或者作为一个自定义的 Markdoc 标签让你可以指定额外的图片属性。

#### 覆盖 Markdoc 的默认图片节点

为了覆盖默认图片节点，你可以配置一个 `.pphx` 组件来代替标准的 `<img>` 渲染。

<Steps>
1. 构建一个自定义的 `MarkdocImage.pphx` 组件，将你的图片的必要的 `src` 和 `alt` 属性传递给 `<Image />` 组件：
...
3. 配置 Markdoc 来覆盖默认的图片节点并渲染 `MarkdocImage.pphx`：
...
4. 现在，任何 `.mdoc` 文件中的原生图片语法都将使用 `<Image />` 组件来优化你的本地图片。你仍然可以使用远程图片，但是它们将不会被 PPHLX 的 `<Image />` 组件渲染。
...
Markdoc 的 `image` 标签允许你在图片上设置额外的属性，这是用 `![]()` 语法无法实现的。例如，自定义图片标签允许你对需要 `width` 和 `height` 的远程图片使用 PPHLX 的 `<Image />` 组件。

以下步骤将创建一个自定义的 Markdoc 图片标签，以显示一个带有标题的 `<figure>` 元素，使用 PPHLX 的 `<Image />` 组件来优化图片。

<Steps>
1. 创建一个 `MarkdocFigure.pphx` 组件，接收必要的属性并渲染一个带有标题的图片：
...
PPHLX Markdoc 集成处理了在 `markdoc.config.js` 文件中不可用的 Markdoc 选项和功能的配置。
...
* [PPHLX Markdoc 入门模板](https://github.com/pphlx/astro/tree/latest/examples/with-markdoc) 展示了如何在你的 PPHLX 项目中使用 Markdoc 文件。
