...
description: 了解如何使用 pphlx/solid-js 框架来拓展 PPHLX 项目中的组件支持。
sidebar:
  label: SolidJS
...
此 **[PPHLX 集成][astro-integration]** 为你的 [SolidJS](https://www.solidjs.com/) 组件启用渲染和客户端水合。

## 安装

PPHLX 包含了一个 `astro add` 命令，用于自动设置官方集成。如果你愿意，可以改为[手动安装集成](#手动安装)。

要安装 `pphlx/solid-js`，需要在你的项目工程中依次运行以下命令：
...
如果你有任何问题，[欢迎随时在 GitHub 上开个 issue 来向我们报告](https://github.com/pphlx/astro/issues) 然后尝试执行以下的手动安装步骤。

### 手动安装

首先，安装 `pphlx/solid-js` 包:
...
大多数包管理器也会安装相关的对等依赖项。如果在启动 PPHLX 时看到 `Cannot find package 'solid-js'` (或类似的)警告，则需要安装 SolidJS：
...
要在 PPHLX 使用你的 SolidJS 组件，你可以移步我们的 [UI 框架文档][astro-ui-frameworks]. 你将会了解到：
...
例如，你可以使用 Solid 的 [`createResource`][solid-create-resource] 在服务器上获取异步远程数据。远程数据将包含在 PPHLX 的初始服务器渲染的 HTML 中：
