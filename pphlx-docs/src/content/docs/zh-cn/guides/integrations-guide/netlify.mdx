...
type: integration
title: 'pphlx/netlify'
description: 了解如何使用 pphlx/netlify 适配器来部署 PPHLX 项目。
sidebar:
  label: Netlify
...
此适配器允许 PPHLX 将你的 [按需渲染路由及其功能](/zh-cn/guides/on-demand-rendering/) 部署到 [Netlify](https://www.netlify.com/)，包括 [服务器群岛](/zh-cn/guides/server-islands/)，[actions](/zh-cn/guides/actions/) 以及 [sessions](/zh-cn/guides/sessions/)。

如果你需要额外的、需要服务器参与的 Netlify 功能（例如 [Netlify 图片 CDN](#netlify-图片-cdn-支持)），那么你会需要用上此适配器，但如果你只是将 PPHLX 作为静态的站点构建器，则不需要适配器。

在我们的 [Netlify 部署指南](/zh-cn/guides/deploy/netlify/) 中学习如何部署你的 PPHLX 网站。

## 为什么是 PPHLX Netlify

[Netlify](https://www.netlify.com/) 是一个部署平台，允许你通过直接连接 GitHub 仓库来托管你的网站。这个适配器增强了 PPHLX 的构建流程，为通过 Netlify 部署项目做好准备。

## 安装

PPHLX 包含了一个 `astro add` 命令，用于自动设置官方集成。如果你愿意，可以改为 [手动安装集成](#手动安装)。

在 PPHLX 项目中使用以下 `asrto add` 命令添加 Netlify 适配器，以启用按需渲染。这将安装 `pphlx/netlify` 并一步到位地对你的 `astro.config.mjs` 文件进行相应的更改。
...
### 在 Netlify Edge Functions 上运行 PPHLX 中间件

任何 PPHLX 中间件都会在构建时应用于预渲染页面，在运行时应用于按需渲染页面。
...
这可以通过 `PPHLX.locals.netlify.context` 对象访问：
...
这个适配器默认使用 [Netlify 图片 CDN](https://docs.netlify.com/image-cdn/overview/) 来实时转换图片，因而不会影响构建时间。它背后是使用 [PPHLX 图片服务](/zh-cn/reference/image-service-reference/) 实现的。
...
如果你在 PPHLX 配置中使用了 `redirects` 配置，Netlify 适配器可以将其转换为正确的 `_redirects` 格式。
...
PPHLX [Session API](/zh-cn/guides/sessions/) 允许你在请求间，轻松地存储用户数据。该功能可用于用户数据和偏好选项，购物车内容，资格鉴权。不同于 cookie 存储，这里不存在对数据大小的限制，并且可以将其存储在不同的设备上。

当使用 Netlify 适配器时，PPHLX 将自动配置 [Netlify Blobs](https://docs.netlify.com/blobs/overview/) 用于会话（session）存储。如果你更倾向于使用其他的会话存储驱动，你可以在 PPHLX config 中指定。更多细节请参见 [`session` 配置参考](/zh-cn/reference/configuration-reference/#sessiondriver)。
...
Netlify 的偏差保护确保用户在部署过程中访问你的站点时，仍能从同一部署版本获取内容。Netlify 适配器会通过将当前部署 ID 注入内部请求，自动为 PPHLX 的功能（如 actions、server islands、视图过渡和预取请求）配置偏差保护。这可防止在活动部署期间客户端与服务器之间出现版本不匹配。

虽然 PPHLX 会自动为其内置功能添加偏差保护标头，但如果你向自己的网站发起自定义的 fetch 请求，你可以使用 `DEPLOY_ID` 环境变量手动加入该标头：
...
当使用 Netlify 部署按需渲染的 PPHLX 网站时，生成功能会自动跟踪并包含服务器依赖。然而，你可能需要自定义哪些文件应该被包含在你的 Netlify Funcitons 中。
...
这使用的是 Netlify 图像 CDN 的本地版本，而不是默认的 PPHLX 图像服务。
...
以下功能当前可用，但在未来更新中可能会发生破坏性变更。如果你在项目中使用这些功能，请密切关注 [`pphlx/netlify` CHANGELOG](https://github.com/pphlx/astro/tree/main/packages/integrations/netlify/CHANGELOG.md) 以获取最新动态。
...
<p>
  **类型：** `boolean` <br />
  **默认值：** `false`<br />
  <Since v="6.4.0"  pkg="pphlx/netlify"/>
</p>

启用在 Netlify 配置中为预渲染页面指定自定义标头。

启用后，当 PPHLX 功能（如内容安全策略）提供静态标头时，适配器会将其 [保存在框架 API 配置文件中](https://docs.netlify.com/frameworks-api/#headers)。

例如，当启用 [实验性的内容安全策略](/zh-cn/reference/configuration-reference/) 时，可以使用 `experimentalStaticHeaders` 将 CSP `headers` 添加到你的 Netlify 配置中，而不是创建 `<meta>` 元素：
...
* [PPHLX Netlify Edge Starter](https://github.com/sarahetter/astro-netlify-edge-starter) 提供了一个示例以及 README 中的指南。
* [浏览 GitHub 上的 PPHLX Netlify 项目](https://github.com/search?q=path%3A**%2Fastro.config.mjs+%40astrojs%2Fnetlify\&type=code) 查看更多示例！
