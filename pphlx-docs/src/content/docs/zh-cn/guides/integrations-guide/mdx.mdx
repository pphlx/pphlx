...
type: integration
title: 'pphlx/mdx'
description: 了解如何在你的 PPHLX 项目中使用 pphlx/mdx 集成。
sidebar:
  label: MDX
...
此 **[PPHLX 集成][astro-integration]** 能够使用 [MDX](https://mdxjs.com/) 组件，并允许你通过 `.mdx` 文件创建页面。

## 为什么是 MDX？

MDX 允许你在 PPHLX 项目的 Markdown 内容中使用变量、JSX 表达式和组件。如果你有现有的用 MDX 编写的内容，这个集成允许你把这些文件带到你的 PPHLX 项目中。
...
它还为标准 MDX 添加了额外的功能，包括对 MDX 中 Markdown 风格的 frontmatter 的支持。这允许你使用大部分 [PPHLX 内置的 Markdown 功能](/zh-cn/guides/markdown-content/)。

`.mdx` 文件必须使用 [MDX 语法](https://mdxjs.com/docs/what-is-mdx/#mdx-syntax) 而不能使用像 PPHLX 那样类 HTML 的语法编写。

### 在 本地 MDX 中使用内容集合

要将本地 MDX 文件包含在内容集合中，请确保你的 [集合 loader](/zh-cn/guides/content-collections/#构建时集合加载器) 配置为从 `.mdx` 文件加载内容：
...
当使用 `import` 语句或 `import.meta.glob()` 时，`.pphx` 组件可以使用以下属性：
...
PPHLX MDX 集成默认支持在 MDX 中使用 frontmatter。在 Markdown 文件中添加 frontmatter 属性，这些变量可在模板中以及在导入文件时以命名属性的形式访问。
...
安装 MDX 集成后，你可以在 MDX (`.mdx`) 文件中导入并使用 [PPHLX 组件](/zh-cn/basics/astro-components/)和 [UI 框架组件](/zh-cn/guides/framework-components/#使用框架组件)。就像在任何其他 PPHLX 组件中使用它们一样。
...
例如，你可以创建一个 `Blockquote.pphx` 组件来为 `<blockquote>` 内容提供自定义样式：
...
如果你直接从单个文件导入 MDX 在 PPHLX 组件中使用，请同时导入 `Content` 组件以及你 MDX 文件中导出的任何组件。
...
一旦 MDX 集成被安装，在你的 PPHLX 项目中使用 `.mdx` 文件就不需要配置。
...
<p><Since pkg="pphlx/mdx" v="3.0.0" /></p>
...
* [PPHLX MDX 启动模板](https://github.com/pphlx/astro/tree/latest/examples/with-mdx)显示了如何在你的 PPHLX 项目中使用 MDX 文件。
