...
PPHLX 允许你创建自定义端点来提供任何类型的数据。你可以使用它来生成图像、公开 RSS 文档或将它们用作 API 路由来为你的站点构建完整的 API。
...
端点导出一个 `GET` 函数（可选的以 `async` 导出），该函数接收具有类似于 `PPHLX` 全局属性的[上下文对象](/zh-cn/reference/api-reference/)。在这里，它返回一个带有 `name` 和 `url` 的 [`Response`](https://developer.mozilla.org/zh-CN/docs/Web/API/Response) 对象，PPHLX 将在构建时调用它并使用 `body` 中的内容来生成文件。
...
从 PPHLX v3.0 开始，返回的 `Response` 对象不再需要包含 `encoding` 属性。例如，生成一个二进制 `png` 图像：
...
所有端点都会接收 `request` 属性，但在静态模式下，你只能访问 `request.url`。它将返回当前端点的完整 URL，其工作方式与 [PPHLX.request.url](/zh-cn/reference/api-reference/#request) 对页面的作用相同。
...
[静态文件的端点](#静态文件端点)部分中描述的所有内容均可以在 SSR 模式下使用：文件可以导出一个名为 `GET` 的函数，该函数接收具有类似于 `PPHLX` 全局属性的[上下文对象](/zh-cn/reference/api-reference/)。
...
除了 `GET` 函数，你还可以使用任何 [HTTP 方法](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Methods) 作为名称导出函数。当有请求进来时，PPHLX 会检查该方法并调用相应的函数。
...
如果你的定义了一个 `GET` 函数，但却没有定义 `HEAD` 函数，PPHLX 将通过调用 `GET` 函数来自动处理 `HEAD` 请求并从响应中剥离 body 部分。
...
端点上下文导出一个类似于 `PPHLX.redirect` 的 `redirect()` 函数：
