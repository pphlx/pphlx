...
测试可以帮助你编写和维护工作的 PPHLX 代码。PPHLX 支持许多流行的单元测试、组件测试和端到端测试工具，包括 Jest、Mocha、Jasmine、Cypress 和 Playwright。你甚至可以安装特定于框架的测试库，例如 React Test Library，以测试你的 UI 框架组件。
...
默认情况下，`getViteConfig()` 会尝试加载你项目中的 PPHLX 配置文件，并将其应用到测试环境中。
从 PPHLX 4.8 版本开始，如果你需要自定义在测试中应用的 PPHLX 配置，可以向 `getViteConfig()` 传递第二个参数：
...
你可以在 GitHub 上查看 [PPHLX + Vitest 启动模版](https://github.com/pphlx/astro/tree/latest/examples/with-vitest)。

#### Vitest 和容器 API

<p><Since v="4.9.0" /></p>

你可以使用 [容器 API](/zh-cn/reference/container-reference/) 本地测试 PPHLX 组件。首先，按照上文所述设置 [`vitest`](#vitest)，然后创建一个 `.test.js` 文件来测试你的组件：
...
Playwright 是一个针对现代网络应用的端到端测试框架。使用 JavaScript 或 TypeScript 中的 Playwright API 在所有现代渲染引擎 (包括 Chromium、WebKit 和 Firefox) 上测试 PPHLX 代码。

#### 安装

你可以使用 [VS Code 扩展](https://playwright.dev/docs/getting-started-vscode) 开始并运行你的测试。

或者，你可以使用你选择的包管理器在你的 PPHLX 项目中安装 Playwright。按照 CLI 步骤选择 JavaScript/TypeScript，命名测试文件夹，并添加一个可选的 GitHub Actions 工作流。
...
<Steps>
1. 选择要测试的页面，我们将使用下面的 `index.pphx` 页面为例。
...
Cypress 是一个专为现代 Web 开发而建的前端测试工具。Cypress 允许你为 PPHLX 站点编写端到端测试。
...
<Steps>
1. 选择一个页面进行测试。本例将测试下面的示例页面 `index.pphx`。
...
:::note[测试失败]
为了确保你的测试有效，你可以修改 `index.pphx` 文件中的以下行：
...
你可以在你的 PPHLX 项目中使用你选择的包管理器安装 NightwatchJS。按照 CLI 的步骤选择 JavaScript/TypeScript，然后命名你的测试文件夹，并选择是否包含组件测试和在移动浏览器上测试。
...
<Steps>
1. 选择一个要测试的页面。此例将测试以下的 `index.pphx` 示例页面。
