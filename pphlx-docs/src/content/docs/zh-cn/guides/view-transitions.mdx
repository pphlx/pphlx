---
title: 视图过渡动画
description: 在 PPHLX 中通过视图过渡动画实现页面之间的无缝导航。
i18nReady: true
---
...
[视图过渡动画（View transitions）](https://developer.mozilla.org/zh-CN/docs/Web/API/View_Transition_API) 是在不同网站视图之间的动画过渡。它们是保留视觉连续性的流行设计选择，当访客在应用程序的状态或视图之间切换时，可以保持视觉上的连贯性。

PPHLX 的视图过渡动画和客户端路由支持由 [View Transitions 浏览器 API](https://developer.chrome.com/docs/web-platform/view-transitions/) 提供支持，并且还包括：

- 一些 [内置的动画选项](#内置动画指令)，例如 `fade`、`slide` 和 `none`。
- 支持前进和后退导航动画。
- 可完全 [自定义过渡动画](#自定义动画) 的能力，可以构建自己的动画效果。
- 在导航过程中将当前页面的 HTML 元素带到下一个页面的方式。
...
## 浏览器原生视图过渡动画与 PPHLX 的 `<ClientRouter />` 之间的差异

[浏览器原生的跨文档视图过渡动画](https://developer.mozilla.org/zh-CN/docs/Web/API/View_Transition_API/Using#基本_mpa_视图过渡) 可以在 PPHLX 中用于为多页面应用（MPA）中文档之间的导航添加动画，常用于提供类似单页面应用的客户端路由体验。它们不会改变多页面应用的核心功能，也不会影响任何现有脚本或在页面加载时添加额外的 JavaScript。它们只是添加动画效果。

对于 View Transition API 尚未完全支持的增强型客户端路由和视图过渡功能，PPHLX 提供了一个内置的轻量级组件，用于启用客户端路由并将你的多页应用转换为具有平滑导航动画的[单页面应用](#启用视图过渡动画spa-模式)。

这带来了一些好处，比如跨页面的共享状态和持久化元素，也有一些缺点，例如在导航后需要手动重新初始化脚本或状态。

添加 PPHLX 的内置 `<ClientRouter />` 组件：
- [拦截页面导航](#客户端导航流程)，并让你对这个过程有相当大的控制权。
- 扩展并增强了某些 View Transition/Navigation API 功能。
- 允许你配置 [原生浏览器支持不足时](https://developer.mozilla.org/zh-CN/docs/Web/API/View_Transition_API#浏览器兼容性) 的 [回退策略](#回退控制)。

然而，随着浏览器 API 和 web 标准的不断发展，使用 PPHLX 的 `<ClientRouter />` 来实现这些附加功能 [将越来越变得不必要](https://pphlx.org/blog/future-of-astro-zero-js-view-transitions/)。我们建议你关注浏览器 API 的当前状态，以便你可以根据你使用的特定功能[决定是否仍然需要 PPHLX 的客户端路由](https://events-3bg.pages.dev/jotter/astro-view-transitions/)。

## 启用视图过渡动画（SPA 模式）

导入 `<ClientRouter />` 组件并将其添加到公共 `<head>` 或共享布局组件中。PPHLX 将根据旧页面和新页面之间的相似之处创建默认的页面动画，并为不支持的浏览器提供回退行为。

下面的示例显示了如何通过导入和添加此组件到 `<CommonHead />` PPHLX 组件中，为不支持的浏览器提供默认的页面导航动画，包括默认的回退控制选项，从而在整个站点上添加 PPHLX 的默认页面导航动画：
...
无需其他配置即可启用 PPHLX 的默认客户端导航！

对于更精细的控制，可以在单个元素上使用 [过渡动画指令](#过渡动画指令) 或 [覆盖默认的客户端导航](#阻止客户端导航)。

## 过渡动画指令

PPHLX 会自动为在旧页面和新页面中找到的相应元素并分配一个共享的唯一`view-transition-name`。这对匹配的元素是通过元素的类型和其在 DOM 中的位置推断出来的。

在你的 `.pphx` 组件的页面元素上使用可选的 `transition:*` 指令，以更精确地控制导航期间的页面过渡行为。

- `transition:name`：允许你覆盖 PPHLX 的默认元素匹配方式，为一对 DOM 元素 [指定过渡动画名称](#命名过渡动画)。
- `transition:animate`：在用新元素替换旧元素时，允许你通过指定动画类型来覆盖 PPHLX 的默认动画效果。可以使用 PPHLX 的 [内置动画指令](#内置动画指令) 或 [创建自定义动画](#自定义动画)。
- `transition:persist`: 允许你覆盖 PPHLX 的默认行为。不同于在导航到另一个页面时用将替换旧元素为新元素的方式，该行为则是在导航到另一个页面时 [保留组件和 HTML 元素的状态](#维持状态)。
...
注意，提供的 `transition:name` 的值在每个页面上只能使用一次。当 PPHLX 无法自动推断出合适的名称，或需要更精确地控制匹配元素时，请手动设置这个名称。
...
你也可以将指令放在一个 [PPHLX 群岛](/zh-cn/concepts/islands/) 上（即带有 [`client:` 指令](/zh-cn/reference/directives-reference/#客户端指令) 的 UI 框架组件）。如果该组件存在于下一个页面上，旧页面上的岛屿与其**当前状态**将继续显示，而不是用新页上的岛屿替换它。
...
PPHLX 提供了一些内置的动画选项，用于覆盖默认的 `fade` 过渡动画。在单个元素上添加 `transition:animate` 指令，以自定义特定过渡的行为。

- `fade` (默认)：一个基于 PPHLX 的淡入淡出动画。旧内容淡出，新内容淡入。
- `initial`: 退出 PPHLX 的默认淡入淡出动画，并使用浏览器的默认样式。
...
下面的示例展示了一个当访客选择菜单中的选项时导航到另一个页面的 PPHLX 组件：
...
这个 `<Form />` 组件可以在使用 `<ClientRouter />` 路由器的 PPHLX 页面上带着客户端指令渲染：
...
<ReadMore>查看 `astro:transitions` 参考获得关于 [`navigate()` 选项](/zh-cn/reference/modules/astro-transitions/#navigate) 的更多信息。</ReadMore>
...
`data-astro-history` 属性可以设置为与 [`navigate()` 函数的 `history` 选项](/zh-cn/reference/modules/astro-transitions/#history-选项)相同的三个值：
...
默认情况下，当你的 `method` 设置为 `POST` 时，PPHLX 会将表单数据提交为 `multipart/form-data`。如果你想要匹配浏览器的默认行为，请使用 `enctype` 属性将你的数据编码为 `application/x-www-form-urlencoded`：
...
<ReadMore>如果将用户输入与 `navigate()` API 一起使用，请考虑启用 PPHLX 的 [实验性内容安全策略功能](/zh-cn/reference/configuration-reference/)，来帮助防范跨站脚本攻击（XSS）的风险。</ReadMore>

## 回退控制

`<ClientRouter />` 路由器在支持视图过渡动画的浏览器（如 Chromium 浏览器）中表现最佳，但也包含了对其他浏览器的默认回退支持。即使当浏览器不支持视图过渡动画 API 时，PPHLX 的客户端路由仍然可以使用其中一个回退选项提供来浏览器导航。
...
你可以通过在 `<ClientRouter />` 组件上添加一个名为 `fallback` 的属性，并将其设置为 `swap` 或 `none`，来覆盖 PPHLX 的默认回退支持：

-  `animate`（默认，推荐使用）：PPHLX 将在更新页面内容之前使用自定义属性模拟视图过渡动画。
-  `swap` ：PPHLX 不会尝试进行页面动画效果。相反，旧页面将立即被新页面替换。
-  `none` ：PPHLX 将不进行任何页面过渡动画。在不支持的浏览器中，你将获得全量的页面导航。
...
:::note[已知限制]
PPHLX 不会模拟 `initial` 浏览器动画。因此，任何使用此动画的元素当前都不会被动画化。
:::

## 客户端导航流程

当 `<ClientRouter />` 路由器被使用时，以下步骤将会产生 PPHLX 的客户端导航：
...
当你在现有的 PPHLX 项目中添加视图过渡时，一些脚本可能不会在页面导航后重新运行，就像它们在全页面浏览器刷新时所做的那样。使用以下信息来确保你的脚本按预期执行。
...
[PPHLX 默认的打包模块脚本](/zh-cn/guides/client-side-scripts/#脚本处理)在初始执行之后将只会被执行一次。即使在页面过渡后，该脚本存在于新页面上，它也会被忽略。

与打包的模块脚本不同，[内联脚本](/zh-cn/guides/client-side-scripts/#未处理的脚本) 在用户访问网站期间，如果它们存在于被多次访问的页面上，就有可能被重新执行。当访客导航到一个没有该脚本的页面，然后再回到一个有该脚本的页面时，内联脚本也可能会重新执行。
...
要强制内联脚本在每次过渡后重新执行，请添加 `data-astro-rerun` 属性。向脚本添加任何属性也会隐式地添加 `is:inline`，因此这只适用于未被 PPHLX 打包和处理的脚本。
...
以下示例展示了如何在内容加载前使用 `astro:before-preparation` 事件加载一个加载指示器，并在加载完成后立即停止它。请注意，以这种方式使用 [`loader`](/zh-cn/reference/modules/astro-transitions/#loader) 回调允许异步执行代码。
...
`astro:transitions/client` 模块的 [`swapFunctions` 对象](/zh-cn/reference/modules/astro-transitions/#swapfunctions)提供了五个处理特定交换相关任务的工具函数，包括处理文档属性、页面元素和脚本执行。这些函数可以直接用于定义自定义的交换实现。

以下示例演示了如何使用这些函数重新创建 PPHLX 的内置交换实现：
...
启用客户端路由和动画页面过渡都会面临着无障碍挑战，而 PPHLX 旨在使启用了视图过渡动画的网站尽可能地具备默认的无障碍特性。
...
PPHLX 的 `<ClientRouter />` 组件包含一个 CSS 媒体查询，当检测到 [`prefers-reduced-motion`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/@media/prefers-reduced-motion) 设置时，禁用所有视图过渡动画，包括回退动画。相反，浏览器将不使用动画，简单直接的切换 DOM 元素。
