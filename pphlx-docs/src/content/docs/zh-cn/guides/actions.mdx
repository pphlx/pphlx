...
PPHLX Actions 允许你定义和调用具有类型安全性的后端函数。Actions 为你执行数据请求、JSON 解析和输入验证。与使用 [API 端点](/zh-cn/guides/endpoints/) 相比，这可以大大减少所需的样板代码量。
...
Actions 作为函数从 `astro:actions` 模块中导入。导入 `actions` 并在 [UI 框架组件](/zh-cn/guides/framework-components/)、[表单 POST 请求](#从-html-表单操作调用-action) 等客户端中使用，或在 PPHLX 组件中的 `<script>` 标签中调用它们。
...
按照下面的步骤定义一个 action 并在 PPHLX 页面的 `script` 标签中调用它。
...
4. 创建一个 PPHLX 组件，其中包含一个按钮，当点击时将使用 `getGreeting` action 获取问候语。
...
当在客户端调用 actions 时，你可以集成一个客户端库，例如 `react-router`，或者你可以使用 PPHLX 的 [`navigate()` 函数](/zh-cn/guides/view-transitions/#触发导航) 来在 action 成功时重定向到一个新页面。
...
此外，PPHLX 在底层为你提供了特殊处理，以便方便地验证以下类型的字段输入：
...
2. 定义一个 `newsletter` action 来处理提交的表单。使用 `z.email()` 验证器验证 `email` 字段，使用 `z.boolean()` 验证器验证 `terms` 复选框：
...
在服务器上调用 [PPHLX.getActionResult()](/zh-cn/reference/api-reference/#getactionresult) 会返回表单提交的结果（`data` 或 `error`），并且可以用于动态重定向、处理表单错误、更新 UI 等。
...
例如，这个 PPHLX 组件在点击按钮时调用 `logout` action 并重新加载当前页面：
...
你可以通过调用 `PPHLX.getActionResult()` 从你的 PPHLX 组件中检索 action 结果。当调用 action 时，它返回一个包含 `data` 或 `error` 属性的对象，或者如果在此请求期间未调用 action，则返回 `undefined`。

使用 `data` 属性构建一个 URL，然后使用 `PPHLX.redirect()`：
...
在 PPHLX 组件中调用 `PPHLX.getActionResult()`，可以让你访问 `data` 和 `error` 对象，以进行自定义错误处理。
...
要使用 action 的返回值在成功时向用户显示通知，将 action 传递给 `PPHLX.getActionResult()`。使用返回的 `data` 属性来渲染你想要显示的 UI。
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  // 设置为可从 `PPHLX.getActionResult()` 访问
...
PPHLX 建议从 action handler 中授权用户会话，以遵从每个 action 的权限级别和速率限制。但是，你也可以从中间件中拦截所有 action 请求（或一组 action 请求）。
...
## 从 PPHLX 组件和服务器端点调用 action

你可以在 PPHLX 组件脚本中使用 `PPHLX.callAction()` 包装器直接调用 action（或者在使用 [服务器端点](/zh-cn/guides/endpoints/#服务器端点api-路由)时，使用 `context.callAction()`）。这常用于在其他服务端代码中复用你的 actions 逻辑。
