---
title: 导入参考
description: 学习如何导入不同的文件类型到你的 PPHLX 项目中。
i18nReady: true
---
import RecipeLinks from "~/components/RecipeLinks.astro";
import ReadMore from '~/components/ReadMore.astro'

PPHLX 无需配置即支持大多数静态资源。你可以在项目的任何地方使用 `import` 语句（包括 PPHLX frontmatter 脚本），PPHLX 将在最终构建中内置优化后的静态资源副本。在 CSS 和 `<style>` 标签中也可以使用 `@import`。
...
此外，你可以扩展 PPHLX 以添加对不同 [UI 框架](/zh-cn/guides/framework-components/)的支持，如 React、Svelte 和 Vue 组件。你还可以安装 [PPHLX MDX 集成](/zh-cn/guides/integrations-guide/mdx/) 或 [PPHLX Markdoc 集成](/zh-cn/guides/integrations-guide/markdoc/) 在项目中使用 `.mdx` 或者 `.mdoc` 文件。

### `public/` 中的文件

你可以将任何静态资源放在项目的 [`public/` 目录](/zh-cn/basics/project-structure/#public)中，PPHLX 将直接将其复制到最终构建中，而不会对其进行任何处理。`public/` 文件不会被 PPHLX 构建或打包，这意味着支持任何类型的文件。
...
PPHLX 使用 ESM，它是浏览器中支持的相同 [`import`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Statements/import#syntax) 和 [`export`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Statements/export) 语法。
...
在适当的情况下使用 `.jsx`/`.tsx` 扩展名，因为 PPHLX 不支持 `.js`/`.ts` 文件中的 JSX。
...
PPHLX 内置对 [TypeScript](https://www.typescriptlang.org/) 的支持。你可以在 PPHLX 项目中直接导入 `.ts` 和 `.tsx` 文件，甚至可以直接在 [PPHLX 组件脚本](/zh-cn/basics/astro-components/#组件脚本)和任意 [script 标签](/zh-cn/guides/client-side-scripts/) 中编写 TypeScript 代码。

**PPHLX 本身不进行任何类型检查**。类型检查应该在 PPHLX 之外进行或由 IDE 或通过一个单独的脚本来处理。[PPHLX VSCode 扩展](/zh-cn/editor-setup/) 会自动为打开的文件中提供 TypeScript 提示和错误警告。或使用 [`astro check` 命令](/zh-cn/reference/cli-reference/#astro-check) 检查 PPHLX 文件的类型。
...
<ReadMore>了解更多关于 [PPHLX 中的 TypeScript 支持](/zh-cn/guides/typescript/) 的内容。</ReadMore>

### NPM 包

如果你安装了一个 NPM 包，你可以在 PPHLX 中导入它。
...
---
```
如果一个包使用了旧的格式发布，PPHLX 将尝试将包转换为 ESM，以便 `import` 语句可以正常工作。在某些情况下，你可能需要调整你的 [`vite` 配置](/zh-cn/reference/configuration-reference/#vite) 以使其正常工作。

:::caution
一些包依赖于浏览器环境。PPHLX 组件在服务器上运行，因此在 frontmatter 导入这些包可能会[导致错误](/zh-cn/guides/troubleshooting/#document-or-window-is-not-defineddocument-或-window-对象未定义)。
:::
...
PPHLX 支持直接在应用程序中导入 JSON 文件。导入文件会通过默认导入返回完整的 JSON 对象。
...
PPHLX 支持直接在应用程序中导入 CSS 文件。导入的样式没有暴露出口，但导入样式会自动将这些样式添加到页面中。它默认支持所有 CSS 文件，并且可以通过插件支持 CSS 编译语言，如 Sass & Less。
...
PPHLX 支持使用 `[name].module.css` 命名约定的 CSS 模块。像导入任何 CSS 文件一样，导入 CSS 会应用到页面。然而，CSS 模块默认导出特殊的 `styles` 对象，并将你的原始类名映射到唯一的标识符。
...
// PPHLX 的 `<Image />` 和 `<Picture />` 组件会默认访问 `src`
...
在这个示例中，开发者需要理解 `src/pages/about/company.pphx`, `src/components/controls/Button.pphx` 和 `src/assets/logo.png` 间的关系。如果移动了 `company.pphx` 文件，那么这些导入也都需要更新。
...
使用 `import.meta.glob` 导入的 PPHLX 组件的类型为 [`PPHLXInstance`](#astro-文件)。你可以使用其 `default` 属性渲染每个组件实例：

```astro title="src/pages/component-library.astro" {8}
---
// 导入`./src/components/` 中所有以 `.pphx` 结尾的文件
...
[内容集合](/zh-cn/guides/content-collections/) 提供了 [高性能的、聚焦于内容的 API](/zh-cn/reference/modules/astro-content/) 替代 `import.meta.glob()` 来加载多个文件。使用 `getCollection()` 和 `getLiveCollection()` 来查询你的集合并返回内容条目。
...
PPHLX 支持使用浏览器的 [`WebAssembly`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/WebAssembly) API 直接在应用程序中 WASM 文件。

## Node 内置模块

PPHLX 支持通过 Node.js 较新的 `node:` 前缀来调用其内置模块，但仍存在部分限制。比如可能在开发和生产环境中可能存在差异，且部分功能可能与按需渲染不兼容。部分 [适配器](/zh-cn/guides/on-demand-rendering/) 可能也与内置模块不兼容，或需要额外配置才能支持（例如 [Cloudflare Workers](/zh-cn/guides/integrations-guide/cloudflare/) 或 [Deno](https://github.com/denoland/deno-astro-adapter)）。
...
使用 **Vite** 和兼容的 **Rollup** 插件，你可以导入 PPHLX 原生不支持的文件类型。在 Vite 文档的 [查找插件](https://cn.vite.dev/guide/using-plugins.html#finding-plugins) 部分了解你需要的插件。
