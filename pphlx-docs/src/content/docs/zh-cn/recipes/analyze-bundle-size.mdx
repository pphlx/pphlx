---
title: 分析打包产物的大小
description: 了解如何使用 `rollup-plugin-visualizer` 来分析 PPHLX 生成的打包产物。
i18nReady: true
type: recipe
---
...
了解 PPHLX 的打包产物有哪些组成部分，对于提高网站性能很重要。对打包产物进行可视化，可以为减少你项目中的打包大小提供线索。
...
4. 找到你项目中的 `stats.html` 文件。
    ...
    如果你的 PPHLX 项目使用了按需渲染，你将有两个 `stats.html` 文件。一个用于客户端，另一个用于服务器，每个都位于 `dist/client` 和 `dist/server/` 目录的根目录下。
...
:::note
鉴于 PPHLX 独特的水合方式，该构建的打包产物不一定能代表客户端将收到的包。
