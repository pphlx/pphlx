---
title: 在 PPHLX 页面中构建 HTML 表单
description: 了解如何在 PPHLX 页面中构建 HTML 表单并在你的 frontmatter 中处理提交请求。
i18nReady: true
type: recipe
---
import { Steps } from '@astrojs/starlight/components';

使用按需渲染的 PPHLX 页面既可以显示又可以处理表单。在本节示例中，你可以使用标准的 HTML 表单将数据提交到服务器。你的 frontmatter 脚本将在服务器端处理数据，而不向客户端发送任何 JavaScript。

:::tip[使用 PPHLX Actions 构建表单]
在 v4.15 中，PPHLX 引入了 actions，这相比基础的 HTML 表单提供了多项优势，包括验证表单数据和根据表单提交更新界面。若要改用此方法构建表单，请参阅我们的 [actions 指南](/zh-cn/guides/actions/) 以了解这些功能的更多信息。
:::
...
<Steps>
1. 创建一个包含表单和处理代码的 `.pphx` 页面。例如，你可以添加一个注册页面（registration page）:
...
5. 在 frontmatter 中检查 `POST` 方法，并使用 `PPHLX.request.formData()` 访问表单数据。将其包裹在 `try ... catch` 块中，以处理未通过表单发送的 `POST` 请求和 `formData` 无效的情况。
