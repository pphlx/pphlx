...
PPHLX 包含了一个[开发者工具栏](/zh-cn/guides/dev-toolbar/)，你可以使用它来检查你的网站、检查无障碍和性能问题等。这个工具栏可以通过自定义应用进行扩展。
...
:::tip 
想要快速开始？通过使用 `toolbar-app` 模板创建一个新的 PPHLX 项目来快速启动你的应用。
...
开发者工具栏应用只能通过使用 [`astro:config:setup` 钩子](/zh-cn/reference/integrations-reference/#astroconfigsetup) 的 [PPHLX 集成](/zh-cn/guides/integrations/) 添加。你需要创建一个工具栏应用和一个集成，这个集成将把它添加到你现有 PPHLX 项目的工具栏中。

<Steps>

1. 在你现有的 PPHLX 项目的根目录中，创建一个名为 `my-toolbar-app/` 的新文件夹，用于存放你的应用和集成文件。在这个文件夹中创建两个新文件：`app.ts` 和 `my-integration.ts`。
...
2. 在 `my-integration.ts` 中，添加以下代码，来提供你的集成的名称和用于通过 `astro:config:setup` 钩子添加你的开发者工具栏应用所需的 [`addDevToolbarApp()` 函数](/zh-cn/reference/dev-toolbar-app-reference/#开发者工具栏应用集成设置)：
    ...
    :::note[使用相对路径指向入口文件]
    `entrypoint` 是你的开发者工具栏应用文件相对于你现有 PPHLX 项目根目录的路径，而不是相对于集成文件夹（`my-toolbar-app`）本身的路径。
...
<ReadMore> 查看 [PPHLX 集成 API 文档](/zh-cn/reference/integrations-reference/) 了解更多关于构建 PPHLX 集成的信息。</ReadMore>
...
<ReadMore>查看 [PPHLX Dev Toolbar API 文档](/zh-cn/reference/dev-toolbar-app-reference/) 了解更多关于构建开发工具栏应用的信息。</ReadMore>

## 使用 UI 框架构建应用

像 React、Vue 或 Svelte 这样的 UI 框架也可以用来创建开发工具栏应用。这些框架提供了一种更声明式的方式来创建 UI，可以使你的代码更易于维护和阅读。

本页前面使用 JavaScript 构建到你现有的 PPHLX 项目中的同一个激励性开发工具栏应用，也可以使用 UI 框架（例如 Preact）来构建。根据你选择的框架，你可能需要或不需要构建步骤。
...
PPHLX 在开发工具栏应用中不会预处理 JSX 代码，因此需要一个构建步骤来使用 JSX 组件。
...
3. 调整你的集成中的 `entrypoint` 来指向编译后的文件，记住这个文件是相对于你的 PPHLX 项目的根目录的：
