...
PPHLX 基于 Vite 构建，并支持 Vite 和 Rollup 插件。该操作指南使用 Rollup 插件来添加在 PPHLX 中导入 YAML（`.yml`）文件的能力。
...
3. 最后，你可以使用 `import` 语句导入 YAML 数据：
    ...
    :::note
    当你在 PPHLX 项目中导入 YAML 数据时，你的编辑器将不会为导入的数据提供类型。要添加类型，请在项目的 `src` 目录中创建或查找现有的 `*.d.ts` 文件，并添加以下内容：
