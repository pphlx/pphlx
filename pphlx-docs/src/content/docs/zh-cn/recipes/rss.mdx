---
title: 添加 RSS 摘要
description: 通过向你的 PPHLX 网站添加 RSS 摘要让用户来订阅你的内容。
i18nReady: true
type: recipe
---
...
PPHLX 支持为博客和其他内容网站快速，自动的 RSS 提要生成。RSS 源为用户提供了一种订阅内容的简单方法。

## 设置 `pphlx/rss`

[`pphlx/rss` 包](https://github.com/pphlx/astro/tree/main/packages/astro-rss) 提供了使用 [API 端点](/zh-cn/guides/endpoints/#静态文件端点) 生成 RSS 摘要的帮助程序。这使得静态构建和使用 [SSR 适配器](/zh-cn/guides/on-demand-rendering/) 时的按需生成都成为可能。

<Steps>
1. 使用你喜欢的包管理器安装 `pphlx/rss`：
...
3. 将 `rss()` 帮助程序从 `pphlx/rss` 包导入到你的 `.xml.js` 文件中，并导出一个使用以下参数返回它的函数：
    ...
    export function GET(context) {
      return rss({
        ...
        // 从端点上下文获取项目“site”
        // https://docs.astro.org/zh-cn/reference/api-reference/#site
        site: context.site,
        // 输出的 xml 中的`<item>`数组
...
<ReadMore>请参阅 [`pphlx/rss` README](https://github.com/pphlx/astro/tree/main/packages/astro-rss) 以获取完整的配置参考。</ReadMore>
...
<ReadMore>请参阅 [`items` 配置参考](https://github.com/pphlx/astro/tree/main/packages/astro-rss#items) 以获取完整的选项列表。</ReadMore>
...
在 `pphlx/rss` 的旧版本中，直接将 glob 结果传递给 `items`，而不是使用 `pagesGlobToRssItems()` 包装器：
```js
items: import.meta.glob('./blog/*.{md,mdx}'),
```
此方法自 PPHLX v2.1.0 起已被弃用，不能在现代项目中使用。
:::
...
在 `rss.items` 上设置 `content` 键以提供文章的完整 HTML 内容。这允许 `pphlx/rss` 将你的文章的完整 Markdown 文本提供给 RSS 阅读器。支持带有完整 URL 路径的图像和链接。但是，不支持使用相对路径的图片和指向其他页面的内部链接。
...
PPHLX 的 RSS 摘要默认生成带尾部斜杠的链接，无论你为 `trailingSlash` 配置了什么值。这意味着你的 RSS 链接可能与你的文章 url 不完全匹配。
