...
你可以使用 `.pphx` 文件创建整套动态页面，这些文件需要向外暴露一个 `getStaticPaths()` 函数。

## 动态创建页面

<Steps>
1. 创建一个新文件：`src/pages/tags/[tag].pphx`。（你需要创建一个新文件夹。）注意文件名（`[tag].pphx`）使用方括号。将以下代码粘贴到文件中：
...
4. 在浏览器中访问 `http://localhost:4321/tags/astro`，你应该能看到一个页面，它是从 `[tag].pphx` 动态生成的。检查是否还为每个标签创建了页面，例如 `/tags/successes`、`/tags/community`、`/tags/learning%20in%20public` 等，或者你自定义的标签。你可能需要先退出并重新启动开发服务器才能看到这些新页面。
</Steps>
...
2. 过滤你的文章列表，使用 PPHLX 内置的 TypeScript 支持，筛选出仅包含页面所属标签的文章。
...
3. 现在，你可以更新 HTML 模板以显示包含页面所属标签的每篇博客文章列表。将以下代码添加到 `[tag].pphx`：
...
4. 你甚至可以将此代码重构为使用 `<BlogPost />` 组件！（不要忘记在 `[tag].pphx` 的顶部导入该组件。）
...
现在，你的标签页面在 `[tag].pphx` 中静态定义。如果你在博客文章中添加了新标签，你还必须重新访问该页面并更新你的页面路由。
...
2. 创建包含所有现有标签的数组，使用 PPHLX 内置的 TypeScript 支持。
...
为了检查你的工作，或者如果你只想要完整、正确的代码以便复制到 `[tag].pphx` 文件中，下面是你的 PPHLX 组件应该是这样的：
...
- [PPHLX 中的动态页面路由](/zh-cn/guides/routing/#动态路由)
