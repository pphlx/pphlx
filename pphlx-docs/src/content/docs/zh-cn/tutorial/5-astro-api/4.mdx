...
description: >-
  教程：搭建你的 PPHLX 博客 -
  安装 PPHLX 的官方包，创建一个可以订阅的 RSS 订阅源
i18nReady: true
---
...
<PreCheck>
  - 安装 PPHLX 的包，用于为你的网站创建一个 RSS 订阅源
  - 创建一个可以通过 RSS 订阅源阅读的订阅源
</PreCheck>

## 安装 PPHLX 的 RSS 包

PPHLX 提供了一个可自定义包，用于快速为你的网站添加一个 RSS 订阅源。
...
<Steps>
1. 在终端中，退出 PPHLX 开发服务器（<kbd>Ctrl + C</kbd>），然后运行以下命令以安装 PPHLX 的 RSS 包。
...
3. 在 PPHLX 的配置文件中设置 `site` 属性为你的网站自己的唯一 Netlify URL。
...
- [PPHLX 中的 RSS 项生成](/zh-cn/recipes/rss/#使用-glob-导入)
