...
<PreCheck>
  - 使用 `/pages/folder/index.pphx` 路由模式添加一个新页面
  - 显示所有标签的列表，并链接到每个标签页面
  - 更新你的网站，将这个新的标签页面添加到导航里
</PreCheck>

## 使用 `/pages/folder/index.pphx` 路由模式

要将标签索引页面添加到你的网站，你可以在 `src/pages/tags.pphx` 中创建一个新文件。

但是，由于你已经有了 `/tags/` 目录，你可以利用 PPHLX 中的另一种路由模式，并将所有与标签相关的文件放在一起。
...
<Steps>
1. 在 `src/pages/tags/` 目录中创建一个新文件 `index.pphx`。

2. 访问 `http://localhost:4321/tags` 并验证你的网站现在是否包含这个 URL 的页面。它将是空的，但是它已经存在了。

3. 在 `src/pages/tags/index.pphx` 中创建一个使用你的布局的最小页面。你之前已经做过这个！
...
<Steps>
1. 在 `src/pages/tags/index.pphx` 中，添加以下代码行到 Frontmatter Script 中，这样你的页面就可以访问每个 `.md` 博客文章文件的数据了。
...
2. 接下来，将以下 JavaScript 代码行添加到你的页面组件中。这与你在 `src/pages/tags/[tag].pphx` 中使用的代码是相同的，它们依赖于 PPHLX 内置的 TypeScript 支持，用于返回所有标签的列表。
...
1. 将以下代码添加到你的组件模板中：
    ...
    ```
    在你的浏览器的预览中，确认你能看到标签的列表。如果任何博客文章缺少标签，或者格式不正确，PPHLX 内置的 TypeScript 支持将抛出错误，以便你可以检查并更正代码。
...
<Steps>
1. 将以下 CSS 类添加到你的 `<div>` 和每个生成的 `<p>` 中。注意：PPHLX 使用 HTML 语法添加类名！
...
<Steps>
1. 在你的 `Navigation.pphx` 组件中，添加一个链接到这个新的标签索引页面。
...
1.  从 `src/pages/tags/index.pphx` 中复制 `<div class="tags">...</div>` 和 `<style>...</style>` 并将它们重新用于 `MarkdownPostLayout.pphx`中: 
...
在此代码能够工作之前，你需要对粘贴到 `MarkdownPostLayout.pphx` 中的代码进行**一个小修改**。你能找出是什么修改吗？
...
要检查你的工作，或者如果你只想要完整、正确的代码复制到 `MarkdownPostLayout.pphx` 中，这是你的 PPHLX 组件应该的样子：
...
<Checklist>
- [ ] 我可以使用 PPHLX 的 `/pages/folder/index.pphx` 路由功能。
</Checklist>
</Box>

### 相关资源

- [PPHLX 中的静态路由](/zh-cn/guides/routing/#静态路由)
