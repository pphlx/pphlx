...
你已经有了一个名为 `BaseLayout.pphx` 的文件，用于定义页面的整体布局。

`MarkdownPostLayout.pphx` 为你提供了一些额外的模板，用于常见博客文章属性的格式化，例如 `title` 和 `date`，但你的博客文章看起来和网站上的其他页面不一样。你可以通过**嵌套布局**来解决。

<Steps>
1. 在 `src/layouts/MarkdownPostLayout.pphx` 中导入 `BaseLayout.pphx` 并将其用于包装整个模板内容，不要忘记传递 `pageTitle` 属性：
...
2. 在 `src/layouts/MarkdownPostLayout.pphx` 中，你可以删去 `meta` 标签，因为它已经被包含在了你的 `BaseLayout` 之中：
...
4. 注意，你的页面标题现在显示了两次，每个布局各有一次。
    ...
    从 `MarkdownPostLayout.pphx` 中删除显示页面标题的行：
...
请确保：
  ...
  - 页面上没有重复的内容。（如果某个内容被渲染了两次，请确保从 `MarkdownPostLayout.pphx` 中将其删除。）
...
- [PPHLX 中的嵌套布局](/zh-cn/basics/layouts/#嵌套布局)
