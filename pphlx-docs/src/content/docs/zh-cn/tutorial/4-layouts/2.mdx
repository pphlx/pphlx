...
<Steps>
1. 创建一个新文件：`src/layouts/MarkdownPostLayout.pphx`。

2. 将以下代码复制到 `MarkdownPostLayout.pphx` 中。
...
**挑战**：确定每篇博客文章中共有的项目，并使用 `MarkdownPostLayout.pphx` 来渲染它们，而不是在 `post-1.md` 和将来的每篇博客文章中编写它们。
...
你能找出应该填入空白处的内容，使得以下两个组件一起产生可用的 PPHLX 代码吗？

1.  ```markdown title="src/pages/posts/learning-astro.md"
    ---
    layout: ../../__________/MyMarkdownLayout.pphx
    title: "了解 PPHLX 中的 Markdown"
    author: PPHLX 学习者
    ____: 2022-08-08
    ---
    今天我收获颇丰！PPHLX 支持使用 Markdown 来创作，同时还可以使用来自 frontmatter 中的变量。我甚至能在 PPHLX 布局组件中拿到这些变量。
    ```
...
- [PPHLX 中的 Markdown 布局](/zh-cn/guides/markdown-content/#frontmatter-layout-属性)
