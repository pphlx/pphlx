...
<PreCheck>
  - 在你的 PPHLX 项目中添加 Preact
  - 在主页上包含 PPHLX 岛屿（Preact `.jsx` 组件）
  - 使用 `client:` 指令使岛屿可交互
</PreCheck>
...
<Steps>
1. 如果运行着 PPHLX 开发服务器，请退出以便使用终端（快捷键：<kbd>Ctrl + C</kbd>）。

2. 通过以下单个命令，为你的 PPHLX 项目添加使用 Preact 组件的能力：
...
1. 在 `src/components/` 目录下创建一个名为 `Greeting.jsx` 的新文件。
    ...
    注意 `.jsx` 文件扩展名。这个文件将使用 Preact 编写，而不是 PPHLX。
...
4. 添加第二个 `<Greeting />` 组件，并加上 `client:load` 指令。
  ...
  5. 查看你的页面并比较这两个组件。第二个按钮可工作，因为 `client:load` 指令告诉 PPHLX 在页面加载时将其 JavaScript 发送并重新运行到客户端上，使组件可交互。这称为**被注水的**组件。
...
- [PPHLX 集成指南](/zh-cn/guides/integrations/)
- [在 PPHLX 中使用 UI 框架组件](/zh-cn/guides/framework-components/#使用框架组件)
- [PPHLX client 指令参考](/zh-cn/reference/directives-reference/#客户端指令)
