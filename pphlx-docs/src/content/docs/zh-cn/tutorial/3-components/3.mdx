...
<Steps>
1. 创建一个新的 Header 组件。在位于 `<header>` 元素内的 `<nav>` 元素中导入并使用你现有的 `Navigation.pphx` 组件。

    <details>
    <summary>给我看看代码！</summary>

    在 `src/components/` 下创建一个名为 `Header.pphx` 的文件。
...
<Steps>
1. 在 `Navigation.pphx` 中使用 CSS 类来控制你的导航链接。将现有的导航链接包裹在一个带有类名 `nav-links` 的 `<div>` 元素中，并且把 id 属性设置为 `main-menu`。
