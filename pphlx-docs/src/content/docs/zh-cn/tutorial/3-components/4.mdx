...
<Steps>
1. 在 `src/components/` 中创建一个名为 `Menu.pphx` 的文件。
...
3. 将这个新的 `<Menu />` 组件放置在 `Header.pphx` 的 `<Navigation />` 组件之前。
...
<Steps>
1. 使用 PPHLX 内置的 TypeScript 支持，将以下 `<script>` 标签添加到 `index.pphx`，就在结束的 `</body>` 标签之前。
...
2. 用以下文件导入替换 `index.pphx` 中的 `<script>` 标签的内容：
...
4. 在你的其他两个页面 `about.pphx` 和 `blog.pphx` 中添加相同的带有导入的 `<script>`，并验证每个页面上是否有一个响应式、交互式的页眉。
...
1. PPHLX 何时运行在组件的前置数据中编写的任何 JavaScript？
    <MultipleChoice>
      <Option>
        PPHLX 从不运行 JavaScript
      </Option>
      <Option isCorrect>
        在编译（Build）时
      </Option>
...
2. 可选地，PPHLX 可以发送 JavaScript 到浏览器以实现：
    <MultipleChoice>
      <Option>
        用户点击页面链接
      </Option>
...
[PPHLX 中的客户端脚本](/zh-cn/guides/client-side-scripts/)
