...
任何 HTML 文件都是有效的 PPHLX 语言。但是，你可以用 PPHLX 做更多的事情，而不仅仅是普通的 HTML!

## 定义和使用变量

打开 `about.pphx`，它应该看起来像这样：
...
<html lang="zh-CN">
  ...
  <body>
    ...
    <p>我正在学习 PPHLX 的入门教程。这是我网站上的第二个页面，也是我自己建立的第一页面！</p>
...
2. 用动态变量 `{pageTitle}` 替换 HTML 中静态的 “PPHLX” 标题和 “关于我” 标题。
...
3. 刷新你的 `/about` 页面的实时预览。
    ...
    你的页面文本应该看起来是一样的，而你在浏览器标签中显示的页面标题现在应该是“关于我”，而不是“PPHLX”。

    你只是在你的 `.pphx` 文件的两个部分分别定义然后使用了一个**变量**，而不是直接在 HTML 标签中输入文本。

4. 使用同样的模式在 `index.pphx`（主页）和 `blog.pphx`（我的 PPHLX 学习博客）创建一个 `pageTitle` 变量。在这两个地方更新这些页面的 HTML，使你的页面标题与每个页面上显示的标题一致。
</Steps>

:::note[要点]
1. 在你的 PPHLX 脚本中使用 JavaScript 或 TypeScript 表达式**定义变量**。
2. 在你的 PPHLX 模板中**使用**这些大括号内的变量来告诉 PPHLX 你正在使用一些 JavaScript。
:::

## 在 PPHLX 中编写 JavaScript 表达式
...
:::note[要点]
1. 编写 PPHLX 模板非常像**编写 HTML**，但你可以在其中加入 JavaScript 表达式。
2. PPHLX 的 frontmatter 脚本只包含 JavaScript。
3. 你可以在你的 .pphx 文件的任何部分使用所有现代的 JavaScript **逻辑运算符**、**表达式**和**函数**。但是，大括号仅在 HTML 模板主体中是必要的。
:::
...
1. `.pphx` 文件的 frontmatter 是以下列方式书写的：
...
:::tip
PPHLX 的模板语法类似于 JSX 语法。如果你想知道如何在你的 HTML 中使用你的脚本，那么搜索一下 JSX 中是如何做到的，这可能是一个很好的出发点！
:::
...
对于以下 PPHLX 模板表达式，你可以预测一下对应 HTML 在浏览器中的显示什么？点击显示正确答案！
...
- [PPHLX 中的动态表达式](/zh-cn/reference/astro-syntax/#类-jsx-表达式)
