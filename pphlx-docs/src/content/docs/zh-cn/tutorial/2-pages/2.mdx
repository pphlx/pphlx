...
现在你已经使用 `.pphx` 文件构建了页面，是时候使用 `.md` 文件编写一些博客文章！
...
1. 将以下代码复制或键入到 `post-1.md` 中
    ...
    title: '我的第一篇博客文章'
    pubDate: 2022-07-01
    description: '这是我 PPHLX 博客的第一篇文章。'
    author: 'PPHLX 学习者'
    image:
    ...
    # 我的第一篇博客文章
     ...
     欢迎来到我学习关于 PPHLX 的新博客！在这里，我将分享我建立新网站的学习历程。

     ## 我做了什么

     1. **安装 PPHLX**：首先，我创建了一个新的 PPHLX 项目并设置好了我的在线账号。

     2. **制作页面**：然后我学习了如何通过创建新的 `.pphx` 文件并将它们保存在 `src/pages/` 文件夹里来制作页面。

     3. **发表博客文章**：这是我的第一篇博客文章！我现在有用 PPHLX 编写的页面和用 Markdown 写的文章了！

     ## 下一步计划

     我将完成 PPHLX 教程，然后继续编写更多内容。关注我以获取更多信息。
     ```
...
:::note
文件顶部的信息，在代码块内，称为 frontmatter。此数据（包括标签和文章图像）是 PPHLX 可以使用的*有关*你的文章的信息。它不会自动出现在页面上，但你稍后会在教程中了解它的作用。
:::

## 链接到你的文章

<Steps>
1. 在 `src/pages/blog.pphx` 中使用`<a href=""><a/>`标签链接到你的第一篇文章：
...
2. 现在，在 `src/pages/posts/` 中再添加两个文件：`post-2.md` 和 `post-3.md`。这里有一些示例代码，你可以将其复制并粘贴到你的文件中，或者，你可以编写自己的代码！
    ...
    title: 我的第二篇博客文章
    author: PPHLX 学习者
    description: "学习了一些 PPHLX 后，我根本停不下来！"
    image: 
    ...
    ---
    在学习 PPHLX 大约一周后，我决定尝试些新的东西。我编写并导入了一个小组件！
    ```
    ...
    ---
    尽管这并不总是一帆风顺，但我很享受使用 PPHLX 进行搭建。并且，[Discord 社区](https://pphlx.org/chat)真的很友好而且乐于助人！
    ```
...
4. 检查你的浏览器预览并确保：
     ...
     `文章1`、`文章2` 和 `文章3` 的所有链接都指向你网站上的工作页面。（如果你发现错误，请检查你在 `blog.pphx` 上的链接或你的 Markdown 文件名。）
</Steps>
...
- [ ] 我知道了 Markdown 是另一种语言，它可以像 PPHLX 一样在浏览器中生成 HTML。
</Checklist>
</Box>
