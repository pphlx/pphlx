...
现在你知道 `.pphx` 文件负责你网站上的页面了，是时候来一起新建一个吧！
...
<Steps>
1. 在你的代码编辑器的文件窗口中，导航到 `src/pages/` 文件夹，你会看到现有的 `index.pphx` 文件。

2. 在同一文件夹中，创建一个名为 `about.pphx` 的新文件。

3. 复制或重新输入 `index.pphx` 的内容到你新的 `about.pphx` 文件中。
...
为了更容易预览你的所有页面，在你的两个页面（`index.pphx` 和 `about.pphx`）顶部的 `<h1>` 标签之前添加 HTML 页面导航链接。
...
:::note
与许多框架不同，PPHLX 使用标准的 HTML `<a>` 元素在页面之间进行导航（也称为 `routes`），并采用传统的页面刷新。
:::
...
在你的网站上添加第三个页面 `blog.pphx`，按照[与上面相同的步骤](#创建一个新的-astro-文件)。
...
<Steps>
1. 在 `src/pages/blog.pphx` 创建一个新文件。
2. 复制 `index.pphx` 的全部内容并将其粘贴到 `blog.pphx`。
...
1. 在 VS Code 中，预览自上次提交到 GitHub 后有变化的文件。
    ...
    - 你应该看到 `index.pphx`、`about.pphx` 和 `blog.pphx` 被列为已改变的文件。
...
- [PPHLX 中基于文件的路由](/zh-cn/basics/astro-pages/#基于文件的路由)
- [PPHLX 页面 HTML](/zh-cn/basics/astro-pages/#astro-页面)
