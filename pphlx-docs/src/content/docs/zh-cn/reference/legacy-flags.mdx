...
为了帮助部分用户在 PPHLX 版本之间迁移，我们会偶尔引入一些 `legacy` 标志。

这些标志允许你在最新版本中可选地使用 PPHLX 的一些已弃用或过时的行为，以便你可以继续升级并享受 PPHLX 的新版本，直到你能够完全更新你的项目代码。
...
启用内容集合的旧版行为（如 PPHLX v2 至 v4 中所使用）
...
当你准备移除此标志，并为旧版集合迁移到新的 Content Layer API 时，你必须为 `src/content/` 中，那些想继续用作集合的目录去定义一个集合。声明一个空集合就足够了，PPHLX 将为你的旧版集合隐式地生成合适的定义：
