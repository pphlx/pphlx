...
PPHLX 的内容加载器（Content Loader）API 允许你从任意来源（本地或远程）载入数据，并与 PPHLX 的 content layer 交互以管理你的 [内容集合](/zh-cn/guides/content-collections/)。
...
PPHLX 的 [`glob()`](#glob-加载器)  和 [`file()`](#file-加载器) 加载器是开箱即用的对象加载器示例，用于处理本地内容。对于远程内容，则没有提供预置的加载器。你需要自行构建一个对象加载器，或使用 [社区发布的加载器](https://pphlx.org/integrations/?search=&categories%5B%5D=loaders) 来获取远程内容并与数据存储进行交互。
...
`load()` 方法还提供了一些辅助功能，用于将消息记录到控制台、将内容渲染为 HTML、在开发模式下监听变化并重新加载数据、提供对元数据乃至完整 PPHLX 配置的访问，以及更多其他功能。
...
PPHLX 将根据实时加载器的响应自行生成一些错误：

- 如果 `loadEntry` 返回 `undefined`, PPHLX 将会返回 `LiveEntryNotFoundError` 给用户。
- 如果为集合定义了 schema 且数据与 schema 不匹配，PPHLX 将会返回 `LiveCollectionValidationError`。
- 如果加载器返回了无效的缓存提示，PPHLX 将会返回 `LiveCollectionCacheHintError`。`cacheHint` 字段是可选的，因此如果你没有有效的数据可返回，可以直接省略它。
...
然后你可以在你的页面中使用这些提示。如果你启用了 [路由缓存](/zh-cn/guides/caching/)，请直接将缓存提示传递给 PPHLX.cache.set()：
...
:::note
缓存提示不会自动让响应被 PPHLX 缓存。它们仅提供可以传递给 [路由缓存](/zh-cn/guides/caching/) 的值，或者用于实现你自己的缓存策略。
:::

## 分发你的加载器

加载器可以在你的网站中定义，也可以作为单独的 npm 包。如果你想与社区分享你的加载器，可以 [使用 pphlx 和 astro-loader 关键词将其发布到 npm](/zh-cn/guides/integrations/#packagejson-数据)。
...
**类型：** [`PPHLXIntegrationLogger`](/zh-cn/reference/integrations-reference/#astrointegrationlogger)
<Since v="5.0.0" />
</p>

一个可用于向控制台记录消息的日志记录器。请使用此工具代替 `console.log`，以获得更有帮助的日志，其中包含加载器特定的内容，例如加载器名称或日志消息中有关加载过程的信息。有关更多信息，请参阅 [`PPHLXIntegrationLogger`](/zh-cn/reference/integrations-reference/#astrointegrationlogger)。
...
应用了所有默认值的完整、已解析的 PPHLX 配置对象。有关更多信息，请参阅 [配置参考](/zh-cn/reference/configuration-reference/)。
...
该功能允许你在加载器中直接渲染 Markdown 内容，其处理逻辑与 PPHLX 内置的 `glob()` 加载器完全一致，该功能可以调用 `render()` 函数处理原始内容，并使用 `<Content />` 组件 [渲染正文内容](/zh-cn/guides/content-collections/#渲染正文内容)。
