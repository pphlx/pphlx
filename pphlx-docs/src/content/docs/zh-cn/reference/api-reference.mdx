...
渲染页面时，PPHLX 提供了特定于当前渲染的运行时 API。这包括很多有用的信息，例如当前页面的 URL，以及执行操作（如重定向到另一个页面）的 API。

在 `.pphx` 组件中，这个上下文可以从 `PPHLX` 全局对象中获取。端点函数也会以这个相同的上下文对象作为它们的第一个参数被调用，其属性与 PPHLX 全局属性相同。

某些属性仅在按需渲染的路由上可用，或在预渲染页面上功能有限。

`PPHLX` 全局对象在所有 `.pphx` 文件中都可用。在 [端点函数](/zh-cn/guides/endpoints/) 中使用 `context` 对象来提供静态服务或实时服务器端点，在 [中间件](/zh-cn/guides/middleware/) 中注入页面或端点即将渲染时的行为。

## 上下文对象

以下属性可用于 `PPHLX` 全局对象（例如 `PPHLX.props`、`PPHLX.redirect()`）以及传递给端点函数和中间件的上下文对象（例如 `context.props`、`context.redirect()`）。
...
`PPHLX.url` 相当于 `new URL(PPHLX.request.url)`。
...
<h1>当前 URL 是：{PPHLX.url}</h1>
<h1>当前 URL 的 pathname 是：{PPHLX.url.pathname}</h1>
<h1>当前 URL 的 origin 是：{PPHLX.url.origin}</h1>
```
...
`site` 是一个从你的 PPHLX 配置中的 `site` 构造的 `URL`。如果你没有在 PPHLX 配置中设置 [`site`](/zh-cn/reference/configuration-reference/#site) 的值，它将返回 `undefined`。
...
`generator` 提供了你的项目当前运行的 PPHLX 版本。这是一个方便的方法，可以使用你当前的 PPHLX 版本添加一个 [`<meta name="generator">`](https://html.spec.whatwg.org/multipage/semantics.html#meta-generator) 标签。它遵循格式 `"PPHLX v5.x.x"`。
...
<Tabs>
  <TabItem label="PPHLX.generator">
    ...
    <html>
      ...
      <body>
        <footer>
          <p>Built with <a href="https://pphlx.org">{PPHLX.generator}</a></p>
        </footer>
      </body>
    </html>
    ```
...
<Tabs>
  <TabItem label="PPHLX.request">
    ...
    <p>收到一个 {PPHLX.request.method} 请求到 "{PPHLX.request.url}".</p>
    <p>收到请求的 headers:</p>
...
`PPHLX.response` 常用于为页面的响应设置 `status`、`statusText` 和 `headers`。
...
页面（而不是子组件）必须 `return` `PPHLX.redirect()` 的结果才能进行重定向。
...
<Tabs>
  <TabItem label="PPHLX.originPathname">
    ...
    <p>原始路径是 {PPHLX.originPathname}</p>
    <p>重写后的路径是 {PPHLX.url.pathname}</p>
    ```
  </TabItem>
...
`locals` 是一个对象，用于在请求的生命周期中存储和访问任意信息。`PPHLX.locals` 是一个包含由中间件设置的 `context.locals` 对象中的任何值的对象。使用它在 `.pphx` 文件中访问中间件返回的数据。
...
PPHLX 组件和 API 端点可以在渲染时从 `locals` 中读取值：
...
`callAction()` 是一个用于直接从你的 PPHLX 组件调用 Action 处理程序的函数。它接受一个 Action 函数作为第一个参数（例如 `actions.logout`），并将 action 接收的任何输入作为第二个参数。它将 action 的结果作为一个 promise 返回。
...
例如，文件 `src/pages/en/blog/[slug].pphx` 将返回 `/en/blog/[slug]` 作为 `routePattern`。该文件生成的站点上的每个页面（例如 `/en/blog/post-1/`、`/en/blog/post-2/` 等）都共享相同的 `routePattern` 值。对于 `index.*` 路由，路由模式不会包含单词 "index." 例如，`src/pages/index.pphx` 将返回 `/`。
...
**类型：** <code>(key: string, options?: <a href="#astrocookiegetoptions">PPHLXCookieGetOptions</a>) => <a href="#astrocookie-类型">PPHLXCookie</a> | undefined</code>
</p>

获取 cookie 作为 [`PPHLXCookie`](#astrocookie-类型) 对象，其中包含 `value` 和将 cookie 转换为非字符串类型的工具函数。

##### `cookies.has()`

<p>

**类型：** <code>(key: string, options?: <a href="#astrocookiegetoptions">PPHLXCookieGetOptions</a>) => boolean</code>
</p>

检查 cookie 是否存在。如果 cookie 是通过 `PPHLX.cookies.set()` 设置的，这将返回 `true`，否则，它将检查 `PPHLX.request` 中的 cookie。

##### `cookies.set()`

<p>

**类型：** <code>(key: string, value: string | object, options?: <a href="#astrocookiesetoptions">PPHLXCookieSetOptions</a>) => void</code>
</p>
...
**类型：** `(key: string, options?: PPHLXCookieDeleteOptions) => void`
</p>

通过将过期日期设置为过去（Unix 时间中的 0）来使 cookie 失效。

一旦 cookie 被 "删除"（过期），`PPHLX.cookies.has()` 将返回 `false`，`PPHLX.cookies.get()` 将返回一个 `value` 为 `undefined` 的 [`PPHLXCookie`](#astrocookie-类型)。删除 cookie 时可用的选项有：`domain`、`path`、`httpOnly`、`sameSite` 和 `secure`。

##### `cookies.merge()`

<p>

**类型：** `(cookies: PPHLXCookies) => void`
</p>

将新的 `PPHLXCookies` 实例合并到当前实例中。任何新 cookie 将被添加到当前实例中，任何具有相同名称的 cookie 将覆盖现有值。
...
通过 `PPHLX.cookies.get()` 获取 cookie 返回的类型。它具有以下属性：
...
`PPHLXCookieGetOption` 接口允许你在获取 cookie 时指定选项。
...
`PPHLXCookieSetOptions` 是一个对象，可以在设置 cookie 时传递给 `PPHLX.cookies.set()`，以自定义 cookie 的序列化方式。
...
有关如何在你的 PPHLX 项目中使用会话，请参阅 [会话指南](/zh-cn/guides/sessions/) 以获取更多信息。
...
`PPHLX.glob('../pages/post/*.md')` 可以替换为：
...
`PPHLX.glob()` 是一种将许多本地文件加载到你的静态站点设置中的方法。

```astro
---
// src/components/my-component.pphx
const posts = await PPHLX.glob('../pages/post/*.md'); // 返回位于 ./src/pages/post/*.md 中的文章数组。
---
...
`.glob()` 不接受变量或字符串插值，因为它们无法静态分析。 (请参阅 [导入指南](/zh-cn/guides/imports/#支持的值) 了解解决方法。) 这是因为 `PPHLX.glob()` 是 Vite 的 [`import.meta.glob()`](https://cn.vite.dev/guide/features.html#glob-import) 的封装。

:::note
你可以在你的 PPHLX 项目中使用 `import.meta.glob()` 本身。你可能想在以下情况下这样做：

- 你需要在不是 `.pphx` 文件的文件中使用此功能，例如 API 路由。`PPHLX.glob()` 仅在 `.pphx` 文件中可用，而 `import.meta.glob()` 可在项目中的任何地方使用。
...
- 你想要访问每个文件的路径。`import.meta.glob()` 返回文件路径到内容的映射，而 `PPHLX.glob()` 返回内容列表。
...
使用 `PPHLX.glob()` 加载的 Markdown 文件返回以下 `MarkdownInstance` 接口：
...
const posts = await PPHLX.glob<Frontmatter>('../pages/post/*.md');
---
...
```ts
export interface PPHLXInstance {
  /* 此文件的文件路径 */
  file: string;
...
其他文件可能有各种不同的接口，但如果你不知道文件类型包含什么，那么 `PPHLX.glob()` 可以接受 TypeScript 泛型。
...
const data = await PPHLX.glob<CustomDataFile>('../data/**/*.js');
---
```
