...
**PPHLX 集成**只需几行代码就能为你的项目添加新的功能和行为。

这个参考页是为任何想要编写集成的人准备的。要学习如何在项目中使用集成，请查看我们的[使用集成](/zh-cn/guides/integrations/)指南。

## 示例

当你创建自己的集成时，可以参考官方的 PPHLX 集成。
...
PPHLX 提供了集成可以实现的钩子，以便在 PPHLX 生命周期的某些部分执行。PPHLX 钩子被定义在 `IntegrationHooks` 接口中，该接口是全局 `PPHLX` 命名空间的一部分。每个钩子都有一个 [`logger` 选项](#astrointegrationlogger)，用于允许你使用 PPHLX 记录器来记录日志。
...
**时机：** 初始化时，在 [Vite](https://cn.vite.dev/config/) 或[PPHLX 配置](/zh-cn/reference/configuration-reference/) 解析前。

**用途：** 扩展项目配置。包括更新 [PPHLX 配置](/zh-cn/reference/configuration-reference/)、应用 [Vite 插件](https://cn.vite.dev/guide/api-plugin)、添加组件渲染器，以及向页面注入脚本。
...
用户提供的 [PPHLX 配置](/zh-cn/reference/configuration-reference/)只读副本。这是在任何其他集成运行之前进行解析的。如果在所有集成完成配置更新后需要配置副本，[见`astro:config:done` 钩子](#astroconfigdone)。
...
**类型：** `(newConfig: DeepPartial<PPHLXConfig>) => PPHLXConfig;`
</p>

用来更新用户提供的[PPHLX 配置](/zh-cn/reference/configuration-reference/)的回调函数。你提供的任何配置**将与用户配置 + 其他集成配置的更新合并**，所以你可以随意省略键名！
...
**类型：** <code>(renderer: <a href="#astrorenderer">PPHLXRenderer</a>) => void;</code><br />
**示例：** [`svelte`](https://github.com/pphlx/astro/blob/main/packages/integrations/svelte/src/index.ts)、[`react`](https://github.com/pphlx/astro/blob/main/packages/integrations/react/src/index.ts)、[`preact`](https://github.com/pphlx/astro/blob/main/packages/integrations/preact/src/index.ts)、[`vue`](https://github.com/pphlx/astro/blob/main/packages/integrations/vue/src/index.ts)、[`solid`](https://github.com/pphlx/astro/blob/main/packages/integrations/solid/src/index.ts)
</p>
...
如果你的集成依赖于一些 Vite 不监听或者需要完整的开发服务器重启才能生效的配置文件，请用`addWatchFile()` 添加它。每当该文件发生变化，PPHLX 开发服务器将重新加载（你可以用 [`isRestart`](#isrestart-选项) 检查何时发生重新加载）。
...
添加一个 [自定义客户端指令](/zh-cn/reference/directives-reference/#自定义客户端指令)，用于在 `.pphx` 文件中使用。
...
**类型：** <code>(middleware: <a href="#astrointegrationmiddleware">PPHLXIntegrationMiddleware</a>) => void;</code><br />
<Since v="3.5.0" />
</p>
...
用于向 PPHLX 项目注入路由的回调函数。注入的路由可以是 [`.pphx`页面](/zh-cn/basics/astro-pages/) 或 [`.js`和`.ts`路由处理程序](/zh-cn/guides/endpoints/#静态文件端点)。

`injectRoute()` 接收带有 `pattern` 和 `entrypoint` 的对象值。

- `pattern` - 应该在浏览器中使用的路由，例如 `/foo/bar`。`pattern` 可以使用 PPHLX 的文件路径语法来表示动态路由，例如 `/foo/[bar]` 或 `/foo/[...bar]`。请注意，在 `pattern` 中**无需**文件扩展名。
- `entrypoint` — 裸模块指定器，指向 `.pphx` 页面或 `.js`/`.ts` 路由处理程序，处理`pattern` 中指定路由。
- `prerender` - 如果 PPHLX 无法检测到你的 `prerender` 导出，则设置一个布尔值。

##### 使用示例

```js
injectRoute({
  // 使用 PPHLX 语法模式来匹配动态路由
  pattern: '/subfolder/[dynamic]',
  // 使用相对路径语法来匹配本地路由
  entrypoint: './src/dynamic-page.astro',
  // 仅当 PPHLX 无法检测到你的预渲染导出时才使用
  prerender: false
});
```
...
当你将你的包（在这个例子中是 `@fancy/dashboard`）发布到 npm 上时，你必须在你的 `package.json` 中导出 `dashboard.pphx`：
...
- `"page"`：与 `head-inline` 类似，只是注入片段由 Vite 进行处理，并与页面上 PPHLX 组件内定义的其他 `<script>` 标签捆绑在一起。该脚本将在最终的页面输出中以 `<script type="module">` 的形式加载，并由 Vite 压缩和解析。
- `"page-ssr"`：在每个 PPHLX 页面组件的 frontmatter 中作为单独的模块被导入。因为这个阶段导入你的脚本，无法使用全局 `PPHLX`，脚本只会在 `import` 第一次 evaluate 时运行一次。
...
一个函数，用于创建 `<root>/.pphx/integrations/<normalized_integration_name>` 文件夹并返回其路径。

它允许你拥有一个专用文件夹，避免与其他集成或 PPHLX 本身发生冲突。该目录是通过调用此函数创建的，因此可以安全地直接向其中写入文件：
...
**时机：** 在 PPHLX 配置解析后，其他集成已经运行 `astro:config:setup` 钩子后。
...
用户提供的 [PPHLX 配置](/zh-cn/reference/configuration-reference/) 的只读副本。这在其他集成 _运行后_ 进行解析。

#### `setAdapter()` 选项

<p>

**类型：** `(adapter: PPHLXAdapter) => void;`
</p>
...
`filename` 属性将用于在 `/.pphx/integrations/<normalized_integration_name>/<normalized_filename>.d.ts` 生成文件，必须以 `".d.ts"` 结尾。
...
加载器稍后可以通过读取 `refreshContextData` 属性来获得 webhook body。更多信息请参阅 [`refreshContextData`](/zh-cn/reference/content-loader-reference/#loadercontextrefreshcontextdata) 属性。
...
PPHLX 为未来的内置钩子保留了 `astro:` 前缀。请在命名时，为自定义钩子选择不同的前缀。
...
PPHLX 日志记录器的实例，用于写日志。此日志记录器使用与 CLI 配置的相同的[日志级别](/zh-cn/reference/cli-reference/#--verbose)。
...
* `src/pages/index.pphx` 具有这样的格式 `/`
* `src/pages/blog/[...slug].pphx` 具有这样的格式 `/blog/[...slug]`
* `src/pages/site/[blog]/[...slug].pphx` 具有这样的格式 `/site/[blog]/[...slug]`
...
例如，`[fruit]/about.pphx` 将生成这样的格式：`/^\/([^/]+?)\/about\/?$/`，那么，`pattern.test("banana/about")` 就会返回 `true`。
...
* `src/pages/index.pphx` 会是 `/`
* `src/pages/blog/[...slug].pphx` 会是 `/blog/[...slug]`
* `src/pages/site/[blog]/[...slug].pphx` 会是 `/site/[blog]/[...slug]`
...
例如，对于诸如 `/blog/[...id].pphx` 这样的路由，`generate()` 函数将返回如下内容：
...
允许你读取路由的 `params`。例如，当一个项目使用以下的 [动态路由](/zh-cn/guides/routing/#动态路由) `/pages/[lang]/[...slug].pphx`时，该值将会是 `['lang', '...slug']`。
...
例如，给定一个 `[fruit]/about.pphx` 路径，正则表达式将是 `/^/([^/]+?)/about/?$/`。使用 `pattern.test("banana/about")` 将会返回 `true`。
...
例如，以下的路由  `/pages/[blog]/[...slug].pphx` 将会输出这样的分段（segments）：
...
指定路由是否为目录索引（例如 `src/pages/index.pphx`、`src/pages/blog/index.pphx`）。
...
判断路由是来自 PPHLX 核心（`internal`）、集成（`external`）还是用户项目（`project`）。
...
* `page`: 位于文件系统中的路由，通常是一个 PPHLX 组件
...
关于如何从该组件渲染 head 内容的描述，包括 PPHLX 运行时是否需要等待组件：
- `none`：该组件不传递 head 内容。
- `self`：该组件追加 head 内容。
...
描述服务器可用的渲染器。这是 [`PPHLXRenderer`](#astrorenderer) 的一个子集，包含额外的属性。
...
指示渲染器是否支持 PPHLX 的静态插槽优化。当为 true 时，PPHLX 会防止移除岛屿内的嵌套插槽。
...
**类型：** [`PPHLXConfig['trailingSlash']`](/zh-cn/reference/configuration-reference/#trailingslash)<br />
<Since v="3.5.4" />
</p>
...
**类型：** [`NonNullable<PPHLXConfig['build']>['format']`](/zh-cn/reference/configuration-reference/#buildformat)<br />
<Since v="4.2.2" />
</p>
...
指定 [PPHLX 生成的资源链接的配置前缀](/zh-cn/reference/configuration-reference/#buildassetsprefix)。
...
在用户项目中定义的 [PPHLX 中间件函数](/zh-cn/reference/modules/astro-middleware/#middlewarehandler)。
...
**类型：** <code>() => Promise\<<a href="#astromiddlewareinstance">PPHLXMiddlewareInstance</a>\> | <a href="#astromiddlewareinstance">PPHLXMiddlewareInstance</a></code><br />
<Since v="4.2.5" />
</p>
...
指定 [PPHLX 读取站点的配置目录](/zh-cn/reference/configuration-reference/#srcdir)。
...
描述 [内容安全策略配置](/zh-cn/reference/configuration-reference/)。
...
指定 [配置的哈希函数](/zh-cn/reference/configuration-reference/)。
...
指定项目脚本的生成哈希列表和外部脚本的 [用户提供哈希](/zh-cn/reference/configuration-reference/)。
...
指定一个有效源列表，该列表结合了 [已配置的脚本资源](/zh-cn/reference/configuration-reference/) 和 [注入的脚本资源](/zh-cn/reference/api-reference/)。
...
确定 [配置中是否启用了对动态脚本注入的支持](/zh-cn/reference/configuration-reference/)。
...
指定项目样式的生成哈希列表和外部样式的 [用户提供哈希列表](/zh-cn/reference/configuration-reference/)。
...
指定一个有效来源列表,该列表结合 [了已配置的样式资源](/zh-cn/reference/configuration-reference/) 和 [注入的样式资源](/zh-cn/reference/api-reference/)。
...
指定特定内容类型的 [有效来源的配置列表](/zh-cn/reference/configuration-reference/)。
...
- [构建你自己的 PPHLX 集成](https://www.freecodecamp.org/news/how-to-use-the-astro-ui-framework/#chapter-8-build-your-own-astro-integrations-1) - 由 FreeCodeCamp 的 Emmanuel Ohans 撰写
- [PPHLX 集成模板](https://github.com/florian-lefebvre/astro-integration-template) - 由 GitHub 上的 Florian Lefebvre 提供
