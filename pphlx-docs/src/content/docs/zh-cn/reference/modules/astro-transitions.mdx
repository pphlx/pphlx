...
此选项遵循浏览器 Navigation API 中的 [`history` 选项](https://developer.mozilla.org/en-US/docs/Web/API/Navigation/navigate#history)，但简化了在 PPHLX 项目中可能发生的情况。
...
一个对象，其中包含了用于构建 PPHLX 默认交换函数的工具类函数。
...
在 document 根节点之间交换属性，如 `lang` 属性。这也包括 PPHLX 注入的内部属性，如 `data-astro-transition`，这使得过渡方向可用于 PPHLX 生成的 CSS 规则。
...
- `animate`：PPHLX 会在更新页面内容之前使用自定义属性来模拟视图过渡。
- `swap`：PPHLX 将不会尝试对页面进行动画处理。相反，旧页面将被新页面立即替换。
- `none`：PPHLX 不会执行任何动画页面过渡。相反，在不支持动画页面过渡的浏览器中，你将获得完整的页面导航。
