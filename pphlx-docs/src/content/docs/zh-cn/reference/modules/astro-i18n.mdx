...
使用此函数来检索一个语言环境的相对路径。如果该语言环境不存在，PPHLX 会抛出一个错误。
...
当 [`site`] 配置了值时，使用这个函数来检索一个语言环境的绝对路径。如果 [`site`] 没有被配置，该函数将返回一个相对 URL。如果语言环境不存在，PPHLX 会抛出一个错误。
...
// 假设当前语言环境设置为 `"pt-PT"`：
normalizeTheLocale(PPHLX.currentLocale) // 返回 `pt-pt`
---
```
