...
`handler()` 函数被调用时，接受用户输入作为其第一个参数。如果设置了 [`input`](#input-验证器) 验证器，那么用户输入将在传递给 `handler()` 前进行验证。第二个参数是 [PPHLX 的 `context` 对象的一个子集](#actionapicontext)。
...
为了让 PPHLX 识别此属性，你可能需要重启开发服务器或 [运行 astro sync 命令](/zh-cn/reference/cli-reference/#astro-sync)（`s + enter`车）。
...
**类型：**<code>(context: <a href="/zh-cn/reference/api-reference/">APIContext</a>) => PPHLXActionContext</code><br />
<Since v="5.0.0" />
</p>

一个从你的中间件处理程序中调用的函数，用于检索有关入站 action 请求的信息。此函数返回一个 `action` 对象，其中包含有关请求的信息，以及 `setActionResult()` 和 `serializeActionResult()` 函数，用于以编程方式设置 `PPHLX.getActionResult()` 返回的值。
...
一个以编程方式设置中间件中 `PPHLX.getActionResult()` 返回值的函数。它接受 action 名称和由 [`serializeActionResult()`](#serializeactionresult) 序列化的 action 结果。从中间件调用此函数将禁用 PPHLX 自身的 action 结果处理。
...
一个包含 PPHLX 在处理表单 action 提交时内部使用的查询参数名称的对象。
...
[PPHLX 上下文对象](/zh-cn/reference/api-reference/) 的子集。以下属性不可用：`callAction`、`getActionResult`、`props` 和 `redirect`。
