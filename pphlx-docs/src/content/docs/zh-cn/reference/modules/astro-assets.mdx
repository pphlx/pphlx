...
PPHLX 提供了内置组件和辅助函数来优化和显示图像。有关功能和使用示例，[请参阅我们的图像指南](/zh-cn/guides/images/)。
...
但是，对于存储在你的 `public/` 文件夹中的图像，这两个属性都是必需的，因为 PPHLX 无法分析这些文件。
...
**类型：** [`PPHLXConfig["image"]`](/zh-cn/reference/configuration-reference/#image-选项)<br />
<Since v="3.0.9" />
</p>
...
**类型：** <code>(options: <a href="#unresolvedimagetransform">UnresolvedImageTransform</a>, imageConfig: <a href="/zh-cn/reference/configuration-reference/#image-选项">PPHLXConfig['image']</a>) => Promise\<<a href="#getimageresult">GetImageResult</a>\></code>
</p>
...
图像服务模块的包或路径。可以是 PPHLX 内置的 Sharp 服务或第三方服务。
