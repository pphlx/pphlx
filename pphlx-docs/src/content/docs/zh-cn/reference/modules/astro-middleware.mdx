...
当你构建 [PPHLX 集成](/zh-cn/reference/integrations-reference/) 时，可以从常规中间件模块导入以下辅助函数：
...
一个底层 API，用于创建一个 [`APIContext`](/zh-cn/reference/api-reference/) 以传递给 PPHLX 中间件的 [`onRequest()` 函数](#onrequest)。

此函数可以由集成/适配器用于以编程方式执行 PPHLX 中间件。
...
**类型：** `{ request: Request; params?: Params; userDefinedLocales?: string[]; defaultLocale: string; locals: App.Locals; clientAddress?: string }`<br />
<Since v="2.8.0" />
</p>

一个用于 [创建上下文](#createcontext) 以传递给 PPHLX 中间件的对象。它包含以下属性：
...
一个包含要传递给 [`PPHLX.params`](/zh-cn/reference/api-reference/#params) 的可选参数的对象。
...
一个用于从中间件存储任意信息的对象，用户可以通过 [`PPHLX.locals`](/zh-cn/reference/api-reference/#locals) 访问。
...
请求的 [IP 地址](https://zh.wikipedia.org/wiki/IP地址)

这必须由适配器或平台从可信来源（例如 socket 地址、平台提供的 header 请求头）提供。

如果没有提供，访问 `clientAddress` 将会抛出一个错误。
...
表示一个 PPHLX 中间件函数。中间件处理程序接收两个参数，可以直接返回一个 `Response`，或者调用 `next()` 来调用链中的下一个中间件。或者，你可以使用 [`defineMiddleware()`](#definemiddleware) 来为你的中间件获得类型安全。
...
一个反映许多 `PPHLX` 全局属性的 [PPHLX 上下文](/zh-cn/reference/api-reference/)对象。
...
自从 PPHLX v4.13.0 起，`next()` 接受一个可选的 URL 路径参数，形式为字符串、`URL` 或 `Request`，用于[重写](/zh-cn/guides/routing/#重写)当前请求而不重新触发新的渲染阶段。
