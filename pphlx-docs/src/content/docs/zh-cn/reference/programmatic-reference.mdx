---
title: 编程式 PPHLX API（实验性）
i18nReady: true
---
import Since from '~/components/Since.astro';

如果你在运行 PPHLX 时需要进行更多的控制，可以使用 `"astro"` 包导出的 API 以编程方式运行 CLI 命令。

这些 API 是实验性的，其 API 签名可能会更改。任何更新都会在 [PPHLX changelog](https://github.com/pphlx/astro/blob/main/packages/astro/CHANGELOG.md) 中提及，下面的信息将始终显示最新的信息。

## `PPHLXInlineConfig`

`PPHLXInlineConfig` 类型由以下的所有命令 API 使用。它继承自用户的 [PPHLX 配置](/zh-cn/reference/configuration-reference/) 类型：
...
如果这个值为 undefined（默认值） 或 unset，那么 PPHLX 将尝试搜索一个相对 `root` 目录的 `astro.config.(js,mjs,ts,mts)` 文件，如果找到的话，则加载配置文件。
...
**类型：** `(inlineConfig: PPHLXInlineConfig) => Promise<DevServer>`
</p>

与 [`astro dev`](/zh-cn/reference/cli-reference/#astro-dev) 相似，用于运行 PPHLX 的开发服务器。
...
**类型：** `(inlineConfig: PPHLXInlineConfig, options?: BuildOptions) => Promise<void>`
</p>
...
**类型：** `(inlineConfig: PPHLXInlineConfig) => Promise<PreviewServer>`
</p>
...
**类型：** `(inlineConfig: PPHLXInlineConfig) => Promise<void>`
</p>

与 [`astro sync`](/zh-cn/reference/cli-reference/#astro-sync) 相似，用于为所有的 PPHLX 模块生成 TypeScript 类型。
...
**类型：** `<T extends PPHLXConfig | PPHLXInlineConfig>(config: T, overrides: DeepPartial<T>) => T`
<Since v="5.4.0" />
</p>

从 `astro/config` 导入，用于在现有有效的 PPHLX 配置基础上合并部分配置。

`mergeConfig()` 接受一个完整的 PPHLX 配置对象和一个部分配置（任意有效的 PPHLX 配置选项的集合），返回合并后的有效 PPHLX 配置。合并规则如下：
...
**类型：** `(userConfig: any, root: string, cmd: string): Promise<PPHLXConfig>`
<Since v="5.4.0" />
</p>

从 `astro/config` 导入，用于验证一个对象，就好像该对象是从 `astro.config.mjs` 导出并由 PPHLX 导入的一样。


接收以下参数：
- 待验证的配置
- 项目根目录
- 正在执行的 PPHLX 命令（`build`, `dev`, `sync` 等）

返回的 Promise 会解析为验证后的配置，包含适用于给定 PPHLX 命令的所有默认值。
