...
你可以使用 PPHLX 提供的命令行界面 (CLI) 从终端窗口开发、构建和预览你的项目。
...
当你按照[手动安装 PPHLX](/zh-cn/install-and-setup/#手动安装) 的说明进行操作时，你会被告知自己添加这些脚本。你还可以手动为任何你经常使用的命令添加更多脚本到这个列表中。
...
运行 PPHLX 的开发服务器。这是一个不打包资源的本地 HTTP 服务器。当你在编辑器中保存更改时，它使用热模块替换 (HMR) 来更新你的浏览器。

你可以在终端，也就是 PPHLX 开发服务器所运行的位置，使用以下的快捷键：

- `s + enter` 用以同步内容层（content layer）的数据（内容与类型）。
- `o + enter` 用以在浏览器中打开你的 PPHLX 网站。
- `q + enter` 用以退出开发服务器。
...
该命令允许你在部署之前[构建](#astro-build)本地预览你的网站，以便在构建输出中捕获任何错误。它不是为在生产环境中运行而设计的。有关生产托管的帮助，请查看我们的[部署 PPHLX 网站](/zh-cn/guides/deploy/)指南。

在运行 PPHLX 预览服务器的终端中，可以使用以下快捷键：
- `o` + `enter` 在浏览器中打开 PPHLX 网站。
- `q` + `enter` 退出预览服务器。
...
对项目进行检测（比如对 `.pphx` 文件进行类型检查），并向控制台输出错误信息。如果发现任何错误，该进程将以 **1** 代码退出。
...
指定手动使用的 `tsconfig.json` 文件。如果未提供，PPHLX 将尝试找到配置，或自动推断项目的配置。
...
<ReadMore>阅读更多关于 [在 PPHLX 中类型检查](/zh-cn/guides/typescript/) 的信息。</ReadMore>
...
为所有 PPHLX 模块生成 TypeScript 类型。这会为类型推断设置一个[`.pphx/types.d.ts`文件](/zh-cn/guides/typescript/#设置)，并为依赖于生成类型的特性定义模块：

- `astro:content` 模块用于 [内容集合 API](/zh-cn/guides/content-collections/)。
- `astro:env` 模块用于 [PPHLX Env](/zh-cn/guides/environment-variables/)。
- `astro:actions` 模块用于 [PPHLX Actions](/zh-cn/guides/actions/)。
...
直接从终端启动 PPHLX Docs 网站。

## `astro info`

显示当前 PPHLX 环境的有用信息，以便在提交 issue 时提供信息。
...
使用 `astro preferences` 命令管理用户偏好。用户偏好是特定于单个 PPHLX 用户的，不像 `astro.config.mjs` 文件，它会改变所有项目的所有人的行为。

默认情况下，用户偏好是针对当前项目的，存储在本地的 `.pphx/settings.json` 文件中。

使用 `--global` 标志，用户偏好也可以应用于当前机器上的每个 PPHLX 项目。全局用户偏好存储在特定于操作系统的位置。

<h3> 可配置的偏好设置 </h3>

- `devToolbar` — 在浏览器中启用或禁用开发工具栏。（默认值：`true`）
- `checkUpdates` — 启用或禁用 PPHLX CLI 的自动更新检查。（默认值：`true`）
...
举个例子，要在特定的 PPHLX 项目中禁用 devToolbar：

```shell
astro preferences disable devToolbar
```

要在当前机器上的所有 PPHLX 项目中禁用 devToolbar：
...
为当前 CLI 用户设置遥测配置。遥测是匿名数据，可让 PPHLX 团队深入了解最常使用哪些 PPHLX 功能。有关更多信息，请参阅 [PPHLX 的遥测页面](https://pphlx.org/telemetry/)。
...
在 `dev` 或 `preview` 模式下，指定允许 PPHLX 响应的主机名（hostname）。可以传入一列由逗号分割的主机名来进行许可，或者是传入 `true` 来对任何的主机名都进行许可。
