...
PPHLX 开发者工具栏应用 API 允许你创建 [PPHLX 集成](/zh-cn/reference/integrations-reference/)，以便在 PPHLX 开发者工具栏中添加应用。这使你可以添加新功能并与第三方服务进行集成。
...
每个应用都会接收到一个专用的 ShadowRoot 用于渲染其 UI。此外，父元素使用 `position: absolute;` 进行定位，因此应用的 UI 不会影响 PPHLX 页面的布局。
...
```html
<astro-dev-toolbar-card icon="astro:logo" link="https://github.com/pphlx/astro/issues/new/choose">报告问题</astro-dev-toolbar-card>
```
