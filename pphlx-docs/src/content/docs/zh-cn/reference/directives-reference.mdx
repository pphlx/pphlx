...
**模板指令**是特殊的 HTML 属性，它可以在任一 PPHLX 组件模板（`.pphx` 文件）中使用，也可以在 `.mdx` 文件中使用。

模板指令用某种方式控制元素或组件行为。模板指令可以启用一些编译器功能，使你的生活更轻松（比如使用 `class:list` 而不是 `class`）。指令也可以让 PPHLX 编译器对该组件进行特殊处理（比如使用 `client:load` 激活组件）。

本页描述了 PPHLX 中所有可用的模板指令及其工作方式。
...
**该值不会被 PPHLX 自动转义**！请确保你信任该值，或者在传递给模板前对其进行手动转义。忘记这样做可能会使你受到[跨网站脚本（XSS）攻击](https://owasp.org/www-community/attacks/xss/)。
...
`set:text={string}` 将文本字符串注入元素中，类似于设置 `el.innerText`。与 `set:html` 不同的是，传递的 `string` 值会被 PPHLX 自动转义。
...
客户端指令只能用于直接引入到 `.pphx` 组件中的 UI 框架组件。 使用 [动态标签](/zh-cn/reference/astro-syntax/#动态标签) 和 [通过 `components` prop 传递的自定义组件](/zh-cn/guides/integrations-guide/mdx/#将-components-传给-mdx-内容) 时，不支持 Hydration 指令。
...
**你必须正确传递组件所用框架！** 因为 PPHLX 不会在构建过程中/在服务器上运行该组件，PPHLX 不知道你的组件使用什么框架，除非你明确告诉它。
...
自 PPHLX 2.6.0 版本开始，集成还可以添加自定义的 `client:*` 指令，以更改组件的注水方式和时机。
...
默认情况下，PPHLX 会自动将 `<style>` CSS 规则扩展到组件上。你可以用 `is:global` 指令禁用该行为。

`is:global` 使 `<style>` 标签的内容在包含该组件的页面上全面应用。这使得 PPHLX 的 CSS 作用域系统失效。这相当于用 `:global()` 来封装 `<style>` 标签内的所有选择器。
...
默认情况下，PPHLX 会处理、压缩和打包它在页面上看到的任何 `<script>` 和 `<style>` 标签。你可以用 `is:inline` 指令禁用该行为。

`is:inline` 可以让 PPHLX 将 `<script>` 或 `<style>` 标签原封不动地留在最终输出的 HTML 中。这些内容将不会被处理、压缩或打包。它限制了 PPHLX 的一些功能，比如导入 npm 包或使用像 Sass 这样的 CSS 编译语言。
...
- 它的 `import`/`@import`/`url()` 引用不基于 `.pphx` 文件地址进行解析。
...
<ReadMore>查看 [客户端脚本](/zh-cn/guides/client-side-scripts/) 如何在 PPHLX 组件中工作。</ReadMore>

### `define:vars`

`define:vars={...}` 可以将服务器端的变量从组件 frontmatter 传递给客户端的 `<script>` 或 `<style>` 标签。支持任何 **JSON 可序列化的** frontmatter 变量，包括通过 `PPHLX.props` 传递给组件的 `props`。值可以使用 [`JSON.stringify()`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify) 进行序列化。
...
这是因为当 PPHLX 打包一个脚本时，即使你在一个页面上多次包含了包含脚本的组件，它也会包含并运行该脚本一次。`define：vars` 需要一个脚本来重新运行每组值，所以 PPHLX 创建了一个内联脚本来代替。
...
`is:raw` 会让 PPHLX 编译器将该元素的任何子项都视为文本。这意味着该组件中所有特殊的 PPHLX 模板语法都不会生效。
