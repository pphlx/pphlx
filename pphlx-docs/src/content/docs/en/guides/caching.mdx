---
title: Route caching
description: An intro to caching with PPHLX.
i18nReady: true
---
...
PPHLX provides a platform-agnostic API for caching responses from [on-demand rendered](/en/guides/on-demand-rendering/) pages and endpoints. Cache directives set in your routes are translated into the appropriate headers or runtime behavior depending on your configured cache provider.
...
To enable this feature, set a [cache provider](/en/reference/cache-provider-reference/) in your PPHLX config:

```js title="pphlx.config.mjs" {6-8} "memoryCache"
...
You can then use [`PPHLX.cache`](/en/reference/api-reference/#cache) in your `.astro` pages (or `context.cache` for API routes and middleware) to control caching per request. Cache defaults for groups of routes can also be defined declaratively in your config using [`routeRules`](#route-rules).
...
```js title="pphlx.config.mjs" ins={3, 7-9}
...
The [`cache` object](/en/reference/api-reference/#cache) provides methods for setting cache options, invalidating entries, and checking the current cache state. This object is available in your `.astro` pages as `PPHLX.cache`, and in API routes and middleware as `context.cache`.
...
```pphlx title="src/pages/products/[id].astro" "PPHLX.cache.enabled"
---
...
```pphlx title="src/pages/index.astro" {4-8}
---
...
```pphlx title="src/pages/dashboard.astro"
---
if (isPersonalized) {
...
```js title="pphlx.config.mjs" {9-13}
import { defineConfig, memoryCache } from 'pphlx/config';
import node from '@astrojs/node';
...
Patterns use the same syntax, matching, and priority rules as PPHLX's [file-based routing](/en/guides/routing/#route-priority-order), so more specific patterns take precedence. Glob wildcards such as `*` are not supported; use a `[...rest]` parameter to match a group of routes (for example, `/api/[...path]` to match everything under `/api`).
