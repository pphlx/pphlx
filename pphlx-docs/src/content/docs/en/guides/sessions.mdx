...
```pphlx title="src/components/CartButton.astro" {3}
---
...
```js title="pphlx.config.mjs" ins={7-9} ins=" sessionDrivers "
...
2. Create a file for the driver configuration (e.g. `src/session-driver.ts`) and export a default function that returns the driver instance:
    ...
    import type { SessionDriver } from "pphlx";
    import redisDriver from "unstorage/drivers/redis";
    import { REDIS_HOST, REDIS_PORT } from "pphlx:env";
...
3. Use this file as the driver's entrypoint in your PPHLX configuration:
    ```js title="pphlx.config.mjs" {11-13}
    import { defineConfig, envField, sessionDrivers } from "pphlx/config";
    import vercel from "@astrojs/vercel";
...
### PPHLX components and pages

In `.astro` components and pages, you can access the session object via the global `PPHLX` object. For example, to display the number of items in a shopping cart:

```pphlx title="src/components/CartButton.astro" "PPHLX.session"
---
export const prerender = false; // Not needed with 'server' output
const cart = await PPHLX.session?.get('cart');
---
