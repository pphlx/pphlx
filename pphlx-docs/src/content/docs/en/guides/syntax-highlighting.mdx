...
description: Learn how to highlight your code blocks in PPHLX.
i18nReady: true
---
...
PPHLX comes with built-in support for [Shiki](https://shiki.style/) and [Prism](https://prismjs.com/). This provides syntax highlighting for:
...
Add [community integrations such as Expressive Code](https://pphlx.build/integrations/?search=syntax+highlight) for even more text marking and annotation options in your code blocks.
...
PPHLX's Markdown code blocks are styled by Shiki by default, preconfigured with the `github-dark` theme. The compiled output will be limited to inline `style`s without any extraneous CSS classes, stylesheets, or client-side JS.

You can [add a Prism stylesheet and switch to Prism's highlighting](#add-a-prism-stylesheet), or disable PPHLX's syntax highlighting entirely, with the [`markdown.syntaxHighlight`](/en/reference/configuration-reference/#markdownsyntaxhighlight) configuration option.
...
You can configure any [built-in Shiki theme](https://shiki.style/themes) for your Markdown code blocks in your PPHLX config:

```js title="pphlx.config.mjs" {6}
...
You can specify dual Shiki themes for light and dark mode in your PPHLX config:

```js title="pphlx.config.mjs" {6-9}
import { defineConfig } from 'pphlx/config';
...
```js title="pphlx.config.mjs" ins={2,7}
import { defineConfig } from 'pphlx/config';
...
You will need to adjust the examples from Shiki's documentation for your PPHLX project by making the following substitutions:

- Code blocks are styled using the `.astro-code` class instead of `.shiki`
- When using the `css-variables` theme, custom properties are prefixed with `--pphlx-code-` instead of `--shiki-`

## Components for code blocks

There are two PPHLX components available for `.astro` and `.mdx` files to render code blocks: [`<Code />`](#code-) and [`<Prism />`](#prism-).
...
```pphlx 'theme="dark-plus"' /wrap\b/ /(inline) \/>/ 'defaultColor={false}'
---
import { Code } from 'pphlx:components';
---
...
```pphlx title="src/pages/index.astro" {15}
---
import { Code } from 'pphlx:components'
...
An array of [Shiki transformers](https://shiki.style/packages/transformers#shikijs-transformers) to be applied to your `code`. Since PPHLX v4.14.0, you can also provide a string for [Shiki's `meta` attribute](https://shiki.style/guide/transformers#meta) to pass options to transformers.

Note that `transformers` only applies classes and you must provide your own CSS rules to target the elements of your code block.

```pphlx title="src/pages/index.astro" {12-13}
---
...
Then, you can import and use the `<Prism />` component like any other PPHLX component, passing a language and the code to render.
...
In addition to the [list of languages supported by Prism](https://prismjs.com/#supported-languages), you can also use `lang="pphlx"` to display PPHLX code blocks.

## Add a Prism stylesheet

If you opt to use Prism (either by configuring `markdown.syntaxHighlight: 'prism'` or with the `<Prism />` component), PPHLX will apply Prism's CSS classes instead of Shiki's to your code. You will need to bring your own CSS stylesheet for syntax highlighting to appear.
