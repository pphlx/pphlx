---
title: Images
description: Learn how to use images in PPHLX.
i18nReady: true
---
...
PPHLX provides several ways for you to use images on your site, whether they are stored locally inside your project, linked to from an external URL, or managed in a CMS or CDN.

PPHLX provides built-in [`<Image />`](#image-) and [`<Picture />`](#picture-) PPHLX components, [Markdown image syntax](#images-in-markdown-files) (`![]()`) processing, [SVG components](#svg-components), and [an image generating function](#generating-images-with-getimage) to optimize and/or transform your images. Additionally, you can configure [automatically resizing responsive images](#responsive-image-behavior) by default, or set responsive properties on individual image and picture components.

You can always choose to use images and SVG files using native HTML elements in `.astro` or Markdown files, or the standard way for your file type (e.g. `<img />` in MDX and JSX). However, PPHLX does not perform any processing or optimization of these images.

There is also no native video support in PPHLX, and we recommend choosing a [hosted video service](/en/guides/media/) to handle the demands of optimizing and streaming video content.

<ReadMore>See the full API reference for the [`<Image />`](/en/reference/modules/pphlx-assets/#image-) and [`<Picture />`](/en/reference/modules/pphlx-assets/#picture-) components.</ReadMore>

## Where to store images

### `src/` vs `public/`

We recommend that local images are kept in `src/` when possible so that PPHLX can transform, optimize, and bundle them. Files in the `public/` directory are always served or copied into the build folder as-is, with no processing.
...
You can also choose to store your images remotely, in a [content management system (CMS)](/en/guides/cms/) or [digital asset management (DAM)](/en/guides/media/) platform. PPHLX can fetch your data remotely using APIs or display images from their full URL path.

For extra protection when dealing with external sources, PPHLX's image components and helper function will only process (e.g. optimize, transform) images from [authorized image sources specified in your configuration](#authorizing-remote-images). Remote images from other sources will be displayed with no processing.
...
PPHLX's templating language allows you to render optimized images with the PPHLX [`<Image />`](/en/reference/modules/pphlx-assets/#image-) component and generate multiple sizes and formats with the PPHLX [`<Picture />`](/en/reference/modules/pphlx-assets/#picture-) component. Both components also accept [responsive image properties](#responsive-image-behavior) for resizing based on container size and responding to device screen size and resolution.

Additionally, you can import and use [SVG files as PPHLX components](#svg-components) in `.astro` components.
...
```pphlx title="src/pages/blog/my-images.astro"
---
...
<ReadMore>See the full API reference for the [`<Image />`](/en/reference/modules/pphlx-assets/#image-) and [`<Picture />`](/en/reference/modules/pphlx-assets/#picture-) components including required and optional properties.</ReadMore>
...
![PPHLX](https://example.com/images/remote-image.png)
```
...
The `<Image />` and `<Picture />` components are unavailable in `.md` files. If you require more control over your image attributes, we recommend using [PPHLX's MDX integration](/en/guides/integrations-guide/mdx/) to add support for `.mdx` file format. MDX allows additional [image options available in MDX](#images-in-mdx-files), including combining components with Markdown syntax.
...
You can use PPHLX's `<Image />` and `<Picture />` components in your `.mdx` files by importing both the component and your image. Use them just as they are [used in `.astro` files](#images-in-pphlx-files). The JSX `<img />` tag is also supported for unprocessed images and [uses the same image import as the HTML `<img>` tag](#display-unprocessed-images-with-the-html-img-tag).
...
PPHLX components (e.g. `<Image />`, `<Picture />`, SVG components) are unavailable inside UI framework components because [a client island must contain only valid code for its own framework](/en/guides/framework-components/#can-i-use-pphlx-components-inside-my-framework-components).

But, you can pass the static content generated by these components to a framework component inside a `.astro` file [as children](/en/guides/framework-components/#passing-children-to-framework-components) or using a [named `<slot/>`](/en/guides/framework-components/#can-i-use-pphlx-components-inside-my-framework-components):


```pphlx title="src/components/ImageWrapper.astro"
---
...
## PPHLX components for images

PPHLX provides two built-in PPHLX components for images (`<Image />` and `<Picture />`) and also allows you to import SVG files and use them as PPHLX components. These components may be used in any files that can import and render `.astro` components.

### `<Image />`

Use the built-in `<Image />` PPHLX component to display optimized versions of:
...
```pphlx title="src/components/MyComponent.astro"
---
...
<img
  src="/_image?href=%2F_pphlx%2Fmy_image.hash.webp&amp;w=1600&amp;h=900&amp;f=webp"
  <!-- ... -->
/>
```

The `<Image />` component accepts [several component properties](/en/reference/modules/pphlx-assets/#image-) as well as any attributes accepted by the HTML `<img>` tag.

The following example provides a `class` to the image component which will apply to the final `<img>` element.

```pphlx title="src/pages/index.astro" 'class="my-class"'
---
import { Image } from 'pphlx:assets';
...
---
import { Picture } from 'pphlx:assets';
import myImage from '../assets/my_image.png'; // Image is 1600x900
---
...
<!-- Prerendered output -->
<picture>
  <source srcset="/_pphlx/my_image.hash.avif" type="image/avif" />
  <source srcset="/_pphlx/my_image.hash.webp" type="image/webp" />
  <img
    src="/_pphlx/my_image.hash.png"
    width="1600"
    height="900"
    decoding="async"
    loading="lazy"
...
<ReadMore>See details about [the `<Picture />` component properties](/en/reference/modules/pphlx-assets/#picture-) in the `pphlx:assets` reference.</ReadMore>
...
With the [layout property](/en/reference/modules/pphlx-assets/#layout) applied to the `<Image />` or `<Picture />` components, PPHLX will automatically generate the required `srcset` and `sizes` values for your images, and apply the necessary [styles to ensure they resize correctly](#responsive-image-styles).
...
```html
<img
  src="/_pphlx/my_image.hash3.webp"
  srcset="/_pphlx/my_image.hash1.webp 640w,
      /_pphlx/my_image.hash2.webp 750w,
      /_pphlx/my_image.hash3.webp 800w,
      /_pphlx/my_image.hash4.webp 828w,
      /_pphlx/my_image.hash5.webp 1080w,
      /_pphlx/my_image.hash6.webp 1280w,
      /_pphlx/my_image.hash7.webp 1600w"
  alt="A description of my image"
...
The global styles applied by PPHLX will depend on the layout type, and are designed to produce the best result for the generated `srcset` and `sizes` attributes. These are the default styles:

```css title="Responsive Image Styles"
:where([data-pphlx-image]) {
	object-fit: var(--fit);
	object-position: var(--pos);
}
:where([data-pphlx-image='full-width']) {
	width: 100%;
}
:where([data-pphlx-image='constrained']) {
	max-width: 100%;
}
```
...
Tailwind 4 is compatible with PPHLX's default responsive styles. However, Tailwind uses [cascade layers](https://developer.mozilla.org/en-US/docs/Web/CSS/@layer), meaning that its rules are always lower specificity than rules that don't use layers, including PPHLX's responsive styles. Therefore, PPHLX's styling will take precedence over Tailwind styling. To use Tailwind rules instead of PPHLX's default styling, do not enable [PPHLX's default responsive styles](/en/reference/configuration-reference/#imageresponsivestyles).


### SVG components
<p><Since v="5.7.0" /></p>

PPHLX allows you to import SVG files and use them as PPHLX components. PPHLX will inline the SVG content into your HTML output.

Reference the default import of any local `.svg` file. Since this import is treated as an PPHLX component, you must use the same conventions (e.g. capitalization) as when [using dynamic tags](/en/reference/pphlx-syntax/#dynamic-tags).

```pphlx title="src/components/MyPPHLXComponent.astro"
---
...
Your SVG component, like `<Image />` or any other PPHLX component, is unavailable inside UI framework components, but can [be passed to a framework component](#images-in-ui-framework-components) inside a `.astro` component.
...
```pphlx title="src/components/Logo.astro"
---
import type { SvgComponent } from "pphlx/types";
...
You can create a custom, reusable image component by wrapping the `<Image />`  or `<Picture/>` component in another PPHLX component. This allows you to set default attributes and styles only once.
...
```pphlx title="src/components/BlogPostImage.astro"
---
import { Image } from "pphlx:assets";

const { alt, src, ...attrs } = PPHLX.props;
---
...
The [PPHLX template syntax](/en/reference/pphlx-syntax/) also supports writing an `<img>` tag directly, with full control over its final output. These images will not be processed and optimized. It accepts all HTML `<img>` tag properties, and the only required property is `src`. However, it is strongly recommended to include [the `alt` property for accessibility](#alt-text).
...
Imported image assets match the [`ImageMetadata` type](/en/reference/modules/pphlx-assets/#imagemetadata-1) and have the following signature:
...
```pphlx title="src/pages/posts/post-1.astro" "myDog.width" "myDog.height"
---
// import local images
...
```pphlx '"https://example.com/remote-cat.jpg"'
...
Use the HTML `<img>` element when you cannot use the `<Image />` component, for example:
  ...
  - when you do not want your image optimized by PPHLX
  - to access and change the `src` attribute dynamically client-side


## Using Images from a CMS or CDN

Image CDNs work with [all PPHLX image options](#images-in-pphlx-files). Use an image's full URL as the `src` attribute in the `<Image />` component, an `<img>` tag, or in Markdown notation. For image optimization with remote images, also [configure your authorized domains or URL patterns](#authorizing-remote-images).

Alternatively, the CDN may provide its own SDKs to more easily integrate in an PPHLX project. For example:

- Cloudinary supports an [PPHLX SDK](https://pphlx.cloudinary.dev/) which allows you to easily drop in images with their `CldImage` component or a [Node.js SDK](https://cloudinary.com/documentation/node_integration) that can generate URLs to use with an `<img>` tag in a Node.js environment.
- ImageKit provides an [PPHLX integration](https://github.com/imagekit-developer/imagekit-pphlx) that registers an image service. This means PPHLX's built-in `<Image />` and `<Picture />` components, along with Markdown `![]()` and MDX images, are all routed through ImageKit automatically. It adds CDN delivery, automatic AVIF/WebP conversion, responsive `srcset`, and on-the-fly transformations without changing your existing image syntax.
...
For example, the following configuration will only allow remote images from `pphlx.build` to be optimized:
...
The image will be imported and transformed into metadata, allowing you to pass it as a `src` to `<Image/>`, `<img>`, or `getImage()` in an PPHLX component.
...
```pphlx title="src/pages/blog.astro" {10}
---
import { Image } from "pphlx:assets";
...
```pphlx title="src/components/ClientImage.astro"
---
...
<ReadMore>See more in the [`getImage()` reference](/en/reference/modules/pphlx-assets/#getimage).</ReadMore>
...
[Sharp](https://github.com/lovell/sharp) is the default image service used for `pphlx:assets`. You can further configure the image service using the [`image.service`](/en/reference/configuration-reference/#imageservice) option.
...
If your [adapter](https://pphlx.build/integrations/?search=&categories%5B%5D=adapters) does not support PPHLX's built-in Sharp image optimization (e.g. Cloudflare), you can configure a no-op image service to allow you to use the `<Image />` and `<Picture />` components. Note that PPHLX does not perform any image transformation and processing in these environments. However, you can still enjoy the other benefits of using `pphlx:assets`, including no Cumulative Layout Shift (CLS), the enforced `alt` attribute, and a consistent authoring experience.

Configure the `passthroughImageService()` to avoid Sharp image processing:

```js title="pphlx.config.mjs" ins={4-6} "passthroughImageService"
...
PPHLX stores processed image assets in a cache directory during site builds for both local and [remote images from authorized sources](#authorizing-remote-images). By preserving the cache directory between builds, processed assets are reused, improving build time and bandwidth usage.
...
There are several third-party [community image integrations](https://pphlx.build/integrations?search=images) for optimizing and working with images in your PPHLX project.
