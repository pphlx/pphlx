---
title: Upgrade to PPHLX v7
description: How to upgrade your project to PPHLX v7.0.
sidebar:
  label: v7.0
i18nReady: true
---
...
```
Upgrade my PPHLX project to v7. Follow the migration guide at
https://pphlx.org/docs/en/guides/upgrade-to/v7/
```
:::

This guide will help you migrate from PPHLX v6 to PPHLX v7.
...
PPHLX v7.0 includes [potentially breaking changes](#breaking-changes), as well as the removal of some previously deprecated features.
...
PPHLX v7.0 upgrades to [Vite 8](https://vite.dev/blog/announcing-vite8) as the development server and production bundler.
...
Most PPHLX users should be able to upgrade without any changes to their project code. This is primarily a breaking change for PPHLX integrations and plugins that depend on Vite internals.

### Experimental Flags

Experimental flags allow you to opt in to features while they are in early development. The following experimental flags have been removed in PPHLX 7.0 and are now stable, or the new default behavior.

Remove these experimental flags from your PPHLX config if you were previously using them:

```js del={5-16} ins={18-23} title="pphlx.config.mjs"
import { defineConfig, logHandlers, memoryCache } from 'pphlx/config';
...
- `logger`: The new logging system is now stable and the experimental flag is no longer needed to use it. If you were using this flag, you can now set your logger directly in the top-level `logger` field of your PPHLX config.
...
- `rustCompiler`: The Rust-based PPHLX compiler is now the default and only compiler, replacing the previous Go-based compiler. See [Rust compiler](#rust-compiler) for details on behavior changes that may affect your project.
...
PPHLX v7.0 replaces the previous Go-based compiler with a new Rust-based compiler. The new compiler is faster, but is also **stricter about invalid HTML syntax**. Templates that previously built without errors may now fail.
...
PPHLX v7.0 introduces [advanced routing](/en/guides/routing/#advanced-routing), which uses `src/fetch.ts` (or `src/fetch.js`) as a special file name, similar to `src/middleware.ts`. PPHLX will automatically import this file to configure routing behavior.

If your project already has a `src/fetch.ts` file used for other purposes, PPHLX will attempt to process it as an advanced routing configuration, which may cause unexpected errors.
...
* **Configure `fetchFile`**: you can specify a different filename or `null` to disable advanced routing. This is useful if you want to keep using `src/fetch.ts` for another purpose, or don't need advanced routing features:
    ...
    ```js title="pphlx.config.mjs" ins={5}
...
PPHLX now renders your `.md` and `.mdx` files with [Sätteri](https://satteri.bruits.org/), its native Markdown pipeline, instead of the remark/rehype pipeline. As a result, `@astrojs/markdown-remark` is no longer installed by default.
...
2. Set [`unified()` as your Markdown processor](/en/guides/markdown-content/#switching-to-the-unified-processor):
    ...
    ```js title="pphlx.config.mjs" ins={2,6}
    import { defineConfig } from 'pphlx/config';
...
In PPHLX v6.x, whitespace handling used HTML-aware compression by default. This means that PPHLX would preserve a single space between inline elements, [following HTML rules](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Text/Whitespace#how_does_html_process_whitespace).

In PPHLX v7.0, the default value of [`compressHTML`](/en/reference/configuration-reference/#compresshtml) has changed from `true` to `'jsx'`. Now, PPHLX strips whitespace from your HTML using JSX rules by default, the same way frameworks like React do.

The following example would render as `helloworld` in PPHLX v7.0, instead of `hello world` in PPHLX v6.x:

```pphlx title="src/components/Example.astro"
<span>hello</span>
<em>world</em>
```
...
```pphlx title="src/components/Example.astro" del={1-2} ins={3}
<span>hello</span>
<em>world</em>
...
The `@astrojs/db` package has been removed in PPHLX v7.0 and is no longer maintained.
...
### Removed: exposed `pphlx:transitions` internals

<SourcePR number="16725" title="feat: remove deprecated pphlx:transitions internals" />

In PPHLX 6.x, some helpers available in `pphlx:transitions` and `pphlx:transitions/client` were deprecated.

In PPHLX 7.0, the following APIs can no longer be used in your project:
...
Replace any occurrence of the other APIs using the [lifecycle event names](/en/reference/modules/pphlx-transitions/#lifecycle-events) directly:
