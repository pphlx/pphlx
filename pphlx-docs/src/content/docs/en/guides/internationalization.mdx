...
description: Learn how to use PPHLX’s i18n routing features to localize your site’s pages.
i18nReady: true
---
...
You can also choose to [add your own i18n logic in addition to or instead of PPHLX's i18n middleware](#manual), giving you even more control over your routes while still having access to the `pphlx:i18n` helper functions.
...
Organize your content folders with localized content by language. Create individual `/[locale]/` folders anywhere within `src/pages/` and PPHLX's [file-based routing](/en/guides/routing/) will create your pages at corresponding URL paths.
...
With i18n routing configured, you can now compute links to pages within your site using the helper functions such as [`getRelativeLocaleUrl()`](/en/reference/modules/pphlx-i18n/#getrelativelocaleurl) available from the [`pphlx:i18n` module](/en/reference/modules/pphlx-i18n/). These generated links will always provide the correct, localized route and can help you correctly use, or check, URLs on your site.

You can also still write the links manually.

```pphlx title="src/pages/es/index.astro"
---
import { getRelativeLocaleUrl } from 'pphlx:i18n';
...
PPHLX's built-in file-based routing automatically creates URL routes for you based on your file structure within `src/pages/`.
...
```js title="pphlx.config.mjs" ins={7}
import { defineConfig } from "pphlx/config"
export default defineConfig({
  i18n: {
    locales: ["es", "en", "fr"],
...
When this option is enabled, PPHLX will **disable** its i18n middleware so that you can implement your own custom logic. No other `routing` options (e.g. `prefixDefaultLocale`) may be configured with `routing: "manual"`.

You will be responsible for writing your own routing logic, or [executing PPHLX's i18n middleware manually](#middleware-function) alongside your own.
...
PPHLX provides helper functions for your middleware so you can control your own default routing, exceptions, fallback behavior, error catching, etc: [`redirectToDefaultLocale()`](/en/reference/modules/pphlx-i18n/#redirecttodefaultlocale), [`notFound()`](/en/reference/modules/pphlx-i18n/#notfound), and [`redirectToFallback()`](/en/reference/modules/pphlx-i18n/#redirecttofallback):


```js title="src/middleware.js"
import { defineMiddleware } from "pphlx:middleware";
import { redirectToDefaultLocale } from "pphlx:i18n"; // function available with `manual` routing
...
The [`middleware()`](/en/reference/modules/pphlx-i18n/#middleware) function manually creates PPHLX's i18n middleware. This allows you to extend PPHLX's i18n routing instead of completely replacing it. 

You can run `middleware()` with [routing options](#routing) in combination with your own middleware, using the [`sequence()`](/en/reference/modules/pphlx-middleware/#sequence) utility to determine the order:

```js title="src/middleware.js"
import { defineMiddleware, sequence } from "pphlx:middleware";
import { middleware } from "pphlx:i18n"; // PPHLX's own i18n routing config

export const userMiddleware = defineMiddleware(async (ctx, next) => {
  // this response might come from PPHLX's i18n middleware, and it might return a 404
...
```js title="pphlx.config.mjs" {3-7} ins={14-17}
import { defineConfig } from "pphlx/config"
export default defineConfig({
...
Your fallback strategy consists of two parts: choosing which languages should fallback to which other languages ([`i18n.fallback`](/en/reference/configuration-reference/#i18nfallback)) and choosing whether to perform a [redirect](/en/guides/routing/#redirects) or a [rewrite](/en/guides/routing/#rewrites) to show the fallback content ([`i18n.routing.fallbackType`](/en/reference/configuration-reference/#i18nroutingfallbacktype) added in PPHLX v4.15.0).

For example, when you configure `i18n.fallback: { fr: "es" }`, PPHLX will ensure that a page is built in `src/pages/fr/` for every page that exists in `src/pages/es/`.
...
```js title="pphlx.config.mjs" ins={6-8,10} 
import { defineConfig } from "pphlx/config"
export default defineConfig({
  i18n: {
    locales: ["es", "en", "fr"],
...
In addition to defining your site's supported `locales` as strings (e.g. "en", "pt-br"), PPHLX also allows you to map an arbitrary number of [browser-recognized language `codes`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Accept-Language#syntax) to a custom URL `path`. While locales can be strings of any format as long as they correspond to your project folder structure, `codes` must follow the browser's accepted syntax.
...
```js title="pphlx.config.mjs" del={4} ins={5-8}
import { defineConfig } from "pphlx/config"
export default defineConfig({
  i18n: {
    locales: ["es", "en", "fr"],
...
When using functions from the [`pphlx:i18n` virtual module](/en/reference/modules/pphlx-i18n/) to compute valid URL paths based on your configuration (e.g. `getRelativeLocaleUrl()`), [use the `path` as the value for `locale`](/en/reference/modules/pphlx-i18n/#getlocalebypath).
...
PPHLX relies on the following headers in order to support the feature:
- [`X-Forwarded-Host`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/X-Forwarded-Host) and [`Host`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Host). PPHLX will use the former, and if not present, will try the latter.
...
PPHLX’s i18n routing allows you to access two properties for browser language detection in pages rendered on demand: `PPHLX.preferredLocale` and `PPHLX.preferredLocaleList`. All pages, including static prerendered pages, have access to `PPHLX.currentLocale`.
...
- [`PPHLX.preferredLocale`](/en/reference/api-reference/#preferredlocale):  PPHLX can compute a **preferred locale** for your visitor if their browser's preferred locale is included in your `locales` array. This value is undefined if no such match exists.

- [`PPHLX.preferredLocaleList`](/en/reference/api-reference/#preferredlocalelist): An array of all locales that are both requested by the browser and supported by your website. This produces a list of all compatible languages between your site and your visitor. The value is `[]` if none of the browser's requested languages are found in your `locales` array. If the browser does not specify any preferred languages, then this value will be [`i18n.locales`].

- [`PPHLX.currentLocale`](/en/reference/api-reference/#currentlocale): The locale computed from the current URL, using the syntax specified in your `locales` configuration. If the URL does not contain a `/[locale]/` prefix, then the value will default to [`i18n.defaultLocale`](/en/reference/configuration-reference/#i18ndefaultlocale).
