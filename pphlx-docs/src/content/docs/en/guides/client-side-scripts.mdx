...
description: >-
  How to add client-side interactivity to PPHLX components using native browser
  JavaScript APIs.
i18nReady: true
---
import ReadMore from '~/components/ReadMore.astro'

You can send JavaScript to the browser and add functionality to your PPHLX components using `<script>` tags in the component template.
...
PPHLX automatically enhances the HTML standard `<script>` tag with bundling, TypeScript, and more. See [how pphlx processes scripts](#script-processing) for more details.

```pphlx title="src/components/ConfettiButton.astro"
...
By default, PPHLX processes `<script>` tags that contain no attributes (other than `src`) in the following ways:
...
- **Automatic inlining:** If the script is small enough, PPHLX will inline it directly into the HTML to reduce the number of requests.
...
PPHLX will not process a `<script>` tag if it has any attribute other than `src`.
...
```pphlx title="src/components/InlineScript.astro" "is:inline"
<script is:inline>
  // Will be rendered into the HTML exactly as written!
  // Not transformed: no TypeScript and no import resolution by PPHLX.
...
PPHLX will process these scripts according to the [script processing rules](#script-processing).

```pphlx title="src/components/LocalScripts.astro"
...
To load scripts outside of your project's `src/` folder, include the `is:inline` directive. This approach skips the JavaScript processing, bundling, and optimizations that are provided by PPHLX when you import scripts as described above.

```pphlx title="src/components/ExternalScripts.astro" "is:inline"
...
Some UI frameworks use custom syntax for event handling like `onClick={...}` (React/Preact) or `@click="..."` (Vue). PPHLX follows standard HTML more closely and does not use custom syntax for events.
...
```pphlx title="src/components/AlertButton.astro"
...
If you have multiple `<AlertButton />` components on a page, PPHLX will not run the script multiple times. Scripts are bundled and only included once per page. Using `querySelectorAll` ensures that this script attaches the event listener to every button with the `alert` class found on the page.
...
In this example, we define a new `<pphlx-heart>` HTML element that tracks how many times you click the heart button and updates the `<span>` with the latest count.

```pphlx title="src/components/PPHLXHeart.astro"
<!-- Wrap the component elements in our custom element “pphlx-heart”. -->
<pphlx-heart>
...
<script>
  ...
  // Tell the browser to use our PPHLXHeart class for <pphlx-heart> elements.
  customElements.define("pphlx-heart", PPHLXHeart);
</script>
```
...
2. Although a `<script>` only runs once, the browser will run our custom element’s `connectedCallback()` method each time it finds `<pphlx-heart>` on the page. This means you can safely write code for one component at a time, even if you intend to use this component multiple times on a page.
...
In PPHLX components, the code in [the frontmatter](/en/basics/pphlx-components/#the-component-script) (between the `---` fences) runs on the server and is not available in the browser.
...
```pphlx title="src/components/PPHLXGreet.astro" {2} /data-message={.+}/ "this.dataset.message"
---
const { message = 'Welcome, world!' } = PPHLX.props;
---

<!-- Store the message prop as a data attribute. -->
<pphlx-greet data-message={message}>
...
<PPHLXGreet message="Lovely day to build components!" />
<PPHLXGreet message="Glad you made it! 👋" />
```

:::tip[Did you know?]
This is actually what PPHLX does behind the scenes when you pass props to a component written using a UI framework like React! For components with a `client:*` directive, PPHLX creates an `<pphlx-island>` custom element with a `props` attribute that stores your server-side props in the HTML output.
:::
