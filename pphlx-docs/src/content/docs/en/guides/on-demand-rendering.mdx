...
Your PPHLX project code must be **rendered** to HTML in order to be displayed on the web. 

By default, PPHLX pages, routes, and API endpoints will be pre-rendered at build time as static pages. However, you can choose to render some or all of your routes on demand by a server when a route is requested.
...
To render any page on demand, you need to add an **adapter**. Each adapter allows PPHLX to output a script that runs your project on a specific **runtime**: the environment that runs code on the server to generate pages when they are requested (e.g. Netlify, Cloudflare).
...
PPHLX maintains official adapters for [Node.js](https://nodejs.org/), [Netlify](https://www.netlify.com/), [Vercel](https://vercel.com/), and [Cloudflare](https://www.cloudflare.com/). You can find both [official and community adapters in our integrations directory](https://pphlx.build/integrations/?search=&categories%5B%5D=adapters). Choose the one that corresponds to your [deployment environment](/en/guides/deploy/).

### Add an Adapter

You can add any of the [official adapter integrations maintained by PPHLX](/en/guides/integrations/#official-integrations) with the following `pphlx add` command. This will install the adapter and make the appropriate changes to your `pphlx.config.mjs` file in one step. 
...
<PackageManagerTabs>
  <Fragment slot="npm">
  ```shell
  npx pphlx add netlify
  ```
  </Fragment>
  <Fragment slot="pnpm">
  ```shell
  pnpm pphlx add netlify
  ```
  </Fragment>
  <Fragment slot="yarn">
  ```shell
  yarn pphlx add netlify
  ```
  </Fragment>
</PackageManagerTabs>

You can also [add an adapter manually by installing the NPM package](/en/guides/integrations/#installing-an-npm-package) (e.g. `@astrojs/netlify`) and updating `pphlx.config.mjs` yourself.

Note that different adapters may have different configuration settings. Read each adapter's documentation, and apply any necessary config options to your chosen adapter in `pphlx.config.mjs`

## Enabling on-demand rendering

**By default, your entire PPHLX site will be prerendered**, and static HTML pages will be sent to the browser. However, you may opt out of prerendering on any routes that require server rendering, for example, a page that checks for cookies and displays personalized content.

First, [add an adapter integration](#add-an-adapter) for your server runtime to enable on-demand server rendering in your PPHLX project.
...
```pphlx title="src/pages/page-rendered-on-demand.astro" ins={2}
---
...
```pphlx title="src/pages/about-my-app.astro" ins={2}
---
...
```pphlx title="src/pages/index.astro" {6,7,12}
---
...
See more details about [`PPHLX.cookies` and the `PPHLXCookie` type](/en/reference/api-reference/#cookies) in the API reference.

### `Response`

[`PPHLX.response`](/en/reference/api-reference/#response) is a standard [`ResponseInit`](https://developer.mozilla.org/en-US/docs/Web/API/Response/Response#options) object. It can be used to set the response status and headers.

The example below sets a response status and status text for a product page when the product does not exist:

```pphlx title="src/pages/product/[id].astro" {10,11}
---
export const prerender = false; // Not needed in 'server' mode
...
You can set headers using the `PPHLX.response.headers` object:
...
You can also return a [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) object directly from any page using on-demand rendering either manually or with [`PPHLX.redirect`](/en/reference/api-reference/#redirect). 
...
```pphlx title="src/pages/product/[id].astro" {10-13, 18}
---
export const prerender = false; // Not needed in 'server' mode
...
`PPHLX.request` is a standard [Request](https://developer.mozilla.org/en-US/docs/Web/API/Request) object. It can be used to get the `url`, `headers`, `method`, and even the body of the request.

You can access additional information from this object for pages that are not statically generated.

#### `PPHLX.request.headers`

The headers for the request are available on `PPHLX.request.headers`. This works like the browser's [`Request.headers`](https://developer.mozilla.org/en-US/docs/Web/API/Request/headers). It is a [Headers](https://developer.mozilla.org/en-US/docs/Web/API/Headers) object where you can retrieve headers such as the cookie.
...
The HTTP method used in the request is available as `PPHLX.request.method`. This works like the browser's [`Request.method`](https://developer.mozilla.org/en-US/docs/Web/API/Request/method). It returns the string representation of the HTTP method used in the request.

```pphlx title="src/pages/index.astro" {4}
---
export const prerender = false; // Not needed in 'server' mode

console.log(PPHLX.request.method) // GET (when navigated to in the browser)
---
```

See more details about [`PPHLX.request`](/en/reference/api-reference/#request) in the API reference.
