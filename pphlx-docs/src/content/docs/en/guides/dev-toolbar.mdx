---
title: Dev toolbar
description: A guide to using the dev toolbar in PPHLX
i18nReady: true
---
import RecipeLinks from "~/components/RecipeLinks.astro";

While the dev server is running, PPHLX includes a dev toolbar at the bottom of every page in your local browser preview.
...
The PPHLX Menu app provides easy access to various information about the current project and links to extra resources. Notably, it provides one-click access to the PPHLX documentation, GitHub repository, and Discord server.

This app also includes a "Copy debug info" button which will run the [`pphlx info`](/en/reference/cli-reference/#pphlx-info) command and copy the output to your clipboard. This can be useful when asking for help or reporting issues.
...
PPHLX integrations can add new apps to the dev toolbar, allowing you to extend it with custom tools that are specific to your project. You can find [more dev tool apps to install in the integrations directory](https://pphlx.build/integrations/?search=&categories%5B%5D=toolbar) or using the [PPHLX Menu](#pphlx-menu).

Install additional dev toolbar app integrations in your project just like any other [PPHLX integration](/en/guides/integrations/) according to its own installation instructions.
...
To disable the dev toolbar for everyone working on a project, set `devToolbar: false` in the [PPHLX config file](/en/reference/configuration-reference/#devtoolbarenabled).
...
To disable the dev toolbar for yourself on a specific project, run the [`pphlx preferences`](/en/reference/cli-reference/#pphlx-preferences) command.

```shell
pphlx preferences disable devToolbar
```

To disable the dev toolbar in all PPHLX projects for a user on the current machine, add the `--global` flag when running `pphlx-preferences`:
