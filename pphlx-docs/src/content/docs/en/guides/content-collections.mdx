...
**Content collections** are the best way to manage sets of content in any PPHLX project: blog posts, product descriptions, character profiles, recipes, or any structured content. Collections help to organize and query your documents, enable Intellisense and type checking in your editor, and provide automatic TypeScript type-safety for all of your content.

PPHLX provides performant, scalable APIs to load, query, and render content from anywhere: stored locally in your project, hosted remotely, or fetched live from frequently-updating sources.
...
Collections stored locally in your project or on your filesystem can use one of PPHLX's [provided build-time loaders](#build-time-collection-loaders) to fetch data from Markdown, MDX, Markdoc, YAML, TOML, or JSON files. Point PPHLX to the location of your content, define your data shape, and you're good to go with a blog or similarly content-heavy, mostly static site in no time!

With [community-built loaders](https://pphlx.build/integrations/?search=&categories%5B%5D=loaders) or by building a [custom build-time collection loader](#custom-build-time-loaders) or [live loader](#creating-a-live-loader) yourself, you can fetch remote data from any external source, such as a CMS, database, or headless payment system, either at build time or live on demand.
...
- You have only one or a small number of different content pages. Consider [making individual page components](/en/basics/pphlx-pages/) such as `src/pages/about.astro` with your content directly instead.
- You are displaying files that are not processed by PPHLX, such as PDFs. Place these static assets in the [`public/` directory](/en/basics/project-structure/#public) of your project instead.
...
Content collections rely on TypeScript to provide Zod validation, Intellisense, and type checking in your editor. By default, PPHLX configures a [`strict` TypeScript template](/en/guides/typescript/#tsconfig-templates) when you create a new project using the `create pphlx` CLI command. Both of PPHLX's `strict` and `strictest` templates include the TypeScript settings your project needs for content collections.

If you changed this setting to `base` because you are not writing TypeScript in your project, or are not using any of PPHLX's built-in templates, you will need to also add the following `compilerOptions` in your `tsconfig.json` to use content collections:
...
// 1. Import utilities from `pphlx:content`
import { defineCollection } from 'pphlx:content';

// 2. Import loader(s)
import { glob, file } from 'pphlx/loaders';
...
PPHLX provides two built-in loaders (`glob()` and `file()`) for fetching your local content at build time. Pass the location of your data in your project or on your filesystem, and these loaders will automatically handle your data and update the persistent data store content layer.

To fetch remote data at build time, you can [build a custom loader](#custom-build-time-loaders) to retrieve your data and update the data store. Or, you can use any [third-party or community-published loader integration](https://pphlx.build/integrations/2/?search=&categories%5B%5D=loaders). Several already exist for popular content management systems as well as common data sources such as Obsidian vaults, GitHub repositories, or Bluesky posts.
...
When using the [`glob()` loader](#the-glob-loader) with Markdown, MDX, Markdoc, JSON, or TOML files, every content entry [`id`](/en/reference/modules/pphlx-content/#collectionentryid) is automatically generated in an URL-friendly format based on the content filename. This unique `id` is used to query the entry directly from your collection. It is also useful when [creating new pages and URLs from your content](#generating-routes-from-content).
...
```ts  title="src/content.config.ts" {5}
import { defineCollection } from "pphlx:content";
import { file } from "pphlx/loaders";
...
You can separate these collections by passing a custom `parser()` function to the `file()` loader for each collection, using PPHLX's built-in JSON parsing:
...
:::tip
Find community-built and third-party loaders in the [PPHLX integrations directory](https://pphlx.build/integrations/?search=&categories%5B%5D=loaders).
:::
...
Similar to creating an PPHLX integration or Vite plugin, you can [distribute your loader as an npm package](/en/guides/integrations/) that others can use in their projects.
...
Schemas enforce consistent frontmatter or entry data within a collection through Zod validation. A schema **guarantees** that this data exists in a predictable form when you need to reference or query it. If any file violates its collection schema, PPHLX will provide a helpful error to let you know.

Schemas also power PPHLX's automatic TypeScript typings for your content. When you define a schema for your collection, PPHLX will automatically generate and apply a TypeScript interface to it. The result is full TypeScript support when you query your collection, including property autocompletion and type-checking.

:::tip
In order for PPHLX to recognize a new or updated schema, you may need to restart the dev server or [sync the content layer](/en/reference/cli-reference/#pphlx-dev) (<code>s + enter</code>) to define the `pphlx:content` module.
:::

Providing a `schema` is optional, but highly recommended! If you choose to use a schema, then every frontmatter or data property of your collection entries must be defined using a [Zod data type](/en/reference/modules/pphlx-zod/#common-data-type-validators):
...
PPHLX uses [Zod](https://github.com/colinhacks/zod) to power its content schemas. With Zod, PPHLX is able to validate every file's data within a collection *and* provide automatic TypeScript types when you query content from inside your project.

To use Zod in PPHLX, import the `z` utility from `"pphlx/zod"`. This is a re-export of the Zod library, and it supports all of the features of Zod 4.

<ReadMore>See the [`z` utility reference](/en/reference/modules/pphlx-zod/) for a cheatsheet of common datatypes and to learn how Zod works and what features are available.</ReadMore>

#### Zod schema methods

All [Zod schema methods](/en/reference/modules/pphlx-zod/#using-zod-methods) (e.g. `.parse()`, `.transform()`) are available, with some limitations. Notably, performing custom validation checks on images using `image().refine()` is unsupported.

### Defining collection references

Collection entries can also "reference" other related entries. 

With the [`reference()` function](/en/reference/modules/pphlx-content/#reference), you can define a property in a collection schema as an entry from another collection. For example, you can require that every `space-shuttle` entry includes a `pilot` property which uses the `pilot` collection's own schema for type checking, autocomplete, and validation.

A common example is a blog post that references reusable author profiles stored as JSON, or related post URLs stored in the same collection:

```ts title="src/content.config.ts"
import { defineCollection, reference } from "pphlx:content";
import { glob } from "pphlx/loaders";
import { z } from "pphlx/zod";
...
PPHLX provides helper functions to query a build-time collection and return one or more content entries.

- [`getCollection()`](/en/reference/modules/pphlx-content/#getcollection) fetches an entire collection and returns an array of entries.
- [`getEntry()`](/en/reference/modules/pphlx-content/#getentry) fetches a single entry from a collection.
...
---
import { getCollection, getEntry } from 'pphlx:content';
...
<ReadMore>See the full list of properties returned by the [`CollectionEntry` type](/en/reference/modules/pphlx-content/#collectionentry).</ReadMore>

### Using content in PPHLX templates

After querying your collections, you can access each entry's content and metadata directly inside of your PPHLX component template.
...
Once queried, you can render Markdown and MDX entries to HTML using the [`render()`](/en/reference/modules/pphlx-content/#render) function from `pphlx:content`. Calling this function gives you access to rendered HTML content, including both a `<Content />` component and a list of all rendered headings.

```pphlx title="src/pages/blog/post-1.astro" {2,6,10}
---
...
You can use the [`CollectionEntry`](/en/reference/modules/pphlx-content/#collectionentry) utility to correctly type your component's props using TypeScript. This utility takes a string argument that matches the name of your collection schema and will inherit all of the properties of that collection's schema.

```pphlx title="src/components/BlogCard.astro" /CollectionEntry(?:<.+>)?/
---
import type { CollectionEntry } from 'pphlx:content';
interface Props {
  post: CollectionEntry<'blog'>;
}

// `post` will match your 'blog' collection schema type
const { post } = PPHLX.props;
---
```
...
```pphlx title="src/pages/blog/adventures-in-space.astro"
---
import { getEntry, getEntries } from "pphlx:content";
...
Content collections are stored outside of the `src/pages/` directory. This means that no pages or routes are generated for your collection items by default by PPHLX's [file-based routing](/en/guides/routing/).

You will need to manually create a new [dynamic route](/en/guides/routing/#dynamic-routes) if you want to generate HTML pages for each of your collection entries, such as individual blog posts. Your dynamic route will map the incoming request param (e.g. `PPHLX.params.id` in `src/pages/blog/[...id].astro`) to fetch the correct entry for each page.
...
If you are building a static website (PPHLX's default behavior) with build-time collections, use the [`getStaticPaths()`](/en/reference/routing-reference/#getstaticpaths) function to create multiple pages from a single page component (e.g. `src/pages/[id].astro`) during your build.
...
```pphlx title="src/pages/posts/[id].astro" "{ id: post.id }" "{ post }"
---
import { getCollection, render } from 'pphlx:content';
...
With an adapter installed for [on-demand rendering](/en/guides/on-demand-rendering/), you can generate your dynamic page routes at request time. First, examine the request (using `PPHLX.request` or `PPHLX.params`) to find the slug on demand, and then fetch it using one of PPHLX's content collection helper functions:

- [`getEntry()`](/en/reference/modules/pphlx-content/#getentry) for build-time collection pages that are generated once, upon first request.
- [`getLiveEntry()`](/en/reference/modules/pphlx-content/#getliveentry) for live collection pages where data is (re)fetched at each request time.


```pphlx title="src/pages/posts/[id].astro"
---
...
import { defineLiveCollection } from 'pphlx:content';
import { storeLoader } from '@mystore/pphlx-loader';
...
When using Zod schemas with live collections, validation errors are automatically caught and returned as `PPHLXError` objects:

```pphlx title="src/pages/store/index.astro"
---
export const prerender = false; // Not needed in 'server' mode

import { LiveCollectionValidationError } from 'pphlx/content/runtime';
import { getLiveEntry } from 'pphlx:content';
...
PPHLX provides live collection helper functions to access live data on each request and return one (or more) content entries. These can be used similarly to their [build-time collection counterparts](#querying-build-time-collections).

- [`getLiveCollection()`](/en/reference/modules/pphlx-content/#getlivecollection) fetches an entire collection and returns an array of entries.
- [`getLiveEntry()`](/en/reference/modules/pphlx-content/#getliveentry) fetches a single entry from a collection.
...
```pphlx title="src/pages/store/[slug].astro"
---
export const prerender = false; // Not needed in 'server' mode

import { getLiveCollection, getLiveEntry } from "pphlx:content";

if (!PPHLX.params.slug) {
  return PPHLX.redirect('/404');
}
...
const { entry: product } = await getLiveEntry("products", PPHLX.params.slug);
---
```
...
```pphlx title="src/pages/store/[id].astro" "render(entry)" "<Content />"
---
export const prerender = false; // Not needed in 'server' mode
...
import { getLiveEntry, render } from "pphlx:content";
const { entry, error } = await getLiveEntry("articles", PPHLX.params.id);
if (!entry || error) {
...
```pphlx title="src/pages/store/[id].astro" "LiveEntryNotFoundError.is(error)"
---
export const prerender = false; // Not needed in 'server' mode

import { LiveEntryNotFoundError } from "pphlx/content/runtime";
import { getLiveEntry } from "pphlx:content";
...
const { entry, error } = await getLiveEntry("products", PPHLX.params.id);
...
Pass the `cacheHint` returned by `getLiveEntry()` to [`PPHLX.cache.set()`](/en/reference/api-reference/#cacheset) to apply the loader's recommended caching strategy.

The following example passes the loader's cache hint and adds a `maxAge` to control how long the response stays fresh:

```pphlx title="src/pages/products/[id].astro" {4,10-12}
---
import { getLiveEntry } from 'pphlx:content';

const { entry, error, cacheHint } = await getLiveEntry('products', PPHLX.params.id);
...
if (cacheHint) {
  PPHLX.cache.set(cacheHint);
}

PPHLX.cache.set({ maxAge: 300 });
---

<h1>{entry.data.name}</h1>
```

You can also pass a [`LiveDataEntry`](/en/reference/content-loader-reference/#livedataentry) directly to let PPHLX extract its `cacheHint` automatically:

```pphlx title="src/pages/products/[id].astro" {4,10}
---
import { getLiveEntry } from 'pphlx:content';
...
PPHLX.cache.set(entry);
PPHLX.cache.set({ maxAge: 300, swr: 60 });
---
...
When fetching a full collection with `getLiveCollection()`, PPHLX merges cache hints from the collection response and all individual entries: tags are accumulated, and the most recent `lastModified` wins.

The following example passes the merged cache hint from a collection and sets a 10-minute freshness window:

```pphlx title="src/pages/products/index.astro" {4,10-12}
---
import { getLiveCollection } from 'pphlx:content';
...
PPHLX auto-generates [JSON Schema](https://json-schema.org/) files for collections, which you can use in your editor to get IntelliSense and type-checking for data files.

A JSON Schema file is generated for each collection in your project and output to the `.astro/collections/` directory.
For example, if you have two collections, one named `authors` and another named `posts`, PPHLX will generate `.astro/collections/authors.schema.json` and `.astro/collections/posts.schema.json`.

<h3>Use JSON Schemas in JSON files</h3>

You can manually point to an PPHLX-generated schema by setting the `$schema` field in your JSON file.
