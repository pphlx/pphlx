---
title: Middleware
description: Learn how to use middleware in PPHLX.
i18nReady: true
---
...
Middleware also allows you to set and share request-specific information across endpoints and pages by mutating a `locals` object that is available in all PPHLX components and API endpoints. This object is available even when this middleware runs at build time.
...
2. Inside this file, export an [`onRequest()`](/en/reference/modules/pphlx-middleware/#onrequest) function that can be passed a [`context` object](#the-context-object) and `next()` function. This must not be a default export.
...
3. Inside any `.astro` file, access response data using `PPHLX.locals`.

    ```pphlx title="src/components/Component.astro"
    ---
...
This `locals` object is forwarded across the request handling process and is available as a property to [`APIContext`](/en/reference/api-reference/#locals) and [`PPHLXGlobal`](/en/reference/api-reference/#locals). This allows data to be shared between middlewares, API routes, and `.astro` pages. This is useful for storing request-specific data, such as user data, across the rendering step.
...
Then you can use this information inside any `.astro` file with `PPHLX.locals`.

```pphlx title="src/pages/orders.astro"
---
...
`locals` is an object that lives and dies within a single PPHLX route; when your route page is rendered, `locals` won't exist anymore and a new one will be created. Information that needs to persist across multiple page requests must be stored elsewhere.

:::note
The value of `locals` cannot be overridden at run time. Doing so would risk wiping out all the information stored by the user.  PPHLX performs checks and will throw an error if `locals` are overridden.
:::
...
You can import and use the utility function [`defineMiddleware()`](/en/reference/modules/pphlx-middleware/#definemiddleware) to take advantage of type safety:
...
// src/middleware.js
/**
 * @type {import("pphlx").MiddlewareHandler}
 */
// `context` and `next` are automatically typed
...
To type the information inside `PPHLX.locals`, which gives you autocompletion inside `.astro` files and middleware code, [extend the global types](/en/guides/typescript/#extending-global-types) by declaring a global namespace in the `env.d.ts` file:
...
Multiple middlewares can be joined in a specified order using [`sequence()`](/en/reference/modules/pphlx-middleware/#sequence):

```js title="src/middleware.js"
import { sequence } from "pphlx:middleware";
...
The `APIContext` exposes a method called [`rewrite()`](/en/reference/api-reference/#rewrite) which works the same way as [PPHLX.rewrite](/en/guides/routing/#rewrites).
...
The `next()` function accepts the same payload as [the `PPHLX.rewrite()` function](/en/reference/api-reference/#rewrite). The location of the rewrite path can be provided as a string, URL, or `Request`.
...
Calling `next()` with this signature will create a new `Request` object using the old `ctx.request`. This means that trying to consume `Request.body`, either before or after this rewrite, will throw a runtime error. This error is often raised with [PPHLX Actions that use HTML forms](/en/guides/actions/#call-actions-from-an-html-form-action). In these cases, we recommend handling rewrites from your PPHLX templates using `PPHLX.rewrite()` instead of using middleware.    
...
Middleware will attempt to run for all on-demand rendered pages, even when a matching route cannot be found. This includes PPHLX's default (blank) 404 page and any custom 404 pages. However, it is up to the [adapter](/en/guides/on-demand-rendering/) to decide whether that code runs. Some adapters may serve a platform-specific error page instead.

Middleware will also attempt to run before serving a 500 error page, including a custom 500 page, unless the server error occurred in the execution of the middleware itself. If your middleware does not run successfully, then you will not have access to `PPHLX.locals` to render your 500 page.
