---
title: Share state between PPHLX components
description: Learn how to share state across PPHLX components with Nano Stores.
i18nReady: true
type: recipe
---
...
When building an PPHLX website, you may need to share state across components. PPHLX recommends the use of [Nano Stores](https://github.com/nanostores/nanostores) for shared client storage.
...
3. Import and use the store in a `<script>` tag in the components that will share state.
    ...
    ```pphlx title="src/components/Button.astro"
    ...
    ```pphlx title="src/components/Dialog.astro"
