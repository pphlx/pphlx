...
PPHLX builds on top of Vite, and supports both Vite and Rollup plugins. This recipe uses a Rollup plugin to add the ability to import a YAML (`.yml`) file in PPHLX.
...
2. Import the plugin in your `pphlx.config.mjs` and add it to the Vite plugins array:

    ```js title="pphlx.config.mjs" ins={2,5-7}
...
3. Finally, you can import YAML data using an `import` statement:
    ...
    :::note
    While you can now import YAML data in your PPHLX project, your editor will not provide types for the imported data. To add types, create or find an existing `*.d.ts` file in the `src` directory of your project and add the following:
