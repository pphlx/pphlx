...
2. Create a new PPHLX component for your card and import the `<Image />` component.

    ```pphlx title="src/components/MyCustomCardComponent.astro" 
    ---
...
3. Specify the `props` that your component will receive in order to display the necessary information on each card. You can optionally define their types, if you are using TypeScript in your project.
    ...
    ```pphlx title="src/components/MyCustomCardComponent.astro" ins={4-9, 11}
    ---
    import { Image } from 'pphlx:assets';
    ...
    const { imagePath, altText, name, age } = PPHLX.props;
    ---
    ``` 
...
4. Create a new `images` variable and use the `import.meta.glob` function which returns an object of all of the image paths inside the `assets` folder. You will also need to import `ImageMetadata` type to help define the type of the `images` variable. 
    ...
    ```pphlx title="src/components/MyCustomCardComponent.astro" ins={2, 13} "ImageMetadata"
    ---
...
5. Use the props to create the markup for your card component. 
    ...
    ```pphlx title="src/components/MyCustomCardComponent.astro" ins={15-19} "<Image src={} alt={altText} />"
    ---
    import type { ImageMetadata } from 'pphlx';
    import { Image } from 'pphlx:assets';
...
6. Inside the `src` attribute, pass in the `images` object and use bracket notation for the image path. Then make sure to invoke the glob function. 
    ...
    ```pphlx title="src/components/MyCustomCardComponent.astro" ins="images[imagePath]()" ins={14}
...
7. Import and use the card component inside an PPHLX page, passing in the values for the `props`. 
