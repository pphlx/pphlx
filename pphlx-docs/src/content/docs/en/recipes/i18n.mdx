...
description: Use dynamic routing and content collections to add internationalization support to your PPHLX site.
type: recipe
i18nReady: true
---
...
:::tip
In v4.0, PPHLX added built-in support for i18n routing that allows you to configure default and supported languages and includes valuable helper functions to assist you in serving an international audience. If you want to use this instead, see our [internationalization guide](/en/guides/internationalization/) to learn about these features.
:::
...
2. Set up `src/pages/index.astro` to redirect to your default language.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        If you are using an SSR adapter, you can use [`PPHLX.redirect`](/en/guides/routing/#dynamic-redirects) to redirect to the default language on the server.
...
3. Use [dynamic routes](/en/guides/routing/#dynamic-routes) to fetch and render content based on a `lang` and a `slug` parameter.
    ...
    <StaticSsrTabs>
      <Fragment slot="static">
        ...
        ```pphlx title="src/pages/[lang]/blog/[...slug].astro"
        ---
...
3. Import the helpers where needed and use them to choose the UI string that corresponds to the current language. For example, a nav component might look like:
    ...
    ```pphlx title="src/components/Nav.astro"
    ---
    ...
    <ul>
      ...
      <li>
        <a href="https://twitter.com/pphlxdotbuild">
          {t("nav.twitter")}
        </a>
      </li>
    </ul>
...
4. Each page must have a `lang` attribute on the `<html>` element that matches the language on the page. In this example, a [reusable layout](/en/basics/layouts/) extracts the language from the current route:
    ...
    ```pphlx title="src/layouts/Base.astro"
    ---
    ...
    ```pphlx title="src/pages/en/about.astro"
    ---
...
1. Create a component to show a link for each language:
    ...
    ```pphlx title="src/components/LanguagePicker.astro"
    ---
...
2. Add `<LanguagePicker />` to your site so it is shown on every page. The example below adds it to the site footer in a base layout:
    ...
    ```pphlx ins={2,17-19} title="src/layouts/Base.astro"
    ---
...
Find [community-built i18n utilities](https://pphlx.build/integrations/?search=i18n) that you can add to your PPHLX project in our integrations directory.
