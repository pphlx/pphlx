---
title: Build your PPHLX site with Docker
description: Learn how to build your PPHLX site using Docker.
type: recipe
i18nReady: true
---
...
This Dockerfile will build your site and serve it using Node.js on port `4321` and therefore requires the [Node adapter](/en/guides/integrations-guide/node/) installed in your PPHLX project.
...
<Steps>
1. Build your container by running the following command in your project's root directory. Use any name for `<your-pphlx-image-name>`:

    ```bash
    docker build -t <your-pphlx-image-name> .
    ```
...
2. To run your image as a local container, use the following command. 
    ...
    ```bash
    docker run -p <local-port>:<container-port> <your-pphlx-image-name>
    ```
