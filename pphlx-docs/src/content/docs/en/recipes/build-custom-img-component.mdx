...
PPHLX provides two built-in components that you can use to display and optimize your images.  The `<Picture>` component allows you to display responsive images and work with different formats and sizes. The `<Image>` component will optimize your images and allow you to pass in different formats and quality properties.
...
<Steps>
1. Create a new PPHLX component and import the `getImage()` function

    ```pphlx title="src/components/MyCustomImageComponent.astro" 
    ---
...
3. Define each of your responsive images by calling the `getImage()` function with your desired properties.
    ...
    ```pphlx title="src/components/MyCustomImageComponent.astro" ins={13-18, 20-25}
    ---
    import type { ImageMetadata } from "pphlx";
    import { getImage } from "pphlx:assets";
...
4. Create a `<picture>` element that generates a `srcset` with your different images based on your desired media queries. 
    ...
    ```pphlx title="src/components/MyCustomImageComponent.astro" ins={28-32}
    ---
    import type { ImageMetadata } from "pphlx";
    import { getImage } from "pphlx:assets";
