...
You can use [Tailwind](https://tailwindcss.com)'s Typography plugin to style rendered Markdown from sources such as PPHLX's [**content collections**](/en/guides/content-collections/).

This recipe will teach you how to create a reusable PPHLX component to style your Markdown content using Tailwind's utility classes.

## Prerequisites

An PPHLX project that:

	- has [Tailwind's Vite plugin](/en/guides/styling/#tailwind) installed.
	- uses PPHLX's [content collections](/en/guides/content-collections/).
...
1. Create a `<Prose />` component to provide a wrapping `<div>` with a `<slot />` for your rendered Markdown. Add the style class `prose` alongside any desired [Tailwind element modifiers](https://tailwindcss.com/docs/typography-plugin#element-modifiers) in the parent element.
    ...
    ```pphlx title="src/components/Prose.astro"
    ---
    ---
    <div 
