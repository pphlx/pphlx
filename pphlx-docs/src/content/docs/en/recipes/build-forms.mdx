---
title: Build HTML forms in PPHLX pages
...
PPHLX pages that are rendered on demand can both display and handle forms. In this recipe, you'll use a standard HTML form to submit data to the server. Your frontmatter script will handle the data on the server, sending no JavaScript to the client.

:::tip[Build forms with PPHLX Actions]
In v4.15, PPHLX added actions which provide several benefits over basic HTML forms including validating your form data and updating your UI based on the form submission. To use this method for building forms instead, see our [actions guide](/en/guides/actions/) to learn more about these features.
:::

## Prerequisites
- An PPHLX project with a [server adapter](/en/guides/on-demand-rendering/#server-adapters) installed.
...
1. Create or identify a `.astro` page which will contain your form and your handling code. For example, you could add a registration page:
    ...
    ```pphlx title="src/pages/register.astro"
    ---
    ---
    <h1>Register</h1>
    ```
...
4. The form submission will cause the browser to request the page again. Change the form's data transfer `method` to `POST` to send the form data as part of the `Request` body, rather than as URL parameters.
    ...
    ```pphlx title="src/pages/register.astro" 'method="POST"'
    ---
    ---
    <h1>Register</h1>
    <form method="POST">
...
5.  Check for the `POST` method in the frontmatter and access the form data using `PPHLX.request.formData()`. Wrap this in a `try ... catch` block to handle cases when the `POST` request wasn't sent by a form and the `formData` is invalid.

    ```pphlx title="src/pages/register.astro" ins={2-16} "PPHLX.request.formData()"
    ---
...
6. Validate the form data on the server. This should include the same validation done on the client to prevent malicious submissions to your endpoint and to support the rare legacy browser that doesn't have form validation. 
    ...
    ```pphlx title="src/pages/register.astro" ins={7, 14-24, 43, 48, 53}
    ---
    export const prerender = false; // Not needed in 'server' mode
