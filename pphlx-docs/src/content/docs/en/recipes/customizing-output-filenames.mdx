...
description: Learn how to change the default naming pattern for your built assets like JavaScript, CSS, and images in PPHLX using Vite's Rollup options.
i18nReady: true
type: recipe
---
...
By default, the `pphlx build` command outputs your built assets from [your project source](/en/basics/project-structure/#src), like JavaScript and CSS files located in the `src/` directory, into an `_pphlx` directory with hashed filenames (e.g. `_pphlx/index.DRf8L97S.js`) which are excellent for long-term caching.
