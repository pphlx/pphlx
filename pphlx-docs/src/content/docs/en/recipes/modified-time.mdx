...
4. Display Last Modified Time
   ...
   ```pphlx title="src/pages/posts/[slug].astro" {3-4,6,17,19-21,28}
   ---
   ...
   If you're using a [Markdown layout](/en/basics/layouts/#markdown-layouts), use the `lastModified` frontmatter property from `PPHLX.props` in your layout template.

   ```pphlx title="src/layouts/BlogLayout.astro" {2-3,5,7-9,15}
   ---
   import dayjs from "dayjs";
   import utc from "dayjs/plugin/utc";
