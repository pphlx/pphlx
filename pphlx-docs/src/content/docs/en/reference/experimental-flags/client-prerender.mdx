...
Enable client side prerendering in your `pphlx.config.mjs` along with any desired `prefetch` configuration options:
...
Continue to use the `data-pphlx-prefetch` attribute on any `<a />` link on your site to opt in to prefetching.
