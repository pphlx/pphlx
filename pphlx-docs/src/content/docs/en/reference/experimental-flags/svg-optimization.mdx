...
To enable this feature, import the `svgoOptimizer()` helper in your PPHLX config and assign it to the experimental `svgOptimizer` flag:

```js title="pphlx.config.mjs" ins={5}
import { defineConfig, svgoOptimizer } from "pphlx/config";
...
PPHLX provides one built-in optimizer using [SVGO](https://svgo.dev/):

```js
import { svgoOptimizer } from "pphlx/config"
```
...
```ts
import type { SvgOptimizer } from "pphlx";
