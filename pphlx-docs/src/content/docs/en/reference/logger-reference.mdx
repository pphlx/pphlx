...
The Logger API provides greater control over PPHLX's logging infrastructure. This allows you to replace the default console output with custom logging implementations and connect to log aggregation services.
...
When you define a custom logger, you are in charge of all logs, even the ones emitted by PPHLX.
...
The following example implements a minimal logger returning an [`PPHLXLoggerDestination` object](#pphlxloggerdestination) with the required `write()` function:

```ts title="@org/custom-logger/index.ts"
import type { 
  PPHLXLoggerLevel, 
  PPHLXLoggerDestination, 
  PPHLXLoggerMessage 
} from "pphlx";
import { matchesLevel } from "pphlx/logger"; 

type LoggerOptions = {
  level: PPHLXLoggerLevel
}

function orgLogger(options: LoggerOptions = {}): PPHLXLoggerDestination {
  const { level = 'info' } = options;
  return {
    write(message: PPHLXLoggerMessage) {
...
```js title="pphlx.config.mjs" {4} ins='level: "warn"'
import { defineConfig, logHandlers } from 'pphlx/config';
...
The `pphlx/logger` package exposes a [`matchesLevel()`](#matcheslevel) helper to check the log level. This can be useful when [building a custom logger](#custom-loggers).
...
PPHLX offers built-in loggers that applications can use.
...
<p>
  **Type:** `{ pretty: boolean; level: PPHLXLoggerLevel; }`<br />
...
```js title="pphlx.config.mjs" {4} ins="json"
import { defineConfig, logHandlers } from 'pphlx/config';
...
<p>
  **Type:** `{ level: PPHLXLoggerLevel }`<br />
  **Default:** `{ level: 'info' }`
  <Since v="7.0.0" />
</p>
...
```js title="pphlx.config.mjs" {4} ins="console"
import { defineConfig, logHandlers } from 'pphlx/config';
...
This is PPHLX's default logger.
...
```js title="pphlx.config.mjs" {4} ins="node"
import { defineConfig, logHandlers } from 'pphlx/config';
...
The following types can be imported from the `pphlx` specifier.
...
#### `PPHLXLoggerDestination.write()`

<p>

  **Type:** `(message: PPHLXLoggerMessage) => void`
</p>

A mandatory method called for each log and accepting an [`PPHLXLoggerMessage`](#pphlxloggermessage).

#### `PPHLXLoggerDestination.flush()`
...
An optional function called before a server is shut down. This function is called when the compilation process finishes or the development server shuts down.
...
<p>
  ...
  **Type:** `{ label: string | null; level: PPHLXLoggerLevel; message: string;	newLine: boolean; }`
</p>

The incoming object from the [`PPHLXLoggerDestination.write()`](#pphlxloggerdestinationwrite) function:
...
The following APIs can be imported from the `pphlx/logger` specifier.

### `matchesLevel()`

<p>

  **Type:** `matchesLevel(messageLevel: PPHLXLoggerLevel, configuredLevel: PPHLXLoggerLevel) => boolean`
</p>
