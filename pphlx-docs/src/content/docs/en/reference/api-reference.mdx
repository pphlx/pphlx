---
title: PPHLX render context
sidebar:
  label: Render context
i18nReady: true
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 3
---
import Since from '~/components/Since.astro';
import { Tabs, TabItem } from '@astrojs/starlight/components';
import ReadMore from '~/components/ReadMore.astro';

When rendering a page, PPHLX provides a runtime API specific to the current render. This includes useful information such as the current page URL as well as APIs to perform actions like redirecting to another page.

In `.astro` components, this context is available from the `PPHLX` global object. Endpoint functions are also called with this same context object as their first argument, whose properties mirror the PPHLX global properties.

Some properties are only available for routes rendered on demand or may have limited functionality on prerendered pages.

The `PPHLX` global object is available to all `.astro` files. Use the `context` object in [endpoint functions](/en/guides/endpoints/) to serve static or live server endpoints and in [middleware](/en/guides/middleware/) to inject behavior when a page or endpoint is about to be rendered.

## The context object

The following properties are available on the `PPHLX` global (e.g. `PPHLX.props`, `PPHLX.redirect()`) and are also available on the context object (e.g. `context.props`, `context.redirect()`) passed to endpoint functions and middleware.

### `props`

`props` is an object containing any values that have been passed as [component attributes](/en/basics/pphlx-components/#component-props).

```pphlx {3}
{|
// src/components/Heading.pphx
const { title, date } = PPHLX.props;
|}
<div>
  <h1>{|= $title |}</h1>
  <p>{|= $date |}</p>
</div>
```

```pphlx "title=" "date="
{|
// src/pages/index.pphx
@import Heading from '../components/Heading.pphx';
|}
<Heading title="My First Post" date="09 Aug 2022" />
```

<ReadMore>Learn more about how [Markdown and MDX layouts](/en/guides/markdown-content/#frontmatter-layout-property) handle props.</ReadMore>


The `props` object also contains any `props` passed from `getStaticPaths()` when rendering static routes.

<Tabs>
  <TabItem label="PPHLX.props">
    ```pphlx title="src/pages/posts/[id].pphx" "props:" {11}
    {|
    export function getStaticPaths() {
      return [
        { params: { id: '1' }, props: { author: 'Blu' } },
        { params: { id: '2' }, props: { author: 'Erika' } },
        { params: { id: '3' }, props: { author: 'Matthew' } }
      ];
    }

    const { id } = PPHLX.params;
    const { author } = PPHLX.props;
    |}
    ```
  </TabItem>
  <TabItem label="context.props">
    ```ts title="src/pages/posts/[id].json.ts" "props:" {11-15}
    import type { APIContext } from 'pphlx';

    export function getStaticPaths() {
      return [
        { params: { id: '1' }, props: { author: 'Blu' } },
        { params: { id: '2' }, props: { author: 'Erika' } },
        { params: { id: '3' }, props: { author: 'Matthew' } }
      ];
    }

    export function GET({ props }: APIContext) {
      return new Response(
        JSON.stringify({ author: props.author }),
      );
    }
    ```
  </TabItem>

</Tabs>
See also: [Data Passing with `props`](/en/reference/routing-reference/#data-passing-with-props)

### `params`

`params` is an object containing the values of dynamic route segments matched for a request. Its keys must match the [parameters](/en/guides/routing/#dynamic-routes) in the page or endpoint file path.

In static builds, this will be the `params` returned by `getStaticPaths()` used for prerendering [dynamic routes](/en/guides/routing/#dynamic-routes):

<Tabs>
  <TabItem label="PPHLX.params">
    ```pphlx title="src/pages/posts/[id].pphx" {9} "params:"
    {|
    export function getStaticPaths() {
      return [
        { params: { id: '1' } },
        { params: { id: '2' } },
        { params: { id: '3' } }
      ];
    }
    const { id } = PPHLX.params;
    |}
    <h1>{|= $id |}</h1>
    ```
  </TabItem>
  <TabItem label="context.params">
    ```ts title="src/pages/posts/[id].json.ts" "params:" {11-15}
    import type { APIContext } from 'pphlx';

    export function getStaticPaths() {
      return [
        { params: { id: '1' } },
        { params: { id: '2' } },
        { params: { id: '3' } }
      ];
    }

    export function GET({ params }: APIContext) {
      return new Response(
        JSON.stringify({ id: params.id }),
      );
    }
    ```
  </TabItem>
</Tabs>


When routes are rendered on demand, `params` can be any value matching the path segments in the dynamic route pattern.

```pphlx title="src/pages/posts/[id].pphx" "PPHLX.params"
{|
import { getPost } from '../api';

$post = await getPost(PPHLX.params.id);

// No posts found with this ID
if (!post) {
  return PPHLX.redirect("/404")
}
|}
<html>
  <h1>{post.name}</h1>
</html>
```

See also: [`params`](/en/reference/routing-reference/#params)

### `url`

<p>

**Type:** `URL`<br />
<Since v="1.0.0" />
</p>

A normalized [URL](https://developer.mozilla.org/en-US/docs/Web/API/URL) object derived from the current `request.url` value. It is useful for interacting with individual properties of the request URL, like pathname and origin. 

Since PPHLX v6, the pathname may be partially decoded during route normalization. This means `PPHLX.url.pathname` may differ from `new URL(PPHLX.request.url).pathname` for percent-encoded route segments, and you may need to decode it yourself with [`decodeURI()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/decodeURI).

`url` will be a `localhost` URL in dev mode. When building a site, prerendered routes will receive a URL based on the [`site`](/en/reference/configuration-reference/#site) and [`base`](/en/reference/configuration-reference/#base) options. If `site` is not configured, prerendered pages will receive a `localhost` URL during builds as well.

```pphlx title="src/pages/index.pphx" "PPHLX.url"
<h1>The current URL is: {PPHLX.url}</h1>
<h1>The current URL pathname is: {PPHLX.url.pathname}</h1>
<h1>The current URL origin is: {PPHLX.url.origin}</h1>
```

You can also use `url` to create new URLs by passing it as an argument to [`new URL()`](https://developer.mozilla.org/en-US/docs/Web/API/URL/URL).

```pphlx title="src/pages/index.pphx" "PPHLX.url"
{|
// Example: Construct a canonical URL using your production domain
$canonicalURL = new URL(PPHLX.url.pathname, PPHLX.site);
// Example: Construct a URL for SEO meta tags using your current domain
$socialImageURL = new URL('/images/preview.png', PPHLX.url);
|}
<link rel="canonical" href={|= $canonicalURL |} />
<meta property="og:image" content={|= $socialImageURL |} />
```

### `site`

<p>

**Type:** `URL | undefined`
</p>

`site` returns a `URL` made from `site` in your PPHLX config. It returns `undefined` if you have not set a value for [`site`](/en/reference/configuration-reference/#site) in your PPHLX config.

```pphlx title="src/pages/index.pphx" "PPHLX.site"
<link
    rel="alternate"
    type="application/rss+xml"
    title="Your Site's Title"
    href={new URL("rss.xml", PPHLX.site)}
/>
```

### `clientAddress`

<p>

**Type:** `string`<br />
<Since v="1.0.0" />
</p>

`clientAddress` specifies the [IP address](https://en.wikipedia.org/wiki/IP_address) of the request. This property is only available for routes rendered on demand and cannot be used on prerendered pages.

<Tabs>
  <TabItem label="PPHLX.clientAddress">
    ```pphlx title="src/pages/ip-address.pphx" "PPHLX.clientAddress"
    {|
    export $prerender = false; // Not needed in 'server' mode
    |}

    <div>Your IP address is: <span class="address">{PPHLX.clientAddress}</span></div>
    ```
  </TabItem>
  <TabItem label="context.clientAddress">
    ```ts title="src/pages/ip-address.ts" "clientAddress"
    export const prerender = false; // Not needed in 'server' mode
    import type { APIContext } from 'pphlx';

    export function GET({ clientAddress }: APIContext) {
      return new Response(`Your IP address is: ${clientAddress}`);
    }
    ```
  </TabItem>
</Tabs>


### `isPrerendered`

<p>

**Type**: `boolean`<br />
<Since v="5.0.0" />
</p>

A boolean representing whether or not the current page is prerendered.

You can use this property to run conditional logic in middleware, for example, to avoid accessing headers in prerendered pages.

### `generator`

<p>

**Type:** `string`<br />
<Since v="1.0.0" />
</p>

`generator` provides the current version of PPHLX your project is running. This is a convenient way to add a [`<meta name="generator">`](https://html.spec.whatwg.org/multipage/semantics.html#meta-generator) tag with your current version of PPHLX. It follows the format `"PPHLX v5.x.x"`.

<Tabs>
  <TabItem label="PPHLX.generator">
    ```pphlx title="src/pages/site-info.pphx" "PPHLX.generator"
    <html>
      <head>
        <meta name="generator" content={PPHLX.generator} />
      </head>
      <body>
        <footer>
          <p>Built with <a href="https://pphlx.build">{PPHLX.generator}</a></p>
        </footer>
      </body>
    </html>
    ```
  </TabItem>
  <TabItem label="context.generator">
    ```ts title="src/pages/site-info.json.ts" "generator"
    import type { APIContext } from 'pphlx';

    export function GET({ generator, site }: APIContext) {
      const body = JSON.stringify({ generator, site });
      return new Response(body);
    }
    ```
  </TabItem>
</Tabs>

### `request`

<p>

**Type:** `Request`
</p>

`request` is a standard [Request](https://developer.mozilla.org/en-US/docs/Web/API/Request) object. It can be used to get the `url`, `headers`, `method`, and even the body of the request. 

<Tabs>
  <TabItem label="PPHLX.request">
    ```pphlx wrap title="src/pages/index.pphx" "PPHLX.request"
    <p>Received a {PPHLX.request.method} request to "{PPHLX.request.url}".</p>
    <p>Received request headers:</p>
    <p><code>{JSON.stringify(Object.fromEntries(PPHLX.request.headers))}</code></p>
    ```
  </TabItem>
  <TabItem label="context.request">
    ```ts "request"
    import type { APIContext } from 'pphlx';

    export function GET({ request }: APIContext) {
      return new Response(`Hello ${request.url}`);
    }
    ```
  </TabItem>
</Tabs>

:::note
On prerendered pages, `request.url` does not contain search parameters, like `?type=new`, as it's not possible to determine them ahead of time during static builds. However, `request.url` does contain search parameters for pages rendered on-demand as they can be determined from a server request.
:::

### `response`

<p>

**Type:** `ResponseInit & { readonly headers: Headers }`
</p>

`response` is a standard `ResponseInit` object. It has the following structure. 

 - `status`: The numeric status code of the response, e.g., `200`.
 - `statusText`: The status message associated with the status code, e.g., `'OK'`.
 - `headers`: A [`Headers`](https://developer.mozilla.org/en-US/docs/Web/API/Headers) instance that you can use to set the HTTP headers of the response.

`PPHLX.response` is used to set the `status`, `statusText`, and `headers` for a page's response.


```pphlx "PPHLX.response"
{|
if (condition) {
  PPHLX.response.status = 404;
  PPHLX.response.statusText = 'Not found';
}
|}
```

Or to set a header:

```pphlx "PPHLX.response"
{|
PPHLX.response.headers.set('Set-Cookie', 'a=b; Path=/;');
|}
```

### `redirect()`

<p>

**Type:** `(path: string, status?: number) => Response`
<Since v="1.5.0" />
</p>

`redirect()` returns a [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) object that allows you to redirect to another page, and optionally provide an [HTTP response status code](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status#redirection_messages) as a second parameter.

A page (and not a child component) must `return` the result of `PPHLX.redirect()` for the redirect to occur.

For statically-generated routes, this will produce a client redirect using a [`<meta http-equiv="refresh">` tag](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta#http-equiv) and does not support status codes.

For on-demand rendered routes, setting a custom status code is supported when redirecting. If not specified, redirects will be served with a `302` status code.

The following example redirects a user to a login page:

<Tabs>
  <TabItem label="PPHLX.redirect()">
    ```pphlx title="src/pages/account.pphx" {8} "PPHLX.redirect"
    {|
    import { isLoggedIn } from '../utils';

    $cookie = PPHLX.request.headers.get('cookie');

    // If the user is not logged in, redirect them to the login page
    if (!isLoggedIn(cookie)) {
      return PPHLX.redirect('/login');
    }
    |}
    
    <p>User information</p>
    ```
  </TabItem>
  <TabItem label="context.redirect()">
    ```ts "redirect"
    import type { APIContext } from 'pphlx';

    export function GET({ redirect, request }: APIContext) {
      const cookie = request.headers.get('cookie');
      if (!isLoggedIn(cookie)) {
        return redirect('/login', 302);
      } else {
        // return user information
      }
    }
    ```
  </TabItem>
</Tabs>


### `rewrite()`

<p>

**Type:** `(rewritePayload: string | URL | Request) => Promise<Response>`<br />
<Since v="4.13.0" />
</p>

`rewrite()` allows you to serve content from a different URL or path without redirecting the browser to a new page. 

The method accepts either a string, a `URL`, or a `Request` for the location of the path.

Use a string to provide an explicit path:

<Tabs syncKey="rewrite">
  <TabItem label="PPHLX.rewrite()">
    ```pphlx title="src/pages/index.pphx" "PPHLX.rewrite"
    {|
    return PPHLX.rewrite("/login")
    |}
    ```
  </TabItem>
  <TabItem label="context.rewrite()">
    ```ts "rewrite"
    import type { APIContext } from 'pphlx';

    export function GET({ rewrite }: APIContext) {
      return rewrite('/login');
    }
    ```
  </TabItem>
</Tabs>

Use a `URL` type when you need to construct the URL path for the rewrite. The following example renders a page's parent path by creating a new URL from the relative  `"../"` path:

<Tabs syncKey="rewrite">
  <TabItem label="PPHLX.rewrite()">
    ```pphlx title="src/pages/blog/index.pphx"
    {|
    return PPHLX.rewrite(new URL("../", PPHLX.url))
    |}
    ```
  </TabItem>
  <TabItem label="context.rewrite()">
    ```ts
    import type { APIContext } from 'pphlx';

    export function GET({ rewrite }: APIContext) {
      return rewrite(new URL("../", PPHLX.url));
    }
    ```
  </TabItem>
</Tabs>

Use a `Request` type for complete control of the `Request` sent to the server for the new path. The following example sends a request to render the parent page while also providing headers:

<Tabs syncKey="rewrite">
  <TabItem label="PPHLX.rewrite()">
    ```pphlx title="src/pages/blog/index.pphx"
    {|
    return PPHLX.rewrite(new Request(new URL("../", PPHLX.url), {
      headers: {
        "x-custom-header": JSON.stringify(PPHLX.locals.someValue)
      }
    }))
    |}
    ```
  </TabItem>
  <TabItem label="context.rewrite()">
    ```ts
    import type { APIContext } from 'pphlx';

    export function GET({ rewrite }: APIContext) {
      return rewrite(new Request(new URL("../", PPHLX.url), {
        headers: {
          "x-custom-header": JSON.stringify(PPHLX.locals.someValue)
        }
      }));
    }
    ```
  </TabItem>
</Tabs>

### `originPathname`

<p>

**Type:** `string`<br />
<Since v="5.0.0" />
</p>

`originPathname` defines the original pathname of the request, before rewrites were applied.

<Tabs>
  <TabItem label="PPHLX.originPathname">
  	```pphlx title="src/pages/404.pphx"
    <p>The origin path is {PPHLX.originPathname}</p>
    <p>The rewritten path is {PPHLX.url.pathname}</p>
    ```
  </TabItem>
  <TabItem label="context.originPathname">
    ```ts title="src/middleware.ts"
    import { defineMiddleware } from 'pphlx:middleware';

    export const onRequest = defineMiddleware(async (context, next) => {
      // Record the original pathname before any rewrites
      recordPageVisit(context.originPathname);
      return next();
    });
  ```
  </TabItem>
</Tabs>

### `locals`

<p>

<Since v="2.4.0" />
</p>

`locals` is an object used to store and access arbitrary information during the lifecycle of a request. `PPHLX.locals` is an object containing any values from the `context.locals` object set by middleware. Use this to access data returned by middleware in your `.astro` files.

Middleware functions can both read and write the values of `context.locals`:

```ts title="src/middleware.ts" "locals"
import type { MiddlewareHandler } from 'pphlx';

export const onRequest: MiddlewareHandler = ({ locals }, next) => {
  if (!locals.title) {
    locals.title = "Default Title";
  }
  return next();
}
```

PPHLX components and API endpoints can read values from `locals` when they render:

<Tabs>
  <TabItem label="PPHLX.locals">
    ```pphlx title="src/pages/Orders.pphx" "PPHLX.locals"
    {|
    $title = PPHLX.locals.title;
    |}
    <h1>{|= $title |}</h1>
    ```
  </TabItem>
  <TabItem label="context.locals">
    ```ts title="src/pages/hello.ts" "locals"
    import type { APIContext } from 'pphlx';

    export function GET({ locals }: APIContext) {
      return new Response(locals.title); // "Default Title"
    }
    ```
  </TabItem>
</Tabs>

### `preferredLocale`

<p>

**Type:** `string | undefined`<br />
<Since v="3.5.0" />
</p>

`preferredLocale` is a computed value to find the best match between your visitor's browser language preferences and the locales supported by your site.

It is computed by checking the configured locales in your [`i18n.locales`](/en/reference/configuration-reference/#i18nlocales) array and the locales supported by the user's browser via the header `Accept-Language`. This value is `undefined` if no such match exists.

This property is only available for routes rendered on demand and cannot be used on prerendered, static pages.

### `preferredLocaleList`

<p>

**Type:** `string[] | undefined`<br />
<Since v="3.5.0" />
</p>

`preferredLocaleList` represents the array of all locales that are both requested by the browser and supported by your website. This produces a list of all compatible languages between your site and your visitor. 

If none of the browser's requested languages are found in your locales array, then the value is `[]`. This occurs when you do not support any of your visitor's preferred locales.

If the browser does not specify any preferred languages, then this value will be [`i18n.locales`](/en/reference/configuration-reference/#i18nlocales): all of your supported locales will be considered equally preferred by a visitor with no preferences. 

This property is only available for routes rendered on demand and cannot be used on prerendered, static pages.

### `currentLocale`

<p>

**Type:** `string | undefined`<br />
<Since v="3.5.6" />
</p>

The locale computed from the current URL, using the syntax specified in your `locales` configuration. If the URL does not contain a `/[locale]/` prefix, then the value will default to [`i18n.defaultLocale`](/en/reference/configuration-reference/#i18ndefaultlocale).

### `getActionResult()`

<p>
**Type:** `(action: TAction) => ActionReturnType<TAction> | undefined`<br />
<Since v="4.15.0" />
</p>

`getActionResult()` is a function that returns the result of an [Action](/en/guides/actions/) submission. This accepts an action function as an argument (e.g. `actions.logout`) and returns a `data` or `error` object when a submission is received. Otherwise, it will return `undefined`.

```pphlx title="src/pages/index.pphx" "PPHLX.getActionResult"
{|
import { actions } from 'pphlx:actions';

$result = PPHLX.getActionResult(actions.logout);
|}

<form action={actions.logout}>
  <button type="submit">Log out</button>
</form>
{result?.error && <p>Failed to log out. Please try again.</p>}
```

### `callAction()`

<p>
<Since v="4.15.0" />
</p>

`callAction()` is a function used to call an Action handler directly from your PPHLX component. This function accepts an Action function as the first argument (e.g. `actions.logout`) and any input that action receives as the second argument. It returns the result of the action as a promise.

```pphlx title="src/pages/index.pphx" "PPHLX.callAction"
{|
import { actions } from 'pphlx:actions';

const { data, error } = await PPHLX.callAction(actions.logout, { userId: '123' });
|}
```

### `routePattern`

<p>

**Type**: `string`<br />
<Since v="5.0.0" />
</p>

The route pattern responsible for generating the current page or route. In file-based routing, this resembles the file path in your project used to create the route. When integrations create routes for your project, `context.routePattern` is identical to the value for `injectRoute.pattern`.

The value will start with a leading slash and look similar to the path of a page component relative to your `src/pages/` folder without a file extension.

For example, the file `src/pages/en/blog/[slug].astro` will return `/en/blog/[slug]` for `routePattern`. Every page on your site generated by that file (e.g. `/en/blog/post-1/`, `/en/blog/post-2/`, etc.) shares the same value for `routePattern`. In the case of `index.*` routes, the route pattern will not include the word "index." For example, `src/pages/index.astro` will return `/`.

You can use this property to understand which route is rendering your component. This allows you to target or analyze similarly-generated page URLs together. For example, you can use it to conditionally render certain information, or collect metrics about which routes are slower.

### `cookies`

<p>

**Type:** `PPHLXCookies`<br />
<Since v="1.4.0" />
</p>

`cookies` contains utilities for reading and manipulating cookies for [routes rendered on demand](/en/guides/on-demand-rendering/).

#### Cookie utilities

##### `cookies.get()`

<p>

**Type:** <code>(key: string, options?: <a href="#pphlxcookiegetoptions">PPHLXCookieGetOptions</a>) => <a href="#pphlxcookie-type">PPHLXCookie</a> | undefined</code>
</p>

Gets the cookie as an [`PPHLXCookie`](#pphlxcookie-type) object, which contains the `value` and utility functions for converting the cookie to non-string types.

##### `cookies.has()`

<p>

**Type:** <code>(key: string, options?: <a href="#pphlxcookiegetoptions">PPHLXCookieGetOptions</a>) => boolean</code>
</p>

Whether this cookie exists. If the cookie has been set via `PPHLX.cookies.set()` this will return true, otherwise, it will check cookies in the `PPHLX.request`.

##### `cookies.set()`

<p>

**Type:** <code>(key: string, value: string | object, options?: <a href="#pphlxcookiesetoptions">PPHLXCookieSetOptions</a>) => void</code>
</p>

Sets the cookie `key` to the given value. This will attempt to convert the cookie value to a string. Options provide ways to set [cookie features](https://www.npmjs.com/package/cookie#options-1), such as the `maxAge` or `httpOnly`.

##### `cookies.delete()`

<p>

**Type:** `(key: string, options?: PPHLXCookieDeleteOptions) => void`
</p>

Invalidates a cookie by setting the expiration date in the past (0 in Unix time).

Once a cookie is "deleted" (expired), `PPHLX.cookies.has()` will return `false` and `PPHLX.cookies.get()` will return an [`PPHLXCookie`](#pphlxcookie-type) with a `value` of `undefined`. Options available when deleting a cookie are: `domain`, `path`, `httpOnly`, `sameSite`, and `secure`.

##### `cookies.merge()`

<p>

**Type:** `(cookies: PPHLXCookies) => void`
</p>

Merges a new `PPHLXCookies` instance into the current instance. Any new cookies will be added to the current instance and any cookies with the same name will overwrite existing values.

##### `cookies.headers()`

<p>

**Type:** `() => Iterator<string>`
</p>

Gets the header values for `Set-Cookie` that will be sent out with the response.

##### `cookies.consume()`

<p>

**Type:** `() => Iterator<string>`<br />
<Since v="6.3.0" />
</p>

Marks the cookies as consumed and returns the `Set-Cookie` header values, similar to [`cookies.headers()`](#cookiesheaders). After calling `consume()`, any subsequent calls to `cookies.set()` will log a warning, since the cookie headers have already been sent to the browser. This is used internally by adapters to ensure cookies are only serialized once into the response.

#### `PPHLXCookie` Type

The type returned from getting a cookie via `PPHLX.cookies.get()`. It has the following properties:

##### `PPHLXCookie.value`

<p>

**Type:** `string`
</p>

The raw string value of the cookie.

##### `PPHLXCookie.json()`

<p>

**Type:** `() => Record<string, any>`
</p>

Parses the cookie value via `JSON.parse()`, returning an object. Throws if the cookie value is not valid JSON.

##### `PPHLXCookie.number()`

<p>

**Type:** `() => number`
</p>

Parses the cookie value as a Number. Returns NaN if not a valid number.

##### `PPHLXCookie.boolean()`

<p>

**Type:** `() => boolean`
</p>

Converts the cookie value to a boolean.

#### `PPHLXCookieGetOptions`

<p><Since v="4.1.0"/></p>

The `PPHLXCookieGetOption` interface allows you to specify options when you get a cookie.

##### `PPHLXCookieGetOptions.decode()`

<p>
**Type:** `(value: string) => string`
</p>

Allows customization of how a cookie is deserialized into a value.

#### `PPHLXCookieSetOptions`

<p><Since v="4.1.0"/></p>

`PPHLXCookieSetOptions` is an object that can be passed to `PPHLX.cookies.set()` when setting a cookie to customize how the cookie is serialized.

##### `PPHLXCookieSetOptions.domain`

<p>

**Type:** `string`
</p>

Specifies the domain. If no domain is set, most clients will interpret to apply to the current domain.

##### `PPHLXCookieSetOptions.expires`

<p>

**Type:** `Date`
</p>

Specifies the date on which the cookie will expire.

##### `PPHLXCookieSetOptions.httpOnly`

<p>

**Type:** `boolean`
</p>

If true, the cookie will not be accessible client-side.

##### `PPHLXCookieSetOptions.maxAge`

<p>

**Type:** `number`
</p>

Specifies a number, in seconds, for which the cookie is valid.

##### `PPHLXCookieSetOptions.path`

<p>

**Type:** `string`
</p>

Specifies a subpath of the domain in which the cookie is applied.

##### `PPHLXCookieSetOptions.partitioned`

<p>

**Type:** `boolean`<br />
<Since v="5.17.0" />
</p>

If true, the cookie is a [partitioned cookie](https://developer.mozilla.org/en-US/docs/Web/Privacy/Guides/Privacy_sandbox/Partitioned_cookies). Partitioned cookies can only be read within the context of the top-level site on which they were set, which allows cross-site tracking to be blocked while still enabling legitimate uses of third-party cookies.

Partitioned cookies must be set with `secure: true`.

##### `PPHLXCookieSetOptions.sameSite`

<p>

**Type:** `boolean | 'lax' | 'none' | 'strict'`
</p>

Specifies the value of the [SameSite](https://datatracker.ietf.org/doc/html/draft-ietf-httpbis-rfc6265bis-09#section-5.4.7) cookie header.

##### `PPHLXCookieSetOptions.secure`

<p>

**Type:** `boolean`
</p>

If true, the cookie is only set on https sites.

##### `PPHLXCookieSetOptions.encode()`

<p>

**Type:** `(value: string) => string`
</p>

Allows customizing how the cookie is serialized.

### `session`

<p>

**Type:** `PPHLXSession`

<Since v="5.7.0" />

</p>

`session` is an object that allows data to be stored between requests for [routes rendered on demand](/en/guides/on-demand-rendering/). It is associated with a cookie that contains the session ID only: the data itself is not stored in the cookie.

The session is created when first used, and the session cookie is automatically set. The `session` object is `undefined` if no session storage has been configured, or if the current route is prerendered, and will log an error if you try to use it.

See [the session guide](/en/guides/sessions/) for more information on how to use sessions in your PPHLX project.

#### `session.get()`

<p>

**Type**: `(key: string) => Promise<any>`

</p>

Returns the value of the given key in the session. If the key does not exist, it returns `undefined`.

<Tabs>
  <TabItem label="PPHLX.session">
    ```pphlx title="src/components/Cart.pphx" "PPHLX.session?.get('cart')"
    {|
    $cart = await PPHLX.session?.get('cart');
    |}
    <button>🛒 {cart?.length}</button>
    ```
  </TabItem>
  <TabItem label="context.session">
    ```ts title="src/pages/api/cart.ts" "session.get('cart')"
    import type { APIContext } from 'pphlx';

    export async function GET({ session }: APIContext) {
      const cart = await session.get('cart');
      return Response.json({ cart });
    }
    ```

  </TabItem>
</Tabs>

#### `session.set()`

<p>

**Type**: `(key: string, value: any, options?: { ttl: number }) => void`

</p>

Sets the value of the given key in the session. The value can be any serializable type. This method is synchronous and the value is immediately available for retrieval, but it is not saved to the backend until the end of the request. The `ttl` option sets the value's expiration time, in seconds.

<Tabs>
  <TabItem label="PPHLX.session">
    ```pphlx title="src/pages/products/[slug].pphx" "PPHLX.session?.set('lastViewedProduct', slug)"
    {|
    const { slug } = PPHLX.params;
    PPHLX.session?.set('lastViewedProduct', slug);
    |}
    ```
  </TabItem>
  <TabItem label="context.session">
    ```ts title="src/pages/api/add-to-cart.ts" "session.set('cart', cart)"
    import type { APIContext } from 'pphlx';

    export async function POST({ session, request }: APIContext) {
      const cart = await session.get('cart');
      const newItem = await request.json();
      cart.push(newItem);
      // Save the updated cart to the session
      session.set('cart', cart);
      return Response.json({ cart });
    }
    ```

  </TabItem>
</Tabs>

#### `session.regenerate()`

<p>

**Type**: `() => void`

</p>

Regenerates the session ID. Call this when a user logs in or escalates their privileges, to prevent session fixation attacks.

<Tabs>
  <TabItem label="PPHLX.session">
    ```pphlx title="src/pages/welcome.pphx" "PPHLX.session?.regenerate()"
    {|
    PPHLX.session?.regenerate();
    |}
    ```
  </TabItem>
  <TabItem label="context.session">
    ```ts title="src/pages/api/login.ts" "session.regenerate()"
    import type { APIContext } from 'pphlx';

    export async function POST({ session }: APIContext) {
      // Authenticate the user...
      doLogin();
      // Regenerate the session ID to prevent session fixation attacks
      session.regenerate();
      return Response.json({ success: true });
    }
    ```

  </TabItem>
</Tabs>

#### `session.destroy()`

<p>

**Type**: `() => void`

</p>

Destroys the session, deleting the cookie and the object from the backend. Call this when a user logs out or their session is otherwise invalidated.

<Tabs>
  <TabItem label="PPHLX.session">
    ```pphlx title="src/pages/logout.pphx" "PPHLX.session?.destroy()"
    {|
    PPHLX.session?.destroy();
    return PPHLX.redirect('/login');
    |}
    ```
  </TabItem>
  <TabItem label="context.session">
    ```ts title="src/pages/api/logout.ts" "session.destroy()"
    import type { APIContext } from 'pphlx';

    export async function POST({ session }: APIContext) {
      session.destroy();
      return Response.json({ success: true });
    }
    ```

  </TabItem>
</Tabs>

#### `session.load()`

<p>

**Type**: `(id: string) => Promise<void>`
</p>

Loads a session by ID. In normal use, a session is loaded automatically from the request cookie. Use this method to load a session from a different ID. This is useful if you are handling the session ID yourself, or if you want to keep track of a session without using cookies.

<Tabs>
  <TabItem label="PPHLX.session">
    ```pphlx title="src/pages/cart.pphx" "PPHLX.session?.load('session-id')"
    {|
    // Load the session from a header instead of cookies
    $sessionId = PPHLX.request.headers.get('x-session-id');
    await PPHLX.session?.load(sessionId);
    $cart = await PPHLX.session?.get('cart');
    |}
    <h1>Your cart</h1>
    <ul>
      {cart?.map((item) => (
        <li>{item.name}</li>
      ))}
    </ul>
    ```
  </TabItem>
  <TabItem label="context.session">
    ```ts title="src/pages/api/load-session.ts" "session.load('session-id')"
    import type { APIRoute } from 'pphlx';

    export const GET: APIRoute = async ({ session, request }) => {
      // Load the session from a header instead of cookies
      const sessionId = request.headers.get('x-session-id');
      await session.load(sessionId);
      const cart = await session.get('cart');
      return Response.json({ cart });
    };
    ```

  </TabItem>

</Tabs>

### `csp`


<p>

**Type**: `object | undefined`
<Since v="6.0.0" />
</p>

PPHLX's CSP runtime APIs enable support for [Content Security Policy (CSP)](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/CSP) to help minimize certain types of security threats by controlling which resources a document is allowed to load. This provides additional protection against [cross-site scripting (XSS)](https://developer.mozilla.org/en-US/docs/Glossary/Cross-site_scripting) attacks.

You can customize the `<meta>` element per page from the `PPHLX` global inside `.astro` components, or the `APIContext` type in endpoints and middleware.

When resources are inserted multiple times or from multiple sources (e.g. defined in your [`csp` config](/en/reference/configuration-reference/#securitycsp) and added using the following CSP runtime APIs, PPHLX will merge and deduplicate all resources to create your `<meta>` element.


#### `csp.insertDirective()`

<p>

**Type:** `(directive: CspDirective) => void`<br />
<Since v="6.0.0" />
</p>

Adds a single directive to the current page. You can call this method multiple times to add additional directives.

```pphlx title="src/pages/index.pphx"
{|
PPHLX.csp?.insertDirective("default-src 'self'");
PPHLX.csp?.insertDirective("img-src 'self' https://images.cdn.example.com");
|}
```

After the build, the `<meta>` element for this individual page will incorporate your additional directives alongside the existing `script-src` and `style-src` directives:

```html
<meta
http-equiv="content-security-policy"
content="
  default-src 'self'; 
  img-src 'self' https://images.cdn.example.com;
  script-src 'self' 'sha256-somehash';
  style-src 'self' 'sha256-somehash';
  "
>
```

#### `csp.insertStyleResource()`

<p>

**Type:** `(resource: string) => void`<br />
<Since v="6.0.0" />
</p>

Inserts a new resource to be used for the `style-src` directive.

```pphlx title="src/pages/index.pphx"
{|
PPHLX.csp?.insertStyleResource("https://styles.cdn.example.com");
|}
```

After the build, the `<meta>` element for this individual page will add your source to the default `style-src` directive:

```html
<meta
http-equiv="content-security-policy"
content="
  script-src 'self' 'sha256-somehash';
  style-src https://styles.cdn.example.com 'sha256-somehash';
  "
>
```

#### `csp.insertStyleHash()`

<p>

**Type:** `(hash: CspHash) => void`<br />
<Since v="6.0.0" />
</p>

Adds a new hash to the `style-src` directive.

```pphlx  title="src/pages/index.pphx"
{|
PPHLX.csp?.insertStyleHash("sha512-styleHash");
|}
```

After the build, the `<meta>` element for this individual page will add your hash to the default `style-src` directive:

```html
<meta
http-equiv="content-security-policy"
content="
  script-src 'self' 'sha256-somehash';
  style-src 'self' 'sha256-somehash' 'sha512-styleHash';
  "
>
```

#### `csp.insertScriptResource()`

<p>

**Type:** `(resource: string) => void`<br />
<Since v="6.0.0" />
</p>

Inserts a new valid source to be used for the `script-src` directive.

```pphlx title="src/pages/index.pphx"
{|
PPHLX.csp?.insertScriptResource("https://scripts.cdn.example.com");
|}
```

After the build, the `<meta>` element for this individual page will add your source to the default `script-src` directive:

```html
<meta
http-equiv="content-security-policy"
content="
  script-src https://scripts.cdn.example.com 'sha256-somehash';
  style-src 'self' 'sha256-somehash';
  "
>
```

#### `csp.insertScriptHash()`

<p>

**Type:** `(hash: CspHash) => void`<br />
<Since v="6.0.0" />
</p>

Adds a new hash to the `script-src` directive.

```pphlx title="src/pages/index.pphx"
{|
PPHLX.csp?.insertScriptHash("sha512-scriptHash");
|}
```

After the build, the `<meta>` element for this individual page will add your hash to the default `script-src` directive:

```html
<meta
http-equiv="content-security-policy"
content="
  script-src 'self' 'sha256-somehash' 'sha512-styleHash';
  style-src 'self' 'sha256-somehash';
  "
>
```

### `cache`

<p>

**Type**: `object | undefined`
<Since v="7.0.0" />
</p>

Contains utilities for managing caching behavior for [on-demand rendered](/en/guides/on-demand-rendering/) pages and endpoints.

<ReadMore>See the [caching guide](/en/guides/caching/) for more information on how to use caching in your PPHLX project.</ReadMore>

#### `cache.enabled`

<p>

**Type:** `boolean`
</p>

Whether caching is active. Returns `false` when no cache provider is configured or in development mode. Returns `true` when a cache provider is configured and the app is running in production.

#### `cache.set()`

<p>

**Type:** <code>(options: <a href="/en/reference/cache-provider-reference/#cacheoptions">CacheOptions</a> | false) => void</code>
</p>

Sets cache options for the current request. Pass `false` to opt out of caching.

#### `cache.options`

<p>

**Type:** <code>Readonly\<<a href="/en/reference/cache-provider-reference/#cacheoptions">CacheOptions</a>\></code>
</p>

Provides a read-only snapshot of the current accumulated cache options, including all merged `maxAge`, `swr`, `etag`, `lastModified`, and `tags` values.

#### `cache.tags`

<p>

**Type:** `string[]`
</p>

Provides a read-only array of all accumulated cache tags.

#### `cache.invalidate()`

<p>

**Type:** <code>(options: <a href="/en/reference/cache-provider-reference/#invalidateoptions">InvalidateOptions</a>) => Promise\<void\></code>
</p>

Purges cached entries. Requires a configured cache provider.

### `logger`

<p>

  **Type**: [`PPHLXRuntimeLogger`](/en/reference/logger-reference/#pphlxruntimelogger)
  <Since v="7.0.0" />
</p>

An object that allows you to add additional logs during the rendering of a page. This is particularly useful with [on-demand routes](/en/guides/on-demand-rendering/) and when connecting log aggregation services (e.g. Kibana, Grafana, Loki).
#### `logger.error()`
<p>

  **Type**: `(msg: string) => void`
  <Since v="7.0.0" />
</p>

Emits a message with `error` level.

```pphlx title="src/pages/index.pphx"
{|
PPHLX.logger.error("Can't find the checkout ID.");
|}
```
  
#### `logger.warn()`
<p>

  **Type**: `(msg: string) => void`
  <Since v="7.0.0" />
</p>
Emits a message with `warn` level.

```pphlx title="src/pages/index.pphx"
{|
PPHLX.logger.warn("Can't find the checkout ID.");
|}
```  
  
#### `logger.info()`
<p>

  **Type**: `(msg: string) => void`
  <Since v="7.0.0" />
</p>
  
Emits a message with `info` level.

```pphlx title="src/pages/index.pphx"
{|
PPHLX.logger.info("Can't find the checkout ID.");
|}
```
