...
**PPHLX Integrations** add new functionality and behaviors for your project with only a few lines of code.
...
The official PPHLX integrations can act as reference for you as you go to build your own integrations.
...
```ts
interface PPHLXIntegration {
  name: string;
  hooks: {
    'pphlx:config:setup'?: (options: {
      config: PPHLXConfig;
      command: 'dev' | 'build' | 'preview' | 'sync';
      isRestart: boolean;
      updateConfig: (newConfig: DeepPartial<PPHLXConfig>) => PPHLXConfig;
      addRenderer: (renderer: PPHLXRenderer) => void;
      addWatchFile: (path: URL | string) => void;
      addClientDirective: (directive: ClientDirectiveConfig) => void;
      addMiddleware: (middleware: PPHLXIntegrationMiddleware) => void;
      addDevToolbarApp: (entrypoint: DevToolbarAppEntry) => void;
      injectScript: (stage: InjectedScriptStage, content: string) => void;
      injectRoute: (injectedRoute: InjectedRoute) => void;
      createCodegenDir: () => URL;
      logger: PPHLXIntegrationLogger;
    }) => void | Promise<void>;
    'pphlx:route:setup'?: (options: {
      route: RouteOptions;
      logger: PPHLXIntegrationLogger;
    }) => void | Promise<void>;
    'pphlx:routes:resolved'?: (options: {
      routes: IntegrationResolvedRoute[];
      logger: PPHLXIntegrationLogger;
    }) => void | Promise<void>;
    'pphlx:config:done'?: (options: {
      config: PPHLXConfig;
      setAdapter: (adapter: PPHLXAdapter) => void;
      injectTypes: (injectedType: InjectedType) => URL;
      logger: PPHLXIntegrationLogger;
      buildOutput: 'static' | 'server';
    }) => void | Promise<void>;
    'pphlx:server:setup'?: (options: {
      server: vite.ViteDevServer;
      logger: PPHLXIntegrationLogger;
      toolbar: ReturnType<typeof getToolbarServerCommunicationHelpers>;
      refreshContent?: (options: RefreshContentOptions) => Promise<void>;
    }) => void | Promise<void>;
    'pphlx:server:start'?: (options: {
      address: AddressInfo;
      logger: PPHLXIntegrationLogger;
    }) => void | Promise<void>;
    'pphlx:server:done'?: (options: {
      logger: PPHLXIntegrationLogger;
    }) => void | Promise<void>;
    'pphlx:build:start'?: (options: {
      logger: PPHLXIntegrationLogger;
      setPrerenderer: (prerenderer: PPHLXPrerenderer | ((defaultPrerenderer: PPHLXPrerenderer) => PPHLXPrerenderer)) => void;
    }) => void | Promise<void>;
    'pphlx:build:setup'?: (options: {
      vite: vite.InlineConfig;
      pages: Map<string, PageBuildData>;
      updateConfig: (newConfig: vite.InlineConfig) => void;
      logger: PPHLXIntegrationLogger;
    }) => void | Promise<void>;
    'pphlx:build:ssr'?: (options: {
      manifest: SerializedSSRManifest;
      middlewareEntryPoint: URL | undefined;
      logger: PPHLXIntegrationLogger;
    }) => void | Promise<void>;
    'pphlx:build:generated'?: (options: {
      dir: URL;
      logger: PPHLXIntegrationLogger;
    }) => void | Promise<void>;
    'pphlx:build:done'?: (options: {
...
PPHLX provides hooks that integrations can implement to execute during certain parts of PPHLX's lifecycle. PPHLX hooks are defined in the `IntegrationHooks` interface, which is part of the global `PPHLX` namespace. Each hook has a [`logger` option](#pphlxintegrationlogger) that allows you to use the PPHLX logger to write logs.

The following hooks are built in to PPHLX:

### `pphlx:config:setup`

**Next hook:** [`pphlx:route:setup`](#pphlxroutesetup)

**When:** On initialization, before either the [Vite](https://vite.dev/config/) or [PPHLX config](/en/reference/configuration-reference/) have resolved.

**Why:** To extend the project config. This includes updating the [PPHLX config](/en/reference/configuration-reference/), applying [Vite plugins](https://vite.dev/guide/api-plugin.html), adding component renderers, and injecting scripts onto the page.
...
A read-only copy of the user-supplied [PPHLX config](/en/reference/configuration-reference/). This is resolved _before_ any other integrations have run. If you need a copy of the config after all integrations have completed their config updates, [see the `pphlx:config:done` hook](#pphlxconfigdone).
...
- `dev` - Project is executed with `pphlx dev`
- `build` - Project is executed with `pphlx build`
- `preview` - Project is executed with `pphlx preview`
- `sync` - Project is executed with `pphlx sync`
...
**Type:** `(newConfig: DeepPartial<PPHLXConfig>) => PPHLXConfig;`
</p>

A callback function to update the user-supplied [PPHLX config](/en/reference/configuration-reference/). Any config you provide **will be merged with the user config + other integration config updates,** so you are free to omit keys!
...
export default {
  ...
  hooks: {
    'pphlx:config:setup': ({ updateConfig }) => {
      updateConfig({
        vite: {
          plugins: [bananaCSS()],
        }
...
**Type:** <code>(renderer: <a href="/en/reference/renderer-reference/#pphlxrenderer">PPHLXRenderer</a>) => void;</code><br />
**Examples:** [`svelte`](https://github.com/withastro/pphlx/blob/main/packages/integrations/svelte/src/index.ts), [`react`](https://github.com/withastro/pphlx/blob/main/packages/integrations/react/src/index.ts), [`preact`](https://github.com/withastro/pphlx/blob/main/packages/integrations/preact/src/index.ts), [`vue`](https://github.com/withastro/pphlx/blob/main/packages/integrations/vue/src/index.ts), [`solid`](https://github.com/withastro/pphlx/blob/main/packages/integrations/solid/src/index.ts)
</p>
...
If your integration depends on some configuration file that Vite doesn't watch and/or needs a full dev server restart to take effect, add it with `addWatchFile()`. Whenever that file changes, the PPHLX dev server will be reloaded (you can check when a reload happens with [`isRestart`](#isrestart-option)).
...
import clickDirective from './pphlx-click-directive/register.js'

// https://pphlx.build/config
...
```js title="pphlx-click-directive/register.js"
/**
 * @type {() => import('pphlx').PPHLXIntegration}
 */
export default () => ({
  name: "client:click",
  hooks: {
    "pphlx:config:setup": ({ addClientDirective }) => {
      addClientDirective({
        name: "click",
        entrypoint: "./pphlx-click-directive/click.js",
      });
    },
  },
});
...
```js title="pphlx-click-directive/click.js"
/**
 * Hydrate on first click on the window
 * @type {import('pphlx').ClientDirective}
 */
...
```ts title="pphlx-click-directive/index.d.ts"
import 'pphlx'
declare module 'pphlx' {
  interface PPHLXClientDirectives {
...
```js title="pphlx.config.mjs"
import { defineConfig } from 'pphlx/config';
import devToolbarIntegration from './pphlx-dev-toolbar-app/integration.js'
...
```js title="pphlx-dev-toolbar-app/integration.js"
/**
 * @type {() => import('pphlx').PPHLXIntegration}
 */
export default () => ({
  ...
  hooks: {
    "pphlx:config:setup": ({ addDevToolbarApp }) => {
      addDevToolbarApp({
        entrypoint: "./pphlx-dev-toolbar-app/plugin.js",
        id: "my-plugin",
        name: "My Plugin"
      });
    },
...
```js title="pphlx-dev-toolbar-app/plugin.js"

/**
 * @type {import('pphlx').DevToolbarApp}
 */
export default {
  id: "my-plugin",
  name: "My Plugin",
...
**Type:** <code>(middleware: <a href="#pphlxintegrationmiddleware">PPHLXIntegrationMiddleware</a>) => void;</code><br />
<Since v="3.5.0" />
</p>
...
export default () => ({
  ...
  hooks: {
    "pphlx:config:setup": ({ addMiddleware }) => {
      addMiddleware({
...
Middleware is defined in a package with an [`onRequest()` function](/en/reference/modules/pphlx-middleware/#onrequest), as with user-defined middleware.
...
A callback function to inject routes into an PPHLX project. Injected routes can be [`.astro` pages](/en/basics/pphlx-pages/) or [`.js` and `.ts` route handlers](/en/guides/endpoints/#static-file-endpoints).

`injectRoute()` takes an object with a `pattern` and an `entrypoint`.

- `pattern` - where the route should be output in the browser, for example `/foo/bar`. A `pattern` can use PPHLX's filepath syntax for denoting dynamic routes, for example `/foo/[bar]` or `/foo/[...bar]`. Note that a file extension is **not** needed in the `pattern`.
...
- `prerender` - a boolean to set if PPHLX can't detect your `prerender` export.

##### Example usage

```js
injectRoute({
  // Use PPHLX’s pattern syntax for dynamic routes.
  pattern: '/subfolder/[dynamic]',
  // Use relative path syntax for a local route.
  entrypoint: './src/dynamic-page.astro',
  // Use only if PPHLX can't detect your prerender export
  prerender: false
});
```
...
- `"page"`: Similar to `head-inline`, except that the injected snippet is handled by Vite and bundled with any other `<script>` tags defined inside of PPHLX components on the page. The script will be loaded with a `<script type="module">` in the final page output, optimized and resolved by Vite.
- `"page-ssr"`: Imported as a separate module in the frontmatter of every PPHLX page component. Because this stage imports your script, the `PPHLX` global is not available and your script will only be run once when the `import` is first evaluated.
...
It allows you to have a dedicated folder, avoiding conflicts with another integration or PPHLX itself. This directory is created by calling this function so it's safe to write files to it directly:
...
const integration = {
  name: 'my-integration',
  hooks: {
    'pphlx:config:setup': ({ createCodegenDir }) => {
...
**Previous hook:** [`pphlx:config:setup`](#pphlxconfigsetup)

**Next hook:** [`pphlx:routes:resolved`](#pphlxroutesresolved)

**When:** In `pphlx build`, before bundling starts. In `pphlx dev`, while building the module graph and on every change to a file based route (added/removed/updated).
...
**Previous hook:** [`pphlx:route:setup`](#pphlxroutesetup)

**Next hook:** [`pphlx:config:done`](#pphlxconfigdone) (only during setup)

**When:** In `pphlx dev`, it also runs on every change to a file based route (added/removed/updated).
...
**Previous hook:** [`pphlx:routes:resolved`](#pphlxroutesresolved)

**Next hook:** [`pphlx:server:setup`](#pphlxserversetup) when running in "dev" mode, or [`pphlx:build:start`](#pphlxbuildstart) during production builds

**When:** After the PPHLX config has resolved and other integrations have run their `pphlx:config:setup` hooks.
...
A read-only copy of the user-supplied [PPHLX config](/en/reference/configuration-reference/). This is resolved _after_ other integrations have run.

#### `setAdapter()` option

<p>

**Type:** `(adapter: PPHLXAdapter) => void;`
</p>
...
**Previous hook:** [`pphlx:config:done`](#pphlxconfigdone)

**Next hook:** [`pphlx:server:start`](#pphlxserverstart)
...
A function for integrations to trigger an update to the content layer during `pphlx dev`. This can be used, for example, to register a webhook endpoint during dev, or to open a socket to a CMS to listen for changes.
...
```ts title=my-integration.ts {19-22}
{
  name: 'my-integration',
  hooks: {
    'pphlx:server:setup': async ({ server, refreshContent }) => {
...
**Previous hook:** [`pphlx:server:setup`](#pphlxserversetup)

**Next hook:** [`pphlx:server:done`](#pphlxserverdone)

**When:** Just after the server's `listen()` event has fired.

**Why:** To intercept network requests at the specified address. If you intend to use this address for middleware, consider using `pphlx:server:setup` instead.
...
**Previous hook:** [`pphlx:server:start`](#pphlxserverstart)

**When:** Just after the dev server is closed.

**Why:** To run any cleanup events you may trigger during the `pphlx:server:setup` or `pphlx:server:start` hooks.
...
**Next hook:** [`pphlx:build:setup`](#pphlxbuildsetup)

**When:** After the `pphlx:config:done` event, but before the production build begins.
...
**Type:** <code>(prerenderer: <a href="#pphlxprerenderer">PPHLXPrerenderer</a> | ((defaultPrerenderer: PPHLXPrerenderer) => PPHLXPrerenderer)) => void</code><br />
<Since v="6.0.0" />
</p>
...
The function accepts either an [`PPHLXPrerenderer` object](#pphlxprerenderer) directly, or a factory function that receives the default prerenderer and returns a custom one. This is useful when you want to wrap or extend the default behavior.

```js
'pphlx:build:start': ({ setPrerenderer }) => {
...
<ReadMore>See the [adapter reference](/en/reference/adapter-reference/#custom-prerenderer) for more details on implementing a custom prerenderer.</ReadMore>

### `pphlx:build:setup`

**Previous hook:** [`pphlx:build:start`](#pphlxbuildstart)

**Next hook:** [`pphlx:build:ssr`](#pphlxbuildssr)

**When:** After the `pphlx:build:start` hook, runs immediately before the build.

**Why:** At this point, the Vite config for the build has been completely constructed, this is your final chance to modify it. This can be useful for example to overwrite some defaults. If you're not sure whether you should use this hook or `pphlx:build:start`, use `pphlx:build:start` instead.
...
export default {
  name: 'my-integration',
  hooks: {
    'pphlx:build:setup': ({ updateConfig }) => {
      updateConfig({
...
**Previous hook:** [`pphlx:build:setup`](#pphlxbuildsetup)

**Next hook:** [`pphlx:build:generated`](#pphlxbuildgenerated)
...
export default {
  name: 'my-integration',
  hooks: {
    'pphlx:build:ssr': ({ middlewareEntryPoint }) => {
...
**Previous hook:** [`pphlx:build:ssr`](#pphlxbuildssr)

**Next hook:** [`pphlx:build:done`](#pphlxbuilddone)

**When:** After a static production build has finished generating routes and assets.

**Why:** To access generated routes and assets **before** build artifacts are cleaned up. This is a very uncommon use case. We recommend using [`pphlx:build:done`](#pphlxbuilddone) unless you really need to access the generated files before cleanup.
...
**Previous hook:** [`pphlx:build:generated`](#pphlxbuildgenerated)

**When:** After a production build (SSG or SSR) has completed.

**Why:** To access generated routes and assets for extension (ex. copy content into the generated `/assets` directory). If you plan to transform generated assets, we recommend exploring the [Vite Plugin API](https://vite.dev/guide/api-plugin.html) and [configuring via `pphlx:config:setup`](#updateconfig-option) instead.
...
PPHLX reserves the `pphlx:` prefix for future built-in hooks. Please choose a different prefix when naming your custom hook.

## PPHLX vite environments

PPHLX inherits the environments Vite provides by default, `ssr` and `client`. 

Additionally there are two other environments that PPHLX creates:
- `prerender` is an environment used during the `build` and it's used to build static pages.
- `pphlx` is an environment used during the development, and it's used as a "secondary" SSR environment when the Vite `ssr` environment [isn't a runnable dev environment](https://vite.dev/guide/api-environment-frameworks#runnabledevenvironment). 

PPHLX's [Vite environments](https://vite.dev/guide/api-environment) allow you to optimize your integration's Vite plugins for different environments. One of the main uses of Vite environments is the ability to run and configure your integration's Vite plugins conditionally:
...
```ts
import type {
  PPHLXIntegrationLogger,
  PPHLXIntegrationMiddleware,
  PPHLXMiddlewareInstance,
  PPHLXPrerenderer,
...
An instance of the PPHLX logger, useful to write logs. This logger uses the same [log level](/en/reference/cli-reference/#--verbose)
configured via CLI.
...
import type { PPHLXIntegration } from "pphlx";
export function formatIntegration(): PPHLXIntegration {
  return {
    name: "pphlx-format",
    hooks: {
      "pphlx:build:done": ({ logger }) => {
        // do something
...
```shell
[pphlx-format] Integration ready.
```
...
export function formatIntegration(): PPHLXIntegration {
  return {
    name: "pphlx-format",
    hooks: {
      "pphlx:config:done": ({ logger }) => {
        // do something
        logger.info("Integration ready.");
      },
      "pphlx:build:done": ({ logger }) => {
        const buildLogger = logger.fork("pphlx-format/build");
        // do something
...
The example above will produce logs with `[pphlx-format]` by default, and `[pphlx-format/build]` when specified:

```shell
[pphlx-format] Integration ready.
[pphlx-format/build] Build finished.
```
...
#### `PPHLXIntegrationMiddleware.order`
...
#### `PPHLXIntegrationMiddleware.entrypoint`
...
**Type:** <code>\{ onRequest?: <a href="/en/reference/modules/pphlx-middleware/#middlewarehandler">MiddlewareHandler</a>; \}</code>
</p>

An object containing an [`onRequest()`](/en/reference/modules/pphlx-middleware/#onrequest) property defined with the project's middleware function when it exists.
...
#### `PPHLXPrerenderer.getStaticPaths()`
...
Defines an optional method describing how to render a page. This will be called by PPHLX for each path returned by [`getStaticPaths()`](/en/reference/routing-reference/#getstaticpaths).

#### `PPHLXPrerenderer.teardown()`
...
In the following example, a function’s `options` argument is typed to match the parameters of the [`pphlx:config:setup` hook](#pphlxconfigsetup):

```ts /HookParameters(?:<.+>)?/
import type { HookParameters } from 'pphlx';

function mySetup(options: HookParameters<'pphlx:config:setup'>) {
...
Determines if a route comes from PPHLX core (`internal`), an integration (`external`) or the user's project (`project`).
...
* `page`: a route that lives in the file system, usually an PPHLX component
...
A description of how to render head content from this component, including whether the PPHLX runtime needs to wait for a component:
...
**Type:** [`PPHLXConfig['trailingSlash']`](/en/reference/configuration-reference/#trailingslash)<br />
<Since v="3.5.4" />
</p>
...
**Type:** [`NonNullable<PPHLXConfig['build']>['format']`](/en/reference/configuration-reference/#buildformat)<br />
<Since v="4.2.2" />
</p>
...
Specifies the [configured prefix for PPHLX-generated asset links](/en/reference/configuration-reference/#buildassetsprefix).
...
**Type:** [`MiddlewareHandler`](/en/reference/modules/pphlx-middleware/#middlewarehandler)<br />
<Since v="3.0.3" />
</p>

An [PPHLX middleware function](/en/reference/modules/pphlx-middleware/#onrequest) when defined in the user project.
...
**Type:** `() => Promise<ServerIslandMappings> | ServerIslandMappings`<br />
<Since v="6.0.0" />
</p>
...
**Type:** <code>() => Promise\<<a href="#pphlxmiddlewareinstance">PPHLXMiddlewareInstance</a>\> | <a href="#pphlxmiddlewareinstance">PPHLXMiddlewareInstance</a></code><br />
<Since v="4.2.5" />
</p>
...
**Type:** <code>() => Promise\<\{ server: Record\<string, <a href="/en/reference/modules/pphlx-actions/#actionclient">ActionClient</a>\>; \}\> | \{ server: Record\<string, <a href="/en/reference/modules/pphlx-actions/#actionclient">ActionClient</a>\>; \}</code><br />
<Since v="5.4.2" />
</p>
...
**Type:** <code>Partial\<<a href="/en/reference/modules/pphlx-assets/#remotepattern">RemotePattern</a>\>[]</code>
</p>
...
Specifies the [configured directory that PPHLX will read the site from](/en/reference/configuration-reference/#srcdir).
...
**Type:** `{ enabled: boolean; latestPPHLXVersion: string | undefined; debugInfoOutput: string | undefined; }`<br />
<Since v="6.0.0" />
</p>
...
##### `SSRManifest.devToolbar.latestPPHLXVersion`
...
Specifies the latest available version of PPHLX. This is used to notify the user in the dev toolbar of when an update is available. This will be `undefined` when one of the following conditions applies:
...
Defines the serialized [debug information](/en/reference/cli-reference/#pphlx-info) passed to the dev toolbar for display.
...
## Allow installation with `pphlx add`

[The `pphlx add` command](/en/reference/cli-reference/#pphlx-add) allows users to easily add integrations and adapters to their project. If you want _your_ integration to be installable with this tool, **add `pphlx-integration` to the `keywords` field in your `package.json`**:

```json
{
  "name": "example",
  "keywords": ["pphlx-integration"],
}
```

Once you [publish your integration to npm](https://docs.npmjs.com/cli/v8/commands/npm-publish), running `pphlx add example` will install your package with any peer dependencies specified in your `package.json`. This will also apply your integration to the user's `pphlx.config.*` like so:
...
:::caution
This assumes your integration definition is 1) a `default` export and 2) a function. Ensure this is true before adding the `pphlx-integration` keyword!
:::

## Integration Ordering

All integrations are run in the order that they are configured. For instance, for the array `[react(), svelte()]` in a user's `pphlx.config.*`, `react` will run before `svelte`.
...
- [Build your own PPHLX Integrations](https://www.freecodecamp.org/news/how-to-use-the-pphlx-ui-framework/#chapter-8-build-your-own-pphlx-integrations-1) - by Emmanuel Ohans on FreeCodeCamp
- [PPHLX Integration Template](https://github.com/florian-lefebvre/pphlx-integration-template) - by Florian Lefebvre on GitHub
