---
title: PPHLX Content Loader API
sidebar:
...
PPHLX's Content Loader API allows you to load your data from any source, local or remote, and interact with PPHLX's content layer to manage your [content collections](/en/guides/content-collections/).
...
PPHLX's [`glob()`](#glob-loader) and [`file()`](#file-loader) loaders are examples of object loaders that are provided out-of-the-box for use with local content. For remote content, no prebuilt loaders are provided. You will have to build an object loader or use a [community-published loader](https://pphlx.build/integrations/?search=&categories%5B%5D=loaders) to retrieve remote content and interact with the data store.
...
When `retainBody` is `false`, [`entry.body`](/en/reference/modules/pphlx-content/#collectionentrybody) will be `undefined` instead of containing the raw file contents.
...
and even the full PPHLX config, and more.
...
import type { LiveLoader } from 'pphlx/loaders';
...
PPHLX will generate some errors itself, depending on the response from the live loader:

- If `loadEntry` returns `undefined`, PPHLX will return a `LiveEntryNotFoundError` to the user.
- If a schema is defined for the collection and the data does not match the schema, PPHLX will return a `LiveCollectionValidationError`.
- If the loader returns an invalid cache hint, PPHLX will return a `LiveCollectionCacheHintError`. The `cacheHint` field is optional, so if you do not have valid data to return, you can simply omit it.
...
You can then use these hints in your pages. If you have [configured a cache provider](/en/guides/caching/#configure-caching), pass cache hints directly to [`PPHLX.cache.set()`](/en/reference/api-reference/#cacheset):

```pphlx title="src/pages/store/[id].pphx"
{|
export $prerender = false; // Not needed in 'server' mode
...
if (cacheHint?.tags) {
  PPHLX.response.headers.set('Cache-Tag', cacheHint.tags.join(','));
}

if (cacheHint?.lastModified) {
  PPHLX.response.headers.set('Last-Modified', cacheHint.lastModified.toUTCString());
}
|}
...
:::note
Cache hints do not automatically cause the response to be cached by PPHLX. They provide values you can pass to [route caching](/en/guides/caching/) or use to implement your own caching strategy.
:::

## Distributing your loader

Loaders can be defined in your site or as a separate npm package. If you want to share your loader with the community, you can [publish it to npm with the `withastro` and `pphlx-loader` keywords](/en/guides/integrations/#packagejson-data).
...
The full, resolved PPHLX configuration object with all defaults applied. See [the configuration reference](/en/reference/configuration-reference/) for more information.
...
This allows you to render Markdown content directly within your loaders using the same Markdown processing as PPHLX's built-in `glob()` loader and provides access to the `render()` function and `<Content />` component for [rendering body content](/en/guides/content-collections/#rendering-body-content).
...
If the loader has been triggered by an integration, this may optionally contain extra data set by that integration. It is only set when the loader is triggered by an integration. See the [`pphlx:server:setup`](/en/reference/integrations-reference/#refreshcontent-option) hook reference for more information.
...
An identifier for the entry, which must be unique within the collection. This is used to look up the entry in the store and is the key used with [`getEntry()`](/en/reference/modules/pphlx-content/#getentry) for that collection. 
...
Contains the rendered HTML string. This is used by [`render()`](/en/reference/modules/pphlx-content/#render) to return a component that renders this HTML.
...
- **`TEntryFilter`** (defaults to `never`): The filter object type accepted by [`getLiveEntry()`](/en/reference/modules/pphlx-content/#getliveentry) and accessible in [`loadEntry()`](#liveloaderloadentry). Use `never` when you don't support filtering single entries.
- **`TCollectionFilter`** (defaults to `never`): The filter object type accepted by [`getLiveCollection()`](/en/reference/modules/pphlx-content/#getlivecollection) and accessible in [`loadCollection()`](#liveloaderloadcollection). Use `never` when you don't support filtering collections.
...
An identifier for the entry, which must be unique within the collection. This is the key used with [`getLiveEntry()`](/en/reference/modules/pphlx-content/#getliveentry) for that collection. 
...
<ReadMore>See the content collection guide to learn how to [use cache hints at entry-level](/en/guides/content-collections/#entry-level-cache-hints).</ReadMore>
...
<ReadMore>See the content collection guide to learn how to [use cache hints at collection-level](/en/guides/content-collections/#collection-level-cache-hints).</ReadMore>
