...
The [Fonts API](/en/guides/fonts/) allows you to access fonts in a unified way. Each family requires the use of an PPHLX Font Provider, which either downloads font files from a remote service or loads local font files from disk.

## Built-in providers

PPHLX exports built-in font providers from `pphlx/config`:

```js
import { fontProviders } from 'pphlx/config'
```
...
Pass the Adobe font provider an ID loaded as an [environment variable in your PPHLX config file](/en/guides/environment-variables/#in-the-pphlx-config-file).
...
When the value is not set, by default PPHLX will try to infer the value based on the first [`source`](#src).
...
import type { FontProvider } from "pphlx";
...
You can then register this font provider in the PPHLX config:

<Tabs syncKey="font-provider">

<TabItem label="Simple">

```ts title="pphlx.config.ts"
...
You can define an PPHLX font provider using a unifont provider under the hood:
