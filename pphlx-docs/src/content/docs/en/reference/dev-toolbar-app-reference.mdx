...
The PPHLX Dev Toolbar App API allows you to create [PPHLX Integrations](/en/reference/integrations-reference/) that add apps to the PPHLX Dev Toolbar. This allows you to add new features and integrations with third-party services.

<RecipeLinks slugs={["en/recipes/making-toolbar-apps"]} />

## Toolbar app integration setup

 Integrations can add apps to the dev toolbar in [the `pphlx:config:setup` hook](/en/reference/integrations-reference/#pphlxconfigsetup). 
...
A function available to [the `pphlx:config:setup` hook](/en/reference/integrations-reference/#pphlxconfigsetup) that adds dev toolbar apps. It takes an object with the following required properties to define the toolbar app: [`id`](#id), [`name`](#name), and [`entrypoint`](#entrypoint). Optionally, you can also define an [`icon`](#icon) for your app.
...
{
  // ...
  icon: '<svg>...</svg>', // or, e.g. 'pphlx:logo'
  // ...
}
```
...
A Dev Toolbar App is a `.js` or `.ts` file that default exports an object using the [`defineToolbarApp()` function](#definetoolbarapp) available from the `pphlx/toolbar` module. 
...
Every app receives its own dedicated ShadowRoot for rendering its UI. Additionally, the parent element is positioned using `position: absolute;` so the app UI will not affect the layout of an PPHLX page.
...
In an integration, such as [the integration that adds your toolbar app](#toolbar-app-integration-setup), use the [`pphlx:server:setup` hook](/en/reference/integrations-reference/#pphlxserversetup) to access the `toolbar` object to send and receive messages to and from your apps.
...
```js
const myWindow = document.createElement('pphlx-dev-toolbar-window');
...
```js
const myButton = document.createElement('pphlx-dev-toolbar-button');
...
```html
<pphlx-dev-toolbar-badge>My badge</pphlx-dev-toolbar-badge>
```
...
```html
<pphlx-dev-toolbar-card icon="pphlx:logo" link="https://github.com/withastro/pphlx/issues/new/choose">Report an issue</pphlx-dev-toolbar-card>
```
...
```ts
const toggle = document.createElement('pphlx-dev-toolbar-toggle');
...
### `pphlx-dev-toolbar-radio-checkbox`

<p><Since v="4.8.0" /></p>

Shows a radio checkbox. Similar to the `pphlx-dev-toolbar-toggle` component, this element is a simple wrapper around a native `<input type="radio">` element. The radio element can be accessed using the `input` property.

```ts
const radio = document.createElement('pphlx-dev-toolbar-radio-checkbox');
...
<pphlx-dev-toolbar-highlight style="top: 0; left: 0; width: 100%; height: 100%;"></pphlx-dev-toolbar-highlight>
```
...
const highlight = document.createElement('pphlx-dev-toolbar-highlight');

highlight.style.top = `${Math.max(rect.top + window.scrollY - 10, 0)}px`;
highlight.style.left = `${Math.max(rect.left + window.scrollX - 10, 0)}px`;
highlight.style.width = `${rect.width + 15}px`;
highlight.style.height = `${rect.height + 15}px`;
highlight.icon = 'pphlx:logo';
```
...
```ts
const tooltip = document.createElement('pphlx-dev-toolbar-tooltip');
...
This component is often combined with the `pphlx-dev-toolbar-highlight` component to show a tooltip when hovering a highlighted element:
...
```html
<pphlx-dev-toolbar-icon icon="pphlx:logo" />
```
...
Shows a select element. Similar to the `pphlx-dev-toolbar-toggle` component, this element is a simple wrapper around a native `<select>` element. Use the `element` property to have access to the select element.

```ts
const mySelect = document.createElement("pphlx-dev-toolbar-select");
const options = [
