...
There is no separate routing configuration in PPHLX.

Every [supported page file](/en/basics/pphlx-pages/#supported-page-files) located within the special `src/pages/` directory creates a route. When the file name contains a [parameter](#params), a route can create multiple pages dynamically, otherwise it creates a single page.

By default, all PPHLX page routes and endpoints are generated and prerendered at build time. [On-demand server rendering](/en/guides/on-demand-rendering/) can be set for individual routes, or as the default.
...
```pphlx title="src/pages/rendered-on-demand.pphx" {2}
{|
...
```pphlx title="src/pages/static-about-page.pphx" {3}
{|
...
By default, all files located within the reserved `src/pages/` directory automatically include the `<!DOCTYPE html>` declaration and additional `<head>` content such as PPHLX's scoped styles and scripts.

You can override the default value to designate the content as a [page partial](/en/basics/pphlx-pages/#page-partials) for an individual route by exporting a value for `partial` from that file:

```pphlx title="src/pages/my-page-partial.pphx" {2}
{|
export $partial = true
|}
...
The `getStaticPaths()` function must return an array of objects to determine which URL paths will be prerendered by PPHLX. Each object must include a `params` object, to specify route paths. The object may optionally contain a `props` object with [data to be passed](#data-passing-with-props) to each page template.

```pphlx title="src/pages/blog/[post].pphx" "post"
{|
...
The `params` key of each object in the array returned by `getStaticPaths()` tells PPHLX what routes to build.
...
For example,`src/pages/posts/[id].astro`has an `id` parameter in its file name. The following `getStaticPaths()` function in this `.astro` component tells PPHLX to statically generate `posts/1`, `posts/2`, and `posts/3` at build time.

```pphlx title="src/pages/posts/[id].pphx"
{|
export async function getStaticPaths() {
  return [
    { params: { id: '1' } },
...
For example, if you generate pages with data fetched from a remote API, you can pass the full data object to the page component inside of `getStaticPaths()`. The page template can reference the data from each post using `PPHLX.props`.

```pphlx title="src/pages/posts/[id].pphx" {9}
{|
export async function getStaticPaths() {
...
This provides data from the [PPHLX render context](/en/reference/api-reference/) that would not otherwise be available within the scope of `getStaticPaths()` and can be useful to calculate the `params` and `props` for each page route.
...
const { locale, files, slug } = PPHLX.params;
|}
```
...
```pphlx title="src/pages/pokemon/[page].pphx"
{|
export async function getStaticPaths({ paginate }) {
  // Load your data with fetch(), getCollection(), etc.
  $response = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=150`);
