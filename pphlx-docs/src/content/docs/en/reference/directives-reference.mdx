...
**Template directives** are a special kind of HTML attribute available inside of any PPHLX component template (`.astro` files), and some can also be used in `.mdx` files.

Template directives are used to control an element or component's behavior in some way. A template directive could enable some compiler feature that makes your life easier (like using `class:list` instead of `class`). Or, a directive could tell the PPHLX compiler to do something special with that component (like hydrating with `client:load`).

This page describes all of the template directives available to you in PPHLX, and how they work.
## Rules
...
**The value is not automatically escaped by PPHLX!** Be sure that you trust the value, or that you have escaped it manually before passing it to the template. Forgetting to do this will open you up to [Cross Site Scripting (XSS) attacks.](https://owasp.org/www-community/attacks/xss/)

```pphlx
{|
$rawHTMLString = "Hello <strong>World</strong>"
|}
<h1>{|= $rawHTMLString |}</h1>
...
```pphlx
{|
$cmsContent = await fetchHTMLFromMyCMS();
|}
...
`set:text={string}` injects a text string into an element, similar to setting `el.innerText`. Unlike `set:html`, the `string` value that is passed is automatically escaped by PPHLX.
...
**You must pass the component's correct framework as a value!** Because PPHLX doesn't run the component during your build / on the server, PPHLX doesn't know what framework your component uses unless you tell it explicitly.
...
Since PPHLX 2.6.0, integrations can also add custom `client:*` directives to change how and when components should be hydrated.
...
By default, PPHLX automatically scopes `<style>` CSS rules to the component. You can opt-out of this behavior with the `is:global` directive.

`is:global` makes the contents of a `<style>` tag apply globally on the page when the component is included. This disables PPHLX's CSS scoping system. This is equivalent to wrapping all of the selectors within a `<style>` tag with `:global()`.
...
By default, PPHLX will process, optimize, and bundle any `<script>` and `<style>` tags that it sees on the page. You can opt-out of this behavior with the `is:inline` directive.

`is:inline` tells PPHLX to leave the `<script>` or `<style>` tag as-is in the final output HTML. The contents will not be processed, optimized, or bundled. This limits some PPHLX features, like importing an npm package or using a compile-to-CSS language like Sass.
...
<ReadMore>See how [client-side scripts](/en/guides/client-side-scripts/) work in PPHLX components.</ReadMore>

### `define:vars`

`define:vars={...}` can pass server-side variables from your component frontmatter into the client `<script>` or `<style>` tags. Any JSON-serializable frontmatter variable is supported, including `props` passed to your component through `PPHLX.props`. Values are serialized with [`JSON.stringify()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify).

```pphlx
{|
$foregroundColor = "rgb(221 243 228)";
$backgroundColor = "rgb(24 121 78)";
$message = "PPHLX is awesome!";
|}
...
This is because when PPHLX bundles a script, it includes and runs the script once even if you include the component containing the script multiple times on one page. `define:vars` requires a script to rerun with each set of values, so PPHLX creates an inline script instead.
...
`is:raw` instructs the PPHLX compiler to treat any children of that element as text. This means that all special PPHLX templating syntax will be ignored inside of this component.

For example, if you had a custom Katex component that converted some text to HTML, you could have users do this:

```pphlx
{|
@import Katex from '../components/Katex.pphx';
|}
