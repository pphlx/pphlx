...
Create an integration and call the `setAdapter()` function in the [`pphlx:config:done`](/en/reference/integrations-reference/#pphlxconfigdone) hook. This allows you to define a server entrypoint and the features supported by your adapter.

The following example creates an adapter with a server entrypoint and stable support for PPHLX static output:
...
Specifies the method PPHLX will use to resolve the server entrypoint: `"auto"` (recommended) or `"explicit"` (default, but deprecated):
...
- **`"explicit"` (deprecated)**: You must provide the exports required by the host in the server entrypoint using a `createExports()` function before passing them to `setAdapter()` as an [`exports`](#exports) list. This supports adapters built using the PPHLX 5 version of the Adapter API. By default, all adapters will receive this value to allow backwards compatibility. **However, no new adapters should be created with this value.** Existing adapters should override this default value with `"auto"` as soon as they are able to migrate to the new v6 API.

The following example defines the `entrypointResolution` and `serverEntrypoint` to tell PPHLX that a custom entrypoint is provided:
...
The following example defines the `entrypointResolution` and Rollup options to tell PPHLX that a custom entrypoint is provided:
...
**Type:** `PPHLXAdapterFeatureMap`<br />
<Since v="3.0.0" />
</p>

A map of PPHLX's built-in features supported by the adapter. This allows PPHLX to determine which features an adapter supports, so appropriate error messages can be provided.

<ReadMore>Discover the [available PPHLX features](#pphlx-features) that an adapter can configure.</ReadMore>

### `adapterFeatures`

<p>

**Type:** `PPHLXAdapterFeatures`<br />
<Since v="3.0.0" />
</p>
...
A configuration object for PPHLX's client-side code.
...
Defines the headers to inject into PPHLX's internal fetch calls (e.g. Actions, View Transitions, Server Islands, Prefetch). This can be an object of headers or a function that returns headers.
...
export default function createIntegration() {
  return {
    name: '@example/my-adapter',
    hooks: {
      'pphlx:config:done': ({ config, setAdapter }) => {
        setAdapter({
          name: '@example/my-adapter',
          entrypointResolution: 'auto',
          serverEntrypoint: '@example/my-adapter/server.js',
...
Defines the path or ID of a module in the adapter's package that is responsible for starting up the built server when `pphlx preview` is run.
...
<ReadMore>Learn more about how to [build a preview entrypoint](#building-a-preview-entrypoint).</ReadMore>
...
The following example defines an `args` object with a property that identifies where assets generated by PPHLX are located:
...
Adapters can provide a custom prerenderer to control how pages are prerendered by using the [`setPrerenderer()`](/en/reference/integrations-reference/#setprerenderer-option) function in the `pphlx:build:start` hook.
...
You will need to create a file that executes during server-side requests to enable on-demand rendering with your particular host. PPHLX's adapter API attempts to work with any type of host and gives a flexible way to conform to the host APIs.

You can import and use [`createApp()`](/en/reference/modules/pphlx-app/#createapp) to access methods that allow you to work with standard [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request) and [`Response`](https://developer.mozilla.org/en-US/docs/Web/API/Response) objects.
...
<ReadMore>Learn more about the methods and utilities available in the [Adapter Server Entrypoint API Reference](/en/reference/modules/pphlx-app/).</ReadMore>

### Passing build-time configuration

If you need to access build-time configuration in your server entrypoint, you can pass via a virtual module. For example, your server might need to identify where assets generated by PPHLX are located.
...
export default function createIntegration() {
  ...
  return {
    name: '@example/my-adapter',
    hooks: {
      'pphlx:config:setup': ({ config, updateConfig }) => {
        _config = config;
...
When your adapter supports [hybrid](#hybridoutput) or [server output](#serveroutput), you can provide a preview entrypoint to enable support for [`pphlx preview`](/en/reference/cli-reference/#pphlx-preview). Static output does not require a preview entrypoint.
...
import type { CreatePreviewServer } from 'pphlx';
...
PPHLX features are a way for an adapter to tell PPHLX whether they are able to support a feature, and also the adapter’s level of support.

When using these properties, PPHLX will:
...
The following configuration tells PPHLX that this adapter has experimental support for the Sharp-powered built-in image service:
...
If the Sharp image service is used, PPHLX will log a warning and error to the terminal based on your adapter's support:
...
Defines whether the adapter is able to support `getSecret()` exported from [`pphlx:env/server`](/en/reference/modules/pphlx-env/). When enabled, this feature allows your adapter to retrieve secrets configured by users in `env.schema`.
...
The `pphlx/env/setup` module allows you to provide an implementation for `getSecret()`. In [your server entrypoint](#building-a-server-entrypoint), call `setGetEnv()` as soon as possible:

```js ins={2,4}
import { createApp } from 'pphlx/app/entrypoint';
import { setGetEnv } from "pphlx/env/setup"
...
The `classic` mode matches the default behavior of PPHLX. On prerendered pages, middleware is run at build time, and when the page is requested, the middleware is not run again. On dynamic pages, middleware is run at request time only. The middleware code is part of your server bundle.
...
Then, consume the hook [`pphlx:build:ssr`](/en/reference/integrations-reference/#pphlxbuildssr), which will give you a `middlewareEntryPoint`, an `URL` to the physical file on the file system.
...
Whether or not the adapter provides support for setting response headers for static pages. When this feature is enabled, PPHLX will return a map of the `Headers` emitted by the static pages. This map is available as `routeToHeaders` in the [`pphlx:build:generated` hook](/en/reference/integrations-reference/#pphlxbuildgenerated) and can be used to generate platform-specific output that controls HTTP headers, for example, to create a `_headers` file for platforms that support it.
...
export default function createIntegration() {
  return {
    ...
    hooks: {
      ...
      'pphlx:build:generated': ({ routeToHeaders }) => {
...
If PPHLX's default log message is redundant, or confusing to the user in combination with your [custom `message`](#adaptersupportwithmessagemessage), you can use `suppress: "default"` to suppress the default message and only log your message:
...
Describes the function an adapter should export to start a preview server when [`pphlx preview`](/en/reference/cli-reference/#pphlx-preview) is run.
...
The built server entry module generated by PPHLX.
...
**Type:** [`PPHLXIntegrationLogger`](/en/reference/integrations-reference/#pphlxintegrationlogger)
</p>

Defines an instance of the PPHLX logger that can be used to write logs when the preview server is running.
...
## Allow installation via `pphlx add`

[The `pphlx add` command](/en/reference/cli-reference/#pphlx-add) allows users to easily add integrations and adapters to their project.  To allow your adapter to be installed with this command, **add `pphlx-adapter` to the `keywords` field in your `package.json`**:

```json
{
  "name": "example",
  "keywords": ["pphlx-adapter"],
}
```

Once you [publish your adapter to npm](https://docs.npmjs.com/cli/v8/commands/npm-publish), running `pphlx add example` will install your package with any peer dependencies specified in your `package.json` and instruct users to update their project config manually.
