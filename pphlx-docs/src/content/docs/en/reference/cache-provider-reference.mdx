---
title: PPHLX Cache Provider API
sidebar:
...
PPHLX Cache Provider API allows you to configure and manage [route caching](/en/guides/caching/) behavior. This API includes built-in support for an in-memory cache provider and the ability to create custom providers for various caching strategies and runtimes.
...
Runtime providers implement `onRequest()` to intercept requests and cache responses in-process. They add an `X-PPHLX-Cache` response header for observability:
...
PPHLX includes a built-in in-memory LRU runtime cache provider suitable for single-instance deployments. Import `memoryCache()` from `pphlx/config`:

```js title="pphlx.config.mjs" ins="memoryCache"
import { defineConfig, memoryCache } from 'pphlx/config';
...
2. **The config helper** — A function exported for users to call in `pphlx.config.mjs`. It returns a [`CacheProviderConfig` object](#cacheproviderconfig) that tells PPHLX where to find the runtime module and what options to pass it. This is the same pattern used by the built-in [`memoryCache()` provider](#built-in-memory-cache-provider).
...
import type { CacheProviderConfig } from 'pphlx';
...
The config helper is then called in the PPHLX config:
...
import type { CacheProviderFactory } from 'pphlx';
...
When writing a CDN provider, PPHLX provides [shared utilities](#cache-provider-utilities) for building cache-control headers and path-based invalidation tags.

## Cache provider utilities

PPHLX exports shared helpers for CDN provider authors from `pphlx/cache/provider-utils`. These are the same helpers used by the first-party Netlify, Vercel, and Cloudflare providers.
...
import {
  ...
  normalizeTags,
} from 'pphlx/cache/provider-utils';
```
...
Generates a cache tag for a given path (`pphlx-path:{path}`). Used to support [`invalidate({ path })`](/en/reference/api-reference/#cacheinvalidate) on platforms that only offer tag-based purging.
...
<ReadMore>Learn how to [use cache options](/en/guides/caching/#setting-cache-options) to control caching behavior.</ReadMore>
...
**Type:** <code>(context: \{ request: Request; url: URL; waitUntil?: (promise: Promise\<unknown\>) => void \}, next: <a href="/en/reference/modules/pphlx-middleware/#middlewarenext">MiddlewareNext</a>) => Promise\<Response\></code>
</p>
...
The factory function type. Receives the provider's serializable config object from the PPHLX config.
