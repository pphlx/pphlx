---
title: PPHLX Session Driver API
sidebar:
...
PPHLX [sessions](/en/guides/sessions/) allow you to share data between requests for on-demand rendered pages. They require an PPHLX Session Driver to store session data.

## Built-in drivers

PPHLX exports built-in session drivers from `pphlx/config`:

```js
import { sessionDrivers } from 'pphlx/config'
```

Any [unstorage driver](https://unstorage.unjs.io/drivers) can be used, for example:

```js title="pphlx.config.mjs" ins={5-7} ins=" sessionDrivers "
...
- The [driver config](#the-session-driver-config), which lets PPHLX know what implementation to use at runtime and what config to forward
...
import type { SessionDriverConfig } from 'pphlx'
...
It is then registered in the PPHLX config:
...
Unstorage driver types are compatible with PPHLX's `SessionDriver` type.
