...
PPHLX provides built-in components and helper functions for optimizing and displaying your images. For features and usage examples, [see our image guide](/en/guides/images/).

## Imports from `pphlx:assets`
...
```pphlx title="src/components/MyComponent.pphx"
{|
...
- **Local images in `src/`** - you must **also import the image** using a relative file path or configure and use an [import alias](/en/guides/imports/#aliases). Then use the import name as the `src` value:
  ...
  ```pphlx title="src/pages/index.pphx" "myImportedImage" "{|= $myImportedImage |}"
  {|
  import { Image } from 'pphlx:assets';
...
- **Images in the `public/` folder** -  use the image's **file path relative to the public folder**:
  ...
  ```pphlx title="src/pages/index.pphx" '"/images/my-public-image.png"'
  {|
  import { Image } from 'pphlx:assets';
  |}
  <Image
...
- **Remote images** - use the image's **full URL** as the property value:
  ...
  ```pphlx title="src/pages/index.pphx" '"https://example.com/remote-image.jpg"'
  {|
  import { Image } from 'pphlx:assets';
  |}
  <Image
...
However, both of these properties are required for images stored in your `public/` folder as PPHLX is unable to analyze these files.
...
<img
  src="/_pphlx/my_image.hash.webp"
  srcset="
    /_pphlx/my_image.hash.webp 1.5x
    /_pphlx/my_image.hash.webp 2x
  "
  alt="A description of my image."
  width="800"
  height="450"
...
<img
  src="/_pphlx/my_image.hash.webp"
  srcset="
    /_pphlx/my_image.hash.webp 240w,
    /_pphlx/my_image.hash.webp 540w,
    /_pphlx/my_image.hash.webp 720w,
		/_pphlx/my_image.hash.webp 1600w
  "
  sizes="
...
```pphlx title="src/components/MyComponent.pphx" "inferSize"
{|
import { Image } from 'pphlx:assets';
|}
...
As of PPHLX 5.17.3, `inferSize` only fetches dimensions for [authorized remote image domains](/en/guides/images/#authorizing-remote-images). Remote images outside the allowlist are not fetched.
...
```pphlx title="src/components/MyComponent.pphx" "priority"
{|
import { Image } from 'pphlx:assets';
@import myImage from '../assets/my_image.png';
|}
...
```pphlx title="src/components/MyComponent.pphx" "background"
<Image
  src={|= $myImage |}
  alt="A description of my image"
  format="jpeg"
...
```pphlx title="src/components/Head.pphx"
{|
import { Font } from "pphlx:assets";
|}
...
The [`cssVariable`](/en/reference/configuration-reference/#fontcssvariable) registered in your PPHLX configuration:

```pphlx title="src/components/Head.pphx" "cssVariable"
{|
import { Font } from "pphlx:assets";
|}
...
```pphlx title="src/components/Head.pphx" "preload"
{|
import { Font } from "pphlx:assets";
|}
...
```pphlx title="src/components/Head.pphx" {7-10}
{|
import { Font } from "pphlx:assets";
|}
...
```pphlx title="src/components/Background.pphx"
{|
import { getImage } from "pphlx:assets";
@import myBackground from '../background.png'

$optimizedBackground = await getImage({src: myBackground, format: 'avif'})
|}
...
```ts
import { inferRemoteSize } from 'pphlx:assets';
...
**Type:** [`PPHLXConfig["image"]`](/en/reference/configuration-reference/#image-options)<br />
<Since v="3.0.9" />
</p>
...
```ts
import { fontData } from "pphlx:assets"
...
import { fontData, experimental_getFontFileURL } from "pphlx:assets";
...
import { baseService } from "pphlx/assets";
...
See [`getConfiguredImageService()`](#getconfiguredimageservice) from `pphlx:assets`.

### `getImage()`

<p>

**Type:** <code>(options: <a href="#unresolvedimagetransform">UnresolvedImageTransform</a>, imageConfig: <a href="/en/reference/configuration-reference/#image-options">PPHLXConfig['image']</a>) => Promise\<<a href="#getimageresult">GetImageResult</a>\></code>
</p>

A function similar to [`getImage()`](#getimage) from `pphlx:assets` with two required arguments: an `options` object with [the same properties as the Image component](#image-) and a second object for the [image configuration](/en/reference/configuration-reference/#image-options).
...
See [`LocalImageProps`](#localimageprops) from `pphlx:assets`.

### `RemoteImageProps`

See [`RemoteImageProps`](#remoteimageprops) from `pphlx:assets`.

## Imports from `pphlx/assets/utils`
...
```ts
import { isRemoteAllowed } from 'pphlx/assets/utils';
...
```ts
import { matchHostname } from 'pphlx/assets/utils';
...
```ts
import { matchPathname } from 'pphlx/assets/utils';
...
```ts
import { matchPattern } from 'pphlx/assets/utils';
...
```ts
import { matchPort } from 'pphlx/assets/utils';
...
```ts
import { matchProtocol } from 'pphlx/assets/utils';
...
```ts
import { isESMImportedImage } from 'pphlx/assets/utils';
...
```ts
import { isRemoteImage } from 'pphlx/assets/utils';
...
```ts
import { resolveSrc } from 'pphlx/assets/utils';
import localImage from "./images/photo.jpg";

const resolvedLocal = await resolveSrc(localImage);
// Example value: `{ src: '/@fs/home/username/dev/pphlx-project/src/images/photo.jpg', width: 800, height: 600, format: 'jpg' }`
...
// Example value: `{ src: '/@fs/home/username/dev/pphlx-project/src/images/dynamic-image.jpg', width: 800, height: 600, format: 'jpg' }`
```
...
```ts
import { imageMetadata } from 'pphlx/assets/utils';
...
// Example value:
// {
//    src: '/@fs/home/username/dev/pphlx-project/src/images/photo.jpg?origWidth=800&origHeight=600&origFormat=jpg',
//    width: 800,
//    height: 600,
//    format: 'jpg',
//    contents: Uint8Array([...])
...
Emits a client asset that will be moved to the client directory for assets (e.g. `dist/client/_pphlx/`) during SSR builds. This function is intended for integration authors who need to emit assets (such as images) from server-rendered content that should be available on the client.
...
```ts
import { emitClientAsset } from 'pphlx/assets/utils';
...
```ts
import { getOrigQueryParams } from 'pphlx/assets/utils';
...
```ts
import { inferRemoteSize } from 'pphlx/assets/utils';
...
```ts
import { propsToFilename } from 'pphlx/assets/utils';
...
```ts
import { hashTransform } from 'pphlx/assets/utils';
...
const imageService = 'pphlx/assets/services/sharp';
...
Describes the result of the transformation after the call to [`getImage()`](/en/reference/modules/pphlx-assets/#getimage).
...
<ReadMore>See also the [`inferSize` attribute](/en/reference/modules/pphlx-assets/#infersize) available on `<Image />`.</ReadMore>
...
**Type:** `{ entrypoint: 'pphlx/assets/services/sharp' | string; config?: T; }`<br />
<Since v="2.3.3" />
</p>
...
**Type:** `'pphlx/assets/services/sharp' | string`
</p>

A package or path to the image service module. This can be PPHLX's built-in Sharp service or a third-party service.
