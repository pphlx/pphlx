...
## Imports from `pphlx:static-paths`

```js
import { 
  StaticPaths,
} from 'pphlx:static-paths';
```
...
import { StaticPaths } from 'pphlx:static-paths';
...
export function createHandler(app) {
  return async (request) => {
    ...
    if (pathname === '/__pphlx_static_paths') {
      const staticPaths = new StaticPaths(app);
