...
:::note
This API is compatible with the `<ClientRouter />` included in `pphlx:transitions`, but can't be used with native browser MPA routing.
:::

For features and usage examples, [see our View Transitions guide](/en/guides/view-transitions/).

## Imports from `pphlx:transitions`
...
```pphlx title="src/pages/index.pphx" ins={2,7}
{|
...
```pphlx {2} /slide\\(.+\\)/
{|
import { slide } from 'pphlx:transitions';
|}
...
## Imports from `pphlx:transitions/client`
...
This option follows the [`history` option](https://developer.mozilla.org/en-US/docs/Web/API/Navigation/navigate#history) from the browser Navigation API but simplified for the cases that can happen on an PPHLX project.
...
Arbitrary data to be included in the `pphlx:before-preparation` and `pphlx:before-swap` events caused by this navigation.
...
- [`pphlx:before-preparation`](#pphlxbefore-preparation-event)
- [`pphlx:before-swap`](#pphlxbefore-swap-event)
...
An object containing the utility functions used to build PPHLX’s default swap function.
...
Marks scripts in the new document that should not be executed. Those scripts are already in the current document and are not flagged for re-execution using [`data-pphlx-rerun`](/en/guides/view-transitions/#data-pphlx-rerun).
...
Swaps the attributes between the document roots, like the `lang` attribute. This also includes PPHLX-injected internal attributes like `data-pphlx-transition`, which makes the transition direction available to PPHLX-generated CSS rules.
...
## `pphlx:transitions/client` types
...
- `animate`: PPHLX will simulate view transitions using custom attributes before updating page content.
...
Represents an [`pphlx:before-preparation` event](#pphlxbefore-preparation-event). This can be useful to type the event received by a listener:

```pphlx title="src/pages/index.pphx" "TransitionBeforePreparationEvent"

<script>
  import type { TransitionBeforePreparationEvent } from "pphlx:transitions/client";

  function listener(event: TransitionBeforePreparationEvent) {
    /* do something */
  }

  document.addEventListener("pphlx:before-preparation", listener);
</script>
```
...
Represents an [`pphlx:before-swap` event](#pphlxbefore-swap-event). This can be useful to type the event received by a listener:

```pphlx title="src/pages/index.pphx" "TransitionBeforeSwapEvent"

<script>
  import type { TransitionBeforeSwapEvent } from "pphlx:transitions/client";

  function listener(event: TransitionBeforeSwapEvent) {
    /* do something */
  }

  document.addEventListener("pphlx:before-swap", listener);
</script>
```

## Lifecycle events

### `pphlx:before-preparation` event
...
<ReadMore>Read more about how to use this event on the [View Transitions guide](/en/guides/view-transitions/#pphlxbefore-preparation).</ReadMore>

### `pphlx:after-preparation` event
...
<ReadMore>Read more about how to use this event on the [View Transitions guide](/en/guides/view-transitions/#pphlxafter-preparation).</ReadMore>

### `pphlx:before-swap` event
...
<ReadMore>Read more about how to use this event on the [View Transitions guide](/en/guides/view-transitions/#pphlxbefore-swap).</ReadMore>

### `pphlx:after-swap` event
...
The following attributes are common to both the [`pphlx:before-preparation`](#pphlxbefore-preparation-event) and [`pphlx:before-swap`](#pphlxbefore-swap-event) events, except for some that are only available with one or the other.
...
* In an [`pphlx:before-preparation` event](#pphlxbefore-preparation-event), this can be used to define custom directions. The property is writable and accepts any string.
* In an [`pphlx:before-swap` event](#pphlxbefore-swap-event), this can be used to retrieve the transition direction. The property is readonly and its value can be a predefined [`Direction`](#direction) or any string that an `pphlx:before-preparation` event listener might have set.
...
**Available in:** [`pphlx:before-preparation` event](#pphlxbefore-preparation-event)
</p>
...
**Available in:** [`pphlx:before-swap` event](#pphlxbefore-swap-event)
</p>
