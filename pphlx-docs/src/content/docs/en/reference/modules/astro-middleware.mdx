...
The following helpers can be imported from the regular middleware module when you build an [PPHLX Integration](/en/reference/integrations-reference/):
...
A low-level API to create an [`APIContext`](/en/reference/api-reference/)to be passed to an PPHLX middleware [`onRequest()` function](#onrequest).

This function can be used by integrations/adapters to programmatically execute the PPHLX middleware.

### `defineMiddleware()`

See [`defineMiddleware()`](#definemiddleware) from `pphlx:middleware`.

### `sequence()`

See [`sequence()`](#sequence) from `pphlx:middleware`.
...
An object to [create a context](#createcontext) to be passed to an PPHLX middleware. This contains the following properties:
...
An object containing the optional parameters to be passed to [`PPHLX.params`](/en/reference/api-reference/#params).
...
An object for storing arbitrary information from a middleware, accessible to the user via [`PPHLX.locals`](/en/reference/api-reference/#locals).
...
Represents an PPHLX middleware function. Middleware handlers receive two arguments and can either return a `Response` directly or call `next()` to invoke the next middleware in the chain. Alternatively, you can use [`defineMiddleware()`](#definemiddleware) to get type safety for your middleware.
...
An [PPHLX context](/en/reference/api-reference/) object mirroring many of the `PPHLX` global properties.
...
Since PPHLX v4.13.0, `next()` accepts an optional URL path parameter in the form of a string, `URL`, or `Request` to [rewrite](/en/guides/routing/#rewrites) the current request without retriggering a new rendering phase.
