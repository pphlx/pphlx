...
// Expose your defined collection to PPHLX
...
An optional Zod object or function that returns a Zod object to configure the type and shape of document frontmatter for a collection. Each value must use [a Zod validator](/en/reference/modules/pphlx-zod/#common-data-type-validators). (For live collections, see the [live `schema`](#schema-1) property.)
...
import { storeLoader } from '@example/pphlx-loader';
...
```pphlx title="src/pages/[posts].pphx"
...
$draftBlogPosts = await getCollection('blog', ({ data }) => {
...
```pphlx title="src/pages/shop/index.pphx"
{|
...
{|
import { getEntry } from 'pphlx:content';

// Get `src/content/blog/enterprise.md`
$enterprisePost = await getEntry('blog', 'enterprise');

// Get `src/content/captains/picard.json`
$picardProfile = await getEntry('captains', 'picard');

// Get the profile referenced by `data.captain`
$enterpriseCaptainProfile = await getEntry(enterprisePost.data.captain);
|}
```
...
```pphlx title="src/pages/blog/[id].pphx"
{|
import { getLiveEntry } from 'pphlx:content';

const { entry: liveCollectionsPost } = await getLiveEntry('blog', PPHLX.params.id);
...
```pphlx title="src/pages/blog/enterprise/index.pphx"
{|
import { getEntries, getEntry } from 'pphlx:content';

$enterprisePost = await getEntry('blog', 'enterprise');

// Get related posts referenced by `data.relatedPosts`
$enterpriseRelatedPosts = await getEntries(enterprisePost.data.relatedPosts);
|}
```
...
- `<Content />` - A component used to render the document's contents in an PPHLX file.
- `headings` - A generated list of headings, [mirroring PPHLX's `getHeadings()` utility](/en/guides/markdown-content/#available-properties) on Markdown and MDX imports.
...
```pphlx title="src/pages/blog/entry-1.pphx"
{|
import { getEntry, render } from 'pphlx:content';
$entry = await getEntry('blog', 'entry-1');
...
Query functions including [`getCollection()`](#getcollection), [`getEntry()`](#getentry), and [`getEntries()`](#getentries) each return entries with the `CollectionEntry` type. This type is available as a utility from `pphlx:content`:
...
A unique ID. Note that all IDs from PPHLX's built-in `glob()` loader are slugified.
...
import { type CollectionKey, getCollection } from 'pphlx:content';
...
import { defineCollection, type SchemaContext } from "pphlx:content";
import { z } from 'pphlx/zod';
import { glob } from 'pphlx/loaders';
...
If you have [configured a cache provider](/en/guides/caching/#configure-caching), pass the cache hint directly to [`PPHLX.cache.set()`](/en/reference/api-reference/#cacheset):
...
```pphlx title="src/pages/shop/[id].pphx"
{|
import { getLiveEntry } from 'pphlx:content';

const { entry } = await getLiveEntry('products', PPHLX.params.id);
|}
```
...
const { cacheHint } = await getLiveEntry('products', PPHLX.params.id);
