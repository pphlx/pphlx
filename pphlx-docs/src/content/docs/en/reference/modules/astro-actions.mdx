...
Actions help you build a type-safe backend you can call from client code and HTML forms. All utilities to define and call actions are exposed by the `pphlx:actions` module. For examples and usage instructions, [see the Actions guide](/en/guides/actions/).

## Imports from `pphlx:actions`
...
The `handler()` is called with user input as its first argument. If an [`input`](#input-validator) validator is set, the user input will be validated before being passed to the handler. The second argument is [a subset of PPHLX's `context` object](#actionapicontext).
...
An optional property that accepts a [Zod validator](/en/reference/modules/pphlx-zod/#common-data-type-validators) (e.g. Zod object, Zod discriminated union) to validate handler inputs at runtime. If the action fails to validate, [a `BAD_REQUEST` error](#actionerror) is returned and the `handler` is not called.
...
```pphlx title="src/pages/index.pphx" {5,8}
...
In order for PPHLX to recognize this property, you may need to restart the dev server or [run the `pphlx sync` command](/en/reference/cli-reference/#pphlx-sync) (`s + enter`).
...
```pphlx title="src/pages/index.pphx" {9-12} "isActionError"

<script>
import { isActionError, actions } from 'pphlx:actions';
...
```pphlx title="src/pages/index.pphx" {9-12} "ActionError"

<script>
import { ActionError, actions } from 'pphlx:actions';
...
**Type:** <code>(context: <a href="/en/reference/api-reference/">APIContext</a>) => PPHLXActionContext</code><br />
<Since v="5.0.0" />
</p>
...
A function to programmatically set the value returned by `PPHLX.getActionResult()` in middleware. It is passed the action name and an action result serialized by [`serializeActionResult()`](#serializeactionresult). Calling this function from middleware will disable PPHLX's own action result handling.
...
```pphlx title="src/components/my-component.pphx" {8,11}
<script>
import { actions, getActionPath } from 'pphlx:actions'
...
```pphlx title="src/components/my-component.pphx" {5} "sendBeacon"
<script>
import { actions, getActionPath } from 'pphlx:actions'
...
An object containing the query parameter names used internally by PPHLX when handling form action submissions.
...
import { ACTION_QUERY_PARAMS } from 'pphlx:actions'
...
A subset of the [PPHLX context object](/en/reference/api-reference/). The following properties are not available: `callAction`, `getActionResult`, `props`, and `redirect`.
...
```pphlx title="src/pages/posts/post-1.pphx" "data" "error"
...
```pphlx title="src/pages/postal-service.pphx" "queryString"
{|
import { actions } from 'pphlx:actions';
@import FeedbackForm from '../components/FeedbackForm.pphx';

$feedbackUrl = new URL('/feedback', PPHLX.url);
...
```pphlx title="src/pages/posts/post-1.pphx" "orThrow"
...
```pphlx title="src/components/Form.pphx" {5}
{|
import { actions, ActionInputSchema } from 'pphlx:actions';
import { z } from 'pphlx/zod';
...
```pphlx title="src/components/Form.pphx" {4}
{|
import { actions, ActionReturnType } from 'pphlx:actions';
