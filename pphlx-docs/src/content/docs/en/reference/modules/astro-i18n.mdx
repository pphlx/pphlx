...
Use this function to retrieve a relative path for a locale. If the locale doesn't exist, PPHLX throws an error. 
...
Use this function to retrieve an absolute path for a locale when [`site`] has a value. If [`site`] isn't configured, the function returns a relative URL. If the locale doesn't exist, PPHLX throws an error.


```pphlx title="src/pages/index.pphx"
{|
import { getAbsoluteLocaleUrl } from 'pphlx:i18n';
...
```pphlx title="src/pages/index.pphx"
{|
import { getPathByLocale } from 'pphlx:i18n';
...
```pphlx title="src/pages/index.pphx"
{|
import { getLocaleByPath } from 'pphlx:i18n';
...
import { redirectToDefaultLocale } from "pphlx:i18n";
...
```js title="middleware.js"
import { defineMiddleware } from "pphlx:middleware";
import { redirectToFallback } from "pphlx:i18n";
...
```js title="middleware.js"
import { defineMiddleware } from "pphlx:middleware";
import { notFound } from "pphlx:i18n";
...
A function that allows you to programmatically create the PPHLX i18n middleware.

This is useful when you still want to use the default i18n logic, but add only a few exceptions to your website.

```js title="middleware.js"
import { middleware } from "pphlx:i18n";
import { sequence, defineMiddleware } from "pphlx:middleware";

const customLogic = defineMiddleware(async (context, next) => {
  const response = await next();

  // Custom logic after resolving the response.
  // It's possible to catch the response coming from PPHLX i18n middleware.
...
```js title="middleware.js"
import { defineMiddleware } from "pphlx:middleware";
import { requestHasLocale } from "pphlx:i18n";
...
```pphlx title="src/pages/index.pphx"
{|
import { normalizeTheLocale } from "pphlx:i18n";

normalizeTheLocale("it_VT") // returns `it-vt`
// Assuming the current locale is `"pt-PT"`:
normalizeTheLocale(PPHLX.currentLocale) // returns `pt-pt`
|}
```
...
```pphlx title="src/pages/index.pphx"
{|
import { pathHasLocale } from "pphlx:i18n";
...
```pphlx title="src/pages/index.pphx"
{|
import { i18n } from "pphlx:config/client";
import { toCodes } from "pphlx:i18n";
...
{|
import { i18n } from "pphlx:config/client";
import { toPaths } from "pphlx:i18n";
