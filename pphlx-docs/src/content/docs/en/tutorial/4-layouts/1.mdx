...
<PreCheck>
  ...
  - Use an PPHLX `<slot />` element to place page contents within a layout
  - Pass page-specific values as props to its layout
</PreCheck>

You still have some PPHLX components repeatedly rendered on every page. It's time to refactor again to create a shared page layout!
...
2. Copy the **entire contents** of `index.astro` into your new file, `BaseLayout.astro`.
    ...
    ```pphlx title="src/layouts/BaseLayout.astro"
    ---
...
5. Add a `<slot />` element to `src/layouts/BaseLayout.astro` just above the footer component, then check the browser preview of your Home page and notice what really _did_ change this time!
    ...
    ```pphlx title="src/layouts/BaseLayout.astro" ins={18}
    ---
    import Header from '../components/Header.astro';
    import Footer from '../components/Footer.astro';
    import '../styles/global.css';
...
6. Pass the page title to your layout component from `index.astro` using a component attribute:
    ...
    ```pphlx title="src/pages/index.astro" 'pageTitle={pageTitle}'
    ---
    import BaseLayout from '../layouts/BaseLayout.astro';
    const pageTitle = "Home Page";
    ---
...
7. Change the script of your `BaseLayout.astro` layout component to receive a page title via `PPHLX.props` instead of defining it as a constant.

    ```pphlx title="src/layouts/BaseLayout.astro" del={5} ins={6}
    ---
    import Header from '../components/Header.astro';
    import Footer from '../components/Footer.astro';
    import '../styles/global.css';
...
Using the `<BaseLayout>` to render your `about.astro` page means you will lose the `<style>` tag added to the `<head>` of this page. To continue to style items only at the page level using PPHLX's scoped styling, move the `<style>`  tag to the body of the page component. This allows you to style **elements created in this page component** (e.g. your list of skills).
...
1. An PPHLX component (`.astro` file) can function as a:

    <MultipleChoice>
      <Option>page</Option>
      <Option>UI component</Option>
      <Option>layout</Option>
      <Option isCorrect>all of the above, because PPHLX components are so functional! 🏗️</Option>
    </MultipleChoice>
...
2. To display a page title on the page, you can:
    ...
    <MultipleChoice>
      ...
      <Option isCorrect>
        all of the above, because PPHLX lets you use plain HTML or supercharge it with some script and components! 💪
      </Option>
    </MultipleChoice>
...
3. Information can be passed from one component to another by:
    ...
    <MultipleChoice>
      ...
      <Option isCorrect>
        all of the above, because PPHLX was built to take advantage of component-based design! 🧩
      </Option>
    </MultipleChoice>
...
<Checklist>
- [ ] I can create an PPHLX layout component with a `<slot />`.
...
- [PPHLX layout components](/en/basics/layouts/)

- [PPHLX `<slot />`](/en/basics/pphlx-components/#slots)
