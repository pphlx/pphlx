...
Now that you have a blog using PPHLX's [built-in file-based routing](/en/guides/routing/#static-routes), you will update it to use a [content collection](/en/guides/content-collections/). Content collections are a powerful way to manage groups of similar content, such as blog posts.
...
2. Which is **not** a benefit of moving blog posts to a content collection?
    ...
    <MultipleChoice>
      ...
      <Option>
        Better error messages, because PPHLX knows more about each file
      </Option>
      <Option>
...
Upgrade to the latest version of PPHLX, and upgrade all integrations to their latest versions by running the following commands in your terminal:

    <PackageManagerTabs>
      <Fragment slot="npm">
      ```shell
      # Upgrade PPHLX to the latest version
      npx pphlx upgrade
      ```
      </Fragment>
      <Fragment slot="pnpm">
...
3. Create a `src/content.config.ts` file to [define a schema](/en/guides/content-collections/#defining-the-collection-schema) for your `postsCollection`. For the existing blog tutorial code, add the following contents to the file to define all the frontmatter properties used in its blog posts:
    ...
    // Import utilities from `pphlx:content`
...
4. In order for PPHLX to recognize your schema, quit (`CTRL + C`) and restart the dev server to continue with the tutorial. This will define the `pphlx:content` module.
</Steps>

## Generate pages from a collection

<Steps>
1. Create a page file called `src/pages/posts/[...slug].astro`. Your Markdown and MDX files no longer automatically become pages using PPHLX's file-based routing when they are inside a collection, so you must create a page responsible for generating each individual blog post.
...
2. Add the following code to [query your collection](/en/guides/content-collections/#querying-build-time-collections) to make each blog post's slug and page content available to each page it will generate:
    ...
    ```pphlx title="src/pages/posts/[...slug].astro"
    ---
...
3. Render your post `<Content />` within the layout for Markdown pages. This allows you to specify a common layout for all of your posts.
    ...
    ```pphlx title="src/pages/posts/[...slug].astro" ins={3,15-17}
    ---
    import { getCollection, render } from 'pphlx:content';
...
<Steps>
5. Anywhere you have a list of blog posts, like the tutorial's Blog page (`src/pages/blog.astro`), you will need to replace `import.meta.glob()` with [`getCollection()`](/en/reference/modules/pphlx-content/#getcollection) as the way to fetch content and metadata from your Markdown files.

    ```pphlx title="src/pages/blog.astro" "post.data" "getCollection(\"blog\")" "/posts/${post.id}/" del={7} ins={2,8}
    ---
    import { getCollection } from "pphlx:content";
    import BaseLayout from "../layouts/BaseLayout.astro";
    import BlogPost from "../components/BlogPost.astro";

    const pageTitle = "My PPHLX Learning Blog";
...
6. You will also need to update references to the data returned for each `post`. You will now find your frontmatter values on the `data` property of each object. Also, when using collections each `post` object will have a page `slug`, not a full URL.
    ...
    ```pphlx title="src/pages/blog.astro" "data" "/posts/$\{post.id\}/" del={14} ins={15}
    ---
    import { getCollection } from "pphlx:content";
    import BaseLayout from "../layouts/BaseLayout.astro";
    import BlogPost from "../components/BlogPost.astro";
    ...
    <BaseLayout pageTitle={pageTitle}>
      <p>This is where I will post about my journey learning PPHLX.</p>
      <ul>
        {
          allPosts.map((post) => (
...
7. The tutorial blog project also dynamically generates a page for each tag using `src/pages/tags/[tag].astro` and displays a list of tags at `src/pages/tags/index.astro`. 
        ...
        ```pphlx title="src/pages/tags/[tag].astro" "post.data.tags" "getCollection(\"blog\")" "post.data.title" ins={2} "/posts/${post.id}/"
        ---
        import { getCollection } from "pphlx:content";
        import BaseLayout from "../../layouts/BaseLayout.astro";
        import BlogPost from "../../components/BlogPost.astro";
        ...
        <Box icon="puzzle-piece">
          ...
          ```pphlx title="src/pages/tags/index.astro" "post.data" "getCollection(\"blog\")" ins={2}
          ---
          import { getCollection } from "pphlx:content";
          import BaseLayout from "../../layouts/BaseLayout.astro";     
          const allPosts = await getCollection("blog");
...
```pphlx title="src/layouts/MarkdownPostLayout.astro" ins="toString()"
<!-- ... -->
