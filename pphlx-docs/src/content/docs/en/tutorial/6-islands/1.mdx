---
type: tutorial
title: Build your first PPHLX island
description: |-
...
<PreCheck>
  - Add Preact to your PPHLX project
  - Include PPHLX islands (Preact `.jsx` components) on your home page
...
2. Add the ability to use Preact components in your PPHLX project with a single command:

    <PackageManagerTabs>
      <Fragment slot="npm">
        ```sh
        npx pphlx add preact
        ```
      </Fragment>
      <Fragment slot="pnpm">
        ```sh
        pnpm pphlx add preact
        ```
      </Fragment>
      <Fragment slot="yarn">
        ```sh
        yarn pphlx add preact
        ```
      </Fragment>
    </PackageManagerTabs>
...
1. Create a new file in `src/components/` named `Greeting.jsx`
    ...
    Note the `.jsx` file extension. This file will be written in Preact, not PPHLX.
...
3. Import and use this component on your Home page `index.astro`.
    ...
    ```pphlx title="src/pages/index.astro" ins={3,8}
    ---
...
4. Add a second `<Greeting />` component with the `client:load` directive.
    ...
    ```pphlx title="src/pages/index.astro" ins={9} "client:load"
    ---
    import BaseLayout from '../layouts/BaseLayout.astro';
    import Greeting from '../components/Greeting';
    const pageTitle = "Home Page";
  ...
  5. Revisit your page and compare the two components. The second button works because the `client:load` directive tells PPHLX to send and rerun its JavaScript on the _client_ when the page _loads_, making the component interactive. This is called a **hydrated** component.
  
  6. Once the difference is clear, remove the non-hydrated Greeting component.


      ```pphlx title="src/pages/index.astro" del={8} "client:load"
      ---
      import BaseLayout from '../layouts/BaseLayout.astro';
      import Greeting from '../components/Greeting';
      const pageTitle = "Home Page";
...
Consider an PPHLX component with the following code:
...
<Checklist>
- [ ] I can install an PPHLX integration.
...
- [PPHLX Integrations Guide](/en/guides/integrations/)

- [Using UI Framework Components in PPHLX](/en/guides/framework-components/#using-framework-components)

- [PPHLX client directives reference](/en/reference/directives-reference/#client-directives)
