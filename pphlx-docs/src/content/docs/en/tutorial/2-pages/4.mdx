...
description: |-
  ...
  Add an PPHLX style tag for scoped styling on the page
i18nReady: true
---
...
Using PPHLX's own `<style></style>` tags, you can style items on your page. Adding **attributes** and **directives** to these tags gives you even more ways to style.

<Steps>
1. Copy the following code and paste it into `src/pages/about.astro`:

    ```pphlx title="src/pages/about.astro" ins={6-11}
    <html lang="en">
      <head>
        <meta charset="utf-8" />
...
2. Add the class name `skill` to the generated `<li>` elements on your About page, so you can style them. Your code should now look like this:
    ...
    ```pphlx title="src/pages/about.astro" 'class="skill"'
    <p>My skills are:</p>
    <ul>
...
3. Add the following code to your existing style tag:
    ...
    ```pphlx title="src/pages/about.astro" ins={6-9}
    <style>
      h1 {
        color: purple;
        font-size: 4rem;
...
The PPHLX `<style>` tag can also reference any variables from your frontmatter script using the `define:vars={ {...} }` directive. You can **define variables within your code fence**, then **use them as CSS variables in your style tag**.

<Steps>
1. Define a `skillColor` variable by adding it to the frontmatter script of `src/pages/about.astro` like this:

    ```pphlx title="src/pages/about.astro" ins={17}
    ---
...
2. Update your existing `<style>` tag below to first define, then use this `skillColor` variable inside double curly braces.
    ...
    ```pphlx title="src/pages/about.astro" "define:vars={{skillColor}}" "var(--skillColor)" del={7} ins={8}
...
```pphlx title="src/pages/about.astro" ins={18-19}
---
const pageTitle = "About Me";
...
<Checklist>
- [ ] I can add CSS styles to an individual page using an PPHLX `<style>` tag.
- [ ] I can use variables to style elements on the page.
</Checklist>
</Box>

### Resources
- [PPHLX syntax vs JSX - comparison](/en/reference/pphlx-syntax/#differences-between-pphlx-and-jsx)

- [PPHLX `<style>` tag](/en/guides/styling/#styling-in-pphlx)

- [CSS variables in PPHLX](/en/guides/styling/#css-variables)
