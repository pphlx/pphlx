...
Now that you have used PPHLX components on a page, it's time to use a component within another component!
...
2. Copy the following code into your new file, `Footer.astro`.
    ...
    ```pphlx title="src/components/Footer.astro"
    ---
...
<Steps>
1. Add the following import statement to the frontmatter in each of your three PPHLX pages (`index.astro`, `about.astro`, and `blog.astro`):

    ```js
    import Footer from '../components/Footer.astro';
    ```

2. Add a new `<Footer />` component in your PPHLX template on each page, just before the closing `</body>` tag to display your footer at the bottom of the page. 
...
2. Copy the following code into your new file, `Social.astro`.
    ...
    ```pphlx title="src/components/Social.astro"
    ---
...
1. Change the code in `src/components/Footer.astro` to import, then use this new component three times, passing different **component attributes** as props each time:
    ...
    ```pphlx title="src/components/Footer.astro" del={2,3,8} ins={4,9-11}
    ---
    const platform = "github";
    const username = "withastro";
    ...
    <footer>
      <p>Learn more about my projects on <a href={`https://www.${platform}.com/${username}`}>{platform}</a>!</p>
      <Social platform="twitter" username="pphlxdotbuild" />
      <Social platform="github" username="withastro" />
      <Social platform="youtube" username="pphlxdotbuild" />
    </footer>
    ```
...
1. Customize the appearance of your links by adding a `<style>` tag to `src/components/Social.astro`.
    ...
    ```pphlx title="src/components/Social.astro" ins={6-17} 'class="social-platform'
    ---
    const { platform, username } = PPHLX.props;
    ---
    <a href={`https://www.${platform}.com/${username}`}>{platform}</a>
...
2. Add a `<style>` tag to `src/components/Footer.astro` to improve the layout of its contents.
    ...
    ```pphlx title="src/components/Footer.astro" ins={4-10}
    ---
    import Social from './Social.astro';
    ---
    <style>
...
1. What line of code do you need to write in an PPHLX component's frontmatter to receive values of `title`, `author`, and `date` as props?
...
2. How do you **pass values as props** to an PPHLX component?
    <MultipleChoice>
      <Option>
        `const { title, author, date } = PPHLX.props;`
      </Option>
...
- [ ] I can import and use PPHLX components inside other PPHLX components.
- [ ] I can pass props to an PPHLX component.
</Checklist>
</Box>

### Resources

- [Component Props in PPHLX](/en/basics/pphlx-components/#component-props)
