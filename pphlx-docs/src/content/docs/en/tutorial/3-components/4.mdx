...
description: >-
  ...
  Add client-side interactivity to your mobile navigation with an PPHLX script
  tag
i18nReady: true
---
...
2. Copy the following code into your component. It creates a button that will be used to toggle the visibility of the navigation links on mobile devices. (You will add the new CSS styles to `global.css` later.)
    ...
    ```pphlx title="src/components/Menu.astro"
    --- 
    ---
...
3. Place this new `<Menu />` component just before your `<Navigation />` component in `Header.astro`. 
    ...
    ```pphlx title="src/components/Header.astro" ins={2,7}
    ---
...
<Steps>
1. Add the following `<script>` tag, using PPHLX's built-in TypeScript support, to `index.astro`, just before the closing `</body>` tag.

    ```pphlx title="src/pages/index.astro" ins={2-9}
      <Footer />
      <script>
...
2. Replace the contents of the `<script>` tag on `index.astro` with the following file import:
    ...
    ```pphlx title="src/pages/index.astro" ins={10} del={3-8}
      <Footer />
      <script>
        const menu = document.querySelector('.menu');
...
4. Add the same `<script>` with import to your other two pages, `about.astro` and `blog.astro` and verify that you have a responsive, interactive header on each page.
    ...
    ```pphlx title="src/pages/about.astro & src/pages/blog.astro" ins={2-4}
      <Footer />
      <script>
        import "../scripts/menu.js";
      </script>
...
1. When does PPHLX run any JavaScript written in a component's frontmatter?
    <MultipleChoice>
      <Option>
       PPHLX never runs JavaScript
      </Option>
      <Option isCorrect>
        at build-time
      </Option>
...
2. Optionally, PPHLX can send JavaScript to the browser to allow:
    <MultipleChoice>
      <Option>
...
[Client-side scripts in PPHLX](/en/guides/client-side-scripts/)
