...
But, since you already have the directory `/tags/`, you can take advantage of another routing pattern in PPHLX, and keep all your files related to tags together.
...
3. Create a minimal page at `src/pages/tags/index.astro` that uses your layout. You have done this before!
    ...
    <details>
      ...
      2. Import and use your `<BaseLayout>`.
          ...
          ```pphlx title=" src/pages/tags/index.astro"
          ---
      ...
      3. Define a page title, and pass it to your layout as a component attribute.
          ...
          <details>
          <summary>Show the code</summary>
          ```pphlx title="src/pages/tags/index.astro" ins={3} "pageTitle"
          ---
          import BaseLayout from '../../layouts/BaseLayout.astro';
...
<details>
    ...
    ---
    import BaseLayout from '../../layouts/BaseLayout.astro';    
    const tags = ['pphlx', 'blogging', 'learning in public', 'successes', 'setbacks', 'community']
    const pageTitle = "Tag Index";
    ---
...
1. In `src/pages/tags/index.astro`, add the line of code to the frontmatter script that will give your page access to the data from every `.md` blog post file.
    ...
    <details>
    <summary>See the code</summary>
    ```pphlx title = "src/pages/tags/index.astro" ins={3}
    ---
    import BaseLayout from '../../layouts/BaseLayout.astro';
...
2. Next, add the following line of JavaScript to your page component. This is the same code relying on PPHLX's built-in TypeScript support you used in `src/pages/tags/[tag].astro` to return a list of unique tags.

    ```pphlx title = "src/pages/tags/index.astro" ins={4}
    ---
    import BaseLayout from '../../layouts/BaseLayout.astro';
    const allPosts = Object.values(import.meta.glob('../posts/*.md', { eager: true }));
...
1. Add the following code to your component template:
    ...
    ```pphlx title="src/pages/tags/index.astro" ins={2}
    <BaseLayout pageTitle={pageTitle}>
      <div>{tags.map((tag) => <p>{tag}</p>)}</div>
    </BaseLayout>
    ```
    In your browser preview, verify that you can see your tags listed. If any blog posts are missing tags, or they are improperly formatted, PPHLX's built-in TypeScript support will show you errors so you can check and correct your code.

2. To make each tag link to its own page, add the following `<a>` link to each tag name:

    ```pphlx title="src/pages/tags/index.astro" '/tags/${tag}'
    <BaseLayout pageTitle={pageTitle}>
      <div>
        {tags.map((tag) => (
...
<Steps>
1. Add the following CSS classes to style both your `<div>` and each `<p>` that will be generated. Note: PPHLX uses HTML syntax for adding class names!

    ```pphlx title="src/pages/tags/index.astro" 'class="tags"' 'class="tag"'
    <BaseLayout pageTitle={pageTitle}>
      <div class="tags">
        {tags.map((tag) => (
...
1. In your `Navigation.astro` component, include a link to this new tag index page.
    ...
    ```pphlx title="src/components/Navigation.astro" ins={4}
    <a href="/">Home</a>
...
1. Copy the `<div class="tags">...</div>` and `<style>...</style>` from `src/pages/tags/index.astro` and reuse it inside `MarkdownPostLayout.astro`: 
    ...
    ```pphlx title="src/layouts/MarkdownPostLayout.astro" ins={11-15, 19-37}
    ---
...
```pphlx title="src/layouts/MarkdownPostLayout.astro" "frontmatter"
    <div class="tags">
...
To check your work, or if you just want complete, correct code to copy into `MarkdownPostLayout.astro`, here is what your PPHLX component should look like:

```pphlx title="src/layouts/MarkdownPostLayout.astro"
---
import BaseLayout from './BaseLayout.astro';
const { frontmatter } = PPHLX.props;
--- 
...
<Checklist>
- [ ] I can use PPHLX's `/pages/folder/index.astro` routing feature.
</Checklist>
</Box>

### Resources

- [Static Routing in PPHLX](/en/guides/routing/#static-routes)
