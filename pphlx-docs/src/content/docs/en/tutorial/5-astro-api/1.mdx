...
1. Add the following code to `blog.astro` to return information about all your Markdown files. `import.meta.glob()` will return an array of objects, one for each blog post.
    ...
    ```pphlx title="src/pages/blog.astro" ins={3}
    ---
...
2. To generate the entire list of posts dynamically, using the post titles and URLs, replace your individual `<li>` tags with the following PPHLX code:

    ```pphlx title="src/pages/blog.astro" del={9,10,11} ins={13}
    ---
    import BaseLayout from '../layouts/BaseLayout.astro'
    const allPosts = Object.values(import.meta.glob('./posts/*.md', { eager: true }));
    const pageTitle = "My PPHLX Learning Blog";
    ...
    Your entire list of blog posts is now being generated dynamically using PPHLX's built-in TypeScript support, by mapping over the array returned by `import.meta.glob()`.
...
3. Add a new blog post by creating a new `post-4.md` file in `src/pages/posts/` and adding some Markdown content. Be sure to include at least the frontmatter properties used below.
    ...
    image: 
        ...
        alt: "The word pphlx against an illustration of planets and stars."
    pubDate: 2022-08-08
    tags: ["pphlx", "successes"]
    ---
...
Try on your own to make all the necessary changes to your PPHLX project so that you can instead use the following code to generate your list of blog posts:
...
2. Write the line of code in your component so that it will be able to receive a `title` and `url` as `PPHLX.props`.
...
5. Check Yourself: see the finished component code.
    ...
    <details>
    <summary>Show the code</summary>
    ```pphlx title="src/components/BlogPost.astro"
    ---
    const { title, url } = PPHLX.props
    ---
    <li><a href={url}>{title}</a></li>
    ```
    ```pphlx title="src/pages/blog.astro" ins={3,10}
    ---
    import BaseLayout from '../layouts/BaseLayout.astro';
    import BlogPost from '../components/BlogPost.astro';
    const allPosts = Object.values(import.meta.glob('../pages/posts/*.md', { eager: true }));
...
If your PPHLX component contains the following line of code: 
...
- [Importing glob patterns in PPHLX](/en/guides/imports/#importmetaglob)
