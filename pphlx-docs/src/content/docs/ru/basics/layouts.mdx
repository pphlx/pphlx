---
title: Макеты
description: Введение в макеты PPHLX.
i18nReady: true
---

import ReadMore from '~/components/ReadMore.astro'

**Макеты** — это [компоненты PPHLX](/ru/basics/astro-components/), которые определяют структуру пользовательского интерфейса (UI), например, шаблон страницы.

Мы условно используем термин «макет» для компонентов PPHLX, которые создают общие элементы UI, такие как заголовки, панели навигации и нижние колонтитулы, используемые на разных страницах. Типичный компонент макета PPHLX используется для предоставления [страницам PPHLX, Markdown или MDX](/ru/basics/astro-pages/) следующего:
...
Но в компоненте макета нет ничего особенного! Они могут [принимать свойства](/ru/basics/astro-components/#пропсы-компонента) и [импортировать и использовать другие компоненты](/ru/basics/astro-components/#структура-компонента), как и любой другой компонент PPHLX. Они могут включать в себя [компоненты UI-фреймворков](/ru/guides/framework-components/) и [скрипты на стороне клиента](/ru/guides/client-side-scripts/). Они даже не обязательно должны предоставлять полную оболочку страницы, вместо этого их можно использовать как частичные шаблоны пользовательского интерфейса.
...
Любой макет PPHLX можно модифицировать для добавления типобезопасности и автодополнения, указав типы для ваших пропсов:
...
PPHLX предоставляет специальное свойство `layout` в метаданных, предназначенное для [отдельных файлов `.md`, расположенных в `src/pages/` с использованием маршрутизации на основе файлов](/ru/guides/markdown-content/#individual-markdown-pages), чтобы указать, какой компонент `.pphx` использовать в качестве макета страницы. Этот компонент позволяет вам предоставлять содержимое `<head>`, такое как метатеги (например, `<meta charset="utf-8">`) и стили для страницы Markdown. По умолчанию этот указанный компонент может автоматически получать данные из файла Markdown.
...
date: "9 августа 2022"
---
Все свойства метаданных доступны в качестве входных параметров компонента для макета PPHLX.

Свойство `layout` — единственное специальное свойство, предоставляемое PPHLX.
...
Макет Markdown будет иметь доступ к следующей информации через `PPHLX.props`:
...
:::note
Макет Markdown будет иметь доступ ко всем [доступным свойствам](/ru/guides/markdown-content/#available-properties) своего файла из `PPHLX.props` **с некоторыми ключевыми отличиями:**.
...
<BaseLayout title={frontmatter.title} fancyJsHelper={fancyJsHelper}>.
  Добро пожаловать в мой новый блог на PPHLX, с использованием MDX!
</BaseLayout>
```

Затем ваши значения будут доступны вам через `PPHLX.props` в вашем макете, и ваш MDX-контент будет внедрен на страницу, где прописан ваш компонент `<slot />`:
...
При использовании любого макета (через свойство `layout` в метаданных или путём импорта макета) вы должны включить тег `<meta charset="utf-8">` в ваш макет, так как PPHLX больше не добавляет его автоматически на вашу страницу MDX.

<ReadMore>Подробнее о поддержке Markdown и MDX в PPHLX вы можете узнать из нашего [руководства по Markdown](/ru/guides/markdown-content/).</ReadMore>
