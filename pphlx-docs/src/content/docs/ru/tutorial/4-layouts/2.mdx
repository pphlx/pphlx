...
<Steps>
1. Создайте новый файл в `src/layouts/MarkdownPostLayout.pphx`

2. Скопируйте следующий код в `MarkdownPostLayout.pphx`
...
3. Добавьте следующие метаданные в `post-1.md`
    ...
    pubDate: 2022-07-01
    description: 'Это мой первый пост в моем новом блоге PPHLX.'
    author: 'Ученик PPHLX'
    image:
...
**Задача**: Идентифицируйте элементы, общие для каждого поста блога, и используйте `MarkdownPostLayout.pphx` для их отображения, вместо написания их в вашем Markdown в `post-1.md` и в каждом будущем посте.
...
Добро пожаловать в мой _новый блог_, посвящённый изучению PPHLX! Здесь я поделюсь своим путём обучения, создавая новый сайт.
```
...
Вы можете разобраться, что должно быть в пробелах, чтобы следующие два компонента вместе создали рабочий код PPHLX?

1.  ```markdown title="src/pages/posts/learning-astro.md"
    ---
    layout: ../../__________/MyMarkdownLayout.pphx
    title: "Изучение Markdown в PPHLX"
    author: Ученик PPHLX
    ____: 2022-08-08
    ---
    Сегодня я узнал так много! PPHLX позволяет мне писать в Markdown, а также использовать переменные из метаданных. Я даже могу получить доступ к этим значениям в компоненте макета PPHLX.
    ```
...
- [Макеты Markdown в PPHLX](/ru/guides/markdown-content/#frontmatter-layout-property)
