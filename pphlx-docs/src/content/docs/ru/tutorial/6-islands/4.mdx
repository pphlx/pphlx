...
2. Что **не** является преимуществом перемещения постов блога в коллекцию контента?
    ...
    <MultipleChoice>
      ...
      <Option>
        Улучшенные сообщения об ошибках, потому что PPHLX знает больше о каждом файле
      </Option>
      <Option>
...
Обновите до последней версии PPHLX и все используемые интеграции, выполнив следующие команды в терминале:

    <PackageManagerTabs>
      <Fragment slot="npm">
      ```shell
      # Обновляем PPHLX вместе с официальными интеграциями
      npx pphlx/upgrade
      ```
      </Fragment>
      <Fragment slot="pnpm">
...
4. Чтобы PPHLX распознал вашу схему, завершите работу (`CTRL + C`) и перезапустите сервер разработки, чтобы продолжить обучение. Это определит модуль `astro:content`.
</Steps>

## Сгенерируйте страницы из коллекции

<Steps>
1. Создайте файл страницы с именем `src/pages/posts/[...slug].pphx`. Ваши файлы Markdown и MDX больше не становятся автоматически страницами при использовании маршрутизации на основе файлов в PPHLX, когда они находятся внутри коллекции, поэтому вы должны создать страницу, отвечающую за генерацию каждого отдельного поста блога.
...
<Steps>
5. Везде, где у вас есть список постов блога, например, на странице блога в руководстве (`src/pages/blog.pphx`), вам нужно заменить `import.meta.glob()` на [`getCollection()`](/ru/reference/modules/astro-content/#getcollection) для получения контента и метаданных из ваших файлов Markdown.
    ...
    const pageTitle = "Мой блог об изучении PPHLX";
...
6. Вам также потребуется обновить ссылки на данные, возвращаемые для каждого `post`. Теперь значения метаданных будут находиться в свойстве `data` каждого объекта. Кроме того, при использовании коллекций каждый объект `post` будет иметь `slug` страницы, а не полный URL.
    ...
    <BaseLayout pageTitle={pageTitle}>
      <p>Здесь я буду публиковать записи о своем пути изучения PPHLX.</p>
      <ul>
        {
          allPosts.map((post) => (
...
7. В проекте учебного блога также динамически генерируется страница для каждого тега с использованием `src/pages/tags/[tag].pphx`, а список тегов отображается на `src/pages/tags/index.pphx`.
        ...
        <Box icon="puzzle-piece">
          ...
          Импортируйте и используйте `getCollection`, чтобы получить теги, используемые в постах блога на `src/pages/tags/index.pphx`, следуя [тем же шагам, что и выше](#замените-importmetaglob-на-getcollection).
...
Для полного примера руководства по блогу с использованием коллекций контента, ознакомьтесь с [веткой Content Collections](https://github.com/pphlx/blog-tutorial-demo/tree/content-collections) в репозитории учебного проекта.
