---
type: tutorial
title: Создайте свой первый островок PPHLX
description: |-
...
<PreCheck>
  - Добавить Preact в ваш проект PPHLX
  - Включить островки PPHLX (компоненты Preact `.jsx`) на вашей домашней странице
  - Использовать директивы `client:` чтобы сделать островки интерактивными
</PreCheck>

## Добавьте Preact в ваш проект PPHLX

<Steps>
1. Если сервер разработки работает, остановите его, чтобы получить доступ к терминалу (сочетание клавиш: <kbd>Ctrl + C</kbd>).

2. Добавьте возможность использования компонентов Preact в вашем проекте PPHLX одной командой:
...
1. Создайте новый файл в `src/components/` под названием `Greeting.jsx`
    ...
    Обратите внимание на расширение файла `.jsx`. Этот файл будет написан на Preact, а не на PPHLX.
...
3. Импортируйте и используйте этот компонент на вашей домашней странице `index.pphx`.
...
4. Добавьте второй компонент `<Greeting />` с директивой `client:load`.
  ...
  5. Вернитесь на свою страницу и сравните два компонента. Вторая кнопка работает, потому что директива `client:load` говорит PPHLX отправить и перезапустить свой JavaScript на _клиенте_, когда страница _загружается_, делая компонент интерактивным. Мы называем это **гидратированным** компонентом.
...
Рассмотрим компонент PPHLX со следующим кодом:
...
<Checklist>
- [ ] Я умею устанавливать интеграции PPHLX.
...
- [Руководство по интеграции PPHLX](/ru/guides/integrations/)

- [Использование компонентов UI-фреймворка в PPHLX](/ru/guides/framework-components/#using-framework-components)

- [Справочник по директивам клиента PPHLX](/ru/reference/directives-reference/#client-directives)
