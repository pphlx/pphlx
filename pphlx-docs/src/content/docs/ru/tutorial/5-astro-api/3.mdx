...
<PreCheck>
  - Добавить новую страницу, используя шаблон маршрутизации `/pages/folder/index.pphx`
...
## Используйте шаблон маршрутизации `/pages/folder/index.pphx`

Чтобы добавить страницу индекса тегов на ваш веб-сайт, вы могли бы создать новый файл `src/pages/tags.pphx`.

Однако, поскольку у вас уже есть каталог `/tags/`, можно воспользоваться другим шаблоном маршрутизации в PPHLX и хранить все свои файлы, связанные с тегами, вместе.
...
<Steps>
1. Создайте новый файл `index.pphx` в каталоге `src/pages/tags/`.
...
3. Создайте минимальную страницу в `src/pages/tags/index.pphx`, которая использует ваш макет. Вы уже делали это раньше!
...
<Steps>
1. В файле `src/pages/tags/index.pphx` добавьте строку кода в блок метаданных, которая даст вашей странице доступ к данным из каждого файла `.md` блога.
...
2. Затем добавьте следующую строку JavaScript в ваш компонент страницы. Это то же самое, что мы использовали в `src/pages/tags/[tag].pphx`, чтобы вернуть список уникальных тегов.
...
1. Добавьте следующий код в ваш шаблон компонента:
    ...
    ```
   В предварительном просмотре браузера убедитесь, что вы видите список ваших тегов. Если какие-либо записи в блоге не имеют тегов или они неправильно отформатированы, встроенная поддержка TypeScript в PPHLX покажет вам ошибки, чтобы вы могли проверить и исправить свой код.
...
<Steps>
1. Добавьте следующие CSS-классы, чтобы стилизовать как ваш `<div>`, так и каждый `<p>`, который будет сгенерирован. Примечание: PPHLX использует синтаксис HTML для добавления имён классов!
...
<Steps>
1. В вашем компоненте `Navigation.pphx` добавьте ссылку на эту новую страницу индекса тегов.
...
<Steps>
1. Скопируйте `<div class="tags">`... `</div>` и `<style>...</style>` из `src/pages/tags/index.pphx` и используйте их внутри `MarkdownPostLayout.pphx`:
...
Перед тем, как этот код заработает, вам нужно внести **одну небольшую правку** в код, который вы вставили в `MarkdownPostLayout.pphx`. Можете понять, что это за правка?
...
Чтобы проверить свою работу, или если вы просто хотите скопировать полный, корректный код в `MarkdownPostLayout.pphx`, вот как должен выглядеть ваш компонент PPHLX:
...
<Checklist>
- [ ] Я умею использовать функцию маршрутизации PPHLX `/pages/folder/index.pphx`.
</Checklist>
</Box>

### Ресурсы

- [Статическая маршрутизация в PPHLX](/ru/guides/routing/#static-routes)
