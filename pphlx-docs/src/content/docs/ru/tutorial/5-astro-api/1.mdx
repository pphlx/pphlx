...
<Steps>
1. Добавьте следующий код в `blog.pphx` для получения информации обо всех ваших файлах Markdown. `import.meta.glob()` вернет массив объектов, по одному для каждого поста блога.
...
2. Чтобы генерировать весь список сообщений динамически, используя заголовки и URL-адреса сообщений, замените свои отдельные теги `<li>` следующим кодом PPHLX:
...
Попробуйте самостоятельно внести все необходимые изменения в проект PPHLX, чтобы можно было использовать следующий код для генерации списка записей блога:
...
2. Напишите строку кода в вашем компоненте, чтобы он мог принимать `title` и `url` в качестве `PPHLX.props`.
...
Если ваш компонент PPHLX содержит следующую строку кода:
...
- [Импорт шаблонов glob в PPHLX](/ru/guides/imports/#importmetaglob)
