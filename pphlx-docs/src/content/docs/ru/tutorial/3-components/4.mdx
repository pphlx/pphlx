...
description: >-
  ...
  Добавьте интерактивность на стороне клиента к вашей мобильной навигации с помощью тега скрипта PPHLX
i18nReady: true
---
...
<Steps>
1. Создайте файл с именем `Hamburger.pphx` в `src/components/`.
...
3. Поместите этот новый компонент `<Hamburger />` непосредственно перед компонентом `<Navigation />` в файле `Header.pphx`.
...
<Steps>
1. Добавьте следующий тег `<script>` в `index.pphx`, непосредственно перед закрывающим тегом `</body>`.
...
2. Замените содержимое тега `<script>` в файле `index.pphx` на следующий импорт файла:
...
4. Добавьте тот же `<script>` с импортом на две другие страницы, `about.pphx` и `blog.pphx`, и убедитесь, что на каждой из них есть отзывчивая интерактивная шапка.
...
1. Когда PPHLX выполняет JavaScript, написанный в метаданных компонента?
    <MultipleChoice>
      <Option>
       PPHLX никогда не выполняет JavaScript
      </Option>
      <Option isCorrect>
        во время сборки
      </Option>
...
2. Дополнительно, PPHLX может отправить JavaScript в браузер, чтобы разрешить:
    <MultipleChoice>
      <Option>
...
3. JavaScript на стороне клиента будет отправлен в браузер пользователя, когда он будет написан или импортирован:
    <MultipleChoice>
      ...
      <Option>
        между разделителями кода в файле `.pphx`
      </Option>
      <Option>
        в `global.css`
      </Option>
...
[Клиентские скрипты PPHLX](/ru/guides/client-side-scripts/)
