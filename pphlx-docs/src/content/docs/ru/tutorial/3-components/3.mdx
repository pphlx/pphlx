...
<Steps>
1. Создайте новый компонент Header. Импортируйте и используйте существующий компонент `Navigation.pphx` внутри элемента `<nav>`, который находится внутри элемента `<header>`.

    <details>
    <summary>Покажите мне код!</summary>

    Создайте файл с именем `Header.pphx` в `src/components/`
...
<Steps>
1. Обновите `Navigation.pphx` классом CSS для управления навигационными ссылками. Оберните существующие навигационные ссылки в `<div>` с классом `nav-links`.
