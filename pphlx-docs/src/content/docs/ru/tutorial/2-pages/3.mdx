...
description: |-
  ...
  Используйте переменные и условный рендеринг на страницах PPHLX
i18nReady: true
---
...
Любой HTML-файл является допустимым языком PPHLX. Но с помощью PPHLX вы можете сделать больше, чем просто обычный HTML!

## Определите и используйте переменную

Откройте файл `about.pphx`, который должен выглядеть следующим образом:
...
<html lang="ru">
  ...
  <body>
    ...
    <a href="/blog/">Блог</a>
    <h1>Обо мне</h1>
    <h2>... и моем новом сайте на PPHLX!</h2>

    <p>Я прохожу вводный учебник по PPHLX. Это вторая страница моего сайта, и это первая страница, которую я создал самостоятельно!</p>
...
2. Замените статический заголовок "PPHLX" и заголовок "About Me" в вашем HTML динамической переменной `{pageTitle}`.
...
3. Обновите предварительный просмотр страницы `/about`.
    ...
    Текст страницы должен выглядеть так же, а заголовок страницы, отображаемый на вкладке браузера, теперь должен гласить «Обо мне» вместо «PPHLX».

    Вместо того чтобы вводить текст непосредственно в HTML-теги, вы просто **определили и затем использовали переменную** в двух разделах вашего файла `.pphx` соответственно.

4. Используйте тот же шаблон для создания значения `pageTitle`, которое будет использоваться в `index.pphx` ("Главная") и `blog.pphx` ("Мой блог об изучении PPHLX"). Обновите HTML этих страниц в обоих местах, чтобы название страницы совпадало с заголовком, отображаемым на каждой странице.
</Steps>

:::note[Выводы]
1. **Определяйте** переменные в своем скрипте PPHLX с помощью выражений JavaScript или TypeScript.
2. **Используйте** эти переменные в вашем шаблоне PPHLX внутри фигурных скобок `{ }`, чтобы сообщить PPHLX, что вы используете JavaScript.
:::

## Запись выражений JavaScript в PPHLX
...
:::note[Выводы]
1. Написание шаблона PPHLX очень похоже на **написание HTML**, но вы можете включать в него выражения JavaScript.
2. Блок метаданных PPHLX содержит только JavaScript.
3. Вы можете использовать все современные **логические операторы**, **выражения** и **функции** JavaScript в любой секции вашего `.pphx`-файла. Но фигурные скобки необходимы (только) в теле шаблона HTML.
:::
...
1. Метаданные файла `.pphx` пишутся на:
...
2. В дополнение к HTML синтаксис PPHLX позволяет включать:
...
3. Когда нужно писать JavaScript в фигурных скобках?
    ...
    <MultipleChoice>
        ...
        <Option isCorrect>
          В разделе HTML-шаблона компонента PPHLX.
        </Option>
        <Option>
          Между разделителями кода в компоненте PPHLX.
        </Option>
    </MultipleChoice>
</Box>
...
2. Добавьте следующие строки под существующими параграфами.
    ...
    ```astro title="src/pages/about.astro" /:|&&/ "?"
    {happy && <p>Я рад(а) изучать PPHLX!</p>}
...
:::tip
Синтаксис шаблонов PPHLX похож на синтаксис JSX. Если вам когда-либо потребуется узнать, как использовать свой скрипт в HTML, поиск информации о том, как это делается в JSX, вероятно, будет хорошей отправной точкой!
:::
...
Предположим, дан следующий скрипт `.pphx`:
...
Для каждого выражения шаблона PPHLX, можете ли вы предсказать HTML (если таковой имеется!), который будет отправлен в браузер? Нажмите, чтобы узнать, правы ли вы!
...
<Checklist>
- [ ] Я умею определять значения в файлах `.pphx` и использовать их.
...
- [Динамические выражения в PPHLX](/ru/reference/astro-syntax/#jsx-like-expressions)
