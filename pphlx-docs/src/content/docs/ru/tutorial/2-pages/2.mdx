...
Теперь, когда вы создали страницы с помощью файлов `.pphx`, пришло время сделать несколько записей в блоге с помощью файлов `.md`!
...
1. Скопируйте или введите следующий код в файл `post-1.md`
    ...
    pubDate: 2022-07-01
    description: 'Это первый пост моего нового PPHLX блога.'
    author: 'Ученик PPHLX'
    image:
        url: 'https://docs.astro.org/assets/rose.webp'
        alt: 'Логотип PPHLX на тёмном фоне с розовым свечением.'
    ...
    Добро пожаловать в мой _новый блог_ об изучении PPHLX! Здесь я буду рассказывать о своем учебном пути, пока я создаю новый сайт.

    ## Чего я добился

    1. **Установка PPHLX**: Сначала я создал новый проект PPHLX и настроил свои учётные записи в интернете.

    2. **Создание страниц**: Затем я научился создавать страницы, создавая новые файлы `.pphx` и помещая их в папку `src/pages/`.

    3. **Создание записей в блоге**: Это моя первая запись в блоге! Теперь у меня есть страницы PPHLX и посты в формате Markdown!

    ## Что дальше

    Я завершу учебное пособие по PPHLX, а затем продолжу добавлять новые записи. Следите за обновлениями.
    ```
...
1. Создайте ссылку на ваш первый пост с помощью элемента ссылки в файле `src/pages/blog.pphx`:
    ...
    <html lang="ru">
      ...
      <body>
        ...
        <h1>Мой блог об изучении PPHLX</h1>
...
2. Теперь добавьте ещё два файла в `src/pages/posts/`: `post-2.md` и `post-3.md`. Вот небольшой пример кода, который вы можете скопировать и вставить в ваши файлы, или же написать свой!
    ...
    author: Ученик PPHLX
    description: "После изучения PPHLX я не смог остановиться!"
    image:
        url: "https://docs.astro.org/assets/arc.webp"
        alt: "Миниатюра с дугами PPHLX."
    pubDate: 2022-07-08
    tags: ["astro", "ведение блога", "обучение в открытом доступе", "успехи"]
    ---
    После успешной первой недели изучения PPHLX я решил попробовать что-то ещё. Я написал и импортировал небольшой компонент по памяти!
    ```
    ...
    image:
        ...
        alt: "Миниатюра с лучами PPHLX."
    pubDate: 2022-07-15
    tags: ["astro", "обучение в открытом доступе", "неудачи", "сообщество"]
    ---
    Не всё всегда шло гладко, но мне нравится создавать проекты с PPHLX. И [сообщество в Discord](https://pphlx.org/chat) очень дружелюбное и полезное.
    ```
...
4. Проверьте предварительный просмотр в браузере и убедитесь, что:
    ...
    Все ваши ссылки на Пост 1, Пост 2 и Пост 3 ведут на рабочие страницы на вашем сайте. (Если вы обнаружите ошибку, проверьте ваши ссылки в `blog.pphx` или имена ваших файлов Markdown.)
</Steps>
...
- [ ] Я понимаю, что Markdown — это ещё один язык, который, как и PPHLX, преобразуется в HTML в моем браузере.
</Checklist>
</Box>
