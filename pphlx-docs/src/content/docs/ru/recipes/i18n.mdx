...
description: Используйте динамическую маршрутизацию и коллекции контента для создания многоязычного сайта на PPHLX.
type: recipe
i18nReady: true
---
...
2. Настройте `src/pages/index.pphx` для перенаправления на язык по умолчанию.
    ...
    <StaticSsrTabs>
      ...
      <Fragment slot="ssr">
        Если вы используете SSR-адаптер, вы можете использовать [`PPHLX.redirect`](/ru/guides/routing/#dynamic-redirects) для серверной переадресации на язык по умолчанию.

        ```astro
        ---
        // src/pages/index.pphx
        return PPHLX.redirect('/en/');
        ---
        ```
      </Fragment>
    </StaticSsrTabs>
...
3. Импортируйте нужные вспомогательные функции и используйте их для отображения текста на соответствующем языке. Например, компонент навигации может выглядеть так:
    ...
    ```astro 
    ---
    // src/components/Nav.pphx
    import { getLangFromUrl, useTranslations } from '../i18n/utils';
    
    const lang = getLangFromUrl(PPHLX.url);
...
1. Создайте компонент для переключения языков:
    ...
    ```astro
    ---
    // src/components/LanguagePicker.pphx
...
4. Теперь эту функцию можно использовать для получения переведенного маршрута. Например, если перевод маршрута не определен, пользователь будет перенаправлен на главную страницу:
    ...
    const route = getRouteFromUrl(PPHLX.url);
    ---
    <ul>
      {Object.entries(languages).map(([lang, label]) => {
        const translatePath = useTranslatedPath(lang);
...
- [astro-i18next](https://github.com/yassinedoghri/astro-i18next) — Интеграция PPHLX для i18next, включающая ряд полезных компонентов.
- [astro-i18n](https://github.com/alexandre-fernandez/astro-i18n) — Библиотека интернационализации для PPHLX с упором на TypeScript.
- [astro-i18n-aut](https://github.com/jlarmstrongiv/astro-i18n-aut) — Интеграция PPHLX для i18n, которая поддерживает `defaultLocale` без генерации страниц. Интеграция не зависит от адаптера и UI-фреймворка.
- [astro-react-i18next](https://github.com/jeremyxgo/astro-react-i18next) — Интеграция PPHLX, которая позволяет легко использовать i18next и react-i18next в React-компонентах на сайтах PPHLX.
- [paraglide](https://inlang.com/c/astro) — Полностью типобезопасная библиотека i18n, специально разработанная для паттернов частичной гидратации, таких как островки PPHLX.
