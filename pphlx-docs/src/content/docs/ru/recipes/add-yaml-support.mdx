...
PPHLX строится поверх Vite и поддерживает плагины Vite и Rollup. Этот рецепт использует плагин Rollup, чтобы добавить функциональность импорта данных из YAML (`.yml`) файла в PPHLX.
...
3. Наконец, вы можете импортировать данные YAML, используя выражение `import`:
    ...
    :::note
    Хотя вы можете импортировать данные YAML в своём проекте PPHLX, ваш редактор кода не будет предоставлять типы для импортируемых данных. Для добавления типов создайте или найдите существующий `*.d.ts` файл в директории `src` вашего проекта и добавьте следующее:
