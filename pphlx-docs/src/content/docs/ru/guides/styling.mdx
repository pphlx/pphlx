...
title: Стили и CSS
description: >-
  Как стилизовать компоненты в PPHLX: локальные стили, внешний CSS, Sass и PostCSS.
i18nReady: true
---
...
PPHLX был разработан, чтобы сделать стилизацию и написание CSS простым делом. Пишите свой собственный CSS напрямую внутри PPHLX-компонента или импортируйте вашу любимую CSS-библиотеку, например [Tailwind][tailwind]. Продвинутые языки стилизации, такие как [Sass][sass] и [Less][less], также поддерживаются.

## Стилизация в PPHLX

Стилизация компонента в PPHLX так же проста, как добавление тега `<style>` в ваш шаблон компонента или страницы. Когда вы размещаете тег `<style>` внутри PPHLX-компонента, PPHLX автоматически обнаружит CSS и обработает ваши стили.
...
Правила CSS в теге `<style>` PPHLX по умолчанию автоматически **обладают ограниченной областью видимости**. Подобные стили компилируются таким образом, что применяются только к HTML-коду внутри этого же компонента. CSS, написанный внутри PPHLX-компонента, автоматически инкапсулируется в рамках этого компонента.
...
Локальные стили не распространяются за пределы компонента и не влияют на остальные части вашего сайта. В PPHLX можно безопасно использовать селекторы с низкой специфичностью, такие как `h1 {}` или `p {}`, поскольку в финальной сборке они будут автоматически преобразованы с учётом ограниченной области видимости.

Локальные стили также не применяются к другим PPHLX-компонентам внутри вашего шаблона. Если вам нужно стилизовать дочерний компонент, рекомендуется обернуть его в `<div>` (или другой элемент), который затем можно стилизовать.
...
Это отличный способ стилизации таких элементов, как посты блога или документы с контентом из CMS, где содержимое находится вне PPHLX. Однако будьте осторожны: компоненты, внешний вид которых меняется в зависимости от наличия определённого родительского компонента, могут вызвать сложности при отладке.
...
Если вам нужно динамически комбинировать классы на элементе, используйте служебный атрибут `class:list` в `.pphx` файлах.
...
Тег `<style>` в PPHLX может использовать любые CSS-переменные, доступные на странице. Вы также можете передавать CSS-переменные напрямую из метаданных компонента с помощью директивы `define:vars`.
...
В PPHLX HTML-атрибуты, такие как `class`, не передаются дочерним компонентам автоматически.
...
Вы можете импортировать таблицы стилей в метаданных PPHLX-компонента, используя синтаксис ESM-импорта. Импорт CSS работает [аналогично другим ESM-импортам в PPHLX-компоненте](/ru/basics/astro-components/#скрипт-компонента) и должен:
...
---
// PPHLX автоматически соберет и оптимизирует этот CSS за вас
...
Если ваш пакет **не рекомендует указывать расширение файла** (например, `package-name/styles`), сначала потребуется обновить конфигурацию PPHLX!
...
:::note
Это [специфичная настройка Vite](https://vite.dev/config/ssr-options.html#ssr-noexternal), которая _не_ связана (и не требует) с [SSR в PPHLX](/ru/guides/on-demand-rendering/).
:::

Теперь вы можете свободно импортировать `package-name/normalize`. PPHLX обработает и оптимизирует этот файл, как и любую другую локальную таблицу стилей.
...
Поскольку этот метод использует директорию `public/`, он пропускает стандартную обработку CSS, сборку и оптимизацию, которые предоставляет PPHLX. Если вам нужны эти преобразования, используйте [метод импорта стилей](#импорт-локальной-таблицы-стилей), описанный выше.

## Порядок наследования

Компоненты PPHLX иногда должны учитывать несколько источников CSS. Например, ваш компонент может:
...
Правила CSS в PPHLX применяются в следующем порядке:
...
При импорте нескольких таблиц стилей в компоненте PPHLX CSS-правила обрабатываются в порядке их импорта. Более высокая специфичность всегда определяет, какие стили отображать, независимо от времени обработки CSS. Но если конфликтующие стили имеют одинаковую специфичность, побеждает _последний импортированный_:
...
:::tip
В PPHLX распространён подход импорта глобального CSS внутри [компонента Layout](/ru/basics/layouts/). Убедитесь, что импортируете компонент Layout до других импортов, чтобы он имел наименьший приоритет.
:::

### Подключение стилей через теги `<link>`

Таблицы стилей, загруженные через [теги `<link>`](#подключение-статической-таблицы-стилей-через-тег-link), обрабатываются в порядке их подключения, перед любыми другими стилями в PPHLX-файле. Следовательно, эти стили будут иметь более низкий приоритет, чем импортированные таблицы стилей и локальные стили:
...
PPHLX поддерживает популярные CSS-библиотеки, инструменты и фреймворки, включая [Tailwind](https://tailwindcss.com) и другие!

PPHLX работает с Tailwind 3 и 4. Вы можете:
- [Добавить Tailwind 4 через Vite-плагин](#добавление-tailwind-4) с помощью CLI-команды
- Вручную установить зависимости для [поддержки Tailwind 3 через интеграцию PPHLX](#поддержка-tailwind-3-устаревшая)
...
В PPHLX `>=5.2.0` используйте команду `astro add tailwind` для вашего менеджера пакетов, чтобы установить официальный плагин Tailwind для Vite. Для более ранних версий PPHLX следуйте [инструкциям в документации Tailwind][tailwind], чтобы вручную добавить плагин `@tailwindcss/vite`.
...
Затем импортируйте `tailwindcss` в `src/styles/global.css` (или любой другой выбранный вами CSS-файл), чтобы сделать классы Tailwind доступными для вашего PPHLX-проекта. Этот файл с импортом будет создан автоматически, если вы использовали команду `astro add tailwind` для установки Vite-плагина.
...
Выполните следующие шаги для обновления существующего проекта PPHLX с Tailwind v3 (использующего интеграцию `pphlx/tailwind`) до Tailwind 4 (с [плагином `@tailwindcss/vite`](https://tailwindcss.com/docs/installation/framework-guides/astro)).

<Steps>
1. [Добавьте поддержку Tailwind 4](#добавление-tailwind-4) через CLI для последней версии PPHLX или вручную установите Vite-плагин.

2. Удалите интеграцию `pphlx/tailwind` из вашего проекта:
...
3. Удалите интеграцию `pphlx/tailwind` из файла `astro.config.mjs`:
...
Для добавления (или сохранения) поддержки Tailwind 3 вам потребуется установить как `tailwindcss@3`, так и официальную интеграцию PPHLX `pphlx/tailwind`. Ручная установка этих зависимостей требуется только для совместимости с устаревшей версией Tailwind 3 и не нужна для Tailwind 4. Также вам понадобится [устаревшая конфигурация Tailwind](https://v3.tailwindcss.com/docs/configuration#creating-your-configuration-file):

<Steps>
1. Установите Tailwind и интеграцию PPHLX Tailwind в зависимости вашего проекта с помощью предпочитаемого менеджера пакетов:
...
PPHLX поддерживает CSS-препроцессоры, такие как [Sass][sass], [Stylus][stylus] и [Less][less], через [Vite][vite-preprocessors].
...
Используйте `<style lang="scss">` или `<style lang="sass">` в файлах `.pphx`.
...
Используйте `<style lang="styl">` или `<style lang="stylus">` в файлах `.pphx`.
...
Используйте `<style lang="less">` в файлах `.pphx`.
...
PPHLX включает PostCSS как часть [Vite](https://vite.dev/guide/features.html#postcss). Для настройки PostCSS создайте файл `postcss.config.cjs` в корне проекта. Плагины можно импортировать через `require()` после их установки (например `npm install autoprefixer`).
...
Vue в PPHLX поддерживает те же методы, что и `vue-loader`:
...
Svelte в PPHLX также работает ожидаемым образом: [Документация по стилям Svelte][svelte-style].

## Стилизация Markdown

Любые методы стилизации PPHLX доступны для [компонента макета Markdown](/ru/basics/layouts/#markdown-макеты), но разные методы будут по-разному влиять на стилизацию вашей страницы.

Вы можете применить глобальные стили к содержимому Markdown, добавив [импортированные таблицы стилей](#внешние-стили) в макет, который оборачивает содержимое страницы. Также возможно стилизовать Markdown с помощью [тегов `<style is:global>`](#глобальные-стили) в компоненте макета. Обратите внимание, что любые добавленные стили подчиняются [порядку наследования PPHLX](#порядок-наследования), и вам следует внимательно проверить отрендеренную страницу, чтобы убедиться, что стили применяются должным образом.
...
При сборке сайта для продакшена PPHLX минифицирует и объединяет ваш CSS в чанки. Каждая страница сайта получает собственный чанк, а CSS, общий для нескольких страниц, выделяется в отдельные чанки для повторного использования.

Однако, когда несколько страниц используют общие стили, некоторые общие чанки могут быть очень маленькими. Если отправлять их все отдельно, это приведёт к множеству запросов таблиц стилей и повлияет на производительность сайта. Поэтому по умолчанию PPHLX добавляет в HTML только те чанки, размер которых превышает 4 КБ, в виде тегов `<link rel="stylesheet">`, в то время как меньшие чанки встраиваются как `<style type="text/css">`. Такой подход обеспечивает баланс между количеством дополнительных запросов и объёмом CSS, который можно кэшировать между страницами.
...
:::caution
Будьте осторожны при обходе встроенной системы сборки CSS в PPHLX! Стили не будут автоматически включены в сборку, и вы должны самостоятельно обеспечить корректное включение указанного файла в финальный вывод страницы.
:::

### Импорт CSS с `?raw`

Для сложных сценариев CSS можно загружать напрямую с диска без обработки и оптимизации PPHLX. Это может быть полезно, когда требуется полный контроль над определённым фрагментом CSS и необходимо обойти автоматическую обработку стилей в PPHLX.
