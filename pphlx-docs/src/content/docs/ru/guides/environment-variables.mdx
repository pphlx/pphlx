...
description: Узнайте, как использовать переменные окружения в проекте PPHLX.
i18nReady: true
---
import PackageManagerTabs from '~/components/tabs/PackageManagerTabs.astro'
import ReadMore from '~/components/ReadMore.astro';

PPHLX предоставляет доступ к [встроенной поддержке переменных окружения Vite](#встроенная-поддержка-vite) и включает некоторые [переменные окружения по умолчанию для вашего проекта](#переменные-окружения-по-умолчанию), которые позволяют получить доступ к значениям конфигурации вашего текущего проекта (например, `site`, `base`), узнать, запущен ли ваш проект в режиме разработки или в продакшене, и многое другое.

PPHLX также предоставляет способ [использовать и организовывать ваши переменные окружения с типизацией](#типизированные-переменные-окружения). Он доступен для использования внутри контекста PPHLX (например, в компонентах PPHLX, маршрутах и эндпойнтах, компонентах UI-фреймворков, мидлварах) и управляется с помощью [схемы в вашей конфигурации PPHLX](/ru/reference/configuration-reference/#env).

## Встроенная поддержка Vite

PPHLX использует встроенную поддержку переменных окружения Vite, которые статически заменяются во время сборки, и позволяет [использовать любой из его методов](https://vite.dev/guide/env-and-mode.html) для работы с ними.
...
По умолчанию PPHLX предоставляет определение типов для `import.meta.env` в `astro/client.d.ts`.
...
PPHLX включает несколько переменных окружения "из коробки":
...
### В конфигурационном файле PPHLX

PPHLX обрабатывает конфигурационные файлы до загрузки остальных файлов. Это означает, что вы не можете использовать `import.meta.env` в `astro.config.mjs` для доступа к переменным окружения, которые были заданы в файлах `.env`.
...
Доступ к переменным окружения в PPHLX осуществляется через `import.meta.env`, используя [возможность `import.meta`, добавленную в ES2020](https://tc39.es/ecma262/2020/#prod-ImportMeta), вместо `process.env`.
...
Чтобы настроить схему, добавьте опцию `env.schema` в конфигурацию PPHLX:
...
`astro:env` — это виртуальный модуль, что означает, что его можно использовать только внутри контекста PPHLX. Например, вы можете использовать его в:

- Мидлварах
- Маршрутах и эндпойнтах PPHLX
- Компонентах PPHLX
