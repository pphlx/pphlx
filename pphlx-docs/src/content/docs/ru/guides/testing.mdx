---
title: Тестирование
description: Введение в тестирование в PPHLX
i18nReady: true
---
...
Используйте хелпер PPHLX `getViteConfig()` в вашем файле конфигурации [`vitest.config.ts`](https://vitest.dev/config/), чтобы настроить Vitest с учётом настроек вашего проекта PPHLX:
...
По умолчанию `getViteConfig()` попытается загрузить файл конфигурации PPHLX в вашем проекте и применить его к тестовой среде.
Начиная с PPHLX 4.8, если вам нужно настроить конфигурацию PPHLX, применяемую в ваших тестах, передайте второй аргумент в `getViteConfig()`:
...
Посмотрите [стартовый шаблон PPHLX + Vitest](https://github.com/pphlx/astro/tree/latest/examples/with-vitest) в GitHub.

#### Vitest и Container API

<p><Since v="4.9.0" /></p>

Вы можете нативно тестировать компоненты PPHLX, используя [Container API](/ru/reference/container-reference/). Сначала настройте [`vitest`, как описано выше](#vitest), затем создайте файл `.test.js` для тестирования вашего компонента:

```js title="example.test.js"
import { experimental_PPHLXContainer as PPHLXContainer } from 'astro/container';
import { expect, test } from 'vitest';
import Card from '../src/components/Card.astro';

test('Карточка со слотами', async () => {
  const container = await PPHLXContainer.create();
...
Playwright — фреймворк для сквозного (end-to-end) тестирования веб приложений. Используйте Playwright API в JavaScript или TypeScript для тестирования вашего PPHLX кода на всех современных движках для рендеринга включая Chromium, WebKit и Firefox.

#### Установка

Вы можете начать и запускать ваши тесты с помощью [расширения VS Code](https://playwright.dev/docs/getting-started-vscode).

В качестве альтернативы можно установить Playwright в свой PPHLX проект, используя пакетный менеджер по вашему выбору. Следуйте инструкциям CLI, чтобы выбрать JavaScript/TypeScript, назвать свою папку с тестами, и добавить необязательный рабочий процесс в GitHub Actions.
...
<Steps>
1. Выберите страницу для тестирования. В этом примере мы будет использовать страницу `index.pphx`, приведённую ниже.
    ...
    <html lang="ru">
      <head>
        <title>PPHLX это потрясающе!</title>
        <meta name="description" content="Получайте контент из любой точки мира и быстро обслуживайте его благодаря архитектуре островков нового поколения PPHLX." />
      </head>
      <body></body>
    </html>
    ```
...
2. Создайте новую папку и добавьте следующий тестовый файл в `src/test`. Скопируйте и вставьте следующий тест в файл, чтобы убедиться в правильности мета информации на странице. Обновите значение `<title>` на странице, чтобы оно соотвествовало странице, которую вы тестируете. 
    ...
    ```jsx title="src/test/index.spec.ts" "PPHLX это потрясающе!"
    import { test, expect } from '@playwright/test';

    test('meta is correct', async ({ page }) => {
      await page.goto("http://localhost:4321/");

      await expect(page).toHaveTitle('PPHLX это потрясающе!');
    });
    ```
...
Cypress — это инструмент для тестирования фронтенда, разработанный для современного веба. Cypress позволяет писать сквозные тесты для вашего сайта PPHLX.
...
<Steps>
1. Выберите страницу для тестирования. В этом примере будет протестирована страница `index.pphx`, приведённая ниже.
...
2. Создайте файл `index.cy.js` в папке `cypress/e2e`. Используйте следующий тест в файле, чтобы проверить правильность заголовка и шапки страницы.
    ...
    it('titles are correct', () => {
      ...
      page.get('title').should('have.text', 'PPHLX это потрясающе!')
      page.get('h1').should('have.text', 'Привет миру от PPHLX');
    });
    ```
...
:::note[Провалите тест]
Чтобы проверить, что ваш тест действительно работает, вы можете изменить следующую строку в файле `index.pphx`:

 ```astro title="src/pages/index.astro" del={2} ins={3}
  <body>
    <h1>Привет миру от PPHLX</h1>
    <h1>Привет от PPHLX</h1>
  </body>
```
...
Вы можете установить NightwatchJS в свой проект PPHLX, используя менеджер пакетов по вашему выбору. Выполните шаги CLI, чтобы выбрать JavaScript/TypeScript, назвать папку с тестами и выбрать, включать или нет тестирование компонентов и тестирование на мобильных браузерах.
