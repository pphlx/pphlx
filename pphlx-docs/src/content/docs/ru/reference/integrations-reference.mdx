---
title: API интеграции PPHLX
i18nReady: true
---
import Since from '~/components/Since.astro'

**Интеграции PPHLX** добавляют новую функциональность и поведение в ваш проект всего несколькими строками кода.

Эта справочная страница предназначена для тех, кто пишет свои собственные интеграции. Чтобы узнать, как использовать интеграцию в своем проекте, ознакомьтесь с нашим [Руководством по использованию интеграций](/ru/guides/integrations/).

## Примеры

Официальные интеграции PPHLX могут выступить для вас в качестве примера при создании ваших собственных интеграций.
...
PPHLX предоставляет хуки, которые интеграции могут реализовать для выполнения на определённых этапах жизненного цикла PPHLX. Хуки PPHLX определены в интерфейсе `IntegrationHooks`, который является частью глобального пространства имён `PPHLX`. Каждый хук имеет опцию [`logger`](#astrointegrationlogger), позволяющую использовать логгер PPHLX для записи логов.

В PPHLX встроены следующие хуки:

### `astro:config:setup`

**Следующий хук:** [`astro:route:setup`](#astroroutesetup)

**Когда:** При инициализации, перед тем как [Vite](https://vite.dev/config/) или [конфигурация PPHLX](/ru/reference/configuration-reference/) будут разрешены.

**Зачем:** Для расширения конфигурации проекта. Это включает обновление [конфигурации PPHLX](/ru/reference/configuration-reference/), применение [плагинов Vite](https://vite.dev/guide/api-plugin.html), добавление рендеров компонентов и внедрение скриптов на страницу.
...
Доступная только для чтения копия пользовательской [конфигурации PPHLX](/ru/reference/configuration-reference/). Это разрешается _до_ запуска любых других интеграций. Если вам нужна копия конфигурации после того, как все интеграции завершили обновление своих конфигураций, [см. хук `astro:config:done`](#astroconfigdone).
...
**Тип:** `(newConfig: DeepPartial<PPHLXConfig>) => PPHLXConfig;`
</p>

Функция обратного вызова для обновления предоставленной пользователем [конфигурации PPHLX](/ru/reference/configuration-reference/). Любая предоставленная вами конфигурация **будет объединена с пользовательской конфигурацией + обновлениями конфигурации других интеграций,** поэтому вы можете не включать ключи!
...
**Тип:** `(renderer:` [`PPHLXRenderer`](https://github.com/pphlx/astro/blob/fdd607c5755034edf262e7b275732519328a33b2/packages/astro/src/%40types/astro.ts#L872-L883) `) => void;`<br />
**Примеры:** [`svelte`](https://github.com/pphlx/astro/blob/main/packages/integrations/svelte/src/index.ts), [`react`](https://github.com/pphlx/astro/blob/main/packages/integrations/react/src/index.ts), [`preact`](https://github.com/pphlx/astro/blob/main/packages/integrations/preact/src/index.ts), [`vue`](https://github.com/pphlx/astro/blob/main/packages/integrations/vue/src/index.ts), [`solid`](https://github.com/pphlx/astro/blob/main/packages/integrations/solid/src/index.ts)
</p>
...
Если ваша интеграция зависит от какого-то конфигурационного файла, за которым Vite не следит и/или для вступления в силу которого требуется полный перезапуск сервера разработки, добавьте его с помощью `addWatchFile`. Каждый раз, при изменении этого файла, сервер разработки PPHLX будет перезагружен (вы можете проверить, когда произойдет перезагрузка с помощью `isRestart`).
...
**Тип:** `(directive:` [`ClientDirectiveConfig`](https://github.com/pphlx/astro/blob/00327c213f74627ac9ca1dec774efa5bf71e9375/packages/astro/src/%40types/astro.ts#L1872-L1875) `) => void;`<br />
<Since v="2.6.0" />
</p>

Добавляет [пользовательскую клиентскую директиву](/ru/reference/directives-reference/#custom-client-directives) для использования в файлах `.pphx`.
...
**Тип:** `(middleware:` [`PPHLXIntegrationMiddleware`](https://github.com/pphlx/astro/blob/852ac0f75dfca1b2602e9cdbfa0447d9998e2449/packages/astro/src/%40types/astro.ts#L2124-L2127) `) => void;`<br />
<Since v="3.5.0" />
</p>
...
Функция обратного вызова для инъекции маршрутов в проект PPHLX. Инжектируемые маршруты могут быть [страницами `.pphx`](/ru/basics/astro-pages/) или [обработчиками маршрутов `.js` и `.ts`](/ru/guides/endpoints/#эндпойнты-для-статических-файлов).

`injectRoute` принимает объект с `pattern` и `entrypoint`.

- `pattern` — куда маршрут должен быть выведен в браузере, например `/foo/bar`. Шаблон может использовать синтаксис пути к файлу PPHLX для обозначения динамических маршрутов, например `/foo/[bar]` или `/foo/[...bar]`. Обратите внимание, что расширение файла **не** требуется в `pattern`.
- `entrypoint` — обычный модульный указатель, указывающий на страницу `.pphx` или обработчик маршрута `.js`/`.ts`, который обрабатывает маршрут, обозначенный в `pattern`.
- `prerender` — булево значение, которое нужно установить, если PPHLX не может определить ваш экспорт `prerender`.

##### Пример использования:

```js
injectRoute({
  // Используйте синтаксис шаблонов PPHLX для динамических маршрутов.
  ...
  // Используйте только в том случае, если PPHLX не может определить ваш экспорт `prerender`
  prerender: false
});
```
...
При публикации вашего пакета (в данном случае `@fancy/dashboard`) на npm, вы должны экспортировать `dashboard.pphx` в ваш `package.json`:
...
- `"page"`: Аналогично `head-inline`, за исключением того, что импортируемый фрагмент обрабатывается Vite и объединяется с любыми другими тегами `<script>`, определенными внутри компонентов PPHLX на странице. Скрипт будет загружен с помощью `<script type="module">` в конечный вывод страницы, оптимизирован и разрешен Vite.
- ``page-ssr``: Импортируется как отдельный модуль в метаданных каждого компонента страницы PPHLX. Поскольку на этом этапе импортируется ваш скрипт, глобальный `PPHLX` недоступен, и ваш скрипт будет запущен только один раз при первом анализе `import`.
...
Функция, которая создаёт папку `<root>/.pphx/integrations/<normalized_integration_name>` и возвращает её путь.

Она позволяет вам иметь выделенную папку, избегая конфликтов с другими интеграциями или самим PPHLX. Эта директория создаётся при вызове данной функции, поэтому можно безопасно записывать файлы напрямую:
...
**Тип:** [`RouteOptions`](https://github.com/pphlx/astro/blob/3b10b97a4fecd1dfd959b160a07b5b8427fe40a7/packages/astro/src/types/public/integrations.ts#L14-L27)
</p>
...
**Когда:** После того, как конфигурация PPHLX была разрешена, и другие интеграции выполнили свои хуки `astro:config:setup`.
...
Доступная только для чтения копия предоставленной пользователем [конфигурации PPHLX](/ru/reference/configuration-reference/). Это разрешается _после_ выполнения других интеграций.

#### Опция `setAdapter()`

<p>

**Тип:** `(adapter: PPHLXAdapter) => void;`
</p>
...
Свойство `filename` будет использоваться для создания файла по пути `/.pphx/integrations/<normalized_integration_name>/<normalized_filename>.d.ts` и должно заканчиваться на `".d.ts"`.
...
Загрузчик может затем получить доступ к свойству `refreshContextData`, чтобы получить тело вебхука. Подробнее см. в описании свойства [`refreshContextData`](/ru/reference/content-loader-reference/#loadercontextrefreshcontextdata).
...
**Тип:** <code>Map\<string, <a href="https://github.com/pphlx/astro/blob/3b10b97a4fecd1dfd959b160a07b5b8427fe40a7/packages/astro/src/core/build/types.ts#L17-L23">PageBuildData</a>\></code>
</p>
...
**Тип:** [`SerializedSSRManifest`](https://github.com/pphlx/astro/blob/3b10b97a4fecd1dfd959b160a07b5b8427fe40a7/packages/astro/src/core/app/types.ts#L91-L109)
</p>
...
PPHLX резервирует префикс `astro:` для будущих встроенных хуков. Пожалуйста, выберите другой префикс при именовании вашего пользовательского хука.

## Справочник по типам интеграций

### `PPHLXIntegrationLogger`

Экземпляр логгера PPHLX, полезный для записи логов. Этот логгер использует тот же [уровень логирования](/ru/reference/cli-reference/#--verbose), что и настроенный через CLI.
...
* `src/pages/index.pphx` будет `/`
* `src/pages/blog/[...slug].pphx` будет `/blog/[...slug]`
* `src/pages/site/[blog]/[...slug].pphx` будет `/site/[blog]/[...slug]`
...
Например, для пути `[fruit]/about.pphx` регулярное выражение будет `/^\/([^/]+?)\/about\/?$/`. Использование `pattern.test("banana/about")` вернёт `true`.
...
Например, для маршрута, такого как `/blog/[...id].pphx`, функция `generate` может вернуть:
...
Позволяет получить доступ к `params` маршрута. Например, когда проект использует следующие [динамические маршруты](/ru/guides/routing/#dynamic-routes) `/pages/[lang]/[...slug].pphx`, значение будет `['lang', '...slug']`.
...
Для обычных маршрутов значение будет представлять собой путь URL, по которому будет обслуживаться этот маршрут. Когда проект использует [динамические маршруты](/ru/guides/routing/#dynamic-routes) (т. е. `[dynamic]` или `[...spread]`), путь будет неопределённым.

#### `segments`

<p>

**Тип:** <code><a href="https://github.com/pphlx/astro/blob/3b10b97a4fecd1dfd959b160a07b5b8427fe40a7/packages/astro/src/types/public/internal.ts#L154-L158">RoutePart</a>[][]</code>
</p>
...
Например, следующий маршрут `/pages/[blog]/[...slug].pphx` выведет сегменты:
...
* `page`: маршрут, который находится в файловой системе, обычно это компонент PPHLX
...
**Тип:** <code><a href="https://github.com/pphlx/astro/blob/3b10b97a4fecd1dfd959b160a07b5b8427fe40a7/packages/astro/src/types/public/config.ts#L39-L44">RedirectConfig</a> | undefined</code>
</p>
...
Определяет, поступает ли маршрут из ядра PPHLX (`internal`), интеграции (`external`) или проекта пользователя (`project`).
...
Например, для пути `[fruit]/about.pphx` регулярное выражение будет `/^\/([^/]+?)\/about\/?$/`. Использование `pattern.test("banana/about")` вернет `true`.
...
Например, следующий маршрут `/pages/[lang]/index.pphx` выведет сегменты `[[ { content: 'lang', dynamic: true, spread: false } ]]`.
...
- [Создание ваших собственных интеграций PPHLX](https://www.freecodecamp.org/news/how-to-use-the-astro-ui-framework/#chapter-8-build-your-own-astro-integrations-1) - Эммануэль Оханс на FreeCodeCamp
- [Шаблон интеграции PPHLX](https://github.com/florian-lefebvre/astro-integration-template) - Флориан Лефебр на GitHub
