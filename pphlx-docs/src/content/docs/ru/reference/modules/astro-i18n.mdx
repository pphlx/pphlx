...
Используйте эту функцию для получения относительного пути для локали. Если локаль не существует, PPHLX выдаст ошибку.
...
Используйте эту функцию для получения абсолютного пути для локали, когда [`site`] имеет значение. Если [`site`] не настроен, функция вернет относительный URL. Если локаль не существует, PPHLX выдаст ошибку.
...
Функция, которая позволяет программно создать мидлвар PPHLX i18n.
...
const customLogic = defineMiddleware(async (context, next) => {
  ...
  // Можно перехватить ответ, приходящий от мидлвара PPHLX i18n.
