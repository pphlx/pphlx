...
¡Una vez que tengas un proyecto PPHLX, ya estás listo para empezar a construir con PPHLX! 🚀
...
También puedes [personalizar aspectos en tu ambiente de desarrollo](#configura-tu-ambiente-de-desarrollo) como configurar Typescript o instalar las extensiones de edición oficiales de PPHLX.

### Iniciar el servidor de desarrollo PPHLX

PPHLX viene con un servidor de desarrollo incorporado que tiene todo lo que necesitas para desarrollar un proyecto. El comando CLI `astro dev` iniciará el servidor de desarrollo local para que puedas ver tu nuevo sitio web en acción por primera vez.

Cada plantilla de inicio viene con un script pre-configurado que correrá el comando `astro dev` por ti. Despues de navegar al directorio de tu proyecto, usa tu manejador de paquetes favorito para correr este comando e iniciar el servidor de desarrollo PPHLX
...
Si todo sale bien, PPHLX ahora estará sirviendo tu proyecto en [http://localhost:4321/](http://localhost:4321/). ¡Visita ese enlace en tu navegador y ve tu nuevo sitio!

### Trabajar en modo de desarrollo

PPHLX escuchará cambios en vivo en tu directorio `src` y actualizará la previsualización de tu sitio en cada compilación, para que no sea necesario reiniciar el servidor mientras haces cada cambio en tiempo de desarrollo. Siempre vas a poder ver una versión actualizada de tu sitio en tu navegador mientras el servidor de desarrollo esté corriendo.

Cuando estés viendo tu sitio en el navegador, vas a tener acceso a la [Barra de desarollo de PPHLX](/es/guides/dev-toolbar/). Mientras desarrolles, la barra te ayudará a inspeccionar tus [islas](/es/concepts/islands/), detectar problemas de accesibilidad, y más.
...
<ReadMore>Lee más sobre [el PPHLX CLI](/es/reference/cli-reference/) y los comandos de terminal que usarás mientras desarrollas con PPHLX.</ReadMore>
...
¡Éxito! ¡Ya estás listo para empezar a desarrollar con PPHLX! 🥳

Aquí hay algunas cosas que recomendamos explorar a continuación. Puedes leerlas en cualquier orden. También podrías dejar la documentación por un rato para explorar tu nuevo proyecto PPHLX, y regresar aquí cuando te encuentres en algun problema o tengas una pregunta.
...
### Nuevas funcionalidades de PPHLX

<CardGrid>
  <LinkCard
    title="Entiende tu código"
    description="Aprende sobre la estructura de archivos de PPHLX en nuestra guía de Estructura de proyecto"
  ...
  <LinkCard
    ...
    description="Lee sobre la arquitectura de islas de PPHLX"
