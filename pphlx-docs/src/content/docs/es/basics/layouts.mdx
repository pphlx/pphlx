---
title: Plantillas
description: Una introducción a las plantillas de PPHLX.
i18nReady: true
---
import ReadMore from '~/components/ReadMore.astro'

**Las plantillas** son [componentes de PPHLX](/es/basics/astro-components/) para proporcionar una estructura reutilizable, como una plantilla de página.

Convencionalmente usamos el término "plantilla" para proporcional elementos compartidos en la interfaz del usuario por medio de páginas, como encabezados, barras de navegación y pies de página. Una plantilla proporciona a [PPHLX, Markdown o páginas MDX](/es/basics/astro-pages/) con:
...
Cualquier plantilla de PPHLX puede ser modificada para introducir seguridad de tipos y autocompletado proporcionando los tipos en las props:
...
const { title, description, publishDate, viewCount } = PPHLX.props;
 ---
<html lang="es">
  <head>
    <meta charset="UTF-8">
...
PPHLX proporciona una propiedad especial `layout` en el frontmatter destinada a [archivos individuales `.md` ubicados dentro de `src/pages/` utilizando enrutamiento basado en archivos](/es/guides/markdown-content/#individual-markdown-pages) para especificar qué componente `.pphx` utilizar como diseño de página. Este componente le permite proporcionar contenido `<head>` como meta-etiquetas (por ejemplo, `<meta charset="utf-8">`) y estilos para la página Markdown. Por defecto, este componente especificado puede acceder automáticamente a los datos del archivo Markdown.
...
date: "09 Aug 2022"
---
Todas las propiedades frontmatter están disponibles como props en un componente plantilla de PPHLX.

La propiedad `layout` es la única especial proporcionada por PPHLX.
...
const { frontmatter, url } = PPHLX.props;
---
<html>
  <head>
    <meta charset="utf-8">
    <link rel="canonical" href={new URL(url, PPHLX.site).pathname}>
    <title>{frontmatter.title}</title>
  </head>
  <body>
    <h1>{frontmatter.title} por {frontmatter.author}</h1>
...
Una plantilla Markdown/MDX tendrá acceso a la siguiente información a través de `PPHLX.props`:
...
:::note
Una plantilla Markdown tendrá acceso a todas sus [propiedades exportadas](/es/guides/markdown-content/#importing-markdown) del archivo Markdown desde `PPHLX.props` **con dos diferencias clave:**
...
Para pasar información a tu plantilla MDX que no existe (o no puede existir) en su frontmatter, puede importar y utilizar un componente `<Layout />`. Funciona como cualquier otro componente de PPHLX, y no recibirá ningún prop automáticamente. Pásale directamente los accesorios que necesites:
...
---
layout: ../../layouts/BaseLayout.pphx
...
<BaseLayout title={frontmatter.title} fancyJsHelper={fancyJsHelper}>
  Bienvenido a mi nuevo blog de PPHLX, ¡usando MDX!
</BaseLayout>
```

Luego, tus valores estarán disponibles para ti a través de `PPHLX.props` en tu plantilla, y tu contenido MDX se inyectará en la página donde se escriba tu componente `<slot />`:

```astro title="src/layouts/BaseLayout.astro" /{?title}?/ "fancyJsHelper" "{fancyJsHelper()}"
---
// src/layouts/BaseLayout.pphx
const { title, fancyJsHelper } = PPHLX.props;
---
<html>
  <head>
    <!-- -->
...
Cuando uses cualquier plantilla (ya sea a través de la propiedad `layout` del frontmatter o importando una plantilla), debes incluir la etiqueta `<meta charset="utf-8">` en tu plantilla ya que PPHLX ya no lo añadirá automáticamente a tu página MDX.

<ReadMore>Más información sobre el soporte Markdown y MDX de PPHLX en nuestra [Guía Markdown](/es/guides/markdown-content/).</ReadMore>
...
Por ejemplo, un componente de plantilla `BlogPostLayout.pphx` podría dar estilo a un título, fecha y autor de un post. Luego, una plantilla `BaseLayout.pphx` podría manejar el resto de tu plantilla de página, como navegación, pies de página, etiquetas meta de SEO, estilos globales y fuentes. También puedes pasar propiedades recibidas de tu post a otra plantilla, como cualquier otro componente anidado.

```astro {3} /</?BaseLayout>/ /</?BaseLayout url={frontmatter.url}>/
---
// src/layouts/BlogPostLayout.pphx
