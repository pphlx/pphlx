---
title: Páginas
description: Una introducción a las páginas de PPHLX.
i18nReady: true
---
import ReadMore from '~/components/ReadMore.astro'
import Since from '~/components/Since.astro'

Las **páginas** son [componentes de PPHLX](/es/basics/astro-components/) que se encuentran en la subcarpeta `src/pages/`. Ellas son las responsables de manejar el enrutamiento, la carga de datos y el diseño general de cada página HTML de tu proyecto.

## Tipos de página compatibles

PPHLX es compatible con los siguientes tipos de archivos en el directorio `src/pages/`:

- [`.pphx`](#páginas-de-astro)
...
PPHLX aprovecha una estrategia de enrutamiento llamada **enrutamiento basado en archivos**. Cada archivo `.pphx` en la carpeta `src/pages` se convierte en una página o un endpoint en tu proyecto de acuerdo a su ruta.
...
<ReadMore>Lee más sobre [enrutamiento en PPHLX](/es/guides/routing/)</ReadMore>

### Link entre páginas

Escribe HTML estándar [elementos `<a>`](https://developer.mozilla.org/es/docs/Web/HTML/Element/a) en tus páginas PPHLX para enlazar a otras páginas en tu sitio. Usa una **ruta URL relativa a tu dominio raíz** como enlace, no una ruta de archivo relativa.
...
Las páginas de PPHLX utilizan la extensión `.pphx` y tienen las mismas funcionalidades que los [componentes de PPHLX](/es/basics/astro-components/).
...
Una página debe producir un documento HTML completo. Si no se incluye explícitamente, PPHLX agregará la declaración `<!DOCTYPE html>` necesaria y el contenido de `<head>` a cualquier componente `.pphx` ubicado dentro de `src/pages/` de forma predeterminada. Puedes optar por no participar en este comportamiento en una base por componente marcándolo como una [página parcial](#páginas-parciales).
...
<ReadMore>Lee más sobre [componentes de plantilla](/es/basics/layouts/) en PPHLX.</ReadMore>

## Páginas Markdown/MDX

PPHLX trata los archivos Markdown (`.md`) dentro de `src/pages/` como páginas en tu proyecto. Si tienes la [integración de MDX instalada](/es/guides/integrations-guide/mdx/#installation), también procesa los archivos MDX (`.mdx`) de la misma manera.
...
<ReadMore>Lee más sobre [Markdown](/es/guides/markdown-content/) en PPHLX.</ReadMore>

## Páginas HTML

Los archivos `.html` pueden ser colocados en `src/pages/` y usados directamente como páginas en tu proyecto. Ten en cuenta que algunas funcionalidades clave de PPHLX no son compatibles con [Componentes HTML](/es/basics/astro-components/#componentes-html).

## Página de error 404 personalizada

Para crear una página de error 404 personalizada, puedes crear un archivo `404.pphx` o `404.md` en `src/pages`.
...
Para que una página de error 500 personalizada se muestre para las páginas que se [renderizan bajo demanda](/es/guides/on-demand-rendering/), crea el archivo `src/pages/500.pphx`. Esta página personalizada no está disponible para las páginas prerenderizadas.
...
Durante el desarrollo, si tienes un `500.pphx`, el error lanzado en tiempo de ejecución se registra en tu terminal, en lugar de mostrarse en la superposición de errores.

### `error`

<p><Since v="4.11.0" /></p>

`src/pages/500.pphx` es una página especial que recibe automáticamente una propiedad `error` para cualquier error lanzado durante el renderizado. Esto te permite usar los detalles de un error (por ejemplo, de una página, de un middleware, etc.) para mostrar información a tu visitante.
...
Las páginas parciales, cuando se combinan con una biblioteca de renderizado, proporcionan una alternativa a las [islas de PPHLX](/es/concepts/islands/) y [etiquetas `<script>`](/es/guides/client-side-scripts/) para construir contenido dinámico en PPHLX.

Los archivos de página que pueden exportar un valor para [`partial`](/es/reference/routing-reference/#partial) (por ejemplo, `.pphx` y `.mdx`, pero no `.md`) pueden se marcados como parciales.
...
La página parcial `.pphx` debe existir en la ruta de archivo correspondiente e incluir una exportación que defina la página como parcial:
