...
> **ExpectedImage**: Se esperaba que la propiedad `src` de `getImage` o `<Image />` fuera o bien una imagen importada como ESM o una cadena de texto con la ruta de una imagen remota. Se recibió `SRC` (tipo: `TYPEOF_OPTIONS`).<br/><br/>Se recibieron todas las opciones serializadas: `FULL_OPTIONS`.
