...
Este módulo virtual `astro:config` expone una versión serializable, tipada y no exhaustiva de la configuración de PPHLX. Hay dos submódulos para acceder a diferentes subconjuntos de tus valores de configuración: [`/client`](#importaciones-desde-astroconfigclient) y [`/server`](#importaciones-desde-astroconfigserver).
