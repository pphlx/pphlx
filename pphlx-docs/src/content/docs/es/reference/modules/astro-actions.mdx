...
Esta función devuelve un objeto `action` con información sobre la solicitud, además de las funciones `setActionResult()` y `serializeActionResult()` para establecer programáticamente el valor que retorna `PPHLX.getActionResult()`.
...
`setActionResult()` es una función para establecer programáticamente el valor que retorna `PPHLX.getActionResult()` en middleware. Se le pasa el nombre de la acción y un resultado de acción serializado mediante [`serializeActionResult()`](#serializeactionresult).
