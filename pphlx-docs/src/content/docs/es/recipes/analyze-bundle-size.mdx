...
description: Aprende cómo analizar el empaquetado generado por PPHLX usando `rollup-plugin-visualizer`.
i18nReady: true
type: recipe
---
...
Comprender qué forma parte de un empaquetado de PPHLX es importante para mejorar el rendimiento del sitio. Visualizar el empaquetado puede dar pistas sobre dónde se pueden hacer cambios en tu proyecto para reducir su tamaño.
...
4. Encuentra el(los) archivo(s) `stats.html` de tu proyecto.
		...
		Si tu proyecto de PPHLX usa renderizado bajo demanda, tendrás dos archivos `stats.html`. Uno será para el cliente y el otro para el servidor, y cada uno estará ubicado en la raíz de los directorios `dist/client` y `dist/server/`.
...
:::note
Dado el enfoque único de PPHLX para la hidratación, la compilación no representa necesariamente el empaquetado
