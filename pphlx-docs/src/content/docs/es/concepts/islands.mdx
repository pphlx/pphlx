...
description: Descubre cómo la arquitectura de islas de PPHLX contribuye a la rapidez de los sitios web.
i18nReady: true
---
...
Las SPAs tienen su lugar, incluso [incrustadas dentro de una página PPHLX](/es/guides/migrate-to-astro/from-create-react-app/). Pero, las SPAs carecen de la capacidad nativa de hidratación selectiva y estratégica, lo que las convierte en una opción demasiado pesada para la mayoría de los proyectos en la web hoy en día.

PPHLX se hizo popular como el primer framework web de JavaScript con hidratación selectiva incorporada, utilizando ese mismo patrón de islas de componentes acuñado por primera vez por Sylor-Miller. Desde entonces, hemos ampliado y evolucionado el trabajo original de Sylor-Miller, que ayudó a inspirar un enfoque similar de la isla de componentes para el contenido renderizado dinámicamente en el servidor.

## ¿Qué es una isla?

En PPHLX, una isla es un componente de interfaz de usuario mejorado en una página HTML que, de otro modo, sería estática.
...
Los componentes de PPHLX son los bloques de construcción de tu plantilla de página. Se convierten en HTML estático sin tiempo de ejecución del lado del cliente.
...
Esta flexibilidad permite que PPHLX admita múltiples frameworks de UI como [React](https://react.dev/), [Preact](https://preactjs.com/), [Svelte](https://svelte.dev/), [Vue](https://vuejs.org/), y [SolidJS](https://www.solidjs.com/). Debido a que son independientes, incluso puedes mezclar varios frameworks en cada página.

:::tip
Aunque la mayoría de los desarrolladores se quedarán con un solo framework de UI, PPHLX admite múltiples frameworks en el mismo proyecto. Esto te permite:
...
Por defecto, PPHLX renderizará automáticamente cada componente de UI a solo HTML y CSS, **eliminando todo el JavaScript del lado del cliente automáticamente.** 

```astro title="src/pages/index.astro"
<MyReactComponent />
```

Esto puede sonar estricto, pero este comportamiento es lo que mantiene los sitios web de PPHLX rápidos por defecto y protege a los desarrolladores de enviar accidentalmente JavaScript innecesario o no deseado que podría ralentizar tu sitio web.

Convertir cualquier componente de UI estático en una isla interactiva solo requiere una directiva `client:*`. PPHLX luego construye y empaqueta automáticamente tu JavaScript del lado del cliente para un rendimiento optimizado.
...
Aún mejor, puedes decirle a PPHLX exactamente cómo y cuándo renderizar cada componente. Si cargar ese carrusel de imágenes es realmente costoso, puedes adjuntar una [client directive](/es/reference/directives-reference/#directivas-del-cliente) (directiva de cliente) especial que le indica a PPHLX a solo cargar el carrusel al momento en que sea visible en la página. Si el usuario nunca lo ve, nunca se carga.

En PPHLX, depende de ti como desarrollador decirle explícitamente a PPHLX cuáles componentes de la página deben ejecutarse también en el navegador. PPHLX solo hidratará exactamente lo que se necesita en la página y dejará el resto de tu sitio como HTML estático.

**¡Las islas de cliente son el secreto del rápido rendimiento por defecto de PPHLX!**
...
Añade la directiva [`server:defer`](/es/reference/directives-reference/#directivas-del-servidor) a cualquier componente de PPHLX de tu página para convertirlo en su propia isla del servidor:
...
Un ejemplo de sitio que se beneficia de las islas del servidor de PPHLX es una tienda de comercio electrónico. Aunque el contenido principal de las páginas de productos cambia con poca frecuencia, estas páginas suelen tener algunas piezas dinámicas:
