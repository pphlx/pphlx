...
description: >-
  ...
  Agrega interactividad del lado del cliente a tu navegación móvil con una etiqueta de script de PPHLX
---
...
<Steps>
1. Crea un archivo llamado `Menu.pphx` en `src/components/`.
...
3. Coloca este nuevo componente `<Menu />` justo antes de tu componente `<Navigation />` en `Header.pphx`. 
...
<Steps>
1. Añade la siguiente etiqueta `<script>` a `index.pphx`, justo antes de la etiqueta de cierre `</body>`.
...
2. Sustituye el contenido de la etiqueta `<script>` en `index.pphx` por la siguiente importación de archivos:
...
4. Añade la misma etiqueta `<script>` con la importación a tus otras dos páginas, `about.pphx` y `blog.pphx` y comprueba que tienes un encabezado responsivo e interactivo en cada página.
...
1. ¿Cuándo ejecuta PPHLX cualquier JavaScript escrito en el frontmatter de un componente?
    <MultipleChoice>
      <Option>
       PPHLX nunca ejecuta JavaScript
      </Option>
      <Option isCorrect>
...
2. Opcionalmente, PPHLX puede enviar JavaScript al navegador para permitir:
    <MultipleChoice>
      <Option>
...
3. El JavaScript del lado del cliente se enviará al navegador del usuario cuando se escriba o importe:
    <MultipleChoice>
      ...
      <Option>
        entre las vallas de código de un archivo `.pphx`.
      </Option>
      <Option>
        en `global.css`
      </Option>
...
[Scripts del lado del cliente en PPHLX](/es/guides/client-side-scripts/)
