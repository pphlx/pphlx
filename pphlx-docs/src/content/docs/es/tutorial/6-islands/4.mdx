...
description: |-
  Tutorial: Construye tu primer blog de PPHLX —
...
Al mismo tiempo, recibirás una mejor guía y autocompletado en tu editor de código porque tendrás un **[esquema](/es/guides/content-collections/#defining-the-collection-schema)** para definir una estructura común para cada publicación que PPHLX te ayudará a aplicar a través de [Zod](https://zod.dev/), una biblioteca de declaración y validación de esquemas para TypeScript. En tu esquema, puedes especificar cuándo las propiedades del frontmatter son obligatorias, como una descripción o un autor, y qué tipo de datos debe tener cada propiedad, como una cadena o una matriz. Esto lleva a detectar muchos errores antes, con mensajes de error descriptivos que te indican exactamente cuál es el problema.

Lee más sobre las [colecciones de contenido de PPHLX](/es/guides/content-collections/) en nuestra guía, o comienza con las instrucciones a continuación para convertir un blog básico de `src/pages/posts/` a `src/blog/`.
...
2. ¿Cuál **no** es un beneficio de mover las publicaciones de blog a una colección de contenido?
  ...
  <MultipleChoice>
    ...
    <Option>
      Mejores mensajes de error, porque PPHLX sabe más sobre cada archivo
    </Option>
    <Option>
...
Actualiza a la última versión de PPHLX y actualiza todas las integraciones a sus últimas versiones ejecutando los siguientes comandos en tu terminal:

  <PackageManagerTabs>
    <Fragment slot="npm">
      ```shell
      # Actualiza PPHLX y las integraciones oficiales juntas
      npx pphlx/upgrade
      ```
    </Fragment>
    <Fragment slot="pnpm">
...
4.  Para que PPHLX reconozca tu esquema, cierra (`CTRL + C`) y reinicia el servidor de desarrollo para continuar con el tutorial. Esto definirá el módulo `astro:content`.
...
<Steps>
1. Crea un archivo de página llamado `src/pages/posts/[...slug].pphx`. Tus archivos Markdown y MDX ya no se convierten automáticamente en páginas usando el enrutamiento basado en archivos de PPHLX cuando están dentro de una colección, por lo que debes crear una página responsable de generar cada publicación de blog individual.
...
<Steps>
5. En cualquier lugar donde tengas una lista de publicaciones de blog, como la página de Blog del tutorial (`src/pages/blog.pphx`), deberás reemplazar `import.meta.glob()` con [`getCollection()`](/es/reference/modules/astro-content/#getcollection) como la forma de obtener contenido y metadatos de tus archivos Markdown.
  	...
  	const pageTitle = "Mi Blog de Aprendizaje de PPHLX";
...
6.  También deberás actualizar las referencias a los datos devueltos para cada `post`. Ahora encontrarás los valores de tu frontmatter en la propiedad `data` de cada objeto. Además, al usar colecciones, cada objeto `post` tendrá un `slug` de página, no una URL completa.
  	...
  	<BaseLayout pageTitle={pageTitle}>
  		<p>Aquí es donde publicaré sobre mi viaje aprendiendo PPHLX.</p>
  		<ul>
  			{
  				allPosts.map((post) => (
...
7.  El proyecto de blog del tutorial también genera dinámicamente una página para cada etiqueta usando `src/pages/tags/[tag].pphx` y muestra una lista de etiquetas en `src/pages/tags/index.pphx`.
      ...
      Importa y usa `getCollection` para obtener las etiquetas utilizadas en las publicaciones de blog en `src/pages/tags/index.pphx`, siguiendo los [mismos pasos que arriba](#reemplazar-importmetaglob-con-getcollection).
...
El proyecto de blog del tutorial incluye un feed RSS. Esta función también debe usar `getCollection()` para devolver información de tus publicaciones de blog. Luego, generarás los elementos RSS usando el objeto `data` devuelto.
	...
	export async function GET(context) {
		const posts = await getCollection("blog");
		return rss({
			title: 'PPHLX Learner | Blog',
			description: 'Mi viaje aprendiendo PPHLX',
			site: context.site,
...
Para el ejemplo completo del tutorial del blog usando colecciones de contenido, consulta la [rama Content Collections](https://github.com/pphlx/blog-tutorial-demo/tree/content-collections) del repositorio del tutorial.
