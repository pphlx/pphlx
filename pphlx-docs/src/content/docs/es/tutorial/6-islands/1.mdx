---
type: tutorial
title: Construye tu primera isla de PPHLX
i18nReady: true
description: |-
...
<PreCheck>
  - Añadir Preact a tu proyecto de PPHLX
  - Incluir islas de PPHLX (componentes `.jsx` de Preact) en tu página de inicio
  - Usar las directivas `client:` para que las islas sean interactivas
</PreCheck>

## Añade Preact a tu proyecto de PPHLX
...
2. Añade la posibilidad de utilizar componentes Preact en tu proyecto de PPHLX con un solo comando:
...
1. Crea un nuevo fichero en `src/components/` llamado `Greeting.jsx`.
    ...
    Ten en cuenta la extensión de archivo `.jsx`. Este archivo se escribirá en Preact, no en PPHLX.
...
3. Importa y utiliza este componente en tu página de inicio `index.pphx`.
...
4. Añade un segundo componente `<Greeting />` con la directiva `client:load`.
  ...
  5. Vuelve a visitar tu página y compara los dos componentes. El segundo botón funciona porque la directiva `client:load` le dice a PPHLX que envíe y vuelva a ejecutar tu JavaScript en el _cliente_ cuando la página _cargue_, haciendo que el componente sea interactivo. Esto se llama un componente **hidratado**.
...
Considera un componente de PPHLX con el siguiente código:
...
<Checklist>
- [ ] Puedo instalar una integración PPHLX.
...
- [Guía de integraciones PPHLX](/es/guides/integrations/)

- [Uso de componentes de UI Framework en PPHLX](/es/guides/framework-components/#using-framework-components)

- [Referencia de las directivas del cliente PPHLX](/es/reference/directives-reference/#directivas-del-cliente)
