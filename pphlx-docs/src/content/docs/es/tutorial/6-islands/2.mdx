...
Ahora que puedes construir islas de PPHLX para elementos interactivos, ¡no olvides que puedes llegar bastante lejos con solo JavaScript y CSS básicos!
...
<Steps>
1. Crea un nuevo archivo en `src/components/ThemeIcon.pphx` y pega el siguiente código en él:
...
Para añadir interactividad a un componente de PPHLX, puedes utilizar una etiqueta `<script>`. Este script puede comprobar y establecer el tema actual desde `localStorage` y alternar el tema cuando se hace clic en el icono. 

<Steps>
1. Añade la siguiente etiqueta `<script>` en `src/components/ThemeIcon.pphx` después de tu etiqueta `<style>`:
...
Elige si cada una de las siguientes afirmaciones describe etiquetas de PPHLX `<script>`, componentes del framework UI, o ambos.

1. Te permiten incluir elementos de UI interactivos en tu sitio web.

    <MultipleChoice>
      <Option>
        Etiquetas `<script>` de PPHLX
      </Option>
      <Option>
...
- [Script del lado del cliente `<script>` en PPHLX](/es/guides/client-side-scripts/)
