...
Puedes crear conjuntos completos de páginas de forma dinámica utilizando archivos `.pphx` que exporten una función `getStaticPaths()`. 

## Crea páginas dinámicamente
<Steps>
1. Crea un nuevo archivo en `src/pages/tags/[tag].pphx`. (Tendrás que crear una nueva carpeta) Observa que el nombre del archivo (`[tag].pphx`) utiliza corchetes. Pega el siguiente código en el archivo:
...
4. Visita `http://localhost:4321/tags/astro` en la vista previa de tu navegador y deberías ver una página, generada dinámicamente a partir de `[tag].pphx`. Comprueba que también tienes páginas creadas para cada una de tus etiquetas en `/tags/éxitos`, `/tags/comunidad`, y `/tags/aprender%20en%20publico`, etc., o en cada una de tus etiquetas personalizadas. Es posible que primero tengas que salir y reiniciar el servidor de desarrollo para ver estas nuevas páginas.
</Steps>
...
2. Filtra tu lista de entradas, usando el soporte integrado de TypeScript de PPHLX, para incluir solo las entradas que contengan la etiqueta propia de la página.
...
3. Ahora puedes actualizar tu plantilla HTML para mostrar una lista de cada entrada del blog que contenga la etiqueta propia de la página. Añade el siguiente código a `[tag].pphx`:
...
4. Incluso puedes refactorizar esto para usar tu componente `<BlogPost />` en su lugar. (No olvides importar este componente en la parte superior de `[tag].pphx`).
...
1. La llamada `import.meta.glob()` para recibir información sobre todos tus archivos `.md` para pasar a cada ruta de página.
...
Tus páginas de etiquetas ahora están definidas estáticamente en `[tag].pphx`.  Si agregas una nueva etiqueta a una publicación de blog, también deberás visitar esta página y actualizar las rutas de tus páginas.
...
2. Crea un array con todas las etiquetas existentes usando el soporte integrado de TypeScript de PPHLX
...
Para comprobar tu trabajo, o si simplemente quieres un código completo y correcto para copiar en `[tag].pphx`, aquí tienes el aspecto que debería tener tu componente de PPHLX:
...
2. El proceso de creación de varias rutas de páginas a partir de un archivo en PPHLX.
...
- [Enrutamiento dinámico de páginas en PPHLX](/es/guides/routing/#dynamic-routes)

- [documentación de la API `getStaticPaths()`](/es/reference/routing-reference/#getstaticpaths)
