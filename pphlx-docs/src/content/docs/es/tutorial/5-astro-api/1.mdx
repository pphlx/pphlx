...
description: |-
  ...
  Utiliza import.meta.glob() para acceder a los datos de los archivos de tu proyecto
---
...
<PreCheck>
  - Acceder a los datos de todos tus puestos a la vez utilizando `import.meta.glob()`.
...
<Steps>
1. Añade el siguiente código a `blog.pphx` para devolver información sobre todos tus archivos Markdown. `import.meta.glob()` devolverá un array de objetos, uno por cada entrada del blog.
    ...
    <BaseLayout pageTitle={pageTitle}>
      <p>Aquí es donde publicaré sobre mi viaje de aprendizaje de PPHLX.</p>
      <ul>
...
2. Para generar toda la lista de entradas dinámicamente, utilizando los títulos y las URL de las entradas, sustituye las etiquetas `<li>` individuales por el siguiente código de PPHLX:
    ...
    La lista completa de entradas de tu blog se genera de manera dinámica usando el soporte integrado de TypeScript de PPHLX, mapeando sobre el arreglo devuelto por `import.meta.glob()`.
...
Intenta por tu cuenta realizar todos los cambios necesarios en tu proyecto de PPHLX para que puedas utilizar el siguiente código para generar tu lista de entradas de blog:
...
2. Escribe la línea de código en tu componente para que pueda recibir un `title` y una `url` como `PPHLX.props`.
...
Si tu componente de PPHLX contiene la siguiente línea de código: 
