...
<PreCheck>
  - Agregar una nueva página utilizando el patrón de enrutamiento `/pages/folder/index.pphx`
...
## Utiliza el patrón de enrutamiento `/pages/folder/index.pphx`

Para añadir una página de índice de etiquetas a tu sitio web, puedes crear un nuevo archivo en `src/pages/tags.pphx`. 

Pero, como ya tienes el directorio `/tags/`, puedes aprovechar otro patrón de enrutamiento en PPHLX, y mantener juntos todos tus archivos relacionados con etiquetas.
...
<Steps>
1. Crea un nuevo fichero `index.pphx` en el directorio `src/pages/tags/`.
...
3. Crea una página mínima en `src/pages/tags/index.pphx` que utilice tu plantilla. Esto ya lo has hecho antes.
...
<Steps>
1. En `src/pages/tags/index.pphx`, añade la línea de código al script frontmatter que dará acceso a tu página a los datos de cada archivo de entrada de blog `.md`.
...
2. A continuación, añade la siguiente línea de JavaScript a tu componente de página. Este es el mismo código que utiliza el soporte integrado de TypeScript de PPHLX que usaste en `src/pages/tags/[tag].pphx` para devolver una lista de etiquetas únicas.
...
1. Añade el siguiente código a tu plantilla de componentes:
    ...
    ```
    En la vista previa de tu navegador, comprueba que puedes ver tus etiquetas en la lista. Si alguna publicación del blog no tiene etiquetas o están mal formateadas, el soporte integrado de TypeScript de PPHLX te mostrará errores para que puedas revisar y corregir tu código.
...
<Steps>
1. Añade las siguientes clases CSS para dar estilo a tu `<div>` y a cada `<p>` que se genere. Nota: PPHLX utiliza sintaxis HTML para añadir nombres de clases.
...
<Steps>
1. En tu componente `Navigation.pphx`, incluye un enlace a esta nueva página de índice de etiquetas.
...
<Steps>
1. Copia el `<div class="tags">...</div>` y `<style>...</style>` de `src/pages/tags/index.pphx` y reutilízalo dentro de `MarkdownPostLayout.pphx`: 
...
Antes de que este código funcione, necesitas hacer **una pequeña edición** al código que pegaste en `MarkdownPostLayout.pphx`. ¿Puedes averiguar cuál es?
...
Para comprobar tu trabajo, o si simplemente quieres un código completo y correcto para copiar en `MarkdownPostLayout.pphx`, aquí tienes el aspecto que debería tener tu componente de PPHLX:
...
<Checklist>
- [ ] Puedo utilizar la función de enrutamiento `/pages/folder/index.pphx` de PPHLX.
</Checklist>
</Box>

### Recursos

- [Enrutamiento estático en PPHLX](/es/guides/routing/#static-routes)
