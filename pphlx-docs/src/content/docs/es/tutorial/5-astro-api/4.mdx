...
description: >-
  ...
  Instala el paquete oficial de PPHLX para crear un feed al que tus lectores puedan
  suscribirse
---
...
<PreCheck>
  - Instalar un paquete de PPHLX para crear un feed RSS para tu sitio web
  - Crear un feed que pueda ser suscrito y leído por lectores de RSS.
</PreCheck>

## Instala el paquete RSS de PPHLX

PPHLX ofrece un paquete personalizado para agregar rápidamente un feed RSS a tu sitio web.
...
<Steps>
1. Sal del servidor de desarrollo de PPHLX y ejecuta el siguiente comando en el terminal para instalar el paquete RSS de PPHLX.
...
2. Reinicia el servidor de desarrollo para empezar a trabajar de nuevo en tu proyecto de PPHLX.
...
2. Copia el siguiente código en este nuevo documento. Personaliza las propiedades `title` y `description` y si es necesario, especifica un idioma diferente en `customData`:
    ...
    export async function GET(context) {
      return rss({
        title: 'Alumno de PPHLX | Blog',
        description: 'Mi viaje de aprendizaje de PPHLX',
        site: context.site,
...
3. Agrega la propiedad  `site` al archivo de configuración de PPHLX con el URL único de tu sitio Netlify. 
...
<Checklist>
- [ ] Puedo instalar un paquete de PPHLX utilizando la línea de comandos.
...
- [Generación de elementos RSS en PPHLX](/es/recipes/rss/#using-glob-imports)
