...
Ya tienes un `BaseLayout.pphx` para definir la plantilla general de tus páginas. 

El `MarkdownPostLayout.pphx` te proporciona algunas plantillas adicionales para las propiedades comunes de las publicaciones de tu blog, como `title` y `date`, pero las publicaciones de tu blog no tienen el mismo aspecto que el resto de páginas de tu sitio. Puedes adaptar el aspecto de las entradas de tu blog al resto de tu sitio **anidando plantillas**.

<Steps>
1. En `src/layouts/MarkdownPostLayout.pphx`, importa `BaseLayout.pphx` y úsalo para envolver todo el contenido de la plantilla. No olvides pasar la propiedad `pageTitle`:
...
2. En `src/layouts/MarkdownPostLayout.pphx`, ahora puedes eliminar la etiqueta `meta`, ya que ya está incluida en tu `BaseLayout`:
...
3. Comprueba la vista previa de tu navegador en `http://localhost:4321/posts/post-1`. Ahora debería ver el contenido renderizado por:
...
4. Observa que el título de tu página se muestra ahora dos veces, una por cada plantilla.

    Elimina la línea que muestra el título de tu página de `MarkdownPostLayout.pphx`:
...
5. Comprueba de nuevo la vista previa de tu navegador en `http://localhost:4321/posts/post-1` y verifica que esta línea ya no aparece y que su título sólo se muestra una vez. Realiza cualquier otro ajuste necesario para asegurarte de que no tienes contenido duplicado.
</Steps>
  ...
  Asegúrate de que:
    ...
    - Ningún contenido se duplica en una página. (Si algo se está renderizando dos veces, asegúrate de eliminarlo de `MarkdownPostLayout.pphx`).
...
- [Plantillas anidadas en PPHLX](/es/basics/layouts/#plantillas-anidadas)
