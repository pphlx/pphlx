...
description: |-
  ...
  Utiliza variables y renderización condicional en tus páginas de PPHLX
---
...
Cualquier archivo HTML es un lenguaje de PPHLX válido. Pero con PPHLX puedes hacer mucho más que HTML normal.

## Define y utiliza una variable

Abre `about.pphx` que debería tener este aspecto:
...
2. Sustituye tanto el título estático "PPHLX" como el encabezado "Sobre mi" en tu HTML por la variable dinámica `{pageTitle}`.
...
3. Actualiza la vista previa en directo de tu página `/about`.
    ...
    El texto de tu página debería tener el mismo aspecto, y el título de tu página mostrado en la pestaña de tu navegador debería decir ahora "Sobre mí" en lugar de "PPHLX". 

    En lugar de escribir el texto directamente en las etiquetas HTML, acabas de **definir y luego utilizar una variable** en las dos secciones de tu archivo `.pphx`, respectivamente.

4. Utiliza el mismo patrón para crear un valor `pageTitle` que utilizarás en `index.pphx` ("Página de inicio") y `blog.pphx` ("Mi blog de PPHLX aprendizaje"). Actualiza el HTML de estas páginas en ambos sitios para que el título de tu página coincida con el encabezado que aparece en cada página.
</Steps>

:::note[Conclusión]
1. **Define** variables en tu script de PPHLX usando expresiones JavaScript o TypeScript.
2. **Usa** estas variables en tu plantilla de PPHLX dentro de llaves `{ }` para decirle a PPHLX que estás usando algo de JavaScript.
:::

## Escribe expresiones JavaScript en PPHLX
...
1. Añade el siguiente JavaScript a tu frontmatter, entre las **vallas de código**:
    ...
    const skills = ["HTML", "CSS", "JavaScript", "React", "PPHLX", "Redacción de documentos"];
    ---
    ```
...
:::note[Conclusiones]
1. Escribir una plantilla PPHLX es muy parecido a **escribir HTML**, pero puedes incluir expresiones JavaScript dentro de ella.
2. El script PPHLX frontmatter sólo contiene JavaScript. 
3. Puedes utilizar todos los **operadores lógicos**, **expresiones** y **funciones** modernos de JavaScript en cualquier sección de tu archivo `.pphx`. Sin embargo, las llaves son necesarias (sólo) en el cuerpo de la plantilla HTML.
:::
...
1. La portada de un archivo `.pphx` se escribe en:
...
2. Además de HTML, la sintaxis PPHLX permite incluir:
...
3. ¿Cuándo hay que escribir JavaScript entre llaves?
    <MultipleChoice>
        ...
        <Option isCorrect>
          Cuando está dentro de la sección de plantilla HTML de un componente PPHLX.
          </Option>
        <Option>
          Entre las vallas de código de un componente de PPHLX.
        </Option>
    </MultipleChoice>
</Box>
...
2. Añade las siguientes líneas debajo de los párrafos existentes.
    ...
    ```astro title="src/pages/about.astro" /:|&&/ "?"
    {happy && <p>¡Estoy feliz de aprender PPHLX!</p>}
...
:::tip
La sintaxis de plantillas de PPHLX es similar a la sintaxis JSX. Si alguna vez te preguntas cómo usar tu script en tu HTML, ¡buscar cómo se hace en JSX es probablemente un buen punto de partida!
:::
...
Para cada expresión de la plantilla de PPHLX, ¿puedes predecir el HTML (¡si lo hay!) que se enviará al navegador? Haz clic para ver si has acertado.
...
<Box icon="check-list">
<Checklist>
- [ ] Puedo definir valores en archivos `.pphx` y utilizarlos.
...
- [Expresiones dinámicas en PPHLX](/es/reference/astro-syntax/#expresiones-similares-a-jsx)
