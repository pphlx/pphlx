...
3. En `about.pphx`, añade la siguiente sentencia import a tu frontmatter: 
...
Añade la línea de código necesaria a cada archivo `.pphx` de tu proyecto para aplicar tus estilos globales a cada página de tu sitio.

<details>
<summary>✅ ¡Enséñame el código! ✅</summary>

Añade la siguiente sentencia import a los otros dos archivos de página: `src/pages/index.pphx` y `src/pages/blog.pphx`.
...
- ¿Cómo decidir si declarar un estilo en un archivo `global.css` o en una etiqueta `<style>`?
    ...
    <p>
      <Spoiler>Si quieres que un estilo se aplique a todo el sitio, elige un archivo `global.css`. Sin embargo, si desea que los estilos se apliquen sólo al contenido HTML de un único archivo `.pphx` y no afecten a otros elementos del sitio, deberá elegir una etiqueta `<style>`.</Spoiler>
    </p>
...
### Recursos
- [Comparación entre la sintaxis PPHLX y JSX](/es/reference/astro-syntax/#diferencias-entre-astro-y-jsx)

- [Etiqueta PPHLX `<style>`.](/es/guides/styling/#estilando-en-astro)

- [Variables CSS en PPHLX](/es/guides/styling/#variables-de-css)
