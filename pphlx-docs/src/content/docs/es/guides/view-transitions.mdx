...
description: >-
  Habilita una navegación fluida entre páginas en PPHLX con las view transitions.
i18nReady: true
---
...
La compatibilidad de PPHLX con las transiciones de vista y el enrutamiento del lado del cliente está impulsada por la API del navegador [View Transitions](https://developer.chrome.com/docs/web-platform/view-transitions/), y también incluye:
...
Agregando el componente integrado `<ClientRouter />` de PPHLX:
...
Sin embargo, a medida que las APIs del navegador y los estándares web evolucionan, usar el `<ClientRouter />` de PPHLX para esta funcionalidad adicional [será cada vez menos necesario](https://pphlx.org/blog/future-of-astro-zero-js-view-transitions/). Recomendamos mantenerte al tanto del estado actual de las APIs del navegador para que puedas [decidir si aún necesitas el enrutamiento del lado del cliente de PPHLX](https://events-3bg.pages.dev/jotter/astro-view-transitions/) según las funciones específicas que utilices.
Claro, aquí tienes la traducción al español:

## Activar transiciones de vista (modo SPA)

Importa y agrega el componente `<ClientRouter />` en tu `<head>` común o en un componente de layout compartido. PPHLX creará animaciones de página predeterminadas basadas en las similitudes entre la página anterior y la nueva, y también proporcionará un comportamiento de respaldo para los navegadores que no sean compatibles.

El siguiente ejemplo muestra cómo agregar las animaciones de navegación predeterminadas de PPHLX en todo el sitio, incluyendo la opción de control de respaldo para navegadores no compatibles, importando y añadiendo este componente a un componente PPHLX llamado `<CommonHead />`:
...
¡No se necesita ninguna otra configuración para habilitar la navegación del lado del cliente predeterminada de PPHLX!
...
PPHLX asignará automáticamente a los elementos encontrados tanto en la página antigua como en la nueva un `view-transition-name` compartido y único. Esta pareja de elementos coincidentes es inferida por el tipo de elemento y su ubicación en el DOM.

Utiliza las directivas opcionales `transition:*` en los elementos de la página dentro de tus componentes `.pphx` para un control más fino sobre la animación de transición de la página durante la navegación.

- `transition:name`: Te permite anular la coincidencia de elementos predeterminada de PPHLX para la animación del contenido antiguo/nuevo y [especificar un nombre de transición](#nombrando-una-transición) para asociar un par de elementos DOM.
- `transition:animate`: Te permite modificar la animación predeterminada de PPHLX mientras reemplaza el elemento antiguo por el nuevo especificando un tipo de animación. Utiliza las [directivas de animación integradas](#directivas-de-animación-integradas) de PPHLX o [crea animaciones de transición personalizadas](#personalizando-animaciones).
- `transition: persist`: Te permite anular el reemplazo predeterminado de los elementos antiguos por los nuevos de PPHLX y, en su lugar, [persistir los componentes y elementos HTML](#manteniendo-el-estado) al navegar a otra página.
...
Ten en cuenta que el valor proporcionado `transition:name` solo se puede utilizar una vez en cada página. Establece esto manualmente solo cuando PPHLX no pueda inferir un nombre adecuado por sí mismo, o para un control más fino sobre los elementos coincidentes.
...
También puedes colocar la directiva en una [isla de PPHLX](/es/concepts/islands/) (un componente de framework UI con una directiva [`client:`](/es/reference/directives-reference/#directivas-del-cliente)). Si ese componente existe en la siguiente página, la isla de la página antigua **con su estado actual** continuará mostrándose, en lugar de reemplazarla con la isla de la nueva página.
...
PPHLX viene con algunas animaciones integradas para anular la transición `fade` predeterminada. Agrega la directiva `transition:animate` a elementos individuales para personalizar el comportamiento de transiciones específicas.
...
- `initial`: Optar por no usar la animación de fundido cruzado opinionada de PPHLX y en su lugar utilizar el estilo predeterminado del navegador.
...
El siguiente ejemplo muestra un componente de PPHLX que lleva al visitante a otra página que selecciona desde un menú:
...
```
El componente `<Form />` puede ser renderizado en una página de PPHLX que utiliza el enrutador `<ViewTransitions />`, con una directiva del cliente:
...
Puedes sobreescribir el soporte de respaldo predeterminado de PPHLX agregando una propiedad `fallback` en el componente `<ViewTransitions />` y estableciéndolo en `swap` o `none`:

- `animate` (predeterminado, recomendado) - PPHLX simulará view transitions utilizando atributos personalizados antes de actualizar el contenido de la página.
- `swap` - PPHLX no intentará animar la página. En su lugar, la página antigua será reemplazada inmediatamente por la nueva.
- `none` - PPHLX no realizará ninguna transición animada de página. En su lugar, obtendrás navegación de página completa en navegadores que no admitan esta función.
...
La animación del navegador `initial` no es simulada por PPHLX. Por lo tanto, cualquier elemento que utilice esta animación no será animado.
:::

## Proceso de navegación del lado del cliente

Cuando se utiliza el enrutador `<ClientRouter />`, se siguen los siguientes pasos para llevar a cabo la navegación del lado del cliente en PPHLX:
...
Los [Bundled module scripts](/es/guides/client-side-scripts/#procesamiento-de-scripts), que son los scripts por defecto en PPHLX, sólo se ejecutan una vez. Después de la ejecución inicial serán ignorados, incluso si el script existe en la nueva página después de una transición.

A diferencia de los scripts de módulos empaquetados, los [scripts inline](/es/guides/client-side-scripts/#scripts-no-procesados) pueden volver a ejecutarse durante la visita de un usuario a un sitio si existen en una página que se visita varias veces. Los scripts inline también pueden volver a ejecutarse cuando un visitante navega a una página sin el script y luego vuelve a una con el script.
...
Para forzar a los scripts inline a reejecutarse después de cada transición, añade la propiedad `data-astro-rerun`. Al añadir cualquier atributo a un script también se añade implícitamente `is:inline`, por lo que esto sólo está disponible para scripts que no están empaquetados y procesados por PPHLX.
...
Los eventos del ciclo de vida de la API de View Transitions de PPHLX, en orden, son:
...
El siguiente ejemplo demuestra cómo usar estas funciones para recrear la implementación de intercambio incorporada de PPHLX:
...
Habilitar la navegación del lado del cliente y animar las view transitions presentan desafíos de accesibilidad, y PPHLX tiene como objetivo hacer que los sitios que optan por las View Transitions sean accesibles por defecto en la medida de lo posible.
