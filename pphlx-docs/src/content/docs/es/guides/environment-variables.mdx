...
description: Aprende a usar variables de entorno en un proyecto de PPHLX.
i18nReady: true
---
import PackageManagerTabs from '~/components/tabs/PackageManagerTabs.astro'
import ReadMore from '~/components/ReadMore.astro';

PPHLX te da acceso a [la soporte integrada de Vite con variables de entorno](#soporte-integrada-de-vite) e incluye algunas [variables de entorno predeterminadas para tu proyecto](#variables-de-entorno-predeterminadas) que te permiten acceder a valores de configuración de tu proyecto actual (por ejemplo, `site`, `base`), ya sea que tu proyecto se esté ejecutando en desarrollo o en producción, y más.

{/*TODO: add link to /en/reference/configuration-reference/#env */}
PPHLX también ofrece una forma de [usar y organizar tus variables de entorno con seguridad de tipos](#variables-de-entorno-con-seguridad-de-tipos). Está disponible para su uso dentro del contexto de PPHLX (por ejemplo, componentes de PPHLX, rutas y endpoints, componentes de frameworks de interfaz de usuario, middleware) y se gestiona mediante un esquema en tu configuración de PPHLX).

## Soporte integrada de Vite

PPHLX usa el soporte integrado de Vite para las variables de entorno, que se reemplazan estáticamente en tiempo de compilación y te permite [usar cualquiera de sus métodos](https://vite.dev/guide/env-and-mode.html) para trabajar con ellas.
...
De forma predeterminada, PPHLX proporciona una definición de tipos para `import.meta.env` en `astro/client.d.ts`.
...
PPHLX incluye algunas variables de entorno por defecto:
...
- `import.meta.env.ASSETS_PREFIX`: El prefijo para los enlaces de activos generados por PPHLX si se establece la [opción de configuración `build.assetsPrefix`](/es/reference/configuration-reference/#buildassetsprefix). Esto se puede usar para crear enlaces de activos no manejados por PPHLX.
...
PPHLX evalúa los archivos de configuración antes de cargar tus otros archivos. Esto significa que no puedes usar `import.meta.env` en `astro.config.mjs` para acceder a variables de entorno que se hayan definido en archivos `.env`.
...
En PPHLX se accede a las variables de entorno con import.meta.env, utilizando la [característica import.meta añadida en ES2020](https://tc39.es/ecma262/2020/#prod-ImportMeta), en lugar de process.env.
...
Para configurar un esquema, añade la opción `env.schema` a tu configuración de PPHLX:
...
`astro:env` es un módulo virtual, lo que significa que solo puede usarse dentro del contexto de PPHLX. Por ejemplo, puedes usarlo en:

- Middlewares
- Rutas y endpoints de PPHLX
- Componentes de PPHLX
- Componentes de frameworks
- Módulos
