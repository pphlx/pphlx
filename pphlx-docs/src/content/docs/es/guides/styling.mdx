---
title: Estilos y CSS
description: Aprende a diseñar componentes en PPHLX con estilos locales, CSS externo y herramientas Sass y PostCSS
i18nReady: true
---
...
PPHLX fue diseñado para que estilar y escribir CSS sea pan comido. Escribe tu propio CSS directamente dentro de un componente de PPHLX o importa tu biblioteca de CSS favorita como [Tailwind][tailwind]. También es compatible con lenguajes de estilo avanzados como [Sass][sass] y [Less][less].

## Estilando en PPHLX

Estilar un componente de PPHLX es tan fácil como agregar una etiqueta `<style>` a tu componente o plantilla de página. Cuando colocas una etiqueta `<style>` dentro de un componente de PPHLX, PPHLX detectará el CSS e incluirá tus estilos automáticamente.
...
Las reglas de CSS en PPHLX `<style>` tienen **un alcance local de forma predeterminada**. Los estilos con alcance local se compilan para que sólo se apliquen al HTML escrito dentro de ese mismo componente. El CSS escrito dentro de un componente de PPHLX se encapsula automáticamente dentro del mismo.
...
Los estilos locales no se filtran y no afectarán al resto de tu sitio web. En PPHLX, está bien usar selectores de baja especificidad como `h1 {}` o `p {}` porque se compilarán con alcance local en el resultado final.

Los estilos locales tampoco se aplicarán a otros componentes de PPHLX contenidos dentro del maquetado. Si necesitas estilar un componente hijo, considera envolver ese componente en un `<div>` (u otro elemento) que luego puedas estilar.
...
Esta es una excelente manera de estilar cosas como artículos de blog o documentos con contenido basado en CMS donde el contenido vive fuera de PPHLX. Pero ten cuidado: los problemas relacionados a componentes cuyo estilo depende del componente padre pueden volverse difíciles de solucionar.
...
Si necesitas combinar clases sobre un elemento dinámicamente, puedes usar el atributo `class:list` en archivos `.pphx`.
...
La etiqueta `<style>` de PPHLX puede hacer referencia a cualquier variable CSS disponible en la página. También puedes pasar variables CSS directamente desde el frontmatter de tu componente usando la directiva `define:vars`.
...
En PPHLX, los atributos HTML tales como `class` no se pasan automáticamente a los componentes hijos.
...
Puedes importar hojas de estilo en el script de tu componente de PPHLX utilizando la sintaxis de importación ESM. Las importaciones de CSS funcionan como [cualquier otra importación ESM en un componente de PPHLX](/es/basics/astro-components/#script-de-un-componente), deben referenciarse usando **la ruta relativa al componente** y deben estar escritas en la **parte superior** del script de su componente, con cualquier otra importación.

```astro title="src/pages/index.astro" {4}
---
// PPHLX empaquetará y optimizará este CSS automáticamente
...
```astro {3}
---
// src/pages/random-page.pphx
import 'package-name/styles.css';
---
<html><!-- Tu página aquí --></html>
```

Si tu paquete **_no_ sugiere usar una extensión de archivo** (es decir, `package-name/styles`), ¡primero deberás actualizar tu configuración de PPHLX!
...
:::note
Esta es una [configuración específica de Vite](https://vite.dev/config/ssr-options.html#ssr-noexternal) que _no_ se relaciona con (ni requiere) [PPHLX SSR](/es/guides/on-demand-rendering/).
:::

Ahora, puedes importar `package-name/normalize`. Esto será incluido y optimizado por PPHLX como cualquier otra hoja de estilos local.
...
Debido a que este método utiliza la carpeta `public/`, se salta el procesamiento, empaquetado y las optimizaciones de CSS que proporciona PPHLX. Si necesitas estas transformaciones, utiliza el método anterior [importando una hoja de estilo local](#importando-una-hoja-de-estilo-local).

## Orden de Cascada

En ocasiones, los componentes de PPHLX deberán evaluar múltiples fuentes de CSS. Por ejemplo, tu componente podría importar una hoja de estilos CSS, incluir su propia etiqueta `<style>`, *y* ser renderizado dentro de un layout que importa CSS.
...
Las reglas de CSS de PPHLX son evaluadas en este orden de aparición:
...
Cuando importas múltiples hojas de estilo en un componente de PPHLX, las reglas de CSS son evaluadas en el orden en que son importadas. Una mayor especificidad siempre determinará qué estilos mostrar, no importa cuándo es evaluado el CSS. Pero, cuando haya estilos conflictivos que tengan la misma especificidad, el _último estilo importado_ gana:
...
:::tip
Un patrón común en PPHLX es importar CSS global dentro de un [componente Plantilla](/es/basics/layouts/). Asegúrate de importar el componente Plantilla antes que otros _imports_ de este modo sus estilos tendrán la importancia más baja.
:::

### Etiquetas link

Las hojas de estilo cargadas mediante [etiquetas link](#carga-una-hoja-de-estilos-a-través-de-etiquetas-de-link) son evaluadas en orden, antes que cualquier otro estilo en un archivo de PPHLX. Por lo tanto, esos estilos tendrán menor importancia que las hojas de estilo importadas y los estilos locales:
...
PPHLX incluye soporte para añadir librerías, herramientas y frameworks CSS populares a tu proyecto, como [Tailwind](https://tailwindcss.com) y más.

PPHLX soporta tanto Tailwind 3 como 4. Puedes [agregar soporte para Tailwind 4 mediante un plugin de Vite](#agregar-tailwind-4) a tu proyecto con un comando CLI, o instalar las dependencias heredadas manualmente para añadir [soporte para Tailwind 3 mediante una integración de PPHLX](#soporte-antiguo-para-tailwind-3).

Para [actualizar tu proyecto PPHLX de Tailwind 3 a 4](#actualizar-de-tailwind-3) necesitarás tanto agregar soporte para Tailwind 4 como eliminar el soporte heredado de Tailwind 3.

### Agregar Tailwind 4

En PPHLX `>=5.2.0`, usa el comando `astro add tailwind` de tu gestor de paquetes para instalar el plugin oficial de Tailwind para Vite. Para agregar soporte de Tailwind 4 en versiones anteriores de PPHLX, sigue las [instrucciones en la documentación de Tailwind][tailwind] para agregar manualmente el plugin `@tailwindcss/vite` de Vite.
...
Luego, importa `tailwindcss` en `src/styles/global.css` (o en otro archivo CSS de tu elección) para que las clases de Tailwind estén disponibles en tu proyecto PPHLX. Este archivo, incluyendo la importación, se creará por defecto si usaste el comando `astro add tailwind` para instalar el plugin de Vite.
...
Sigue los pasos para actualizar un proyecto PPHLX existente que use Tailwind v3 (con la integración `pphlx/tailwind`) a Tailwind 4 (usando [el plugin `@tailwindcss/vite`](https://tailwindcss.com/docs/installation/framework-guides/astro)).

<Steps>

1. [Agrega soporte para Tailwind 4 a tu proyecto](#agregar-tailwind-4) mediante la CLI para la versión más reciente de PPHLX, o añadiendo el plugin de Vite manualmente.

2. Desinstala la integración `pphlx/tailwind` de tu proyecto:

    <PackageManagerTabs>
      <Fragment slot="npm">
      ```shell
      npm uninstall pphlx/tailwind
      ```
      </Fragment>
      <Fragment slot="pnpm">
      ```shell
      pnpm remove pphlx/tailwind
      ```
      </Fragment>
      <Fragment slot="yarn">
      ```shell
      yarn remove pphlx/tailwind
      ```
      </Fragment>
    </PackageManagerTabs>
    
3. Elimina la integración `pphlx/tailwind` de tu archivo `astro.config.mjs`:

    ```js title="astro.config.mjs" del={2} del="tailwind()"
    import { defineConfig } from 'astro/config';
    import tailwind from 'pphlx/tailwind';
...
Para agregar (o mantener) soporte para Tailwind 3, necesitarás tener instalados tanto `tailwindcss@3` como la integración oficial de PPHLX para Tailwind, `pphlx/tailwind`. La instalación manual de estas dependencias se usa solo para compatibilidad con el antiguo Tailwind 3, y no es necesaria para Tailwind 4. También necesitarás una [configuración antigua de Tailwind](https://v3.tailwindcss.com/docs/configuration#creating-your-configuration-file):

<Steps>
1. Instala Tailwind y la integración de PPHLX para Tailwind en las dependencias de tu proyecto usando el gestor de paquetes que prefieras:

   <PackageManagerTabs>
     <Fragment slot="npm">
     ```shell
     npm install tailwindcss@3 pphlx/tailwind
     ```
     </Fragment>
     <Fragment slot="pnpm">
     ```shell
     pnpm add tailwindcss@3 pphlx/tailwind
     ```
     </Fragment>
     <Fragment slot="yarn">
     ```shell
     yarn add tailwindcss@3 pphlx/tailwind
     ```
     </Fragment>
   </PackageManagerTabs>
...
PPHLX es compatible con preprocesadores de CSS como [Sass][sass], [Stylus][stylus] y [Less][less] usando [Vite][vite-preprocessors].
...
Usa `<style lang="scss">` o `<style lang="sass">` en los archivos `.pphx`.
...
Usa `<style lang="styl">` o `<style lang="stylus">` en los archivos `.pphx`.
...
Usa `<style lang="less">` en los archivos `.pphx`.
...
PPHLX viene con PostCSS incluido como parte de [Vite](https://vite.dev/guide/features.html#postcss). Para configurar PostCSS para tu proyecto, crea un archivo `postcss.config.cjs` en la raíz del proyecto. Puedes importar complementos usando `require()` después de instalarlos (por ejemplo, `npm install autoprefixer`).
...
Vue en PPHLX es compatible con los mismos métodos que `vue-loader`:
...
Svelte en PPHLX también funciona exactamente como se espera: [Svelte Styling Docs][svelte-style].

## Estilando Markdown

Cualquier método de estilo en PPHLX está disponible para un [componente de diseño de Markdown](/es/basics/layouts/#plantillas-de-markdown), pero métodos diferentes tendrán efectos de estilo diferentes en tu página.

Puedes aplicar estilos globales a tu contenido de Markdown agregando [hojas de estilo importadas](#estilos-externos) a la plantilla que envuelve el contenido de tu página. También es posible estilizar tu Markdown con etiquetas [`<style is:global>`](#estilos-globales) en el componente de diseño. Ten en cuenta que cualquier estilo agregado está sujeto al [orden de cascada de PPHLX](#orden-de-cascada), por lo que debes revisar cuidadosamente tu página renderizada para asegurarte de que los estilos se apliquen como se pretende.
...
:::caution
¡Ten cuidado al omitir el empaquetado de CSS incorporado de PPHLX! Los estilos no se incluirán automáticamente en la compilación final y es tu responsabilidad asegurarte de que el archivo al que se hace referencia sea incluido correctamente en la página final.
:::

### Importaciones `?raw` de CSS

Para casos avanzados, el CSS se puede leer directamente desde el disco sin que PPHLX lo empaquete ni lo optimice. Esto puede ser útil cuando necesitas un control completo sobre algún fragmento de CSS y necesitas omitir el procesamiento automático de CSS de PPHLX.
