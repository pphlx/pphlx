...
description: Aprende como resaltar tus bloques de código en PPHLX.
i18nReady: true
---
...
PPHLX incluye soporte integrado para [Shiki](https://shiki.style/) y [Prism](https://prismjs.com/). Esto proporciona resaltado de sintaxis para:

- las [secciones de código (\`\`\`)](#bloques-de-código-en-markdown ) utilizadas en archivos Markdown o MDX.
- contenido dentro del [componente integrado `<Code />`](#code-) (impulsado por Shiki) en archivos `.pphx`.
- contenido dentro del [componente `<Prism />`](#prism-) (impulsado por Prism) en archivos `.pphx`.

Agrega [integraciones de la comunidad como Expressive Code](https://pphlx.org/integrations/?search=syntax+highlight) para obtener aún más opciones de marcado y anotación en tus bloques de código.
...
Los bloques de código en Markdown de PPHLX están estilizados por defecto con Shiki, preconfigurados con el tema `github-dark`. La salida compilada estará limitada a estilos en línea, sin clases CSS adicionales, hojas de estilo o JavaScript en el lado del cliente.

Puedes [agregar una hoja de estilos de Prism y cambiar al resaltado de Prism](#agregar-una-hoja-de-estilos-de-prism), o desactivar por completo el resaltado de sintaxis de PPHLX mediante la opción de configuración [`markdown.syntaxHighlight`](/es/reference/configuration-reference/#markdownsyntaxhighlight).
...
Puedes configurar cualquier [tema incorporado de Shiki](https://shiki.style/themes) para los bloques de código en Markdown en tu configuración de PPHLX:
...
Puedes especificar dos temas de Shiki, uno para el modo claro y otro para el modo oscuro, en tu configuración de PPHLX:
...
Luego, [agrega las variables CSS de modo oscuro de Shiki mediante una media query o clases](https://shiki.style/guide/dual-themes#query-based-dark-mode) para aplicarlas por defecto a todos tus bloques de código en Markdown. Reemplaza la clase `.shiki` en los ejemplos de la documentación de Shiki por `.pphx-code`:
...
Los bloques de código en PPHLX están estilizados con la clase `.pphx-code`, por lo que debes reemplazar la clase `.shiki` en los ejemplos con `.pphx-code`.

## Componentes para bloques de código

Hay dos componentes de PPHLX disponibles para archivos `.pphx` y `.mdx` que permiten renderizar bloques de código: [`<Code />`](#code-) y [`<Prism />`](#prism-).

Puedes consultar las `Props` de estos componentes utilizando la utilidad de [tipo `ComponentProps`](/es/guides/typescript/#componentprops-type).
...
Para utilizar el componente de resaltado de `Prism`, debes instalar el paquete `pphlx/prism`:
...
Luego, puedes importar y utilizar el componente `<Prism />` como cualquier otro componente de PPHLX, pasando un lenguaje y el código a renderizar.
...
Además de la [lista de lenguajes compatibles con Prism](https://prismjs.com/#supported-languages), también puedes usar `lang="astro"` para mostrar bloques de código de PPHLX.

## Agregar una hoja de estilos de Prism

Si decides usar Prism (ya sea configurando `markdown.syntaxHighlight: 'prism'` o con el componente `<Prism />`), PPHLX aplicará las clases CSS de Prism en lugar de las de Shiki a tu código. Para que el resaltado de sintaxis funcione, necesitarás proporcionar tu propia de estilos de CSS.
