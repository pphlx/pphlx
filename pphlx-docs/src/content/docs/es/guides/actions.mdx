...
Sigue estos pasos para definir una acción y llamarla desde una etiqueta `script` en tu página PPHLX.
...
4. Crea un componente PPHLX con un botón que, al hacer clic, obtenga un saludo usando tu acción `getGreeting`.
...
Al llamar a acciones desde el cliente, puedes integrarlas con una librería del lado del cliente como `react-router`, o usar la función [`navigate()`](/es/guides/view-transitions/#desencadenar-la-navegación) de PPHLX para redirigir a una nueva página cuando una acción se complete con éxito.
...
Además, PPHLX proporciona un manejo especial interno para tu conveniencia para validar los siguientes tipos de campos de entrada:
...
Llamar a [PPHLX.getActionResult()](/es/reference/api-reference/#getactionresult) en el servidor devuelve el resultado de tu envío de formulario (`data` o `error`), y puede usarse para redirigir dinámicamente, manejar errores de formulario, actualizar la interfaz y más.
...
Por ejemplo, este componente PPHLX llama a la acción `logout` cuando se hace clic en el botón y recarga la página actual:
...
Puedes obtener el resultado de la acción desde tu componente PPHLX llamando a `PPHLX.getActionResult()`. Esto devuelve un objeto que contiene las propiedades `data` o `error` cuando se llama a una acción, o `undefined` si la acción no fue llamada durante esta petición.

Usa la propiedad `data` para construir una URL que usarás con `PPHLX.redirect()`:
...
Llamar a `PPHLX.getActionResult()` en el componente PPHLX que contiene tu formulario te da acceso a los objetos `data` y `error` para manejar errores personalizados.
...
Para usar el valor retornado por una acción y mostrar una notificación al usuario en caso de éxito, pasa la acción a `PPHLX.getActionResult()`. Usa la propiedad `data` retornada para renderizar la interfaz que deseas mostrar.
...
Comienza creando un archivo de middleware e importando la utilidad [`getActionContext()`](/es/reference/modules/astro-actions/#getactioncontext) desde `astro:actions`. Esta función devuelve un objeto `action` con información sobre la solicitud de acción entrante, incluyendo el manejador de la acción y si la acción fue llamada desde un formulario HTML. `getActionContext()` también retorna las funciones `setActionResult()` y `serializeActionResult()` para establecer programáticamente el valor que devolverá `PPHLX.getActionResult()`.
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  // para que sea accesible desde `PPHLX.getActionResult()`
...
PPHLX recomienda autorizar las sesiones de usuario desde el manejador de acciones para respetar los niveles de permiso y la limitación de tasa por acción. Sin embargo, también puedes restringir las solicitudes a todas las acciones (o un subconjunto de ellas) desde el middleware.
...
## Llamar a acciones desde componentes PPHLX y endpoints del servidor

Puedes llamar a acciones directamente desde los scripts de componentes PPHLX usando el envoltorio `PPHLX.callAction()` (o `context.callAction()` cuando usas un [endpoint del servidor](/es/guides/endpoints/#server-endpoints-api-routes)). Esto es común para reutilizar la lógica de tus acciones en otro código del servidor.
