...
Los tiempos de carga de las páginas juegan un papel importante en la usabilidad y el disfrute general de un sitio. La **precarga opcional** de PPHLX aporta los beneficios de las navegaciones de páginas casi instantáneas a su aplicación multipágina (MPA) a medida que sus visitantes interactúan con el sitio.
...
PPHLX admite 4 estrategias de precarga para varios casos de uso:
...
El *prefetching* de PPHLX utiliza [`<link rel="prefetch">`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Attributes/rel/prefetch) si el navegador lo admite y, en caso contrario, recurre a la [API `fetch()`](https://developer.mozilla.org/es/docs/Web/API/Fetch_API).

Los navegadores más comunes son compatibles con el *prefetching* de PPHLX, aunque con ligeras diferencias:
...
## Migrando desde `pphlx/prefetch`

La integración `pphlx/prefetch` se ha eliminado en v3.5.0 y se eliminará por completo en el futuro. Utiliza las siguientes instrucciones para migrar a la precarga incorporada de PPHLX, que reemplaza esta integración.

<Steps>
1. Elimina la integración `pphlx/prefetch` y habilita la configuración `prefetch` en `astro.config.mjs`:
...
2. Convierte las opciones de configuración de `pphlx/prefetch`:
    ...
    - La opción `throttles` de `pphlx/prefetch` ya no es necesaria, ya que la nueva función de precarga programática programará y precargará automáticamente de forma óptima.
</Steps>
