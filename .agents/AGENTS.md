# Piplex Compiler & Component Syntax Guidelines
...
*   **Rule:** When implementing template interpolation for variables (like `{{title}}`), always verify that system asset anchors (like `{{PIPLEX_CSS}}` and `{{PIPLEX_JS}}`) are skipped and preserved for the final asset injection stage.

## 3. Configuration Architecture
*   **Rule:** Maintain a strict separation of configuration files:
    *   `piplex.json` (Project Manifest): Holds project metadata, script triggers (e.g., `"build": "piplex build"`), and dependency declarations.
    *   `piplex.config.json` (Compiler config): Holds specific build configurations (e.g., `srcDir`, `outDir`, `cssOut`, and `jsOut`).
