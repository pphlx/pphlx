# Piplex Model Context Protocol (MCP) Specification

This specification defines the Model Context Protocol (MCP) server for the Piplex compilation framework. It allows AI models (such as Claude, Gemini, or ChatGPT running in Cursor/VS Code) to interact with Piplex syntax, tools, compilation states, and best practices.

---

## 1. Architectural Overview

The Piplex MCP Server runs as a lightweight Node.js/TypeScript CLI (or built-in to the Rust/Go compiler CLI) communicating over **Standard Input/Output (stdio)** JSON-RPC 2.0.

```
+------------------+                   +--------------------+
|  AI Assistant    | <--- stdio --->   | Piplex MCP Server  |
...
### `piplex/search_docs`
*   **Description**: Searches the Piplex documentation database for syntax, lifecycle details, and routing specifications.
*   **Arguments**:
    *   `query` (string, required): The search keyword or phrase (e.g., "how to import svelte component").

### `piplex/generate_island`
...
### `piplex/check_compilation`
*   **Description**: Invokes the local Piplex compiler watcher on the workspace and returns diagnostic logs to help the AI self-debug compiler errors.
*   **Arguments**:
    *   `workspacePath` (string, required): Path to the Piplex project root.
...
```json
{
  "name": "piplex-mcp",
  "version": "1.0.0",
  "description": "Model Context Protocol Server for Piplex",
  "main": "build/index.js",
  "type": "module",
  "scripts": {
    "build": "tsc",
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description: "Get best practices guidelines on core Piplex development topics.",
        inputSchema: {
          type: "object",
          properties: {
            topic: { 
...
async function main() {
  ...
  console.error("Piplex MCP server running on stdio");
}
